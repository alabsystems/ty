...
# This script discovers which liveness specs TY handles correctly

set -eo pipefail

echo "=== TY Liveness Spec Testing ==="
echo "Date: $(date)"
echo ""
...
# Huang takes ~300s total (TLC + TY), so allow 600s per tool.
TLC_TIMEOUT_SECS="${TLC_TIMEOUT_SECS:-600}"
TY_TIMEOUT_SECS="${TY_TIMEOUT_SECS:-600}"
...
# Classify a single tool's output via the ty-liveness-classify Rust
# binary (ported from scripts/liveness_verdict_lib.py).
...
classify_tool() {
    local _scripts_dir="$1"  # kept for backwards compat with old callers
    local output_file="$2"
    local rc="$3"
    local tool="$4"

    "$TARGET_DIR/release-canary/ty-liveness-classify" \
        "$output_file" "$rc" "$tool"
}
...
# The classify_tool helper above shells out to the Rust port of
# scripts/liveness_verdict_lib.py; build it from tla-petri.
cargo build --profile release-canary \
    -p tla-petri --bin ty-liveness-classify 2>/dev/null
...
run_liveness_test() {
    ...
    if [ -n "$tlc_metadir" ]; then
        run_with_timeout "$TLC_TIMEOUT_SECS" "${TLC_JAVA_ENV[@]}" java "${TLC_JAVA_SINGLE_THREAD_ARGS[@]}" -DTLA-Library="$TLC_TLA_LIBRARY" -cp "$TLC_CP" tlc2.TLC -metadir "$tlc_metadir" -config "$config" "$spec" -workers 1 > "$_tlc_tmp" 2>&1
    else
        run_with_timeout "$TLC_TIMEOUT_SECS" "${TLC_JAVA_ENV[@]}" java "${TLC_JAVA_SINGLE_THREAD_ARGS[@]}" -DTLA-Library="$TLC_TLA_LIBRARY" -cp "$TLC_CP" tlc2.TLC -config "$config" "$spec" -workers 1 > "$_tlc_tmp" 2>&1
    fi
    local tlc_rc=$?
    set -e
    if [ -n "$tlc_metadir" ]; then
    ...
    # Classify TLC verdict via ty-liveness-classify (Rust port of liveness_verdict_lib.py).
    ...
    if [ "$tlc_status" = "timeout" ]; then
        ...
        SKIP=$((SKIP + 1))
        rm -f "$_tlc_tmp" "$_ty_tmp"
        return
    fi

    if [ "$tlc_status" = "error" ] || [ "$tlc_status" = "unknown" ]; then
        echo "[ SKIP ] $name - TLC $tlc_status"
        echo "RESULT|name=$name|tlc_status=$tlc_status|ty_status=skipped|overall=SKIP" >> "$RESULTS_FILE"
        SKIP=$((SKIP + 1))
        rm -f "$_tlc_tmp" "$_ty_tmp"
        return
    fi
    ...
    # Run TY
    set +e
    run_with_timeout "$TY_TIMEOUT_SECS" "${TY_FAIR_ENV[@]}" "$TY" check "$spec" --config "$config" --workers 1 $extra_args > "$_ty_tmp" 2>&1
    local ty_rc=$?
    set -e

    # Classify TY verdict via ty-liveness-classify (Rust port of liveness_verdict_lib.py).
    local ty_verdict ty_status ty_states
    ty_verdict=$(classify_tool "$SCRIPT_DIR" "$_ty_tmp" "$ty_rc" "ty")
    IFS='|' read -r ty_status ty_states <<< "$ty_verdict"

    # Clean up temp files
    rm -f "$_tlc_tmp" "$_ty_tmp"

    evaluate_verdicts "$name" "$tlc_status" "$ty_status" "$tlc_states" "$ty_states"
}
...
evaluate_verdicts() {
    local name="$1"
    local tlc_status="$2" ty_status="$3"
    local tlc_states="$4" ty_states="$5"

    # Handle TY timeout/error
    if [ "$ty_status" = "timeout" ]; then
        echo "[ ERROR ] $name: TY timeout (${TY_TIMEOUT_SECS}s) (TLC: ${tlc_states:-?} states)"
        echo "RESULT|name=$name|tlc_status=$tlc_status|ty_status=timeout|tlc_states=${tlc_states:-}|ty_states=|overall=ERROR" >> "$RESULTS_FILE"
        ERROR=$((ERROR + 1))
        return
    fi

    if [ "$ty_status" = "error" ] || [ "$ty_status" = "unknown" ]; then
        echo "[ ERROR ] $name: TY $ty_status (TLC: ${tlc_states:-?} states)"
        echo "RESULT|name=$name|tlc_status=$tlc_status|ty_status=$ty_status|tlc_states=${tlc_states:-}|ty_states=${ty_states:-}|overall=ERROR" >> "$RESULTS_FILE"
        ERROR=$((ERROR + 1))
        return
    fi
    ...
    [ "$tlc_status" = "$ty_status" ] && verdict_match="yes"
    [ -n "$tlc_states" ] && [ -n "$ty_states" ] && [ "$tlc_states" = "$ty_states" ] && state_match="yes"

    if [ "$verdict_match" = "no" ]; then
        overall="FAIL"
    elif [ "$tlc_status" = "success" ] && [ -n "$tlc_states" ] && [ -n "$ty_states" ] && [ "$tlc_states" != "$ty_states" ]; then
        overall="FAIL"
    else
        overall="PASS"
    fi
    ...
    echo "RESULT|name=$name|tlc_status=$tlc_status|ty_status=$ty_status|tlc_states=${tlc_states:-}|ty_states=${ty_states:-}|verdict_match=$verdict_match|state_match=$state_match|overall=$overall" >> "$RESULTS_FILE"

    if [ "$overall" = "PASS" ]; then
        echo "[ PASS ] $name: ${ty_states:-?} states (TLC: ${tlc_states:-?})"
        echo "PASS: $name (${ty_states:-?} states)" >> "$RESULTS_FILE"
        PASS=$((PASS + 1))
    else
        echo "[ FAIL ] $name: verdict=$tlc_status/$ty_status states=TY:${ty_states:-?}/TLC:${tlc_states:-?}"
        echo "FAIL: $name (verdict=$tlc_status/$ty_status, TY=${ty_states:-?}, TLC=${tlc_states:-?})" >> "$RESULTS_FILE"
        FAIL=$((FAIL + 1))
    fi
}
...
# NOTE: Skipped - MC.tla uses spec aliases (spec_1293897152943429000 == LSpec) which TY
