...
#
# This script builds ty and runs MCVoting. Exit code 0 = good (77 states),
...
if ! cargo build --profile release-canary --bin ty 2>/dev/null; then
    echo "BISECT: build failed, skipping"
    exit 125
fi

# Run MCVoting — capture output
OUTPUT=$(timeout 120 target/release-canary/ty check "$SPEC" --config "$CFG" 2>&1) || true
