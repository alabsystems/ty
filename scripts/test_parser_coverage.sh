...
# Tests TY parser against all specs in tlaplus/Examples repository
...
find "$EXAMPLES_DIR" -name "*.tla" -type f | sort | while read f; do
    ...
    # Run parser
    output=$($TY parse "$f" 2>&1) && exit_code=$? || exit_code=$?
