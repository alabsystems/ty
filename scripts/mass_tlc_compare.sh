...
# Compares TY against TLC for all specs with .cfg files in tlaplus/Examples
...
# CSV header
echo "spec,cfg,ty_states,tlc_states,ty_time,tlc_time,match,error" > "$CSV_FILE"
...
echo "=== TY vs TLC Mass Comparison ==="
echo "Date: $(date)"
...
process_spec() {
    ...
    # Run TY with timeout
    local ty_start=$(date +%s.%N)
    local ty_out=$(timeout "${TIMEOUT}s" "${TY_FAIR_ENV[@]}" "$TY" check "$tla" --config "$cfg" --workers 1 --force 2>&1) && ty_rc=$? || ty_rc=$?
    local ty_end=$(date +%s.%N)

    if [ $ty_rc -eq 124 ]; then
        error="TY timeout"
    elif [ $ty_rc -ne 0 ]; then
        error="TY exit $ty_rc"
    else
        ty_states=$(echo "$ty_out" | grep -oE "States found: [0-9,]+" | tr -d ',' | grep -oE "[0-9]+" || echo "-")
        ty_time=$(echo "$ty_end - $ty_start" | bc 2>/dev/null || echo "-")
    fi

    # Run TLC with timeout (only if TY succeeded)
    if [ -z "$error" ]; then
        local tlc_meta_spec=$(mktemp -d)
        local tlc_start=$(date +%s.%N)
        local tlc_out=$(timeout "${TIMEOUT}s" "${TLC_JAVA_ENV[@]}" java "${TLC_JAVA_SINGLE_THREAD_ARGS[@]}" -cp "$TLC_CP" tlc2.TLC \
        ...
        if [ $tlc_rc -eq 124 ]; then
            ...
            tlc_states=$(echo "$tlc_out" | grep -oE "[0-9,]+ distinct states found" | tail -1 | tr -d ',' | grep -oE "^[0-9]+" || echo "-")
    ...
    if [ -n "$error" ]; then
        match="ERROR"
    elif [ "$ty_states" = "-" ] || [ "$tlc_states" = "-" ]; then
        match="SKIP"
    elif [ "$ty_states" = "$tlc_states" ]; then
        match="YES"
    else
    ...
    # Output result
    echo "$spec_name,$cfg,$ty_states,$tlc_states,$ty_time,$tlc_time,$match,$error" >> "$CSV_FILE"

    case "$match" in
        YES)
            echo "[ PASS ] $rel_path: $ty_states states (TY) = $tlc_states (TLC)"
            return 0
            ;;
        NO)
            echo "[ FAIL ] $rel_path: $ty_states (TY) != $tlc_states (TLC)"
            return 1
            ;;
        ERROR)
