...
impl BmcTranslator {
    ...
    pub(super) fn extract_universe_from_exprs(
        ...
        // Merge concretely-stored set members so subset / membership encodings
        // see elements that were introduced via `assert_concrete_state` but do
        // not appear in any literal `SetEnum`/`Range` argument. Without this,
        // `T \in SUBSET S` where T was concretely set to a value outside the
        // literal base universe would silently pass.
        values.extend(self.tracked_universe_ints.iter().copied());
        Ok(values
            .into_iter()
        ...
        match &expr.node {
            Expr::SetEnum(elements) => {
                for e in elements {
                    ...
                    // Non-literal elements are skipped (same as AYTranslator)
                }
            }
            Expr::Range(lo, hi) => {
    ...
    pub(super) fn encode_symbolic_subset(&mut self, base: &Spanned<Expr>) -> AYResult<Term> {
        ...
        for &u in &universe {
            ...
            let flag_name = format!("__bmc_subset_flag_{id}");
            let flag = self.solver.declare_const(&flag_name, Sort::Bool);
    ...
    pub(super) fn try_translate_func_equality(
        ...
        Some(self.solver.try_and(dom_eq, map_eq).map_err(AYError::Solver))
    }
    ...
    pub(super) fn try_translate_set_eq(
        ...
        let result = (|| -> AYResult<Term> {
            ...
            for &u in &universe {
                ...
                let equiv =
                    crate::dispatch_shared::encode_bool_eq(&mut self.solver, l_member, r_member)?;
