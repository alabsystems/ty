...
impl Default for KInductionConfig {
    fn default() -> Self {
        Self {
            max_k: 20,
            start_k: 1,
            solve_timeout: Some(Duration::from_mins(5)),
            incremental: false,
            debug: false,
        }
    }
...
///
/// ```no_run
/// use tla_ay::bmc::kinduction::{KInductionChecker, KInductionConfig, KInductionResult};
/// use tla_ay::TlaSort;
/// use tla_core::Spanned;
/// use tla_core::ast::Expr;
///
...
impl KInductionChecker {
    ...
    ///
    /// An inconclusive search returns `Ok(KInductionResult::Unknown { .. })`, not
    /// an error; only setup/translation/solver failures produce an `Err`.
    ///
    /// # Errors
    /// Returns [`AYError::UntranslatableExpr`] if no state variables were declared,
    /// plus any error from translating Init/Next/Safety or running the underlying
    /// BMC and inductive-step solves (e.g. [`AYError::Solver`],
    /// [`AYError::UnsupportedOp`]).
    pub fn check(&self, config: &KInductionConfig) -> AYResult<KInductionResult> {
        ...
        match bmc_result {
            ...
            BmcBaseResult::BoundReached { max_depth } => {
                if config.debug {
                    eprintln!("[ay-kind] BMC base case passed up to depth {max_depth}");
                }
            }
        }
    ...
    fn run_bmc_base_case(&self, config: &KInductionConfig) -> AYResult<BmcBaseResult> {
        for depth in 0..=config.max_k {
            if config.debug {
                eprintln!("[ay-kind] BMC base depth {depth}");
            }
    ...
    /// /\ ¬Safety(sk)
    /// ```
    fn run_inductive_step(&self, config: &KInductionConfig) -> AYResult<KInductionResult> {
        for k in config.start_k..=config.max_k {
            if config.debug {
                eprintln!("[ay-kind] inductive step k={k}");
            }
            ...
            match translator.try_check_sat()? {
                SolveResult::Unsat(_) => {
                    if config.debug {
                        eprintln!("[ay-kind] PROVED: property is {k}-inductive");
                    }
    ...
    fn run_inductive_step_incremental(
        ...
        for k in config.start_k..=config.max_k {
            if config.debug {
                eprintln!("[ay-kind-incr] inductive step k={k}");
            }
            ...
            match translator.try_check_sat()? {
                SolveResult::Unsat(_) => {
                    if config.debug {
                        eprintln!("[ay-kind-incr] PROVED: property is {k}-inductive");
                    }
                    return Ok(KInductionResult::Proved { k });
                }
                SolveResult::Sat => {
