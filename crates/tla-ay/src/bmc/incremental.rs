...
//!
//! Reuses the ay solver across increasing bound depths so that conflict clauses
...
///
/// ```no_run
/// use tla_ay::bmc::incremental::IncrementalBmc;
/// use tla_ay::{TlaSort, AYResult};
/// use tla_core::Spanned;
/// use tla_core::ast::Expr;
///
/// fn example() -> AYResult<()> {
...
impl IncrementalBmc {
    ...
    pub fn new(max_k: usize, vars: &[(String, TlaSort)]) -> AYResult<Self> {
    ...
    /// # Errors
    /// Returns error if `max_k` exceeds the BMC bound limit.
    pub fn new_with_arrays(max_k: usize, vars: &[(String, TlaSort)]) -> AYResult<Self> {
    ...
    /// exists within the bound. A solver `Unknown` at some depth returns
    /// `Ok(IncrementalBmcResult::Unknown { .. })`, not an error.
    ///
    /// # Errors
    /// Returns an [`AYError`](crate::AYError) if translating Init/Next/Safety
    /// fails, or if the underlying solver, scope push/pop, or model extraction
    /// errors (e.g. [`Solver`](crate::AYError::Solver),
    /// [`UnsupportedOp`](crate::AYError::UnsupportedOp)).
    pub fn run(
        &mut self,
        init: &Spanned<Expr>,
        next: &Spanned<Expr>,
        safety: &Spanned<Expr>,
    ) -> AYResult<IncrementalBmcResult> {
        ...
        for k in 0..=self.max_k {
            ...
            match result {
                ...
                ay_dpll::api::SolveResult::Unsat(_) => {
    ...
    ///
    /// # Errors
    /// Same as [`run`](Self::run): an [`AYError`](crate::AYError) from translation,
    /// solving, scoping, or model extraction.
