...
fn test_bmc_bounds_validation() {
    ...
    match result {
        Err(AYError::BmcBoundTooLarge { bound, max }) => {
...
fn test_bmc_unknown_variable_error() {
    ...
    match result {
        Err(AYError::UnknownVariable(msg)) => {
...
fn test_bmc_step_exceeds_bound_error() {
    ...
    match result {
        Err(AYError::UntranslatableExpr(msg)) => {
            assert!(
...
/// Test error path: membership in an unsupported set type. `Nat`/`Int` now
/// have sound BMC arms (`x \in Nat => x >= 0`, `x \in Int => (= x x)`; see the
/// positive coverage in `set_encoding.rs`), so this uses an UNKNOWN
/// non-set-typed constant identifier — which is neither a built-in set nor a
/// declared set-typed variable and must still fall through to the fail-closed
/// catch-all.
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_bmc_membership_unsupported_set_error() {
    ...
    // Try x \in UnknownConst (an Ident that is NOT Nat/Int/BOOLEAN and NOT a
    // declared set-typed variable): fail-closed at the catch-all.
