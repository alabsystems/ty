...
//! Tuple operations for AY translation.
//!
...
use super::{AYTranslator, TlaSort, TupleVarInfo};
...
impl AYTranslator {
    ...
    pub(super) fn translate_tuple_apply_bool(
        ...
        if let Some(idx) = self.try_expr_to_int(index) {
            ...
            return info
                ...
                .get(&(idx as usize))
                .copied()
                .ok_or_else(|| AYError::UnsupportedOp(format!("{tuple_name}[{idx}] not found")));
        }
    ...
    fn build_tuple_ite_chain_int(
        ...
        let mut result =
            info.element_terms.get(&last_idx).copied().ok_or_else(|| {
                AYError::UnsupportedOp(format!("{tuple_name}[{last_idx}] not found"))
            })?;
        for &idx in int_indices.iter().rev().skip(1) {
            ...
            let elem_term =
                info.element_terms.get(&idx).copied().ok_or_else(|| {
                    AYError::UnsupportedOp(format!("{tuple_name}[{idx}] not found"))
                })?;
    ...
    pub(super) fn translate_tuple_apply_string(
        ...
        Err(AYError::UnknownVariable(format!("{var_name} not a tuple")))
    }
