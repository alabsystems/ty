...
impl AYTranslator {
    ...
    pub(super) fn translate_choose_int(
        ...
        let domain = bound
            .domain
            .as_ref()
            .ok_or_else(|| AYError::UnsupportedOp("CHOOSE without domain".to_string()))?;
        ...
        // Fast-path: when the domain is a small concrete SetEnum or
        // integer range, encode CHOOSE as an ITE chain over the constant
        // domain instead of a Skolem-plus-membership disjunction. With the
        // Skolem encoding, `(sk = e_1 \/ ... \/ sk = e_n) /\ P(sk)` mixes a
        // disjunction with theory atoms in QF_LIA, which the ay-dpll lazy
        // theory pipeline currently returns Unknown(Incomplete) for. The
        // ITE form eliminates the disjunction-over-theory entirely:
        //
        //     sk = ITE(P(e_1), e_1, ITE(P(e_2), e_2, ..., default))
        //
        // and the membership disjunction becomes `P(e_1) \/ ... \/ P(e_n)`,
        // which can constant-fold per disjunct when the body is a literal
        // predicate over the constant.
        if let Some(elements) = collect_choose_int_domain(domain) {
            return self.translate_choose_int_eager(var_name, body, &elements);
        }
    ...
    /// Eager ITE-chain encoding for `CHOOSE x \in {e_1, ..., e_n} : P(x)`.
    ///
    /// Builds `sk = ITE(P(e_1), e_1, ITE(P(e_2), e_2, ..., e_n))` and
    /// asserts the disjunction `P(e_1) \/ ... \/ P(e_n)` so that at least
    /// one branch is taken. Each `P(e_i)` is the result of substituting the
    /// constant `e_i` into the original body and translating it to Bool —
    /// this lets the solver simplify branches whose predicates evaluate to
    /// constants at translation time.
    fn translate_choose_int_eager(
        &mut self,
        var_name: &str,
        body: &Spanned<Expr>,
        elements: &[BigInt],
        ...
        if elements.is_empty() {
            // CHOOSE over empty domain is undefined in TLA+; assert FALSE
            // so the solver detects the contradiction, then return a
            // placeholder Int constant.
            let f = self.solver.bool_const(false);
            self.assert(f);
            return Ok(self.solver.int_const(0));
        }

        // Translate each predicate-instantiated-at-constant and collect
        // them alongside the integer constants for the ITE chain.
        let mut pred_terms = Vec::with_capacity(elements.len());
        let mut value_terms = Vec::with_capacity(elements.len());
        for v in elements {
            let v_i64: i64 = i64::try_from(v).map_err(|_| {
                AYError::IntegerOverflow(format!("CHOOSE domain element {v} too large for i64"))
            })?;
            let pred = self.substitute_and_translate(body, var_name, &Expr::Int(v.clone()))?;
            pred_terms.push(pred);
            value_terms.push(self.solver.int_const(v_i64));
        }

        // Assert that at least one branch's predicate holds.
        let any_pred = self.combine_bool_terms(&pred_terms, false)?;
        self.assert(any_pred);

        // Build the ITE chain right-to-left. The innermost else is the
        // last element's constant — when its predicate is false the chain
        // still returns a well-typed Int, and the asserted disjunction
        // forbids that branch via the global assertion above.
        let last_idx = elements.len() - 1;
        let mut acc = value_terms[last_idx];
        for i in (0..last_idx).rev() {
            acc = self.solver.try_ite(pred_terms[i], value_terms[i], acc)?;
        }
        Ok(acc)
    }
    ...
    pub(super) fn translate_choose_bool(
        ...
        // For BOOLEAN domain, eagerly expand to an ITE over the two
        // constants {TRUE, FALSE}. The previous Skolem-plus-assertion
        // encoding (`declare-const sk Bool; assert P(sk)`) triggers the
        // ay-dpll lazy theory pipeline's Bool-only Unknown(Incomplete) path
        // on otherwise-trivial queries (reproducible by asserting just
        // `(declare-const sk Bool) (assert sk)` without any Int sort in
        // scope). Substituting both constants and ITE-ing eliminates the
        // free Bool variable and lets the solver constant-fold each
        // branch.
        if let Expr::Ident(name, _) = &domain.node {
            if name == "BOOLEAN" {
                let pred_true = self.substitute_and_translate(body, var_name, &Expr::Bool(true))?;
                let pred_false =
                    self.substitute_and_translate(body, var_name, &Expr::Bool(false))?;
                // Assert that at least one branch holds.
                let any = self.solver.try_or(pred_true, pred_false)?;
                self.assert(any);
                // sk = ITE(P[true], true, false)
                let true_t = self.solver.bool_const(true);
                let false_t = self.solver.bool_const(false);
                let sk = self.solver.try_ite(pred_true, true_t, false_t)?;
                return Ok(sk);
            }
        }
    ...
    fn try_expand_finite_quantifier(
        ...
        // Part of #3826: wire nested SUBSET encoding into ay Init enumeration
    ...
    /// compound set ops.
    fn extract_universe_ints_for_quantifier(&self, expr: &Spanned<Expr>) -> AYResult<Vec<i64>> {
    ...
    fn combine_bool_terms(&mut self, terms: &[Term], is_forall: bool) -> AYResult<Term> {
    ...
    fn assert_skolem_membership(&mut self, sk_term: Term, domain: &Spanned<Expr>) -> AYResult<()> {
        match &domain.node {
    ...
    fn extract_domain_elements(&self, expr: &Spanned<Expr>) -> AYResult<Vec<Spanned<Expr>>> {
        match &expr.node {
...
/// Domain-size cap for the eager ITE-chain CHOOSE encoding. Beyond this
/// the chain depth makes the resulting term shape unhelpful; the Skolem
/// path takes over.
const CHOOSE_EAGER_DOMAIN_LIMIT: i64 = 64;

/// Try to collect a `CHOOSE` domain as an enumerable list of integer
/// constants. Recognises literal `SetEnum` of `Int` and small literal
/// `Range`s. Returns `None` for any non-literal or oversize domain — the
/// caller then falls back to the Skolem encoding.
fn collect_choose_int_domain(domain: &Spanned<Expr>) -> Option<Vec<BigInt>> {
    match &domain.node {
        Expr::SetEnum(elements) => {
            ...
            if out.len() as i64 > CHOOSE_EAGER_DOMAIN_LIMIT {
                return None;
            }
            Some(out)
        }
        Expr::Range(lo, hi) => {
            if let (Expr::Int(lo_n), Expr::Int(hi_n)) = (&lo.node, &hi.node) {
                let size = hi_n - lo_n + BigInt::from(1);
                let size_i64 = i64::try_from(&size).ok()?;
                if size_i64 <= 0 || size_i64 > CHOOSE_EAGER_DOMAIN_LIMIT {
                    return None;
                }
                let mut out = Vec::with_capacity(size_i64 as usize);
                let mut cur = lo_n.clone();
                while &cur <= hi_n {
                    out.push(cur.clone());
                    cur += 1;
                }
