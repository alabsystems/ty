...
//! Record and tuple encoding dispatch for ay translator via [`RecordEncoder`].
//!
...
impl AYTranslator {
    ...
    /// Translate record field access `r.field` returning String (interned Int).
    ///
    /// Used for the `tag` field of a desugared Apalache variant, which is a
    /// `String`-sorted field. Mirrors the Bool/Int field-access methods.
    pub(super) fn translate_record_access_string(
        &mut self,
        base: &Spanned<Expr>,
        field: &str,
    ) -> AYResult<Term> {
        ...
        // Verify field exists and has String sort
        let sort = enc.field_sort(self, &record_name, field)?;
    ...
    fn extract_record_name(&self, base: &Spanned<Expr>) -> AYResult<String> {
        match &base.node {
            Expr::Ident(name, _) => {
                if self.record_vars.contains_key(name) {
                    Ok(name.clone())
            ...
            _ => Err(AYError::UnsupportedOp(
                "record field access on non-variable expression not supported in ay".to_string(),
            )),
        }
    }
}
