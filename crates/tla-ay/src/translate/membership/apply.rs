...
use super::{AYTranslator, FunctionVarInfo};
...
impl AYTranslator {
    ...
    /// Convert a string key back to a ay term
    fn key_to_term(&mut self, key: &str) -> AYResult<Term> {
        ...
        Err(AYError::UnsupportedOp(format!(
            "cannot convert key '{key}' to ay term"
        )))
    }
}
