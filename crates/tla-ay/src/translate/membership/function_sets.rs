...
impl AYTranslator {
    ...
    fn extract_finite_domain(&self, domain: &Spanned<Expr>) -> AYResult<Vec<String>> {
        match &domain.node {
    ...
    fn infer_range_sort(&self, range: &Spanned<Expr>) -> AYResult<TlaSort> {
        match &range.node {
    ...
    /// which avoids disjunctions that trigger ay QF_LIA incompleteness.
