...
//! Set membership, function application, and domain helpers for AY translation.
//!
//! Handles:
...
use super::{AYTranslator, FunctionVarInfo, TlaSort};
...
impl AYTranslator {
    ...
    pub(super) fn translate_membership(
        ...
        match &set.node {
            ...
            // term that references `elem` so ay assigns it in the model. This matters for
            ...
            Expr::SetEnum(elements) => {
                ...
                if is_tuple_set {
                    ...
                    // disjunctions. This avoids ay QF_LIA incompleteness on
            ...
            Expr::Domain(func_expr) => {
                ...
                Err(AYError::UnsupportedOp(
                    "DOMAIN on expression not supported in membership context".to_string(),
                ))
            }
    ...
    pub(super) fn translate_func_apply_bool(
        ...
        // Option-A native path: `s[i]` where `s` is any native sequence
        // expression (var, literal, Tail/Append/SubSeq/\o). Falls through when
        // `func` is not a native sequence.
        if let Some(result) = self.try_native_seq_index(func, arg) {
            return result;
        }
