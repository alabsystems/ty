...
//! [`AYTranslator::declare_record_var`]. The encoder operates on these
//! pre-declared variables to build constraints.
//!
//! # Usage
//!
//! ```no_run
//! use tla_ay::translate::record_encoder::RecordEncoder;
//! use tla_ay::{AYTranslator, TlaSort};
//!
...
//! # Errors
//!
//! These encoder methods operate on records/tuples already declared on the
//! translator, so the fallible ones share three error causes:
//! [`AYError::UnknownVariable`](crate::AYError::UnknownVariable) when a record,
//! tuple, or field name does not resolve;
//! [`AYError::UnsupportedOp`](crate::AYError::UnsupportedOp) for a structural
//! mismatch (e.g. a field/arity disagreement or an out-of-range tuple index); and
//! a propagated [`AYError::Solver`](crate::AYError::Solver) when an underlying ay
//! term constructor rejects its arguments.
//!
//! Part of #3749
...
impl RecordEncoder {
    ...
    ///
    /// Returns the ay term for the field's SMT variable.
    /// The record must have been previously declared via
    /// [`AYTranslator::declare_record_var`].
    ...
    ///
    /// The `field_terms` map provides pre-translated ay terms for each
        ...
        for (field_name, _sort) in &info.field_sorts {
            let var_term =
                info.field_terms.get(field_name).copied().ok_or_else(|| {
                    AYError::UnknownVariable(format!("{record_name}.{field_name}"))
                })?;
    ...
    pub fn encode_record_except(
        ...
        for (fname, _sort) in &source_info.field_sorts {
            let target_term = target_info
                ...
                .get(fname)
                .copied()
                .ok_or_else(|| AYError::UnknownVariable(format!("{target_name}.{fname}")))?;
            ...
            if fname == field_name {
                ...
                let source_term =
                    source_info.field_terms.get(fname).copied().ok_or_else(|| {
                        AYError::UnknownVariable(format!("{source_name}.{fname}"))
                    })?;
    ...
    ///
    /// The `element_terms` slice provides pre-translated ay terms for each
    ...
    pub fn encode_tuple_construction(
        ...
        for (i, &val_term) in element_terms.iter().enumerate() {
            ...
            let var_term = info
                ...
                .get(&idx)
                .copied()
                .ok_or_else(|| AYError::UnknownVariable(format!("{tuple_name}[{idx}]")))?;
            let eq = trans.solver_mut().try_eq(var_term, val_term)?;
            conjuncts.push(eq);
        }
    ...
    ///
    /// Returns the ay term for the element at position `index` (1-based).
    ...
    pub fn encode_record_equality(
        ...
        for (field_name, _sort) in &left_info.field_sorts {
            let left_term = left_info
                ...
                .get(field_name)
                .copied()
                .ok_or_else(|| AYError::UnknownVariable(format!("{left_name}.{field_name}")))?;
            let right_term = right_info
                .field_terms
                .get(field_name)
                .copied()
                .ok_or_else(|| AYError::UnknownVariable(format!("{right_name}.{field_name}")))?;
    ...
    /// where each element is a pre-translated ay term.
    ...
    /// `r__f1 = v1 /\ ... /\ r__fn = vn`.
    ///
    /// The `literal_field_terms` map provides pre-translated ay terms for
...
fn build_conjunction(trans: &mut AYTranslator, conjuncts: &[Term]) -> AYResult<Term> {
