...
use ay_dpll::api::{Logic, Model, SolveResult, Solver, Sort, Term};
...
pub struct NestedPowersetEncoder {
    ...
    /// True once any constraint may have been asserted (via `assert_constraint`,
    /// the `assert_*_cardinality` helpers, or `solver_mut` — the latter is
    /// counted conservatively since the caller can assert through it).
    /// While false, `enumerate_all` bypasses AllSAT entirely: the solution set
    /// is exactly the 2^n subsets of the base, which can be enumerated by
    /// bitmask in O(2^n) without any solver calls.
...
impl NestedPowersetEncoder {
    ...
    pub fn new(base_elements: Vec<BaseElement>) -> AYResult<Self> {
    ...
    ///
    /// # Panics
    /// Panics if `term` is not Bool-sorted.
    ...
    ///
    /// # Errors
    /// Returns [`AYError::Solver`] if building or asserting the cardinality-sum
    /// constraint fails.
    pub fn assert_exact_cardinality(&mut self, k: usize) -> AYResult<()> {
        self.has_constraints = true;
    ...
    /// Returns [`AYError::Solver`] if building or asserting the cardinality-sum
    /// constraint fails.
    pub fn assert_max_cardinality(&mut self, k: usize) -> AYResult<()> {
        self.has_constraints = true;
        let sum = self.build_cardinality_sum()?;
        let k_term = self.solver.int_const(k as i64);
    ...
    /// Returns [`AYError::Solver`] if building or asserting the cardinality-sum
    /// constraint fails.
    pub fn assert_min_cardinality(&mut self, k: usize) -> AYResult<()> {
        self.has_constraints = true;
        let sum = self.build_cardinality_sum()?;
        let k_term = self.solver.int_const(k as i64);
    ...
    ///
    /// When no constraint has ever been asserted on the encoder, the solution
    /// set is exactly the powerset of the base and is enumerated directly by
    /// bitmask (no solver calls) — this keeps the common unconstrained
    /// `SUBSET(...)` Init/quantifier path at ~milliseconds instead of paying
    /// for 2^n incremental SAT checks.
    ///
    /// # Errors
    ...
    pub fn enumerate_all(
    ...
    ) -> AYResult<NestedPowersetSolution> {
        ...
        // Fast path: with no constraints, every subset of the base is a
        // solution — enumerate 2^n bitmasks directly instead of AllSAT.
        if !self.has_constraints {
            return self.enumerate_unconstrained(config);
        }
        ...
        loop {
            ...
            match result.into_inner() {
                SolveResult::Sat => {
                    let model = self.solver.try_get_model_for_consumer().map_err(|e| {
                        AYError::UnsupportedOp(format!("model extraction failed: {e}"))
                    })?;
    ...
    /// Enumerate the full powerset of the base by bitmask (unconstrained case).
    ///
    /// Sound only when no constraint has been asserted: then every truth
    /// assignment to the base variables satisfies the (empty) formula, so the
    /// AllSAT loop would enumerate exactly these 2^n subsets, just via 2^n
    /// solver checks. `n <= MAX_NESTED_POWERSET_BASE` (enforced in `new`), so
    ...
    fn build_cardinality_sum(&mut self) -> AYResult<Term> {
    ...
    fn add_blocking_clause(&mut self, model: &Model) -> AYResult<()> {
    ...
    ///
    /// Conservatively marks the encoder as constrained (disabling the
    /// unconstrained bitmask fast path in [`Self::enumerate_all`]), since the
    /// caller can assert arbitrary terms through the returned reference.
...
mod tests {
    ...
    // (848 = sum C(10,k), k=4..10), not performance. Measured dev-profile
    // wall-clock of the 848-solution ALLSAT enumeration: ~54.8s at ay pin
    // d7177e70 (91% of a 60s budget) and ~67-69s at pin b85e00f9 — a diffuse
    // upstream perf drift spread across ~100+ ay commits (the independent
    // model gate's 848 per-SAT confirmations account for only ~2s of it), so
    // a 60s budget was a coin-flip gate. 3x headroom keeps the correctness
    // gate honest; if enumeration LATENCY matters, track it separately with a
    // release-profile benchmark.
    ...
    // Re-enabled 2026-07-20: was #[ignore]d for the ay-align(2026-06-20) AllSAT
    // perf regression (1024 Init states timed out; see
    // docs/ay-alignment-followup-2026-06-20.md). This workload is unconstrained,
    // so it now takes enumerate_all's bitmask fast path (~ms, no solver calls).
