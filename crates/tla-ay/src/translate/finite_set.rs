...
//!
//! ```no_run
//! use tla_ay::translate::finite_set::FiniteSetEncoder;
//! use tla_ay::AYTranslator;
//!
...
//! # Errors
//!
//! Every fallible encoder method here builds ay array/Bool/Int terms through the
//! solver and propagates an [`AYError::Solver`](crate::AYError::Solver) if a term
//! constructor rejects its arguments (for example a sort mismatch in a `select` /
//! `store`). They report no other error variants. Individual method docs note this
//! only where additional context applies.
//!
//! Part of #3749

use ay_dpll::api::{Sort, Term};

use crate::error::AYResult;
...
impl FiniteSetEncoder {
    ...
    pub fn empty_set(&self, trans: &mut AYTranslator) -> AYResult<Term> {
    ...
    ///
    /// Note: For literal sets {e1, ..., en}, this method still uses the array
    /// term produced by `encode_set_enum`. The higher-level `translate_membership`
    /// in `membership/mod.rs` optimizes literal SetEnum to disjunctions
    /// to bypass ay array bugs for small constant sets.
    ...
    fn declare_fresh_set(&self, trans: &mut AYTranslator, prefix: &str) -> AYResult<Term> {
...
impl AYTranslator {
    ...
    pub(crate) fn solver_mut(&mut self) -> &mut ay_dpll::api::Solver {
        &mut self.solver
    }
    ...
    ///
    /// IMPORTANT: the generated name must NOT begin with `__ay_`. That prefix
    /// is reserved by the AY solver for its own internal/auxiliary encoding
    /// symbols (store decomposition, extensionality witnesses, Skolem terms,
    /// etc.). AY excludes any `__ay_*` symbol from the consumer-facing model
    /// during model validation, so a *user* constant declared with that prefix
    /// is silently dropped from the model. When such a constant is the witness
    /// for a SAT instance, AY cannot certify a complete model and conservatively
    /// degrades the result from `Sat` to `Unknown`. Using a ty-owned prefix
    /// (`tyv_`) keeps fresh sets/sequences/functions/records visible to the
    /// model verifier so genuinely satisfiable encodings return `Sat`.
...
mod tests {
    ...
    fn array_translator() -> AYTranslator {
        AYTranslator::new_with_arrays()
    }
    ...
    fn test_set_enum_membership_sat() {
        ...
        // Constrain x to a finite index domain. This array-`select` membership
        // encoding is only meant to be used over the finite domains that TLA+
        // model checking always operates on (see the module-level docs). AY's
        // array solver cannot existentially synthesize an *unbounded* integer
        // index that reads `true` from a const-array store-chain, so without a
        // domain bound it soundly returns `Unknown` instead of `Sat` for this
        // free-index read. The bound is logically redundant — `x \in {1,2,3}`
        // already implies `x \in -10..10` — and is wider than the set, so the
        // model still genuinely picks a member rather than being pinned. (The
        // production `translate_membership` path avoids this entirely by
        // lowering literal `SetEnum` membership to a disjunction; see the note
        // on `encode_membership`.)
        let lo = trans.solver_mut().int_const(-10);
        let hi = trans.solver_mut().int_const(10);
        let ge_lo = trans.solver_mut().try_ge(x, lo).unwrap();
        let le_hi = trans.solver_mut().try_le(x, hi).unwrap();
        trans.assert(ge_lo);
        trans.assert(le_hi);
