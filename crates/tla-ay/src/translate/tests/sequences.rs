...
//! Tests for sequence operations in the AY translator.
//!
...
fn constrain_seq(trans: &mut AYTranslator, name: &str, values: &[i64]) {
...
fn test_encoder_single_tail_sat() {
    ...
    let mut trans = AYTranslator::new_with_arrays();
    let enc = SequenceEncoder::new(ay_dpll::api::Sort::Int);
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_ay_arithmetic_lt_sub() {
    // Debug: check if ay-dpll handles 1 < (4 - 1) correctly
    let mut trans = AYTranslator::new_with_arrays();
    let four = trans.solver_mut().int_const(4);
    let one = trans.solver_mut().int_const(1);
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_ay_implies_with_array_select() {
    // Debug: check if implies(true, select(a,i) = select(b,j)) works
    let mut trans = AYTranslator::new_with_arrays();
    let arr_sort = ay_dpll::api::Sort::array(ay_dpll::api::Sort::Int, ay_dpll::api::Sort::Int);
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_ay_chained_implies_select() {
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_ay_implies_lt_sub_array() {
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_ay_double_tail_manual_constraints() {
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_ay_declare_const_same_name_is_same_term() {
...
fn test_concat_via_seq_var_to_seq_term() {
    ...
    let ay_sort_s = info_s.element_sort.to_ay().unwrap();
    let seq_s = trans.seq_var_to_seq_term(&info_s, &ay_sort_s).unwrap();

    let info_t = trans.get_seq_var("t").unwrap().clone();
    let ay_sort_t = info_t.element_sort.to_ay().unwrap();
    let seq_t = trans.seq_var_to_seq_term(&info_t, &ay_sort_t).unwrap();

    // Skip pre-check to avoid solver state mutation
    // Now do concat
    let enc = SequenceEncoder::new(ay_sort_s);
