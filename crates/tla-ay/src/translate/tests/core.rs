...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_try_get_model_flows_through_ay_consumer_boundary() {
    let mut trans = AYTranslator::new();
    trans.declare_var("x", TlaSort::Int).unwrap();
    ...
    assert_eq!(trans.try_check_sat().unwrap(), SolveResult::Sat);
    let direct = trans
        .solver
    ...
    let wrapped = trans
        .try_get_model()
        .expect("TY translator should consume AY consumer model");

    assert_eq!(direct.int_val("x").cloned(), Some(bi(7)));
    assert_eq!(wrapped.int_val("x").cloned(), Some(bi(7)));
    assert!(trans.get_model().is_some());
}

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_tla_sort_to_ay_rejects_function_sort() {
    let sort = TlaSort::Function {
        domain_keys: vec!["k".to_string()],
        range: Box::new(TlaSort::Int),
    };

    let err = sort.to_ay().unwrap_err();
    assert!(matches!(err, AYError::UnsupportedOp(_)));
    assert!(
        err.to_string()
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_tla_sort_to_ay_rejects_tuple_sort() {
