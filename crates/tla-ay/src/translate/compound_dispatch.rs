...
//! Compound type expression dispatch for the ay translator.
//!
...
impl AYTranslator {
    ...
    pub(crate) fn translate_func_def_bool(
        ...
        if bounds.len() != 1 {
            return Err(AYError::UnsupportedOp(
                "multi-variable function definitions not yet supported in ay".to_string(),
            ));
        }
        ...
        let domain = bound.domain.as_ref().ok_or_else(|| {
            AYError::UnsupportedOp(
                "function definition without domain not supported in ay".to_string(),
            )
        })?;
    ...
    pub(crate) fn translate_domain_bool(&mut self, func_expr: &Spanned<Expr>) -> AYResult<Term> {
        ...
        let name = match &func_expr.node {
            ...
            _ => {
                return Err(AYError::UnsupportedOp(
                    "DOMAIN on non-variable expression not supported in ay".to_string(),
                ))
            }
        };
    ...
    pub(crate) fn translate_except_bool(
        ...
        let base_name = match &base.node {
            ...
            _ => {
                return Err(AYError::UnsupportedOp(
                    "EXCEPT on non-variable base not supported in ay".to_string(),
                ))
            }
        };
    ...
    fn translate_record_except_bool(
        ...
        if specs.len() != 1 || specs[0].path.len() != 1 {
            return Err(AYError::UnsupportedOp(
                "multi-field or nested record EXCEPT not yet supported in ay".to_string(),
            ));
        }
    ...
    fn translate_func_except_bool(
        ...
        let info = self
            ...
            .get(base_name)
            .cloned()
            .ok_or_else(|| AYError::UnknownVariable(format!("function {base_name}")))?;
        ...
        let new_info = self
            ...
            .get(&fresh_prefix)
            .cloned()
            .ok_or_else(|| AYError::UnknownVariable(format!("function {fresh_prefix}")))?;
        ...
        for spec in specs {
            if spec.path.len() != 1 {
                return Err(AYError::UnsupportedOp(
                    "nested function EXCEPT paths not supported in ay".to_string(),
                ));
            }
            ...
            if let Some(&new_term) = new_info.element_terms.get(&key) {
                ...
                return Err(AYError::UnknownVariable(format!("{fresh_prefix}[{key}]")));
            }
        }
        ...
        for key in &info.domain_keys {
            if !updated_keys.contains(key) {
                let old_term = info
                    ...
                    .get(key)
                    .copied()
                    .ok_or_else(|| AYError::UnknownVariable(format!("{base_name}[{key}]")))?;
                let new_term =
                    new_info.element_terms.get(key).copied().ok_or_else(|| {
                        AYError::UnknownVariable(format!("{fresh_prefix}[{key}]"))
                    })?;
        ...
        if self.func_vars.contains_key(&left_name) && self.func_vars.contains_key(&right_name) {
            ...
            for key in &f_info.domain_keys {
                let f_term = f_info
                    ...
                    .get(key)
                    .copied()
                    .ok_or_else(|| AYError::UnknownVariable(format!("{left_name}[{key}]")))?;
                let g_term = g_info
                    .element_terms
                    .get(key)
                    .copied()
                    .ok_or_else(|| AYError::UnknownVariable(format!("{right_name}[{key}]")))?;
    ...
    pub(crate) fn translate_set_enum_term(&mut self, elements: &[Spanned<Expr>]) -> AYResult<Term> {
    ...
    fn extract_set_enum_ints(&self, set: &Spanned<Expr>) -> AYResult<Vec<i64>> {
        match &set.node {
    ...
    pub(super) fn translate_cardinality_int(&mut self, set_expr: &Spanned<Expr>) -> AYResult<Term> {
        let enc = FiniteSetEncoder::new();
    ...
    pub(crate) fn translate_set_expr_term(&mut self, expr: &Spanned<Expr>) -> AYResult<Term> {
        match &expr.node {
            ...
            Expr::BigUnion(inner) => {
                ...
                match &inner.node {
                    ...
                    _ => Err(AYError::UnsupportedOp(
                        "UNION over non-enumerated set-of-sets not supported in ay; \
    ...
    fn extract_universe_terms(&mut self, set_exprs: &[&Spanned<Expr>]) -> AYResult<Vec<Term>> {
    ...
    pub(super) fn extract_int_literal(&self, expr: &Spanned<Expr>) -> AYResult<i64> {
        match &expr.node {
            Expr::Int(n) => n.try_into().map_err(|_| {
    ...
    fn try_expr_to_key_static(&self, expr: &Spanned<Expr>) -> AYResult<String> {
        match &expr.node {
