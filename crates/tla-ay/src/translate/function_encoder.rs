...
//!
//! ```no_run
//! use tla_ay::translate::function_encoder::FunctionEncoder;
//! use tla_ay::AYTranslator;
//!
//! let mut trans = AYTranslator::new_with_arrays();
//! let enc = FunctionEncoder::new(ay_dpll::api::Sort::Int, ay_dpll::api::Sort::Int);
//! // ... use enc methods to build function terms
//! ```
//!
//! # Errors
//!
//! Fallible encoder methods build ay array terms through the solver and propagate
//! an [`AYError::Solver`](crate::AYError::Solver) on a term-constructor rejection
//! (e.g. a sort mismatch). The exception is documented inline:
//! [`encode_func_def`](FunctionEncoder::encode_func_def) also returns
//! [`AYError::UnsupportedOp`](crate::AYError::UnsupportedOp) when the domain and
//! value lists differ in length.
//!
//! Part of #3749
...
impl FunctionEncoder {
    ...
    /// # Errors
    ///
    /// Returns `AYError::UnsupportedOp` if `domain_elements` and `range_values`
    ...
    pub fn encode_func_set(
        ...
        domain_set: Term,
        universe: &[Term],
        range_constraint: impl Fn(&mut AYTranslator, Term) -> AYResult<Term>,
    ) -> AYResult<FuncTerm> {
    ...
    fn declare_fresh_mapping(&self, trans: &mut AYTranslator, prefix: &str) -> AYResult<Term> {
