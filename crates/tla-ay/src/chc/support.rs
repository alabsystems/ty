...
pub(super) fn tla_sort_to_chc(sort: &TlaSort) -> AYResult<ChcSort> {
    match sort {
        TlaSort::Bool => Ok(ChcSort::Bool),
        TlaSort::Int => Ok(ChcSort::Int),
        TlaSort::String => Ok(ChcSort::Int),
        TlaSort::Function { .. } => Err(AYError::UnsupportedOp(
        ...
        TlaSort::Record { .. } => Err(AYError::UnsupportedOp(
...
pub(super) fn extract_finite_domain_keys(domain: &Spanned<Expr>) -> AYResult<Vec<String>> {
    match &domain.node {
