...
pub(super) fn pdr_config(max_frames: usize, max_iterations: usize) -> PdrConfig {
    let mut cfg = PdrConfig::default()
        ...
        // Default `max_obligations` is 100_000 — way too high for unit tests
        // that have a 10s `ntest::timeout` wall-clock cap. Cap at 1_000 to
        // bound per-strengthen work.
        .with_max_obligations(1_000);
    // Set a wall-clock budget so the PDR `solve_deadline` triggers
    // `is_cancelled` between startup-discovery passes. Without this, the
    // discovery pipeline (ay-chc::pdr::solver::startup) and inner SMT
    // queries in `push_lemmas`/`can_push_lemma` have no enforceable upper
    // bound for non-LIA problems, causing the 10s test wrapper to fire.
    //
    // 5s leaves headroom for the proof-grade validation re-solve. Tests in
    // this file accept Safe or Unknown — only Unsafe on a safe spec (or
    // Safe on an unsafe spec) is treated as a test failure.
    //
    // NOTE: Some specs (set-enum membership, multiplication, in-range
    // unsafe) still time out at 10s even with this budget because individual
    // SMT queries in `can_push_lemma` may inherit a 10s timeout (see
    // ay-chc::pdr::solver::model::push_lemmas:247) and `solve_deadline` is
    // only checked between operations, not mid-SMT-call. Fixing that
    // requires changes in ay-chc proper.
