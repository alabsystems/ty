...
// 30s wall-clock budget (matching the other slow PDR tests in this file, e.g.
// `test_implies_operator`). The multiplication `x' = x*2` is now linearized to
// `x + x` by the CHC translator (`mul` in `chc/translation.rs`), so the SMT
// queries stay in decidable LIA and the solve terminates instead of hanging.
// It still returns a sound `Unknown` rather than `Safe`: the reachable set
// {1,2,4,8} is non-convex, so PDR over LIA does not find an inductive invariant
// and exhausts its budget. That bounded search runs ~12s here — past the old
// 10s wrapper because ay-chc only checks `solve_deadline` *between* operations
// (see the NOTE in `helpers::pdr_config`) — so a 30s wrapper lets the sound
// result be observed deterministically. The test only fails on `Unsafe`.
