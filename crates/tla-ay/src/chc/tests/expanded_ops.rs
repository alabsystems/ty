...
// `test_let_in_int_context`). This spec is the acknowledged "set-enum
// membership" slow case from `pdr_config`'s NOTE: an inner SMT query in
// ay-chc's `can_push_lemma` inherits a hard ~10s timeout that the configured
// `solve_timeout` (5s) cannot interrupt mid-call. The PDR solve therefore
// returns a *sound* `Unknown` at ~10s — correct for this safe spec, since the
// test only fails on `Unsafe` — but that overran the previous 10s wrapper by a
// few ms. A 30s wrapper lets the solve report its result deterministically.
