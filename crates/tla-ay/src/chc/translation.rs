...
use ay_chc::{ChcExpr, ChcOp, ChcVar};
...
use super::{ChcClauseCtx, ChcTranslator};
...
impl ChcTranslator {
    fn intern_tagged_atom(&mut self, tag: &str, value: &str) -> AYResult<i64> {
        ...
        let next_id = i64::try_from(self.atom_intern.len() + 1)
            .map_err(|_| AYError::IntegerOverflow("too many interned CHC atoms".to_string()))?;
    ...
    /// Lower TLA+ `\div` / `%` to `ChcOp::Div` / `ChcOp::Mod`.
    ///
    /// TLA+ requires the divisor to be positive (TLC raises a runtime error
    /// otherwise); for a positive divisor, TLA+ `\div`/`%` coincide with the
    /// SMT-LIB Euclidean `div`/`mod` that ay-chc implements (ay-core
    /// `mk_intdiv`/`mk_mod`: `a = b*q + r ∧ 0 <= r < |b|`, underspecified for
    /// `b = 0`). Acceptance is therefore gated on divisor positivity:
    ///
    /// - Positive integer literal: accept as-is (fast path, no obligation).
    /// - Non-positive integer literal: hard decline — TLC errors on EVERY
    ///   evaluation of this occurrence, so PDR must return Unknown and let
    ///   the explicit lane surface the error (never mask it via augmentation).
    /// - Non-literal divisor in Next (unprimed) or Safety: accept, and record
    ///   the side-condition `divisor > 0` (over current-state variables). The
    ///   side-conditions are conjoined into the safety obligation
    ///   (`finalize_query_clauses` in `builder.rs`), so a `Safe` verdict
    ///   proves the divisor positive in EVERY reachable state. That covers
    ///   every state where TLC can evaluate the occurrence: Safety is
    ///   evaluated at reachable states, and an unprimed divisor in Next is
    ///   evaluated at reachable pre-states.
    /// - Non-literal divisor mentioning primed variables: decline. TLC
    ///   evaluates primed sub-expressions against candidate successor
    ///   assignments during Next enumeration — including candidates later
    ///   pruned by other conjuncts — so no reachable-state side-condition can
    ///   rule out a TLC division error there.
    /// - Non-literal divisor in Init: decline. TLC evaluates Init over
    ///   candidate initial assignments (again including candidates pruned by
    ///   later conjuncts), while a side-condition can only be checked on the
    ///   states that actually satisfy Init — too weak to exclude a TLC error
    ///   during initial-state enumeration.
    fn lower_div_mod(
        &mut self,
        op: ChcOp,
        display: &str,
        ...
        match &rhs {
            ChcExpr::Int(k) if *k > 0 => {
                // Literal positive divisor: always well-defined, no obligation.
                Ok(ChcExpr::Op(op, vec![Arc::new(lhs), Arc::new(rhs)]))
            }
            ChcExpr::Int(_) => Err(AYError::UnsupportedOp(format!(
                "CHC cannot translate {display} with a non-positive literal divisor \
                 (TLC requires divisor > 0; the explicit checker will surface the error)"
            ))),
            _ => {
                if self.clause_ctx == ChcClauseCtx::Init {
                    return Err(AYError::UnsupportedOp(format!(
                        "CHC cannot translate {display} with a non-constant divisor in Init \
                         (TLC evaluates Init over candidate initial assignments, so a \
                         reachable-state positivity side-condition cannot rule out a \
                         division error during initial-state enumeration)"
                    )));
                }
                if rhs.vars().iter().any(ChcVar::is_primed) {
                    return Err(AYError::UnsupportedOp(format!(
                        "CHC cannot translate {display} with a primed (next-state) divisor \
                         (TLC evaluates primed divisors against candidate successor \
                         assignments, which a reachable-state side-condition cannot cover)"
                    )));
                }
                // Non-literal, current-state divisor in Next/Safety: accept
                // and record the positivity side-condition at the occurrence.
                let side_condition = ChcExpr::gt(rhs.clone(), ChcExpr::Int(0));
                if !self.side_conditions.contains(&side_condition) {
                    self.side_conditions.push(side_condition);
                }
                Ok(ChcExpr::Op(op, vec![Arc::new(lhs), Arc::new(rhs)]))
            }
        }
    ...
    fn lookup_declared_value(&self, name: &str) -> AYResult<ChcValue> {
        let sort = self
            .var_sorts
            .get(name)
        ...
        match sort.clone() {
            TlaSort::FunctionSym { .. } => Err(AYError::UnsupportedOp(
                "symbolic-domain function sorts are not supported in the CHC lowering".to_string(),
            )),
            ...
            TlaSort::Record { .. } => {
                ...
                for (field_name, _) in &info.field_sorts {
                    let var = vars
                        .get(field_name)
                        .ok_or_else(|| AYError::UnknownVariable(format!("{name}.{field_name}")))?;
            ...
            TlaSort::Tuple { .. } => Err(AYError::UnsupportedOp(
                "tuple-valued CHC state is not supported".to_string(),
            )),
            TlaSort::Set { .. } => Err(AYError::UnsupportedOp(
                "set-valued CHC state is not yet supported".to_string(),
            )),
            TlaSort::Sequence { .. } => Err(AYError::UnsupportedOp(
                "sequence-valued CHC state is not yet supported".to_string(),
            )),
        }
    }

    /// Lower a scalar equality, requiring both sides to have the SAME
    /// inferred sort.
    ///
    /// SOUNDNESS: the per-variable sorts are heuristically inferred (see
    /// `infer_var_sorts` in tla-check's `ay_shared.rs`: one scalar sort per
    /// variable from the first matching Init/TypeOK pattern, default Int),
    /// not checked. A variable ranging over both ints and model values
    /// (`Init == x = N \/ x = 1`, `N` a model value interned as String) is
    /// sorted Int, so constant-folding a cross-sort equality like `x = N` to
    /// FALSE would mask the reachable `x = N` states and let PDR report a
    /// false Safe. Cross-sort equality therefore fails closed with a decline
    /// (the PDR lane returns Unknown), mirroring `lower_div_mod`.
    fn scalar_eq(
        &self,
        left_expr: ChcExpr,
        left_sort: &TlaSort,
        ...
        if left_sort == right_sort {
            Ok(ChcExpr::eq(left_expr, right_expr))
        } else {
            Err(AYError::UnsupportedOp(format!(
                "CHC cannot translate equality between values of different inferred sorts \
                 ({left_sort:?} vs {right_sort:?}); TLA+ sort inference is heuristic, so \
                 cross-sort equality cannot soundly lower to false"
            )))
        }
    }
    ...
    fn translate_value(&mut self, expr: &Spanned<Expr>) -> AYResult<ChcValue> {
        match &expr.node {
            ...
            Expr::Int(n) => {
                let val: i64 = n
                    .try_into()
                    .map_err(|_| AYError::IntegerOverflow(format!("integer {n} too large")))?;
                Ok(ChcValue::Scalar {
                    expr: ChcExpr::Int(i128::from(val)),
                    sort: TlaSort::Int,
                })
            }
            Expr::String(s) => Ok(ChcValue::Scalar {
                expr: ChcExpr::Int(i128::from(self.intern_tagged_atom("str", s)?)),
                sort: TlaSort::String,
            }),
            Expr::Ident(name, _) | Expr::StateVar(name, _, _) => {
                if self.var_sorts.contains_key(name) {
                    ...
                    Ok(ChcValue::Scalar {
                        expr: ChcExpr::Int(i128::from(self.intern_tagged_atom("id", name)?)),
                        sort: TlaSort::String,
                    })
                }
            }
            ...
            _ => {
                ...
                let int_err = match dispatch_translate_int(self, expr) {
                    Ok(int_expr) => {
                ...
                // A deliberate policy decline (UnsupportedOp — e.g. the
                // divisor-positivity rules in `lower_div_mod`) carries an
                // actionable message; surface it instead of the generic one.
                if matches!(int_err, AYError::UnsupportedOp(_)) {
                    return Err(int_err);
                }
            ...
            (ChcValue::Function(then_fun), ChcValue::Function(else_fun)) => {
                ...
                for key in &then_fun.domain_keys {
                    let then_elem = then_fun
                        .elements
                        .get(key)
                        .ok_or_else(|| AYError::UnknownVariable(format!("ITE[{key}]")))?;
            ...
            (ChcValue::Record(then_record), ChcValue::Record(else_record)) => {
                ...
                for (field_name, _) in &then_record.field_sorts {
                    let then_field = then_record
                        .fields
                        .get(field_name)
                        .ok_or_else(|| AYError::UnknownVariable(format!("ITE.{field_name}")))?;
    ...
    fn translate_func_apply_value(
        ...
        if let Some(key) = self.try_expr_to_key(arg) {
            // Literal index fast path: an in-domain constant index needs no
            // obligation; a constant OUT-of-domain index hard-declines
            // (mirroring a non-positive literal divisor in `lower_div_mod`) —
            // TLC raises an out-of-domain error on EVERY evaluation of this
            // occurrence, so PDR must return Unknown and let the explicit
            // lane surface the error rather than totalize the function.
            let elem = func_val.elements.get(&key).ok_or_else(|| {
                AYError::UnsupportedOp(format!(
                    "CHC cannot translate f[{key}]: constant index outside the function's \
                     finite domain (TLC errors on every evaluation; the explicit checker \
                     will surface it)"
                ))
            })?;
            return Ok(ChcValue::Scalar {
        ...
        // Symbolic (non-literal) index: the ITE chain below only encodes
        // TLA+ semantics for IN-domain indices (its innermost else-branch is
        // the last key's element, which would silently totalize the partial
        // function for out-of-domain indices). Mirror `lower_div_mod`:
        // accept only where a "holds in every reachable state" side-condition
        // covers every TLC evaluation of the occurrence, and record the
        // domain-membership obligation `arg ∈ {domain keys}` so
        // `finalize_query_clauses` conjoins it into the safety obligation —
        // a Safe verdict then proves the index in-domain at every reachable
        // state where the application is evaluated.
        if self.clause_ctx == ChcClauseCtx::Init {
            return Err(AYError::UnsupportedOp(
                "CHC cannot translate f[i] with a non-constant index in Init \
                 (TLC evaluates Init over candidate initial assignments, so a \
                 reachable-state domain-membership side-condition cannot cover \
                 initial-assignment enumeration)"
                    .to_string(),
            ));
        }
        if arg_expr.vars().iter().any(ChcVar::is_primed) {
            return Err(AYError::UnsupportedOp(
                "CHC cannot translate f[i] with a primed (next-state) index \
                 (TLC evaluates primed indices against candidate successor \
                 assignments, which a reachable-state side-condition cannot cover)"
                    .to_string(),
            ));
        }

        // Non-literal, current-state index in Next/Safety: build the per-key
        // equality tests once, record the membership disjunction as a
        // side-obligation, and encode the application as an ITE chain.
        let mut key_eqs = Vec::with_capacity(func_val.domain_keys.len());
        for key in &func_val.domain_keys {
            ...
            key_eqs.push(self.scalar_eq(arg_expr.clone(), &sort, key_expr, &key_sort)?);
        }

        let side_condition = or_all(key_eqs.clone());
        if !self.side_conditions.contains(&side_condition) {
            self.side_conditions.push(side_condition);
        }

        let mut keyed = func_val.domain_keys.iter().zip(key_eqs).rev();
        let (last_key, _) = keyed.next().ok_or_else(|| {
            AYError::UnsupportedOp("function application on empty domain".to_string())
        })?;
        let mut result = func_val
            .elements
            .get(last_key)
            .ok_or_else(|| AYError::UnknownVariable(format!("f[{last_key}]")))?
            .clone();

        for (key, cond) in keyed {
            let elem = func_val
                .elements
                .get(key)
                .ok_or_else(|| AYError::UnknownVariable(format!("f[{key}]")))?
                .clone();
    ...
    fn translate_function_expr(
        ...
        let domain = bound.domain.as_ref().ok_or_else(|| {
            AYError::UnsupportedOp("FuncDef without finite domain is not supported".to_string())
        })?;
    ...
    fn translate_record_access_value(
        ...
        let field_sort = record
            ...
            .ok_or_else(|| AYError::UnknownVariable(format!("record field {field_name}")))?;
            ...
            match (&spec.path[0], &mut base_value) {
                (ExceptPathElement::Index(index), ChcValue::Function(func)) => {
                    ...
                    for key in &func.domain_keys {
                        ...
                        let old_expr = func
                            .elements
                            .get(key)
                            .ok_or_else(|| AYError::UnknownVariable(format!("EXCEPT[{key}]")))?
                            .clone();
                ...
                (ExceptPathElement::Field(field), ChcValue::Record(record)) => {
                    ...
                    let expected_sort = record
                        ...
                        .map(|(_, sort)| sort.clone())
                        .ok_or_else(|| {
                            AYError::UnknownVariable(format!("record field {field_name}"))
                        })?;
    ...
    fn translate_quantifier(
        ...
        let domain = bound.domain.as_ref().ok_or_else(|| {
            AYError::UnsupportedOp("unbounded CHC quantifiers are not supported".to_string())
        })?;

        if let Expr::SetFilter(filter_bound, pred) = &domain.node {
            let inner_domain = filter_bound.domain.as_ref().ok_or_else(|| {
                AYError::UnsupportedOp("SetFilter without domain not supported".to_string())
            })?;
        ...
        if let Expr::Range(lo, hi) = &domain.node {
            let lo_val: i64 = match &lo.node {
                Expr::Int(n) => n.try_into().map_err(|_| {
                    AYError::IntegerOverflow("range lower bound too large".to_string())
                })?,
                _ => 0,
            };
            let hi_val: i64 = match &hi.node {
                Expr::Int(n) => n.try_into().map_err(|_| {
                    AYError::IntegerOverflow("range upper bound too large".to_string())
                })?,
                _ => 0,
            };
    ...
    fn translate_eq_internal(
        ...
        match (left, right) {
            ...
            ) => self.scalar_eq(left_expr, &left_sort, right_expr, &right_sort),
            (ChcValue::Function(left_fun), ChcValue::Function(right_fun)) => {
                ...
                for key in &left_fun.domain_keys {
                    let left_expr = left_fun
                        .elements
                        .get(key)
                        .ok_or_else(|| AYError::UnknownVariable(format!("lhs[{key}]")))?
                        .clone();
                    let right_expr = right_fun
                        .elements
                        .get(key)
                        .ok_or_else(|| AYError::UnknownVariable(format!("rhs[{key}]")))?
                        .clone();
            ...
            (ChcValue::Record(left_record), ChcValue::Record(right_record)) => {
                ...
                for (field_name, field_sort) in &left_record.field_sorts {
                    let left_expr = left_record
                        .fields
                        .get(field_name)
                        .ok_or_else(|| AYError::UnknownVariable(format!("lhs.{field_name}")))?
                        .clone();
                    let right_expr = right_record
                        .fields
                        .get(field_name)
                        .ok_or_else(|| AYError::UnknownVariable(format!("rhs.{field_name}")))?
                        .clone();
...
impl TranslateExpr for ChcTranslator {
    ...
    fn int_const(&mut self, val: i64) -> AYResult<ChcExpr> {
        Ok(ChcExpr::Int(i128::from(val)))
    }

    fn lookup_bool_var(&mut self, name: &str) -> AYResult<ChcExpr> {
        match self.lookup_declared_value(name)? {
            ChcValue::Scalar {
                expr,
                sort: TlaSort::Bool,
            } => Ok(expr),
            ChcValue::Scalar { sort, .. } => Err(AYError::TypeMismatch {
                name: name.to_string(),
                expected: "Bool".to_string(),
                actual: format!("{sort}"),
            }),
            ChcValue::Function(_) => Err(AYError::TypeMismatch {
                name: name.to_string(),
                expected: "Bool".to_string(),
                actual: "Function".to_string(),
            }),
            ChcValue::Record(_) => Err(AYError::TypeMismatch {
                name: name.to_string(),
                expected: "Bool".to_string(),
    ...
    fn lookup_int_var(&mut self, name: &str) -> AYResult<ChcExpr> {
        match self.lookup_declared_value(name)? {
            ChcValue::Scalar {
                expr,
                sort: TlaSort::Int,
    ...
    fn mul(&mut self, lhs: ChcExpr, rhs: ChcExpr) -> AYResult<ChcExpr> {
        // Constant-fold and linearize multiplication by an integer literal.
        //
        // CHC/PDR reasoning here operates over linear integer arithmetic (LIA).
        // A multiplication where one operand is a literal — e.g. `x * 2` — is in
        // fact linear, but emitting it as a raw `(* x 2)` term drives the
        // underlying SMT queries into nonlinear arithmetic (NIA). In that regime
        // the PDR fixpoint loop fails to converge AND the per-query SMT calls do
        // not terminate, so `solve_deadline` (which is only checked *between*
        // operations) never fires and the configured `solve_timeout` is silently
        // ignored — the solve appears to hang indefinitely (the root cause of
        // `test_multiplication` timing out well past its wall-clock budget).
        //
        // Lowering `e * c` to the equivalent linear sum `e + e + ... + e`
        // (|c| copies, negated when c < 0) keeps the problem in LIA: every SMT
        // query stays decidable and fast, so the PDR budget actually bounds the
        // search and the solve returns a sound result (Safe/Unknown) promptly.
        // Genuinely nonlinear products (var * var) and very large coefficients
        // are left as `(* a b)` unchanged.
        if let (ChcExpr::Int(a), ChcExpr::Int(b)) = (&lhs, &rhs) {
            if let Some(p) = a.checked_mul(*b) {
                return Ok(ChcExpr::Int(p));
            }
        }
        let (expr, coeff): (ChcExpr, i128) = match (&lhs, &rhs) {
            (ChcExpr::Int(c), _) => (rhs.clone(), *c),
            (_, ChcExpr::Int(c)) => (lhs.clone(), *c),
            // Both operands non-literal: genuinely nonlinear — keep as-is.
            _ => return Ok(ChcExpr::mul(lhs, rhs)),
        };
        // Bound the unrolling so a huge literal coefficient cannot blow up the
        // term. Beyond the threshold, fall back to the raw product.
        const MAX_UNROLL: u128 = 1024;
        let magnitude = coeff.unsigned_abs();
        if magnitude == 0 {
            return Ok(ChcExpr::Int(0));
        }
        if magnitude > MAX_UNROLL {
            return Ok(ChcExpr::mul(lhs, rhs));
        }
        let mut acc = expr.clone();
        for _ in 1..magnitude {
            acc = ChcExpr::add(acc, expr.clone());
        }
        Ok(if coeff < 0 { ChcExpr::neg(acc) } else { acc })
    }
    ...
    fn div(&mut self, lhs: ChcExpr, rhs: ChcExpr) -> AYResult<ChcExpr> {
        self.lower_div_mod(ChcOp::Div, "\\div", lhs, rhs)
    }

    fn modulo(&mut self, lhs: ChcExpr, rhs: ChcExpr) -> AYResult<ChcExpr> {
        self.lower_div_mod(ChcOp::Mod, "%", lhs, rhs)
    }
    ...
    fn translate_eq(&mut self, left: &Spanned<Expr>, right: &Spanned<Expr>) -> AYResult<ChcExpr> {
        self.translate_eq_internal(left, right)
    }

    fn translate_bool_extended(&mut self, expr: &Spanned<Expr>) -> Option<AYResult<ChcExpr>> {
        match &expr.node {
            ...
            Expr::FuncApply(func, arg) => Some(match self.translate_func_apply_value(func, arg) {
                ...
                Ok(ChcValue::Scalar { sort, .. }) => Err(AYError::TypeMismatch {
                    name: "function application".to_string(),
                    expected: "Bool".to_string(),
                    actual: format!("{sort}"),
                }),
                Ok(ChcValue::Function(_)) => Err(AYError::TypeMismatch {
                    name: "function application".to_string(),
                    expected: "Bool".to_string(),
                    actual: "Function".to_string(),
                }),
                Ok(ChcValue::Record(_)) => Err(AYError::TypeMismatch {
                    name: "function application".to_string(),
                    expected: "Bool".to_string(),
                    actual: "Record".to_string(),
                }),
            ...
            Expr::Int(n) => {
                let val: i64 = match n.try_into() {
                    Ok(v) => v,
                    Err(_) => {
                        return Some(Err(AYError::IntegerOverflow(format!(
                            "integer {n} too large for CHC"
                        ))))
                    }
                };
                // Keep the existing i64 fragment gate (decline larger TLA ints
                // above), then widen losslessly into the i128 ChcExpr.
                Some(Ok(ChcExpr::Int(i128::from(val))))
            }
            Expr::Add(..)
            | Expr::Sub(..)
            | Expr::Mul(..)
    ...
    fn translate_int_extended(&mut self, expr: &Spanned<Expr>) -> Option<AYResult<ChcExpr>> {
        match &expr.node {
