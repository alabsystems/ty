...
//! Integration tests verifying ay SAT/SMT solver integration with TY.
//!
//! Tests cover:
//! - TLA+ expression translation to ay and satisfiability solving
//! - BCP (Boolean Constraint Propagation) via ay's DPLL solver
...
use tla_ay::{AYError, AYTranslator, SolveResult, TlaSort};
...
// ---------------------------------------------------------------------------
// Section 1: TLA+ formula translation to ay — SAT with model validation
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Section 2: BCP (Boolean Constraint Propagation) via ay's DPLL solver
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Section 4: BCP with integer theory (LIA) — ay DPLL + theory propagation
// ---------------------------------------------------------------------------

#[test]
#[timeout(10_000)]
fn test_lia_bcp_integer_bounds_sat() {
    // Integer arithmetic with BCP: ay's DPLL solver coordinates with the
...
fn test_unknown_variable_rejected() {
    ...
    assert!(
        matches!(
            err,
            AYError::UnknownVariable(_) | AYError::UntranslatableExpr(_)
        ),
