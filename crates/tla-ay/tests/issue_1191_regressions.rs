...
use tla_ay::{AYError, AYTranslator, BmcTranslator, BmcValue, SolveResult, TlaSort};
...
#[test]
#[timeout(10_000)]
fn test_quantifier_substitutes_statevar_body() -> Result<(), AYError> {
...
fn test_multi_bound_quantifier_errors_are_covered() {
    ...
    assert!(matches!(forall_err, Err(AYError::UnsupportedOp(_))));
    ...
    assert!(matches!(exists_err, Err(AYError::UnsupportedOp(_))));
}

#[test]
#[timeout(10_000)]
fn test_bmc_extract_trace_multi_variable_completeness() -> Result<(), AYError> {
    let k = 3;
    ...
    let model = trans
        .get_model()
        .ok_or_else(|| AYError::UntranslatableExpr("expected model for SAT result".to_string()))?;
