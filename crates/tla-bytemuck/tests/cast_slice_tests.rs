...
fn test_boxed_slices() {
    ...
    let result: Result<Box<[u16]>, (PodCastError, Box<[i8]>)> = try_cast_slice_box(boxed_i8_slice);
    let (error, boxed_i8_slice) = result.expect_err("u16 and i8 have different alignment");
    assert_eq!(error, PodCastError::AlignmentMismatch);

    let result: Result<&[[i8; 3]], PodCastError> = try_cast_slice(&*boxed_i8_slice);
    let error = result.expect_err("slice of [i8; 3] cannot be made from slice of 4 i8s");
    ...
    let result: Result<Box<[()]>, (PodCastError, Box<[i8]>)> = try_cast_slice_box(boxed_i8_slice);
    let (error, boxed_i8_slice) =
...
fn test_rc_slices() {
    ...
    let result: Result<Rc<[u16]>, (PodCastError, Rc<[i8]>)> = try_cast_slice_rc(rc_i8_slice);
    let (error, rc_i8_slice) = result.expect_err("u16 and i8 have different alignment");
    assert_eq!(error, PodCastError::AlignmentMismatch);
    ...
    let result: Result<Rc<[[i8; 3]]>, (PodCastError, Rc<[i8]>)> = try_cast_slice_rc(rc_i8_slice);
    ...
    let result: Result<Rc<[()]>, (PodCastError, Rc<[i8]>)> = try_cast_slice_rc(rc_i8_slice);
    let (error, rc_i8_slice) = result.expect_err("slice of ZST cannot be made from slice of 4 u8s");
    assert_eq!(error, PodCastError::OutputSliceWouldHaveSlop);
...
fn test_arc_slices() {
    ...
    let result: Result<Arc<[u16]>, (PodCastError, Arc<[i8]>)> = try_cast_slice_arc(arc_i8_slice);
    let (error, arc_i8_slice) = result.expect_err("u16 and i8 have different alignment");
    assert_eq!(error, PodCastError::AlignmentMismatch);
    ...
    let result: Result<Arc<[()]>, (PodCastError, Arc<[i8]>)> = try_cast_slice_arc(arc_i8_slice);
    let (error, arc_i8_slice) =
        result.expect_err("slice of ZST cannot be made from slice of 4 u8s");
    assert_eq!(error, PodCastError::OutputSliceWouldHaveSlop);
