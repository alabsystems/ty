...
fn test_transparent_wrapper() {
    ...
    let _: &mut Foreign = Wrapper::peel_mut(bytemuck::cast_mut(&mut u8::default()));

    let _: &[u8] = bytemuck::cast_slice(Wrapper::wrap_slice(&[Foreign::default()]));
    let _: &[Foreign] = Wrapper::peel_slice(bytemuck::cast_slice(&[u8::default()]));

    let _: &mut [u8] = bytemuck::cast_slice_mut(Wrapper::wrap_slice_mut(&mut [Foreign::default()]));
    let _: &mut [Foreign] = Wrapper::peel_slice_mut(bytemuck::cast_slice_mut(&mut [u8::default()]));
    ...
    let _: &mut Foreign = Wrapper::peel_mut(bytemuck::from_bytes_mut(&mut [u8::default()]));
