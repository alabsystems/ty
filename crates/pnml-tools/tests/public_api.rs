...
fn test_place_and_transition_idx_types_accessible() {
    ...
    let _: pnml_tools::PlaceIdx = pnml_tools::PlaceIdx(0);
    let _: pnml_tools::TransitionIdx = pnml_tools::TransitionIdx(0);
...
#[test]
fn test_legacy_facade_exposes_shared_mcc_config_resolver() {
    let dir = TempDir::new().unwrap();
    write_model(&dir);

    let model = load_model_dir(dir.path()).expect("minimal model should load");
    let config = pnml_tools::cli::build_exploration_config(
        &model,
        &pnml_tools::cli::PetriRunArgs {
            threads: 4,
            ..pnml_tools::cli::PetriRunArgs::default()
        },
    )
    .expect("shared MCC config should build through pnml-tools");

    assert_eq!(config.workers(), 4);
    assert_eq!(
        config.storage_mode(),
        pnml_tools::explorer::StorageMode::FingerprintOnly
    );
    assert_eq!(
        config.fpset_backend(),
        pnml_tools::explorer::FpsetBackend::Cas
    );
}
...
fn test_examination_record_to_mcc_line_state_space() {
    ...
    assert_eq!(lines[1], "STATE_SPACE TRANSITIONS 250 TECHNIQUES EXPLICIT");
    assert_eq!(
        lines[2],
    ...
    assert_eq!(
        lines[3],
        "STATE_SPACE MAX_TOKEN_PER_MARKING 12 TECHNIQUES EXPLICIT"
    );
}
...
// over tla-petri. The functionality is now in `ty mcc --simplify-report`.
