...
fn bug_3148_voteproof_real_spec_passes_with_tlc_state_parity() {
    ...
    // The regression only needs success plus TLC state-count parity. Avoid
    // full-state trace storage for this real-spec canary; on current dev
    // builds it pushes VoteProof over the 120s test budget without adding
    // coverage for #3148.
    checker.set_store_states(false);
    // Hermetic: this canary asserts TLC's UNREDUCED 6,962-state baseline.
    // Auto-symmetry (ON by default) soundly orbit-reduces Value/Acceptor
    // (Quorum is permutation-invariant; the executable spec is CHOOSE-free
    // over both sets) to 705 states; disable it so the #3148 evaluator-path
    // assertion stays an unreduced TLC-parity check.
