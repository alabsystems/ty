...
/// states. Before the fix, TY would count violating states but NOT explore their
...
///
/// TLC baseline: `java -jar ~/tlaplus/tytools.jar -workers 1 -continue`
...
"#;
    ...
    match &result {
        CheckResult::InvariantViolation { stats, .. } => {
            ...
            // TY --continue-on-error: must also find 6 states
            assert_eq!(
...
"#;
    ...
    match &result {
        CheckResult::InvariantViolation { stats, .. } => {
            // TLC -continue: 6 distinct states found
            // TY parallel --continue-on-error: must also find 6 states
            assert_eq!(
                stats.states_found, 6,
