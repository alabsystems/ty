...
fn run_check(src: &str, config: &Config, bytecode_mode: BytecodeMode) -> CheckResult {
    let _bytecode_guard = match bytecode_mode {
        BytecodeMode::Default => common::EnvVarGuard::remove("TY_BYTECODE_VM"),
        BytecodeMode::Disabled => common::EnvVarGuard::set("TY_BYTECODE_VM", Some("0")),
        BytecodeMode::Enabled => common::EnvVarGuard::set("TY_BYTECODE_VM", Some("1")),
    };
...
fn assert_default_bytecode_matches_enabled(name: &str, src: &str, config: &Config) {
    ...
    assert_eq!(
        ...
        "{name}: unset TY_BYTECODE_VM state count ({default_states}) != explicit enable ({enabled_states})"
    );
}
...
"#,
    ...
    // API-surface gate: keep the `CompileError` type nameable through the
    // public bytecode API (the binding itself is a no-op).
    let _: Option<CompileError> = None;
