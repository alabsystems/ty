...
fn bug_3149_acp_nb_tlc_ac4_alt_passes_with_tlc_state_parity() {
    ...
    // Unreduced parity axis: with auto-symmetry pinned off, the sequential
    // checker must match TLC's raw 4,284 states (the original #3149 pin).
    ...
    // Production default: AC4_alt is safety-only, so auto-symmetry engages on
    // `participants = {p0, p1}` (the cfg's own commented-out `SYMMETRY Perms`).
    // TLC with `SYMMETRY Perms` and PROPERTIES AC4_alt reports exactly 2,199
    // distinct states (cross-checked 2026-06-10) — the orbit-reduced count
    // must match it.
    let mut checker_sym = ModelChecker::new_with_extends(&module, &checker_module_refs, &config);
    checker_sym.set_store_states(true);
    checker_sym.set_auto_symmetry(true);
    checker_sym.set_fairness(fairness.clone());
    checker_sym.set_stuttering_allowed(stuttering_allowed);
    assert_acp_passes(
        checker_sym.check(),
        "sequential checker (auto-symmetry)",
        2_199,
    );
