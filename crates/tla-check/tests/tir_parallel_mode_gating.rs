...
fn assert_parallel_next_only_tir_eval_reports_probe_hit() {
    let _guard = EnvVarGuard::set("TY_TIR_EVAL", Some("StepProbe3285Next"));
...
fn assert_parallel_constrained_next_only_tir_eval_reports_probe_hit() {
    let _guard = EnvVarGuard::set("TY_TIR_EVAL", Some("StepProbe3285ConstrainedNext"));
    clear_for_test_reset();
    clear_global_name_interner();
    enable_tir_eval_probe();
    let before = tir_eval_probe_snapshot();
...
fn assert_parallel_resolved_next_selection_reports_probe_hit() {
    let _guard = EnvVarGuard::set("TY_TIR_EVAL", Some("StepResolved3285Impl"));
    clear_for_test_reset();
    clear_global_name_interner();
    enable_tir_eval_probe();
    let before = tir_eval_probe_snapshot();
...
fn assert_parallel_all_tir_eval_reports_probe_hit() {
    ...
    match checker.check() {
        ...
        other => panic!("expected direct parallel TY_TIR_EVAL=all to succeed, got: {other:?}"),
    }
    ...
    assert!(
        delta.expr_evals > 0,
        "expected direct parallel TY_TIR_EVAL=all to hit the expr probe, delta={delta:?}"
    );
    assert_eq!(
        delta.named_op_evals, 0,
        "direct parallel TY_TIR_EVAL=all leaf TIR should not require named-op replay, delta={delta:?}"
    );
}
...
fn assert_parallel_binding_form_next_uses_tir() {
    let _guard = EnvVarGuard::set("TY_TIR_EVAL", Some("Next"));
    clear_for_test_reset();
    clear_global_name_interner();
    enable_tir_eval_probe();
    let before = tir_eval_probe_snapshot();
...
fn test_tir_modes_force_sequential_or_fail_closed() {
    {
        let _guard = EnvVarGuard::set("TY_TIR_EVAL", Some("Valid"));
    ...
    assert_parallel_rejected_for_env("TY_TIR_EVAL", "Inv", "next-only TY_TIR_EVAL");
    assert_parallel_rejected_for_env("TY_TIR_EVAL", "Next,Inv", "next-only TY_TIR_EVAL");
    assert_parallel_rejected_for_env("TY_TIR_PARITY", "Inv", "TY_TIR_PARITY");
    ...
    // Adaptive still stays sequential for explicit TY_TIR_EVAL=all (#3324).
    {
        let _guard = EnvVarGuard::set("TY_TIR_EVAL", Some("all"));
        let module = parse_module(LARGE_SPEC);
        let config = Config {
        ...
        assert!(
            matches!(analysis.strategy, Strategy::Sequential),
            "TY_TIR_EVAL=all must still force sequential in adaptive mode, got {:?}",
            analysis.strategy
        );
        assert!(
            !matches!(result, CheckResult::Error { .. }),
            "adaptive TY_TIR_EVAL=all sequential forcing should succeed, got {result:?}"
        );
    }
}
