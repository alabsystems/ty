...
/// that TY produces exactly 374,727 states (matching TLC baseline).
///
...
\* TLC: fatal error. TY post-#1552: fatal error.
...
"#;
    ...
    // Post-#1552, TY propagates this fatally (no action-level suppression).
    match &result {
        CheckResult::Error { error, .. } => {
            let msg = format!("{}", error);
            assert!(
