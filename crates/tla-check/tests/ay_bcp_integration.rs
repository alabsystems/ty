...
//! AY BCP integration tests — verified BCP component drops into ay-sat.
//!
//! These tests model-check the BCP (Boolean Constraint Propagation / Unit
//! Propagation) TLA+ spec from `specs/ay/BCP.tla` and verify exact state
...
/// Returns the path to `specs/ay/` in the workspace root.
fn ay_spec_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../specs/ay")
        .canonicalize()
        .expect("specs/ay directory should exist")
}

/// Load a ay spec from .tla + .cfg, returning everything needed for ModelChecker.
///
...
/// Run the model checker on a ay spec, returning (states_found, initial_states, transitions).
fn run_ay_spec(spec_name: &str, cfg_name: &str) -> (CheckResult, usize, usize, usize) {
    let (module, checker_modules, config, fairness, stuttering_allowed) =
        load_ay_spec(spec_name, cfg_name);
...
#[test]
fn ay_bcp_3var_sat_state_count_matches_tlc() {
    let (result, states_found, initial_states, transitions) = run_ay_spec("MCBCP", "MCBCP");
...
#[cfg_attr(test, ntest::timeout(30000))]
#[test]
fn ay_bcp_3var_sat_no_deadlock_check() {
    let (result, states_found, _initial, _transitions) = run_ay_spec("MCBCP", "MCBCP_noDeadlock");
...
#[test]
fn ay_bcp_pigeonhole_unsat_state_count_matches_tlc() {
    let (result, states_found, initial_states, transitions) =
        run_ay_spec("MCBCP_Pigeon", "MCBCP_Pigeon");
...
#[cfg_attr(test, ntest::timeout(30000))]
#[test]
fn ay_bcp_all_four_invariants_verified() {
    ...
    let (_module, _checker_modules, config, _fairness, _stuttering) =
        load_ay_spec("MCBCP_Pigeon", "MCBCP_Pigeon");
    assert_eq!(
        config.invariants.len(),
        4,
    ...
    let (result_3var, _, _, _) = run_ay_spec("MCBCP", "MCBCP");
    match &result_3var {
        CheckResult::Success(_) => {}
    ...
    let (result_pigeon, _, _, _) = run_ay_spec("MCBCP_Pigeon", "MCBCP_Pigeon");
    match &result_pigeon {
        CheckResult::Success(_) => {}
        CheckResult::InvariantViolation { invariant, .. } => {
...
// This ensures the spec structure matches what the ay-sat solver requires.

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn ay_bcp_spec_structure_has_expected_operators() {
    let spec_dir = ay_spec_dir();
