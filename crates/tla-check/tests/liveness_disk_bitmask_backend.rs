...
fn check_liveness(src: &str, property: &str, use_disk_bitmasks: bool) -> CheckResult {
    ...
    let _disk_successor_guard = common::EnvVarGuard::remove("TY_LIVENESS_DISK_SUCCESSORS");
    let _disk_bitmask_env_guard = common::EnvVarGuard::remove("TY_LIVENESS_DISK_BITMASKS");
    let _disk_bitmask_flush_threshold_guard =
        common::EnvVarGuard::remove("TY_LIVENESS_DISK_BITMASK_FLUSH_THRESHOLD");
