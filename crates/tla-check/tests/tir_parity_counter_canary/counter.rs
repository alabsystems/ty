...
fn test_tir_parity_counter_canary() {
    let _guard = EnvVarGuard::set("TY_TIR_PARITY", Some("Init,Next"));
...
/// when `TY_TIR_EVAL` selects `Init`. The binding-form gate in
...
fn test_tir_eval_counter_renamed_boot_only() {
    let _guard = EnvVarGuard::set("TY_TIR_EVAL", Some("Boot"));
