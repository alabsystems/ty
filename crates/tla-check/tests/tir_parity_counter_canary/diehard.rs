...
fn test_tir_parity_diehard() {
    let _guard = EnvVarGuard::set("TY_TIR_PARITY", Some("Init,Next,TypeOK"));
...
/// this test uses `TY_TIR_EVAL` to make TIR the PRIMARY evaluator for
...
fn test_tir_eval_diehard_invariant() {
    let _guard = EnvVarGuard::set("TY_TIR_EVAL", Some("TypeOK"));
    let module = parse_module(DIEHARD_SPEC);
    let config = Config::parse("INIT Init\nNEXT Next\nINVARIANT TypeOK\n").expect("config parse");
...
///
/// This exercises the real-spec successor path with `TY_TIR_EVAL=Next`,
...
fn test_tir_eval_diehard_init_only() {
    let _guard = EnvVarGuard::set("TY_TIR_EVAL", Some("Init"));
    let module = parse_module(DIEHARD_SPEC);
    let config = Config::parse("INIT Init\nNEXT Next\nINVARIANT TypeOK\n").expect("config parse");
...
fn test_tir_eval_diehard_next_and_typeok() {
    let _guard = EnvVarGuard::set("TY_TIR_EVAL", Some("Next,TypeOK"));
    let module = parse_module(DIEHARD_SPEC);
    let config = Config::parse("INIT Init\nNEXT Next\nINVARIANT TypeOK\n").expect("config parse");
...
fn test_tir_eval_rich_invariant() {
    let _guard = EnvVarGuard::set("TY_TIR_EVAL", Some("Inv"));
...
fn test_tir_parity_rich_canary() {
    let _guard = EnvVarGuard::set("TY_TIR_PARITY", Some("Init,Next,Inv"));
    let module = parse_module(RICH_SPEC);
    let config = Config::parse("INIT Init\nNEXT Next\nINVARIANT Inv\n").expect("config parse");
