...
fn test_tir_parity_constraints_action_constraints_and_terminal() {
    let _guard = EnvVarGuard::set("TY_TIR_PARITY", Some("Bound,ActionOk,Done"));
...
fn test_tir_eval_constraints_action_constraints_and_terminal() {
    let _guard = EnvVarGuard::set("TY_TIR_EVAL", Some("Bound,ActionOk,Done"));
    let module = parse_module(CONSTRAINT_TERMINAL_SPEC);
    let config = constraint_terminal_config();
...
fn test_tir_eval_lambda_round_trip() {
    let _guard = EnvVarGuard::set("TY_TIR_EVAL", Some("Init,Next,TypeOK"));
    tla_check::eval::clear_for_test_reset();
    tla_core::clear_global_name_interner();
...
fn test_tir_eval_multi_action_except_regression_3276() {
    ...
    let baseline_states = {
        let _no_tir = EnvVarGuard::remove("TY_TIR_EVAL");
        let _no_parity = EnvVarGuard::remove("TY_TIR_PARITY");
    ...
    {
        let _tir = EnvVarGuard::set("TY_TIR_EVAL", Some("Next"));
    ...
    {
        let _tir = EnvVarGuard::set("TY_TIR_EVAL", Some("all"));
...
fn test_tir_default_on_fires_probes_without_env_vars() {
    let _eval_guard = EnvVarGuard::remove("TY_TIR_EVAL");
