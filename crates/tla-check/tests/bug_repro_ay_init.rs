...
//! ay enumeration, FunAsSeq, prime quantifier - Bugs #349, #374, #523, #633
//!
...
"#;
    ...
    match &result {
        CheckResult::Success(stats) => {
            ...
            // TY may or may not count self-loops as transitions, so just verify > 0
            assert!(
                stats.transitions >= 1,
...
// ============================================================================
// Bug #523: ay Init enumeration unsound for heterogeneous SetEnum
// ============================================================================

/// Bug #523: Heterogeneous SetEnum must not under-enumerate
///
/// ay-based Init enumeration must either:
...
fn test_bug_523_heterogeneous_setenum_no_underenumeration() {
    // Set env var to force ay enumeration attempt
    // (The ay path should refuse heterogeneous sets and fall back)
    let _guard = common::EnvVarGuard::set("TY_FORCE_AY", Some("1"));
...
"#;
    ...
    match &result {
        CheckResult::Success(stats) => {
            ...
            // If only 1 state is found, ay is under-enumerating (soundness bug!)
            assert_eq!(
                ...
                "Bug #523 regression! Heterogeneous SetEnum must enumerate BOTH states. \
                 ...
                 This indicates ay is under-enumerating.",
...
// ============================================================================
// Feature #633: ay Init enumeration smoke test
// ============================================================================

/// Feature #633: Smoke test for ay-based Init enumeration
///
/// This test verifies that the ay feature correctly enumerates initial states
/// for a simple spec with multiple integer-valued Init states.
///
/// Purpose: Fast verification that ay dependency bumps don't break Init enumeration.
///
/// TLC baseline (verified 2026-01-28):
/// - Command: `java -jar tytools.jar -config AYSmokeTest.cfg AYSmokeTest.tla`
/// - Output: "5 distinct states generated" (initial), "6 distinct states found" (total)
///
/// Part of #633 (ay smoke test)
#[cfg(feature = "ay")]
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_feature_633_ay_init_enumeration_smoke() {
    // Force ay enumeration for this test
    let _guard = common::EnvVarGuard::set("TY_FORCE_AY", Some("1"));
...
\* Init with multiple integer values - should enumerate via ay
Init == x \in 1..5
...
"#;
    ...
    match &result {
        CheckResult::Success(stats) => {
            // ay should enumerate all 5 initial states (x=1,2,3,4,5)
            assert_eq!(
                stats.initial_states, 5,
                "ay smoke test: Expected 5 initial states, got {}. \
                 This may indicate ay enumeration is broken.",
                stats.initial_states
            );
            // Total states: x=1,2,3,4,5,6 = 6 states
            assert_eq!(
                stats.states_found, 6,
                "ay smoke test: Expected 6 total states, got {}",
                stats.states_found
            );
        }
        other => {
            panic!(
                "ay smoke test: Unexpected result {:?}. \
...
/// ay Init enumeration file-based test (Part of #634)
///
/// Tests ay enumeration using the file-based spec at test_specs/AYSmokeTest.tla.
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_feature_634_ay_init_file_based() {
    use std::fs;

    let _guard = common::EnvVarGuard::set("TY_FORCE_AY", Some("1"));

    // Load spec from file
    let spec_path = "../../test_specs/AYSmokeTest.tla";
    ...
    match &result {
        CheckResult::Success(stats) => {
            ...
            assert_eq!(
                stats.initial_states, 5,
                "ay file test: Expected 5 initial states (TLC baseline), got {}",
                stats.initial_states
            );
            assert_eq!(
                stats.states_found, 6,
                "ay file test: Expected 6 total states (TLC baseline), got {}",
                stats.states_found
            );
        }
        other => {
            panic!(
                "ay file test: Unexpected result {:?}. \
