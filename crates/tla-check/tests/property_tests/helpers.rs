...
impl EnvVarGuard {
    pub fn set(key: &str, value: Option<&str>) -> Self {
        ...
        if let Some(v) = value {
            tla_check::env_guard::set_var(key, v);
        } else {
            tla_check::env_guard::remove_var(key);
        }
        Self {
            key: key.to_string(),
            prev,
...
impl Drop for EnvVarGuard {
    fn drop(&mut self) {
        match self.prev.take() {
            Some(v) => tla_check::env_guard::set_var(&self.key, v),
            None => tla_check::env_guard::remove_var(&self.key),
        }
        self.lock.take();
    }
}
...
pub fn eval_str(src: &str) -> Result<Value, String> {
    // Each call parses, evaluates, and DROPS an independent AST. Several eval
    // caches (e.g. the constant-set cache) are keyed by raw AST-node pointer
    // address and rely on the documented run-boundary reset contract; without a
    // reset between calls the allocator can reuse a freed node's address and the
    // cache replays a STALE value from a prior, structurally-different eval.
    // Treat each call as a fresh run (matching production's run/phase clears).
    //
    // Use the thread-local-only reset, NOT `reset_global_state()`: this binary
    // runs its tests in parallel and `reset_global_state()` (compiled here in
    // non-test mode) clears the global name interner unconditionally, which
    // invalidates `NameId`s held by a sibling test thread mid-eval and panics
    // with an index-out-of-bounds. The thread-local reset honors the pointer-
    // keyed-cache contract without ever touching shared interners.
...
pub fn unique_temp_path(suffix: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "ty_property_{pid}_{nanos}.{suffix}",
        pid = std::process::id(),
        nanos = now.as_nanos()
    ))
}
...
pub fn eval_with_extends(src: &str, extends: &[&str]) -> Result<Value, String> {
    // See eval_str: reset eval caches so each independently-parsed AST is a
    // fresh run and cannot replay a stale pointer-keyed cache entry. The
    // thread-local-only reset never clears shared interners, so it is safe under
    // this binary's parallel test execution (see eval_str for the full rationale).
    tla_check::clear_thread_local_eval_caches();
