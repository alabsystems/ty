...
fn eval_tlcext_str(src: &str) -> Result<Value, String> {
    // Run-boundary reset (see helpers::eval_str): clears pointer-keyed eval
    // caches so a freed AST node from a prior call can't alias a stale entry.
    // Only the fresh-ctx entry resets; `_with_ctx` keeps its caller's ctx (and
    // any state it set up, e.g. CounterExample) intact.
