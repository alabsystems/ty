...
fn eval_fsext_str(src: &str) -> Result<Value, String> {
    // Run-boundary reset (see helpers::eval_str): each call re-parses an
    // independent module at FileId(0) with `Op` at the same offset, so without
    // clearing the pointer-keyed eval caches a freed AST node from a prior call
    // can alias a stale cache entry and replay the wrong value.
...
fn eval_finitesetsext_str(src: &str) -> Result<Value, String> {
    // Run-boundary reset: see eval_fsext_str.
    tla_check::clear_thread_local_eval_caches();
    let module_src = format!(
