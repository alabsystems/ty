...
use tla_check::{check_module_parallel, CheckResult, Config, ConstantValue, ModelChecker};
...
fn test_bug_2566_recursive_operators_no_oom() {
    ...
    // Verified against real TLC (tla2tools.jar, -workers 1, cfg with no SYMMETRY
    // declaration): "4 distinct states found, No error has been found."
    //
    // This test exercises the RECURSIVE-operator eval path; its premise is the
    // raw distinct-state count. The model values `ADDRS = {a1, a2}` form a
    // symmetry group that TY's auto-symmetry feature would detect (the spec is
    // safety-only), collapsing the two cross-swapped functions
    // ({a1->1,a2->2} and {a1->2,a2->1}) into one orbit representative and
    // reporting 3. That reduction is verdict-preserving but changes the count,
    // diverging from the TLC baseline that has no SYMMETRY set. Pin auto-symmetry
    // off so the count matches TLC, per the documented convention for tests that
    // assert raw distinct-state counts (use `set_auto_symmetry`, never the
    // process-global `TY_AUTO_SYMMETRY` env var). The parallel checker does not
    // apply auto-symmetry, so its 4-state count already matches the baseline.
    ...
    check("sequential", checker.check());
...
/// Bug #2822: TY raised "Type error: expected Record, got ModelValue" when
...
"#;
    ...
    // TLC baseline: 4 states (verified against tla2tools.jar, no SYMMETRY in cfg).
    //
    // `Nodes` is a 2-element model-value set, a symmetry group that TY's
    // auto-symmetry would detect on this safety-only spec, orbit-reducing the
    // count to 3. Pin auto-symmetry off so the raw distinct-state count matches
    // the TLC baseline (use `set_auto_symmetry`, never the process-global
    // `TY_AUTO_SYMMETRY` env var).
    let mut checker = ModelChecker::new(&module, &config);
    checker.set_auto_symmetry(false);
    let result = checker.check();
    match &result {
...
"#;
    ...
    // Total: 2^2 = 4 states.
    // TLC baseline: 4 states (no SYMMETRY declared in cfg).
    //
    // `Keys` is a 2-element model-value set, a symmetry group that TY's
    // auto-symmetry would detect on this safety-only spec, orbit-reducing the
    // count to 3. Pin auto-symmetry off so the raw distinct-state count matches
    // the TLC baseline (use `set_auto_symmetry`, never the process-global
    // `TY_AUTO_SYMMETRY` env var).
