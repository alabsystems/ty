...
//! Regression test for #3402: `TY_TIR_EVAL=PCNext` must preserve the
...
fn load_paxoscommit() -> (
    ...
    // This fixture pins the raw depth-10 state count. Auto-POR may soundly
    // reduce the explored graph, so keep the count assertion on the unreduced path.
...
fn run_paxoscommit_depth_10(
    ...
    // Like auto-POR above: this fixture pins the RAW depth-10 state count, so
    // auto-symmetry's sound orbit reduction must not engage. (PaxosCommit with
    // SYMMETRY also currently trips a pre-existing, timing-dependent eval-cache
    // bug — `expected Set, got Int` at the Decide LET — which reproduces on
    // baseline binaries with declared SYMMETRY and is orthogonal to the
    // auto-detection path.)
...
/// Part of #3354: `TY_LEGACY_EVAL` removed. This function now runs the
...
fn run_baseline_paxoscommit_depth_10(
    ...
    let baseline_stats = {
        let _tir_eval = EnvVarGuard::remove("TY_TIR_EVAL");
        let _tir_parity = EnvVarGuard::remove("TY_TIR_PARITY");
        let _tir_leaf_parity = EnvVarGuard::remove("TY_TIR_LEAF_PARITY");
...
fn run_tir_paxoscommit_depth_10(
    ...
    match tir_eval {
        Some(mode) => {
            let _tir_eval = EnvVarGuard::set("TY_TIR_EVAL", Some(mode));
            let _tir_parity = EnvVarGuard::remove("TY_TIR_PARITY");
            let _tir_leaf_parity = EnvVarGuard::remove("TY_TIR_LEAF_PARITY");
            run_paxoscommit_depth_10(
                module,
...
#[cfg_attr(test, ntest::timeout(360000))]
#[test]
fn bug_3402_paxoscommit_tir_pcnext_matches_legacy_depth_count_and_stays_stable() {
    ...
    for (mode_label, tir_eval) in [("default TIR", None), ("PCNext", Some("PCNext"))] {
        for run_idx in 1..=2 {
            ...
            let label =
                format!("Bug #3402 regression: {mode_label} PaxosCommit depth-10 run {run_idx}");
            assert_depth_10_stats_match(&legacy_stats, &stats, &label);
            assert_eq!(
                stats.states_found, 52_833,
                "Bug #3402 regression: {mode_label} run {run_idx} should match TLC/legacy"
            );
        }
    }
}
