...
/// contains `Serializable'`), TY was not validating it because:
...
///
/// Without the fix: TY generates states with self-loops (invariant violated)
/// With the fix: TY correctly filters self-loops (no invariant violation)
...
"#;
    ...
    match &result {
        ...
        CheckResult::InvariantViolation { invariant, .. } => {
            panic!(
                "Bug exists! Invariant {} violated - TY is generating states with self-loops",
                invariant
            );
        }
