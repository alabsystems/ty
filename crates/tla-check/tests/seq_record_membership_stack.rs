...
const CHILD_ENV: &str = "TY_SEQ_RECORD_MEMBERSHIP_CHILD";
...
fn seq_record_membership_child() {
    ...
    let stack_size = std::env::var("TY_SEQ_RECORD_MEMBERSHIP_STACK_SIZE_BYTES")
        .ok()
...
fn seq_record_membership_does_not_abort_on_small_stack() {
    ...
    let output = Command::new(exe)
        .env(CHILD_ENV, "1")
        .env(
            "TY_SEQ_RECORD_MEMBERSHIP_STACK_SIZE_BYTES",
            (1024 * 1024).to_string(),
        )
        .arg("--exact")
