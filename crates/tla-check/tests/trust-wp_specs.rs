...
//! Integration tests for trust-wp repo TLA+ specs (#2830).
//!
//! These are real-world specs from the trust-wp infrastructure tool that exercise
...
/// Resolve the workspace-root-relative `tests/specs/trust-wp/` directory.
fn trust_wp_spec_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/specs/trust-wp")
        .canonicalize()
        .expect("tests/specs/trust-wp directory should exist")
}

/// Load a trust-wp spec from .tla + .cfg, returning everything needed for ModelChecker.
fn load_trust_wp_spec(
    name: &str,
) -> (
    tla_core::ast::Module,
    ...
    let spec_dir = trust_wp_spec_dir();
...
fn run_and_assert_pass(name: &str, expected_states: usize) {
    let (module, checker_modules, config, fairness, stuttering_allowed) = load_trust_wp_spec(name);
    ...
    match checker.check() {
        CheckResult::Success(stats) => {
            assert_eq!(
                ...
                "trust-wp/{name}: expected {expected_states} states (TLC baseline), got {}",
                stats.states_found
            );
        }
        CheckResult::LivenessViolation { property, .. } => {
            panic!("trust-wp/{name}: unexpected liveness violation for {property}");
        }
        other => panic!("trust-wp/{name}: expected pass, got {other:?}"),
    }
}
    ...
    match checker.check() {
        CheckResult::InvariantViolation { invariant, .. } => {
            assert_eq!(
                ...
                "trust-wp/{name}: expected invariant violation for {expected_invariant}, got {invariant}"
            );
        }
        CheckResult::Error { error, .. } => {
            ...
            assert!(
                ...
                "trust-wp/{name}: expected invariant violation, got: {error_str}"
            );
        }
        CheckResult::Success(stats) => {
            panic!(
                "trust-wp/{name}: expected invariant violation but passed with {} states",
                stats.states_found
            );
        }
        other => panic!("trust-wp/{name}: expected invariant violation, got {other:?}"),
    }
}
...
#[test]
fn trust_wp_iteration_tags_passes_with_tlc_parity() {
...
#[cfg_attr(test, ntest::timeout(30000))]
#[test]
fn trust_wp_iteration_tags_with_commit_failure_passes_with_tlc_parity() {
...
#[cfg_attr(test, ntest::timeout(30000))]
#[test]
fn trust_wp_cargo_lock_with_toctou_finds_mutual_exclusion_violation() {
...
#[test]
fn trust_wp_cargo_lock_passes_with_tlc_parity() {
