...
// Part of #t1-instance-pinning: AST-only INSTANCE/ModuleRef body resolver, used
// by the liveness static prime-pinning proof (`liveness::enabled_eval`) to look
// through `Instance!Op` action leaves.
pub(crate) use module_ref::resolve_module_ref_body_ast;
// Part of #guard-moduleref: the `Named`-target arm of the same resolver, with
// the resolved operator's formal parameter names — used by the ENABLED
// guard-prefix extraction (`liveness::enabled_provenance`).
pub(crate) use module_ref::resolve_named_module_ref_body_ast_with_params;
