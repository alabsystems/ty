...
fn handle_let(
    ...
    if has_parameterized {
        ...
        // Eager id (one HAMT walk) instead of INVALIDATED. NOTE: only the
        // parameterized subset is inserted here, so the LET-block span memo
        // (which keys on the full def set) must NOT be used — compute the id of
        // the actual map directly.
        inner_ctx.set_local_ops_eager(Arc::new(local_ops));
    }
