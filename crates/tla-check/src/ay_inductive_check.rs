...
impl From<AYError> for InductiveCheckError {
    fn from(err: AYError) -> Self {
        match err {
            AYError::Solver(inner) => InductiveCheckError::SolverFailed(inner.to_string()),
...
impl InductiveCheckConfig {
    /// Creates a config for the named invariant with default settings.
    ///
    /// The per-solve timeout defaults to 300 seconds, overridable via the
    /// `TY_INDUCTIVE_CHECK_TIMEOUT_SECS` environment variable; debug logging
    /// follows the crate's inductive-check debug flag.
    pub fn new(invariant: String) -> Self {
        let timeout_secs: u64 = std::env::var("TY_INDUCTIVE_CHECK_TIMEOUT_SECS")
            .ok()
...
debug_flag!(debug_inductive_check_enabled, "TY_DEBUG_INDUCTIVE_CHECK");
...
pub fn check_inductive(
    ...
    let inv_expr = ay_shared::get_operator_body(&symbolic_ctx, inv_resolved)
    ...
    let var_sorts = ay_shared::infer_var_sorts(&vars, &init_expanded, &inv_names, &symbolic_ctx);
