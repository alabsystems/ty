...
pub fn bind_constants_from_config(ctx: &mut EvalCtx, config: &Config) -> Result<(), EvalError> {
    ...
    for name in names {
        ...
        let value = match const_val {
            ConstantValue::Value(value_str) => match parse_constant_value(value_str) {
                Ok(v) => v,
                Err(lit_err) => {
                    // The lightweight literal parser only handles a fixed grammar
                    // (ints, strings, sets, tuples, records, model values). Fall back
                    // to the full TLA+ parser + evaluator so CASE expressions, function
                    // literals `[x \in S |-> e]`, IF/THEN/ELSE, arithmetic, and operator
                    // applications all work. Earlier constants (in config order) and the
                    // loaded module's operators are already in scope on `ctx`, so the
                    // expression can reference model values and definitions it depends on.
                    eval_constant_value_expr(ctx, value_str).map_err(|expr_err| {
                        EvalError::Internal {
                            message: format!(
                                "Error parsing CONSTANT {name}: could not parse '{value_str}' as a \
                                 literal ({lit_err}) nor evaluate it as a TLA+ expression ({expr_err})"
                            ),
                            span: None,
                        }
                    })?
...
/// Evaluate a CONSTANT value string as a full TLA+ expression.
///
/// Used as the fallback when [`parse_constant_value`] (the lightweight literal
/// parser) cannot handle the value — e.g. a CASE expression, a function literal
/// `[x \in S |-> e]`, an `IF/THEN/ELSE`, arithmetic, or an operator application.
///
/// The value is wrapped in a synthetic single-operator module, parsed and lowered
/// with the real TLA+ front-end, and its body is evaluated against `ctx`. Because
/// constants are bound in config-file order, any constant/model value the
/// expression depends on (declared earlier in the config) is already bound on
/// `ctx`, and operators from the loaded spec module are in scope too.
fn eval_constant_value_expr(ctx: &EvalCtx, value_str: &str) -> Result<Value, String> {
    // The operator name is deliberately namespaced so it cannot shadow or be
    // shadowed by a real spec definition during lowering.
    let src =
        format!("---- MODULE TyConstEval ----\n__ty_const_eval_result == {value_str}\n====\n");
    let tree = parse_to_syntax_tree(&src);
    let lowered = lower(FileId(0), &tree);
    let Some(module) = lowered.module else {
        return Err(format!(
            "value is not a valid TLA+ expression ({} parse error(s))",
            lowered.errors.len()
        ));
    };

    let body = module.units.iter().find_map(|unit| match &unit.node {
        Unit::Operator(op) if op.name.node == "__ty_const_eval_result" => Some(&op.body),
        _ => None,
    });
    ...
    eval_entry(ctx, body).map_err(|e| e.to_string())
}
