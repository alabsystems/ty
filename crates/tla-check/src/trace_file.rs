...
impl TraceFile {
    ...
    pub fn create_temp() -> io::Result<Self> {
        ...
        let filename = format!("ty_trace_{}_{}.st", std::process::id(), counter);
    ...
    /// Write a batch of successor state records, stopping at the first I/O error.
    ///
    /// Returns the locations for the successfully written prefix plus the first
    /// error, if any. Callers that treat trace writes as best-effort can keep
    /// using the prefix locations while marking the trace degraded.
    pub(crate) fn write_states_batch_until_error<I>(
        &mut self,
        records: I,
    ) -> (Vec<u64>, Option<io::Error>)
    where
        I: IntoIterator<Item = (u64, Fingerprint)>,
    {
        let iter = records.into_iter();
        let (lower_bound, _) = iter.size_hint();
        let mut locations = Vec::with_capacity(lower_bound);

        for (predecessor_loc, fingerprint) in iter {
            match self.write_state(predecessor_loc, fingerprint) {
                Ok(loc) => locations.push(loc),
                Err(error) => return (locations, Some(error)),
            }
        }
