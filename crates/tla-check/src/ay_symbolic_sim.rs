...
//! AY-based symbolic simulation for TLA+ specs (Apalache Gap 9).
//!
//! Symbolic simulation explores state trajectories using ay SMT solving to
...
impl Default for SymbolicSimConfig {
    fn default() -> Self {
        let timeout_secs: u64 = std::env::var("TY_SYMSIM_TIMEOUT_SECS")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(300);

        let num_runs: usize = std::env::var("TY_SYMSIM_RUNS")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(10);

        let max_depth: usize = std::env::var("TY_SYMSIM_DEPTH")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(100);
        ...
        Self {
            ...
            debug: debug_ay_symsim_enabled(),
        }
    }
}

debug_flag!(debug_ay_symsim_enabled, "TY_DEBUG_AY_SYMSIM");
...
impl From<AYError> for SymbolicSimError {
    fn from(err: AYError) -> Self {
        match err {
            AYError::Solver(inner) => SymbolicSimError::SolverFailed(inner.to_string()),
...
fn run_single_simulation(
    ...
    let init_state = {
        let mut translator = ay_shared::make_translator(var_sorts, 0)?;
    ...
    if config.debug {
        eprintln!(
            "[ay-symsim] run {}: init state has {} vars",
            run_index,
    ...
    {
        let mut checker = ay_shared::make_translator(var_sorts, 0)?;
    ...
    for depth in 0..config.max_depth {
        ...
        let mut translator = ay_shared::make_translator(var_sorts, 1)?;
        translator.set_timeout(config.solve_timeout);
        for (name, sort) in var_sorts {
            translator.declare_var(name, sort.clone())?;
        }
        ...
        match translator.try_check_sat()? {
            ...
            SolveResult::Unsat(_) => {
                ...
                if config.debug {
                    eprintln!("[ay-symsim] run {}: deadlock at depth {}", run_index, depth);
                }
        ...
        // Check if the new state violates safety.
        {
            ...
            if let SolveResult::Sat = checker.try_check_sat()? {
                if config.debug {
                    eprintln!(
                        "[ay-symsim] run {}: violation at depth {}",
                        run_index,
                        depth + 1
                    );
                }
        ...
        if config.debug && (depth + 1) % 10 == 0 {
            eprintln!("[ay-symsim] run {}: reached depth {}", run_index, depth + 1);
        }
    }
...
pub fn symbolic_simulate(
    ...
    let symbolic_ctx =
        ay_shared::symbolic_ctx_with_config(ctx, config).map_err(SymbolicSimError::MissingSpec)?;
    ...
    if sim_config.debug {
        eprintln!(
            "[ay-symsim] {} vars, {} runs, max depth {}, {} invariants",
            var_sorts.len(),
            sim_config.num_runs,
    ...
    for run_index in 0..sim_config.num_runs {
        ...
        match result {
            SingleRunResult::Violation { depth, trace } => {
                if sim_config.debug {
                    eprintln!(
                        "[ay-symsim] violation found in run {} at depth {}",
                        run_index, depth
                    );
                }
            ...
            SingleRunResult::Deadlock { depth, trace: _ } => {
                ...
                if sim_config.debug {
                    eprintln!(
                        "[ay-symsim] run {} deadlocked at depth {}",
                        run_index, depth
                    );
                }
            }
            SingleRunResult::SolverInconclusive { depth, reason } => {
                ...
                if sim_config.debug {
                    eprintln!(
                        "[ay-symsim] run {} inconclusive at depth {}: {}",
