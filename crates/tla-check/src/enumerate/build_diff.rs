...
pub fn build_successor_diffs_from_array_into(
    ...
    let base_fp = current_array.cached_fingerprint();
    let _ = enumerate_diff_changes_only(&choices, 0, &mut changes, &mut |changes| {
        ...
        // emission for any active frame. `Multi` (InSet) entries may equal the
        // base value, so state-change is decided by exact per-value comparison
        // (the same test `diff_is_witness` uses). #frame-fp-pop: the third
        // closure supplies the emitted successor's fingerprint (computed
        // lazily, only when a FALSE-eligible frame record is open) via the
        ...
        crate::liveness::enabled_provenance::note_emission(
            ...
            || {
                changes
                    .iter()
                    .any(|(idx, v)| current_array.get(*idx) != **v)
            },
            || {
