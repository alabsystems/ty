...
"#;
    ...
    /// TELESCOPING-DOMAIN capture: TLA+ scopes each earlier bound var into all LATER domains — in
    /// `\E n \in 2..2, j \in 1..n : …`, the bound `n` is IN SCOPE inside `1..n`. A shadowed name
    /// (`LET n == 42`) must NOT be inlined into that later domain — the bound `n` shadows it. Before
    /// the telescoping fix, only the BODY was shadowed, so `1..n` was widened to `1..42` (capture).
    #[test]
    fn test_expand_operators_telescoping_later_domain_shadows_let() {
        let src = r#"
---- MODULE ExpandTelescoping ----
Expr == LET n == 42 IN \E n \in 2..2, j \in 1..n : j = 1
====
"#;
        let module = parse_module(src);
        let mut ctx = EvalCtx::new();
        ...
        // Navigate Let -> Exists and inspect the SECOND bound's domain `1..n`: it must still mention
        // the bound `n` (Ident), NOT have been widened to `1..42` by capturing the LET `n == 42`.
        fn second_domain_keeps_bound_n(e: &Expr) -> bool {
            match e {
                Expr::Let(_, body) => second_domain_keeps_bound_n(&body.node),
                Expr::Exists(bounds, _) => bounds
                    .get(1)
                    .and_then(|bv| bv.domain.as_ref())
                    .is_some_and(|d| crate::expr_visitor::expr_contains_ident_v(&d.node, "n")),
                _ => false,
            }
        }
        assert!(
            second_domain_keeps_bound_n(&expanded.node),
            "later domain `1..n` must keep the earlier bound `n`, not inline the LET `42`: {:?}",
            expanded.node
        );
    }
}
