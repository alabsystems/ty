...
//
// ALIASING FIX (2026-07-07): keys are (node pointer, source span), not bare
// pointers. `clear_expr_analysis_caches()` protects run BOUNDARIES, but
// WITHIN a run ephemeral AST trees are queried and freed (prepare-time
// expansions, per-call substituted bodies in the module-ref/symbolic paths),
// and the allocator recycles their addresses into later-allocated queried
// nodes — a stale bare-pointer hit then returns the WRONG predicate for an
// unrelated expression. Observed end-to-end: a recycled address made
// `expr_is_action_level` claim a plain bound-variable argument was
// action-level, routing `GuardW(c)` through call-by-name substitution whose
// capture-avoidance dropped the parameter mapping — nondeterministic
// `UndefinedVar` failures in `por_hidden_capture_dep_regression` (the
// nondeterminism seed is per-process std-HashMap ordering upstream of
// allocation order). The span check is the same belt-and-braces used by
// `subst_cache::EnumSubstCacheKey`: a recycled address can only collide when
// the new node also carries the SAME span, i.e. it is a clone of the same
// original source node. (Residual risk, accepted as in subst_cache: two
// same-span clones whose SUBSTITUTED children diverge in prime-ness; the
// injected children carry their own argument spans, so the colliding roots
// share lineage and the predicates agree in all but pathological cases.)
struct ExprAnalysisCaches {
    might_need_prime_binding: FxHashMap<(usize, tla_core::Span), bool>,
    expr_is_action_level: FxHashMap<(usize, tla_core::Span), bool>,
    ...
    is_operator_ref_guard_unsafe: FxHashMap<(usize, tla_core::Span), bool>,
}
...
/// Part of #3063: Cached per AST node (pointer, span) — result is deterministic
/// since AST and operator definitions are immutable during model checking; the
/// span validates recycled addresses (see `ExprAnalysisCaches`).
pub(super) fn expr_is_action_level(ctx: &EvalCtx, expr: &Spanned<Expr>) -> bool {
    let key = (&expr.node as *const Expr as usize, expr.span);
    EXPR_ANALYSIS_CACHES.with(|caches| {
        let mut c = caches.borrow_mut();
        if let Some(&result) = c.expr_is_action_level.get(&key) {
            return result;
        }
        let result = crate::expr_visitor::expr_is_action_level_v(ctx, &expr.node);
...
/// that was 18% of MultiPaxos runtime per profiling.
...
/// since AST and operator definitions are immutable during model checking; the
/// span validates recycled addresses (see `ExprAnalysisCaches`).
pub(super) fn might_need_prime_binding(ctx: &EvalCtx, expr: &Spanned<Expr>) -> bool {
    let key = (&expr.node as *const Expr as usize, expr.span);
    EXPR_ANALYSIS_CACHES.with(|caches| {
        let mut c = caches.borrow_mut();
        ...
        let result = crate::expr_visitor::might_need_prime_binding_v(ctx, &expr.node);
...
/// Delegated to generic visitor in expr_visitor.rs (Part of #1192 Wave 2).
/// Part of #3073: Cached per AST node (pointer, span) — result is deterministic
/// since AST and operator definitions are immutable during model checking; the
/// span validates recycled addresses (see `ExprAnalysisCaches`).
pub(super) fn is_operator_reference_guard_unsafe(ctx: &EvalCtx, expr: &Spanned<Expr>) -> bool {
    let key = (&expr.node as *const Expr as usize, expr.span);
    EXPR_ANALYSIS_CACHES.with(|caches| {
        let mut c = caches.borrow_mut();
        ...
        let result = crate::expr_visitor::is_operator_reference_guard_unsafe_v(ctx, &expr.node);
...
mod tests {
    ...
    fn test_clear_expr_analysis_caches_empties_all() {
        ...
        let dummy_span = tla_core::Span::dummy();
        EXPR_ANALYSIS_CACHES.with(|caches| {
            let mut c = caches.borrow_mut();
            c.expr_is_action_level.insert((0xDEAD, dummy_span), false);
            c.might_need_prime_binding
                .insert((0xBEEF, dummy_span), true);
            c.is_operator_ref_guard_unsafe
                .insert((0xCAFE, dummy_span), false);
        });
...
mod alias_tests {
    ...
    /// Regression (2026-07-07): the analysis caches were keyed by BARE node
    /// pointer. Within a run, an ephemeral queried expression could be freed
    /// and its address recycled by a DIFFERENT expression — the stale entry
    /// then returned the wrong predicate (observed as nondeterministic
    /// `UndefinedVar` failures in `por_hidden_capture_dep_regression`: a
    /// plain bound-variable argument aliased an action-level entry and was
    /// routed through capture-dropping call-by-name substitution whose
    /// capture-avoidance dropped the parameter mapping). Keys now include the
    /// source span, so a recycled address carrying a different span misses
    /// and recomputes.
    ///
    /// Deterministic pin (no allocator roulette): simulate the stale entry a
    /// bare-pointer key would have produced — poison the cache under the
    /// LIVE expression's ADDRESS but a DIFFERENT span (exactly what a freed
    /// action-level expr leaves behind when its address is recycled) — and
    /// assert the query does NOT consume it. A revert to bare-pointer keys
    /// makes the poisoned entry shadow the query (and does not compile with
    /// this key shape in the first place).
    #[cfg_attr(test, ntest::timeout(10000))]
    #[test]
    fn recycled_address_does_not_alias_action_level_cache() {
        clear_expr_analysis_caches();
        let ctx = EvalCtx::new();

        // Hermetic spans: FileId(u32::MAX) cannot collide with any real spec.
        let span_stale = Span::new(FileId(u32::MAX), 10, 20);
        let span_live = Span::new(FileId(u32::MAX), 30, 40);

        let live = Spanned::new(
            Expr::Ident("k".to_string(), tla_core::NameId::INVALID),
            span_live,
        );
        let addr = &live.node as *const Expr as usize;

        // The stale residue of a freed action-level expression that occupied
        // this address earlier (what a bare-pointer key would return).
        EXPR_ANALYSIS_CACHES.with(|caches| {
            caches
                .borrow_mut()
                .expr_is_action_level
                .insert((addr, span_stale), true);
        });

        assert!(
            !expr_is_action_level(&ctx, &live),
            "the poisoned same-address/different-span entry must NOT alias the live query"
        );

        // The live query is cached under its own (addr, span) and the stale
        // entry is untouched — both coexist, keyed apart by span.
        EXPR_ANALYSIS_CACHES.with(|caches| {
            let c = caches.borrow();
            assert_eq!(c.expr_is_action_level.get(&(addr, span_live)), Some(&false));
            assert_eq!(c.expr_is_action_level.get(&(addr, span_stale)), Some(&true));
        });
