...
use tla_core::{expr_mentions_name_v, get_primed_var_refs_spanned_v};
...
pub(super) fn try_let_guard_first_shortcircuit(
    ...
    if expr_is_action_level(ctx, first_conjunct) {
        return None;
    }
...
pub(super) fn conjunct_let(
    ...
    // Merged env (ambient + all LET defs) is memoized on Arc identity of the
    ...
    // Enter the LET scope with the memoized scope id instead of INVALIDATED, so
    // cache-key builds inside the body do not re-walk the merged HAMT.
    let saved_local_ops_id = ctx.enter_let_scope_premerged(merged, merged_id, merged_recursive);

    // A zero-arg LET referenced under prime must remain in local_ops so `a'`
    // evaluates the body of `a` in the next-state context.
        ...
        if def.params.is_empty() {
            ...
            match eval_leaf(ctx, &def.body, p.tir_leaf) {
                ...
                Err(e) => {
                    ...
                    ctx.restore_local_ops_with_id(saved_local_ops, saved_local_ops_id);
                    return Err(e);
                }
            }
        }
...
pub(super) fn conjunct_apply(
    ...
    if let Expr::Ident(op_name, _) = &op_expr.node {
        ...
        if let Some(def) = ctx.get_op(resolved_name) {
            ...
            // TRUE-only ENABLED provenance (#3208 redo of #3100): collect the
            // evaluated argument values (call-by-value only — the values ARE
            // the bindings the body is enumerated under) when this operator
            // definition is provenance-registered. A successor emitted while
            // this frame is live satisfies the operator body with exactly
            // these argument values, witnessing ENABLED of the matching
            // fairness action. `wants_frame` is a cheap thread-local gate that
            // is false whenever recording is inactive.
            let mut prov_args: Option<smallvec::SmallVec<[crate::Value; 4]>> =
            ...
            let result = if has_outer_scope_restore {
                ...
                let inner = Cont {
                    ...
                    scope_restore: Some(Rc::new(ScopeRestore {
                        ...
                        saved_local_ops_id: ctx.local_ops_scope_id(),
                        saved_skip_prime: skip_guard.prev(),
                        full_ctx_mark: None,
                    })),
                };
...
pub(super) fn conjunct_ident(
    ...
    if let Some(def) = ctx.get_op(resolved) {
        ...
        let def = Arc::clone(def);
        if def.params.is_empty() {
            // TRUE-only ENABLED provenance (#3208 redo of #3100): zero-arg
            // operator frame (e.g. `WF_vars(Schedule)`). See conjunct_apply.
            let _prov_frame =
...
pub(super) fn conjunct_module_ref(
    ...
    if m.certified_complete_action {
        let ModuleTarget::Named(registered_instance_name) = instance_name else {
            return Err(EvalError::Internal {
                message: "complete-action certificate reached a non-named module reference"
                    .to_string(),
                span: Some(conjunct.span),
            });
        };

        // Prepare the body with the evaluator's exact call-by-value INSTANCE
        // machinery. Actuals are evaluated in caller scope, lazy WITH
        // substitutions retain their definition-site chain, formals are
        // rebound after that chain is rebuilt, and instance-local operators
        // keep runtime precedence.
        let full_ctx_mark = ctx.mark_enum();
        let binding_mark = ctx.mark_stack();
        let saved_local_ops = ctx.local_ops().clone();
        let saved_local_ops_id = ctx.local_ops_scope_id();
        let saved_skip_prime = ctx.skip_prime_validation();
        let (instance_ctx, prepared_def) = tla_eval::prepare_registered_named_module_ref_call(
            ctx,
        ...
        *ctx = instance_ctx;
        ...
        // Always interpose the restore frame, even when the ModuleRef is the
        // terminal conjunct. Successor sinks can evaluate invariants inline;
        // they must observe the caller context, not instance-local operators,
        // substitutions, or formal bindings.
        let inner = Cont {
            conjuncts: &[],
            next_idx: 0,
            scope_restore: Some(Rc::new(ScopeRestore {
                ...
                saved_local_ops_id,
                saved_skip_prime,
                full_ctx_mark: Some(full_ctx_mark.clone()),
            })),
        };
        let result = enumerate_conjuncts(ctx, &inner, Some(&prepared_def.body), p, m);

        // This restores short-circuit/error exits which never reached the
        // frame. Re-restoring after a normal frame exit makes the helper
        // strictly scope-neutral on return.
        ctx.pop_to_enum_mark(&full_ctx_mark);
        return result;
    }
    ...
    let saved_local_ops = ctx.local_ops().clone();
    let saved_local_ops_id = ctx.local_ops_scope_id();
    // Eager id (one HAMT walk) instead of INVALIDATED (walk per lookup).
    ctx.set_local_ops_eager(Arc::new(merged_ops));
