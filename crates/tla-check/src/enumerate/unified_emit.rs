...
use super::{
    debug_enum_trace, is_action_level_error, is_guard_expression,
    is_operator_reference_guard_unsafe, is_speculative_eval_fallback, SymbolicAssignment,
};
...
pub(super) fn emit_successor(
    ...
    if !has_complex && !undo.is_empty() && p.full_mask != 0 {
        ...
        // TRUE-only ENABLED provenance (#3208 redo of #3100): note the RAW
        // emission for any active frame BEFORE the sink sees it (pre-dedup —
        // a successor later deduplicated away still witnesses ENABLED). On
        // this fast path `changes` holds only values that genuinely differ
        // from the base state, so emptiness decides state-change exactly.
        // #frame-fp-pop: the third closure supplies the successor fingerprint
        // (lazily; same incremental XOR routine the BFS worker uses).
...
pub(super) fn process_conjunct_guard_or_assignment(
    ...
    if is_operator_reference_guard_unsafe(ctx, conjunct) {
        if trace {
            eprintln!(
    ...
    match eval_result {
        ...
        // Fix #1467: only treat UndefinedVar/UndefinedOp as a speculative fallback
        // when the conjunct could actually be an *assignment* that
        // `is_primed_assignment` failed to detect — i.e. it syntactically contains a
        // prime (e.g. `f[x'] = 5`). For a PURE guard (no primes, and not an unsafe
        // operator reference — both already excluded above), an unresolved
        // operator/variable is a genuine spec error that must propagate fatally, not
        // be silently swallowed as guard=false. Otherwise a typo'd guard operator
        // would disable the action and under-explore the state space.
        Err(e) if is_speculative_eval_fallback(&e) && !is_guard_expression(&conjunct.node) => {
            if trace {
                eprintln!(
            ...
            // TLC handles these structurally; TY's speculative eval hits them at runtime.
