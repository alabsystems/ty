...
/// Lazy powerset domains must stay lazy in the conjunct-specialized successor
/// route. Eagerly collecting this domain would try to build 2^40 subsets
/// before the capped sink can stop enumeration.
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_capped_successor_enumeration_streams_powerset_exists_inside_conjunction() {
    let src = r#"
---- MODULE Test ----
EXTENDS Integers
VARIABLE x
Init == x = 0
Next == TRUE /\ \E S \in SUBSET (1..40) : x' = x + 1
====
"#;
    let (module, mut ctx, vars) = setup_module(src);
    let current_state = State::from_pairs([("x", Value::int(0))]);
    ...
    let (successors, capped) =
        crate::enumerate::action_successors::enumerate_action_successors_capped(
            &mut ctx,
            &next_def.body,
            &current_state,
            &vars,
        ...
        .expect("capped successor enumeration should stream the powerset");
    assert!(capped, "the second generated diff should trip the cap");
    assert_eq!(successors.len(), 1, "the sink must retain only its prefix");
}
