...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_extract_init_constraints_preserves_non_reifiable_let_lambda_4502() {
    let src = r#"
---- MODULE Test ----
VARIABLE x

Init == UseFilter(LAMBDA v : TRUE)

UseFilter(Filter(_)) ==
    LET local_filter == Filter IN
        x = 1
====
"#;
    let (module, ctx, vars) = setup_module(src);
    ...
    let branches = extract_init_constraints(&ctx, &init_def.body, &vars, None)
        .expect("non-reifiable LET lambda should defer to source body, not panic");

    assert_eq!(branches.len(), 1);
    assert_eq!(branches[0].len(), 1);
    assert!(
        matches!(&branches[0][0], Constraint::Eq(name, value) if name == "x" && value == &Value::int(1)),
        "expected x = 1 after preserving the LET source body, got {:?}",
        branches[0][0]
    );
}
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_extract_init_constraints_preserves_function_containing_lambda_4502() {
    let src = r#"
---- MODULE Test ----
VARIABLE x

Init ==
    LET local_filter == [a \in {1} |-> LAMBDA v : TRUE] IN
        x = 1
====
"#;
    let (module, ctx, vars) = setup_module(src);
    ...
    let branches = extract_init_constraints(&ctx, &init_def.body, &vars, None)
        .expect("function values containing lambdas should defer to source body, not panic");
...
fn test_extract_init_constraints_preserves_lazy_filtered_record_domain_3228() {
    // The filter domain must exceed tla-eval's SMALL_FILTER_MATERIALIZE_CAP
    // (256, fe380668): smaller enumerable domains are eagerly materialized by
    // eval_set_filter and extract as InitDomain::Concrete (pinned below in
    // test_extract_init_constraints_materializes_small_filtered_record_domain).
    // [black : 0..20, white : 0..20] has 441 elements, so the filter stays a
    // lazy SetPred and must survive init extraction as InitDomain::Enumerable
    // (regression #3228).
    let src = r#"
---- MODULE Test ----
EXTENDS Naturals
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_extract_init_constraints_streams_small_filtered_record_domain() {
    // Direct RecordSet filters deliberately remain lazy even below
    // SMALL_FILTER_MATERIALIZE_CAP (256): the init bulk lane recognizes this
    // structural fragment and streams it without materializing the source.
    // [black : 0..2, white : 0..2] has 9 elements; the filter must still
    // enumerate exactly the 5 records with black + white \in 1..2.
    let src = r#"
---- MODULE Test ----
EXTENDS Naturals
VARIABLE can
...
"#;
    ...
    match &branches[0][0] {
        ...
        other => panic!("expected streamable lazy init domain, got {other:?}"),
    }

    let states = enumerate_states_from_constraint_branches(Some(&ctx), &vars, &branches)
        .unwrap()
        .expect("small filtered record domain should enumerate through the lazy lane");
    assert_eq!(
        states.len(),
        5,
        "expected the 5 records with black + white \\in 1..2, got {states:?}"
    );
}
