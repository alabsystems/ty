...
struct EnumSubstCacheKey {
    shared_id: u64,
    call_site_ptr: usize,
    /// Source span of the call site. Belt-and-braces against address reuse:
    /// callers must pass run-stable AST nodes (see `enumerate_successors_body`),
    /// but if an ephemeral node ever reaches this cache, a freed-and-reused
    /// address can only collide when the new node also has the SAME span and
    /// resolves to the SAME operator — in which case it is a clone of the same
    /// original call site and the substituted body is identical anyway.
    call_site_span: tla_core::Span,
