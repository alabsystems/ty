...
pub(super) struct PcGuardHoist {
    ...
    /// Per-enumeration cache for extracted branch guard labels.
    pub pc_guard_label_cache: crate::checker_ops::pc_dispatch::PcGuardLabelCache,
}
...
impl<'a> EnumParams<'a> {
    ...
    pub fn with_pc_guard_hoist(mut self, pc_var_idx: VarIndex) -> Self {
        ...
        self.pc_guard_hoist = Some(PcGuardHoist {
            pc_var_idx,
            current_pc,
            pc_guard_label_cache: crate::checker_ops::pc_dispatch::new_pc_guard_label_cache(),
        });
        self
    }
}
...
/// 1. Restore a full INSTANCE caller context, or pop to `binding_mark`
/// 2. Restore the ordinary `local_ops` and `skip_prime_validation` scope
...
pub(super) struct ScopeRestore<'a> {
    ...
    /// Scope id that was valid for `saved_local_ops` when the LET block was
    /// entered. Restored alongside `saved_local_ops` so the enclosing scope id
    /// is not re-derived (no HAMT walk) on LET exit.
    pub(super) saved_local_ops_id: u64,
    pub(super) saved_skip_prime: bool,
    /// Full caller context used when an INSTANCE call swaps in the canonical
    /// module evaluation context. Ordinary LET/apply frames use the narrower
    /// fields above and leave this as `None`.
    pub(super) full_ctx_mark: Option<EnumMark>,
}

impl<'a> ScopeRestore<'a> {
    /// Restore either the full caller context or the ordinary lexical scope.
    pub(super) fn restore_ctx(&self, ctx: &mut EvalCtx) {
        if let Some(mark) = &self.full_ctx_mark {
            ctx.pop_to_enum_mark(mark);
        } else {
            ctx.pop_to_mark(&self.binding_mark);
            ctx.restore_local_ops_with_id(self.saved_local_ops.clone(), self.saved_local_ops_id);
            ctx.set_skip_prime_validation(self.saved_skip_prime);
        }
    }
...
///
/// Extends `RecState` with 4 additional fields for continuation-based conjunct
