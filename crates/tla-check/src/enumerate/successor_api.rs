...
pub(crate) fn enumerate_successors(
    ...
    enumerate_successors_body(ctx, &next_def.body, current_state, vars)
}

/// Enumerate successors directly from an action-body expression.
///
/// SOUNDNESS CONTRACT: `body` (and every node reachable from it) MUST be
/// run-stable — allocated once and kept alive for the whole model-checking
/// run. The unified enumerator memoizes per-call-site results in caches
/// keyed by raw node address (`subst_cache`, `const_domain_cache`,
/// `expr_analysis`, state-independent branch replay). Passing a per-state
/// clone allows freed node addresses to be reused by a different expression,
/// replaying stale cache entries (e.g. an INSTANCE operator body substituted
/// with the WRONG call-site arguments) — mis-bound parameters, false
/// invariant violations, false deadlocks.
pub(crate) fn enumerate_successors_body(
    ctx: &mut EvalCtx,
    body: &Spanned<Expr>,
    current_state: &State,
    vars: &[Arc<str>],
    ...
    let diffs = run_unified(ctx, body, &current_array, vars, &registry)?;
    Ok(diffs
        .into_iter()
        .map(|d| {
...
pub(crate) fn enumerate_successors_array_with_tir<'a>(
    ...
    enumerate_successors_array_body_with_tir(ctx, &next_def.body, current_array, vars, tir_leaf)
}

/// Body-expression variant of [`enumerate_successors_array_with_tir`].
///
/// See [`enumerate_successors_body`] for the run-stable AST soundness contract.
pub(crate) fn enumerate_successors_array_body_with_tir<'a>(
    ctx: &mut EvalCtx,
    body: &Spanned<Expr>,
    current_array: &ArrayState,
    vars: &'a [Arc<str>],
    ...
    let diffs = run_unified_with_tir(ctx, body, current_array, vars, &registry, tir_leaf)?;
    Ok(diffs
        .into_iter()
        .map(|d| {
...
pub(crate) fn enumerate_successors_array_as_diffs<'a>(
    ...
    enumerate_successors_array_as_diffs_body(ctx, &next_def.body, current_array, vars, tir_leaf)
}

/// Body-expression variant of [`enumerate_successors_array_as_diffs`].
///
/// See [`enumerate_successors_body`] for the run-stable AST soundness contract.
pub(crate) fn enumerate_successors_array_as_diffs_body<'a>(
    ctx: &mut EvalCtx,
    body: &Spanned<Expr>,
    current_array: &ArrayState,
    vars: &'a [Arc<str>],
    ...
    emit_debug_line(
        debug,
        format_args!(
            "enumerate_successors_array_as_diffs: body span={:?}",
            body.span
        ),
    );
...
/// Body-expression diff enumeration with an explicit materialization cap.
///
/// `successor_cap` is the remaining budget of the containing parent. As with
/// the ordinary batch sink, exactly `successor_cap` emissions are accepted if
/// enumeration then completes; the first additional emission fails closed.
pub(crate) fn enumerate_successors_array_as_diffs_body_with_cap<'a>(
    ctx: &mut EvalCtx,
    body: &Spanned<Expr>,
    current_array: &ArrayState,
    vars: &'a [Arc<str>],
    ...
    let diffs = run_unified_with_tir_and_cap(
        ctx,
        body,
        current_array,
        vars,
