...
pub(crate) fn try_value_to_expr(value: &Value) -> Option<Expr> {
    match value {
        ...
        Value::Func(func) => try_func_to_expr(func.mapping_iter(), func.domain_is_empty()),
        Value::IntFunc(f) => {
...
///
/// Part of #251: Made pub(crate) for ay_enumerate constant pre-evaluation.
///
/// # Panics
...
/// Try to build a function expression as a chain of `:>` and `@@` operators.
/// Shared by `Func` and `IntFunc` arms.
fn try_func_to_expr<'a>(
    ...
    if is_empty {
        ...
        for (k, v) in entries {
            let single = Expr::Apply(
                ...
                vec![
                    Spanned::dummy(try_value_to_expr(k)?),
                    Spanned::dummy(try_value_to_expr(v)?),
                ],
            );
        ...
        Some(result.expect("entries non-empty when is_empty is false"))
    }
}
