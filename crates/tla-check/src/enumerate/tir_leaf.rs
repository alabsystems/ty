...
feature_flag!(tir_leaf_parity_enabled, "TY_TIR_LEAF_PARITY");
feature_limit!(
    tir_leaf_parity_log_limit,
    "TY_TIR_LEAF_PARITY_LOG_LIMIT",
    20
);
...
fn record_tir_leaf_parity(
    ...
    if idx < limit {
        eprintln!(
            "TY_TIR_LEAF_PARITY mismatch[{}/{}] op={} span={:?} expr={} AST={} TIR={}",
            idx + 1,
            limit,
            label,
            expr.span,
        ...
        eprintln!(
            "TY_TIR_LEAF_PARITY additional mismatches suppressed after {} lines; \
set TY_TIR_LEAF_PARITY_LOG_LIMIT to raise the cap",
            limit
        );
    }
}
...
mod tests {
    ...
    struct LeafParityTestContext {
        _context: crate::ModelCheckContextGuard,
        _serial: std::sync::MutexGuard<'static, ()>,
    }

    fn reset_leaf_parity_test_state() -> LeafParityTestContext {
        let serial = crate::test_utils::acquire_interner_lock();
        ...
        let context = crate::enter_model_check_context();
        LeafParityTestContext {
            _context: context,
            _serial: serial,
        }
    }
    ...
    fn test_eval_leaf_parity_counts_matching_comparison() {
        let _isolation = reset_leaf_parity_test_state();
