...
fn extract_constraints_rec_inner(
    ...
    match expr {
        ...
        // `v \subseteq S` with a state-var LHS is a GENERATOR: v ranges over the
        // POWERSET of S — synthesized as `v \in SUBSET S` and routed through the
        // identical membership-domain path (same evaluation, same domain caps).
        // EXACT (every subset is a candidate; filters prune). A non-state-var LHS
        // stays a filter, byte-identical to before.
        Expr::Subseteq(lhs, rhs) => {
            if matches!(&lhs.node, Expr::Ident(name, _) | Expr::StateVar(name, _, _)
                if is_state_var(name, ctx))
            {
                let powerset = tla_core::Spanned {
                    node: Expr::Powerset(rhs.clone()),
                    span: rhs.span,
                };
                extract_in_constraints(ctx, expr, lhs, &powerset, tir)
            } else {
                as_dummy_filter(expr)
            }
        }
