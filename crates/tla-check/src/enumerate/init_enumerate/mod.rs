...
use state_enumeration::enumerate_states_from_constraints_probed;
...
pub fn enumerate_states_from_constraint_branches(
    ...
    enumerate_states_from_constraint_branches_probed(ctx, vars, branches, &mut None)
}

/// As [`enumerate_states_from_constraint_branches`] with a COOPERATIVE
/// deadline/memory probe: the combination cross-product, deferred-constraint
/// expansion, filter pass and branch/dedup loops tick it and abort with an
/// eval error on expiry (the certifying caller maps that to an honest
/// decline). `&mut None` (the plain entry above) is byte-identical.
#[allow(clippy::mutable_key_type)]
pub fn enumerate_states_from_constraint_branches_probed(
    ctx: Option<&EvalCtx>,
    vars: &[Arc<str>],
    branches: &[Vec<Constraint>],
        ...
        if deferred.is_empty() {
            if !filter_constraints.is_empty() {
                for filter_constraint in filter_constraints {
                    if !var_values.is_empty() && !filter_constraint.requires_deferred {
                        continue;
                    }
