...
impl<'a> ExpandOperators<'a> {
    ...
    fn resolve_apply_operator(&self, name: &str) -> Option<ResolvedOperator<'_>> {
        ...
        // A config-bound closure/value shadows a same-named global operator at
        // runtime. Keep the token so dependency analysis sees the config value
        // instead of inlining the shadowed definition.
        if self.ctx.is_config_constant(name) {
            return None;
        }
        self.resolve_global_operator(name)
    ...
    /// own domain has been folded), then fold `body` with EVERY bound name shadowed. Without this an
    /// earlier bound var referenced in a later domain was left UNSHADOWED, so a colliding LET / config
    /// CONSTANT / zero-arity operator captured it (`\E n \in 2..2, j \in 1..n` with `CONSTANT n = 10`
    /// widened `1..n` to `1..10`) — the TELESCOPING-DOMAIN capture. Restores the scope afterwards;
    ...
    fn fold_telescoping_binder(
        ...
        for bv in bounds {
            ...
            // shadowed and never inline a colliding operator/constant/LET over the bound occurrence.
    ...
    fn fold_apply_expr(
        ...
        if let Expr::Ident(op_name, _) = &op_expr.node {
            if let Some(resolved) = self.resolve_apply_operator(op_name) {
                // Part of #2834: Do not inline any definition that runtime
                // replaces with a Rust builtin. This includes both complete
                // module overrides and placeholder bodies (IO/Random/CSV/JSON/
                // VectorClocks/etc.); inlining a placeholder would erase the
                // builtin token and its stateful arguments before analysis.
                let resolved_name = self.ctx.resolve_op_name(op_name);
                if resolved.def.params.len() != args.len()
                    || tla_eval::should_prefer_builtin_override(
                        resolved_name,
                        resolved.def,
                        args.len(),
                        self.ctx,
...
impl ExprFold for ExpandOperators<'_> {
    fn fold_expr(&mut self, expr: Spanned<Expr>) -> Spanned<Expr> {
        ...
        match expr.node {
            ...
            // shadows all earlier bound names, then folds the body with every bound name shadowed —
            // preventing incorrect LET / operator / constant inlining both in later domains and in
            // the body when a bound variable shadows it. (#1558 + telescoping-domain fix)
