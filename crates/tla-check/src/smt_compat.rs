...
//! Times, Tuple, BigUnion) since ay has encoders for all of these.
//! Also accepts bounded CHOOSE and SetBuilder since ay translates these
...
//! bounded quantifiers that ay already supports.
//! Further widened in #3954 Phase 1: CHOOSE (bounded) and SetBuilder
//! (bounded) accepted -- ay CHC/BMC translators handle both via
...
pub(crate) fn is_expr_smt_compatible(expr: &Spanned<Expr>) -> bool {
    match &expr.node {
        ...
        // === SMT-compatible: record/tuple (any size — ay has RecordEncoder) ===
        ...
        // [x \in S |-> e] — ay FunctionEncoder (1056 LOC)
        ...
        // === SMT-compatible: bounded CHOOSE (ay Skolemization) ===
        // CHOOSE x \in S : P(x) -- translated via Skolem constants in ay.
        ...
        // === SMT-compatible: bounded SetBuilder (ay quantifier expansion) ===
        // {e : x \in S} -- translated via membership rewriting in ay:
...
mod tests {
    ...
    fn test_smt_compatibility_finite_set_operations() {
        ...
        assert!(
            ...
            "SetEnum should be SMT-compatible (finite, ay Array encoding)"
        );
        ...
        assert!(
            ...
            "x \\in S should be SMT-compatible (ay array select)"
        );

        // SUBSET S -- Powerset IS SMT-compatible (ay powerset encoder)
        ...
        assert!(
            ...
            "SUBSET S should be SMT-compatible (ay powerset encoder)"
        );
    }
    ...
    fn test_smt_compatibility_bounded_choose_compatible() {
        ...
        // ay translates via Skolemization (quantifier.rs, translate_bmc.rs)
        ...
        assert!(
            ...
            "Bounded CHOOSE should be SMT-compatible (ay Skolemization)"
        );
    }
    ...
    fn test_smt_compatibility_bounded_set_builder_compatible() {
        ...
        // ay translates via quantifier expansion (membership rewriting)
        ...
        assert!(
            ...
            "Bounded SetBuilder should be SMT-compatible (ay quantifier expansion)"
        );
    }
    ...
    fn test_smt_compatibility_big_union_powerset_inner_compatible() {
        ...
        // since ay has encoders for both. Inner Powerset(S) is accepted
