...
fn eval_action_leaf_array(
    ...
    match expr {
        ...
        LiveExpr::ActionPred {
            ...
            // Soundness (#liveness-leaf-cache): an `ActionPred` leaf is a
            // deterministic function of the (current, next) state PAIR alone,
            // fully identified by `(current_fp, next_fp)`; its quantifier
            // `bindings` are fixed per `tag`. The boolean result is independent of
            // `entry_mode` (which only governs eval-cache lifecycle, not the
            // computed value). Cache keyed on `(current_fp, next_fp, tag)` so a
            // repeated transition / cross-property tag reuse skips the
            // AST/INSTANCE re-walk. Caching is only active in the production
            // array-native path (non-empty registry); see leaf_result_cache.rs.
            let use_cache = !ctx.var_registry().is_empty();
            // Part of #liveness-leaf-memo: the ENABLED successor scan for this
            // state may have already evaluated this exact predicate on this
            // exact transition (same resolved action + bindings, see
            // EnabledActionGroup::action_pred_tag); reuse its result.
            if use_cache {
                if let Some(result) =
                    super::super::checker::get_scan_pred_result(current_fp, next_fp, *tag)
                {
                    return Ok(result);
                }
            }
            super::super::checker::eval_action_pred_cached(
                use_cache,
                current_fp,
                next_fp,
                *tag,
