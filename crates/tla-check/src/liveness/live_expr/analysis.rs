...
impl LiveExpr {
    ...
    fn extract_nested_ae_inner(&self, ae_bodies: &mut Vec<LiveExpr>) -> LiveExpr {
        match self {
            ...
            // Do NOT extract []<> bodies out of a DISJUNCTION: [](A \/ []<>Q) ≡
            // []A \/ []<>Q, NOT []<>Q. Lifting Q into a REQUIRED (conjunctive)
            // global AE constraint on the PEM would silently drop the other
            // disjunct and report a false HOLD. The unextracted disjunction stays
            // in the tableau term, which handles it soundly (same reason nested
            // EA extraction is disabled, #1741). Mirrors the Not arm above.
            // (audit3 N1)
            LiveExpr::Or(_) => self.clone(),
            LiveExpr::Bool(_)
            | LiveExpr::StatePred { .. }
            | LiveExpr::ActionPred { .. }
            | LiveExpr::Enabled { .. }
