...
debug_flag!(pub(crate) debug_scc, "TY_DEBUG_SCC");
debug_flag!(pub(crate) debug_action_pred, "TY_DEBUG_ACTION_PRED");
debug_flag!(pub(crate) debug_changed, "TY_DEBUG_CHANGED");
debug_flag!(pub(crate) debug_subscript, "TY_DEBUG_SUBSCRIPT");
debug_flag!(pub(crate) debug_bindings, "TY_DEBUG_BINDINGS");
...
pub enum LivenessResult {
    ...
    /// A candidate violating cycle was found, but the witness re-verification
    /// gate (#liveness-wf) could not run because a cycle node's concrete state
    /// is unavailable (fingerprint-only mode without a materialized state
    /// cache). The verdict is UNKNOWN: callers MUST materialize concrete states
    /// (e.g. via the fp-only replay cache) and re-run the check, or fail loudly.
    ///
    /// SOUNDNESS (#liveness-fp-only-false-hold): mapping this to `Satisfied`
    /// silently masks genuine violations (false HOLD — the worst failure
    /// class); mapping it to `Violated` would skip the fairness re-verification
    /// that prevents false counterexamples (the SingleLaneBridge bug class).
    CandidateStatesUnavailable {
        /// A fingerprint on the candidate cycle whose state was missing.
...
impl InlineCheckResults<'_> {
    /// Whether BOTH the state and action backends can serve the full
    /// multi-word `LiveBitmask` (tags >= 64).
    ///
    /// Part of #4159 follow-up: gates the multi-word reconstruction fast path.
    /// Fail-closed — if either backend is u64-only (the cross-property
    /// `FxHashMap<_, u64>` caches or the disk cold tier), reconstruction of a
    /// tag >= 64 leaf must fall back to eval, so we report `false`.
    pub(crate) fn multiword_capable(&self) -> bool {
        self.state_bitmasks.multiword_capable() && self.action_bitmasks.multiword_capable()
    }
}
