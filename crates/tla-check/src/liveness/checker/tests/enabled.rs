...
fn test_enabled_uses_successor_fingerprints_from_shared_cache() {
    ...
    shared.insert(
        current_fp,
        crate::state::ArrayState::from_state(&current, &registry),
    );
    shared.insert(
        succ_fp,
        crate::state::ArrayState::from_state(&succ, &registry),
    );
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_gate_reseed_preserves_owned_pre_pruning_successors_for_enabled() {
    crate::liveness::clear_enabled_cache();
    ...
    let current = State::from_pairs([("x", Value::int(0))]);
    let pruned_successor = State::from_pairs([("x", Value::int(1))]);
    let current_fp = current.fingerprint();
    let pruned_successor_fp = pruned_successor.fingerprint();
    ...
    assert!(checker
        .graph
        .try_add_init_node_with_fp(&current, current_fp, 0)
        .expect("initial behavior node"));
    checker
        .graph
        .add_successor_with_fp(current_node, &current, current_fp, 0)
        .expect("behavior self-loop");

    // This mirrors owned compact exploration: every real successor payload and
    // fingerprint is captured before tableau consistency pruning, but only the
    // stuttering successor survives as a behavior edge.
    checker
        .graph
        .cache_owned_state(pruned_successor_fp, &pruned_successor);
    checker
        .state_successor_fps
    ...
    let behavior_successors = checker
        .graph
    ...
    checker
        ...
        .expect("gate successor reseed");

    assert!(
        !checker.state_successors.contains_key(&current_fp),
        "stale full-state adjacency must not shadow complete owned fingerprints"
    );
    assert_eq!(
        checker
            .state_successor_fps
            .get(&current_fp)
            .expect("complete pre-pruning successor fingerprints")
            .as_slice(),
        &[current_fp, pruned_successor_fp]
    );
    assert!(
        checker
            .eval_check_on_state(&enabled_changed_x, &current)
            .expect("ENABLED should evaluate from owned successor fingerprints"),
        "ENABLED <<TRUE>>_x must see the changed-x successor pruned from behavior edges"
    );
}

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_owned_compact_enabled_fails_closed_on_missing_payload() {
    crate::liveness::clear_enabled_cache();

    let enabled_true = LiveExpr::enabled_with_bindings(
        Arc::new(spanned(Expr::Bool(true))),
        false,
        None,
        4013,
    ...
    checker
        .graph
        .add_successor_with_fp(current_node, &successor, successor_fp, 0)
        .expect("successor behavior node");
    checker
        .state_successor_fps
        .insert(current_fp, Arc::new(vec![successor_fp]));
    checker
        .state_successors
        .insert(current_fp, Arc::new(vec![current.clone()]));
    checker.graph.remove_owned_state_for_test(successor_fp);

    let error = checker
        .eval_check_on_state(&enabled_true, &current)
        .expect_err("missing owned payload must not turn ENABLED into false");
    assert!(
        error.to_string().contains("missing successor payload"),
        "unexpected missing-payload error: {error}"
    );
}

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_owned_compact_enabled_fails_closed_on_missing_adjacency() {
    crate::liveness::clear_enabled_cache();
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_owned_compact_enabled_prefers_complete_adjacency_over_stale_full_states() {
    crate::liveness::clear_enabled_cache();
    ...
    assert_eq!(
        checker
            .successor_states_for_enabled(current_fp)
            .expect("authoritative owned successors"),
        vec![successor.clone()]
    );
    assert!(
        checker
            .eval_check_on_state(&enabled_changed_x, &current)
            .expect("owned adjacency should override stale full-state adjacency"),
        "the changed-x owned successor must make ENABLED <<TRUE>>_x true"
    );
}

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_gate_reseed_owned_fails_closed_on_missing_adjacency() {
    let current = State::from_pairs([("x", Value::int(0))]);
    let current_fp = current.fingerprint();
    let current_node = BehaviorGraphNode::new(current_fp, 0);
...
fn test_owned_compact_enabled_fails_closed_on_missing_leaf_batch_adjacency() {
    ...
    let error = checker
        .populate_node_check_masks(&[enabled_true], &[], &[true], &[], 0)
        .expect_err("leaf batching must reject missing owned adjacency");
    assert!(
        error
            .to_string()
            .contains("missing successor adjacency for leaf-batch source"),
        "unexpected leaf-batch adjacency error: {error}"
    );
}
