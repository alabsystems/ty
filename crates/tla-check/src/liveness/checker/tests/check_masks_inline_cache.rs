...
fn test_populate_node_check_masks_skips_subscript_precompute_on_all_cached_actions() {
    ...
    *inline_action_bitmasks
    ...
    assert!(
        subscript_cache::get_subscript_fp_cached(s0.fingerprint(), tag).is_none(),
        "all-cached path should skip subscript precompute for the source state"
    );
    assert!(
        subscript_cache::get_subscript_fp_cached(s1.fingerprint(), tag).is_none(),
...
fn test_populate_node_check_masks_reconstructs_property_scoped_inline_cache() {
    ...
    *inline_action_bitmasks
    ...
    assert!(
        subscript_cache::get_subscript_fp_cached(s0.fingerprint(), property_action_tag).is_none(),
        "property-scoped inline cache should skip subscript precompute for the source state"
    );
    assert!(
        subscript_cache::get_subscript_fp_cached(s1.fingerprint(), property_action_tag).is_none(),
...
/// #4159 follow-up — LIVE multi-word fast path: a leaf with fairness tag >= 64
/// is reconstructed correctly from the multi-word `LiveBitmask` (in-memory
/// `StateBitmaskMap` / `ActionBitmaskMap`), instead of falling back to the slow
/// eval path. This drives the real `populate_node_check_masks_with_inline_cache`
/// entry point: `all_checks_structurally_cached` admits the >63 leaf (because the
/// backend is multi-word capable) and `populate_node_check_masks_from_cache`
/// routes it through `storage::reconstruct_check_from_bitmask`.
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_populate_node_check_masks_multiword_fast_path_tag_ge_64() {
    let mut checker = make_checker_with_vars(LiveExpr::always(LiveExpr::Bool(true)), &["x"]);
    let mut get_successors = empty_successors;
    let s0 = State::from_pairs([("x", Value::int(0))]);
    let s1 = State::from_pairs([("x", Value::int(1))]);
    ...
    // Fairness tags BEYOND the u64 first-word ceiling — only representable in a
    // multi-word LiveBitmask. The u64 first-word path would silently read these
    // as 0; the multi-word path reads them faithfully.
    let state_tag: u32 = 95;
    let action_tag: u32 = 130;
    let check_state = vec![super::helpers::state_pred_x_eq(0, state_tag)];
    let subscript = Arc::new(spanned(Expr::Ident(
        "x".into(),
        tla_core::name_intern::NameId::INVALID,
    ...
    // Real in-memory bitmask maps (multi-word capable), populated exactly as BFS
    // recording would: s0 satisfies state_tag, s1 does not; s0->s1 sets action_tag,
    // s1->s0 does not. Presence of a fingerprint/key means "all tags evaluated".
    let mut state_bitmasks = StateBitmaskMap::in_memory();
    state_bitmasks.set_tag(s0.fingerprint(), state_tag);
    state_bitmasks.get_or_insert_default(s1.fingerprint());
    let mut action_bitmasks = ActionBitmaskMap::in_memory();
    action_bitmasks.set_tag((s0.fingerprint(), s1.fingerprint()), action_tag);
    action_bitmasks.get_or_insert_default((s1.fingerprint(), s0.fingerprint()));

    // Sanity: the backends advertise multi-word capability, and the structural
    // gate now ADMITS the >63 leaves (it would have rejected them on the u64 path).
    let inline = InlineCheckResults {
        max_tag: action_tag,
        state_bitmasks: &state_bitmasks,
        action_bitmasks: &action_bitmasks,
    ...
    assert!(
        inline.multiword_capable(),
        "in-memory bitmask maps must be multi-word capable"
    );
    assert!(
        LivenessChecker::all_checks_structurally_cached(
            &check_state,
            &check_action,
            &[true],
            &[true],
        ...
        "multi-word backend must admit tag>=64 leaves to the fast reconstruction path"
    );
    ...
    checker
        ...
        .expect("multi-word fast path should reconstruct masks for tags >= 64");

    let n0_info = checker.graph().get_node_info(&n0).unwrap();
    let n1_info = checker.graph().get_node_info(&n1).unwrap();
    // These would all be FALSE if the >63-tag leaves were silently read from the
    // u64 first word (bit 95 / bit 130 live in words 1 and 2). They are TRUE only
    // because the multi-word reconstruction read the correct words.
    assert!(
        n0_info.state_check_mask.get(0),
        "state leaf at tag 95 must be TRUE on s0 via multi-word reconstruction"
    );
    assert!(
        !n1_info.state_check_mask.get(0),
        "state leaf at tag 95 must be FALSE on s1 (entry present, bit unset)"
    );
    assert!(
        n0_info.action_check_masks.first().is_some_and(|m| m.get(0)),
        "action leaf at tag 130 must be TRUE on s0->s1 via multi-word reconstruction"
    );
    assert!(
        !n1_info.action_check_masks.first().is_some_and(|m| m.get(0)),
        "action leaf at tag 130 must be FALSE on s1->s0"
    );
}

/// #4159 follow-up — FAIL-CLOSED: a u64-only backend (`FxHashMap<_, u64>`, as
/// used by the cross-property fairness caches / disk cold tier) must NOT admit a
/// tag>=64 leaf to the fast path, because it cannot represent words beyond the
/// first. The structural gate stays closed so the checker uses the correct eval
/// fallback rather than silently reading the high tag as 0.
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_u64_backend_fails_closed_for_tag_ge_64() {
    let state_tag: u32 = 95;
    let check_state = vec![super::helpers::state_pred_x_eq(0, state_tag)];

    let state_bitmasks: FxHashMap<crate::state::Fingerprint, u64> = FxHashMap::default();
    let action_bitmasks: FxHashMap<(crate::state::Fingerprint, crate::state::Fingerprint), u64> =
        FxHashMap::default();
    let inline = InlineCheckResults {
        max_tag: state_tag,
    ...
    assert!(
        !inline.multiword_capable(),
        "u64 FxHashMap backend must NOT advertise multi-word capability"
    );
    assert!(
        !LivenessChecker::all_checks_structurally_cached(
            &check_state,
        ...
        "u64-only backend must keep the tag>=64 leaf OFF the fast path (fail-closed)"
    );

    // And a tag < 64 leaf on the same u64 backend IS still admitted (no regression).
    let low_check = vec![super::helpers::state_pred_x_eq(0, 5)];
    let inline_low = InlineCheckResults {
        max_tag: 5,
        state_bitmasks: &state_bitmasks,
        action_bitmasks: &action_bitmasks,
    };
    assert!(
        ...
        "u64 backend must still admit tag<64 leaves (no regression)"
    );
}
