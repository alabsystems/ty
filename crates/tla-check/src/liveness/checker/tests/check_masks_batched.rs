...
fn test_composite_action_check_populates_cross_property_leaf_caches() {
    ...
    // ENABLED is the TLA+ semantic operator: ENABLED Inc at s1 (x=1) asks whether
    // some next state satisfies x' = x + 1 (i.e. x' = 2). That witness always
    // exists, so Inc is ENABLED at s1 regardless of which successors the explored
    // graph happens to contain. The populated-VarRegistry path enumerates the
    // action from scratch (the sound #liveness-wf behavior), so ~ENABLED Inc is
    // FALSE at s1 and the lone s1->s0 step is not an Inc step — the composite check
    // is false on that edge.
    assert!(
        n1_info
            .action_check_masks
            .first()
            .is_some_and(|mask| !mask.get(0)),
        "~ENABLED Inc \\/ Inc should fail on the non-Inc s1->s0 step: Inc is semantically \
         ENABLED at s1 (x' = 2 exists), so ~ENABLED Inc is false and the step is not an Inc step"
    );
}
...
fn test_composite_action_checks_batched_leaf_reconstruction_matches_fallback() {
    ...
    // NOTE: this test deliberately drives ONLY the populated-VarRegistry path.
    // The previous empty-registry `make_checker` "fallback" decided ENABLED purely
    // by scanning the explored behavior-graph successors, which is unsound for an
    // action whose witness successor was never explored (see the soundness note in
    // inline_leaf_eval/state.rs and enabled_eval.rs). After the #liveness-wf fix the
    // registry path enumerates the action from scratch, so the two paths legitimately
    // disagree on ENABLED for s1 (Inc's witness x' = 2 is unexplored). We therefore
    // assert the batched leaf-reconstruction against hand-derived TLA+ semantics
    // rather than against the obsolete graph-based fallback.
    let masks = {
        let mut checker = make_checker_with_vars(formula, &["x"]);
        let mut get_successors = empty_successors;
        let init = checker
    ...
    // ENABLED Inc is semantically TRUE at every state (x' = x + 1 always exists),
    // so ~ENABLED Inc is FALSE everywhere and the three checks reduce to:
    //   check[0] = ~ENABLED Inc \/ Inc  == Inc
    //   check[1] = changed /\ Inc       == Inc            (changed is implied by Inc here)
    //   check[2] = ~ENABLED Inc /\ changed == false
    let n0 = &masks.0.action_check_masks;
    let n1 = &masks.1.action_check_masks;

    // n0 has two successor edges, in insertion order: s0->s1 (Inc step) then the
    // s0->s0 self-loop (stutter).
