...
impl LivenessChecker {
    ...
    pub(super) fn clone_state_for_bfs(&self, node: BehaviorGraphNode) -> EvalResult<State> {
        self.graph.get_state(&node).ok_or_else(|| {
        ...
        G: FnMut(&State) -> EvalResult<Fingerprint>,
    {
        ...
        // Flat-state win: when a compact `ArrayState` cache is installed,
        // retain only successor *fingerprints* (`state_successor_fps`, 8B each)
        // and reconstruct successor `State`s lazily from that cache during
        // ENABLED evaluation, instead of holding a `Vec<State>` of fresh
        // `im::OrdMap`s per source state. All ENABLED consumers already prefer
        // the fingerprint path (eval.rs `enabled_successors`,
        // `eval_enabled_for_node`, leaf-batch). The recorded set is exactly the
        // get_successors set (including the stuttering self-loop), so soundness
        // is identical to the legacy `state_successors` path.
        ...
        let record_fps = fp_mode && !self.state_successor_fps.contains_key(&from.state_fp);
        let mut succ_fps_acc: Vec<Fingerprint> = if record_fps {
        ...
        for succ_state in successors {
            ...
            // ENABLED/action evaluation needs the complete state-graph
            // successor set, including successors inconsistent with every
            // tableau node. Capture their payloads before consistency pruning.
            self.graph.cache_owned_state(succ_fp, succ_state);
            if record_fps {
                succ_fps_acc.push(succ_fp);
            }
        ...
        if record_fps {
            self.state_successor_fps
                .insert(from.state_fp, Arc::new(succ_fps_acc));
        }
        ...
        G: FnMut(&State) -> EvalResult<Fingerprint>,
    {
        ...
        while let Some(current) = queue.pop_front() {
            ...
            let add_start = Instant::now();
            let record_fps = fp_mode && !self.state_successor_fps.contains_key(&current.state_fp);
            let mut successor_fps = if record_fps {
                Vec::with_capacity(state_successors.len())
            } else {
                Vec::new()
            };
            for succ_state in state_successors.iter() {
                let succ_fp = state_fp_of(succ_state)?;
                if record_fps {
                    // Preserve generation order (and duplicates): ENABLED and
                    // action checks must see the same successor sequence as the
                    // legacy concrete-State cache.
                    successor_fps.push(succ_fp);
                }
                if self
                    .graph
            ...
            if record_fps {
                self.state_successor_fps
                    .insert(current.state_fp, Arc::new(successor_fps));
            }
            self.stats.add_successors_time_us += add_start.elapsed().as_micros() as u64;
