...
fn enabled_successors(
    ...
    state_fp: Fingerprint,
) -> EvalResult<Vec<State>> {
    // Owned compact exploration has exact raw fingerprints and records the
    // complete ordered adjacency before tableau pruning. Do not let stale
    // symmetry witnesses or full-state adjacency shadow that authority.
    if graph.has_owned_state_cache() {
        let Some(succ_fps) = state_successor_fps.get(&state_fp) else {
            return Err(EvalError::Internal {
                message: format!(
                    "owned liveness compact cache is missing successor adjacency for source {state_fp}"
                ),
                span: None,
            });
        };
        ...
        for succ_fp in succ_fps.iter().copied() {
            let successor = graph.get_state_by_fp(succ_fp).ok_or_else(|| EvalError::Internal {
                message: format!(
                    "owned liveness compact cache is missing successor payload {succ_fp} for source {state_fp}"
                ),
                span: None,
            })?;
    ...
    for fp in [lookup_fp, state_fp] {
        ...
        if let Some(succ_fps) = state_successor_fps.get(&fp) {
            let mut successors = Vec::with_capacity(succ_fps.len());
            for succ_fp in succ_fps.iter().copied() {
                if let Some(successor) = graph.get_state_by_fp(succ_fp) {
                    successors.push(successor);
                }
            }
            return Ok(successors);
