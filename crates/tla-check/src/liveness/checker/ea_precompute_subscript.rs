...
impl LivenessChecker {
    ...
    pub(super) fn precompute_subscript_values(
        ...
        action_used: &[bool],
        state_cache: &FxHashMap<Fingerprint, crate::state::ArrayState>,
        ...
        if !registry_is_empty {
            ...
            for (fp, arr) in state_cache {
                // Reconstruct transient values (dropped after this state's
                // groups) for binding; the cache retains only ArrayState.
                let values = arr.to_state(&registry).to_values(&registry);
                ...
                for (repr_idx, group_indices) in &expr_groups {
                    ...
                    // Store the value's content fingerprint for all tags sharing
                    // this expression (computed once per group).
                    let value_fp = crate::state::value_fingerprint(&value);
                    for &idx in group_indices {
                        super::subscript_cache::set_subscript_fp_cache(
                            *fp,
                            sc_infos[idx].tag,
                            value_fp,
                        );
            ...
            // Fallback: HashMap-based for empty VarRegistry (tests). Reconstruct
            // State transiently (dropped after the group loop) for .vars().
            let registry = self.ctx.var_registry().clone();
            for (fp, arr) in state_cache {
