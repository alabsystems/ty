...
pub(crate) use enabled_cache::{
    ...
    is_enabled_cached, release_enabled_cache_storage, set_enabled_cache,
};
pub(crate) use leaf_result_cache::{
    ...
    eval_action_pred_cached,
    extend_enabled_action_pred_pairs, extend_enum_exact_tags, extend_full_population_tags,
    extend_whole_next_action_tags, extend_whole_next_enabled_tags, full_population_tag,
    get_scan_pred_result, insert_scan_pred_result, note_enabled_outcome,
...
impl LivenessChecker {
    ...
    pub fn populate_state_successor_fps_from_graph(&mut self) -> EvalResult<()> {
        for node in self.graph.node_keys() {
            ...
            let succ_fps: Vec<Fingerprint> = info
                .successors
                .iter()
                .map(|succ| succ.state_fp)
                .collect();
