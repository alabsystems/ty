...
pub(crate) mod debug;
mod enabled_eval;
pub(crate) mod enabled_provenance;
...
pub(crate) use checker::census_action_pred_cache_len;
pub(crate) use checker::census_enabled_cache_len;
pub(crate) use checker::census_scan_pred_len;
pub(crate) use checker::census_subscript_cache_len;
pub(crate) use checker::clear_enabled_cache;
pub(crate) use checker::clear_leaf_result_cache;
...
pub(crate) use checker::clear_scan_pred_results;
pub(crate) use checker::extend_enabled_action_pred_pairs;
pub(crate) use checker::extend_enum_exact_tags;
pub(crate) use checker::extend_full_population_tags;
pub(crate) use checker::extend_whole_next_enabled_tags;
pub(crate) use checker::{extend_whole_next_action_tags, whole_next_action_tag};
// Re-exported for the inline-fairness whole-Next detection tests only; the
...
pub(crate) fn release_regen_thread_local_storage() {
    ...
    checker::release_leaf_result_cache_storage();
    enabled_provenance::release_enabled_provenance_storage();
}
...
pub(crate) fn seed_regen_thread_local_storage_for_test(
    ...
    // Test-runner threads are reused, so make the seed independent of any
    // adaptive-disable state left by a preceding test on the same thread.
    release_regen_thread_local_storage();
    checker::seed_regen_thread_local_storage_for_test(current_fp, next_fp);
}
// TRUE-only ENABLED provenance (#3208 redo of #3100): subscript coverage proof
// used by the registration gate in `prepare_inline_fairness_cache`.
pub(crate) use checker::InlineCheckResults;
pub use checker::{GroupedLivenessPlan, LivenessChecker, LivenessResult};
pub(crate) use enabled_eval::subscript_covers_all_vars;
