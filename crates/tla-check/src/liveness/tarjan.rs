...
///
/// When ON *and* the behavior graph is disk-backed (which maintains a stable
/// dense contiguous `u32` id per node in its pointer table), `build_arena`
/// resolves successor arena ids via the pointer table and DROPS the parallel
/// `node_to_id` hash map (the single largest arena term) — a RAM-only memory
/// win that leaves the produced arena indices, and hence the SCCs, bit-for-bit
/// identical to the `node_to_id` path (both equal the `all_nodes` position).
///
...
impl ArenaTarjan {
    ...
    ///
    /// # Dense-id path (`cf1s-tarjan`)
    ///
    /// Each node's arena id is exactly its position in `graph.node_keys()`. For
    /// the disk-backed store that position is the node's STABLE dense id, which
    /// the store already indexes in its pointer table. When
    /// [`tarjan_dense_ids_enabled`] is ON and the store
    /// [`supports_dense_ids`](BehaviorGraph::supports_dense_ids), we resolve
    /// successor arena ids via that pointer table instead of building a second
    /// parallel `node_to_id` hash map (the single largest arena term, dropped
    /// here) and reuse the `node_keys()` vec directly as `id_to_node` instead of
    /// cloning a second copy. Because both paths map a node to the SAME value
    /// (its `all_nodes` position), the produced arena — and therefore the SCCs
    /// and the liveness verdict — are bit-for-bit identical.
    fn build_arena(graph: &BehaviorGraph, edge_filter: EdgeFilter<'_>) -> Self {
        ...
        // Arena memory estimate: node_to_id (dropped on the dense-id path) +
        // id_to_node (16 bytes) + succ_info (8 bytes) + states (12 bytes) +
        // succ_arena (~4*n * 4 bytes). The warning threshold is env-configurable
        // (`TY_TARJAN_ARENA_WARN_NODES`, default 1M) so small-memory agents can
        // lower it and larger hosts can raise it to suppress noise.
        if n > tarjan_arena_warn_threshold() {
            let estimated_mb = (n * 36 + n * 4 * 4) / (1024 * 1024);
            eprintln!(
        ...
        // Reverse index node -> arena id. On the dense-id path this is `None`
        // and we consult the store's pointer table instead (see module doc);
        // otherwise build the historical `node_to_id` map.
        ...
        let node_to_id: Option<FxHashMap<BehaviorGraphNode, u32>> = if use_dense {
            ...
            let mut map: FxHashMap<BehaviorGraphNode, u32> =
        ...
        for (arena_id, node) in all_nodes.iter().enumerate() {
            // Dense-id invariant: `node_keys()` is in dense order, so the
            // enumerate position must equal the store's dense id. Debug-only
            // (stripped in release) so it costs nothing on the hot path.
            debug_assert!(
                    ...
                    if let Some(ef) = edge_filter {
                        for (succ_idx, succ) in info.successors.iter().enumerate() {
                            if ef(&info, succ_idx, succ) {
                                if let Some(succ_id) =
                                    Self::resolve_arena_id(graph, node_to_id.as_ref(), succ)
                                {
                                ...
                                // Successors not resolvable are outside the graph
        ...
        // Reuse the `node_keys()` vec directly as `id_to_node`: its position IS
        // the arena id in both paths, so a second clone is unnecessary.
        let id_to_node = all_nodes;
    ...
    /// - `node_to_id = Some(map)`: historical in-RAM reverse index (in-memory
    ///   store, or the dense-id kill-switch).
    /// - `node_to_id = None`: dense-id path — the disk store's pointer table
    ///   returns the stable dense id directly (pure in-RAM, no disk read).
    #[inline]
    fn resolve_arena_id(
        graph: &BehaviorGraph,
