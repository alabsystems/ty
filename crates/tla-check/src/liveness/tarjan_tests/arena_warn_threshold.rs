...
//!
//! `tarjan_arena_warn_threshold()` reads `TY_TARJAN_ARENA_WARN_NODES` from
...
fn arena_warn_threshold_is_nonzero_by_default() {
    // Unless `TY_TARJAN_ARENA_WARN_NODES=0` is set in the environment (which
...
#[test]
// `row * n + col` adjacency-matrix indexing is kept uniform across the three
// edges for readability (the `0 * n` / `1 * n` terms are intentional).
