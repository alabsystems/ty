...
impl DiskGraphStore {
    ...
    fn persist_node(
        ...
        // Assign the node's STABLE dense id: its `all_nodes` insert position on
        // a fresh insert, or its existing id on a record rewrite (offset moves,
        // id must not). This lets the Tarjan SCC pass index per-node arrays by a
        // contiguous `u32` without a second parallel `node_to_id` hash map.
        let dense_id = match self.ptr_table.get_dense_id(node.state_fp, node.tableau_idx) {
            Some(existing) => existing,
            None => u32::try_from(self.all_nodes.len())
                .map_err(|_| DiskGraphError::DenseIdOverflow(self.all_nodes.len()))?,
        };

        let inserted = self
            .ptr_table
            .insert(node.state_fp, node.tableau_idx, offset, dense_id)
        ...
        if inserted {
            debug_assert_eq!(
                dense_id as usize,
                self.all_nodes.len(),
                "fresh dense id must equal the all_nodes insert position"
            );
            self.node_count += 1;
    ...
    /// Return the node's stable dense contiguous id (its `all_nodes` insert
    /// position), or `None` if the node is not in the store.
    ///
    /// Pure in-RAM pointer-table lookup: no disk read, so it is safe on the
    /// Tarjan SCC hot path. `dense_id_of(all_nodes()[i]) == Some(i as u32)`.
    pub(crate) fn dense_id_of(&self, node: BehaviorGraphNode) -> Option<u32> {
        self.ptr_table.get_dense_id(node.state_fp, node.tableau_idx)
    }
...
pub(crate) enum DiskGraphError {
    ...
    /// Node count exceeded the `u32` dense-id space (> ~4.29 billion nodes).
    DenseIdOverflow(usize),
}
...
impl std::fmt::Display for DiskGraphError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            DiskGraphError::DenseIdOverflow(count) => {
                write!(
                    f,
                    "disk graph dense node id overflowed u32 at node count {count}"
                )
            }
        }
    }
...
impl std::error::Error for DiskGraphError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            ...
            DiskGraphError::DuplicateNode(_)
            | DiskGraphError::MissingNode(_)
            | DiskGraphError::DenseIdOverflow(_) => None,
        }
    }
}
