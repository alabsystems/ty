...
fn assert_funcset_value_to_expr() {
    let codomain = make_set_value([Value::String(tla_value::Rp::from("a"))]);
...
fn assert_recordset_value_to_expr() {
    let recordset = Value::RecordSet(Rp::new(RecordSetValue::new([
        (
            Arc::<str>::from("a"),
...
fn assert_tupleset_value_to_expr() {
    let tupleset = Value::TupleSet(Rp::new(TupleSetValue::new([
        make_set_value([Value::SmallInt(1), Value::SmallInt(2)]),
        make_set_value([Value::String(tla_value::Rp::from("a"))]),
    ])));
...
fn test_value_to_expr_supports_intfunc() {
    let f = Value::IntFunc(Rp::new(IntIntervalFunc::new(
        3,
        4,
...
fn test_value_to_expr_big_int() {
    ...
    let expr = value_to_expr(&Value::Int(Rp::new(big.clone()))).unwrap();
    match expr {
...
fn test_value_to_expr_func_empty() {
    let f = Value::Func(Rp::new(FuncBuilder::new().build()));
    let expr = value_to_expr(&f).unwrap();
...
fn test_value_to_expr_func_non_empty() {
    ...
    let f = Value::Func(Rp::new(b.build()));
    let expr = value_to_expr(&f).unwrap();
...
fn test_value_to_expr_interval() {
    let interval = Value::Interval(Rp::new(IntervalValue::new(
        BigInt::from(1),
        BigInt::from(6),
    )));
