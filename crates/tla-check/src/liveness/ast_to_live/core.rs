...
debug_flag!(pub(super) debug_resolve, "TY_DEBUG_RESOLVE");
debug_flag!(pub(super) debug_let, "TY_DEBUG_LET");
debug_flag!(pub(super) debug_instance_resolve, "TY_DEBUG_INSTANCE_RESOLVE");
debug_flag!(pub(super) debug_subst, "TY_DEBUG_SUBST");
...
pub struct ActionPredHint {
    ...
    /// #3208 wildcard frames: existentially-quantified formal positions of a
    /// `\E x \in D, .. : Op(.., x, ..)` fairness action. Each entry pairs the
    /// formal parameter name with the CONST-evaluated, enumerated domain of
    /// its quantifier — a BFS emission through `Op` whose value at that
    /// position lies IN the domain witnesses the existential. Empty for plain
    /// `Op(args)` hints. Consumers that cannot interpret wildcards (the
    /// split-action provenance matcher) must SKIP hints with non-empty
    /// wildcards.
    pub wildcard_arg_domains: Vec<(Arc<str>, Vec<Value>)>,
}
...
pub struct AstToLive {
    ...
    /// The config's next-state operator name (as written, e.g. "Next"), when
    /// known. Used to recognize a whole-Next fairness action `WF_vars(Next)` /
    /// `SF_vars(Next)`: since every BFS-recorded (non-stuttering) transition is a
    /// Next transition, `<<Next>>_vars` collapses to the cheap `StateChanged`
    /// leaf, letting the fairness expansion drop the per-transition `ActionPred`
    /// re-evaluation of the whole Next relation. `None` disables the recognition.
    next_name: Option<Arc<str>>,
    /// Tags of `ENABLED(<<Next>>_vars)` leaves recognized as the whole next-state
    /// relation (see `record_whole_next_enabled_tag`).
    whole_next_enabled_tags: RefCell<Vec<u32>>,
}
...
impl AstToLive {
    ...
    pub fn new() -> Self {
        Self {
            ...
            next_name: None,
            whole_next_enabled_tags: RefCell::new(Vec::new()),
        }
    }

    /// Set the config's next-state operator name so `WF_vars(Next)` /
    /// `SF_vars(Next)` can be recognized and its `ActionPred` leaf simplified.
    pub fn with_next_name(mut self, next_name: Option<impl Into<Arc<str>>>) -> Self {
        self.next_name = next_name.map(Into::into);
        self
    }

    /// Whether `action` is the whole next-state relation named by the config
    /// (`Ident("Next")` where "Next" is the resolved NEXT operator), evaluated
    /// outside any INSTANCE target and any quantifier binding. Only such a
    /// top-level `WF_vars(Next)` may drop its `ActionPred` conjunct: the action
    /// must equal the exact relation the BFS explored so every recorded
    /// transition provably satisfies it.
    /// Whether `name` is the config's next-state operator name. Used by the
    /// config-driven fairness path (`checker_ops::fairness`) where the fairness
    /// action is carried as a bare operator name string.
    pub(crate) fn is_config_next_name(&self, name: &str) -> bool {
        self.next_name.as_deref() == Some(name)
    }

    /// Record the tag of an `ENABLED(<<Next>>_vars)` leaf recognized as the whole
    /// next-state relation, so the inline evaluator can decide it by scanning the
    /// (complete) BFS successor set for a subscript change instead of re-running a
    /// full from-scratch Next enumeration per state.
    pub(crate) fn record_whole_next_enabled_tag(&self, tag: u32) {
        self.whole_next_enabled_tags.borrow_mut().push(tag);
    }

    /// Take the recorded whole-Next ENABLED tags (drains the accumulator).
    pub(crate) fn take_whole_next_enabled_tags(&self) -> Vec<u32> {
        std::mem::take(&mut self.whole_next_enabled_tags.borrow_mut())
    }
