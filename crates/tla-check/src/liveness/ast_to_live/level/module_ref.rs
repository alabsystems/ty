...
fn resolve_chained_target_ctx(ctx: &EvalCtx, base_expr: &Spanned<Expr>) -> Option<EvalCtx> {
    ...
    // Preserve intermediate module operators for chained liveness references.
    // EWD998Chan!EWD998!TD!Live enters TD while still needing EWD998-local
    // operators for implicit substitutions inside TD predicates.
...
impl AstToLive {
    ...
    pub(super) fn level_module_ref_with_ctx(
        ...
        if let Some(instance_ctx) = resolve_module_target_ctx(ctx, target) {
            ...
            } else if is_stdlib_builtin_module_op(ctx, target, op_name) {
                // Builtin stdlib module op (e.g., `G == INSTANCE Graphs` then
                // `G!Transpose(...)`): stdlib modules are implemented natively
                // and never loaded from `.tla` sources, so `get_op` misses.
                // These operators are pure functions of their arguments (no
                // state access), so the op itself contributes Constant level;
                // the overall level stays the max of target/argument levels.
                // Without this, TreeWithRoot-style `[](... G!IsTreeWithRoot(O, r))`
                // properties are misclassified Temporal and hard-error (#2835
                // fallback) instead of checking as state predicates.
            } else {
            ...
            // Same as above, for targets whose instance context cannot be
            // materialized (module absent from instance_ops entirely).
            // Level stays the max of target/argument levels.
        } else {
...
/// True when a module reference ultimately targets a builtin stdlib module
/// (e.g., `G == INSTANCE Graphs` ... `G!Transpose(...)`) and `op_name` is one
/// of that module's declared operators.
///
/// Stdlib/community modules are implemented natively (TLC-parity: Java module
/// overrides) and never loaded into `instance_ops`, so operator-body lookup
/// fails for them even though evaluation succeeds via the builtin dispatch
/// (see `eval_module_ref`'s stdlib fallback). Their operators are pure
/// functions of their arguments — Constant level contribution.
fn is_stdlib_builtin_module_op(ctx: &EvalCtx, target: &ModuleTarget, op_name: &str) -> bool {
    let instance_name = match target {
        ModuleTarget::Named(n) | ModuleTarget::Parameterized(n, _) => n,
        ModuleTarget::Chained(_) => return false,
    };
    // Direct module reference (`Graphs!Transpose`) or via a named instance
    // (`G == INSTANCE Graphs` — module-level or LET-local).
    if tla_core::stdlib_module_has_op(instance_name, op_name) {
        return true;
    }
    match resolve_instance_info(ctx, instance_name) {
        Some((module_name, _)) => tla_core::stdlib_module_has_op(&module_name, op_name),
        None => false,
    }
}
