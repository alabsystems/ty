...
use tla_core::ast::{BoundPattern, BoundVar, ExceptPathElement, Expr, ModuleTarget};
...
fn expr_references_local_op(
    expr: &Expr,
    local_ops: &tla_core::OpEnv,
    bound: &mut HashSet<String>,
) -> bool {
    match expr {
        Expr::Ident(name, _) => !bound.contains(name) && local_ops.contains_key(name.as_str()),
        Expr::StateVar(..) | Expr::Bool(_) | Expr::Int(_) | Expr::String(_) | Expr::OpRef(_) => {
            false
        }
        Expr::Apply(op, args) => {
            expr_references_local_op(&op.node, local_ops, bound)
                || args
                    .iter()
                    .any(|arg| expr_references_local_op(&arg.node, local_ops, bound))
        }
        Expr::ModuleRef(target, _, args) => {
            let target_refs = match target {
                ModuleTarget::Parameterized(_, params) => params
                    .iter()
                    .any(|param| expr_references_local_op(&param.node, local_ops, bound)),
                ModuleTarget::Chained(base) => {
                    expr_references_local_op(&base.node, local_ops, bound)
                }
        ...
        Expr::InstanceExpr(_, subs) => subs
            .iter()
            .any(|sub| expr_references_local_op(&sub.to.node, local_ops, bound)),
        Expr::SubstIn(subs, body) => {
            subs.iter()
                .any(|sub| expr_references_local_op(&sub.to.node, local_ops, bound))
                || expr_references_local_op(&body.node, local_ops, bound)
        }
        Expr::Lambda(params, body) => {
            for param in params {
                bound.insert(param.node.clone());
            }
            let result = expr_references_local_op(&body.node, local_ops, bound);
            for param in params {
                bound.remove(&param.node);
            }
            result
        }
        Expr::Forall(bounds, body) | Expr::Exists(bounds, body) | Expr::FuncDef(bounds, body) => {
            if bounds
                .iter()
                .filter_map(|bv| bv.domain.as_ref())
                .any(|domain| expr_references_local_op(&domain.node, local_ops, bound))
            {
                return true;
            }
            let names: Vec<_> = bounds.iter().flat_map(bound_names_from_bv).collect();
            for name in &names {
        ...
        Expr::Choose(bv, body) => {
            if let Some(domain) = &bv.domain {
                if expr_references_local_op(&domain.node, local_ops, bound) {
                    return true;
                }
            }
            let names = bound_names_from_bv(bv);
            for name in &names {
                bound.insert(name.clone());
            }
            let result = expr_references_local_op(&body.node, local_ops, bound);
        ...
        Expr::SetFilter(bv, pred) => {
            ...
            let result = expr_references_local_op(&pred.node, local_ops, bound);
            for name in &names {
                bound.remove(name);
            }
            result
        ...
        Expr::Let(defs, body) => {
            if defs
                .iter()
                .any(|def| expr_references_local_op(&def.body.node, local_ops, bound))
            {
                return true;
            }
            let names: Vec<_> = defs.iter().map(|def| def.name.node.clone()).collect();
            for name in &names {
                bound.insert(name.clone());
            }
            let result = expr_references_local_op(&body.node, local_ops, bound);
        ...
        Expr::Label(label) => expr_references_local_op(&label.body.node, local_ops, bound),
        Expr::And(a, b)
        | Expr::Or(a, b)
        | Expr::Implies(a, b)
        | Expr::Equiv(a, b)
        ...
        | Expr::Pow(a, b)
        | Expr::Range(a, b) => {
            expr_references_local_op(&a.node, local_ops, bound)
                || expr_references_local_op(&b.node, local_ops, bound)
        }
        Expr::Not(e)
        | Expr::Powerset(e)
        | Expr::BigUnion(e)
        ...
        | Expr::Enabled(e)
        | Expr::Unchanged(e)
        | Expr::Neg(e) => expr_references_local_op(&e.node, local_ops, bound),
        Expr::SetEnum(values) | Expr::Tuple(values) | Expr::Times(values) => values
            .iter()
            .any(|value| expr_references_local_op(&value.node, local_ops, bound)),
        Expr::Except(base, specs) => {
            expr_references_local_op(&base.node, local_ops, bound)
                || specs.iter().any(|spec| {
                    spec.path.iter().any(|path| match path {
                        ExceptPathElement::Index(index) => {
                            expr_references_local_op(&index.node, local_ops, bound)
                        }
                        ExceptPathElement::Field(_) => false,
                    }) || expr_references_local_op(&spec.value.node, local_ops, bound)
                })
        }
        Expr::Record(fields) | Expr::RecordSet(fields) => fields
            .iter()
            .any(|(_, value)| expr_references_local_op(&value.node, local_ops, bound)),
        Expr::RecordAccess(record, _) => expr_references_local_op(&record.node, local_ops, bound),
        Expr::If(cond, then_expr, else_expr) => {
            expr_references_local_op(&cond.node, local_ops, bound)
                || expr_references_local_op(&then_expr.node, local_ops, bound)
                || expr_references_local_op(&else_expr.node, local_ops, bound)
        }
        Expr::Case(arms, other) => {
            arms.iter().any(|arm| {
                expr_references_local_op(&arm.guard.node, local_ops, bound)
                    || expr_references_local_op(&arm.body.node, local_ops, bound)
            }) || other
                .as_ref()
                .is_some_and(|expr| expr_references_local_op(&expr.node, local_ops, bound))
        }
        Expr::WeakFair(subscript, action) | Expr::StrongFair(subscript, action) => {
            expr_references_local_op(&subscript.node, local_ops, bound)
                || expr_references_local_op(&action.node, local_ops, bound)
        }
    }
}

fn substitution_rhs_needs_instance_scope(ctx: &EvalCtx, expr: &Expr) -> bool {
    let Some(local_ops) = ctx.local_ops().as_ref() else {
        return false;
    };
    expr_references_local_op(expr, local_ops, &mut HashSet::new())
}
            ...
            Expr::Ident(name, _) => {
                ...
                if let Some(sub) = active_instance_sub {
                    let outer_ctx = if ctx.local_ops().is_some() {
                        if substitution_rhs_needs_instance_scope(ctx, &sub.to.node) {
                            ctx.clone()
                        } else {
                            ctx.with_outer_resolution_scope()
                        }
