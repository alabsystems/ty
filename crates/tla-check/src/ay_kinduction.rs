...
//! AY-based k-induction for proving TLA+ safety properties.
//!
...
use tla_ay::{AYError, SolveResult, UnknownReason};
...
impl From<AYError> for KInductionError {
    fn from(err: AYError) -> Self {
        match err {
            AYError::Solver(inner) => KInductionError::SolverFailed(inner.to_string()),
...
impl Default for KInductionConfig {
    fn default() -> Self {
        let timeout_secs: u64 = std::env::var("TY_KINDUCTION_TIMEOUT_SECS")
            .ok()
        ...
        Self {
            ...
            debug: debug_ay_kinduction_enabled(),
            incremental: true,
        }
    }
}

debug_flag!(debug_ay_kinduction_enabled, "TY_DEBUG_AY_KINDUCTION");

/// Render the frontend-neutral shared AY k-induction lane metadata evidence row.
pub fn kinduction_shared_engine_lane_evidence(scope: &str) -> String {
    tla_ay::render_ay_shared_engine_lane_evidence(scope, tla_ay::AYSharedEngineLane::KInduction)
}
...
pub fn check_kinduction_with_portfolio(
    ...
    let symbolic_ctx =
        ay_shared::symbolic_ctx_with_config(ctx, config).map_err(KInductionError::MissingSpec)?;
    ...
    match bmc_result {
        ...
        BmcBaseResult::BoundReached { max_depth } => {
            if kind_config.debug {
                telemetry_eprintln!("[ay-kind] BMC base case passed up to depth {max_depth}");
            }
        }
    }
...
fn run_bmc_base_case(
    ...
    for depth in 0..=kind_config.max_k {
        ...
        if kind_config.debug {
            telemetry_eprintln!("[ay-kind] BMC base depth {depth}");
        }

        let mut translator = ay_shared::make_translator(var_sorts, depth)?;
...
fn run_inductive_step(
    ...
    for k in kind_config.start_k..=kind_config.max_k {
        ...
        if kind_config.debug {
            telemetry_eprintln!("[ay-kind] inductive step k={k}");
        }

        let mut translator = ay_shared::make_translator(var_sorts, k)?;
        translator.set_timeout(kind_config.solve_timeout);
        ...
        match translator.try_check_sat()? {
            SolveResult::Unsat(_) => {
                if kind_config.debug {
                    telemetry_eprintln!("[ay-kind] PROVED: property is {k}-inductive");
                }
            ...
            SolveResult::Sat => {
                ...
                if kind_config.debug {
                    telemetry_eprintln!(
                        "[ay-kind] induction SAT at k={k} (may be spurious), trying deeper"
                    );
                }
            }
            SolveResult::Unknown => {
...
fn run_inductive_step_incremental(
    ...
    let mut translator = ay_shared::make_translator(var_sorts, kind_config.max_k)?;
    translator.set_timeout(kind_config.solve_timeout);
    ...
    for k in kind_config.start_k..=kind_config.max_k {
        ...
        if kind_config.debug {
            telemetry_eprintln!("[ay-kind-incr] inductive step k={k}");
        }
        ...
        match translator.try_check_sat()? {
            SolveResult::Unsat(_) => {
                if kind_config.debug {
                    telemetry_eprintln!("[ay-kind-incr] PROVED: property is {k}-inductive");
                }
                return Ok(KInductionResult::Proved { k });
            }
            SolveResult::Sat => {
                ...
                if kind_config.debug {
                    telemetry_eprintln!(
                        "[ay-kind-incr] induction SAT at k={k} (may be spurious), trying deeper"
                    );
                }
            }
            SolveResult::Unknown => {
...
pub fn check_kinduction_cooperative(
    ...
    let result = match result {
        KInductionResult::Proved { k } => {
            telemetry_eprintln!(
                "[ay-kind] k-induction proved safety at k={k}, publishing to cooperative state"
            );
            cooperative.set_kinduction_proved_k(k);
            cooperative.set_invariants_proved();
            // SOUNDNESS: k-induction proves the safety invariant ONLY (no
            // deadlock-freedom, no liveness/temporal/trace obligations). Only let it
            // resolve the cooperative verdict — which truncates the BFS lane into an
            // indistinguishable Success — when safety is the run's SOLE obligation;
            // otherwise leave BFS authoritative so a symbolic-safe win cannot silently
            // mask a reachable deadlock or a liveness violation. See
            // ay_shared::symbolic_safety_proof_covers_all_obligations.
            if crate::ay_shared::symbolic_safety_proof_covers_all_obligations(config) {
                // PROOF-PRODUCING: resolve the cooperative verdict with a symbolic
                // Satisfied ONLY when the proven invariant re-discharges a STRICT,
                // independently re-checkable certificate; otherwise leave the slot
                // unresolved (BFS authoritative). A symbolic proof that does not re-check
                // is never trusted as the user's verdict.
        ...
        KInductionResult::Counterexample { depth, trace } => {
            // SOUNDNESS (fail closed): the base case is solved on the SMT
            // TRANSLATION of Init/Next/Inv, not on the spec itself, so a SAT
            // model can be spurious (over-approximation / translation gap).
            // Publishing `Violated` truncates the racing BFS lane into a
            // result indistinguishable from a clean Success, so it may happen
            // ONLY after the explicit-state evaluator (the permanent oracle)
            // has re-confirmed the counterexample. An unconfirmed base-case
            // model downgrades the WHOLE lane result to Unknown: the lane
            // could not discharge its own base case, so it must neither win
            // the race with `Violated` nor contribute a "no error" summary —
            // and BFS keeps running, unharmed.
            ...
            if cv.engine_agrees {
                // The base case found a real counterexample from Init — definitive.
                telemetry_eprintln!(
                    "[ay-kind] k-induction base case found counterexample at depth {depth} \
                     (confirmed by the explicit-state evaluator)"
                );
                cooperative
                    .verdict
                ...
                telemetry_eprintln!(
                    "[ay-kind] k-induction base case SAT at depth {depth} but the \
                     explicit-state evaluator did NOT confirm the counterexample ({}) — \
                ...
                KInductionResult::Unknown {
                    max_k: depth,
                    reason: format!(
                        "base case found a depth-{depth} counterexample but the \
                         explicit-state evaluator did not confirm it ({}) — failing closed",
                        cv.detail
                    ),
                }
            }
        ...
        KInductionResult::Unknown { max_k, reason } => {
            ...
            telemetry_eprintln!("[ay-kind] k-induction inconclusive at max_k={max_k}: {reason}");
            KInductionResult::Unknown { max_k, reason }
        }
    };
...
fn check_kinduction_cooperative_inner(
    ...
    for depth in 0..=kind_config.max_k {
        ...
        if kind_config.debug {
            telemetry_eprintln!("[ay-kind-coop] BMC base depth {depth}");
        }

        let mut translator = ay_shared::make_translator(&var_sorts, depth)?;
        translator.set_timeout(kind_config.solve_timeout);
        // Cooperative teardown: let the fused orchestrator abort this solver
        // mid-`check_sat` once another lane resolves the verdict, instead of
        // the per-depth poll above being the only exit (a single solve can
        // otherwise hold the fused join for minutes — the DieHard defect).
    ...
    if kind_config.debug {
        telemetry_eprintln!(
            "[ay-kind-coop] BMC base case passed up to depth {}",
            kind_config.max_k
        );
    }
    ...
    for k in kind_config.start_k..=kind_config.max_k {
        ...
        if kind_config.debug {
            telemetry_eprintln!("[ay-kind-coop] inductive step k={k}");
        }

        let mut translator = ay_shared::make_translator(&var_sorts, k)?;
        translator.set_timeout(kind_config.solve_timeout);
        // Cooperative teardown: see the BMC-base-case registration above.
        cooperative.register_solver_interrupt(translator.interrupt_handle());
        ...
        match translator.try_check_sat()? {
            SolveResult::Unsat(_) => {
                if kind_config.debug {
                    telemetry_eprintln!("[ay-kind-coop] PROVED: property is {k}-inductive");
                }
                return Ok(KInductionResult::Proved { k });
            }
            SolveResult::Sat => {
                if kind_config.debug {
                    telemetry_eprintln!(
                        "[ay-kind-coop] induction SAT at k={k} (may be spurious), trying deeper"
                    );
                }
            }
            SolveResult::Unknown => {
