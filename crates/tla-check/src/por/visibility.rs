...
impl VisibilitySet {
    ...
    ///
    /// FAIL CLOSED (hole #3): if the expanded invariant still contains residue
    /// the extractor cannot analyze (an un-inlined operator reference, e.g. a
    /// zero-arg FuncDef kept by the #2955 perf guard), the invariant's read
    /// set is unknowable — fall back to `mark_all_visible`, the same fallback
    /// already used for unresolved invariant names. This disables reduction
    /// rather than letting C2 miss a visible write.
    ...
    ///
    /// FAIL CLOSED (hole #3): an OPAQUE action's writes are unknowable, so it
    /// must count as visible — it may write any invariant-read variable.
    pub(crate) fn is_action_visible(&self, deps: &ActionDependencies) -> bool {
        self.all_visible || deps.opaque || !deps.writes.is_disjoint(&self.vars)
    }
}
