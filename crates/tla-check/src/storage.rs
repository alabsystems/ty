...
pub use contracts::{
    BatchInsertedIndexAdmission, CapacityStatus, FingerprintSet, InsertOutcome, LookupOutcome,
    StorageFault, StorageStats,
};
pub use disk::DiskFingerprintSet;
pub use mmap::MmapFingerprintSet;
#[allow(unused_imports)]
pub use open_addressing::MmapError;
pub(crate) use payload_witness::FingerprintPayloadWitnesses;
#[allow(unused_imports)]
...
///
/// Override via the `TY_INMEMORY_FP_MAX` environment variable.
///
...
pub enum FingerprintStorage {
    ...
    InMemory {
        /// The in-memory fingerprint set.
        set: parking_lot::RwLock<FpHashSet>,
        /// Soft capacity at which a warning is emitted (0 = no limit).
        max_capacity: usize,
        /// Last-warned multiple of `max_capacity` (0 = never warned).
...
impl FingerprintStorage {
    ...
    ///
    /// Test-only helper that bypasses the `TY_INMEMORY_FP_MAX` env var,
                ...
                .is_ok()
        {
            eprintln!(
                "Warning: in-memory fingerprint storage has {count} entries \
                 ...
                 TY_INMEMORY_FP_MAX to raise this limit."
            );
        }
    }
    ...
    fn insert_batch_prefix_inserted_indices_checked_into(
        fingerprints: &[Fingerprint],
    ...
    fn insert_batch_value_prefix_inserted_indices_checked_into(
        fingerprint_values: &[u64],
        admission: &mut BatchInsertedIndexAdmission,
        mut insert: impl FnMut(Fingerprint) -> InsertOutcome,
    ) {
    ...
    /// Insert a batch of fingerprints and return indices that were newly
    /// admitted.
    pub fn insert_batch_inserted_indices_checked(
        &self,
        fingerprints: &[Fingerprint],
    ...
    /// Insert a batch of fingerprints into caller-owned admission scratch.
    pub fn insert_batch_inserted_indices_checked_into(
        &self,
        fingerprints: &[Fingerprint],
        admission: &mut BatchInsertedIndexAdmission,
    ) {
        match self {
            ...
            FingerprintStorage::Mmap(set) => {
                set.insert_fingerprints_inserted_indices_checked_into(fingerprints, admission);
            }
            FingerprintStorage::Disk(set) => {
                Self::insert_batch_prefix_inserted_indices_checked_into(
                    fingerprints,
                    admission,
                    |fp| set.insert_checked(fp),
                );
            }
        }
    }
    ...
    /// Insert raw 64-bit fingerprint values and return indices that were newly
    /// admitted.
    pub fn insert_batch_fingerprint_values_inserted_indices_checked(
        &self,
        fingerprint_values: &[u64],
    ) -> BatchInsertedIndexAdmission {
    ...
    /// scratch.
    pub fn insert_batch_fingerprint_values_inserted_indices_checked_into(
        &self,
        fingerprint_values: &[u64],
        admission: &mut BatchInsertedIndexAdmission,
    ) {
        match self {
            ...
            FingerprintStorage::Mmap(set) => {
                set.insert_fingerprint_values_inserted_indices_checked_into(
                    fingerprint_values,
                    admission,
                );
            }
            FingerprintStorage::Disk(set) => {
                Self::insert_batch_value_prefix_inserted_indices_checked_into(
                    fingerprint_values,
                    admission,
                    |fp| set.insert_checked(fp),
                );
...
impl FingerprintSet for FingerprintStorage {
    ...
    fn insert_batch_inserted_indices_checked(
        ...
        FingerprintStorage::insert_batch_inserted_indices_checked(self, fingerprints)
    }
    ...
    fn insert_batch_inserted_indices_checked_into(
        ...
        FingerprintStorage::insert_batch_inserted_indices_checked_into(
            self,
            fingerprints,
            admission,
        )
    ...
    fn insert_batch_fingerprint_values_inserted_indices_checked(
        ...
        FingerprintStorage::insert_batch_fingerprint_values_inserted_indices_checked(
            self,
            fingerprint_values,
        )
    }
    ...
    fn insert_batch_fingerprint_values_inserted_indices_checked_into(
        ...
        FingerprintStorage::insert_batch_fingerprint_values_inserted_indices_checked_into(
            self,
            fingerprint_values,
            admission,
        )
    ...
    fn reserve(&self, additional: usize) {
        // Only the growable in-memory backend benefits; Mmap/Disk are sized at
        // construction. Pure allocation hint — never affects admission.
        if let FingerprintStorage::InMemory { set, .. } = self {
            set.write().reserve(additional);
        }
    }
