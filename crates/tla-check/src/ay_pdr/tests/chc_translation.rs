...
"#;
    ...
    // T2 widening 4: the non-recursive LET def (`t == 1`, unreferenced) inlines
    // and the wrapper is DROPPED — the body's operators still expand.
    ...
    match &e3_expanded.node {
        Expr::In(_, set) => match &set.node {
            ...
            other => panic!("expected Range after LET inlining, got {other:?}"),
        },
        other => panic!("expected In (LET dropped), got {other:?}"),
    }
}
...
"#;
    ...
    // T2 widening 4: the capture-safe chain now FULLY beta-reduces —
    // `Wrap(y)` => `LET g(z) == y IN g(0)` => `y` (the LET-local `g` inlines at
    // its application and the wrapper drops). The capture-UNSAFE sibling test
    // above still pins that `Wrap(y)` with a colliding param is NOT inlined.
    match &expanded.node {
        Expr::Ident(name, _) | Expr::StateVar(name, _, _) => {
            assert_eq!(name, "y", "the fully reduced body must be the state var y");
        }
        other => panic!("expected fully beta-reduced `y`, got {other:?}"),
    }
}
