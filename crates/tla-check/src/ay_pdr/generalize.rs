...
//!    safety property (using the ay SMT solver with push/pop).
...
///
/// Uses the ay CHC solver to check whether dropping individual conjuncts
...
impl<'a> LemmaGeneralizer<'a> {
    ...
    fn check_lemma_blocks(&self, candidate: &Spanned<Expr>) -> Result<bool, PdrError> {
        ...
        //
        // A bounded `solve_timeout` + `max_obligations` are REQUIRED here, not
        // just nice-to-have: `PdrConfig::default()` leaves `solve_timeout =
        // None`, so `ay_chc::engines::solve_pdr_proof` installs no
        // `ScopedSolveDeadline` and the per-candidate PDR solve can run
        // unbounded. Lemma generalization probes many candidate sub-lemmas
        // (one per dropped literal); on multi-variable lemmas an unbounded
        // candidate solve makes the whole generalize pass hang. A short budget
        // makes each probe return a sound Safe/Unknown (Unknown ==
        // conservatively keep the literal), so generalization always
        // terminates regardless of the lemma shape.
        ...
        match result {
            Ok(tla_ay::chc::PdrCheckResult::Safe { .. }) => Ok(true),
            Ok(tla_ay::chc::PdrCheckResult::Unsafe { .. }) => Ok(false),
            Ok(tla_ay::chc::PdrCheckResult::Unknown { .. }) => {
