...
impl ResultClass {
    ...
    pub fn from_check_result(result: &CheckResult) -> Self {
        match result {
            ...
            // A vacuous run is not a successful one: classify it as an error so
            // the minimizer's oracle never treats `VACUOUS` as a passing result.
            CheckResult::Vacuous { .. }
            | CheckResult::Error { .. }
            | CheckResult::LimitReached { .. } => ResultClass::Error,
        }
    }
}
