...
//! # Design
//!
//! In standard TY, `ArrayState` stores `Box<[Value]>` where `Value` contains
...
pub(crate) fn has_active_model_check_runs() -> bool {
    let active = ACTIVE_MODEL_CHECK_RUNS.load(Ordering::Acquire);
    active > 0 && active != RESET_SENTINEL
}
...
/// Restores a non-sentinel counter value if boundary cleanup unwinds.
///
/// A poisoned interner must not strand every future model check spinning on
/// `RESET_SENTINEL`. The begin transition rolls back to zero until cleanup has
/// completed; the end transition always releases to zero.
struct ModelCheckCounterTransition {
    release_to: usize,
}

impl Drop for ModelCheckCounterTransition {
    fn drop(&mut self) {
        ACTIVE_MODEL_CHECK_RUNS.store(self.release_to, Ordering::Release);
    }
}
...
/// run completes.
///
/// Part of #4067: Spin-wait if a concurrent reset is in progress (counter ==
/// RESET_SENTINEL). This is a production semantic boundary: library callers
/// can invoke `reset_global_state()` concurrently too.
pub(crate) fn begin_model_check_run() {
    loop {
        ...
        if current == 0 {
            // Own the zero-to-one boundary while clearing stale run-scoped
            // interners. Publishing `1` before the clear would allow a second
            // run to start and observe its live handles being invalidated.
            if ACTIVE_MODEL_CHECK_RUNS
                .compare_exchange(0, RESET_SENTINEL, Ordering::AcqRel, Ordering::Acquire)
                .is_ok()
            {
                let mut transition = ModelCheckCounterTransition { release_to: 0 };
                clear_run_scoped_interners();
                transition.release_to = 1;
                drop(transition);
                return;
            }
            continue;
        }
        let Some(next) = current.checked_add(1).filter(|next| *next < RESET_SENTINEL) else {
            panic!("active model-check run counter overflow");
        };
...
pub(crate) fn end_model_check_run() {
    ...
    loop {
        ...
        // Own the one-to-zero boundary until cleanup is complete. Publishing
        // `0` first would let a reset or a new run race the final clear.
        let next = if active == 1 {
            RESET_SENTINEL
        } else {
            active - 1
        };
        match ACTIVE_MODEL_CHECK_RUNS.compare_exchange_weak(
            ...
            Ok(_) if active == 1 => {
                let transition = ModelCheckCounterTransition { release_to: 0 };
                clear_run_scoped_interners();
                drop(transition);
                return;
            }
