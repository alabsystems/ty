...
//! Direct ay incremental-solving coverage

// This test demonstrates ay ALL-SAT directly at the solver level.
// Previously blocked by ay#294/ay#309: fixed in ay commit 8ea188b6
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_ay_incremental_directly() {
    use std::collections::BTreeSet;
    use tla_ay::{Logic, SolveResult, Solver, Sort};
    ...
    for i in 0..10 {
        match solver.check_sat().result() {
            SolveResult::Sat => {
                let model = solver.try_get_model_for_consumer().unwrap().into_inner();
                let x_val: i64 = model
                    .int_val("x")
                    .expect("missing x")
                    .try_into()
...
// Test ay ALL-SAT directly with arithmetic constraints (x + y = 5).
//
// Regression test for ay#949: incremental solving + blocking clauses must not
// return UNSAT after the first model when arithmetic equalities are present.
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_ay_incremental_arithmetic_directly() {
    use std::collections::BTreeSet;
    use tla_ay::{Logic, SolveResult, Solver, Sort};
