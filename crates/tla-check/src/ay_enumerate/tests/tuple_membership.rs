...
fn test_enumerate_heterogeneous_setenum_does_not_silently_underenumerate() {
    ...
    match enumerate_init_states_ay(&ctx, &init, &vars, None, &config) {
        Err(e) => {
            assert!(
                matches!(
                    e,
                    AYEnumError::UnsupportedVarType { .. }
                        | AYEnumError::TranslationFailed(_)
                ),
...
fn test_enumerate_tuple_set_membership_from_parsed_module() {
    // Integration-ish: parse a TLA+ module, then enumerate Init via ay.
    let spec = r#"
...
"#;
    ...
    let states = enumerate_init_states_ay(&ctx, &init_def.body, &vars, None, &config).unwrap();
    assert_eq!(states.len(), 2);
