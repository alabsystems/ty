...
//! Value-to-Expr conversion and AST walking helpers for ay enumeration.
...
/// Convert a `Value` into an `Expr` (with spans) that is safe to pass through the ay translation path.
///
/// This intentionally only supports a narrow subset of `Value` variants that map cleanly onto
/// expression forms supported by `tla-ay` translation (notably set membership domains).
///
/// Part of #251 / #515: Safe constant pre-evaluation gating.
pub(super) fn try_value_to_ay_spanned_expr(value: &Value, span: Span) -> Option<Spanned<Expr>> {
    // Avoid materializing enormous SetEnum disjunctions during pre-evaluation.
    const MAX_AY_SETENUM_ELEMS: usize = 5_000;
    ...
    let node = match value {
        ...
        Value::Tuple(elems) => {
            let exprs: Vec<Spanned<Expr>> = elems
                .iter()
                .map(|e| try_value_to_ay_spanned_expr(e, span))
        ...
        Value::Func(f) => {
            if f.domain_len() > MAX_AY_SETENUM_ELEMS {
                return None;
            }
            ...
            for (i, (k, v)) in f.mapping_iter().enumerate() {
                ...
                tuple_elems.push(try_value_to_ay_spanned_expr(v, span)?);
            }
        ...
        Value::IntFunc(f) => {
            ...
            if f.min() != 1 || len != f.values().len() || len > MAX_AY_SETENUM_ELEMS {
                return None;
            }
        ...
        Value::Set(set) => {
            if set.len() > MAX_AY_SETENUM_ELEMS {
                return None;
            }
        ...
        // Materialize small finite lazy-set values into SetEnum for ay-safe membership domains.
        Value::Subset(_)
        | Value::FuncSet(_)
        | Value::RecordSet(_)
        | Value::TupleSet(_)
        ...
        | Value::BigUnion(_) => {
            ...
            if len > BigInt::from(MAX_AY_SETENUM_ELEMS) {
                return None;
            }
