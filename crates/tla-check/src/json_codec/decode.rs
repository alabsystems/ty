...
/// Decodes a [`JsonValue`] into a TLA+ [`Value`].
///
/// # Errors
/// Returns a [`JsonValueDecodeError`] for an invalid `big_int`, a [`JsonValue::Undefined`]
/// input, or a malformed function (duplicate/missing/out-of-domain keys).
pub fn json_to_value(value: &JsonValue) -> Result<Value, JsonValueDecodeError> {
    Ok(match value {
        ...
        JsonValue::Record(fields) => {
            ...
            items.sort_unstable_by_key(|(ka, _)| *ka);
        ...
        JsonValue::Interval { lo, hi } => Value::Interval(Rp::new(IntervalValue::new(
...
/// Like [`json_to_value`], but on failure annotates the error with the JSON path
/// of the offending value (see [`JsonValueDecodeErrorAtPath`]).
///
/// # Errors
/// Returns a [`JsonValueDecodeErrorAtPath`] wrapping the same failure cases as
/// [`json_to_value`], with the path to the failing sub-value.
...
fn json_to_value_inner(
    ...
    Ok(match value {
        ...
        JsonValue::Interval { lo, hi } => {
            ...
            Value::Interval(Rp::new(IntervalValue::new(low, high)))
        }
        JsonValue::Undefined => {
