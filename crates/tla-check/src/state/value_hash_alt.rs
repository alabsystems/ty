...
fn serialize_value_for_hash(buffer: &mut Vec<u8>, value: &Value) {
    match value {
        ...
        Value::Bag(bag) => {
            // Same stream as the equivalent Func (counts are SmallInts).
            buffer.push(4); // type tag
            buffer.extend_from_slice(&(bag.len() as u32).to_le_bytes());
            for (k, c) in bag.entries() {
                serialize_value_for_hash(buffer, k);
                serialize_value_for_hash(buffer, &tla_value::Value::SmallInt(c));
            }
        }
        Value::Tuple(t) => {
...
fn hash_value_ahash(hasher: &mut ahash::AHasher, value: &Value) {
    ...
    match value {
        ...
        Value::Bag(bag) => {
            // Same stream as the equivalent Func (counts are SmallInts).
            hasher.write_usize(bag.len());
            for (k, c) in bag.entries() {
                hash_value_ahash(hasher, k);
                hash_value_ahash(hasher, &tla_value::Value::SmallInt(c));
            }
        }
        Value::IntFunc(func) => {
