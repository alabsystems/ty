...
impl DiffSuccessor {
    ...
    pub fn materialize(&self, base: &ArrayState, registry: &VarRegistry) -> ArrayState {
        ...
        let values = Arc::make_mut(&mut result.values);
        for (idx, value) in &self.changes {
            ...
            values[idx_usize] = CompactValue::from(value.clone());
        }
    ...
    pub fn into_array_state(
        ...
        if let Some(fp) = precomputed_fp {
            ...
            for (idx, value) in self.changes {
                ...
                values[idx_usize] = CompactValue::from(value);
            }
    ...
    pub fn into_array_state_with_fp(
        ...
        let values = Arc::make_mut(&mut result.values);
        for (idx, value) in self.changes {
            values[idx.as_usize()] = CompactValue::from(value);
        }
    ...
    pub fn materialize_into(
        ...
        let values = Arc::make_mut(&mut target.values);
        values.clone_from_slice(&base.values);
        for (idx, value) in self.changes {
            values[idx.as_usize()] = CompactValue::from(value);
        }
...
/// Compute the successor fingerprint of `base` with `changes` applied, from
/// borrowed `(VarIndex, &Value)` pairs (#frame-fp-pop).
///
/// Bit-identical to [`compute_diff_fingerprint_with_xor`] /
/// [`DiffSuccessor::materialize`]'s internal fingerprinting (same
/// `incremental_fp_base` + salted per-variable XOR contribution swap + FNV
/// finalization), so a fingerprint computed at RAW emission time compares
/// exactly against the fingerprints the BFS worker later computes for the
/// materialized successors. Duplicate `VarIndex` entries are NOT supported
/// (matching the invariant of every production diff builder).
pub(crate) fn compute_changes_fingerprint<'a>(
    base: &ArrayState,
    changes: impl IntoIterator<Item = (VarIndex, &'a Value)>,
    registry: &VarRegistry,
) -> Fingerprint {
    let (mut combined_xor, base_value_fps) = base.incremental_fp_base(registry);
    for (idx, new_value) in changes {
