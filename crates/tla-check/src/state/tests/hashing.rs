...
fn test_nonenumerable_recordset_different_fields_different_hashes() {
    ...
    let h1 = fnv_hash(&Value::RecordSet(Rp::new(rs1)));
    let h2 = fnv_hash(&Value::RecordSet(Rp::new(rs2)));
...
fn test_nonenumerable_tupleset_different_components_different_hashes() {
    ...
    let h1 = fnv_hash(&Value::TupleSet(Rp::new(ts1)));
    let h2 = fnv_hash(&Value::TupleSet(Rp::new(ts2)));
    assert_ne!(
        h1, h2,
...
fn test_interval_hash_bigint_bounds_small_length() {
    ...
    let val_a = Value::Interval(Rp::new(iv_a));
    let val_b = Value::Interval(Rp::new(iv_b));
