...
fn test_worker_fp_parity_string() {
    let value = Value::String(Rp::from("hello"));
    let shared_fp = shared_value_fingerprint(&value);
    let mut memo = WorkerFpMemo::new();
    let worker_fp = memo.value_fingerprint(&value);
...
fn test_worker_fp_parity_model_value() {
    let value = Value::ModelValue(Rp::from("mv1"));
    let shared_fp = shared_value_fingerprint(&value);
    let mut memo = WorkerFpMemo::new();
    let worker_fp = memo.value_fingerprint(&value);
...
fn test_worker_fp_parity_func() {
    let value = Value::Func(Rp::new(FuncValue::from_sorted_entries(vec![
        ...
        (Value::SmallInt(3), Value::String(Rp::from("x"))),
    ])));
    let shared_fp = shared_value_fingerprint(&value);
    let mut memo = WorkerFpMemo::new();
    let worker_fp = memo.value_fingerprint(&value);
...
fn test_worker_fp_parity_nested() {
    ...
    let value = Value::Func(Rp::new(FuncValue::from_sorted_entries(vec![
        (Value::String(Rp::from("a")), set_val.clone()),
        (Value::String(Rp::from("b")), Value::Bool(false)),
    ])));
    let shared_fp = shared_value_fingerprint(&value);
    let mut memo = WorkerFpMemo::new();
    let worker_fp = memo.value_fingerprint(&value);
...
fn test_worker_array_state_round_trip_primitives() {
    ...
    state.set(VarIndex::new(2), Value::String(Rp::from("hello")));
...
fn test_worker_array_state_unshared_arcs() {
    ...
    let original = Value::Func(Rp::new(FuncValue::from_sorted_entries(vec![(
        Value::SmallInt(1),
        Value::Bool(true),
    )])));
    ...
    if let (Value::Func(orig_arc), Value::Func(new_arc)) = (&original, &unshared) {
        assert_ne!(
            tla_value::Rp::as_ptr(orig_arc),
            tla_value::Rp::as_ptr(new_arc),
