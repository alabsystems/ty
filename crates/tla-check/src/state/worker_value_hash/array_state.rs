...
pub(crate) fn unshare_value(value: &Value) -> Value {
    match value {
        ...
        Value::Int(n) => Value::Int(Rp::new((**n).clone())),
        Value::String(s) => Value::String(Rp::from(&**s)),
        Value::ModelValue(s) => Value::ModelValue(Rp::from(&**s)),
        Value::Func(f) => Value::Func(Rp::new((**f).clone())),
        Value::IntFunc(f) => Value::IntFunc(Rp::new((**f).clone())),
        Value::Seq(s) => Value::Seq(Rp::new((**s).clone())),
        Value::Tuple(t) => {
