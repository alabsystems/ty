...
///
/// SmallInt and Bool use precomputed lookup tables to eliminate byte-at-a-time
/// FP64 computation on the worker hot path.
pub(super) fn worker_value_fingerprint(value: &Value, memo: &mut FxHashMap<usize, u64>) -> u64 {
    ...
    // to the equivalent Func's additive fingerprint (BagValue invariant).
    if let Value::Bag(bag) = value {
        return bag.additive_fp();
    }

    if let Value::Func(func) = value {
        let key = Rp::as_ptr(func) as usize;
    ...
    if let Value::Seq(seq) = value {
        let key = Rp::as_ptr(seq) as usize;
        if let Some(&cached) = memo.get(&key) {
            return cached;
        }
    ...
    if let Value::Tuple(elems) = value {
        ...
        let key = Rp::as_ptr(elems).cast::<Value>() as usize;
        if let Some(&cached) = memo.get(&key) {
            return cached;
        }
