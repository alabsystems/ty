...
impl ArrayState {
    ...
    pub fn bind(&mut self, idx: VarIndex, value: Value, undo: &mut Vec<UndoEntry>) {
        ...
        let old = std::mem::replace(&mut values_mut[idx.as_usize()], compact);
        undo.push(UndoEntry {
            idx,
            old_value: old,
        });
    ...
    pub fn unbind(&mut self, undo: &mut Vec<UndoEntry>) {
        ...
        let values_mut = Arc::make_mut(&mut self.values);
        values_mut[entry.idx.as_usize()] = entry.old_value;
        self.fp_cache = None; // Invalidate fingerprint
    }
    ...
    /// fingerprint cache once at the end (ay solver pattern).
    ///
    /// # Arguments
    /// * `undo` - The undo stack
    ...
    pub fn unbind_to(&mut self, undo: &mut Vec<UndoEntry>, target_len: usize) {
        ...
        let values_mut = Arc::make_mut(&mut self.values);
        // Inline loop - avoid repeated function call overhead (ay solver pattern)
