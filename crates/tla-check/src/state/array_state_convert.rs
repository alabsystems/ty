...
use super::value_hash::{compact_value_fingerprint, finalize_fingerprint_xor};
...
impl ArrayState {
    ...
    pub fn from_state_with_fp(state: &State, registry: &VarRegistry) -> Self {
        ...
        for (name, value) in state.vars() {
            if let Some(idx) = registry.get(name) {
                ...
                let value_fp = compact_value_fingerprint(&compact);
                let salt = registry.fp_salt(idx);
                combined_xor ^= salt.wrapping_mul(value_fp.wrapping_add(1));
                values[idx_usize] = compact;
                assigned += 1;
            }
        }
        assert_eq!(
    ...
    pub fn from_successor_state(
        ...
        for (i, new_val) in values.iter().enumerate() {
            let old_val = &base_array.values[i];
            if new_val != old_val {
