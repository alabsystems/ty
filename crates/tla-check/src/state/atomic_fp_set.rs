...
/// # Thread Safety
///
/// Writers coordinate with atomic CAS operations. An inserter that encounters
/// a slot claimed by another writer waits for that slot to commit before
/// deciding whether the fingerprint is already present or whether probing
/// should continue. Readers encountering a concurrent write perform bounded
/// spinning (at most [`MAX_SPIN`] iterations with `spin_loop` hints). Multiple
/// threads may insert and query concurrently without external synchronization.
...
impl AtomicFpSet {
    ...
    ///
    /// Writers use atomic CAS to claim empty slots. When a probe encounters a
    /// slot that is still being written, insertion waits until that slot is
    /// committed before deciding whether it matches; otherwise a duplicate
    /// racing insert could skip the in-flight slot and claim a later empty one.
    #[must_use]
                    ...
                    match self.state[idx].compare_exchange(
                        ...
                        Err(actual) => {
                            ...
                            if actual == SLOT_WRITING {
                                if self.wait_for_writer_and_check(idx, fp_hi, fp_lo) {
                                    self.record_probe_stats(probe_len);
                                    return InsertResult::AlreadyPresent;
                                }
                            } else if actual == SLOT_COMMITTED
                                && self.read_committed_matches(idx, fp_hi, fp_lo)
                            {
                                self.record_probe_stats(probe_len);
                                return InsertResult::AlreadyPresent;
                            }
    ...
    /// Wait for a WRITING slot in the insert probe chain to commit, then check
    /// if it matches the target fingerprint.
    ///
    /// Unlike [`Self::spin_and_check`], this is intentionally not bounded by
    /// `MAX_SPIN`: insertion cannot safely skip an in-flight slot because it
    /// might be the same fingerprint and claiming a later empty slot would
    /// create a duplicate entry.
    #[inline]
    fn wait_for_writer_and_check(&self, idx: usize, target_hi: u64, target_lo: u64) -> bool {
...
mod tests {
    ...
    #[test]
    fn test_atomic_fp_set_insert_waits_for_in_flight_duplicate_slot() {
        let set = Arc::new(AtomicFpSet::new(128));
        let fp: u128 = 0xCAFE_BABE_DEAD_BEEF_1234_5678_9ABC_DEF0;
        let (fp_hi, fp_lo) = AtomicFpSet::split(fp);
        let idx = set.probe_start(fp);

        assert_eq!(
            set.state[idx].compare_exchange(
                SLOT_EMPTY,
                SLOT_WRITING,
        ...
        let handle = thread::spawn(move || worker_set.insert_if_absent(fp));

        while set.total_inserts() == 0 {
            thread::yield_now();
        }
        ...
        while Instant::now() < deadline {
            assert_eq!(
                set.len(),
                0,
                "insert must not claim a later empty slot while an earlier \
                 WRITING slot may contain the same fingerprint"
            );
            thread::yield_now();
        }

        set.high[idx].store(fp_hi, AtomicOrdering::Relaxed);
        set.low[idx].store(fp_lo, AtomicOrdering::Relaxed);
        set.state[idx].store(SLOT_COMMITTED, AtomicOrdering::Release);
        set.count.fetch_add(1, AtomicOrdering::Relaxed);

        assert_eq!(handle.join().unwrap(), InsertResult::AlreadyPresent);
        assert_eq!(set.len(), 1);
        assert_eq!(set.collect_all().iter().filter(|&&v| v == fp).count(), 1);
    }
