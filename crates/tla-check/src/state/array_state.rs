...
pub struct ArrayState {
    ...
    pub(crate) values: Arc<[CompactValue]>,
...
impl ArrayState {
    ...
    pub fn new(num_vars: usize) -> Self {
        ...
        ArrayState {
            values: Arc::from(vec![false_cv; num_vars]),
            fp_cache: None,
        }
    }
    ...
    pub fn from_values(values: Vec<Value>) -> Self {
        ...
        ArrayState {
            values: Arc::from(compact),
            fp_cache: None,
        }
    }

    /// Create an ArrayState from an exact compact-value snapshot.
    ///
    /// Unlike [`Self::from_values`], this does not canonicalize compact tags.
    /// It is used when restoring a collision witness whose exact representation
    /// (including legacy heap-tagged scalars and inline interned tags) is part of
    /// the duplicate-authorization contract.
    #[inline]
    pub(crate) fn from_compact_values(values: Arc<[CompactValue]>) -> Self {
        #[cfg(feature = "memory-stats")]
        {
            crate::value::memory_stats::inc_array_state();
            crate::value::memory_stats::inc_array_state_bytes(values.len());
    ...
    /// Clone the exact compact-value backing slice without cloning its slots.
    ///
    /// Subsequent mutation remains copy-on-write through `Arc::make_mut`, so a
    /// retained witness snapshot cannot be modified through the source state.
    #[inline]
    pub(crate) fn compact_values_arc(&self) -> Arc<[CompactValue]> {
    ...
    pub fn overwrite_from_slice(&mut self, values: &[Value]) {
        ...
        for (slot, v) in values_mut.iter_mut().zip(values.iter()) {
        ...
        I: IntoIterator<Item = Result<Value, E>>,
    {
        ...
        let values_mut = Arc::make_mut(&mut self.values);
        for slot in values_mut.iter_mut() {
            let value = iter
                .next()
    ...
    pub fn set(&mut self, idx: VarIndex, value: Value) {
        let values_mut = Arc::make_mut(&mut self.values);
        values_mut[idx.as_usize()] = CompactValue::from(value);
        self.fp_cache = None; // Invalidate cached fingerprint + per-var cache
    }
