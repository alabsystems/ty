...
impl ArrayState {
    ...
    pub fn set_with_registry(&mut self, idx: VarIndex, value: Value, registry: &VarRegistry) {
        if self.fp_cache.is_none() {
            let values_mut = Arc::make_mut(&mut self.values);
            values_mut[idx.as_usize()] = value.into();
            return;
        }
        ...
        let values_mut = Arc::make_mut(&mut self.values);
        values_mut[idx_usize] = value.into();
    }
    ...
    /// Compute the state fingerprint while OVERRIDING specific variables'
    /// per-value fingerprints (nested-set discovery A5 monitored dedup).
    ///
    /// `overrides[i]` is `Some(value_fp)` to use `value_fp` as variable `i`'s
    /// contribution instead of `compact_value_fingerprint(self.values[i])`, or
    /// `None` to use the default. The combination scheme is byte-identical to
    /// [`Self::fingerprint`] (salted XOR + `finalize_fingerprint_xor`), so when
    /// an override equals the variable's own `value_fingerprint` the result is
    /// byte-identical to the un-overridden fingerprint.
    ///
    /// Does NOT touch `self.fp_cache`: this is an out-of-band recompute used only
    /// when nested-set monitors are active, so the cached (un-monitored)
    /// fingerprint never leaks into the dedup set.
    #[must_use]
    pub(crate) fn fingerprint_with_var_fp_overrides(
        &self,
        registry: &VarRegistry,
        overrides: &[Option<u64>],
    ) -> Fingerprint {
        debug_assert_eq!(self.values.len(), overrides.len());
        let mut combined_xor = 0u64;
        for (i, cv) in self.values.iter().enumerate() {
            let vfp = match overrides.get(i).copied().flatten() {
                Some(fp) => fp,
                None => compact_value_fingerprint(cv),
            };
            let salt = registry.fp_salt(VarIndex::new(i));
            combined_xor ^= salt.wrapping_mul(vfp.wrapping_add(1));
        }
    ...
    /// Return the cached pre-finalization XOR accumulator, when present.
    ///
    /// A value of zero is deliberately returned as `Some(0)`: zero is both a
    /// valid accumulator and the marker used by final-fingerprint-only caches.
    /// Callers that restore it may safely install the cache; incremental users
    /// already recompute from values when the accumulator is zero and no
    /// per-slot fingerprints are available.
    #[inline]
    pub(crate) fn cached_combined_xor(&self) -> Option<u64> {
        self.fp_cache.as_ref().map(|cache| cache.combined_xor)
    }
    ...
    /// Populate this state's fingerprint cache incrementally from a predecessor.
    ///
    /// Computes the canonical fingerprint and pre-finalization `combined_xor`
    /// by XOR-ing the salted contributions of only the variables that differ
    /// from `base`, starting from the base's `combined_xor`. This is the exact
    /// algorithm `ArrayState::from_successor_state` uses (registry-index order,
    /// #158), so the resulting fingerprint is byte-identical — it just operates
    /// on an already-materialized successor `ArrayState` instead of round-
    /// tripping through a `State`. Installing `combined_xor` (not just the final
    /// fingerprint) keeps this state usable as an incremental base when it later
    /// becomes a parent in the BFS.
    ///
    /// Both states must be registered against the same `registry` (same length
    /// and index ordering).
    #[inline]
    pub fn ensure_incremental_fp_cache_from(&mut self, base: &ArrayState, registry: &VarRegistry) {
        debug_assert_eq!(self.values.len(), base.values.len());
        let (base_combined_xor, _) = base.incremental_fp_base(registry);
        let mut combined_xor = base_combined_xor;
        for (i, new_val) in self.values.iter().enumerate() {
            let old_val = &base.values[i];
            if new_val != old_val {
                let new_fp = compact_value_fingerprint(new_val);
                let old_fp = compact_value_fingerprint(old_val);
