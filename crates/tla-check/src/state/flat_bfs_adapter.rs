...
fn flat_layout_failure_summary(layout: &StateLayout) -> String {
    let mut scalar_vars = 0usize;
    let mut scalar_slots = 0usize;
    let mut compound_vars = 0usize;
    let mut compound_slots = 0usize;
    let mut dynamic_vars = 0usize;
    ...
    for var in layout.iter() {
        match &var.kind {
            ...
            | VarLayoutKind::FixedScalar { .. } => {
                scalar_vars += 1;
                scalar_slots += var.slot_count;
            }
            VarLayoutKind::Dynamic => {
                dynamic_vars += 1;
                dynamic_slots += var.slot_count;
            }
            _ => {
                compound_vars += 1;
                compound_slots += var.slot_count;
            }
        }
    }

    format!(
        "layout vars={} scalar_vars={} scalar_slots={} compound_vars={} compound_slots={} dynamic_vars={} dynamic_slots={} total_slots={} total_bytes={} fully_flat={} flat_primary_safe={}",
        layout.var_count(),
        scalar_vars,
        scalar_slots,
        compound_vars,
...
fn first_differing_flat_byte_summary(expected_slots: &[i64], actual_slots: &[i64]) -> String {
    for (slot_index, (expected, actual)) in
        expected_slots.iter().zip(actual_slots.iter()).enumerate()
    {
        if expected == actual {
            continue;
        }
        let expected_bytes = expected.to_ne_bytes();
        let actual_bytes = actual.to_ne_bytes();
        for (byte_index, (expected_byte, actual_byte)) in
            expected_bytes.iter().zip(actual_bytes.iter()).enumerate()
        {
            if expected_byte != actual_byte {
                return format!(
                    "first_differing_byte_offset={} slot={} expected_byte={} actual_byte={} expected_i64={} actual_i64={}",
                    slot_index * std::mem::size_of::<i64>() + byte_index,
                    slot_index,
                    expected_byte,
                    actual_byte,
                    expected,
    ...
    if expected_slots.len() != actual_slots.len() {
        return format!(
            "flat_slot_count_mismatch expected_slots={} actual_slots={}",
            expected_slots.len(),
            actual_slots.len()
        );
    }

    "first_differing_byte_offset=none flat_bytes_identical=true".to_string()
}
...
impl FlatBfsAdapter {
    ...
    pub(crate) fn diagnose_roundtrip(
                ...
                .try_to_array_state_with_fallback(&flat, registry, initial_state)
            {
                ...
                Err(err) => {
                    return Some(format!(
                        ...
                        flat_layout_failure_summary(self.bridge.layout())
                    ));
                }
            };
        let original_values = initial_state.values();
        ...
        let roundtrip_flat = self.bridge.to_flat(&roundtrip);
        let byte_summary =
            first_differing_flat_byte_summary(flat.buffer(), roundtrip_flat.buffer());
        ...
        {
            if orig != rt {
                let var_summary = layout
                    .var_layout(idx)
                    .map(|vl| {
                        format!(
                            "var[{idx}] name={} offset={} slots={} kind={:?}",
                            vl.name, vl.offset, vl.slot_count, &vl.kind
                        )
                    })
                    .unwrap_or_else(|| format!("var[{idx}] name=? offset=? slots=? kind=?"));
                let orig_ty = value_type_tag(orig);
                let rt_ty = value_type_tag(rt);
                return Some(format!(
                    "{var_summary} original_type={orig_ty} roundtrip_type={rt_ty}; {byte_summary}; {}",
                    flat_layout_failure_summary(layout)
                ));
            }
        }
        None
...
mod tests {
    ...
    fn test_verify_roundtrip_with_tuple_var_passes() {
        ...
        let mut array =
            ArrayState::from_values(vec![Value::SmallInt(0), Value::Tuple(Rp::from(tuple))]);
        let layout = Arc::new(infer_layout(&array, &registry));
        let bridge = FlatBfsBridge::new(layout);
        let mut adapter = FlatBfsAdapter::new(bridge);
    ...
    #[cfg_attr(test, ntest::timeout(10000))]
    #[test]
    fn test_diagnose_roundtrip_failure_includes_layout_and_byte_diff() {
        let registry = crate::var_index::VarRegistry::from_names(["flag"]);
        let mut array = ArrayState::from_values(vec![Value::SmallInt(2)]);
        let layout = Arc::new(StateLayout::new(&registry, vec![VarLayoutKind::ScalarBool]));
        let bridge = FlatBfsBridge::new(layout);
        let mut adapter = FlatBfsAdapter::new(bridge);

        assert!(!adapter.verify_roundtrip(&mut array, &registry));
        assert!(!adapter.roundtrip_verified());

        let detail = adapter
            .diagnose_roundtrip(&array, &registry)
            .expect("mismatched hand-built layout should produce diagnostics");
        assert!(detail.contains("name=flag"), "{detail}");
        assert!(detail.contains("original_type=SmallInt"), "{detail}");
        assert!(detail.contains("roundtrip_type=Bool"), "{detail}");
        assert!(detail.contains("first_differing_byte_offset=0"), "{detail}");
        assert!(detail.contains("expected_i64=2"), "{detail}");
        assert!(detail.contains("actual_i64=1"), "{detail}");
        assert!(detail.contains("layout vars=1"), "{detail}");
        assert!(detail.contains("scalar_slots=1"), "{detail}");
        assert!(detail.contains("total_bytes=8"), "{detail}");
    }
    ...
    fn test_adapter_ewd998_like_roundtrip() {
        ...
        let _array_for_eval = adapter.flat_to_array(&flat_init, &registry, None);
    ...
    fn test_verify_roundtrip_scalar_string_heap_tag() {
        ...
        let mut array = ArrayState::from_values(vec![Value::String(Rp::from("black"))]);
        let layout = Arc::new(infer_layout(&array, &registry));
        let bridge = FlatBfsBridge::new(layout);
        let mut adapter = FlatBfsAdapter::new(bridge);
    ...
    fn test_verify_roundtrip_string_keyed_modelvalue_domain() {
        ...
        let rm_state = Value::Func(Rp::new(FuncValue::from_sorted_entries(vec![
            (
                Value::ModelValue(Rp::from("rm1")),
                Value::String(Rp::from("working")),
            ),
            (
