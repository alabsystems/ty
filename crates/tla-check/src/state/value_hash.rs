...
pub fn value_fingerprint(value: &Value) -> u64 {
    ...
    // Bag: additive fingerprint precomputed at construction — bit-identical
    // to compute_func_additive_fp on the equivalent Func (BagValue invariant),
    // so compact and general representations of the same bag dedup together.
    if let Value::Bag(bag) = value {
        return bag.additive_fp();
    }
...
pub(super) use super::value_hash_state::{
    combined_xor_from_compact_array, compact_value_fingerprint, compute_fingerprint,
    compute_fingerprint_from_min_vals,
};
