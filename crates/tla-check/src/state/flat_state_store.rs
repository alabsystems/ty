...
impl FlatStateStore {
    ...
    /// Push an `ArrayState` directly into the flat arena.
    ///
    /// This avoids constructing a temporary boxed `FlatState` when the caller
    /// already knows the destination is an arena-backed flat frontier.
    /// Returns `false` when the state cannot be represented by this fixed
    /// layout, leaving the store unchanged.
    pub(crate) fn try_push_array_state(&mut self, state: &ArrayState) -> bool {
        if !FlatState::array_state_fits_layout(state, &self.layout) {
            return false;
        }
        ...
        let new_len = old_len
            .checked_add(self.slots_per_state)
            .expect("FlatStateStore::try_push_array_state: slot count overflow");
        self.arena.resize(new_len, 0);
        let written = FlatState::write_array_state_into(
            state,
            &self.layout,
            &mut self.arena[old_len..new_len],
        );
        if written == self.slots_per_state {
            self.state_count = idx + 1;
            true
        } else {
    ...
    /// Discard the first `states` states, shifting the remainder to the front.
    ///
    /// O(remaining) memmove; capacity is retained. Used by the BFS frontier
    /// to reclaim consumed-level memory at level boundaries without
    /// invalidating relative (cursor-based) addressing.
    ///
    /// # Panics
    ///
    /// Panics if `states > len()`.
    pub(crate) fn discard_prefix_states(&mut self, states: usize) {
        if states == 0 {
            return;
        }
        assert!(
            states <= self.state_count,
            "FlatStateStore::discard_prefix_states: discarding {states} of {} states",
            self.state_count
        );
        // Cannot overflow: `states * slots_per_state <= arena.len() <= usize::MAX`.
        let prefix_slots = states * self.slots_per_state;
        self.arena.copy_within(prefix_slots.., 0);
        self.arena.truncate(self.arena.len() - prefix_slots);
...
mod tests {
    ...
    #[cfg_attr(test, ntest::timeout(10000))]
    #[test]
    fn test_store_discard_prefix_states() {
        let (_reg, layout) = scalar_layout_3();
        let mut store = FlatStateStore::new(Arc::clone(&layout));
        ...
        // Zero discard is a no-op.
        store.discard_prefix_states(0);
        assert_eq!(store.len(), 3);
        assert_eq!(store.get(0), Some([1, 2, 3].as_slice()));

        // Discard the first two: remainder shifts to the front.
        store.discard_prefix_states(2);
        assert_eq!(store.len(), 1);
        assert_eq!(store.get(0), Some([7, 8, 9].as_slice()));
        // Capacity is retained.
        assert!(store.capacity() >= cap_before);

        // The store remains usable for pushes after a discard.
        store.push_buffer(&[10, 11, 12]);
        assert_eq!(store.len(), 2);
        assert_eq!(store.get(0), Some([7, 8, 9].as_slice()));
        assert_eq!(store.get(1), Some([10, 11, 12].as_slice()));

        // Discarding everything empties the store.
        store.discard_prefix_states(2);
        assert_eq!(store.len(), 0);
        assert!(store.is_empty());
    }
    ...
    #[cfg_attr(test, ntest::timeout(10000))]
    #[test]
    #[should_panic(expected = "discard_prefix_states")]
    fn test_store_discard_prefix_states_beyond_len_panics() {
        let (_reg, layout) = scalar_layout_3();
        let mut store = FlatStateStore::new(Arc::clone(&layout));
        store.push_buffer(&[1, 2, 3]);
        store.discard_prefix_states(2);
