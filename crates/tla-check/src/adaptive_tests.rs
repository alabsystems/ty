...
fn test_adaptive_allows_assume_only_models_without_init_next() {
    let _serial = crate::test_utils::acquire_interner_lock();
    // V1 vacuity-gate exemption (TRUST_VACUITY_GATE §1.A): an `ASSUME`-only
    // module declares NO checkable basis (no Init/Next/invariant/property), so
    // 0 reachable states is legitimate — it asked for nothing to be checked.
    // The narrowed assertion below verifies this stays `Success` (NOT
    // `Vacuous`), precisely because the model is the genuinely-degenerate case.
    let src = r#"
---- MODULE AssumeOnly ----
ASSUME TRUE
====
"#;
    ...
    match &result {
        ...
        // Narrowed assertion: ASSUME-only must NOT trip the V1 empty-reachable-set
        // verdict, because it declares no checkable basis.
        CheckResult::Vacuous { .. } => {
            panic!("ASSUME-only model must be EXEMPT from the V1 vacuity verdict (declares no checkable basis)");
        }
...
/// V1 vacuity gate (TRUST_VACUITY_GATE §1.A): `Init == FALSE` with an invariant
/// present admits zero reachable states despite declaring a checkable basis →
/// `Vacuous(EmptyReachableSet)`, NOT a silent `Success`.
#[cfg_attr(test, ntest::timeout(60_000))]
#[test]
fn test_v1_init_false_with_invariant_is_vacuous() {
    let _serial = crate::test_utils::acquire_interner_lock();
    let src = r#"
---- MODULE InitFalse ----
VARIABLE x
...
"#;
    ...
    match &result {
        CheckResult::Vacuous { reason, stats } => {
            assert_eq!(
                *reason,
                crate::vacuity::VacuityReason::EmptyReachableSet,
                "expected EmptyReachableSet reason"
            );
            assert_eq!(stats.states_found, 0, "Init == FALSE admits no states");
        }
        other => panic!("expected Vacuous(EmptyReachableSet) for Init == FALSE, got: {other:?}"),
    }
}
