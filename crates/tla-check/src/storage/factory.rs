...
pub enum StorageMode {
    ...
    Auto {
        /// Worker count driving the auto-selection heuristic.
        num_workers: usize,
    },
    ...
    pub fn create(config: StorageConfig) -> io::Result<Arc<dyn FingerprintSet>> {
        match config.mode {
            ...
            StorageMode::Disk => {
                ...
                let dir = config
                    .backing_dir
                    .unwrap_or_else(|| std::env::temp_dir().join("ty_disk_fps"));
            ...
            StorageMode::ShardedDisk => {
                ...
                let dir = config
                    .backing_dir
                    .unwrap_or_else(|| std::env::temp_dir().join("ty_sharded_disk_fps"));
                let storage =
            ...
            StorageMode::ShardedCas => {
                ...
                // Override via TY_FP_CAPACITY for specs needing more capacity.
