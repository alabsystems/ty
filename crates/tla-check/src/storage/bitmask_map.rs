...
///
/// Part of #4159 follow-up: now LIVE. `ea_precompute` routes specs whose
/// fairness leaves exceed tag 63 here (via
/// `StateBitmaskLookup::get_bitmask_words` / `ActionBitmaskLookup::get_bitmask_words`)
/// instead of the slow eval fallback.
...
pub(crate) trait StateBitmaskLookup {
    ...
    /// Whether this backend can serve the FULL multi-word `LiveBitmask`
    /// (tags >= 64), enabling the multi-word reconstruction fast path.
    ///
    /// Part of #4159 follow-up. Defaults to `false` (fail-closed): a backend
    /// that only stores the first u64 word must NOT advertise multi-word
    /// support, or a leaf with tag >= 64 would be silently read as `0`.
    fn multiword_capable(&self) -> bool {
        false
    }

    /// Fetch the full multi-word bitmask for a fingerprint, when this backend
    /// retains it. Returns `None` when absent OR when the backend cannot serve
    /// multi-word bits (fail-closed: caller must fall back to eval).
    fn get_bitmask_words(&self, _fp: &Fingerprint) -> Option<LiveBitmask> {
        None
    }
}
...
pub(crate) trait ActionBitmaskLookup {
    ...
    /// See [`StateBitmaskLookup::multiword_capable`]. Defaults to `false`.
    fn multiword_capable(&self) -> bool {
        false
    }

    /// See [`StateBitmaskLookup::get_bitmask_words`]. Defaults to `None`.
    fn get_bitmask_words(&self, _key: &(Fingerprint, Fingerprint)) -> Option<LiveBitmask> {
        None
    }
}
...
impl StateBitmaskLookup for StateBitmaskMap {
    ...
    fn multiword_capable(&self) -> bool {
        // Only the in-memory tier retains the full multi-word LiveBitmask; the
        // disk cold tier stores first_word only (fail-closed → eval fallback).
        matches!(self, StateBitmaskMap::InMemory(_))
    }

    fn get_bitmask_words(&self, fp: &Fingerprint) -> Option<LiveBitmask> {
        self.get_bitmask(fp).cloned()
    }
}
...
impl StateBitmaskLookup for FxHashMap<Fingerprint, u64> {
    ...
    // u64-only backend: no multi-word capability (default false / None applies).
}
...
impl ActionBitmaskLookup for ActionBitmaskMap {
    ...
    fn multiword_capable(&self) -> bool {
        matches!(self, ActionBitmaskMap::InMemory(_))
    }

    fn get_bitmask_words(&self, key: &(Fingerprint, Fingerprint)) -> Option<LiveBitmask> {
        self.get_bitmask(key).cloned()
    }
}
