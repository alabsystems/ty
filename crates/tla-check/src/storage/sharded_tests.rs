...
#[test]
fn test_prechecked_absent_insert_remains_authoritative_for_races() {
    let set = ShardedFingerprintSet::new(2);
    let fp = Fingerprint(0x1234_5678);

    assert_eq!(set.contains_checked(fp), LookupOutcome::Absent);
    assert_eq!(
        set.insert_prechecked_absent_checked(fp),
