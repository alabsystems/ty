...
pub struct MmapFingerprintSet {
    ...
    /// Cached integer entry threshold for the load factor hot path.
    pub(in crate::storage::mmap) max_load_entries: usize,
...
impl MmapFingerprintSet {
    ...
    pub fn new(capacity: usize, backing_dir: Option<PathBuf>) -> io::Result<Self> {
        ...
        Ok(MmapFingerprintSet {
            ...
            max_load_entries: Self::compute_max_load_entries(capacity, DEFAULT_MAX_LOAD_FACTOR),
    ...
    pub fn with_load_factor(
        ...
        set.max_load_entries = Self::compute_max_load_entries(capacity, load_factor);
        Ok(set)
    }

    fn compute_max_load_entries(capacity: usize, load_factor: f64) -> usize {
        if load_factor.is_nan() {
            return usize::MAX;
        }
        if load_factor <= 0.0 {
            return 0;
        ...
        let entries = (capacity as f64 * load_factor).ceil();
    ...
    pub(crate) fn recreate_empty(&self) -> io::Result<Self> {
        ...
        fresh.max_load_entries = self.max_load_entries;
        Ok(fresh)
    }
