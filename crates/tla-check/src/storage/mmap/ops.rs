...
use crate::storage::{BatchInsertedIndexAdmission, InsertOutcome, StorageFault};
...
impl MmapFingerprintSet {
    ...
    pub(in crate::storage::mmap) fn hash_index(&self, fp: Fingerprint) -> usize {
        self.hash_index_value(fp.0)
    }

    /// Compute the primary hash index for a raw fingerprint value.
    #[inline]
    pub(in crate::storage::mmap) fn hash_index_value(&self, fp: u64) -> usize {
        ...
        // values (#2674).
        let h = (fp & FP_MASK).wrapping_mul(0x9E3779B97F4A7C15); // Golden ratio constant
    ...
    pub fn insert(&self, fp: Fingerprint) -> Result<bool, MmapError> {
        self.insert_fingerprint_value(fp.0)
    }

    /// Insert a raw fingerprint value into the set.
    ///
    /// This is the hot path used by compiled raw batches. It intentionally
    /// avoids constructing `Fingerprint` unless an error payload needs it.
    pub(in crate::storage::mmap) fn insert_fingerprint_value(
        &self,
        fp: u64,
        ...
        let start_index = self.hash_index_value(fp);
        'retry: loop {
            let current_count = self.count.load(Ordering::Relaxed);
            let table_full = current_count >= self.max_load_entries;
    ...
    /// Insert fingerprints into caller-owned outcome scratch.
    ///
    /// The batch stops at the first storage fault and does not attempt suffix
    /// fingerprints after that fault.
    pub(in crate::storage::mmap) fn insert_fingerprints_checked_into(
        &self,
        fingerprints: &[Fingerprint],
        outcomes: &mut Vec<InsertOutcome>,
    ) {
        outcomes.clear();
        outcomes.reserve(fingerprints.len());
        for &fp in fingerprints {
            let outcome = match self.insert_fingerprint_value(fp.0) {
    ...
    pub(crate) fn insert_fingerprint_values_checked_into(
        ...
        for &fp in fingerprint_values {
            let outcome = match self.insert_fingerprint_value(fp) {
                Ok(true) => InsertOutcome::Inserted,
                Ok(false) => InsertOutcome::AlreadyPresent,
                Err(err) => {
                    self.record_error();
    ...
    /// Insert fingerprints and return only newly inserted input indices.
    ///
    /// The batch stops at the first storage fault and does not attempt suffix
    /// fingerprints after that fault.
    pub(crate) fn insert_fingerprints_inserted_indices_checked_into(
        &self,
        fingerprints: &[Fingerprint],
        ...
        for (idx, &fp) in fingerprints.iter().enumerate() {
            match self.insert_fingerprint_value(fp.0) {
                Ok(true) => {
                ...
                Err(err) => {
                    self.record_error();
                    admission.attempted += 1;
                    admission.fault = Some(StorageFault::new("mmap", "insert", err.to_string()));
                    break;
                }
            }
        }
    ...
    /// Insert raw fingerprint values and return only newly inserted input
    /// indices.
    ///
    /// The batch stops at the first storage fault and does not attempt suffix
    /// fingerprints after that fault.
    pub(crate) fn insert_fingerprint_values_inserted_indices_checked_into(
        &self,
        fingerprint_values: &[u64],
        admission: &mut BatchInsertedIndexAdmission,
    ) {
        ...
        for (idx, &fp) in fingerprint_values.iter().enumerate() {
            match self.insert_fingerprint_value(fp) {
                Ok(true) => {
                    admission.attempted += 1;
                    admission.inserted_indices.push(idx);
                }
