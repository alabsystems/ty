...
pub struct ShardedFingerprintSet<S: ShardBackend = FxHashSetShard> {
    ...
    /// stale cached value. See the parallel-checker safety-contracts design,
    /// Contract 3.
    count: AtomicUsize,
}
...
impl<S: ShardBackend> ShardedFingerprintSet<S> {
    ...
    /// Authoritative insert for callers that already observed this fingerprint
    /// absent. This skips the duplicate-friendly read precheck while preserving
    /// race safety through the shard's insert result.
    pub fn insert_prechecked_absent_checked(&self, fp: Fingerprint) -> InsertOutcome {
        let idx = self.shard_index(fp);
        let mut shard = self.shards[idx].write();
        match shard.shard_insert(fp) {
            Ok(true) => {
...
impl<S: ShardBackend> tla_mc_core::FingerprintSet<Fingerprint> for ShardedFingerprintSet<S> {
    ...
    fn insert_prechecked_absent_checked(&self, fp: Fingerprint) -> InsertOutcome {
        ShardedFingerprintSet::insert_prechecked_absent_checked(self, fp)
    }
