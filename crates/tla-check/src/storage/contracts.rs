...
/// Batch admission result for callers that only need newly inserted inputs.
///
/// `inserted_indices` contains input positions that were newly admitted, in
/// strictly increasing input order with no duplicates. `attempted` is the
/// number of prefix inputs actually attempted. On storage fault, suffix inputs
/// are not attempted and `fault` contains the first backend fault.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct BatchInsertedIndexAdmission {
    pub inserted_indices: Vec<usize>,
    pub attempted: usize,
    pub fault: Option<StorageFault>,
}

impl BatchInsertedIndexAdmission {
    pub fn with_capacity(capacity: usize) -> Self {
        Self {
            inserted_indices: Vec::with_capacity(capacity),
            attempted: 0,
            fault: None,
        }
    }
...
pub trait FingerprintSet: tla_mc_core::FingerprintSet<Fingerprint> {
    ...
    /// Insert a batch and return only the indices that were newly admitted.
    ///
    /// This preserves the same prefix-stop fault semantics as
    /// [`Self::insert_batch_checked`]. Returned inserted indices must be unique
    /// and strictly increasing by input position, while avoiding a per-input
    /// [`InsertOutcome`] vector for duplicate-heavy batches.
    fn insert_batch_inserted_indices_checked(
        ...
        self.insert_batch_inserted_indices_checked_into(fingerprints, &mut admission);
        admission
    }

    /// Insert a batch into caller-owned inserted-index scratch.
    ///
    /// Implementations must return unique inserted indices in strictly
    /// increasing input order within the attempted prefix.
    fn insert_batch_inserted_indices_checked_into(
        ...
        for (idx, &fp) in fingerprints.iter().enumerate() {
            match self.insert_checked(fp) {
                InsertOutcome::Inserted => {
                    admission.attempted += 1;
                    admission.inserted_indices.push(idx);
                }
    ...
    /// Insert a batch whose fingerprints have already passed a read-only
    /// absent prefilter.
    ///
    /// This preserves authoritative insertion semantics: if a racing writer
    /// inserts one of these fingerprints before this call, the result for that
    /// input is a duplicate, not an insertion. Backends can use this to skip
    /// redundant read-side prechecks in the mostly-new successor path.
    fn insert_prechecked_absent_batch_inserted_indices_checked_into(
        ...
        admission.inserted_indices.reserve(fingerprints.len());
        for (idx, &fp) in fingerprints.iter().enumerate() {
            match self.insert_prechecked_absent_checked(fp) {
                InsertOutcome::Inserted => {
                    admission.attempted += 1;
                    admission.inserted_indices.push(idx);
                }
    ...
    /// Insert raw 64-bit fingerprint values and return newly admitted indices.
    ///
    /// This is equivalent to
    /// [`Self::insert_batch_inserted_indices_checked`] over
    /// `Fingerprint(value)`, including unique/increasing inserted-index order
    /// and prefix-stop fault semantics.
    fn insert_batch_fingerprint_values_inserted_indices_checked(
        ...
        self.insert_batch_fingerprint_values_inserted_indices_checked_into(
            fingerprint_values,
            &mut admission,
        );
        admission
    ...
    /// Insert raw 64-bit fingerprint values into caller-owned admission
    /// scratch.
    ///
    /// Implementations must return unique inserted indices in strictly
    /// increasing input order within the attempted prefix.
    fn insert_batch_fingerprint_values_inserted_indices_checked_into(
        ...
        for (idx, &fp) in fingerprint_values.iter().enumerate() {
            match self.insert_checked(Fingerprint(fp)) {
                InsertOutcome::Inserted => {
                    admission.attempted += 1;
                    admission.inserted_indices.push(idx);
                }
    ...
    /// Reserve capacity for at least `additional` more fingerprints.
    ///
    /// This is a pure allocation hint: it never changes which fingerprints are
    /// admitted, the dedup decision, or any verdict — it only lets growable
    /// in-memory backends pre-size their table once instead of rehashing
    /// incrementally as states are discovered.
    ///
    /// The default is a no-op so fixed-capacity backends (mmap, disk, sharded,
    /// CAS), which are already sized at construction, are unaffected.
    fn reserve(&self, _additional: usize) {}
    ...
    // These methods bring TY's FingerprintSet to parity, ensuring future
