...
impl MmapTraceLocations {
    ...
    /// Estimate bytes reserved for the mmap trace-location table.
    pub(crate) fn estimate_memory_bytes(&self) -> usize {
        self.capacity.saturating_mul(TRACE_ENTRY_SIZE)
    }
        ...
        match MmapTraceLocations::insert(self, fp, offset) {
            ...
            Err(e) => {
                ...
                if !WARNED.swap(true, Ordering::Relaxed) {
                    eprintln!(
                        "WARNING: mmap trace location insert failed (counterexample traces may be incomplete): {e}"
                    );
                }
                false
            }
...
impl InMemoryTraceLocations {
    ...
    /// Estimate heap bytes for the fingerprint -> trace offset map.
    pub(crate) fn estimate_memory_bytes(&self) -> usize {
        let capacity = self.locations.read().capacity();
        if capacity == 0 {
            0
        } else {
            crate::memory::estimate_hashmap_bytes::<crate::state::Fingerprint, u64>(capacity)
        }
    }
}
...
impl TraceLocationsStorage {
    ...
    /// Estimate memory used by the selected trace-location backend.
    pub(crate) fn estimate_memory_bytes(&self) -> usize {
        match self {
            TraceLocationsStorage::InMemory(locs) => locs.estimate_memory_bytes(),
            TraceLocationsStorage::Mmap(locs) => locs.estimate_memory_bytes(),
        }
    }
}
