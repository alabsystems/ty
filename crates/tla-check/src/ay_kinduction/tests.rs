...
#[test]
fn test_kinduction_shared_engine_lane_evidence_is_frontend_neutral() {
    let evidence = kinduction_shared_engine_lane_evidence("TLA");

    assert!(evidence.contains("TLA ay_shared_engine_lane_metadata"));
    assert!(evidence.contains("lane=k_induction"));
...
"#;
    ...
    match result {
        ...
        KInductionResult::Proved { k } => {
            panic!(
                "x'=x+1 with Safety x<=100 should NOT be provable by k-induction, but got Proved at k={k}"
            );
        }
...
//
// Exercises the Bool sort inference path in ay_shared::infer_var_sorts.
