...
impl Config {
    ...
    /// Whether the model declares at least one of Init/Next/an invariant/a
    /// temporal property — i.e. there is *something to check*.
    ///
    /// Used by the V1 vacuity gate (empty reachable set). A module that
    /// declares none of these is the genuinely-degenerate `ASSUME`-only case
    /// and is EXEMPT from the empty-reachable-set verdict (it admits 0 states
    /// legitimately because it asked for nothing to be checked).
    ///
    /// Design: TRUST_VACUITY_GATE §1.A (V1).
    #[must_use]
    pub(crate) fn declares_checkable_basis(&self) -> bool {
        self.init.is_some()
            || self.next.is_some()
            || self.specification.is_some()
            || !self.invariants.is_empty()
            || !self.properties.is_empty()
            || !self.trace_invariants.is_empty()
    }
