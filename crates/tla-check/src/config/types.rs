...
impl LivenessExecutionMode {
    /// Returns `true` for the [`OnTheFly`](Self::OnTheFly) variant.
    #[must_use]
...
pub struct Config {
    ...
    ///
    /// This records config provenance only. TY follows TLC here: PROPERTY /
    ...
    ///
    /// When set, TLC (and TY) evaluates this operator for each state in
    ...
    /// Environment variables TY_USE_AY and TY_FORCE_AY can override this.
    /// Default: Auto (use ay if analysis recommends it).
    ...
    /// `TY_AUTO_POR` env var. When `None` (default), auto-POR follows the
    ...
    /// enable or `Some(false)` to force disable (equivalent to
    /// `TY_NO_FLAT_BFS=1`).
    ///
    ...
    /// successfully. Set to `Some(true)` to force enable or `Some(false)` to
    /// force disable (equivalent to `TY_NO_COMPILED_BFS=1`).
    ///
    ...
    /// Per-state successor materialization cap override (audit finding #12).
    ///
    /// Outer `None` means "use the env/default cap" (`TY_PER_STATE_SUCCESSOR_CAP`
    /// or the ~50M runaway-memory guard). `Some(inner)` is an explicit override:
    /// `Some(None)` disables the cap entirely, `Some(Some(n))` caps at `n`.
    ///
    /// This replaces the former process-global cap atomic: keeping the cap on
    /// the per-checker `Config` means a test injecting a tiny cap can never leak
    /// into a concurrently-running checker, so no global quiescence lock is
    /// needed and the value is fully parallel-test-safe.
    pub per_state_successor_cap: Option<Option<usize>>,
}
...
pub enum ConfigError {
    ...
    InvalidSyntax {
        /// 1-based source line of the directive.
        line: usize,
        /// The directive keyword that was malformed.
