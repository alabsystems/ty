...
pub enum ActionLabelValidationError {
    ...
    ParamMismatch {
        /// The label whose params mismatch.
        label: String,
        /// Param names the mapping expects.
        expected: Vec<String>,
        /// Param names the trace provided.
        got: Vec<String>,
    },
    /// A param value could not be decoded into a TLA+ value.
    #[error("param decode error for label {label}, param {param}: {source}")]
    ParamDecodeError {
        /// The label being validated.
        label: String,
        /// The param whose value failed to decode.
        param: String,
        /// Underlying decode error.
        #[source]
        source: JsonValueDecodeError,
    },
    /// Evaluating the action operator failed.
    ...
    /// The action operator evaluated to a non-boolean value.
    #[error("action label {label} evaluated to non-boolean value")]
    NotBoolean {
        /// The label being validated.
        label: String,
    },
    /// A stutter transition matched but the mapping disallows stutter.
