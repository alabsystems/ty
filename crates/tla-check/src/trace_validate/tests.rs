...
fn int_step(vars: &[(&str, i64)]) -> TraceStep {
    TraceStep {
        index: None,
        state: vars
            .iter()
            .map(|(name, value)| ((*name).to_string(), JsonValue::Int(*value)))
            .collect(),
        action: None,
    }
}
...
#[test]
#[timeout(10_000)]
fn test_observation_constraint_full_mode_rejects_missing_spec_variable() {
    let spec_vars: Vec<Arc<str>> = vec![Arc::from("x"), Arc::from("y")];
    let step = int_step(&[("x", 1)]);

    // Default (full-observation) mode: an omitted spec variable is an error.
    let err = ObservationConstraint::from_trace_step(&step, 3, &spec_vars, false).unwrap_err();
    assert!(
        matches!(
            err,
            TraceValidationError::MissingSpecVariable { step: 3, ref variable } if variable == "y"
        ),
        "expected MissingSpecVariable for \"y\", got: {err}"
    );
}

#[test]
#[timeout(10_000)]
fn test_observation_constraint_partial_mode_constrains_observed_subset_only() {
    let spec_vars: Vec<Arc<str>> = vec![Arc::from("x"), Arc::from("y")];
    let step = int_step(&[("x", 1)]);

    let obs = ObservationConstraint::from_trace_step(&step, 3, &spec_vars, true).unwrap();
    // Unobserved y is unconstrained...
    assert!(obs.matches(
        &State::new()
            .with_var("x", Value::int(1))
            .with_var("y", Value::int(7))
    ));
    assert!(obs.matches(
        &State::new()
            .with_var("x", Value::int(1))
            .with_var("y", Value::int(8))
    ));
    // ...but the observed x still filters.
    assert!(!obs.matches(
        &State::new()
            .with_var("x", Value::int(2))
            .with_var("y", Value::int(7))
    ));
}
...
#[test]
#[timeout(10_000)]
fn test_observation_constraint_partial_mode_allows_empty_observation() {
    let spec_vars: Vec<Arc<str>> = vec![Arc::from("x")];
    let step = int_step(&[]);

    let obs = ObservationConstraint::from_trace_step(&step, 0, &spec_vars, true).unwrap();
    // An empty observation constrains nothing.
    assert!(obs.matches(&State::new().with_var("x", Value::int(42))));
}

#[test]
#[timeout(10_000)]
fn test_observation_constraint_unknown_variable_errors_in_both_modes() {
    let spec_vars: Vec<Arc<str>> = vec![Arc::from("x")];
    let step = int_step(&[("z", 1)]);

    for allow_partial in [false, true] {
        let err = ObservationConstraint::from_trace_step(&step, 0, &spec_vars, allow_partial)
            .unwrap_err();
        assert!(
            matches!(
                err,
                TraceValidationError::UnknownTraceVariable { ref variable } if variable == "z"
            ),
            "expected UnknownTraceVariable for \"z\" (allow_partial={allow_partial}), got: {err}"
        );
    }
}
