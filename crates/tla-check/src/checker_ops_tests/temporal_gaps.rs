...
// ===========================================================================
// contains_temporal_standalone parity tests
// ===========================================================================

/// `contains_temporal_standalone` detects the `Apply(WF_/SF_)` form.
///
...
/// The standalone scanner must match that behavior because the Apply form can
/// arise from INSTANCE expansion or substitution.
#[test]
fn contains_temporal_standalone_detects_apply_wf_sf() {
    ...
    assert!(
        ...
        "Apply(WF_) should be detected by contains_temporal_standalone"
    );
    ...
    assert!(
        ...
        "Apply(SF_) should be detected by contains_temporal_standalone"
    );

    assert!(
        is_liveness_conjunct(&wf_apply),
        "Apply(WF_) should be classified as liveness by is_liveness_conjunct"
    );
}
...
/// The shared scanner must also detect the Apply(WF_/SF_) form.
#[test]
fn is_liveness_conjunct_always_containing_apply_wf() {
    use tla_core::span::{Span, Spanned};
    ...
    assert!(
        ...
        "Always(Apply(WF_)) should be classified as liveness"
    );
}
