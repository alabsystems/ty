...
fn implied_action_trigger_names(ctx: &EvalCtx, term: &EvalImpliedActionTerm) -> Vec<Vec<String>> {
    let mut triggers: Vec<Vec<String>> = term
        .truth_if_unchanged
        .iter()
        .map(|trigger| {
            let mut names: Vec<String> = trigger
                .iter()
                .map(|&idx| ctx.var_registry().name(idx).to_string())
                .collect();
            names.sort();
            names
        })
...
#[test]
fn classify_action_subscript_scalar_unchanged_gets_truth_trigger() {
    let src = r#"
---- MODULE ClassifyScalarUnchangedTrigger ----
VARIABLE x
Prop == [][FALSE]_x
===="#;
    let (op_defs, ctx, _root) = setup_for_classification(src);
    let result = classify_property_safety_parts(&ctx, &["Prop".to_string()], &op_defs);

    assert_eq!(result.eval_implied_actions.len(), 1);
    assert!(
        implied_action_trigger_names(&ctx, &result.eval_implied_actions[0])
            .contains(&vec!["x".to_string()]),
        "[FALSE]_x should be proven true when x is unchanged"
    );
}

#[test]
fn classify_angle_action_subscript_does_not_get_unchanged_truth_trigger() {
    let src = r#"
---- MODULE ClassifyAngleNoTruthTrigger ----
VARIABLE x
Prop == []<<FALSE>>_x
===="#;
    let (op_defs, ctx, _root) = setup_for_classification(src);
    ...
    assert_eq!(result.eval_implied_actions.len(), 1);
    assert!(
        implied_action_trigger_names(&ctx, &result.eval_implied_actions[0]).is_empty(),
        "<<FALSE>>_x must not be proven true merely because x is unchanged"
    );
}

#[test]
fn classify_action_subscript_tuple_unchanged_requires_all_vars() {
    let src = r#"
---- MODULE ClassifyTupleUnchangedTrigger ----
VARIABLE x, y
Prop == [][FALSE]_<<x, y>>
===="#;
    let (op_defs, ctx, _root) = setup_for_classification(src);
    ...
    assert_eq!(
        ...
        "[FALSE]_<<x, y>> should require both x and y unchanged"
    );
}

#[test]
fn classify_implication_conjunctive_antecedent_gets_false_trigger() {
    let src = r#"
---- MODULE ClassifyConjAntecedentTrigger ----
EXTENDS Integers
VARIABLE x, y
Prop == [][(x' # x /\ y = 0) => FALSE]_<<x, y>>
===="#;
    let (op_defs, ctx, _root) = setup_for_classification(src);
    let result = classify_property_safety_parts(&ctx, &["Prop".to_string()], &op_defs);
    ...
    assert!(
        implied_action_trigger_names(&ctx, &result.eval_implied_actions[0])
            .contains(&vec!["x".to_string()]),
        "unchanged x should prove the antecedent false"
    );
}
