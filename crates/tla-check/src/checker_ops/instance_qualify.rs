...
pub(crate) fn qualify_instance_ops(
    ...
    qualify_instance_ops_with_bound(ctx, target, module_name, expr, &[])
}

/// Like [`qualify_instance_ops`], but with `extra_bound` names pre-seeded as
/// bound variables so they are NEVER qualified into `ModuleRef`s.
///
/// This is required when qualifying an instanced operator's body BEFORE its
/// formal parameters are substituted: the formal-parameter names are outer-scope
/// placeholders that a later substitution replaces with the actual (outer-module)
/// argument expressions, so they must not be rewritten to `M!param` even if the
/// instanced module happens to define an operator of the same name. Seeding them
/// as bound is the exact same immunity the old substitute-then-qualify order got
/// for free (it substituted params away before qualifying).
pub(crate) fn qualify_instance_ops_with_bound(
    ctx: &EvalCtx,
    target: &ModuleTarget,
    module_name: &str,
    expr: Spanned<Expr>,
    ...
    for name in extra_bound {
        *bound_vars.entry(name.clone()).or_insert(0) += 1;
    }
...
impl QualifyInstanceOpsFolder<'_> {
    fn fold_apply_expr(&mut self, op: Spanned<Expr>, args: Vec<Spanned<Expr>>) -> Expr {
        match op.node {
            Expr::Ident(name, name_id) => {
                if !self.bound_vars.contains_key(&name)
                    && self.ctx.get_instance_op(&self.module_name, &name).is_some()
                {
                    return Expr::ModuleRef(self.target.clone(), name, self.fold_vec(args));
                }
                Expr::Apply(
                    Box::new(Spanned::new(Expr::Ident(name, name_id), op.span)),
                    self.fold_vec(args),
                )
            }
            Expr::StateVar(name, idx, name_id) => {
                ...
                Expr::Apply(
                    Box::new(Spanned::new(Expr::StateVar(name, idx, name_id), op.span)),
                    self.fold_vec(args),
                )
            }
            node => Expr::Apply(
                Box::new(Spanned::new(self.fold_expr_inner(node), op.span)),
                self.fold_vec(args),
            ),
        }
    }
...
impl ExprFold for QualifyInstanceOpsFolder<'_> {
    fn fold_expr(&mut self, expr: Spanned<Expr>) -> Spanned<Expr> {
        ...
        let node = match expr.node {
            Expr::Apply(op, args) => self.fold_apply_expr(*op, args),
            node => self.fold_expr_inner(node),
        };
        let result = Spanned { node, span };
        for n in &new_bounds {
        ...
        match target {
            ModuleTarget::Named(name)
                if !self.bound_vars.contains_key(&name)
                    && self.ctx.get_instance_op(&self.module_name, &name).is_some() =>
            {
                ModuleTarget::Chained(Box::new(Spanned::dummy(Expr::ModuleRef(
                    self.target.clone(),
                    name,
                    Vec::new(),
                ))))
