...
pub(crate) fn fairness_to_live_expr(
    ...
    // Whole-Next fast path: `WF_vars(Next)` / `SF_vars(Next)` where the fairness
    // action IS the config's complete next-state relation. Its `ENABLED` leaf is
    // decided below by scanning the (complete-for-the-full-relation) BFS successor
    // set for a subscript change instead of a from-scratch Next re-enumeration per
    // state. The `<<Next>>_vars` ActionPred is left intact (its per-transition
    // cost is small and dropping it churned the provenance/enabled-group plan
    // structure); the win is entirely in the ENABLED short-circuit.
    let is_whole_next = converter.is_config_next_name(action_name);
    ...
    let enabled_tag = converter.alloc_tag();
    if is_whole_next {
        // ENABLED(<<Next>>_vars) = ∃ successor with a subscript change. The BFS
        // successor set IS the complete Next-successor set (unlike a sub-action,
        // whose witness may be deduplicated), so the inline evaluator can decide
        // it by scanning the already-computed successors for a subscript change —
        // no from-scratch Next re-enumeration per state.
        converter.record_whole_next_enabled_tag(enabled_tag);
    }
