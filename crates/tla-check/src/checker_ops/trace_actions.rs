...
/// Attribute TLC-parity action labels to a replay-validated counterexample trace.
///
/// Used by the CLI when a symbolic lane (BMC / k-Induction / PDR) wins the
/// fused verdict race and its explicit-evaluator-confirmed counterexample is
/// promoted to a standard `CheckResult::InvariantViolation`: the replayed
/// trace carries bare states, and without labels the report prints `<Action>`
/// placeholders instead of the `<Name line N, col N to line N, col N of
/// module M>` labels the BFS lane produces (#2470/#2920 parity).
///
/// Best-effort by design: on any setup/resolution failure the trace keeps its
/// placeholder labels — this function never turns a confirmed violation into
/// an error. Labels are only computed when the trace has none yet.
pub fn label_trace_actions(
    module: &tla_core::ast::Module,
    extended_modules: &[&tla_core::ast::Module],
    config: &crate::config::Config,
    file_paths: &[(FileId, PathBuf)],
    trace: &mut crate::check::Trace,
) {
    if trace.states.len() < 2 || !trace.action_labels.is_empty() {
        return;
    }
    let Some(next) = config.next.as_deref() else {
        return;
    };
    let setup = crate::checker_setup::setup_checker_modules(
        module,
        extended_modules,
        config,
        &crate::checker_setup::SetupOptions {
            load_instances: true,
        },
    );
    if setup.setup_error.is_some() {
        return;
    }
    let mut ctx = setup.ctx;
    ctx.resolve_state_vars_in_loaded_ops();
    if crate::bind_constants_from_config(&mut ctx, config).is_err() {
        return;
    }
    let next_name = ctx.resolve_op_name(next).to_string();
    let next_def = ctx.get_op(&next_name).cloned();
    let file_id_to_path: FxHashMap<FileId, PathBuf> = file_paths.iter().cloned().collect();
    let label_ctx = ActionLabelCtx {
        next_name: &next_name,
        next_def: next_def.as_deref(),
        file_id_to_path: &file_id_to_path,
        module_name: &module.name.node,
    };
    trace.action_labels = identify_action_labels(&mut ctx, &label_ctx, &trace.states);
}
...
fn identify_action_for_transition(
    ...
    *ctx.next_state_mut() = Some(Arc::new(next_env));
    ...
    debug_eprintln!(
        _eval_errors > 0 && fired_name.is_none() && crate::check::debug::ty_debug(),
