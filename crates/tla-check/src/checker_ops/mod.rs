...
mod implied_external_seeds;
mod implied_verdict_cache;
...
pub(crate) use implied_action_bytecode::attach_eval_implied_action_bytecode;
pub(crate) use implied_verdict_cache::census_implied_verdict_len;
pub(crate) use implied_verdict_cache::debug_summary as implied_verdict_cache_debug_summary;
pub(crate) use instance_qualify::qualify_instance_ops_with_bound;
pub(crate) use invariants::*;
pub(crate) use prepare::*;
pub(crate) use property_classify::*;
pub(crate) use property_plan::{
    flatten_property_module_refs, plan_property_terms, PlannedPropertyTerm,
};
...
pub use trace_actions::label_trace_actions;
...
/// - `WF_...(...)` and `SF_...(...)` apply forms are treated as temporal
/// - bare `Ident` is not treated as temporal (semantic safety net via `get_level_with_ctx`)
...
thread_local! {
    /// Projection analysis for the VIEW used by the current semantic input.
    ///
    /// The VIEW name alone is not a cross-module identity: two independently
    /// parsed modules can both define `View` with different variable slots.
    /// The run-boundary reset clears this cache before a fresh module is
    /// checked so an old projection can never change its state equivalence.
    static CACHED_VIEW_PROJECTION: std::cell::RefCell<Option<(String, ViewProjection)>> =
        const { std::cell::RefCell::new(None) };
}

pub(crate) fn clear_view_projection_cache() {
    CACHED_VIEW_PROJECTION.with(|cache| *cache.borrow_mut() = None);
}
...
pub(crate) fn compute_view_fingerprint_array(
    ...
    // Thread-local cache for the VIEW projection analysis. The semantic-run
    // boundary clears it before a separately parsed module can reuse a name.
    let fast_result = CACHED_VIEW_PROJECTION.with(|cell| {
...
/// Evaluate a VIEW operator for an `ArrayState` and return the canonical VIEW
/// payload used to authorize same-fingerprint duplicate admission.
pub(crate) fn compute_view_value_array(
    ctx: &mut EvalCtx,
    array_state: &crate::state::ArrayState,
    view_name: &str,
    bfs_level: u32,
) -> Result<crate::Value, CheckError> {
    let _scope_guard = ctx.scope_guard();
    let saved_level = ctx.get_tlc_level();
    ctx.set_tlc_level(bfs_level);
