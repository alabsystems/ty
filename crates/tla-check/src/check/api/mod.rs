...
pub use self::stats::{
    ...
    PropertyCheckStats, SymmetryReductionStats,
};
...
pub enum CheckResult {
    ...
    InvariantViolation {
        ...
        invariant: String,
        /// Counterexample trace ending in the violating state.
        trace: Trace,
        /// Statistics at the point the violation was found.
        stats: CheckStats,
    },
    ...
    PropertyViolation {
        ...
        property: String,
        /// The kind of property violation.
    ...
    Deadlock {
        ...
        trace: Trace,
        /// Statistics at the point the deadlock was found.
        stats: CheckStats,
    },
    /// Vacuity gate triggered: a "Success" that rests on an empty / never-
    /// exercised / unsatisfiable basis is not Success. Distinct from a property
    /// failure — maps to exit code 3 (`VACUOUS`), not 1 (`FAILED`).
    ///
    /// Today only V1 (`VacuityReason::EmptyReachableSet`) is constructed by the
    /// core checker. V2/V3 surface as `stats.vacuity_warnings` and are only
    /// promoted to this verdict at the CLI policy layer under `--strict-vacuity`.
    ///
    /// Design: TRUST_VACUITY_GATE §1.A.
    Vacuous {
        /// Why the run is vacuous.
        reason: crate::vacuity::VacuityReason,
        /// Statistics at the point the vacuity verdict was reached.
        stats: CheckStats,
    },
    ...
    /// Part of #2851.
    Error {
        /// The error that aborted checking.
        error: CheckError,
        /// Statistics at the point the error occurred.
        stats: CheckStats,
...
impl CheckResult {
    ...
    /// Attach PROPERTY-check telemetry to this result.
    #[must_use]
    pub(crate) fn with_property_check_stats(mut self, property_check: PropertyCheckStats) -> Self {
        self.stats_mut().property_check = property_check;
        self
    }

    /// Attach summarizer-ready backend capability/admission evidence to this result.
    #[must_use]
    pub(crate) fn with_backend_capability_report(
        mut self,
...
pub enum LimitType {
    ...
    Disk,
    /// Wall-clock deadline (time budget) elapsed before exploration finished.
    /// Part of #3: time backstop for the BFS loop. The result is a sound
    /// partial — properties remain unresolved (Unknown), never reported safe.
    Time,
}
