...
impl ModelChecker<'_> {
    ...
    pub fn simulate(&mut self, sim_config: &SimulationConfig) -> SimulationResult {
        ...
        // Construction and execution may be separated by another semantic
        // run on this thread. Simulation must not inherit its pointer-keyed
        // analysis facts or proof-observer samples.
        ...
        for _trace_num in 0..sim_config.num_traces {
            ...
            let mut current_state: Option<State> = if use_split_actions {
                ...
                Some(initial_states.state_at(init_idx, &registry))
            };
