...
struct ResumeGuardErrorStatsFlush {
    disarmed: bool,
    run_diagnostics: std::sync::Arc<crate::run_diagnostics::RunDiagnostics>,
}

impl ResumeGuardErrorStatsFlush {
    fn new(run_diagnostics: std::sync::Arc<crate::run_diagnostics::RunDiagnostics>) -> Self {
        Self {
            disarmed: false,
            run_diagnostics,
        }
...
impl Drop for ResumeGuardErrorStatsFlush {
    fn drop(&mut self) {
        if !self.disarmed {
            let count = self.run_diagnostics.take_suppressed_guard_errors();
...
impl<'a> ModelChecker<'a> {
    ...
    pub fn check_with_resume<P: AsRef<std::path::Path>>(
        ...
        // Resume is also a fresh public execution boundary; the checker may
        // have waited while another model populated this thread's caches.
        ...
        // them for the run (inside stack_safe — it may run on a new thread).
        // Legacy globals are NOT reset: concurrent runs own their counts.
        self.run_diagnostics.reset();
        let run_diagnostics = std::sync::Arc::clone(&self.run_diagnostics);
        let mut guard_error_stats_flush =
            ResumeGuardErrorStatsFlush::new(std::sync::Arc::clone(&self.run_diagnostics));

        let result = crate::eval::stack_safe(|| {
            let _diag_scope = crate::run_diagnostics::RunDiagnosticsScope::enter(run_diagnostics);
            self.check_with_resume_inner(checkpoint_dir.as_ref())
        });
        let suppressed_guard_errors = self.run_diagnostics.take_suppressed_guard_errors();
    ...
    fn check_with_resume_inner(
        ...
        self.liveness_cache.witness_intern.clear();
        ...
        Ok(match outcome {
            ...
            BfsLoopOutcome::Complete {
                depth_limit_reached,
                frontier_exhausted: _,
            } => self.finish_check_after_bfs(
                depth_limit_reached.then_some(super::LimitType::Depth),
                true,
                0,
            ),
        })
