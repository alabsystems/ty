...
#[test]
fn test_implicit_default_eval_mode_is_pinned_by_origin() {
    let module = parse_module(
        "\
---- MODULE TirDefaultOrigin ----
VARIABLE x
Init == x = 0
Next == x' = x
====",
    );
    let implicit = TirParityState {
        mode: TirMode::Eval,
    ...
    let explicit_all = TirParityState {
        mode: TirMode::Eval,
    ...
    assert!(implicit.is_implicit_default_eval_mode());
    assert!(
