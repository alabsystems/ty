...
pub(in crate::check::model_checker) struct TirParityState {
    ...
    /// True when eval/parity was selected explicitly rather than installed as
    /// the sequential checker's ordinary default fallback.
    pub(super) explicitly_requested: bool,
    /// When true, all operators are selected (TY_TIR_EVAL=all).
    ...
    /// names with their concrete values when `TY_PARTIAL_EVAL=1` /
...
impl TirParityState {
    ...
    /// 1. `TY_TIR_EVAL=<ops>|all` → Eval mode with explicit selection
    /// 2. `TY_TIR_PARITY=<ops>` → Parity mode
    ...
    pub(in crate::check::model_checker) fn from_env(
        ...
        // Explicit TY_TIR_EVAL takes priority. Env parsing is centralized
        ...
        // TY_TIR_PARITY for shadow-check mode.
    ...
    /// Whether this is the ordinary implicit TIR fallback installed when no
    /// TIR mode was requested by the user.
    ///
    /// Backend-specific optimizations may use this distinction only when an
    /// earlier canonical backend provably owns the selected operator. Explicit
    /// eval/parity requests must remain observable and therefore return false.
    pub(in crate::check::model_checker) fn is_implicit_default_eval_mode(&self) -> bool {
        self.mode == TirMode::Eval
    ...
    /// Borrow the root module and dependencies without cloning.
    ///
    /// Used by bytecode-VM ASSUME evaluation, which only needs read access
    /// to compile constant expressions against the module namespace.
    pub(in crate::check::model_checker) fn modules(&self) -> (&Module, &[Module]) {
        (&self.root, &self.deps)
    }
    ...
    /// this, `TY_TIR_EVAL=TypeOK` would also turn on TIR for unrelated
    /// liveness properties.
    ...
    #[cfg(test)]
    pub(in crate::check) fn test_parity_selected<I, S>(
        root: Module,
        deps: Vec<Module>,
        selected_ops: I,
    ) -> Self
