...
impl ModelChecker<'_> {
    ...
    ///
    /// Delegates to `checker_ops::check_assumes_with_modules` — the single
    /// shared implementation used by both sequential and parallel checkers
    /// (Part of #810). The sequential checker passes its module set so
    /// iteration-heavy constant ASSUMEs can run through the bytecode VM
    /// (AST tree-walker remains the canonical fallback).
    pub(super) fn check_assumes(&self) -> Option<CheckResult> {
        let modules = self
            .tir_parity
            .as_ref()
            .map(super::tir_parity::TirParityState::modules);
        crate::checker_ops::check_assumes_with_modules(&self.ctx, &self.module.assumes, modules)
