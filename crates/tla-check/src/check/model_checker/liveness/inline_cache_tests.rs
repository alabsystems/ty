...
fn liveness_regen_trip_retains_disk_adjacency_but_disables_inline_recording() {
    ...
    assert_eq!(checker.liveness_cache.inline_property_plans.len(), 1);
    ...
    let before_property_entry = checker.liveness_cache_estimated_bytes();
    checker
        .record_inline_liveness_results(
            current_fp,
            &current_arr,
            &[(next_arr.clone(), next_fp)],
            &[],
        )
        .expect("property inline result");
    let after_property_entry = checker.liveness_cache_estimated_bytes();
    assert!(
    ...
    checker
        .liveness_cache
        .successors
        .insert(current_fp, vec![next_fp])
        .expect("successor cache entry");
    checker
        .liveness_cache
        .inline_state_bitmasks
        .get_or_insert_default(current_fp);
    checker
        .liveness_cache
        .inline_action_bitmasks
        .get_or_insert_default((current_fp, next_fp));
    assert!(checker.liveness_cache_estimated_bytes() > after_property_entry);

    crate::liveness::seed_regen_thread_local_storage_for_test(current_fp, next_fp);
    assert!(crate::liveness::census_enabled_cache_len() > 0);
    assert!(crate::liveness::census_subscript_cache_len() > 0);
    assert!(crate::liveness::census_action_pred_cache_len() > 0);
    assert!(crate::liveness::census_scan_pred_len() > 0);

    let pre_trip_estimate = checker.liveness_cache_estimated_bytes();
    checker.trip_liveness_regen(pre_trip_estimate);
    ...
    assert_eq!(checker.liveness_cache.inline_property_plans.capacity(), 0);
    assert_eq!(crate::liveness::census_enabled_cache_len(), 0);
    assert_eq!(crate::liveness::census_subscript_cache_len(), 0);
    assert_eq!(crate::liveness::census_action_pred_cache_len(), 0);
