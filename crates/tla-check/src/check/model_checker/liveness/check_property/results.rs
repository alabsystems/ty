...
impl ModelChecker<'_> {
    pub(in crate::check::model_checker::liveness) fn map_liveness_result(
        ...
        match result {
            ...
            // SOUNDNESS (#liveness-fp-only-false-hold): a candidate violation
            // could not be re-verified because concrete states are unavailable.
            // The grouped-plan caller is expected to materialize the fp-only
            // state cache and retry BEFORE mapping; reaching here means the
            // retry also failed (or a path without retry support hit this).
            // Fail loudly — treating this as Satisfied would silently mask a
            // probable genuine violation.
                ...
                Some(check_error_to_result(
                    LivenessCheckError::RuntimeFailure(format!(
                        "Liveness check for '{prop_name}' found a candidate violating cycle \
                         but could not materialize the concrete state for fingerprint \
                         {missing_fp} to re-verify fairness. The verdict is UNKNOWN \
                         (refusing to report HOLDS or VIOLATED without verification)."
                    ))
                    .into(),
                    &self.stats,
                ))
