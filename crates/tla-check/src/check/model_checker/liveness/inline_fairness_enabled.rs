...
pub(in crate::check::model_checker) struct EnabledActionGroup {
    ...
    /// Part of #liveness-leaf-memo: the tag of the unique `ActionPred` leaf
    /// paired with this group's ENABLED leaf. Both leaves are clones of the
    /// same `resolved_action` with the same quantifier bindings (see
    /// `build_fairness_expansion`), and this extraction only runs on
    /// fairness-conversion output, so the ENABLED successor scan's predicate
    /// evaluations compute exactly this leaf's value for each
    /// (current, successor) pair. `None` when the WF/SF tree does not contain
    /// exactly one `ActionPred` (fail closed — no result sharing).
    pub(in crate::check::model_checker) action_pred_tag: Option<u32>,
}
        ...
        LiveExpr::Or(disjuncts) if disjuncts.len() == 2 => {
            ...
            match (enabled_tag, action_tags) {
                (Some(e), Some(tags)) => Some(EnabledActionGroup {
                    enabled_tag: e,
                    action_pred_tag: extract_unique_action_pred_tag(&disjuncts[1]),
                    action_tags: tags,
                }),
                _ => None,
            }
...
/// Extract the tag of the unique `ActionPred` leaf in a fairness subtree, or
/// `None` if there is not exactly one (#liveness-leaf-memo, fail closed).
fn extract_unique_action_pred_tag(expr: &LiveExpr) -> Option<u32> {
    fn collect(expr: &LiveExpr, out: &mut Vec<u32>) {
        match expr {
            LiveExpr::ActionPred { tag, .. } => out.push(*tag),
            ...
            | LiveExpr::Next(inner) => collect(inner, out),
            _ => {}
        }
    }
    let mut tags = Vec::new();
    collect(expr, &mut tags);
    match tags.as_slice() {
        [tag] => Some(*tag),
        _ => None,
    }
...
pub(super) fn build_action_provenance_from_hints(
    ...
    for hint in hints {
        // #3208 wildcard frames: existentially-quantified hints have no exact
        // binding identity — this positional matcher cannot interpret them,
        // so skip (the TRUE-only ENABLED provenance registration is their
        // only consumer).
        if !hint.wildcard_arg_domains.is_empty() {
            continue;
        }
