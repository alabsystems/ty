...
/// 2026-07 OOM audit: wall-clock backstop for the memory pressure checks.
///
/// The state-count gates above assume states dequeue quickly; a spec whose
/// states each take seconds to expand (but fan out into many MB of
/// successors) could grow unpolled for `progress_interval × seconds` between
/// checks. Whenever this much wall-clock time has passed since the last
/// memory check, the check runs regardless of the state-count counters —
/// the same 1-second cadence the parallel collector loop polls at
/// (parallel/checker/finalize/collect.rs `poll_interval`).
const MEMORY_CHECK_WALL_CLOCK_BACKSTOP: std::time::Duration = std::time::Duration::from_secs(1);
...
impl ModelChecker<'_> {
    ...
    pub(super) fn memory_breakdown(
        ...
        let seen_bytes = if self.state_storage.seen.capacity() > 0 {
        ...
        let trace_locations_bytes = self.trace.trace_locs.estimate_memory_bytes();
        let payload_witness_bytes = active_payload_witness_bytes.saturating_add(
            self.state_storage
        ...
        crate::memory::MemoryBreakdown::new(fp_bytes, seen_bytes, depths_bytes, queue_bytes, 0)
            .with_trace_locations(trace_locations_bytes)
            .with_payload_witnesses(payload_witness_bytes)
    ...
    pub(super) fn maybe_report_progress(
        ...
        current_depth: usize,
        queue_size: usize,
        payload_witness_bytes: impl FnOnce() -> usize,
    ) -> ProgressAction {
        *states_since_progress += 1;
        ...
        // 2026-07 OOM audit: wall-clock backstop — run the memory checks even
        // when the state-count gates have not fired, so slow-expanding but
        // wide states still get polled. Deliberately does NOT reset
        // `states_since_progress` (progress reporting keeps its exact
        // state-count cadence; only the resource checks run early).
        let wall_clock_memory_due = !progress_due
            && !memory_check_due
            && self.hooks.last_memory_check.elapsed() >= MEMORY_CHECK_WALL_CLOCK_BACKSTOP;

        if !progress_due && !memory_check_due && !wall_clock_memory_due {
            return ProgressAction::Continue;
        }
        let payload_witness_bytes = payload_witness_bytes();
        if progress_due || memory_check_due {
            *states_since_progress = 0;
        }
        self.hooks.last_memory_check = Instant::now();
        ...
        let breakdown = self.memory_breakdown(queue_size, payload_witness_bytes);
        let seen_count = self.states_count();
        ...
        if let Some(ref policy) = self.exploration.memory_policy {
            ...
            if pressure == MemoryPressure::Critical {
                ...
                // The Warning-band checkpoint keeps its state-count cadence.
                // A wall-clock-only fire (audit 2026-07-02) must NOT re-trigger
                // it: otherwise a slow-but-wide expansion sitting in the
                // Warning band would checkpoint the whole frontier every
                // MEMORY_CHECK_WALL_CLOCK_BACKSTOP (a checkpoint storm). The
                // wall-clock backstop exists to catch the CRITICAL stop the
                // state-count cadence would miss — handled above — not to
                // amplify Warning checkpointing.
                && !wall_clock_memory_due
            {
                let rss_mb = crate::memory::current_rss_bytes()
                    .map(|b| b / (1024 * 1024))
                    .unwrap_or(0);
