...
mod fingerprint;
mod frontier;
// GPU engine admission (safety-only, fail-closed). Public types re-exported
// from the crate root for the `ty check --gpu` CLI path.
pub mod gpu_admit;
// Hybrid per-action native dispatch (ty-side M0 of wishlist item 4): classify
// actions whose footprint is entirely flat-admissible and route their
// successors through the flat-view projection (native call stubbed).
mod hybrid_dispatch;
pub(crate) mod invariants;
pub(crate) mod liveness;
mod mc_struct;
mod mem_census;
pub(crate) mod petri_native_admission;
...
pub(crate) mod trust_cg_dispatch;
