...
/// Reserve and commit a private, page-aligned, zero-filled anonymous region of
/// `byte_len` bytes, panicking on failure (callers treat backing-store
/// exhaustion as fatal).
///
/// POSIX uses `mmap(MAP_ANON | MAP_PRIVATE, PROT_READ | PROT_WRITE)`; Windows
/// uses `VirtualAlloc(MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)`, which likewise
/// hands back granularity-aligned (hence `i64`/`AtomicUsize`-aligned),
/// zero-initialized read/write pages. Release the region with [`unmap_anon`].
#[cfg(unix)]
...
/// Windows twin of [`map_anon`]: `VirtualAlloc` reserves + commits private,
/// zero-filled, read/write memory whose base is aligned to the system
/// allocation granularity (64 KiB), so it satisfies every alignment this module
/// needs.
#[cfg(windows)]
fn map_anon(byte_len: usize, owner: &str) -> NonNull<u8> {
    assert!(byte_len != 0, "{owner} requires a non-zero mmap size");

    // SAFETY: A null `lpAddress` lets the OS choose the base while reserving and
    // committing `byte_len` bytes of private read/write memory. `VirtualAlloc`
    // returns null on failure and a valid base pointer on success.
    let ptr = unsafe {
        win32::VirtualAlloc(
            ptr::null_mut(),
            byte_len,
    ...
    if ptr.is_null() {
        panic!(
            "{owner} VirtualAlloc failed for {byte_len} bytes: {}",
            io::Error::last_os_error()
        );
    }

    // SAFETY: `ptr` is non-null, so `VirtualAlloc` returned a valid base address
    // for a live committed region owned by this process.
    unsafe { NonNull::new_unchecked(ptr.cast::<u8>()) }
}

/// Release a region previously returned by [`map_anon`], returning `true` on
/// success. Used by the `Drop` impls below.
///
/// # Safety
///
/// `ptr` must be the exact base pointer from a prior [`map_anon`] call whose
/// region (`byte_len` bytes) is still live and has not already been released.
#[cfg(unix)]
unsafe fn unmap_anon(ptr: *mut u8, byte_len: usize) -> bool {
    // SAFETY: The caller guarantees `ptr`/`byte_len` describe a live mapping
    // returned by `map_anon` that has not yet been unmapped.
    unsafe { libc::munmap(ptr.cast(), byte_len) == 0 }
}

/// Windows twin of [`unmap_anon`]: `VirtualFree(.., MEM_RELEASE)` frees the whole
/// region reserved by `VirtualAlloc` and requires the size argument to be zero.
#[cfg(windows)]
unsafe fn unmap_anon(ptr: *mut u8, _byte_len: usize) -> bool {
    // SAFETY: The caller guarantees `ptr` is the exact base returned by a prior
    // `map_anon` (VirtualAlloc) that has not yet been released. `MEM_RELEASE`
    // mandates a zero size.
    unsafe { win32::VirtualFree(ptr.cast(), 0, win32::MEM_RELEASE) != 0 }
}

/// Minimal direct Win32 bindings for anonymous virtual-memory management.
/// `kernel32` is always linked into the process by the Rust runtime, so these
/// resolve without pulling in an extra crate.
#[cfg(windows)]
mod win32 {
    use std::ffi::c_void;
...
impl Drop for WorkerArena {
    fn drop(&mut self) {
        // SAFETY: `base_ptr` and `byte_len` come from a successful `map_anon` call in
        // `new`, have not been released yet, and describe the full extent of the region.
        let ok = unsafe { unmap_anon(self.base_ptr.as_ptr(), self.byte_len) };
        debug_assert!(
            ok,
            "WorkerArena unmap failed: {}",
            io::Error::last_os_error()
        );
    }
}
...
impl Drop for FlatQueue {
    fn drop(&mut self) {
        // SAFETY: `base_ptr` and `byte_len` were returned by `map_anon` in `new` and
        // still describe the owned region at drop time.
        let ok = unsafe { unmap_anon(self.base_ptr.as_ptr(), self.byte_len) };
        debug_assert!(ok, "FlatQueue unmap failed: {}", io::Error::last_os_error());
        // SAFETY: `sequences` and `sequences_byte_len` were returned by `map_anon` in `new`.
        let ok = unsafe { unmap_anon(self.sequences.as_ptr().cast(), self.sequences_byte_len) };
        debug_assert!(
            ok,
            "FlatQueue sequences unmap failed: {}",
            io::Error::last_os_error()
        );
    }
}
...
impl ArenaPool {
    ...
    ///
    /// Called at the end of BFS exploration when `TY_ARENA_STATS=1`.
...
impl Drop for ScratchBuffer {
    fn drop(&mut self) {
        ...
        // region allocated by `map_anon` in `new`. It has not been released yet.
        let ok = unsafe { unmap_anon(self.buf.as_ptr().cast(), self.byte_len) };
        debug_assert!(
            ok,
            "ScratchBuffer unmap failed: {}",
            io::Error::last_os_error()
        );
    }
}
...
mod tests {
    ...
    fn test_arena_bfs_level_lifecycle() {
        ...
        for (pi, &idx) in new_indices.iter().enumerate() {
            // `j` is used both to index `promoted[pi]` and in the expected-value
            // arithmetic `idx * 10 + j`, so a range loop is the clearest form.
    ...
    fn test_arena_exhaustion_fallback_bfs_scenario() {
        ...
        // `i` and `j` are used both as indices into `all_states` and in the
        // expected-value arithmetic `i * 10 + j`, so range loops are clearest.
        #[allow(clippy::needless_range_loop)]
        for i in 0..10 {
            for j in 0..state_len {
