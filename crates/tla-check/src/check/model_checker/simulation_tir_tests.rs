...
fn enter_split_action_probe_test_context() -> crate::ModelCheckContextGuard {
...
fn test_split_action_simulation_selected_tir_eval_hits_next_probe() {
    ...
    split_action_next_probe_body();
}

#[cfg_attr(test, ntest::timeout(10000))]
fn split_action_next_probe_body() {
    let _context = enter_split_action_probe_test_context();
    let src = r#"
...
fn test_split_action_simulation_selected_tir_eval_hits_raw_next_probe_through_replacement() {
    let _serial = crate::test_utils::acquire_interner_lock();
    split_action_raw_replacement_probe_body();
}

#[cfg_attr(test, ntest::timeout(10000))]
fn split_action_raw_replacement_probe_body() {
    let _context = enter_split_action_probe_test_context();
...
fn test_split_action_simulation_selected_tir_eval_hits_resolved_next_probe_through_replacement() {
    let _serial = crate::test_utils::acquire_interner_lock();
    split_action_resolved_replacement_probe_body();
}

#[cfg_attr(test, ntest::timeout(10000))]
fn split_action_resolved_replacement_probe_body() {
    let _context = enter_split_action_probe_test_context();
    let (module, config) = split_action_replacement_probe_fixture();
    let (result, before, after) =
