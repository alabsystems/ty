...
use tla_core::kani_types::HashMap as KaniHashMap;
...
pub(crate) fn precompute_constant_operators(ctx: &mut EvalCtx) {
    ...
    let mut candidates: Vec<(String, Arc<OperatorDef>)> = ctx
        .ops()
        .iter()
        .filter(|(name, def)| {
            def.params.is_empty()
    ...
    // DETERMINISM: `ctx.ops()` is an im::HashMap whose iteration order depends
    // on a per-process random hash seed. Precompute evaluation order decides
    // first-interning order of operator names (NameId assignment) and cache
    // warm-up order — left unsorted, it made whole-run behavior (e.g. record
    // field NameIds in BagsTest) vary run-to-run on the SAME binary. Sort by
    // definition source location for a stable, source-faithful order.
    candidates.sort_by(|(a_name, a_def), (b_name, b_def)| {
        (a_def.body.span.start, a_def.body.span.end, a_name).cmp(&(
            b_def.body.span.start,
            b_def.body.span.end,
            b_name,
        ))
    });
    ...
    let mut precomputed = KaniHashMap::new();
    for (name, def) in &candidates {
        ...
        match result {
            ...
            Ok(Err(_e)) => {
                ...
                debug_eprintln!(ty_debug(), "[precompute] '{}' error: {}", name, _e);
            }
            Err(_) => {
                // Panic (stack overflow, etc.) — skip silently.
                debug_eprintln!(ty_debug(), "[precompute] '{}' panicked", name);
            }
        }
    }
