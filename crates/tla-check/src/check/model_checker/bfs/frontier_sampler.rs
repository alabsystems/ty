...
impl FrontierSampler {
    ...
    pub(crate) fn on_state(&mut self, depth: usize, state: &ArrayState, total_states: u64) {
        ...
        // The BMC lane is the sole consumer of frontier samples (directly and
        // via the wavefront compressor). Once it has exited — most cheaply
        // when its translation degrades instantly on unsupported operators —
        // converting and sending samples is pure waste for the rest of the
        // BFS run; skip it. Level-boundary bookkeeping above still runs (the
        // depth watermark/convergence data feed BFS-side diagnostics).
        if self.cooperative.is_bmc_lane_done() {
            return;
        }
