...
pub(crate) enum BfsLoopOutcome {
    /// BFS loop completed without a local error or violation. Caller should run
    /// post-loop finalization via
    /// `finish_check_after_bfs(depth_limit_reached, resume_mode)`.
    ///
    /// `frontier_exhausted` is true only when `try_dequeue()` returned `None`.
    /// A portfolio lane can otherwise complete after dequeuing an item but
    /// before processing it, so callers must not infer exhaustion from the
    /// `Complete` variant alone.
    Complete {
...
/// Check interval for the wall-clock deadline poll.
///
/// Part of #3 (overrun/OOM hardening): every `DEADLINE_CHECK_INTERVAL` states
/// the BFS loop checks whether `BfsWorkerConfig::deadline` has elapsed and, if
/// so, stops cleanly with a partial result. `Instant::now()` is cheap but not
/// free (a syscall/vDSO read on most platforms), so we amortize it over a batch
/// of states rather than polling on every dequeue. 4096 mirrors the petri
/// explorer's `DEADLINE_CHECK_INTERVAL` and is responsive enough (sub-second on
/// any realistic per-state cost) while adding no measurable hot-loop overhead.
...
///
/// Returns `BfsLoopOutcome::Complete` for normal exhaustion or portfolio exit;
/// callers inspect `frontier_exhausted` to distinguish them. Returns
/// `BfsLoopOutcome::Terminated` for local error, violation, or limit exits.
///
...
pub(crate) fn run_bfs_worker<T: BfsTransport>(
    ...
    // `TY_DIALECT_TRACE=1` is set, emits one `verif.bfs_step` op per BFS
    ...
    while let Some(item) = transport.try_dequeue() {
        // Pin the interner's reclamation epoch once per dequeued state. The many
        // `intern_name`/`resolve_name_id` calls during this state's successor
        // generation, invariant checks, and liveness evaluation then nest under
        // this pin (a thread-local counter bump) instead of each performing a
        // full epoch registration. The guard drops at the end of the iteration,
        // letting the global epoch advance between states so retired frozen
        // snapshots are still reclaimed promptly. Part of the name-interner
        // epoch-reclamation change (replaces the per-read AcqRel reader guard).
        let _interner_epoch = tla_core::name_intern::epoch_pin_guard();
        // Portfolio early-exit check: poll every PORTFOLIO_CHECK_INTERVAL states.
        // Part of #3717.
        if let Some(sv) = portfolio_verdict {
            if states_processed % PORTFOLIO_CHECK_INTERVAL == 0
                && states_processed > 0
                && sv.is_resolved()
            {
                transport.output_profile();
        ...
        // Wall-clock deadline poll (Part of #3: overrun/OOM time backstop).
        // Polled every DEADLINE_CHECK_INTERVAL states so `Instant::now()` is
        // amortized off the hot path. When the deadline has elapsed we release
        // the just-resolved state back to storage (sequential guard contract)
        // and stop cleanly with a partial result — `on_deadline_exceeded`
        // produces a `LimitReached { Time }` (sequential) or silent Exit
        // (parallel), never `Success`, so an interrupted exploration is never
        // reported as a proof of correctness. Mirrors the petri explorer's
        // DEADLINE_CHECK_INTERVAL poll.
        if let Some(deadline) = config.deadline {
            if states_processed % DEADLINE_CHECK_INTERVAL == 0
                && std::time::Instant::now() >= deadline
            {
                let term = transport.on_deadline_exceeded(fp, current);
                return terminate_with_profile(transport, term);
            }
        }
