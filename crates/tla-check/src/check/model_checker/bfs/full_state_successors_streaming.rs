...
impl ModelChecker<'_> {
    ...
    fn cache_streaming_full_state_liveness<S: BfsStorage>(
        ...
        if needs_all_diffs {
            ...
            self.run_state_completion_observers(
                iter_state,
                storage,
                observer,
    ...
    >(
        ...
        // full-state streaming path (inert unless `TY_HYBRID_FLAT_VIEW` is set).
        ...
        let enum_result: Result<bool, CheckError> = (|| {
            ...
            // monitors so the ClosureSink OBSERVES every successor's board on the
            // fast full-state streaming path (escape-only; fingerprint unaffected).
            ...
            let mut sink = ClosureSink::new(|diff: DiffSuccessor| -> ControlFlow<()> {
                ...
                // BEFORE dedup/store; fails closed on escape (bails the var to raw
                // value_fingerprint, the SAME fp the diff already produces, so the
                ...
                // Inline dedup probe. Duplicate payload authorization is
                // deferred to Phase B after materialization.
            ...
            // enumeration (see the batch path in full_state_successors.rs for
            // the full bracketing argument). Any nested enumeration fired from
            // the ClosureSink callback runs at provenance depth ≥ 2 and never
            // records. The guard disarms on scope exit (error paths included).
