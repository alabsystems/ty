...
use super::storage_modes::{BfsAdmissionCandidate, BfsBatchAdmissionResult, BfsStorage};
...
const BFS_ADMISSION_BATCH_TARGET: usize = 64;

/// Bounded staging area for successors that have already passed observer
/// checks and are ready for authoritative storage admission.
pub(super) struct BfsAdmissionBatch<Q> {
    candidates: Vec<BfsAdmissionCandidate>,
    result: BfsBatchAdmissionResult<Q>,
}

impl<Q> BfsAdmissionBatch<Q> {
    pub(super) fn new() -> Self {
        Self {
            candidates: Vec::with_capacity(BFS_ADMISSION_BATCH_TARGET),
            result: BfsBatchAdmissionResult::with_capacity(BFS_ADMISSION_BATCH_TARGET),
        }
    }
    ...
    fn contains_fp(&self, fp: Fingerprint) -> bool {
        self.candidates.iter().any(|candidate| candidate.fp == fp)
    }

    fn push(&mut self, candidate: BfsAdmissionCandidate) {
        self.candidates.push(candidate);
    }

    fn should_flush(&self) -> bool {
        self.candidates.len() >= BFS_ADMISSION_BATCH_TARGET
    }
}

impl ModelChecker<'_> {
    pub(super) fn flush_admission_batch<S: BfsStorage, Q: BfsFrontier<Entry = S::QueueEntry>>(
        &mut self,
        iter_state: &mut BfsIterState,
        storage: &mut S,
        queue: &mut Q,
        batch: &mut BfsAdmissionBatch<S::QueueEntry>,
    ) -> Result<(), BfsIterOutcome> {
        if batch.is_empty() {
            return Ok(());
        }

        let current_fp = iter_state.fp();
        let current_state = iter_state.array();
        let result = &mut batch.result;
        storage.admit_successor_batch_into(
            &mut batch.candidates,
            Some((current_fp, current_state)),
            result,
            self,
        );
        for admitted in result.entries.drain(..) {
            let Some(entry) = admitted.entry else {
                continue;
            };
            self.stats.max_depth = self.stats.max_depth.max(admitted.depth);
            queue.push(entry);
            self.stats.states_found += 1;
            self.stats.max_queue_depth = self.stats.max_queue_depth.max(queue.len());
        }
        if let Some(fault) = result.fault.take() {
            iter_state.return_to(storage, self);
            return Err(BfsIterOutcome::Terminate(fault));
        }
        Ok(())
    }
    ...
    #[allow(clippy::result_large_err)]
    pub(super) fn check_seen_successor_with_profile<S: BfsStorage>(
        &mut self,
        iter_state: &mut BfsIterState,
        storage: &mut S,
        succ_fp: Fingerprint,
        ...
        let already_seen = self.check_seen_with_profile(iter_state, storage, succ_fp, prof)?;
        if already_seen {
            let current = Some((iter_state.fp(), iter_state.array()));
            if let Err(result) =
                storage.enforce_seen_successor_duplicate(succ_fp, succ, current, self)
            {
                iter_state.return_to(storage, self);
    ...
    #[inline]
    #[allow(dead_code)] // Used when ay feature is active
    ...
    ///
    /// Part of #2677 Phase 2.
    pub(in crate::check::model_checker) fn record_transitions(&mut self, count: usize) {
    ...
    >(
        ...
        let current = Some((parent_fp, iter_state.array()));
        let entry = match storage.admit_successor(
            succ_fp,
            succ,
            Some(parent_fp),
            current,
    ...
    /// Batched variant of `finish_prefiltered_successor`.
    ///
    /// Observer order is unchanged. Only the authoritative storage admission is
    /// delayed, and the batch is flushed before any terminal return or when a
    /// later sibling has the same fingerprint as an unflushed candidate.
    #[allow(clippy::result_large_err)]
    #[allow(clippy::too_many_arguments)]
    pub(super) fn finish_prefiltered_successor_batched<
        S: BfsStorage,
        Q: BfsFrontier<Entry = S::QueueEntry>,
    >(
        &mut self,
        ...
        if batch.contains_fp(succ_fp) {
            self.flush_admission_batch(iter_state, storage, queue, batch)?;
            return Ok(());
        }
        ...
        batch.push(BfsAdmissionCandidate {
            fp: succ_fp,
            state: succ,
