...
pub(crate) trait BfsTransport {
    ...
    /// Produce the termination signal for a wall-clock-deadline-triggered stop.
    ///
    /// Part of #3 (overrun/OOM hardening): called by the unified BFS loop when
    /// `BfsWorkerConfig::deadline` has elapsed. Must produce a partial result
    /// that DOES NOT report `Success` — exploration was cut short, so reporting
    /// success would be unsound. The sequential transport returns a
    /// `LimitReached { limit_type: Time }`; the parallel transport sets the
    /// shared stop flag and exits silently (results already flow via channel).
    ///
    /// `fp`/`current` are the state that was dequeued but not yet processed; the
    /// transport releases it back to storage (sequential `BfsIterState` guard
    /// contract) before producing the termination signal.
    ///
    /// Default: release the state and exit silently (used by parallel transports
    /// that signal termination out-of-band).
    fn on_deadline_exceeded(&mut self, fp: Fingerprint, current: ArrayState) -> BfsTermination {
        self.release_state(fp, current);
        BfsTermination::Exit
    }
}
...
pub(crate) struct BfsWorkerConfig {
    ...
    /// Wall-clock deadline for the BFS exploration (`None` = no time backstop).
    ///
    /// Part of #3 (overrun/OOM hardening): the unified BFS loop polls this
    /// deadline every [`DEADLINE_CHECK_INTERVAL`](super::worker_loop::DEADLINE_CHECK_INTERVAL)
    /// states. When `Instant::now() >= deadline`, the loop stops cleanly via
    /// the transport's deadline hook and flushes a partial
    /// `LimitReached { limit_type: Time }` result (NOT `Success`). This is the
    /// time backstop that prevents an unbounded spec from running forever or
    /// growing the state space until the process OOMs.
    ///
    /// Mirrors the petri explorer's `DEADLINE_CHECK_INTERVAL` poll pattern.
