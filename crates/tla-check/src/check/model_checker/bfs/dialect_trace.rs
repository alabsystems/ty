...
//! `TY_DIALECT_TRACE=1` bridge — turns the BFS worker loop into a concrete
...
//! - When `TY_DIALECT_TRACE=1`, every BFS dequeue produces a
...
//! structured output (e.g. for trust-codegen pipeline introspection), this module is
...
///
/// The first call reads `TY_DIALECT_TRACE` once and caches the result with
...
fn init_dialect_trace_state() -> bool {
    let enabled = std::env::var("TY_DIALECT_TRACE")
        .map(|v| v == "1")
        .unwrap_or(false);
...
impl DialectTracer {
    ...
    #[cfg(test)]
    fn new_for_tests(worker_id: u32, enabled: bool) -> Self {
        Self { worker_id, enabled }
    }
...
mod tests {
    ...
    #[test]
    fn wave14_tracer_test_constructor_can_disable_trace() {
        let tracer = DialectTracer::new_for_tests(0, false);
        assert!(
            !tracer.is_enabled(),
            "test constructor must allow the disabled fast path"
        );
    ...
    #[test]
    fn wave14_tracer_test_constructor_can_enable_trace_without_global_env() {
        let tracer = DialectTracer::new_for_tests(3, true);
        assert!(
            tracer.is_enabled(),
            "test constructor must allow enabled trace coverage without mutating TY_DIALECT_TRACE"
        );
        assert_eq!(tracer.worker_id, 3);
    }

    #[test]
    fn wave14_tracer_test_constructor_does_not_mutate_process_env() {
        let before = std::env::var_os("TY_DIALECT_TRACE");
        let tracer = DialectTracer::new_for_tests(4, true);
        tracer.emit_step(1, 2, 3);
        assert_eq!(std::env::var_os("TY_DIALECT_TRACE"), before);
    }
    ...
    fn wave14_tracer_saturates_oversized_fields_to_u32_max() {
        ...
        let tracer = DialectTracer::new_for_tests(0, true);
    ...
    fn wave14_tracer_disabled_path_is_noop() {
        let tracer = DialectTracer::new_for_tests(42, false);
    ...
    fn wave14_tl3_drain_emit_is_noop_when_tracer_disabled() {
        let tracer = DialectTracer::new_for_tests(1, false);
    ...
    fn wave14_tl3_drain_emit_enabled_does_not_panic_on_clamp_boundaries() {
        let tracer = DialectTracer::new_for_tests(2, true);
        assert!(tracer.is_enabled());
    ...
    fn wave14_tl3d_state_fingerprint_and_invariant_emit_are_noop_when_disabled() {
        let tracer = DialectTracer::new_for_tests(5, false);
        assert!(!tracer.is_enabled());
    ...
    {
        let tracer = DialectTracer::new_for_tests(7, true);
        assert!(tracer.is_enabled());
