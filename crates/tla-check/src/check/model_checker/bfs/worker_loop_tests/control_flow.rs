...
fn test_portfolio_early_exit_does_not_claim_frontier_exhaustion() {
    ...
    let outcome = run_bfs_worker(&mut transport, &config, Some(&verdict));
    ...
    assert_eq!(transport.process_calls.get(), 4096);
    assert_eq!(
        transport.dequeue_count.get(),
        4097,
        "the early-exit check runs after dequeuing the unprocessed item"
    );
}

#[test]
fn test_parallel_exit_does_not_claim_frontier_exhaustion() {
    let outcome: BfsLoopOutcome =
        crate::check::model_checker::bfs::transport::BfsTermination::Exit.into();
