    ...
    for SequentialTransport<'a, 'mc, S, Q>
{
    ...
    fn process_successors(
        ...
        // GameOfLife memory fix: bound the per-state-scoped LET / recursive-fold
        // memoization caches to one parent's expansion. These caches key on AST
        // scope + dependency *values* and are dep-validated on lookup, so they
        // are correct as process-long caches — but a spec whose LET / fold body
        // depends on the current state (e.g. GameOfLife's `score`/`Sum` read
        // `grid` through `sc`) produces a fresh, never-reused entry for every
        // distinct state. With the whole reachable space at one BFS level (every
        // GameOfLife state is an initial state, so the per-level arena/boundary
        // reset never fires mid-expansion) they grew without bound (~250K entries
        // / ~1.6 GB on N=4). Clearing once per dequeued parent bounds them while
        // preserving intra-state reuse. Sound: dropping memoized entries only
        // forces recomputation, never changes a value, verdict, or state count.
        crate::eval::clear_state_scoped_let_and_fold_caches();
        ...
        // AUTO-mode lazy trust-cg compile. When setup deferred the eager native
        // action-callout build, this builds it exactly once the distinct-state
        // count crosses the lazy threshold, then per-action native dispatch
        // engages for the remainder of the interpreter loop. No-op (single bool
        // test) in the common case where nothing was deferred (forced backends,
        // small runs that already triggered, or lazy deferral disabled).
        // Placed before successor generation so the per-action dispatch readiness
        // checks downstream observe the freshly built cache for this parent.
        self.checker.maybe_trigger_trust_cg_lazy_compile();
    ...
    fn on_deadline_exceeded(&mut self, fp: Fingerprint, current: ArrayState) -> BfsTermination {
        // Part of #3 (overrun/OOM time backstop): the wall-clock deadline
        // elapsed mid-exploration. Return the dequeued state to storage (the
        // `BfsStorage::return_current` contract — the same release the other
        // early-exit paths perform) and finalize a partial result.
        //
        // Crucially this is `LimitReached { Time }`, NOT `Success`: the state
        // space was only partially explored, so no invariant can be claimed
        // proven. Downstream the pipeline BFS runner leaves every property
        // Unknown on `LimitReached`, fail-closed.
        self.storage.return_current(fp, current, self.checker);
        let result = super::super::CheckResult::LimitReached {
            limit_type: super::super::LimitType::Time,
            stats: self.checker.stats.clone(),
        };
        BfsTermination::result(self.checker.finalize_terminal_result_with_storage(result))
    }
    ...
    fn output_profile(&self) {
        ...
        // Part of #3580: report arena allocation stats when TY_ARENA_STATS=1.
    ...
    fn maybe_sample_frontier(&mut self, _depth: usize, _state: &super::super::ArrayState) {
        #[cfg(feature = "ay")]
        {
            // Deferred `TY_RP_VALUE=1` non-atomic Rp engagement for the fused
            // BFS lane: flips on only after every auxiliary orchestrator
            // thread has terminated and been joined. One-branch no-op when
            // disarmed (the non-fused and opt-out cases).
            self.checker.maybe_engage_rp_nonatomic_deferred();
