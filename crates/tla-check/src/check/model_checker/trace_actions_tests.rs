...
/// `label_trace_actions` attributes TLC-parity `line N, col N to line N, col N
/// of module M` labels to a bare-state trace — the shape a symbolic lane's
/// replay-validated counterexample has when the fused verdict race promotes it
/// (the k-Induction/BMC/PDR masking path, #2470/#2920 parity).
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn label_trace_actions_attributes_line_col_labels_to_bare_trace() {
    use crate::state::State;
    use crate::value::Value;

    let source = r#"
---- MODULE BareTraceLabels ----
EXTENDS Integers
VARIABLE x
Init == x = 0
Inc == x' = x + 1
...
"#;
    ...
    // Bare states (no labels) — x=0 -> x=-1 via Dec, exactly what
    // cross-validation's Init/Next replay reconstructs from a symbolic trace.
    let states = vec![
        State::from_pairs([("x", Value::int(0))]),
        State::from_pairs([("x", Value::int(-1))]),
    ];
    let mut trace = crate::check::Trace::from_states(states);
    assert!(trace.action_labels.is_empty());

    // The source must exist on disk for line/col rendering (the CLI registers
    // FileId -> spec path the same way).
    let dir = tempfile::tempdir().expect("tempdir");
    let spec_path = dir.path().join("BareTraceLabels.tla");
    std::fs::write(&spec_path, source).expect("write spec");

    crate::checker_ops::label_trace_actions(
        &module,
        &[],
        &config,
        &[(tla_core::FileId(0), spec_path)],
        &mut trace,
    );

    assert_eq!(trace.action_labels.len(), 2, "one label per state");
    assert_eq!(trace.action_labels[0].name, "Initial predicate");
    let label = &trace.action_labels[1];
    assert_eq!(label.name, "Dec", "x=0 -> x=-1 must be attributed to Dec");
    let loc = label
        .source_location
        .as_deref()
        .expect("label must carry a source location");
    assert!(
        loc.contains("line ")
            && loc.contains(" col ")
            && loc.contains(" to ")
            && loc.contains("of module BareTraceLabels"),
        "expected TLC line/col location, got '{loc}'"
    );
    assert!(
        !loc.contains("bytes "),
        "byte-offset fallback must not be used when the source path is registered: '{loc}'"
    );
}
