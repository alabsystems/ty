...
use super::{ArrayState, Fingerprint, ModelChecker};
#[cfg(debug_assertions)]
...
impl<'a> ModelChecker<'a> {
    ...
    /// builds when enabled via `TY_DEBUG_SEEN_TLCFP_DEDUP` or
    /// `TY_DEBUG_INTERNAL_FP_COLLISION` env vars. Lazy value tracking remains
    /// debug-only.
