...
#[cfg_attr(test, ntest::timeout(120000))]
#[test]
fn real_disruptor_mpmc_conjunct_cache_activates_under_default_tir_and_matches_explicit_eval() {
    let (module, checker_modules, config) = load_real_disruptor_mpmc_modules();
    let checker_module_refs: Vec<_> = checker_modules.iter().collect();
    let state7 = real_disruptor_mpmc_trace_state7();
    let state8 = real_disruptor_mpmc_counterexample_state();

    let mut cached = ModelChecker::new_with_extends(&module, &checker_module_refs, &config);
    cached.prepare_bfs_common().unwrap();
    assert!(cached.tir_parity.as_ref().is_some_and(
        super::super::super::super::tir_parity::TirParityState::is_implicit_default_eval_mode
    ));
    assert!(cached.bytecode.is_none());
    cached
        .invariant_verdict_cache
        .test_enable_conjunct_cache(&cached.ctx, &config.invariants);
    assert_eq!(
        cached
            .invariant_verdict_cache
            .test_active_conjunct_plan_count(),
        3,
        "real TypeOk has exactly the read/consumed/pc cacheable trailing leaves",
    );

    let registry = cached.ctx.var_registry().clone();
    let array_state7 = ArrayState::from_state(&state7, &registry);
    let array_state8 = ArrayState::from_state(&state8, &registry);
    let cached_state7 = cached.check_invariants_array(&array_state7).unwrap();
    assert_eq!(cached_state7, None);
    let hits_before = cached.invariant_verdict_cache.test_hit_count();
    let cached_state8 = cached.check_invariants_array(&array_state8).unwrap();
    assert_eq!(cached_state8, None);
    assert_eq!(
        cached.invariant_verdict_cache.test_hit_count() - hits_before,
        1,
        "unchanged consumed should activate one real MPMC suffix hit",
    );

    let mut explicit = ModelChecker::new_with_extends(&module, &checker_module_refs, &config);
    explicit.tir_parity = Some(
        super::super::super::super::tir_parity::TirParityState::test_eval_selected(
            module.clone(),
            checker_modules.clone(),
            ["TypeOk", "NoDataRaces"],
    ...
    explicit.prepare_bfs_common().unwrap();
    explicit
        .invariant_verdict_cache
        .test_enable_conjunct_cache(&explicit.ctx, &config.invariants);
    let explicit_registry = explicit.ctx.var_registry().clone();
    let explicit_state7 = ArrayState::from_state(&state7, &explicit_registry);
    let explicit_state8 = ArrayState::from_state(&state8, &explicit_registry);
    assert_eq!(
        cached_state7,
        explicit.check_invariants_array(&explicit_state7).unwrap()
    );
    assert_eq!(
        cached_state8,
        explicit.check_invariants_array(&explicit_state8).unwrap()
    );
    assert_eq!(explicit.invariant_verdict_cache.test_entry_count(), 0);
    assert_eq!(explicit.invariant_verdict_cache.test_hit_count(), 0);
}
...
fn test_check_real_disruptor_mpmc_liveliness_cfg_passes_under_default_tir_eval() {
    ...
    test_check_real_disruptor_mpmc_liveliness_cfg_passes_under_default_tir_eval_body();
}

#[cfg_attr(test, ntest::timeout(900_000))]
fn test_check_real_disruptor_mpmc_liveliness_cfg_passes_under_default_tir_eval_body() {
...
fn test_check_real_disruptor_mpmc_standard_cfg_passes_under_default_tir_eval() {
    let _quiescence = crate::test_utils::acquire_model_check_quiescence_lock();
    test_check_real_disruptor_mpmc_standard_cfg_passes_under_default_tir_eval_body();
}

#[cfg_attr(test, ntest::timeout(1_800_000))]
fn test_check_real_disruptor_mpmc_standard_cfg_passes_under_default_tir_eval_body() {
    let (module, checker_modules, config, resolved) =
        load_real_disruptor_mpmc_modules_with_cfg("Disruptor_MPMC.cfg");
    let checker_module_refs: Vec<_> = checker_modules.iter().collect();

    let mut checker = ModelChecker::new_with_extends(&module, &checker_module_refs, &config);
    // Hermetic: this test asserts the UNREDUCED TLC baseline count. Auto-symmetry
    // (ON by default) folds the Writers/Readers orbits to the TLC-with-SYMMETRY
    // parity count (13,328); disable it to keep the unreduced TIR-parity assertion.
