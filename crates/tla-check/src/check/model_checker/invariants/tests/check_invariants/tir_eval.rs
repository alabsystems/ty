...
#[test]
fn invariant_verdict_cache_is_disabled_during_explicit_tir_eval() {
    let module = parse_module(
        r#"
---- MODULE InvVerdictTirEval ----
EXTENDS Integers, TLC
VARIABLE x
Init == x = 0
Next == x' = x
Safe == /\ TLCGet("level") = 0 /\ x >= 0
====
"#,
    );
    let config = Config {
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn invariant_verdict_cache_is_disabled_during_tir_parity_replay() {
    let module = parse_module(
        r#"
---- MODULE InvVerdictTirParity ----
EXTENDS Integers, TLC
VARIABLE x
Init == x = 0
Next == x' = x
...
"#,
    ...
    let mut mc = ModelChecker::new(&module, &config);
    mc.tir_parity = Some(
        super::super::super::super::tir_parity::TirParityState::test_parity_selected(
            module.clone(),
            Vec::new(),
            ["Safe"],
        ),
