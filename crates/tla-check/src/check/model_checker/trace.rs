                ...
                .find(|s| match self.state_fingerprint(s) {
                    Ok(fp) => {
                        fp == target_init_fp
                            ...
                            // WP-11 slice 2: the trace file's INIT row was
                            // written before the flat-symmetry canonicalizer
                            // installed (layout inference needs an init state),
                            // i.e. in the interpreter SymmetryCanonical domain.
                            // Mirror the CompiledFlat raw-fp fallback above
                            // with the SymmetryCanonical fallback here;
                            // successor rows are post-freeze and match the
                            // canonical `state_fingerprint` directly.
                            || (matches!(
                                self.bfs_fingerprint_domain(),
                                super::fingerprint::BfsFingerprintDomain::FlatSymmetryCanonical
                            ) && s.fingerprint_with_symmetry_fast(&self.symmetry.mvperms)
                                == target_init_fp)
                    }
                    Err(e) => {
                        eprintln!(
