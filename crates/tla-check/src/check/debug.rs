...
debug_flag!(pub(super) debug_states, "TY_DEBUG_STATES");
debug_flag!(pub(super) debug_invariants, "TY_DEBUG_INVARIANTS");
debug_flag!(pub(super) debug_successors, "TY_DEBUG_SUCCESSORS");
debug_flag!(pub(super) debug_successors_actions, "TY_DEBUG_SUCCESSORS_ACTIONS");
debug_flag!(pub(super) debug_successors_actions_all_states, "TY_DEBUG_SUCCESSORS_ACTIONS_ALL");
debug_flag!(pub(super) debug_successors_actions_counts_only, "TY_DEBUG_SUCCESSORS_ACTION_COUNTS_ONLY");
debug_flag!(pub(super) debug_successors_tlc_fp, "TY_DEBUG_SUCCESSORS_TLCFP");
debug_flag!(pub(super) debug_successors_dump_state, "TY_DEBUG_SUCCESSORS_DUMP_STATE");
debug_flag!(pub(super) debug_symmetry_invariant, "TY_DEBUG_SYMMETRY_INVARIANT");
debug_flag!(pub(super) debug_symmetry_invariant_panic, "TY_DEBUG_SYMMETRY_INVARIANT_PANIC");
debug_flag!(pub(super) debug_symmetry_invariant_dump_state, "TY_DEBUG_SYMMETRY_INVARIANT_DUMP_STATE");
...
feature_flag!(pub(crate) debug_seen_tlc_fp_dedup, "TY_DEBUG_SEEN_TLCFP_DEDUP");
debug_flag!(pub(super) debug_tlcfp_decimal, "TY_TLCFP_DECIMAL"); // Part of #599
debug_flag!(pub(super) debug_lazy_values_in_state, "TY_DEBUG_LAZY_VALUES_IN_STATE");
feature_flag!(pub(crate) debug_internal_fp_collision, "TY_DEBUG_INTERNAL_FP_COLLISION");
debug_flag!(pub(super) debug_liveness_formula, "TY_DEBUG_LIVENESS_FORMULA"); // Part of #246
debug_flag!(pub(super) debug_safety_parts, "TY_DEBUG_SAFETY_PARTS");
debug_flag!(pub(super) debug_safety_temporal, "TY_DEBUG_SAFETY_TEMPORAL");
debug_flag!(pub(crate) ty_debug, "TY_DEBUG");
feature_flag!(pub(crate) debug_bytecode_vm, "TY_DEBUG_BYTECODE_VM");
/// Whether `TY_BYTECODE_VM_STATS=1` is set. Cached via `OnceLock`.
/// Part of #4114: replaces ~15 scattered `std::env::var("TY_BYTECODE_VM_STATS")`
/// calls across `run_prepare.rs`, `run_finalize.rs`, `prepare.rs`, and `eval.rs`.
pub(crate) fn bytecode_vm_stats_enabled() -> bool {
    static FLAG: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *FLAG.get_or_init(|| matches!(std::env::var("TY_BYTECODE_VM_STATS").as_deref(), Ok("1")))
}
debug_flag!(pub(super) debug_init, "TY_DEBUG_INIT");
debug_flag!(pub(crate) debug_state_succs, "TY_DEBUG_STATE_SUCCS"); // #3019
debug_flag!(pub(crate) debug_dump_succs, "TY_DUMP_SUCCS"); // #3019

// Feature/operational flags — remain active in release builds
feature_flag!(pub(crate) skip_liveness, "TY_SKIP_LIVENESS");

// 2026-07 memory audit kill switch: disable raw-fp interning of symmetry
// successor witnesses (restores one retained ArrayState per transition).
feature_flag!(pub(crate) no_witness_intern, "TY_NO_WITNESS_INTERN");

// 2026-07 memory audit: opt-in for the native-lane kernel certificate
// (`TY_NATIVE_LANE_CERT=1` restores the previous always-on behavior). The
// certificate forces the full Clean kernel environment build (~37 MB retained
// + ~1 GB churn) to upgrade ONE already-runtime-validated sample; the verdict
// never depends on it.
feature_flag!(pub(crate) native_lane_cert_enabled, "TY_NATIVE_LANE_CERT");

// Whether FlatState-based BFS is force-disabled via env var.
// Suppresses auto-detection even when the layout is fully flat and roundtrip
// verification passes. Useful for debugging or benchmarking the non-flat path.
feature_flag!(pub(crate) flat_bfs_disabled, "TY_NO_FLAT_BFS");

// Whether compiled BFS emits per-level progress lines. Start/completion
// telemetry remains unconditional; this flag only gates periodic progress lines.
feature_flag!(
    pub(crate) compiled_bfs_progress_enabled,
    "TY_COMPILED_BFS_PROGRESS"
);
...
feature_flag!(pub(crate) compiled_bfs_disabled, "TY_NO_COMPILED_BFS");

// Whether trust-codegen native fused launch runs must fail closed instead of falling
// back to flat/interpreter/per-parent execution.
//
// This makes fallback a correctness failure for launch gates and smoke tests:
// when set, compiled BFS not activating is treated as an error rather than a
// silent fall-through to the flat/interpreter/per-parent path.
feature_flag!(
    pub(crate) trust_cg_native_fused_strict,
    "TY_TRUST_CG_NATIVE_FUSED_STRICT"
);

// Whether trust-codegen diagnostic mode should fail closed on flat-state adapter
// roundtrip failures instead of silently falling back to mixed-state storage.
feature_flag!(pub(crate) trust_cg_diag_enabled, "TY_TRUST_CG_DIAG");

// Whether AUTO engine selection is active for this run.
//
// Set by the CLI to `1` ONLY when `ty check` is run with NO `--backend` flag
// (the production default). In AUTO mode the trust-cg native path is the default
// engine, but a cheap structural pre-check may transparently route the run to
// the interpreter when native would not help (low action compilability / low
// native coverage / no admissible fused loop) so the default never regresses
// below the interpreter. An EXPLICIT `--backend trust-cg` does NOT set this, so
// the supremacy harnesses and oracle cross-checks keep forced-native behavior.
pub(crate) fn trust_cg_auto_select_enabled() -> bool {
    // The set-once process-global env snapshot (installed only by the CLI) wins; it mirrors
    // `feature_flag!` (any value present, incl. `=0`). Library/test callers never install it
    // and fall through to the legacy `OnceLock`-cached env path below.
    tla_backend::global_overlay()
        .map(tla_backend::EngineEnvOverlay::auto_select_is_set)
        .unwrap_or_else(|| {
            static FLAG: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
            crate::debug_env::env_flag_is_set(&FLAG, "TY_TRUST_CG_AUTO_SELECT")
        })
}
...
///
/// Default: OFF. The CLI `--jit` flag sets `TY_JIT=1` before this is first called.
/// Can also be enabled directly via `TY_JIT=1` env var.
///
...
pub(crate) fn jit_enabled() -> bool {
    static FLAG: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    crate::debug_env::env_flag_is_set(&FLAG, "TY_JIT")
}
feature_flag!(pub(super) profile_enum, "TY_PROFILE_ENUM");
...
feature_flag!(pub(crate) force_no_diffs, "TY_FORCE_NO_DIFFS");

// Cached debug limits — only compiled in debug builds (Part of #3026).
// Callers are all behind #[cfg(debug_assertions)].
#[cfg(debug_assertions)]
debug_opt_limit!(pub(super) debug_successors_limit, "TY_DEBUG_SUCCESSORS_LIMIT");
// Part of #2841: collision limits active in release builds too.
feature_limit!(pub(crate) debug_seen_tlc_fp_dedup_collision_limit, "TY_DEBUG_SEEN_TLCFP_DEDUP_COLLISION_LIMIT", 10);
#[cfg(debug_assertions)]
debug_limit!(pub(super) debug_lazy_values_in_state_log_limit, "TY_DEBUG_LAZY_VALUES_IN_STATE_LOG_LIMIT", 50);
feature_limit!(pub(crate) debug_internal_fp_collision_limit, "TY_DEBUG_INTERNAL_FP_COLLISION_LIMIT", 10);
...
pub(super) fn debug_successors_action_filter() -> Option<Vec<String>> {
    ...
    FILTER
        .get_or_init(|| {
            let raw = std::env::var("TY_DEBUG_SUCCESSORS_ACTION_FILTER").ok()?;
...
pub(super) fn debug_successors_filter_state_fp() -> Option<Fingerprint> {
    ...
    *FILTER.get_or_init(|| parse_u64_env("TY_DEBUG_SUCCESSORS_STATE").map(Fingerprint))
}

#[cfg(debug_assertions)]
pub(super) fn debug_successors_filter_state_tlc_fp() -> Option<u64> {
    static FILTER: OnceLock<Option<u64>> = OnceLock::new();
    *FILTER.get_or_init(|| parse_u64_env("TY_DEBUG_SUCCESSORS_TLC_STATE"))
}
