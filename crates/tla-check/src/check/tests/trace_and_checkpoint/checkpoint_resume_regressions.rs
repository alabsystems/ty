...
const RESUME_TRACE_DEGRADED_CHILD_ENV: &str = "TY_RESUME_TRACE_DEGRADED_CHILD";
...
"#,
        ...
        // NOTE: suppressed-guard counts are per-run (ModelChecker::run_diagnostics)
        // and drained into CheckStats by the resume path. The former assertion on
        // the legacy global counter was removed: the global is now only a fallback
        // for non-run contexts and other tests legitimately touch it concurrently.
    });
}
