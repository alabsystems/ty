...
"#;
    ...
    match result {
        CheckResult::Success(stats) => {
            ...
            // Part of #4451: in no-trace mode the fingerprint set is the
            // authoritative deduplication structure. The full-state `seen` map
            // additionally retains a per-fingerprint canonical payload witness
            // so the fail-closed admission backend can verify duplicate
            // fingerprints map to the same state. Both stores should report
            // the same cardinality after a clean run.
            assert_eq!(
...
"#;
    ...
    checker_with_view.set_force_explicit_bfs(true);
