...
/// Part of #3210 / #4451: sequential fp-only liveness must still record every
/// explored fingerprint for BFS dedup and reconstruct concrete states later on
/// the cold replay path.
///
/// Part of #4451: the full-state `seen` map is now also populated with
/// payload witnesses (one per admitted fingerprint) so duplicate fingerprints
/// can be fail-closed verified via canonical payload comparison. The
/// witness-store cost is constant per state regardless of `store_full_states`,
/// but the BFS hot path no longer needs full-state reconstruction in fp-only
/// mode — the witness is just enough payload to detect collisions.
#[cfg_attr(test, ntest::timeout(30000))]
#[test]
