...
/// Feature 3 (LIVE) — the model checker FINDS a real safety violation and the violated counterexample
/// trace is promoted to a Clean-kernel-CHECKED finite-trace certificate, end to end. This runs the
/// ACTUAL `ModelChecker::check()` on a violating spec (not a hand-built cleancic toy); the live
/// finalize path (`emit_violated_trace_kernel_cert`) ground-evaluates the counterexample trace
/// `x=0,1,2,3` and the Clean kernel `check_type`s `Init(0) ∧ (1=0+1) ∧ (2=1+1) ∧ (3=2+1) ∧ ¬(3<3)`.
/// We then drain the captured cert and INDEPENDENTLY re-run the kernel via `verify_violated_trace` —
/// the arbiter — proving the cert reached and was accepted by the kernel through the live path.
#[cfg(feature = "clean-cic")]
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn live_safety_violation_yields_kernel_checked_violated_trace_cert() {
    use tla_core::{lower, parse_to_syntax_tree, FileId};

    // Clear any cert captured by a prior test on this thread.
    let _ = crate::cleancic::take_violated_trace_cert();

    // Single Int variable; Init x=0, Next x'=x+1, Safety x<3 — violated at x=3.
    let src = r#"
---- MODULE Counter ----
VARIABLE x
Init == x = 0
...
"#;
    ...
    // LIVE run: the checker must FIND the violation and reconstruct the trace.
    let result = checker.check();
    match &result {
        CheckResult::InvariantViolation {
            invariant, trace, ..
    ...
    // The LIVE finalize path must have produced a kernel-checked violated-trace certificate.
    let cert = crate::cleancic::take_violated_trace_cert()
        .expect("live counterexample must yield a kernel-checked violated-trace cert");
    assert!(!cert.is_empty());

    // INDEPENDENT re-check (the arbiter): re-derive the obligation from the SAME Init/Next/Safety
    // predicates + the concrete trace and re-run the Clean kernel on the carried term. We rebuild the
    // ASTs (`Init: x=0`, `Next: x'=x+1`, `SmallValue: x<3`) exactly as the spec defines them.
    ...
    assert!(
        crate::cleancic::verify_violated_trace(&init, &next, &safety, "x", &[0, 1, 2, 3], &cert),
        "the Clean kernel must re-accept the violated-trace cert for the real trace"
    );
    // Tamper-evidence: a DIFFERENT trace (not the one the cert was built for) must be rejected.
    assert!(
        !crate::cleancic::verify_violated_trace(
            &init,
            &next,
            &safety,
            "x",
        ...
        "a cert built for x=0..3 must NOT re-check against a different (x=0..4) trace"
    );
}
