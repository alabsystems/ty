...
// Module compiles with or without ay: non-ay fallback runs BFS-only.
pub(crate) mod fused;
// Counterexample cross-validation for fused orchestrator (Part of #3836)
#[cfg(feature = "ay")]
pub(crate) mod cross_validation;
// SAFE-direction eval oracle (certifying verification, Leg A; Part of #3836).
// AY-independent — relocated out of cross_validation so the certificate's
// eval-oracle leg builds without the `ay` feature.
pub(crate) mod eval_oracle;
...
pub use self::api::{
    ...
    PropertyCheckStats, PropertyViolationKind, ResolvedSpec, SymmetryReductionStats, Trace,
};
