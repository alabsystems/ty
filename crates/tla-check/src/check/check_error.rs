...
pub enum RuntimeCheckError {
    ...
    Internal(String),
    /// The flat (i64-slot) state layout cannot represent a value the spec
    /// produced at runtime — e.g. a scalar integer whose magnitude exceeds
    /// i64, or a sequence that grew past its proven flat capacity.
    ///
    /// This is NOT a spec error: the run is sound up to the point of
    /// detection, and re-running with flat state storage disabled
    /// (`Config::use_flat_state = Some(false)`) produces the correct verdict.
    /// The CLI catches this variant and transparently retries without flat
    /// storage.
    #[error(
        "flat state layout cannot represent a value produced by this spec: {detail} \
         (re-run with flat state storage disabled, e.g. TY_NO_FLAT_BFS=1)"
    )]
    FlatLayoutUnsupportedValue {
        /// Human-readable description of the unrepresentable value.
        detail: String,
    },
}
...
impl CheckError {
    /// Build the typed "flat layout cannot represent this runtime value" error.
    ///
    /// Used by flat-primary BFS paths when a state cannot be serialized into
    /// the fixed flat i64 layout (scalar > i64, sequence over capacity, ...).
    /// Callers must propagate it — never panic — so the CLI can retry the
    /// check with flat state storage disabled.
    #[must_use]
    pub fn flat_layout_unsupported_value(detail: impl Into<String>) -> Self {
        Self::Runtime(RuntimeCheckError::FlatLayoutUnsupportedValue {
            detail: detail.into(),
        })
    }

    /// If this error is the flat-layout-unsupported-value error, return its
    /// detail message. Used by the CLI to detect the retry-without-flat case.
    #[must_use]
    pub fn flat_layout_unsupported_detail(&self) -> Option<&str> {
        match self {
            Self::Runtime(RuntimeCheckError::FlatLayoutUnsupportedValue { detail }) => {
                Some(detail.as_str())
            }
            _ => None,
        }
