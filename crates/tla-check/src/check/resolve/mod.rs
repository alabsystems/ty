...
/// Resolves Init/Next (and fairness) from the config, or from a `SPECIFICATION`
/// operator when explicit `INIT`/`NEXT` are absent.
///
/// # Errors
/// Returns a [`CheckError`] if neither explicit `INIT`/`NEXT` nor a usable
/// `SPECIFICATION` operator can be resolved from the syntax tree.
