...
fn test_func_structural_equality() {
    ...
    let v1 = Value::Func(Rp::new(f1));
    let v2 = Value::Func(Rp::new(f2));
    assert_eq!(
        v1, v2,
...
fn test_seq_equality_reflexive() {
    let s = Value::Seq(Rp::new(vec![Value::int(1), Value::int(2)].into()));
...
fn test_model_value_equality_by_name() {
    let v1 = Value::ModelValue(Rp::from("m1"));
    let v2 = Value::ModelValue(Rp::from("m1"));
    let v3 = Value::ModelValue(Rp::from("m2"));
...
fn test_model_value_equality_reflexive() {
    for name in ["m1", "m2", "m3"] {
        let v = Value::ModelValue(Rp::from(name));
...
fn test_model_value_type_discrimination() {
    let mv = Value::ModelValue(Rp::from("m1"));
    let b = Value::Bool(true);
    let i = Value::int(1);
    let s = Value::String(Rp::from("m1"));
