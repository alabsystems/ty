...
fn test_json_output_basic() {
    ...
    assert!(json.contains("\"tool\": \"ty\""), "JSON: {}", json);
...
fn test_value_to_json() {
    ...
    assert!(matches!(
        value_to_json(&Value::String(Rp::from("hello"))),
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_jsonl_output_serializes_backend_capability_report_for_summarizer() {
    let evidence = "trust-cg trust_cg_admission_blocker source=NativeInstallGateAdmissionSummary schema=ty.trust_cg.native_admission_evidence_report.v1 status_code=rejected reason_code=missing_manifest";
    let stats = CheckStats {
        backend_capability_report: Some(serde_json::json!({
            "schema": "ty.trust_cg.native_admission_evidence_report.v1",
            "schema_version": 1,
            "backend": "trust-cg",
            "kind": "ty_native_activation",
            "evidence": [evidence],
            "fields": {
    ...
    assert_eq!(
        jsonl_row["backend_capability_report"]["fields"]["reason_code"].as_str(),
