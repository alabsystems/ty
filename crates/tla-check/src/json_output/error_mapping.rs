...
pub(super) fn error_code_from_check_error(error: &CheckError) -> String {
    match error {
        ...
        CheckError::Runtime(RuntimeCheckError::FlatLayoutUnsupportedValue { .. }) => {
            error_codes::SYS_SETUP_ERROR.to_string()
        }
    }
}
...
pub(super) fn suggestion_from_check_error(error: &CheckError) -> Option<ErrorSuggestion> {
    match error {
        ...
        CheckError::Runtime(RuntimeCheckError::FlatLayoutUnsupportedValue { detail }) => {
            Some(ErrorSuggestion {
                action: format!(
                    "Re-run with flat state storage disabled (the CLI does this \
                     automatically): {detail}"
                ),
                example: Some("TY_NO_FLAT_BFS=1 ty check Spec.tla".to_string()),
                options: vec![
                    "The spec produced a value the flat i64 state layout cannot represent \
                     (e.g. an integer beyond i64 or a sequence past its proven capacity)"
                        .to_string(),
                ],
            })
        }
