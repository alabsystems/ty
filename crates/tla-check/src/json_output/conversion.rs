...
impl JsonOutput {
    /// Builder: sets the soundness provenance record.
    pub fn with_soundness(mut self, soundness: SoundnessProvenance) -> Self {
        self.soundness = soundness;
        self
    }

    /// Builder: sets the search-completeness classification.
    pub fn with_completeness(mut self, completeness: SearchCompleteness) -> Self {
        self.completeness = completeness;
        self
    }

    /// Builder: marks the result as a CLI-level error with the given code, message,
    /// and optional remediation suggestion.
    ...
    pub fn with_check_result(mut self, result: &CheckResult, elapsed: Duration) -> Self {
        match result {
            ...
            CheckResult::LimitReached { limit_type, stats } => {
                ...
                self.result.error_type = Some(
                    match limit_type {
                        ...
                        crate::LimitType::Time => "time_limit",
                    }
                    .to_string(),
                );
            ...
            CheckResult::Vacuous { reason, stats } => {
                // VACUOUS is a distinct verdict (exit 3), not a property failure.
                self.result.status = "vacuous".to_string();
                self.result.error_type = Some("vacuous".to_string());
                self.result.error_code = Some(error_codes::TLC_VACUOUS.to_string());
                self.result.error_message = Some(reason.message());
                self.result.suggestion = Some(ErrorSuggestion {
                    action: "The model proved nothing because its basis is empty / \
                             never-exercised. Fix the spec, or relax with \
                             --allow-vacuous=<class>."
                        .to_string(),
                    example: Some(format!(
                        "Vacuity class: {}. Use --allow-vacuous={} to downgrade to a warning.",
                        reason.class().as_str(),
                        reason.class().as_str()
                    )),
                    options: vec![
                        "Ensure Init admits at least one state".to_string(),
                        "Ensure each anchored action is enabled in some reachable state"
                            .to_string(),
                        "Ensure each invariant constrains some reachable state".to_string(),
                    ],
                });
                self.statistics = stats_to_json(stats, elapsed);
            }
    ...
    // NOTE: the former `with_masked_symbolic_violation` (a cex-less JSON shape
    // for evaluator-confirmed symbolic violations) is retired: a CONFIRMED
    // symbolic counterexample is now substituted into the standard
    // `CheckResult::InvariantViolation` and serialized by `with_check_result`,
    // which populates the `counterexample` field `ty verdict-emit` consumes.
    // Only the fail-closed UNVALIDATED shape below remains cex-less.

    /// Mark this run as INCONCLUSIVE because a symbolic lane won the fused
    /// `Violated` race — truncating the BFS lane — with a counterexample the
    /// explicit-state evaluator could not confirm. Fail closed: the truncated
    /// BFS result proves nothing, so neither "no error" nor an unvalidated
    /// violation may be reported.
    pub fn with_unvalidated_symbolic_race(
        mut self,
        lane: &str,
        detail: &str,
        ...
        self.result.status = "error".to_string();
        self.result.error_type = Some("inconclusive_symbolic_race".to_string());
        self.result.error_message = Some(format!(
            "Inconclusive: the {lane} lane won the verdict race with a counterexample the \
             explicit-state evaluator could not confirm; the race-truncated BFS result is \
             not authoritative: {detail}"
        ));
        self.result.suggestion = Some(ErrorSuggestion {
            action: "Re-run with --bfs-only for an authoritative explicit-state verdict"
                .to_string(),
            example: None,
            options: vec![],
        });
...
fn stats_to_json(stats: &CheckStats, elapsed: Duration) -> StatisticsInfo {
    ...
    StatisticsInfo {
        ...
        property_check: stats
            .property_check
            .has_data()
            .then(|| PropertyCheckStatsInfo {
                implied_action_transition_checks: stats
                    .property_check
                    .implied_action_transition_checks,
                implied_action_term_evals: stats.property_check.implied_action_term_evals,
                implied_action_time_seconds: stats.property_check.implied_action_time_seconds(),
            }),
    }
}

/// Convert Trace to CounterexampleInfo with state diffs
pub(crate) fn trace_to_counterexample(
    trace: &Trace,
