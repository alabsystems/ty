...
//!
//! MCKVSSafetySmall (3.4M states): TY barely scales in parallel — ~1.4x
//! at 8 workers vs TLC's ~3.5x. Single-worker TY is ~0.65x TLC.
//!
//! MCBoulanger (7.9M states): TY 1-worker is ~1.1x TLC, but parallel
...
/// Stable sentinel message for the empty-Init ("no solutions") case.
///
/// When Init enumerates to zero initial states, the reachable set is empty.
/// With a declared checkable basis this is the V1 vacuity case
/// (`VacuityReason::EmptyReachableSet`), not a plain configuration error.
/// The verdict remap in [`AdaptiveChecker::check`] keys on this exact string.
/// Design: TRUST_VACUITY_GATE §1.A (V1).
pub(crate) const EMPTY_INIT_NO_SOLUTIONS: &str = "Init predicate has no solutions";

/// True iff `e` is the empty-Init ("no solutions") signal — Init enumerated to
/// zero initial states. Distinct from an Init that *cannot* be enumerated due to
/// an unsupported form (those carry a different hint), so this never
/// false-positives on a genuine enumeration error.
pub(crate) fn is_empty_init_no_solutions(e: &CheckError) -> bool {
    matches!(
        e,
        CheckError::Config(ConfigCheckError::InitCannotEnumerate(msg))
            if msg == EMPTY_INIT_NO_SOLUTIONS
    )
}
...
impl AdaptiveChecker {
    ...
    fn run_pilot(&mut self) -> Result<PilotAnalysis, CheckError> {
        ...
        if num_initial == 0 {
            return Err(
                ConfigCheckError::InitCannotEnumerate(EMPTY_INIT_NO_SOLUTIONS.to_string()).into(),
            );
        }
    ...
    pub fn check(&mut self) -> (CheckResult, Option<PilotAnalysis>) {
        ...
        let mut analysis = match self.run_pilot() {
            Ok(a) => a,
            Err(e) => {
                // V1 vacuity gate (TRUST_VACUITY_GATE §1.A): an over-constrained
                // Init that enumerates zero initial states is the empty-reachable-
                // set vacuity, not a plain config error — when the module declares
                // a checkable basis (Init/Next/invariant/property). Remap to the
                // distinct VACUOUS verdict (exit 3) instead of FAILED (exit 1).
                if is_empty_init_no_solutions(&e) && self.config.declares_checkable_basis() {
                    return (
                        CheckResult::Vacuous {
                ...
                return (check_error_to_result(e, &CheckStats::default()), None);
            }
        };
