...
fn test_from_value_lazy_func_returns_error() {
    ...
    let lf = LazyFuncValue::new(
        ...
        LazyFuncCaptures::new(Arc::new(HashMap::new()), None, None, None),
    );

    let err = SerializableValue::from_value(&Value::LazyFunc(Rp::new(lf))).unwrap_err();
    assert_eq!(err.kind(), io::ErrorKind::InvalidData);
    assert!(
...
fn test_from_value_closure_returns_error() {
    ...
    let err = SerializableValue::from_value(&Value::Closure(Rp::new(cv))).unwrap_err();
    assert_eq!(err.kind(), io::ErrorKind::InvalidData);
    assert!(
