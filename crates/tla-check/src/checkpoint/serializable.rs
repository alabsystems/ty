...
pub enum SerializableValue {
    /// A boolean.
    Bool(bool),
    /// An integer, stored as a decimal string to preserve arbitrary precision.
    ...
    /// A record as field-name/value pairs.
    Record(Vec<(String, SerializableValue)>),
    /// A tuple of values.
    Tuple(Vec<SerializableValue>),
    /// An uninterpreted model value.
    ModelValue(String),
...
impl SerializableValue {
    ...
    pub fn from_value(value: &Value) -> io::Result<Self> {
        match value {
            ...
            Value::Bag(b) => {
                // Serialize bag as set of tuples (like Func — the resume path
                // reconstructs the general representation; sound rep downgrade)
                let pairs: Vec<_> = b
                    .entries()
                    .map(|(k, c)| {
                        Ok(SerializableValue::Tuple(vec![
                            SerializableValue::from_value(k)?,
                            SerializableValue::from_value(&Value::SmallInt(c))?,
                        ]))
                    })
                    .collect::<io::Result<_>>()?;
                Ok(SerializableValue::Set(pairs))
        ...
        match self {
            ...
            SerializableValue::Record(fields) => {
                let record: RecordValue = fields
                    .iter()
                    .map(|(k, v)| Ok((std::sync::Arc::<str>::from(k.as_str()), v.to_value()?)))
                    .collect::<io::Result<RecordValue>>()?;
            ...
            SerializableValue::Interval { lo, hi } => {
                ...
                Ok(Value::Interval(Rp::new(crate::value::IntervalValue::new(
                    lo, hi,
                ))))
            }
        }
...
pub struct SerializableState {
    /// Variable name/value pairs comprising the state.
