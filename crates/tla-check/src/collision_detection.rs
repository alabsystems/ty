...
pub fn format_collision_report(stats: &CollisionStats, mode: CollisionCheckMode) -> String {
    ...
    match mode {
        ...
        CollisionCheckMode::Sampling { interval } => {
            let _ = writeln!(
                report,
                "  Collision detection: sampling (1/{interval} states)"
            );
            let _ = writeln!(report, "  States sampled: {}", stats.states_sampled);
            let _ = write!(
                report,
                "  Collisions detected: {}",
        ...
        CollisionCheckMode::Full => {
            ...
            let _ = writeln!(report, "  States verified: {}", stats.states_sampled);
            let _ = write!(
                report,
                "  Collisions detected: {}",
                stats.collisions_detected
