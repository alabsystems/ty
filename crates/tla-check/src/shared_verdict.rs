...
//!
//! Provides a lock-free [`SharedVerdict`] that enables parallel BFS and ay
...
/// Certificate verification state for analytical-engine satisfaction claims.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CertificateVerification {
    /// A checker/verifier has validated the analytical certificate.
    Verified,
    /// No certificate verifier is available or the certificate has not been checked.
    MissingVerifier,
}
...
impl SharedVerdict {
    ...
    /// Publish an analytical-engine verdict with certificate gating.
    ///
    /// Analytical structural eligibility is not a proof. An analytical lane may
    /// publish `Violated` when it has a real counterexample, but it may publish
    /// `Satisfied` only after a checker/verifier has validated its certificate.
    /// Missing certificate verification leaves the slot unresolved.
    pub fn publish_analytical(
        &self,
        verdict: Verdict,
        certificate: CertificateVerification,
    ) -> bool {
        match verdict {
            Verdict::Satisfied if certificate != CertificateVerification::Verified => false,
            Verdict::Satisfied | Verdict::Violated => self.publish(verdict),
...
mod tests {
    ...
    #[test]
    fn test_analytical_satisfied_requires_certificate_verification() {
        let sv = SharedVerdict::new();
        assert!(
            !sv.publish_analytical(Verdict::Satisfied, CertificateVerification::MissingVerifier)
        );
        assert!(!sv.is_resolved());
        assert_eq!(sv.get(), None);
    }
    ...
    #[test]
    fn test_analytical_satisfied_with_verified_certificate_resolves() {
        let sv = SharedVerdict::new();
        assert!(sv.publish_analytical(Verdict::Satisfied, CertificateVerification::Verified));
        assert!(sv.is_resolved());
        assert_eq!(sv.get(), Some(Verdict::Satisfied));
    }

    #[test]
    fn test_analytical_violated_does_not_require_certificate() {
        let sv = SharedVerdict::new();
        assert!(sv.publish_analytical(Verdict::Violated, CertificateVerification::MissingVerifier));
        assert!(sv.is_resolved());
        assert_eq!(sv.get(), Some(Verdict::Violated));
    }
