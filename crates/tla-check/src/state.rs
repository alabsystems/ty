...
// projects the flat-admissible variable subset while leaving un-flattenable
// vars compound, so per-action native dispatch is no longer whole-state vetoed.
#[allow(dead_code)]
mod flat_state_store;
// WP-11 (wishlist item 9) slice 1: flat-space symmetry canonicalization —
// per-group-element compiled slot actions + streaming lexmin over the closed
// permutation group. DEAD BY DESIGN until slice 2 wires it into the BFS
// fingerprint domain behind the FlatBufferCanonicalizationAuthority fence
...
mod nested_set_discovery;
// Step B slide kernel: `pub(crate)` API is consumed by tests + the (opt-in)
// native successor fast-path; `allow(dead_code)` covers the parts not yet wired.
#[allow(dead_code)]
mod nested_set_slide;
mod payload_witness;
mod state_layout;
mod symmetry;
mod value_hash;
...
pub(crate) use diff_successor::{
    compute_changes_fingerprint, compute_diff_fingerprint_with_xor, DiffChanges, DiffSuccessor,
};
...
#[allow(unused_imports)]
pub(crate) use flat_state::{
    FlatReconstructionError, FlatSerializationError, FlatState, MAX_RECORD_SET_BITMASK_SLOTS,
    MAX_RECORD_SET_BITMASK_UNIVERSE,
};
#[allow(unused_imports)]
pub(crate) use flat_state_store::FlatStateStore;
#[allow(unused_imports)]
pub(crate) use flat_successor::{flat_state_bytes, FlatSuccessor};
...
pub(crate) use layout_inference::{
    ...
    collect_scalar_tuple_union_var_writer_proofs, collect_sequence_capacity_writer_proofs_with_ops,
    collect_sequence_element_layout_proofs_with_ops,
    collect_sequence_element_layout_writer_proofs_with_ops,
    collect_sequence_fixed_domain_type_proofs_with_ops, collect_sequence_universe_proofs,
    ...
    infer_layout_from_wavefront_with_sequence_proofs,
    infer_layout_with_sequence_layout_and_tagged_proofs, infer_layout_with_sequence_layout_proofs,
...
#[allow(unused_imports)]
pub(crate) use state_layout::record_set_native_flat_primary_enabled;
#[allow(unused_imports)]
pub(crate) use state_layout::seq_heuristic_capacity_enabled;
#[allow(unused_imports)]
pub(crate) use state_layout::VarLayoutKind;
#[allow(unused_imports)]
pub(crate) use state_layout::{
    FixedScalarRangeProof, FlatScalarValue, FlatValueLayout, SequenceBoundEvidence, SlotType,
    StateLayout, TupleKeyedArrayRangeEncoding,
};
