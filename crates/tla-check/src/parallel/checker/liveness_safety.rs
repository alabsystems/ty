...
impl ParallelChecker {
    ...
    /// Part of #3746: When `TY_STRICT_LIVENESS=1`, panics on missing states
    ...
    fn get_array_state_for_fp(
        ...
        if result.is_none() && crate::liveness::debug::strict_liveness() {
            panic!(
                "TY_STRICT_LIVENESS: state missing for fp {fp} in \
    ...
    pub(super) fn build_trace_from_fingerprint(
        ...
        // Part of #3801: resolve each fingerprint from self.seen FIRST (per-key
        // DashMap::get is strongly consistent, unlike iteration), falling back
        // to the provided state_cache. This mirrors `get_array_state_for_fp` —
        // the same resolution the violation detector used to find the states —
        // instead of the old all-or-nothing `state_cache.is_empty()` switch,
        // which silently dropped every fingerprint missing from a NON-empty
        // state_cache and emitted an EMPTY counterexample trace for parallel
        // action-property violations (workers > 1) while the sequential path
        // produced the full trace.
