...
impl ParallelChecker {
    ...
    pub(super) fn run_plan_group(
        ...
        state_cache: &Arc<FxHashMap<Fingerprint, ArrayState>>,
        prop_name: &str,
        ...
        // Reconstruct transient `State`s from the compact ArrayState cache for
        // the explore closures (successors consumed immediately; not retained).
            ...
            |state: &State| -> Result<Vec<State>, tla_value::error::EvalError> {
                ...
                let mut succs: Vec<State> = entry
                    .map(|fps| {
                        fps.iter()
                            .filter_map(|sfp| {
                                state_cache.get(sfp).map(|arr| arr.to_state(&registry))
                            })
                            .collect()
                    })
                    .unwrap_or_default();
        ...
        let explore_result = if matches!(&plan.tf, LiveExpr::Bool(true)) {
            ...
            let init_states: Vec<State> = graph
                .init_fps
                .iter()
                .filter_map(|fp| state_cache.get(fp).map(|arr| arr.to_state(&registry)))
                .collect();
        ...
        match liveness_result {
            ...
            // SOUNDNESS (#liveness-fp-only-false-hold): the parallel checker
            // materializes the full liveness state cache up front, so this
            // should be unreachable. Fail loudly rather than silently treating
            // an unverifiable candidate violation as satisfied.
            LivenessResult::CandidateStatesUnavailable { missing_fp } => {
                Some(check_error_to_result(
                    LivenessCheckError::RuntimeFailure(format!(
                        "Liveness check for '{prop_name}' found a candidate violating cycle \
                         but the concrete state for fingerprint {missing_fp} was missing \
                         from the materialized state cache; verdict is UNKNOWN."
                    ))
                    .into(),
                    stats,
                ))
    ...
    pub(super) fn materialize_liveness_state_cache(
        ...
        // The behavior graph already holds the materialized cache behind an
        // `Arc`; share it (cheap refcount bump) instead of deep-copying the
        // entire ~500K-entry map. The map is read-only on the liveness path,
        // so sharing is sound and a pure memory win.
