...
impl ParallelChecker {
    ...
    fn run_liveness(
        ...
        // Declared-SYMMETRY wrong-verdict fix (was Part of #2227's
        // warn-and-continue): declared SYMMETRY is disabled for the whole run
        // when any PROPERTY requires the genuine liveness checker. The
        // decision is normally settled in `prepare_check` (which then installs
        // no permutations); settle it here too so unit-test paths that bypass
        // `prepare_check` agree before the liveness graph is built, and so the
        // warning is printed exactly once per run.
        let _ = self.symmetry_disabled_for_liveness(ctx);
        ...
        let mut materialized_state_cache: Option<Arc<FxHashMap<Fingerprint, ArrayState>>> = None;
    ...
    pub(in crate::parallel::checker) fn split_property_terms_for_liveness(
        ...
        let liveness_expr = if liveness_terms.is_empty() {
            ...
            // Keep only the terms that the shared PROPERTY planner classified as
            // liveness. For named-instance specs like `R!Spec`, using the whole
            // ModuleRef here reintroduces extracted safety terms such as
            // `[][R!Next]_R!vars`; after negation those become bare `<>~Action`
            // formulas, which the liveness checker correctly rejects.
    ...
    fn check_single_property(
        ...
        materialized_state_cache: &mut Option<Arc<FxHashMap<Fingerprint, ArrayState>>>,
        root_name: &str,
        prop_name: &str,
        partial_graph: bool,
    ) -> Option<CheckResult> {
        ...
        let liveness_expr = planned.liveness_expr?;
        if partial_graph {
        ...
        if is_trivially_unsatisfiable(&negated) && !crate::check::debug::skip_liveness() {
            return Some(check_error_to_result(
...
"#;
    ...
    /// Declared-SYMMETRY wrong-verdict fix (parallel engine): with a genuine
    /// liveness PROPERTY, the parallel checker must ignore declared SYMMETRY —
    /// `prepare_check` settles `symmetry_disabled_for_liveness` and installs
    /// no mvperms, so BFS fingerprints raw states (3 states, not the 1-state
    /// orbit quotient) and post-BFS liveness returns the unreduced verdict.
    #[cfg_attr(test, ntest::timeout(30000))]
    #[test]
    fn parallel_declared_symmetry_ignored_for_genuine_liveness_matches_unreduced() {
        ...
        match checker.check() {
            CheckResult::Success(stats) => {
                assert_eq!(
                    ...
                    "parallel: declared SYMMETRY must be ignored under genuine liveness — \
                     expected the unreduced state count (the orbit quotient would give 1)"
                );
            }
            other => panic!(
                "parallel declared SYMMETRY + genuine liveness must return the unreduced \
                 verdict (HOLD), got {other:?}"
            ),
        }
    }
