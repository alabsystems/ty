...
"#;
    ...
    // during successor enumeration, which routes to the run's diagnostics
    // via the worker's RunDiagnosticsScope. Here we install the same scope
    // and inject the count through the production recording entry point.
    {
        let _diag_scope = crate::run_diagnostics::RunDiagnosticsScope::enter(
            std::sync::Arc::clone(&checker.run_diagnostics),
        );
    ...
    // This synthetic finalize feeds 0 states with a declared Init/Next, so the
    // V1 vacuity gate now reports `Vacuous(EmptyReachableSet)` instead of
    // `Success`. Both variants carry the same `CheckStats`; the regression this
    // test guards (suppressed-guard-error carrying) is independent of the verdict.
    let stats = match result {
        CheckResult::Success(stats) | CheckResult::Vacuous { stats, .. } => stats,
        other => panic!("Expected Success or Vacuous, got: {other:?}"),
    };
