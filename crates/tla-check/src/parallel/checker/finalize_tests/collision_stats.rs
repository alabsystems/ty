...
fn test_parallel_collision_counters_appear_in_finalize_success() {
    ...
    // Synthetic 0-state finalize with a declared Init/Next now reports the V1
    // vacuity verdict; both Success and Vacuous carry the same CheckStats.
    let (CheckResult::Success(stats) | CheckResult::Vacuous { stats, .. }) = result else {
        unreachable!("expected Success or Vacuous, got {result:?}");
    };
