...
impl ParallelChecker {
    ...
    fn prepare_on_the_fly_fingerprinting(
        ...
        // Wrong-verdict fix: on-the-fly liveness fingerprinting must match the
        // BFS decision — declared SYMMETRY is disabled for the whole run when
        // any PROPERTY requires the genuine liveness checker.
        let mvperms = if self.symmetry_active(ctx) {
    ...
    fn run_plan_group_on_the_fly(
        ...
        let use_owned_compact_cache =
            !needs_canonical_fp && crate::liveness::debug::liveness_otf_compact_cache_enabled();
        if use_owned_compact_cache {
            // Keep owned compact payloads confined to exact raw fingerprints.
            // VIEW and symmetry continue to retain concrete witness states.
        ...
        match checker.check_liveness_grouped(plan, max_fairness_tag) {
            ...
            // SOUNDNESS (#liveness-fp-only-false-hold): on-the-fly exploration
            // stores concrete states in the behavior graph, so this should be
            // unreachable. Fail loudly rather than silently treating an
            // unverifiable candidate violation as satisfied.
            LivenessResult::CandidateStatesUnavailable { missing_fp } => {
                Some(check_error_to_result(
                    LivenessCheckError::RuntimeFailure(format!(
                        "On-the-fly liveness for '{prop_name}' found a candidate violating \
                         cycle but the concrete state for fingerprint {missing_fp} was \
                         unavailable; verdict is UNKNOWN."
                    ))
                    .into(),
                    stats,
                ))
