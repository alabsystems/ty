...
/// Check if `--show-tiers` / `TY_SHOW_TIERS=1` is enabled.
/// Cached via `OnceLock` (Part of #4114).
fn show_tiers_enabled() -> bool {
    static CACHED: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *CACHED.get_or_init(|| std::env::var("TY_SHOW_TIERS").is_ok_and(|v| v == "1"))
}
...
impl ParallelChecker {
    ...
    pub(super) fn join_and_resolve_terminal(
        ...
        let final_stats = CheckStats {
            ...
            coverage: None,
            phantom_dequeues: 0,
            suppressed_guard_errors: self.run_diagnostics.take_suppressed_guard_errors(),
        ...
        // V3 vacuity gate (TRUST_VACUITY_GATE §1.A): static vacuously-true
        // invariant WARNINGs. (V2 dead-action detection needs per-action coverage
        // which the parallel path does not yet maintain; it is surfaced on the
        // sequential path. V3 is a cheap static analysis available everywhere.)
        let mut final_stats = final_stats;
        for inv_name in &self.config.invariants {
            if let Some(def) = self.op_defs.get(inv_name) {
                if let Some(warning) =
                    crate::coverage::invariant_tension::classify_invariant(inv_name, &def.body)
                {
                    final_stats.vacuity_warnings.push(warning);
                }
            }
        }

        // V1 vacuity gate: zero reachable states + a declared checkable basis
        // (Init/Next/invariant/property) is a vacuous run, not a success.
        // Design: TRUST_VACUITY_GATE §1.A (V1).
        if final_stats.states_found == 0 && self.config.declares_checkable_basis() {
