...
impl ParallelChecker {
        ...
        T::Mode: Clone,
    {
        ...
        for (worker_id, local_queue) in workers.into_iter().enumerate() {
            ...
            let run_diagnostics = Arc::clone(&self.run_diagnostics);

            let handle = thread::Builder::new()
                .stack_size(16 * 1024 * 1024)
                .spawn(move || {
                    crate::eval::set_enabled_hook(crate::enabled::eval_enabled_cp);
                    // Attribute this worker's diagnostics to the owning run.
                    let _diag_scope =
                        crate::run_diagnostics::RunDiagnosticsScope::enter(run_diagnostics);
                    run_worker_unified(
                        worker_id,
                        local_queue,
                        injector,
    ...
    fn seed_and_spawn_shared_queue<T: BfsWorkItem>(
        ...
        // Part of #16: shared via Arc; build_model_config clones the pointer.
        expanded_op_defs: &Arc<FxHashMap<String, tla_core::ast::OperatorDef>>,
