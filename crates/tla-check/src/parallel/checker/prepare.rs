...
/// Build a state-variable name→index map from a `VarRegistry`.
///
...
fn state_var_index_map_for_registry(
...
impl ParallelChecker {
    /// Decide ONCE whether declared SYMMETRY must be ignored for this run
    /// because a PROPERTY requires the genuine liveness checker.
    ///
    /// Symmetry reduction is unsound for liveness: TY does not implement the
    /// Emerson-Sistla annotated quotient, and the plain orbit quotient can
    /// hide or synthesize fair cycles (wrong verdicts). Mirrors the sequential
    /// checker's gate in run_prepare.rs (declared-SYMMETRY wrong-verdict fix).
    /// The decision is memoized so BFS fingerprinting, on-the-fly liveness,
    /// the post-BFS liveness graph, and fp-only replay all see the same
    /// answer; the first caller (normally `prepare_check`, before any worker
    /// fingerprints a state) settles it and prints the warning. Pure-safety
    /// PROPERTY runs (`[]P`) keep symmetry (Part of #2227).
    pub(super) fn symmetry_disabled_for_liveness(&self, ctx: &EvalCtx) -> bool {
        *self.symmetry_disabled_for_liveness.get_or_init(|| {
            let disabled = self.config.symmetry.is_some()
    ...
    /// Whether declared SYMMETRY is active for this run: declared in the
    /// config AND not disabled by the liveness soundness gate above.
    pub(super) fn symmetry_active(&self, ctx: &EvalCtx) -> bool {
        self.config.symmetry.is_some() && !self.symmetry_disabled_for_liveness(ctx)
    }

    /// Post-decision read of `symmetry_active` for callers without an
    /// `EvalCtx` (post-BFS liveness graph building). Falls back to the raw
    /// config flag if the decision was never settled, which is only possible
    /// on unit-test paths that bypass both `prepare_check` and
    /// `run_liveness`'s settle call.
    pub(super) fn symmetry_active_settled(&self) -> bool {
        self.config.symmetry.is_some()
            && !self
                .symmetry_disabled_for_liveness
                .get()
                .copied()
                .unwrap_or(false)
    }
    ...
    fn prepare_por_state(
        ...
        // POR is also disabled when trace invariants (--trace-inv) are
        // present: the ample-set C2 visibility set is built solely from
        // STATE invariants, so the reduction can prune the only history
        // violating a history-dependent trace invariant (false PASS).
        // Adaptive routing already forces trace-invariant runs to the
        // sequential checker (ParallelChecker does not evaluate them), so
        // this gate is defense-in-depth for direct ParallelChecker use;
        // it matches the sequential gate in model_checker/run.rs.
        ...
        // The independence matrix is indexed by `por_actions` — the SAME
        // (non-primed) enumeration decomposition the workers iterate — by
        // construction: `extract_detected_action_dependencies` expands EACH
        // action's expression with primes and extracts one unioned
        // read/write/unchanged set per action. This replaces the old
        // whole-Next re-expansion + re-detection, whose finer with-primes
        // decomposition made C1/C2 read off the WRONG rows and forced a
        // fail-closed POR skip on length mismatch. (audit-2026-07 #11)
        let independence = {
            let action_dependencies =
                crate::por::extract_detected_action_dependencies(ctx, &por_actions);
            debug_assert_eq!(
    ...
    pub(super) fn prepare_base_ctx(&self) -> Result<EvalCtx, CheckResult> {
        ...
        // checker's Config onto the shared context (per-checker, not global).
        crate::enumerate::apply_per_state_successor_cap(&mut ctx, &self.config);
    ...
    pub(super) fn prepare_check(&self) -> Result<CheckPreparation, CheckResult> {
        ...
        // Declared-SYMMETRY wrong-verdict fix: `symmetry_active` settles the
        // memoized liveness soundness decision HERE — before any worker
        // fingerprints a state — and returns false (no permutations installed
        // anywhere in the run) when a PROPERTY requires the genuine liveness
        // checker, mirroring the sequential checker's gate in run_prepare.rs.
        let mvperms: Arc<Vec<crate::value::MVPerm>> = if self.symmetry_active(&ctx) {
        ...
        let eval_implied_actions: Arc<Vec<crate::checker_ops::EvalImpliedActionTerm>> = {
            let mut terms = property_classification
                .as_ref()
            ...
            // term structure once (interpreter callbacks for unsupported leaf
            // operators), shared by all workers via Arc. Kill switch:
            // TY_NO_IMPLIED_ACTION_BYTECODE=1.
            if !terms.is_empty() {
        ...
        let (por_actions, por_independence, por_visibility) =
            self.prepare_por_state(&ctx, detected_actions_info, &eval_state_invariants);
        ...
        // Skipped when TY_SKIP_LIVENESS=1 (the entire liveness pipeline is
        ...
        Ok(CheckPreparation {
            ...
            var_registry,
            // Part of #16: share the operator-definition map across workers via
            // a single Arc instead of a per-worker deep clone.
            expanded_op_defs: Arc::new(expanded_op_defs),
    ...
    fn compile_invariant_bytecode_for_workers(
        ...
        // (mapped to parent state vars) to LoadVar slots. See sequential
        // `compile_invariant_bytecode` for rationale.
        let state_var_map = state_var_index_map_for_registry(&registry);
        let mut compiled =
    ...
    // Only exercised by `prepare_tests`; the `action_bytecode` consumer field was
    // retired (#3960 JIT next-state path never read it), leaving tests as the
    // sole callers.
    ...
    fn compile_action_bytecode_for_workers(
        ...
        let tir_callees =
            tla_eval::bytecode_vm::collect_bytecode_namespace_callees(&root, &dep_refs);
        // INSTANCE coverage: resolve instance-imported variable references
        // (mapped to parent state vars) to LoadVar/StoreVar slots. See
        // sequential `compile_action_bytecode` for rationale.
        let state_var_map = state_var_index_map_for_registry(&registry);
    ...
    fn infer_state_layout_from_initial_states(
        ...
        let var_layouts: Vec<tla_jit_abi::VarLayout> = values
            .iter()
            .map(|v| tla_jit_abi::infer_var_layout(v))
            .collect();
        Some(tla_jit_abi::StateLayout::new(var_layouts))
    }

    // Wave 11a (Part of #4267): `compile_jit_next_state_cache` removed
    // alongside `SharedJitNextStateCache`. The retired compatibility path has been
    // deleted; the parallel next-state dispatch fell
