...
pub(super) struct CheckPreparation {
    ...
    /// Part of #16 (OOM hardening): shared via `Arc` so the per-worker
    /// `WorkerModelConfig` clones the pointer instead of deep-copying the whole
    /// operator-definition map N times (once per worker).
    pub(super) expanded_op_defs: Arc<FxHashMap<String, OperatorDef>>,
