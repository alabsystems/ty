...
/// Part of #3285: Read `TY_PARALLEL_FPSET` env var to select the FPSet backend
...
pub(super) fn parallel_fpset_mode() -> Result<StorageMode, CheckError> {
    #[cfg(test)]
    let val = std::env::var("TY_PARALLEL_FPSET").ok();
    #[cfg(not(test))]
    let val = {
        use std::sync::OnceLock;
        static CACHED: OnceLock<Option<String>> = OnceLock::new();
        CACHED
            .get_or_init(|| std::env::var("TY_PARALLEL_FPSET").ok())
            .clone()
    };
    match val.as_deref() {
        ...
        Some(other) => Err(ConfigCheckError::Setup(format!(
            "TY_PARALLEL_FPSET={other:?} is not recognized. \
...
pub(super) fn parallel_readonly_value_caches_enabled() -> Result<bool, CheckError> {
    #[cfg(test)]
    let val = std::env::var("TY_PARALLEL_READONLY_VALUE_CACHES").ok();
    #[cfg(not(test))]
    let val = {
        use std::sync::OnceLock;
        static CACHED: OnceLock<Option<String>> = OnceLock::new();
        CACHED
            .get_or_init(|| std::env::var("TY_PARALLEL_READONLY_VALUE_CACHES").ok())
            .clone()
    };
    match val.as_deref() {
        ...
        Some("1") => Ok(true),
        Some(other) => Err(ConfigCheckError::Setup(format!(
            "TY_PARALLEL_READONLY_VALUE_CACHES={other:?} is not recognized. \
