...
impl ParallelChecker {
    ...
    pub(super) fn build_shared_state(
        ...
        WorkerSharedState {
            ...
            // Wrong-verdict fix: witness recording exists solely for liveness
            // under symmetry. When declared SYMMETRY is disabled for the run
            // (genuine temporal properties; decision settled in prepare_check
            // before workers spawn), hand workers no witness map — mvperms are
            // empty, so canonical == concrete and witnesses are dead weight.
            successor_witnesses: self
                .successor_witnesses
                .as_ref()
                .filter(|_| self.symmetry_active_settled())
                .map(Arc::clone),
    ...
    pub(super) fn build_model_config(
        ...
        expanded_op_defs: &Arc<FxHashMap<String, OperatorDef>>,
        ...
        eval_implied_actions: &Arc<Vec<crate::checker_ops::EvalImpliedActionTerm>>,
        ...
        WorkerModelConfig {
            ...
            // Part of #16: clone the Arc pointer, not the whole map, per worker.
            op_defs: Arc::clone(expanded_op_defs),
    ...
    pub(crate) fn reconstruct_trace(&self, end_fp: Fingerprint) -> Trace {
        // The `seen` DashMap is populated whenever workers store full states OR
        // cache states for liveness (worker::core_adapter: `store_full_states ||
        // cache_for_liveness`, mirrored here by `successors.is_some()` — the
        // same proxy `get_array_state_for_fp` uses). The old `store_full_states`-
        // only guard returned an EMPTY trace for liveness-cached runs (e.g. an
        // action-level PROPERTY violation with workers > 1, fp-only storage),
        // even though every trace state was resolvable — the sequential checker
        // produced the full trace for the same spec.
        if !self.store_full_states && self.successors.is_none() {
            return Trace::new();
        }
