...
impl ParallelChecker {
    ...
    /// trigger a hard error unless `TY_STRICT_LIVENESS=0` overrides.
    ///
    ...
    pub(super) fn build_liveness_graph_data(
        ...
        // Wrong-verdict fix: consult the settled per-run decision, not the raw
        // config flag — declared SYMMETRY is disabled under genuine liveness.
        let has_symmetry = self.symmetry_active_settled();
        let mut state_fp_to_canon_fp: FxHashMap<Fingerprint, Fingerprint> = if has_symmetry {
            ...
            for (fp, state) in &state_cache {
                let raw_fp =
                    crate::state::compute_fingerprint_from_compact_array(state.values(), &registry);
                map.insert(raw_fp, *fp);
            }
            map
        } else {
            FxHashMap::default()
    ...
    fn build_successor_witnesses(
        ...
        for entry in witness_map.iter() {
            ...
            for (to_fp, arr) in entry.value() {
                let raw_fp =
                    crate::state::compute_fingerprint_from_compact_array(arr.values(), registry);
                state_fp_to_canon_fp.entry(raw_fp).or_insert(*to_fp);
                converted.push((*to_fp, arr.clone()));
            }
    ...
    ///   Set `TY_STRICT_LIVENESS=1` to upgrade to a hard error for debugging.
            ...
            return Err(check_error_to_result(
                crate::LivenessCheckError::RuntimeFailure(format!(
                    "Post-BFS liveness state cache is missing {count} of {total} \
                     ...
                     Set TY_STRICT_LIVENESS=0 (or unset) to downgrade to a warning.",
                    count = missing.len(),
                    total = referenced.len(),
                    preview = preview.join(", "),
                ))
    ...
    fn prepare_fp_only_liveness_replay(
        ...
        // Wrong-verdict fix: fp-only replay must fingerprint exactly like BFS
        // did — `symmetry_active` reuses the run's memoized decision, so when
        // declared SYMMETRY was disabled for liveness this replays raw
        // (non-canonical) fingerprints to match.
    ...
    fn replay_parent_successors(
        ...
        if !remaining.is_empty() && crate::check::debug::liveness_profile() {
