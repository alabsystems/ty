...
impl ParallelChecker {
    ...
    pub(super) fn snapshot_runtime_stats(
        ...
        CheckStats {
            ...
            collision_check_stats: self.collision_check_stats(),
    ...
    pub(super) fn run_periodic_maintenance(
        ...
        // N9: `suspend_all` returns false when the pause threshold was only
        // reached because a worker *departed* (terminated on a violation/error/
        // RSS-limit/deadline path) during the suspension window. A departure
        // always sets the shared stop flag, so the run is already winding down
        // and a terminal result is in flight — there is no clean quiescent
        // snapshot to act on. Skip the maintenance action entirely: resume the
        // (still-blocked) paused workers and return an empty outcome so
        // finalization proceeds without a checkpoint/liveness pass on a
        // degraded state. Without the N9 fix, `suspend_all` would instead HANG
        // forever waiting for the departed worker to pause.
        if !self.barrier.suspend_all() {
        ...
        // 2026-07 OOM audit: re-check LIVE pressure immediately before
        // materializing. `create_checkpoint()` clones O(N) fingerprints,
        // depths, and the parent map — exactly when RSS is already high (the
        // memory-triggered checkpoint fires at 70% of the limit), so the copy
        // itself can push the process over the OOM edge mid-checkpoint. If
        // pressure has reached Critical by this point (it may have been only
        // Warning at the top of this function, or grown during the periodic
        // liveness check above), skip materialization and go straight to the
        // graceful stop. Trade-off: a Critical stop loses checkpoint
        // resumability, but a stop without a checkpoint still reports a clean
        // partial LimitReached result — an OOM kill mid-checkpoint loses
        // everything. Warning-level pressure still checkpoints as before.
        let live_pressure = self
            .memory_policy
            .as_ref()
            .map(|p| p.check())
            .unwrap_or(MemoryPressure::Normal);
        let checkpoint = if checkpoint_due && live_pressure == MemoryPressure::Critical {
            eprintln!(
                "Warning: skipping checkpoint under critical memory pressure — \
                 materializing it would copy the entire state store; \
                 stopping without a checkpoint instead."
            );
            // Engage the same graceful stop the Critical arm above uses, in
            // case pressure escalated to Critical only after the initial
            // check (so the run does not continue for another whole tick).
            let _ = self.states_at_stop.set(current_states);
            self.stop_flag.store(true, Ordering::SeqCst);
            if periodic_result.is_none() {
                let snapshot =
