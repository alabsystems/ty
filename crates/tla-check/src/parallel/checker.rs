...
use super::ty_debug;
use super::{
    check_and_warn_capacity, check_state_constraints_array, timing_enabled, use_handle_state,
    use_shared_queue, FairnessConstraint, FxBuildHasher, ParallelChecker,
    ParallelCollisionDiagnostics, ParentLog, WorkBarrier, WorkerResult, WorkerStats,
    CAPACITY_NORMAL,
};
use super::{
...
impl ParallelChecker {
    ...
    ///
    /// N13: SHA-256 content hashes are computed eagerly here so that a spec or
    /// config edited between checkpoint save and resume is detected even when
    /// the file path is unchanged (path-only validation would silently replay a
    /// stale violation against the modified file). Mirrors the sequential
    /// engine's `ModelChecker::set_checkpoint_paths`
    /// (check/model_checker/setup_config.rs).
    pub fn set_checkpoint_paths(&mut self, spec: Option<String>, config: Option<String>) {
        self.checkpoint_spec_hash = spec
            .as_deref()
            .and_then(|p| crate::checkpoint::compute_file_hash(std::path::Path::new(p)));
        self.checkpoint_config_hash = config
            .as_deref()
            .and_then(|p| crate::checkpoint::compute_file_hash(std::path::Path::new(p)));
    ...
    /// Set fingerprint collision detection mode (`--collision-check`).
    ///
    /// Installs the parallel path's internal-fp collision detection: workers
    /// already call `ParallelCollisionDiagnostics::record_state` on every
    /// admitted state, so an active mode wires those calls to actually store +
    /// verify fingerprints. The SOUNDNESS-relevant event is two distinct states
    /// (distinct TLC canonical fingerprints) that hash to the same internal
    /// 64-bit fingerprint — which would make a worker silently skip a reachable
    /// state. A detected collision is reported as an UNSOUND-result warning.
    ///
    /// If an env-gated debug config already installed diagnostics, that config
    /// wins (it is strictly more detailed). Otherwise `Full`/`Sampling` install
    /// full/sampled internal-fp tracking; `None` leaves detection off.
    pub fn set_collision_check_mode(
        &mut self,
        mode: crate::collision_detection::CollisionCheckMode,
    ) {
        self.collision_check_mode = mode;
        if mode.is_active() && self.collision_diagnostics.is_none() {
            // Mirror the fingerprint-set shard count (construction.rs: 1 << 8).
            // Purely a contention-tuning choice, not a correctness parameter.
            const DIAG_SHARDS: usize = 256;
            self.collision_diagnostics =
                super::ParallelCollisionDiagnostics::for_collision_mode(mode, DIAG_SHARDS)
    ...
    pub(crate) fn memory_breakdown(&self) -> crate::memory::MemoryBreakdown {
        ...
        // Part of Tier-1 #4: the `seen` DashMap (full ArrayState per state) is
        // populated whenever the worker stores full states OR caches states for
        // liveness (see worker::core_adapter: `store_full_states ||
        // cache_for_liveness`). Accounting previously only charged `seen` under
        // `store_full_states`, leaving the liveness-cache case blind — a
        // material under-count on liveness specs. `cache_for_liveness` is
        // mirrored at the checker level by `successors.is_some()` (the same
        // proxy used by the worker transport macros).
        let cache_for_liveness = self.successors.is_some();
        let seen_bytes = if self.store_full_states || cache_for_liveness {
        ...
        // 2026-07 OOM audit: the BFS work queue was counted as ZERO. Estimate
        // it from `work_remaining` (queued + in-flight states across the
        // injector and per-worker deques): one `(state, depth)` tuple per
        // entry plus the same 64 B/var payload heuristic the `seen` estimate
        // uses. HandleState work items are smaller than ArrayState ones, so
        // this errs conservative — the right direction for an OOM guard.
        let queue_bytes = {
            let queue_len = self.work_remaining.load(Ordering::Relaxed);
            let entry_bytes = std::mem::size_of::<(ArrayState, usize)>()
                .saturating_add(self.vars.len().saturating_mul(64));
            crate::memory::apply_fragmentation_overhead(queue_len.saturating_mul(entry_bytes))
        };

        // Part of #4080: estimate liveness cache memory.
        // 2026-07 OOM audit: the DashMap-shell estimates below only counted
        // the per-entry Vec HEADERS; the element payloads (the dominant sink
        // on liveness specs) were invisible. `total_transitions` is the cheap
        // O(1) proxy for the total element count: every recorded transition
        // appends one `Fingerprint` to a `successors` Vec, and (under
        // symmetry) up to one `(Fingerprint, ArrayState)` witness pair.
        let liveness_bytes = {
            ...
            let succ_bytes = self
                ...
                .map(|dm| {
                    ...
                    crate::memory::estimate_dashmap_bytes_raw(estimated_capacity, entry_size)
                        // Element payload: one Fingerprint per recorded edge.
                        .saturating_add(
                            transitions
                                .saturating_mul(std::mem::size_of::<crate::state::Fingerprint>()),
                        )
                })
                .unwrap_or(0);
            ...
            let witness_bytes = self
                ...
                .map(|dm| {
                    ...
                    let entry_size = std::mem::size_of::<crate::state::Fingerprint>()
                        ...
                        >());
                    // Element payload upper bound: one (fp, ArrayState) pair
                    // per recorded transition, with the 64 B/var heuristic
                    // for the retained concrete state.
                    let pair_bytes =
                        std::mem::size_of::<(crate::state::Fingerprint, crate::state::ArrayState)>(
                        )
                        .saturating_add(self.vars.len().saturating_mul(64));
                    crate::memory::estimate_dashmap_bytes_raw(estimated_capacity, entry_size)
                        .saturating_add(transitions.saturating_mul(pair_bytes))
                })
                .unwrap_or(0);
        ...
        // Known remaining under-counts (deliberate — the OS-level RSS check in
        // `MemoryPolicy::check` is the byte-honest backstop; this breakdown is
        // for diagnostics and the opt-in internal cap):
        // - the 64 B/var ArrayState heuristic under-counts states holding
        //   large collection Values and over-counts small ones;
        // - per-worker EvalCtx arenas, interner banks, and crossbeam deque
        //   segment overhead are not attributed;
        // - `liveness_init_states` counts entry shells only (bounded by the
        //   initial-state count, not the state space).
        crate::memory::MemoryBreakdown::new(
            fp_bytes,
            seen_bytes,
            depths_bytes,
            queue_bytes,
    ...
    /// Part of #3: Set a wall-clock deadline for parallel BFS exploration.
    ///
    /// When the deadline elapses, each worker (polling every
    /// `DEADLINE_CHECK_INTERVAL` states) sets the shared stop flag and exits
    /// cleanly. The aggregated result is a partial `LimitReached { Time }` — no
    /// invariant is claimed proven, so the run fails closed.
    pub fn set_deadline(&mut self, deadline: std::time::Instant) {
        self.deadline = Some(deadline);
    }

    /// Part of #3: Set a wall-clock time budget (relative to now) for parallel BFS.
    ///
    /// Convenience wrapper over [`set_deadline`](Self::set_deadline). A
    /// saturating-overflow budget is treated as "no backstop".
    ...
    pub fn check(&self) -> CheckResult {
        ...
        // Reassert the boundary at execution time in case this checker was
        // constructed before an intervening run on the coordinator thread.
    ...
    pub(crate) fn run_check_inner(&self) -> CheckResult {
        ...
        // them on the coordinator thread (initial-state seeding and finalize
        // replay evaluate guards here). Worker threads install the same shared
        // handle at spawn. Legacy globals are NOT reset: concurrent runs in
        // the same process own their counts.
        self.run_diagnostics.reset();
        let _diag_scope =
            crate::run_diagnostics::RunDiagnosticsScope::enter(Arc::clone(&self.run_diagnostics));
        struct GuardErrorStatsFlush(Arc<crate::run_diagnostics::RunDiagnostics>);
        ...
        let _guard_error_stats_flush = GuardErrorStatsFlush(Arc::clone(&self.run_diagnostics));
        ...
        {
            let ctx = match self.prepare_base_ctx() {
                Ok(ctx) => ctx,
                Err(result) => return result,
            };
            if let Some(error) = crate::checker_ops::check_assumes(&ctx, &self.assumes) {
                return check_error_to_result(error, &CheckStats::default());
            }
            return CheckResult::Success(CheckStats::default());
        }
        ...
        if crate::check::debug::jit_enabled() {
            ...
            // removed alongside retired backend cleanup (Epic #4251);
