...
fn check_successor_invariants_hybrid_bytecode(
        ...
        match vm.execute_function(func_idx) {
            ...
            Ok(value) => {
                ...
                return InvariantOutcome::Error(
                    crate::checker_ops::invariant_non_boolean_type_error(&value),
                );
            }
            Err(_) => {
    ...
    // Lever 4 (#EWD998PCal): BytecodeVm now has a Drop impl (register-buffer
    // recycling), which extends its `ctx` borrow to scope end — release it
    // before the interpreter fallback re-borrows `ctx` mutably.
    drop(vm);

    if !unchecked.is_empty() {
        match crate::checker_ops::check_invariants_array_state_type_error(ctx, &unchecked, succ_arr)
        {
            Ok(Some(invariant)) => {
                return InvariantOutcome::Violation {
                    invariant,
...
fn check_successor_invariants_tree_walk(
    ...
    match crate::checker_ops::check_invariants_array_state_type_error(ctx, invariants, succ_arr) {
        Ok(Some(invariant)) => InvariantOutcome::Violation {
            invariant,
            state_fp: succ_fp,
        },
...
mod tests {
    ...
    #[test]
    fn test_parallel_successor_invariants_fail_closed_uses_eval_fallback() {
...
"#,
        ...
        assert_eq!(inv_ctx.jit_hits.get(), 0);
        assert_eq!(inv_ctx.jit_misses.get(), 1);
        assert_eq!(inv_ctx.jit_not_compiled.get(), 1);
    }

    #[cfg_attr(test, ntest::timeout(10000))]
    #[test]
    fn test_parallel_successor_invariants_fail_closed_falls_back_with_unreferenced_compound_var() {
        // Part of #4400 / #4267: the deleted legacy backend must fail
        // closed and let the interpreter evaluate the invariant, even when the
        // state contains compound vars that the former selective-JIT path would
        // not serialize.
        let module = parse_module(
            r#"
...
"#,
    ...
    #[cfg_attr(test, ntest::timeout(10000))]
    #[test]
    fn test_parallel_successor_invariants_fail_closed_checks_all_invariants_by_tree_walk() {
        let module = parse_module(
            r#"
...
"#,
        ...
        // Build a fail-closed compatibility cache with ONLY InvJit. The fail-closed shim still
        // reports no compiled functions, so both invariants must be checked by
        // the interpreter.
        ...
        assert_eq!(inv_ctx.jit_hits.get(), 0);
        assert_eq!(inv_ctx.jit_misses.get(), 1);
        assert_eq!(inv_ctx.jit_not_compiled.get(), 2);
    }

    #[cfg_attr(test, ntest::timeout(10000))]
    #[test]
    fn test_parallel_jit_verify_does_not_cross_check_fail_closed_cache() {
        let module = parse_module(
            r#"
...
"#,
        ...
        assert_eq!(inv_ctx.jit_verify_checked.get(), 0);
        assert_eq!(inv_ctx.jit_verify_mismatches.get(), 0);
        assert_eq!(inv_ctx.jit_hits.get(), 0);
        assert_eq!(inv_ctx.jit_misses.get(), 1);
        assert_eq!(inv_ctx.jit_not_compiled.get(), 1);
    }
