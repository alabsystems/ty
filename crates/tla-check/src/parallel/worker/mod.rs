...
/// State-count cap between per-worker inline RSS polls.
///
/// Used by [`RssCheckGate`]. A state-only cadence is cheap on the hot path but
/// is *not* a bound on memory: a spec whose states hold large set/sequence
/// values can allocate an unbounded amount of RSS within a single 10,000-state
/// window. The wall-clock backstop below closes that gap.
pub(super) const RSS_CHECK_STATE_INTERVAL: u32 = 10_000;

/// Wall-clock backstop between per-worker inline RSS polls.
///
/// Part of resource-audit finding #10: previously each worker only polled RSS
/// once per [`RSS_CHECK_STATE_INTERVAL`] states, with no time bound. For a
/// memory-heavy-but-slow workload (few states, large allocation per state) the
/// process could blow past the configured budget and be OOM-killed by the OS
/// before any worker reached its next state-count poll. Bounding the interval
/// in wall-clock time as well makes the memory budget fail-closed: the RSS
/// check fires after at most this long regardless of how few states ran.
pub(super) const RSS_CHECK_TIME_INTERVAL: std::time::Duration =
    std::time::Duration::from_millis(250);

/// Dual-bounded gate for per-worker inline RSS checks.
///
/// Fires whenever *either* [`RSS_CHECK_STATE_INTERVAL`] states have elapsed
/// since the last check *or* [`RSS_CHECK_TIME_INTERVAL`] has passed in
/// wall-clock time. Encapsulating both bounds in one type keeps the two
/// transport `report_progress` implementations (`ParallelTransport` and
/// `SharedQueueTransport`) in lockstep and lets the gating logic be unit
/// tested independently of a running BFS.
#[derive(Debug)]
pub(super) struct RssCheckGate {
    states_since_check: u32,
    last_check_at: std::time::Instant,
}

impl RssCheckGate {
    /// Create a gate primed to its initial state (zero states, clock started now).
    pub(super) fn new() -> Self {
        Self {
            states_since_check: 0,
            last_check_at: std::time::Instant::now(),
        }
    }

    /// Record that one more state was processed and report whether an RSS poll
    /// is now due. When this returns `true` the gate resets both bounds, so the
    /// caller must perform the (cheap) RSS syscall.
    ///
    /// Uses `now` rather than reading the clock internally so the hot path can
    /// share a single `Instant::now()` with other periodic work and so the
    /// logic is deterministically testable.
    pub(super) fn tick(&mut self, now: std::time::Instant) -> bool {
        self.states_since_check = self.states_since_check.saturating_add(1);
        let due = self.states_since_check >= RSS_CHECK_STATE_INTERVAL
            || now.saturating_duration_since(self.last_check_at) >= RSS_CHECK_TIME_INTERVAL;
        if due {
            self.states_since_check = 0;
...
#[cfg(test)]
mod rss_check_gate_tests {
    use super::{RssCheckGate, RSS_CHECK_STATE_INTERVAL, RSS_CHECK_TIME_INTERVAL};
    use std::time::{Duration, Instant};

    /// The state-count bound fires on exactly the Nth tick when the clock is
    /// held fixed (so the wall-clock bound cannot trigger first), and the gate
    /// resets afterwards so the next window again takes N ticks. This is the
    /// behavior the pre-fix code relied on, preserved unchanged.
    #[test]
    fn fires_on_state_count_bound_with_fixed_clock() {
        let mut gate = RssCheckGate::new();
        let now = Instant::now();
        for i in 1..RSS_CHECK_STATE_INTERVAL {
            assert!(
                !gate.tick(now),
                "tick {i} should not fire before the state-count cap with a fixed clock"
            );
        }
        assert!(
            gate.tick(now),
            "tick {RSS_CHECK_STATE_INTERVAL} should fire on the state-count cap"
        );
        // Window resets: the very next tick must not immediately re-fire.
        assert!(!gate.tick(now), "gate must reset after firing");
    }

    /// Regression for resource-audit finding #10: a memory-heavy-but-slow
    /// workload processes far fewer than `RSS_CHECK_STATE_INTERVAL` states
    /// between RSS polls, yet the wall-clock backstop must still force a poll
    /// once `RSS_CHECK_TIME_INTERVAL` has elapsed. Before the fix this could
    /// never happen — RSS was unobserved until the state-count cap, letting the
    /// process OOM between polls.
    #[test]
    fn wall_clock_backstop_fires_before_state_count_cap() {
        let mut gate = RssCheckGate::new();
        let start = Instant::now();

        // Only a handful of states processed — nowhere near the state cap.
        let due_just_before = gate.tick(
            start
                + RSS_CHECK_TIME_INTERVAL
                    .checked_sub(Duration::from_millis(1))
                    .unwrap(),
        );
        assert!(
            !due_just_before,
            "must not fire before the wall-clock interval elapses (few states processed)"
        );

        // Crossing the wall-clock interval forces a poll despite the tiny
        // state count — this is the fail-closed guarantee.
        let due_at_interval = gate.tick(start + RSS_CHECK_TIME_INTERVAL);
        assert!(
            due_at_interval,
            "wall-clock backstop must force an RSS poll once the time interval elapses, \
             even though far fewer than {RSS_CHECK_STATE_INTERVAL} states were processed"
        );
    }

    /// After a wall-clock-triggered poll, the clock anchor resets so the next
    /// poll is again gated by a full `RSS_CHECK_TIME_INTERVAL`, not fired every
    /// subsequent tick.
    #[test]
    fn wall_clock_anchor_resets_after_firing() {
        let mut gate = RssCheckGate::new();
        let start = Instant::now();
        assert!(
            gate.tick(start + RSS_CHECK_TIME_INTERVAL),
            "first poll fires"
        );
        // Immediately after, with only a tiny additional delta, must not fire.
        assert!(
            !gate.tick(start + RSS_CHECK_TIME_INTERVAL + Duration::from_millis(1)),
            "anchor must reset; a sub-interval delta after firing must not re-fire"
        );
    }
}
...
pub(super) struct InvariantCheckCtx<'a> {
    ...
    pub(super) eval_implied_actions: &'a [crate::checker_ops::EvalImpliedActionTerm],
...
pub(super) struct WorkerModelConfig {
    ...
    /// Part of #16 (OOM hardening): shared across workers via `Arc` instead of
    /// a per-worker deep clone of the whole operator-definition map.
    pub(super) op_defs: Arc<FxHashMap<String, OperatorDef>>,
    ...
    /// Part of #3: Wall-clock deadline for BFS exploration (None = no backstop).
    /// Plumbed into each worker's `BfsWorkerConfig`; polled in the unified loop.
    ...
    pub(super) eval_implied_actions: Arc<Vec<crate::checker_ops::EvalImpliedActionTerm>>,
