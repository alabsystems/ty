...
impl<T: BfsWorkItem> BfsTransport for SharedQueueTransport<T> {
    ...
    fn report_progress(&mut self, _depth: usize, _queue_len: usize) -> Result<(), BfsTermination> {
        ...
        // maintenance barrier with a lightweight check. Part of resource-audit
        // finding #10: the gate fires on whichever comes first —
        // RSS_CHECK_STATE_INTERVAL states OR RSS_CHECK_TIME_INTERVAL of
        // wall-clock time — so a memory-heavy-but-slow workload cannot blow
        // past the budget between two purely state-count-based polls.
    ...
    fn on_deadline_exceeded(&mut self, _fp: Fingerprint, current: ArrayState) -> BfsTermination {
        // Part of #3 + N8 (time backstop): set the shared stop flag so every
        // sibling worker winds down, then SEND a partial `LimitReached { Time }`
        // result before exiting.
        //
        // N8 fix: previously this exited silently, so the coordinator saw no
        // terminal outcome and reported `Success` on a partially explored state
        // space — a false PASS. This now mirrors the MEMORY-limit path in
        // `report_progress` exactly, so the coordinator maps a Time
        // `LimitReached` aggregate to `CheckResult::LimitReached { Time }`,
        // never `Success`.
        self.reclaim_decode_scratch(current);
        self.stop_flag.store(true, Ordering::Relaxed);
        let stop_ctx = StopCtx {
...
impl<T: BfsWorkItem> Drop for SharedQueueTransport<T> {
    fn drop(&mut self) {
        ...
        // suspension barrier. The transport is dropped exactly once, when this
        // worker's BFS run ends without pausing — covering every non-pausing
        // termination (the `snapshot_stop_and_send` + `Exit` paths, incl. the
        // N8 deadline exit, AND the sibling workers that then bail via the
        // stop_flag Done path instead of pausing). Without this,
        // `suspend_all()` hangs waiting for workers that will never pause.
