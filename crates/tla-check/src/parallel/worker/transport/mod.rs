...
fn force_batch_via_env() -> bool {
    #[cfg(test)]
    {
        std::env::var("TY_FORCE_BATCH").is_ok_and(|v| v == "1")
    }
    #[cfg(not(test))]
    {
        static FORCE_BATCH: OnceLock<bool> = OnceLock::new();
        *FORCE_BATCH.get_or_init(|| std::env::var("TY_FORCE_BATCH").is_ok_and(|v| v == "1"))
    }
}
...
pub(in crate::parallel) struct ParallelTransport<T: BfsWorkItem> {
    ...
    op_defs: Arc<FxHashMap<String, OperatorDef>>,
    ...
    depth_limited: bool,
    /// Part of #3397: Cached `TY_FORCE_BATCH=1` env var. Read once at
    ...
    rss_check_gate: super::RssCheckGate,
...
impl<T: BfsWorkItem> ParallelTransport<T> {
    ...
    pub(in crate::parallel) fn new(
        ...
        Self {
            ...
            critical_rss_bytes,
            rss_check_gate: super::RssCheckGate::new(),
            bytecode,
            jit_cache,
