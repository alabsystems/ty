...
fn test_constrained_streaming_disabled_with_implied_actions() {
    ...
    eval_transport.eval_implied_actions =
        Arc::new(vec![crate::checker_ops::EvalImpliedActionTerm {
            name: "Prop".to_string(),
...
fn test_constrained_streaming_disabled_with_force_batch_env() {
    ...
    assert!(
        !transport.constrained_streaming_eligible(),
        "TY_FORCE_BATCH must disable constrained streaming for A/B benchmarking"
    );
}
