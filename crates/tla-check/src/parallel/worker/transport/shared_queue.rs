...
//!
//! Env-gated pilot (`TY_SHARED_QUEUE=1`) that replaces per-worker deques
...
pub(in crate::parallel) struct SharedQueueTransport<T: BfsWorkItem> {
    ...
    // Part of #16: shared via Arc (cloned from WorkerModelConfig.op_defs).
    ...
    /// Dual-bounded (state-count + wall-clock) gate for inline RSS checks.
    /// Part of resource-audit finding #10: the state-only cadence is not a
    /// memory bound, so a wall-clock backstop keeps the budget fail-closed.
    pub(super) rss_check_gate: super::super::RssCheckGate,
...
impl<T: BfsWorkItem> SharedQueueTransport<T> {
    pub(in crate::parallel) fn new(
        ...
        let WorkerModelConfig {
            ...
            // Part of #3: deadline is consumed earlier (run_unified.rs) when
            // building BfsWorkerConfig; not needed inside the transport itself.
            deadline: _,
        } = model;
        let decode_scratch =
        ...
        Self {
            ...
            critical_rss_bytes,
            rss_check_gate: super::super::RssCheckGate::new(),
            bytecode,
            jit_cache,
    ...
    pub(super) fn process_successors_inner(
        ...
        // removal rationale. The retired compatibility path has been deleted and
        // `is_promoted_to_jit()` always returns false, so
