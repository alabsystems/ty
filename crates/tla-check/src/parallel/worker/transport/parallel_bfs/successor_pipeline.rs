...
impl<T: BfsWorkItem> ParallelTransport<T> {
    pub(super) fn process_successors_inner(
        ...
        // compiled legacy JitNextStateCache when the monolithic
        // action was promoted to Tier 1, but always fell through to the
        // interpreter. With the retired compatibility path deleted, `is_promoted_to_jit()`
        // is always false and the path
