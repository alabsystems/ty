...
impl<'a, 'inv> WorkerBfsCtx<'a, 'inv> {
        ...
        F: Fn(ArrayState, usize),
    {
        ...
        for (action_idx, action) in actions.iter().enumerate() {
            let t_enum = timing_start();
            // SOUNDNESS: pass the run-stable `&action.expr` (Arc-shared
            // `por_actions`) — never a per-state clone — to keep the unified
            // enumerator's pointer-keyed caches valid (see
            // `CoverageState::actions`).
            let diffs = match crate::enumerate::enumerate_successors_array_as_diffs_body(
                self.ctx,
                &action.expr,
                current_array,
                vars,
        ...
        let selected_diffs = if per_action_diffs.len() > 1 {
            ...
            // C3 cycle proviso (standard BFS fresh-successor form): the
            // reduced ample set may be used ONLY if it yields at least one
            // fresh successor; otherwise process the FULL enabled successor
            // set (all per-action diffs are already enumerated — full
            // expansion is just skipping the filter). Fails closed whenever
            // freshness cannot be evaluated in the exact dedup fingerprint
            // domain (VIEW / symmetry). See `compute_ample_set`'s C3 doc.
            let ample_set: Option<rustc_hash::FxHashSet<usize>> = if ample_result.reduced {
                ...
                self.reduced_expansion_has_fresh_diff_successor(
                    current_array,
                    fp,
                    cached_view_name,
                    &mut per_action_diffs,
                    &candidate,
                )
                .then_some(candidate)
            } else {
            ...
            if let Some(ample_set) = ample_set {
                // HONEST accounting: only a reduction that survived the
                // proviso counts as one.
                self.stats.por_reductions += 1;
                self.stats.por_actions_skipped += (enabled_indices.len() - ample_set.len()) as u64;
                per_action_diffs
                    .into_iter()
    ...
    /// C3 cycle proviso — standard BFS fresh-successor form (parallel worker).
    ///
    /// Returns whether the REDUCED (ample ⊊ enabled) expansion of the current
    /// state yields at least one FRESH successor: a state that is not the
    /// expanding state (`parent_fp`) and was not in the global visited set at
    /// the moment of this expansion. If not, the caller must process the FULL
    /// enabled successor set instead of the ample subset (see
    /// [`crate::por::compute_ample_set`], C3).
    ///
    /// SOUNDNESS UNDER CONCURRENCY: `stop.seen_fps` is the linearizable
    /// global admission set, and every state is inserted into it BEFORE it is
    /// enqueued (admit-then-enqueue in `run_core_step`), hence before any
    /// worker can dequeue and expand it. For any cycle in the reduced state
    /// graph, order the cycle states by the linearization of their
    /// expansion-time freshness probes: the last one probes after its
    /// in-cycle ample successor was inserted (that successor's own probe
    /// linearized earlier, and its insertion happens-before its expansion via
    /// the queue transfer), so the probe observes it as Present. A racing
    /// insert can only flip a probe from Absent to Present — i.e. make this
    /// check MORE conservative (more full expansions), never less.
    ///
    /// Freshness is evaluated in the exact dedup fingerprint domain of
    /// `process_one_full_state_diff`: lazy diff values are materialized first
    /// (idempotent — the pipeline's later materialize pass then finds nothing
    /// lazy), then `compute_diff_fingerprint_with_xor` reproduces the
    /// pipeline's `succ_fp_from_diff` byte-exactly.
    ///
    /// FAIL-CLOSED cases (return `false` ⇒ full expansion — the reduced set
    /// is simply not used; never an unsound approximation):
    /// - VIEW configured: the dedup fp is an evaluation of the VIEW
    ///   expression on the materialized successor at `succ_level`;
    /// - SYMMETRY configured: the dedup fp is the canonical fp over all
    ///   permutations of the materialized full state. Reproducing either
    ///   here would duplicate the whole downstream pipeline per successor,
    ///   and approximating with the raw diff fp could call a visited state
    ///   "fresh" (unsound) — so neither is attempted;
    /// - a materialization error (the pipeline re-hits the same successor
    ///   and surfaces it authoritatively) or a storage fault on the probe.
    ///
    /// Short-circuits on the first fresh successor.
    fn reduced_expansion_has_fresh_diff_successor(
        &mut self,
        current_array: &ArrayState,
        parent_fp: Fingerprint,
        cached_view_name: &Option<String>,
        per_action_diffs: &mut [(usize, Vec<DiffSuccessor>)],
        ample_set: &rustc_hash::FxHashSet<usize>,
    ) -> bool {
        if cached_view_name.is_some() || !self.mvperms.is_empty() {
            return false; // fail closed — dedup fp not cheaply reproducible here
        }
        let registry = self.inv_ctx.var_registry;
        for (idx, diffs) in per_action_diffs.iter_mut() {
            if !ample_set.contains(idx) {
                continue;
            }
            for diff in diffs.iter_mut() {
                // Match the downstream pipeline exactly: lazy values are
                // resolved BEFORE the dedup fingerprint is computed.
                if crate::materialize::materialize_diff_changes(
                    self.ctx,
                    &mut diff.changes,
                    self.spec_may_produce_lazy,
                )
                ...
                let (succ_fp, _combined_xor) =
                    compute_diff_fingerprint_with_xor(current_array, &diff.changes, registry);
                if succ_fp != parent_fp
                    && matches!(
                        self.stop.seen_fps.contains_checked(succ_fp),
