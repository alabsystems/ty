        ...
        match crate::checker_ops::check_eval_implied_actions_for_transition(
            ...
            crate::checker_ops::InvariantOutcome::Violation {
                ...
                if self.inv_ctx.continue_on_error {
                    ...
                    // This check runs PRE-ADMIT: the violating successor has not
                    // been inserted into `seen` or linked in the parent log yet
                    // (that happens in `ParallelAdapter::admit`). Record both
                    // here — mirroring what admission would have done — so
                    // `reconstruct_trace` can walk succ -> parent -> ... -> init
                    // and emit the full counterexample instead of an EMPTY trace
                    // (the parallel action-property empty-trace bug: sequential
                    // runs produced the 2-state trace, `--workers N` produced
                    // none).
                    if self.stop.store_full_states || self.stop.cache_for_liveness {
                        self.stop.seen.insert(succ_fp, succ_arr.clone());
                        self.inv_ctx
                            .parent_log
                            .append(self.inv_ctx.worker_id, succ_fp, parent_fp);
                    }
                    snapshot_stop_and_send(ctx, self.stats, self.stop, self.result_tx, |stats| {
    ...
    F: Fn(ArrayState, usize),
{
    ...
    for mut diff in diffs {
        ...
        if has_eval_implied_actions
            && succ_fp != parent_fp
            && spc
                .check_eval_implied_actions_pre_admit(ctx, base_array, parent_fp, &scratch, succ_fp)
        {
            return true;
        }
