...
pub(super) fn init_worker_ctx(
    ...
    // Audit finding #12: resolve the per-state successor cap from the shared
    // model Config onto this worker's shared context. Workers build a fresh
    // EvalCtx, so (like the precompute passes above) this must run per-worker —
    // it carries the per-checker cap into every worker thread without any
    // process-global state.
    crate::enumerate::apply_per_state_successor_cap(&mut ctx, &model.config);
