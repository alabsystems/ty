    ...
    F: FnMut(&crate::state::ArrayState, usize),
{
    ...
    fn admit(&mut self, input: &CoreStepInput<'_>) -> Result<bool, Self::Stop> {
        ...
        // Part of #4211: Use lock-free FingerprintSet (seen_fps) as the primary
        // source of truth for all admission, reducing DashMap contention.
        // DashMap (seen) is only updated for genuinely new states.
        let is_new = match self.stop.seen_fps.insert_checked(input.succ_fp) {
            InsertOutcome::Inserted => {
                // Only store full state if requested (full trace or liveness cache).
                // Part of #3233/#3801: DashMap insertion only happens for new states.
            ...
            InsertOutcome::StorageFault(fault) => {
                ...
                let error =
                    crate::checker_ops::storage_fault_to_check_error(self.stop.seen_fps, &fault);
                snapshot_stop_and_send(self.ctx, self.stats, self.stop, self.result_tx, |stats| {
...
// in successors/. Parallel integration tests must match the reference state
// counts for the covered corpus; that is the behavioral equivalence gate.
