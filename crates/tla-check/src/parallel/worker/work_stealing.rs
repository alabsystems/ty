...
mod tests {
    ...
    fn test_load_balance_stats_fields_independent() {
        ...
        let stats = WorkerStats {
            states_explored: 300,
            states_generated: 500,
            max_local_queue_depth: 100,
            idle_ns: 5_000_000,
            steal_latency_ns: 1_000_000,
            steals: 10,
            ..Default::default()
        };
    ...
    fn test_adaptive_steal_limit_controls_batch_size() {
        ...
        assert!(
            local_count < STEAL_LIMIT_CONSERVATIVE,
        ...
        if let Some((val, _)) = stolen2 {
            // Successfully stole remaining items with aggressive limit. `val` is a
            // u32, so this intentionally accepts every value; collapsing it to
            // `val >= 0` would itself trip absurd_extreme_comparisons, so keep the
            // explicit two-way form and silence double_comparisons here.
