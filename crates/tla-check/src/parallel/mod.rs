...
// Part of #4398: keep fail-closed compiled-backend types behind tla-check's backend shim.
...
pub struct ParallelChecker {
    ...
    /// Plumbed into each worker's `BfsWorkerConfig`; when it elapses the worker
    /// sets the shared stop flag and exits cleanly with a partial result.
    ...
    /// User-facing collision-check mode (`--collision-check`). Stored so the run
    /// report reflects the actual mode; `set_collision_check_mode` also installs
    /// `collision_diagnostics` from it when the mode is active.
    ...
    /// telemetry). Shared with every worker thread via `Arc`; installed in a
    /// thread-local scope at worker start so deep recording sites attribute
    /// counts to THIS run instead of a process-global counter that concurrent
    /// runs (cargo test) reset/steal.
    run_diagnostics: Arc<crate::run_diagnostics::RunDiagnostics>,
    ...
    /// N13: SHA-256 content hash of the spec file, computed eagerly at
    /// `set_checkpoint_paths` time. Persisted into checkpoint metadata and
    /// re-checked on resume so a spec edited between save and resume (even at
    /// the same path) is rejected instead of replaying a stale verdict.
    /// Mirrors the sequential engine's `ModelChecker::checkpoint.spec_hash`.
    checkpoint_spec_hash: Option<String>,
    /// N13: SHA-256 content hash of the config file (see `checkpoint_spec_hash`).
    checkpoint_config_hash: Option<String>,
    ...
    /// Declared-SYMMETRY wrong-verdict fix: memoized decision of whether
    /// declared SYMMETRY must be ignored for this run because a PROPERTY
    /// requires the genuine liveness checker (the plain orbit quotient is
    /// unsound for liveness; TY does not implement the Emerson-Sistla
    /// annotated quotient). Settled once in `prepare_check()` (before any
    /// worker fingerprints a state) and consulted by every symmetry consumer
    /// — BFS mvperms, on-the-fly liveness fingerprinting, the post-BFS
    /// liveness graph, and fp-only replay — so the whole run agrees.
    /// See `symmetry_disabled_for_liveness()` in checker/prepare.rs.
    symmetry_disabled_for_liveness: OnceLock<bool>,
}
