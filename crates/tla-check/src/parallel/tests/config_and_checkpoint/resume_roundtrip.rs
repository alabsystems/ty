...
"#;
    ...
    let tmpdir = std::env::temp_dir().join("ty_test_parallel_ckpt_roundtrip");
...
"#;
    ...
    let tmpdir = std::env::temp_dir().join("ty_test_parallel_ckpt_mismatch");
    let _ = std::fs::remove_dir_all(&tmpdir);
    std::fs::create_dir_all(&tmpdir).unwrap();
...
/// N13 (soundness): resuming a parallel checkpoint must REJECT a spec whose
/// on-disk content changed since the checkpoint was saved — even when the spec
/// *path* is unchanged. Without the content-hash gate, `--resume` silently
/// replays a verdict captured against a spec the file no longer contains.
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_parallel_resume_rejects_modified_spec_content() {
    let _serial = super::super::acquire_interner_lock();

    let tmpdir = std::env::temp_dir().join("ty_test_parallel_ckpt_content_hash");
    let _ = std::fs::remove_dir_all(&tmpdir);
    std::fs::create_dir_all(&tmpdir).unwrap();
    let spec_path = tmpdir.join("Counter.tla");

    // Write the ORIGINAL spec to disk; its content hash is captured at save.
    let original_src = r#"
---- MODULE Counter ----
VARIABLE x
Init == x = 0
...
====
"#;
    std::fs::write(&spec_path, original_src).unwrap();
    let spec_path_str = spec_path.to_string_lossy().to_string();

    let module = parse_module(original_src);
    let config = Config {
        init: Some("Init".to_string()),
        next: Some("Next".to_string()),
        invariants: vec!["TypeOK".to_string()],
    ...
    checker.set_deadlock_check(false);
    checker.set_checkpoint(tmpdir.join("ckpt"), Duration::from_secs(3600));
    checker.set_checkpoint_paths(Some(spec_path_str.clone()), None);

    let result = checker.check();
    assert!(
        matches!(result, CheckResult::Success(_)),
        "baseline run should succeed, got: {result:?}"
    );

    let checkpoint = checker.create_checkpoint().unwrap();
    // The saved checkpoint must carry the original spec content hash.
    assert!(
        checkpoint.metadata.spec_hash.is_some(),
        "checkpoint must record the spec content hash for resume validation"
    );
    let ckpt_dir = tmpdir.join("ckpt_saved");
    checkpoint.save(&ckpt_dir).unwrap();

    // EDIT the spec file on disk: SAME path, DIFFERENT bytes (x < 5 -> x < 99).
    let modified_src = r#"
---- MODULE Counter ----
VARIABLE x
Init == x = 0
...
====
"#;
    std::fs::write(&spec_path, modified_src).unwrap();

    // Resume against the modified file — must be rejected on the hash mismatch,
    // NOT replay a stale verdict.
    let mut checker2 = ParallelChecker::new(&module, &config, 2);
    checker2.set_deadlock_check(false);
    checker2.set_checkpoint_paths(Some(spec_path_str), None);

    let resume_err = checker2.check_with_resume(&ckpt_dir);
    assert!(
        resume_err.is_err(),
        "resume against a modified spec must be rejected, got: {:?}",
        resume_err.ok()
    );
    let msg = resume_err.unwrap_err().to_string();
    assert!(
        msg.contains("content hash mismatch"),
        "error should cite the content hash mismatch, got: {msg}"
    );

    let _ = std::fs::remove_dir_all(&tmpdir);
}

/// N13 companion: an UNMODIFIED spec at the same path still resumes cleanly
/// (the content-hash gate must not reject a legitimate resume).
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_parallel_resume_accepts_unmodified_spec_content() {
    let _serial = super::super::acquire_interner_lock();

    let tmpdir = std::env::temp_dir().join("ty_test_parallel_ckpt_content_hash_ok");
    let _ = std::fs::remove_dir_all(&tmpdir);
    std::fs::create_dir_all(&tmpdir).unwrap();
    let spec_path = tmpdir.join("Counter.tla");
...
"#;
    ...
    checker.set_checkpoint(tmpdir.join("ckpt"), Duration::from_secs(3600));
    checker.set_checkpoint_paths(Some(spec_path_str.clone()), None);
    assert!(matches!(checker.check(), CheckResult::Success(_)));

    let checkpoint = checker.create_checkpoint().unwrap();
    let ckpt_dir = tmpdir.join("ckpt_saved");
    checkpoint.save(&ckpt_dir).unwrap();

    // Resume against the UNCHANGED file — matching hash must be accepted.
    let mut checker2 = ParallelChecker::new(&module, &config, 2);
    checker2.set_deadlock_check(false);
    checker2.set_store_states(false);
    checker2.set_checkpoint_paths(Some(spec_path_str), None);
    let resume_result = checker2
        .check_with_resume(&ckpt_dir)
        .expect("unmodified-spec resume should be accepted");
    assert!(
        matches!(resume_result, CheckResult::Success(_)),
        "resume of an unmodified spec should succeed, got: {resume_result:?}"
    );
...
"#;
    ...
    let tmpdir = std::env::temp_dir().join("ty_test_parallel_ckpt_violation");
    let _ = std::fs::remove_dir_all(&tmpdir);
    std::fs::create_dir_all(&tmpdir).unwrap();
...
"#;
    ...
    let tmpdir = std::env::temp_dir().join("ty_test_parallel_ckpt_detected_action_ids");
    let _ = std::fs::remove_dir_all(&tmpdir);
    std::fs::create_dir_all(&tmpdir).unwrap();
