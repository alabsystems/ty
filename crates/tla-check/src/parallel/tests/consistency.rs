...
impl ForceBatchEnvGuard {
    fn set(value: Option<&str>) -> Self {
        ...
        let previous = std::env::var_os("TY_FORCE_BATCH");
        match value {
            Some(value) => crate::env_guard::set_var("TY_FORCE_BATCH", value),
            None => crate::env_guard::remove_var("TY_FORCE_BATCH"),
        }
        Self {
            previous,
            lock: Some(lock),
...
fn test_continue_on_error_states_found_parity_with_sequential() {
    let _serial = super::acquire_interner_lock();
    test_continue_on_error_states_found_parity_with_sequential_body();
}

#[cfg_attr(test, ntest::timeout(30_000))]
fn test_continue_on_error_states_found_parity_with_sequential_body() {
