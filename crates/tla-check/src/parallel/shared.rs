...
// Set TY_DISABLE_HANDLE_STATE=1 to opt out for comparison. Integration tests
...
pub(crate) fn use_handle_state() -> bool {
    ...
    !crate::debug_env::env_flag_is_set(&FLAG, "TY_DISABLE_HANDLE_STATE")
}

/// Temporarily override [`use_handle_state`] for integration tests.
///
/// Production behavior remains env-driven (`TY_DISABLE_HANDLE_STATE`), but
...
// When enabled (TY_SHARED_QUEUE=1), workers share a single FIFO frontier
// instead of per-worker deques with work stealing. This matches TLC's
// queue topology for direct throughput comparison.
feature_flag!(pub(crate) use_shared_queue, "TY_SHARED_QUEUE");
...
// Set TY_NO_PRESERVE_VALUE_FPS=1 to disable.
pub(crate) fn parallel_preserve_value_fps() -> bool {
    static FLAG: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *FLAG.get_or_init(|| std::env::var("TY_NO_PRESERVE_VALUE_FPS").is_err())
}

/// Re-export canonical checker_ops functions for parallel-internal use.
///
/// Part of #2565: constraint evaluation moved to `checker_ops.rs` as the
/// canonical implementation shared by both sequential and parallel checker paths.
/// This re-export preserves the `pub(crate)` import path that
/// `parallel/checker.rs` uses.
pub(crate) use crate::checker_ops::check_state_constraints_array;
...
feature_flag!(pub(crate) timing_enabled, "TY_TIMING");
#[inline(always)]
...
pub(crate) const PARALLEL_PROFILING_ENV: &str = "TY_PARALLEL_PROFILING";
