...
/// Reads a trace from `reader` in the given [`TraceInputFormat`], emitting the
/// header and steps to `sink`.
///
/// # Errors
/// Returns a [`TraceParseError`] on I/O failure, malformed JSON/JSONL, or trace
/// validation failures (missing header, index mismatch, unknown variable, etc.).
...
/// Reads a trace in JSON object form (`{ version, module, variables, steps }`),
/// emitting the header and steps to `sink`. The full `steps` array is buffered.
///
/// # Errors
/// Returns a [`TraceParseError`] on I/O failure, JSON decode errors (with path/
/// position context), an empty `steps` array, or per-step validation failures.
...
/// Reads a trace in JSON Lines form (one `header` event followed by `step`
/// events, one per line), streaming the header and steps to `sink`.
///
/// # Errors
/// Returns a [`TraceParseError`] on I/O failure, a line that fails to decode, an
/// unknown event type, a missing/duplicate header, or per-step validation failures.
