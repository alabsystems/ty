...
/// Global suppression of always-on telemetry (the `[fused]`, `[trust-cg]`,
/// `[flat_state]`, `[flat-bfs]`, `[ay-kind]`, … progress/diagnostic lines printed
/// to stderr). Set by the CLI in TLC-tool output mode, whose protocol requires
/// clean tagged output only. An `AtomicBool` (not a thread-local) because the
/// fused orchestrator emits telemetry from its worker-lane threads.
static TELEMETRY_QUIET: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);

/// Suppress (or re-enable) always-on stderr telemetry process-wide.
pub fn set_telemetry_quiet(quiet: bool) {
    TELEMETRY_QUIET.store(quiet, std::sync::atomic::Ordering::Relaxed);
}

/// Whether always-on stderr telemetry is currently suppressed.
#[inline]
pub fn telemetry_quiet() -> bool {
    TELEMETRY_QUIET.load(std::sync::atomic::Ordering::Relaxed)
}

/// Like `eprintln!`, but suppressed when telemetry is quiet (TLC-tool mode).
/// Use for always-on progress/diagnostic lines — NOT for real warnings/errors.
macro_rules! telemetry_eprintln {
    ($($arg:tt)*) => {
        if !$crate::debug_env::telemetry_quiet() {
            eprintln!($($arg)*);
        }
    };
}
...
///
/// Usage:
///   `feature_flag!(pub(crate) my_flag, "TY_MY_FLAG");`
...
///
/// Usage:
///   `feature_limit!(pub(crate) my_limit, "TY_MY_LIMIT", 10);`
...
///
/// Usage:
///   `debug_flag_strict!(pub(crate) my_flag, "TY_MY_FLAG");`
