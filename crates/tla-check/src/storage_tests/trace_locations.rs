...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_mmap_trace_locations_estimate_memory_bytes() {
    let locs = MmapTraceLocations::new(1000, None).unwrap();

    assert_eq!(locs.estimate_memory_bytes(), 1000 * TRACE_ENTRY_SIZE);
}
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_in_memory_trace_locations_estimate_memory_bytes() {
    let locs = InMemoryTraceLocations::new();
    let empty_bytes = locs.estimate_memory_bytes();

    TraceLocationStorage::insert(&locs, fp(111), 500);
    let populated_bytes = locs.estimate_memory_bytes();
