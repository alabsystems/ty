...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_mmap_dyn_batch_insert_duplicate_at_load_factor_limit_does_not_fault() {
    let set = MmapFingerprintSet::with_load_factor(4, None, 0.25).unwrap();
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_mmap_dyn_raw_inserted_indices() {
    let set = MmapFingerprintSet::new(100, None).unwrap();
    let storage: &dyn FingerprintSet = &set;
    let mut admission = BatchInsertedIndexAdmission::with_capacity(8);
    admission.inserted_indices.push(99);
    admission.attempted = 99;
    admission.fault = Some(StorageFault::new("test", "scratch", "stale"));
    let scratch_ptr = admission.inserted_indices.as_ptr();

    storage.insert_batch_fingerprint_values_inserted_indices_checked_into(
        &[10, 20, 10, 30],
        &mut admission,
    );

    assert_eq!(admission.inserted_indices, vec![0, 1, 3]);
    assert_eq!(admission.attempted, 4);
    assert_eq!(admission.fault, None);
    assert_eq!(
        admission.inserted_indices.as_ptr(),
        scratch_ptr,
        "dyn mmap raw inserted-index admission should reuse caller scratch"
    );
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_mmap_dyn_raw_batch_zero_encoding() {
    let set = MmapFingerprintSet::new(10, None).unwrap();
    let storage: &dyn FingerprintSet = &set;
    let zero_encoded_with_flushed_bit = 1u64 << 63;

    let outcomes =
        storage.insert_batch_fingerprint_values_checked(&[0, zero_encoded_with_flushed_bit, 0, 1]);
    ...
    assert_eq!(storage.contains_checked(fp(0)), LookupOutcome::Present);
    assert_eq!(
        storage.contains_checked(fp(zero_encoded_with_flushed_bit)),
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_mmap_dyn_raw_duplicate_at_load_factor_limit_does_not_fault() {
    let set = MmapFingerprintSet::with_load_factor(4, None, 0.25).unwrap();
    let storage: &dyn FingerprintSet = &set;

    assert_eq!(storage.insert_checked(fp(1)), InsertOutcome::Inserted);

    let admission = storage.insert_batch_fingerprint_values_inserted_indices_checked(&[1]);

    assert_eq!(admission.inserted_indices, Vec::<usize>::new());
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_fingerprint_storage_in_memory_inserted_index_admission_duplicates() {
    let storage = FingerprintStorage::in_memory();

    let admission =
        storage.insert_batch_inserted_indices_checked(&[fp(10), fp(20), fp(10), fp(30)]);
    ...
    let admission =
        storage.insert_batch_fingerprint_values_inserted_indices_checked(&[20, 40, 40, 50]);

    assert_eq!(admission.inserted_indices, vec![1, 3]);
    assert_eq!(admission.attempted, 4);
    assert_eq!(admission.fault, None);
    assert_eq!(storage.len(), 5);
    assert_eq!(storage.contains_checked(fp(40)), LookupOutcome::Present);
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_fingerprint_storage_mmap_inserted_index_admission_stops_at_first_storage_fault() {
    let storage = FingerprintStorage::mmap(1, None).unwrap();
    let mut admission = BatchInsertedIndexAdmission::with_capacity(4);
    let scratch_ptr = admission.inserted_indices.as_ptr();

    storage
        .insert_batch_fingerprint_values_inserted_indices_checked_into(&[1, 2, 0], &mut admission);

    assert_eq!(admission.inserted_indices, vec![0]);
    assert_eq!(admission.attempted, 2);
    assert!(
        admission.fault.is_some(),
        "expected the full mmap table to fault before the suffix"
    );
    assert_eq!(
        admission.inserted_indices.as_ptr(),
        scratch_ptr,
        "mmap inserted-index admission should reuse caller scratch"
    );
    assert_eq!(storage.len(), 1);
    assert_eq!(storage.contains_checked(fp(1)), LookupOutcome::Present);
    assert_eq!(
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_trait_default_inserted_index_admission_reuses_scratch() {
    let set: Box<dyn FingerprintSet> = Box::new(dashmap::DashSet::<Fingerprint>::new());
    let mut admission = BatchInsertedIndexAdmission::with_capacity(8);
    admission.inserted_indices.push(99);
    admission.attempted = 99;
    ...
    set.insert_batch_inserted_indices_checked_into(&[fp(10), fp(20), fp(10)], &mut admission);

    assert_eq!(admission.inserted_indices, vec![0, 1]);
    assert_eq!(admission.attempted, 3);
    assert_eq!(admission.fault, None);
    assert_eq!(
        admission.inserted_indices.as_ptr(),
        scratch_ptr,
        "caller-owned inserted-index scratch should be reused when capacity is sufficient"
    );

    set.insert_batch_fingerprint_values_inserted_indices_checked_into(
        &[20, 30, 30],
        &mut admission,
    );

    assert_eq!(admission.inserted_indices, vec![1]);
    assert_eq!(admission.attempted, 3);
    assert_eq!(admission.fault, None);
    assert_eq!(
        admission.inserted_indices.as_ptr(),
        scratch_ptr,
        "subsequent raw inserted-index batches should keep using the same scratch allocation"
    );
    assert_eq!(set.len(), 3);
}
