...
/// The specific reason a trace param key could not be bound to a `\E` witness.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ActionLabelParamBindErrorKind {
    ...
    AmbiguousCallsite {
        /// The ambiguous param key.
        param: String,
        /// Spans of the candidate operator call sites.
        callsites: Vec<Span>,
    },
    ...
    UnsupportedCallsiteArg {
        ...
        position: usize,
        /// Pretty-printed form of the unsupported argument expression.
        arg: String,
...
pub struct ActionLabelParamBindError {
    /// The action label being bound.
    pub label: String,
    /// The raw operator reference string from the mapping.
    pub operator_raw: String,
    /// Name of the detected action.
    pub action_name: String,
    /// Identifier of the detected action.
    pub action_id: DetectedActionId,
    /// Source span of the detected action expression.
    pub action_span: Span,
    /// Outer `\E` binder sites available in the action.
    pub outer_binders: Vec<BoundVarSite>,
    /// The specific binding failure.
    pub kind: ActionLabelParamBindErrorKind,
}

impl ActionLabelParamBindError {
    /// Returns a fixed remediation hint describing v1's parameter-binding requirements.
...
fn collect_safe_callsites(
    ...
    match &expr.node {
        ...
        Expr::Apply(op, args) if operator_matches_apply(operator, &op.node) => {
