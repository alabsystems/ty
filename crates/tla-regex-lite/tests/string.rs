use {
    ...
    regex_test::{CompiledRegex, Match, RegexTest, Span, TestResult, TestRunner},
};
...
fn run_test(re: &Regex, test: &RegexTest) -> TestResult {
    let hay = match std::str::from_utf8(test.haystack()) {
        Ok(hay) => hay,
        Err(err) => {
            return TestResult::fail(&format!("haystack is not valid UTF-8: {err}",));
        }
    };
...
fn compiler(test: &RegexTest, _patterns: &[String]) -> anyhow::Result<CompiledRegex> {
...
fn skip_or_get_pattern(test: &RegexTest) -> Option<&str> {
    ...
    if test.case_insensitive() && (!pattern.is_ascii() || !test.haystack().is_ascii()) {
        return None;
    }
    Some(pattern)
}
