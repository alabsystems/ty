...
Filed as **Part of #4250** during the ty zero-external-dep push (Wave 21).
...
4. `CARGO_TARGET_DIR=/tmp/ty-fork-regex cargo check --profile agent -p regex`.
5. `CARGO_TARGET_DIR=/tmp/ty-fork-regex cargo check --profile agent --workspace`.
