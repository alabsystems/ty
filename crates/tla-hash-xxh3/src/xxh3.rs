...
// Fork maintainer: Andrew Yates <andrewyates.name@gmail.com>
...
use crate::utils::{get_aligned_chunk_ref, get_unaligned_chunk, Buffer};
...
fn xxh3_128_1to3(input: &[u8], seed: u64, secret: &[u8]) -> u128 {
    ...
    let flip_lo = (read_32le_unaligned(secret, 0) as u64 ^ read_32le_unaligned(secret, 4) as u64)
        .wrapping_add(seed);
    let flip_hi = (read_32le_unaligned(secret, 8) as u64 ^ read_32le_unaligned(secret, 12) as u64)
        .wrapping_sub(seed);
