...
pub enum TirType {
    ...
    /// The string type.
    Str,
    /// A set whose elements have the given type.
    Set(Box<TirType>),
    /// A sequence whose elements have the given type.
    Seq(Box<TirType>),
    /// A function from the domain type (first) to the range type (second).
    Func(Box<TirType>, Box<TirType>),
    /// A tuple with one type per positional component.
    Tuple(Vec<TirType>),
    /// A closed record mapping each named field to its type.
    ...
    /// The dynamic/unknown type — the top-and-bottom of the gradual lattice.
    Dyn,
}
