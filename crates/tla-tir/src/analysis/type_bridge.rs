...
// ============================================================================
// Layout descriptors (native consumer)
// ============================================================================

/// Memory layout classification for native compilation.
///
/// This is the `tla-tir`-level equivalent of the stable ABI
/// `CompoundLayout`. Native backends can convert this without making
/// `tla-tir` depend on backend implementation crates.
...
pub enum TirLayout {
    ...
    Record {
        /// Field names paired with the layout of each field's value.
        fields: Vec<(NameId, TirLayout)>,
    },
    /// Sequence with homogeneous element layout.
    Seq {
        /// Layout shared by every element.
        element: Box<TirLayout>,
    },
    ...
    Func {
        /// Layout of the domain keys.
        key: Box<TirLayout>,
        /// Layout of the range values.
        value: Box<TirLayout>,
    },
    /// Tuple with per-position layouts.
    Tuple {
        /// Layout of each positional component, in order.
        elements: Vec<TirLayout>,
    },
...
pub fn tir_type_to_rust(ty: &TirType) -> String {
    match ty {
        ...
        TirType::Tuple(elems) => {
            let types: Vec<_> = elems.iter().map(tir_type_to_rust).collect();
            format!("({})", types.join(", "))
        }
