...
fn eval_arith(op: TirArithOp, lhs: i64, rhs: i64) -> Option<i64> {
    match op {
        ...
        TirArithOp::Div => {
            ...
            // Declined inexact quotients defer to the runtime engines, which all
            // implement `/` as EXACT-OR-ERROR (int_exact_div_op in tla-eval).
            // Decline i64::MIN / -1 (overflow panic) so it defers to the runtime
            // BigInt path, which yields the correct 2^63.
        ...
        TirArithOp::IntDiv => {
            // TLA+ integer division: floor division (Euclidean). Decline rhs == 0
            // and i64::MIN / -1 (div_euclid overflow panic); the latter defers to
            // the runtime BigInt path for the correct 2^63.
            if rhs == 0 || (lhs == i64::MIN && rhs == -1) {
                None
            } else {
        ...
        TirArithOp::Mod => {
            // TLC/TY require a STRICTLY POSITIVE modulus divisor: the runtime
            // int_mod_op raises ModulusNotPositive for rhs <= 0. Decline folding
            // for any non-positive divisor so evaluation defers to the runtime
            // (correct error parity), which also excludes the i64::MIN % -1 panic.
            if rhs <= 0 {
                None
            } else {
