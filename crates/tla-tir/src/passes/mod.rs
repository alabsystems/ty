...
/// A single TIR-to-TIR transformation pass.
pub trait TirPass {
    /// Short stable name identifying the pass (used for logging/diagnostics).
    #[must_use]
    fn name(&self) -> &'static str;

    /// Apply the pass to `expr`, returning the transformed expression.
    #[must_use]
    fn run(&self, expr: Spanned<TirExpr>) -> Spanned<TirExpr>;
}

/// An ordered collection of [`TirPass`]es run in sequence over an expression.
...
impl PassPipeline {
    ...
    /// Append `pass` to the end of the pipeline and return `&mut self` for chaining.
    #[must_use]
    ...
    /// Build the standard pipeline: NNF, constant folding, skolemization, dead-branch elimination.
    #[must_use]
    ...
    /// Run every pass in order, threading the expression through each.
    #[must_use]
...
/// Rewrites a boolean expression into negation normal form.
pub struct NnfPass;
...
/// Folds constant subexpressions (arithmetic, comparisons) to literals.
...
/// Marks fully-bounded existentials as skolemization candidates.
...
/// Eliminates statically-decidable dead branches in IF/CASE/quantifier/LET nodes.
