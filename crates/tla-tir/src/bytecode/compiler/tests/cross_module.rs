...
/// infinite recursion. The recursion guard causes a compiler-authenticated
/// self-recursive CallExternal in the private on-demand copy.
#[test]
fn test_recursive_callee_terminates_with_call_external() {
    ...
    let func_idx = compiler
        .compile_expression_with_callees("Main", &[], &body, &callees)
        .expect("recursive callee should compile through guarded fallback");
    let chunk = compiler.finish();
    assert!(
        !chunk.get_function(func_idx).instructions.is_empty(),
        "recursive callee should produce non-empty instructions"
    );
    assert!(
        chunk.functions.iter().any(|func| {
            func.name == "Rec"
        ...
        "direct recursion must carry compiler-authenticated provenance: {:?}",
        chunk.functions
    );
}

/// Parameterized direct recursion follows the Apply path used by helpers such
/// as btree's FindLeafNode(node, key).
#[test]
fn test_recursive_apply_marks_call_external_as_self_recursive() {
    let mut compiler = BytecodeCompiler::new();
    let mut callees = std::collections::HashMap::new();
    let recursive_apply = spanned(TirExpr::Apply {
        op: Box::new(name_ident("Rec")),
        args: vec![name_ident("n")],
    });
    callees.insert("Rec".to_string(), make_callee_info(&["n"], recursive_apply));
    let body = spanned(TirExpr::Apply {
        op: Box::new(name_ident("Rec")),
    ...
    compiler
        .compile_expression_with_callees("Main", &[], &body, &callees)
        .expect("parameterized recursive callee should compile through guarded fallback");
    let chunk = compiler.finish();
    assert!(
        chunk.functions.iter().any(|func| {
            func.name == "Rec"
        ...
        "recursive Apply must carry compiler-authenticated provenance: {:?}",
        chunk.functions
    );
}

/// A same-name forced interpreter callback is not evidence of recursion.
#[test]
fn test_force_external_same_name_is_not_marked_self_recursive() {
    let mut compiler = BytecodeCompiler::new();
    compiler.set_force_external_ops(std::collections::HashSet::from(["Rec".to_string()]));
    let idx = compiler
        .compile_expression("Rec", &name_ident("Rec"))
        .expect("forced external should compile");
    let chunk = compiler.finish();
    assert!(
        chunk.get_function(idx).instructions.iter().any(|opcode| {
            matches!(
                opcode,
                Opcode::CallExternal {
                    argc: 0,
        ...
        "forced same-name callback must remain unauthenticated"
    );
}

/// An Unsupported error from on-demand compilation must not be confused with
/// the dedicated recursion-guard error, even when name and arity match.
#[test]
fn test_unsupported_same_name_is_not_marked_self_recursive() {
    let mut compiler = BytecodeCompiler::new();
    let mut callees = std::collections::HashMap::new();
    callees.insert(
        "Bad".to_string(),
    ...
    let idx = compiler
        .compile_expression_with_callees("Bad", &[], &name_ident("Bad"), &callees)
        .expect("unsupported same-name callee should fall back to CallExternal");
    let chunk = compiler.finish();
    assert!(
        chunk.get_function(idx).instructions.iter().any(|opcode| {
            matches!(
        ...
        "unsupported same-name fallback must remain unauthenticated"
    );
}

/// The recursion guard also breaks mutual cycles, but F -> G -> F is not a
/// strict self-call at the failing site and must therefore remain unmarked.
#[test]
fn test_mutual_recursion_is_not_marked_self_recursive() {
    let mut compiler = BytecodeCompiler::new();
    let mut callees = std::collections::HashMap::new();
    callees.insert("F".to_string(), make_callee_info(&[], name_ident("G")));
    callees.insert("G".to_string(), make_callee_info(&[], name_ident("F")));

    compiler
        .compile_expression_with_callees("Main", &[], &name_ident("F"), &callees)
        .expect("mutually recursive callees should terminate through guarded fallback");
    let chunk = compiler.finish();
    let external_markers: Vec<bool> = chunk
        .functions
        .iter()
        .flat_map(|func| &func.instructions)
        .filter_map(|opcode| match opcode {
            Opcode::CallExternal { self_recursive, .. } => Some(*self_recursive),
            _ => None,
        })
        .collect();
    assert!(
        !external_markers.is_empty(),
        "mutual recursion should exercise the guarded external fallback"
    );
    assert!(
        external_markers.iter().all(|marked| !marked),
        "mutual recursion must not authenticate any strict self-call: {:?}",
        chunk.functions
    );
}

/// OperatorRef fallback is deliberately fail-closed: a qualified reference
/// is never marked as a strict recursive Name/Apply site.
#[test]
fn test_qualified_operator_ref_same_name_is_not_marked_self_recursive() {
    let mut compiler = BytecodeCompiler::new();
    let body = make_operator_ref(vec![module_segment("I")], "Rec", vec![]);
    let idx = compiler
        .compile_expression("Rec", &body)
        .expect("qualified unresolved OperatorRef should compile as external fallback");
    let chunk = compiler.finish();
    assert!(
        chunk.get_function(idx).instructions.iter().any(|opcode| {
            matches!(
        ...
        "qualified same-name OperatorRef must remain unauthenticated"
    );
}
