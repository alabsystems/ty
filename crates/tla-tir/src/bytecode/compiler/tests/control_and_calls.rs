...
#[test]
fn test_zero_arg_finite_domain_replacement_inlines_residual_exists_domain() {
    let max_nat_id = intern_name("MaxNat");
    let mut constants = std::collections::HashMap::new();
    constants.insert(max_nat_id, Value::SmallInt(2));

    let mut compiler = BytecodeCompiler::with_resolved_constants(constants);
    let mut replacements = std::collections::HashMap::new();
    replacements.insert("Nat".to_string(), "NatOverride".to_string());
    compiler.set_op_replacements(replacements);
    ...
    callees.insert(
        ...
        CalleeInfo {
            params: vec![],
            body: Arc::new(spanned(TirExpr::Range {
                lo: Box::new(spanned(TirExpr::Const {
                    value: Value::SmallInt(0),
                    ty: TirType::Int,
                })),
                hi: Box::new(spanned(TirExpr::Name(TirNameRef {
                    name: "MaxNat".to_string(),
                    name_id: max_nat_id,
    ...
    let body = spanned(TirExpr::Exists {
        vars: vec![TirBoundVar {
            name: "k".to_string(),
            name_id: intern_name("k"),
            domain: Some(Box::new(spanned(TirExpr::Name(TirNameRef {
                name: "Nat".to_string(),
                name_id: intern_name("Nat"),
                kind: TirNameKind::Ident,
                ty: TirType::Dyn,
            })))),
            pattern: None,
        ...
        body: Box::new(spanned(TirExpr::Cmp {
            left: Box::new(spanned(TirExpr::Name(ident_name("k")))),
            op: TirCmpOp::Eq,
            right: Box::new(spanned(TirExpr::Const {
                value: Value::SmallInt(1),
                ty: TirType::Int,
    ...
    let idx = compiler
        .compile_expression_with_callees("ExistsNat", &[], &body, &callees)
        .expect("finite Nat replacement should compile");
    let chunk = compiler.finish();
    let func = chunk.get_function(idx);
    ...
    assert!(
        ...
        "finite-domain replacement should inline to bytecode instead of leaving an opaque zero-arg Call: {:?}",
        func.instructions
    );
    let exists_domain = func
        .instructions
        .iter()
        .find_map(|op| match op {
            Opcode::ExistsBegin { r_domain, .. } => Some(*r_domain),
            _ => None,
        })
        .expect("test expression should contain EXISTS");
    assert!(
        func.instructions.iter().any(|op| matches!(
            op,
            Opcode::Range { rd, .. } if *rd == exists_domain
        )),
        "EXISTS domain should be produced by an inline Range, got {:?}",
        func.instructions
    );

    let domains = crate::bytecode::action_transform::collect_residual_quantifier_domains(
        &func.instructions,
        Some(&chunk.constants),
    )
    .expect("inline finite-domain replacement should preserve residual EXISTS metadata");
    assert_eq!(domains.len(), 1);
    assert_eq!(
        domains[0].domain.source,
        crate::bytecode::action_transform::ActionDomainSource::Range { pc: 2 }
    );
...
#[test]
fn test_compile_unchanged_zero_arg_tuple_alias() {
    let mut compiler = BytecodeCompiler::new();
    ...
    callees.insert(
        ...
        CalleeInfo {
            params: vec![],
            body: Arc::new(spanned(TirExpr::Tuple(vec![
    ...
    let body = spanned(TirExpr::Unchanged(Box::new(spanned(TirExpr::Name(
        ident_name("Vars"),
    )))));
    let idx = compiler
        .compile_expression_with_callees("Next", &[], &body, &callees)
        .expect("UNCHANGED zero-arg tuple alias should compile");
    let chunk = compiler.finish();
    let func = chunk.get_function(idx);
    ...
    assert!(
        func.instructions
            .iter()
            .any(|op| matches!(op, Opcode::Unchanged { count: 2, .. })),
        "UNCHANGED Vars should emit Unchanged with count=2: {:?}",
        func.instructions
    );
    assert!(
        !func
            .instructions
            .iter()
            .any(|op| matches!(op, Opcode::SetPrimeMode { .. })),
        "UNCHANGED Vars should not use SetPrimeMode fallback: {:?}",
        func.instructions
    );
}
