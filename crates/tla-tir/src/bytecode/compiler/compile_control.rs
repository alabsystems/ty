...
/// callee of the same source name in `compile_apply`.
pub(super) fn fixed_builtin_call(name: &str) -> Option<(BuiltinOp, usize)> {
    Some(match name {
        // Sequences module
        "Len" => (BuiltinOp::Len, 1),
        ...
        // SequencesExt module
        "RemoveAt" => (BuiltinOp::RemoveAt, 2),
        "Seq" => (BuiltinOp::Seq, 1),
        // FiniteSets module
        "Cardinality" => (BuiltinOp::Cardinality, 1),
        "IsFiniteSet" => (BuiltinOp::IsFiniteSet, 1),
        // Functions / SequencesExt module
        "Range" => (BuiltinOp::Range, 1),
        // TLC module
        "ToString" => (BuiltinOp::ToString, 1),
        // Bags / BagsExt modules
        "BagAdd" => (BuiltinOp::BagAdd, 2),
        "BagRemove" => (BuiltinOp::BagRemove, 2),
        "SetToBag" => (BuiltinOp::SetToBag, 1),
        _ => return None,
    })
}

fn is_ident_named(expr: &Spanned<TirExpr>, expected: &str) -> bool {
    matches!(
        &expr.node,
        TirExpr::Name(name_ref)
            if matches!(name_ref.kind, TirNameKind::Ident) && name_ref.name == expected
    )
}

fn is_small_int_const(expr: &Spanned<TirExpr>, expected: i64) -> bool {
    matches!(
        &expr.node,
        TirExpr::Const {
...
/// Exact structural proof for `IF p = <<>> THEN 0 ELSE p[2]`.
///
/// This intentionally recognizes no labels, LET aliases, alternate branches,
/// or partial bodies. Keeping the proof narrower than algebraic equivalence is
/// what makes replacing the complete call safe without an inliner or effect
/// analysis.
pub(super) fn is_round_shape_body(body: &Spanned<TirExpr>, param: &str) -> bool {
    let TirExpr::If { cond, then_, else_ } = &body.node else {
        return false;
    };
    ...
    if *op != TirCmpOp::Eq
        || !is_ident_named(left, param)
        || !matches!(&right.node, TirExpr::Tuple(elements) if elements.is_empty())
        || !is_small_int_const(then_, 0)
    {
        return false;
    }
    let TirExpr::FuncApply { func, arg } = &else_.node else {
        return false;
    };
    is_ident_named(func, param) && is_small_int_const(arg, 2)
}
...
impl<'a> FnCompileState<'a> {
    ...
    /// Compile a flattened And/Or chain with a single accumulator register
    /// (`reg_recycle` mode). The nested `BoolBinOp` tree is walked in-order,
    /// so operands evaluate in exactly the same left-to-right short-circuit
    /// order as the nested compilation and the tree-walking interpreter;
    /// every operand's temporaries are rolled back once its value has been
    /// moved into the accumulator.
    fn compile_bool_chain_recycled(
        &mut self,
        op: TirBoolOp,
        ...
        fn collect<'e>(
            op: TirBoolOp,
            expr: &'e Spanned<TirExpr>,
            out: &mut Vec<&'e Spanned<TirExpr>>,
        ) {
        ...
        let mut operands: Vec<&Spanned<TirExpr>> = Vec::new();
        collect(op, left, &mut operands);
        collect(op, right, &mut operands);
        ...
        for (i, operand) in operands.iter().enumerate() {
            ...
            self.func.emit(Opcode::Move { rd, rs: r });
            self.reg_rollback(cp);
            if i != last {
                let jump_idx = match op {
                    TirBoolOp::And => self.func.emit(Opcode::JumpFalse {
                        rs: rd,
                        offset: 0, // patched
                    }),
                    TirBoolOp::Or => self.func.emit(Opcode::JumpTrue {
                        rs: rd,
                        offset: 0, // patched
                    }),
                    TirBoolOp::Implies | TirBoolOp::Equiv => {
                        unreachable!("chain compilation is only used for And/Or")
                    }
                };
                jumps.push(jump_idx);
            }
    ...
    pub(super) fn compile_bool_binop(
        ...
        // VM-only exact conjunction fusion for Sailfish's edge TypeOK arm.
        // The opcode performs the left equality before it reads the RHS state
        // slot, retaining the source conjunction's short-circuit order.
        if op == TirBoolOp::And && self.tuple2_self_subseteq {
            if let Some((value, set_var_idx)) = self.match_tuple2_self_subseteq(left, right) {
                let rd = self.alloc_reg()?;
                self.func.emit(Opcode::Tuple2SelfSubseteq {
                    rd,
                    value,
                    set_var_idx,
                });
        ...
        // With `reg_recycle`, the result accumulator is allocated BEFORE the
        // operands and each operand's temporaries are rolled back once its
        // value has been moved into the accumulator (or consumed by the final
        // opcode). This keeps register pressure at max-per-operand instead of
        // sum-over-operands, which is what lets wide action disjunctions
        // (implied-action terms) fit the 256-register file. Without the flag
        // the original allocation order is preserved bit-for-bit.
        match op {
            TirBoolOp::And => {
                // Short-circuit: if left is FALSE, skip right.
                if self.reg_recycle || self.flatten_bool_chains {
                    // Flatten the whole same-operator chain and evaluate it
                    // iteratively with ONE accumulator: left-nested chains
                    // would otherwise leak one accumulator register per level
                    // (a 300-conjunct action overflows the register file).
                    // Flattening preserves the exact left-to-right evaluation
                    // and short-circuit order of the nested tree. In
                    // flatten-only mode the checkpoint/rollback inside is a
                    // no-op (registers stay monotone-unique).
                    return self.compile_bool_chain_recycled(TirBoolOp::And, left, right);
                }
            ...
            TirBoolOp::Or => {
                ...
                if self.reg_recycle || self.flatten_bool_chains {
                    // See the And arm: flattened one-accumulator chain.
                    return self.compile_bool_chain_recycled(TirBoolOp::Or, left, right);
                }
                let r1 = self.compile_expr(left)?;
                let rd = self.alloc_reg()?;
                self.func.emit(Opcode::Move { rd, rs: r1 });
            ...
            TirBoolOp::Implies => {
                ...
                if self.reg_recycle {
                    ...
                    // JumpFalse reads r1 immediately; nothing re-reads it after
                    // the jump, so its slot is safe to recycle for `right`.
                    let jump_idx = self.func.emit(Opcode::JumpFalse {
                        rs: r1,
                        offset: 0, // patched
                    });
    ...
    pub(super) fn compile_let(
        ...
        // WP-21 guard-first LET: when the body is a conjunction with a
        // provably pure leading-guard prefix that references no LET-bound
        // name (and every zero-arg def is provably pure), evaluate those
        // guards BEFORE the defs and short-circuit to FALSE without touching
        // any def. Exactly TLC's lazy-LET order; see `let_guard_hoist.rs`
        // for the soundness argument. Any analysis failure falls through to
        // the unchanged eager path below.
        if let Some((guards, rest)) = self.guard_first_split(defs, body) {
            return self.compile_guard_first_let(defs, &guards, &rest);
        }
    ...
    /// Compile `LET defs IN /\ guards /\ rest` with the guard prefix
    /// evaluated BEFORE the defs (WP-21; see `let_guard_hoist.rs`).
    ///
    /// Emits one single-accumulator And chain — the same
    /// `Move rd / JumpFalse rd` shape as [`Self::compile_bool_chain_recycled`],
    /// which the next-state transform already consumes — with the def
    /// evaluations placed between the hoisted guards and the remaining
    /// conjuncts. A FALSE guard short-circuits to the end without evaluating
    /// any def.
    fn compile_guard_first_let(
        &mut self,
        defs: &[crate::nodes::TirLetDef],
        guards: &[&Spanned<TirExpr>],
        rest: &[&Spanned<TirExpr>],
    ) -> Result<Register, CompileError> {
        debug_assert!(!guards.is_empty() && !rest.is_empty());
        let rd = self.alloc_reg()?;
        let cp = self.reg_checkpoint();
        let mut jumps = Vec::new();
        for guard in guards {
            let r = self.compile_expr(guard)?;
            self.func.emit(Opcode::Move { rd, rs: r });
            self.reg_rollback(cp);
            jumps.push(self.func.emit(Opcode::JumpFalse {
                rs: rd,
                offset: 0, // patched
            }));
        }

        // Bind the defs exactly as the eager path does (guards all TRUE).
        let saved_bindings = self.bindings.len();
        let saved_local_ops: std::collections::HashSet<String> =
            self.local_op_indices.keys().cloned().collect();
        for def in defs {
        ...
        // Remaining conjuncts continue the same accumulator chain. The
        // rollback checkpoint sits ABOVE the def registers so recycling can
        // never free a live binding.
        let cp_rest = self.reg_checkpoint();
        let last = rest.len() - 1;
        for (i, conjunct) in rest.iter().enumerate() {
            let r = self.compile_expr(conjunct)?;
            self.func.emit(Opcode::Move { rd, rs: r });
            self.reg_rollback(cp_rest);
            if i != last {
                jumps.push(self.func.emit(Opcode::JumpFalse {
                    rs: rd,
                    offset: 0, // patched
    ...
    pub(super) fn compile_apply(
        ...
        match &op.node {
                    ...
                    && self.lookup_binding(&name_ref.name).is_none() =>
            {
                ...
                // Pinned interpreter callback (see compile_name_expr).
                ...
                if let Some(result) =
                    self.try_compile_round_shape_apply(&name_ref.name, &resolved_name, args)?
                {
                    return Ok(result);
                }
    ...
    /// Emit the VM-only Round-shape superinstruction after proving that this
    /// is a complete direct-global call, not a replacement, builtin, state
    /// variable, LET-local, higher-order binding, or forced external.
    fn try_compile_round_shape_apply(
        &mut self,
        source_name: &str,
        resolved_name: &str,
        args: &[Spanned<TirExpr>],
    ) -> Result<Option<Register>, CompileError> {
        if !self.round_shape_apply
            || args.len() != 1
            || source_name != resolved_name
            || self
                .op_replacements
                .is_some_and(|replacements| replacements.contains_key(source_name))
            || self.local_op_indices.contains_key(source_name)
            || self
                .state_vars
                .is_some_and(|state_vars| state_vars.contains_key(source_name))
            || fixed_builtin_call(source_name).is_some()
            || self.is_force_external(source_name, resolved_name)
        {
            return Ok(None);
        }
        ...
        let Some(info) = self
            .callee_bodies
            .and_then(|callees| callees.get(source_name))
        else {
            return Ok(None);
        };
        ...
        if !is_round_shape_body(&info.body, param) {
            return Ok(None);
        }

        // Compile the source expression exactly once. A one-argument
        // CallBuiltin needs no reserved/moved argument block: the argument's
        // result register itself is a valid one-element slice.
        let args_start = self.compile_expr(&args[0])?;
        let rd = self.alloc_reg()?;
    ...
    pub(super) fn compile_operator_ref(
        ...
        // Pinned interpreter callback (see compile_name_expr).
        if self.is_force_external(op_name, &resolved_name) {
            let name_idx = self.add_const(Value::string(resolved_name.clone()))?;
        ...
        let name_idx = self.add_const(Value::string(op_name))?;
        if !op_ref.args.is_empty() {
            let args_start = self.compile_exprs_into_consecutive(op_ref.args.iter())?;
            let rd = self.alloc_reg()?;
            self.func.emit(Opcode::CallExternal {
    ...
    pub(super) fn compile_closure_const(
        ...
        self.compile_const(&Value::Closure(Rp::new(closure)))
    }
    ...
    fn compile_capturing_lambda(
        ...
        let template_idx = self.add_const(Value::Closure(Rp::new(closure)))?;
    ...
    pub(super) fn compile_callee_on_demand(
        ...
        if !self.on_demand_compiling.insert(name.to_string()) {
            return Err(CompileError::RecursiveOnDemand(name.to_string()));
        }
    ...
    pub(super) fn try_compile_builtin_call(
        ...
        let Some((builtin, expected_argc)) = fixed_builtin_call(name) else {
            return Ok(None);
        };
