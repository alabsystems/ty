...
impl ConstantPool {
    ...
    /// Get a constant value by index without panicking on out-of-range
    /// indices. Used by static bytecode analyses (state-footprint scan) that
    /// must fail closed on malformed chunks instead of panicking.
    #[inline]
    #[must_use]
    pub fn try_get_value(&self, idx: ConstIdx) -> Option<&Value> {
...
pub struct BytecodeFunction {
    ...
    /// Highest instruction index any `patch_jump` has ever targeted in this
    /// function. Used by the Eq-fusion peephole as a fail-closed guard: a
    /// producer instruction may only be fused into its consuming `Eq` when no
    /// patched jump targets a pc BEYOND the producer (such a jump would have
    /// been patched to land on the not-yet-emitted consumer and must keep its
    /// two-instruction landing pad). Targets at or before the producer are
    /// safe: the fused opcode executes the full producer+Eq semantics at the
    /// producer's pc.
    pub max_patched_target: usize,
}
...
impl BytecodeFunction {
    ...
    pub fn patch_jump(&mut self, idx: usize, target: usize) {
        if target > self.max_patched_target {
            self.max_patched_target = target;
        }
...
///
/// Used by the EXISTS-binding trust-codegen adapter (#4270): the trust-ir → trust-codegen path reuses
...
/// Specialize a bytecode function by prepending typed scalar constants.
///
/// This preserves string/model-value provenance by emitting `LoadConst` entries
/// into `constants`, instead of erasing them into raw integer `LoadImm` values.
pub fn specialize_bytecode_function_with_values(
    base: &BytecodeFunction,
    ...
    for (i, value) in binding_values.iter().enumerate() {
        let rd =
            u8::try_from(i).map_err(|_| format!("binding register index {i} exceeds u8::MAX"))?;
        match value {
            Value::SmallInt(value) => {
                func.emit(Opcode::LoadImm { rd, value: *value });
            }
            Value::Int(value) => {
                let value = value
                    .to_i64()
                    .ok_or_else(|| format!("binding value {value} does not fit in i64"))?;
                func.emit(Opcode::LoadImm { rd, value });
            }
            Value::Bool(value) => {
                func.emit(Opcode::LoadBool { rd, value: *value });
            }
            Value::String(_) | Value::ModelValue(_) => {
                let idx = constants.add_value(value.clone());
                func.emit(Opcode::LoadConst { rd, idx });
            }
            other if value_is_finite_load_const_literal(other) => {
                let idx = constants.add_value(other.clone());
                func.emit(Opcode::LoadConst { rd, idx });
            }
            other => {
                return Err(format!(
                    "unsupported non-finite binding value {}",
                    other.type_name()
                ));
            }
        }
...
fn value_is_finite_load_const_literal(value: &Value) -> bool {
    match value {
        Value::SmallInt(_)
        | Value::Int(_)
        | Value::Bool(_)
        ...
        Value::Set(set) => set.iter().all(value_is_finite_load_const_literal),
        Value::Interval(_) => true,
        Value::Seq(seq) => seq.iter().all(value_is_finite_load_const_literal),
        Value::Tuple(tuple) => tuple.iter().all(value_is_finite_load_const_literal),
        Value::Record(record) => record
            .iter()
            .all(|(_, value)| value_is_finite_load_const_literal(value)),
        Value::Func(func) => func.iter().all(|(key, value)| {
            value_is_finite_load_const_literal(key) && value_is_finite_load_const_literal(value)
        }),
        Value::IntFunc(func) => func.values().iter().all(value_is_finite_load_const_literal),
        _ => false,
    }
}
...
mod tests {
    ...
    #[test]
    fn test_specialize_bytecode_function_with_values_preserves_model_value_type() {
        let mut base = BytecodeFunction::new("Action".to_string(), 1);
        base.emit(Opcode::Ret { rs: 0 });

        let mut constants = ConstantPool::new();
        let specialized = specialize_bytecode_function_with_values(
            &base,
            &[Value::ModelValue("p1".into())],
            "Action__p1",
            &mut constants,
        )
        .expect("model-value scalar should specialize");

        assert_eq!(specialized.arity, 0);
        assert!(matches!(
            specialized.instructions.first(),
            Some(Opcode::LoadConst { rd: 0, idx: 0 })
        ));
        assert_eq!(constants.get_value(0), &Value::ModelValue("p1".into()));
    }

    #[test]
    fn test_specialize_bytecode_function_with_values_uses_load_const_for_finite_compound() {
        let mut base = BytecodeFunction::new("Action".to_string(), 1);
        base.emit(Opcode::Ret { rs: 0 });
        ...
        .expect("finite compound binding should specialize through LoadConst");
        ...
        assert_eq!(constants.get_value(0), &binding);
    }
