...
use tla_core::{get_primed_var_refs_v, intern_name, Spanned};
...
impl<'a> Lowerer<'a> {
    ...
    fn lower_non_action_expr(
        ...
        match &expr.node {
            ...
            Expr::Let(defs, body) => {
                let primed_refs = get_primed_var_refs_v(&body.node);
                if defs.iter().any(|def| {
                    def.params.is_empty() && primed_refs.contains(def.name.node.as_str())
                }) {
                    return Err(TirLowerError::UnsupportedExpr {
                        kind: "LetWithPrimedZeroArgDef",
                        span: expr.span,
                    });
                }
