...
impl<'a> TirLoweringEnv<'a> {
    /// Create an environment seeded with `root` as its first known module.
    #[must_use]
    ...
    /// Register `module` so its operators can be resolved during lowering.
    pub fn add_module(&mut self, module: &'a Module) {
        self.modules.insert(module.name.node.clone(), module);
    }

    /// Builder form of [`add_module`](Self::add_module): register `module` and return `self`.
    #[must_use]
...
pub struct TirModule {
    /// Name of the lowered module.
    pub name: String,
    /// Operators defined by the module, in declaration order.
...
pub struct TirOperator {
    /// Operator name as written in source.
    pub name: String,
    ...
    /// Whether the operator is declared `RECURSIVE`.
    pub is_recursive: bool,
    /// The lowered operator body, with its source span.
...
/// from the root module's INSTANCE declaration plus implicit same-name
/// variable/constant substitutions.
///
/// Creates a `LoweringScope` with:
/// - `instance_module` as the current module (sibling operator resolution)
/// - Effective substitution bindings from `substitutions` and implicit
///   same-name targets (name remapping)
...
pub fn lower_operator_in_instance_scope<'a>(
    ...
    let instance_scope =
        root_scope.with_instance_substitutions(instance_module, &root_scope, substitutions);
    Ok(TirOperator {
        name: def.name.node.clone(),
        name_id: intern_name(&def.name.node),
        params: def.params.iter().map(|p| p.name.node.clone()).collect(),
...
/// Permissive variant of [`lower_operator_in_instance_scope`].
///
/// Like [`lower_operator_with_env_permissive`], allows parameterized stdlib
/// builtins (`Append`, `Len`, `Cardinality`, etc.) through as generic
/// `TirExpr::Apply` nodes while preserving the INSTANCE substitution scope.
/// Used ONLY by the bytecode compilation path (which maps these builtins to
/// `CallBuiltin` opcodes); the TIR interpreter cache must keep using the
/// strict variant.
pub fn lower_operator_in_instance_scope_permissive<'a>(
    env: &'a TirLoweringEnv<'a>,
    root_module: &'a Module,
    instance_module: &'a Module,
    def: &OperatorDef,
    ...
    Ok(TirOperator {
        ...
        params: def.params.iter().map(|p| p.name.node.clone()).collect(),
        is_recursive: def.is_recursive,
        body: Lowerer::new_permissive(env).lower_expr(&instance_scope, &def.body)?,
    })
}

/// Compute root-scope INSTANCE substitutions with implicit same-name
/// variable/constant targets included.
#[must_use]
pub fn effective_instance_substitutions(
    root_module: &Module,
    instance_module: &Module,
    substitutions: &[Substitution],
) -> Vec<Substitution> {
    let root_scope = LoweringScope::root(root_module);
    scope::effective_instance_substitutions_for_scope(&root_scope, instance_module, substitutions)
}
