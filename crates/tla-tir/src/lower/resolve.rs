...
impl<'a> Lowerer<'a> {
    ...
    fn resolve_instance_expr(
        ...
        match &expr.node {
            Expr::InstanceExpr(module_name, substitutions) => {
                ...
                Ok(scope.with_instance_substitutions(module, scope, substitutions))
            }
    ...
    fn lookup_imported_operator(
        ...
        self.imported_scopes_in_resolution_order(scope)
            .into_iter()
            .find_map(|import_scope| {
                find_operator(import_scope.module(), name).map(|def| ResolvedOperator {
                    scope: import_scope,
                    def,
                })
            })
    ...
    fn imported_scopes_in_resolution_order(
        &self,
        scope: &LoweringScope<'a>,
    ) -> Vec<LoweringScope<'a>> {
        ...
        fn visit_extends<'a>(
            ...
            out: &mut Vec<LoweringScope<'a>>,
        ) {
            for ext in &scope.module().extends {
                ...
                let ext_scope = scope.with_module(loaded);
                visit_extends(env, &ext_scope, seen, out);
                out.push(ext_scope);
            }
        }

        visit_extends(self.env, scope, &mut extends_seen, &mut extends_order);

        fn collect_standalone_instances<'a>(
            env: &'a crate::lower::TirLoweringEnv<'a>,
            scope: &LoweringScope<'a>,
            queue: &mut VecDeque<LoweringScope<'a>>,
        ) {
            for unit in &scope.module().units {
                if let Unit::Instance(inst) = &unit.node {
                    if let Some(loaded) = env.module(&inst.module.node) {
                        queue.push_back(scope.with_instance_substitutions(
                            loaded,
                            scope,
                            &inst.substitutions,
                        ));
        ...
        collect_standalone_instances(self.env, scope, &mut queue);
        for extended_scope in &extends_order {
            collect_standalone_instances(self.env, extended_scope, &mut queue);
        }

        while let Some(next_scope) = queue.pop_front() {
            if !instance_seen.insert(next_scope.module().name.node.clone()) {
                continue;
            }
            collect_standalone_instances(self.env, &next_scope, &mut queue);
            instance_order.push(next_scope);
        }

        let extends_set: HashSet<&str> = extends_order
            .iter()
            .map(|s| s.module().name.node.as_str())
            .collect();
        instance_order.retain(|scope| !extends_set.contains(scope.module().name.node.as_str()));
