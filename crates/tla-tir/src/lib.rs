...
//! Typed Intermediate Representation (TIR) for TY.
//!
...
mod error;
mod lower;
/// TIR node definitions: the typed expression and unit trees produced by lowering.
pub mod nodes;
/// TIR transformation passes and the pass-driver infrastructure.
pub mod passes;
pub mod preprocess;
/// The gradual TIR type lattice ([`types::TirType`]) and its operations.
pub mod types;
pub mod unify;
...
pub use lower::{
    effective_instance_substitutions, lower_expr, lower_expr_with_env, lower_module,
    lower_module_for_codegen, lower_module_with_env, lower_operator,
    lower_operator_in_instance_scope, lower_operator_in_instance_scope_permissive,
    lower_operator_with_env, lower_operator_with_env_permissive, TirLoweringEnv, TirModule,
    TirOperator,
};
pub use nodes::{
