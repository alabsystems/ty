...
/// A Rust source span: a `(file, start, end)` range with 1-indexed line and
/// column coordinates.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
///
/// Steps are ordered from the initial state to the violating state; each
/// carries the Rust location and a post-step state snapshot.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MappedTrace {
    /// The ordered steps of the trace, initial state first.
