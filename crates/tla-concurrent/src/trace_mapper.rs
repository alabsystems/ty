...
// Kept as the documented top-level entry point (exercised by tests); the
// module is private so it currently has no non-test in-crate callers.
#[allow(dead_code)]
...
mod tests {
    ...
    fn test_build_action_index() {
        ...
        assert!(
            !index.contains_key("Action_main_s1_done_main"),
