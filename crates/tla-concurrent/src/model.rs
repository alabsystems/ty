...
///
/// This is the contract between the extractor (tRust) and the checker (TY).
/// It is serialized as JSON and passed via the `ty thread-check` CLI.
...
///
/// Local variables are private to a single process and never participate in
/// data-race analysis; they exist in the model so transition guards and values
/// can reference per-thread state.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct LocalVar {
    /// Variable name as it appears in the Rust source.
    pub name: String,
    /// Initial value expression in TLA+ syntax, or `None` for an
    /// uninitialized local.
...
pub struct AccessSite {
    ...
    /// Rust source file containing the access, or `None` if unknown.
    pub source_file: Option<String>,
    /// 1-indexed source line of the access, or `None` if unknown.
    pub source_line: Option<u32>,
    /// 1-indexed source column of the access, or `None` if unknown.
    pub source_col: Option<u32>,
}

/// Kind of access to a shared variable.
///
/// Determines the guard discipline checked by `DataRaceFreedom`: reads may
/// share a lock, writes require exclusivity.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum AccessKind {
    /// A pure read of the shared variable's value.
    Read,
    /// A write that overwrites the shared variable's value.
    Write,
    /// An atomic read-modify-write (e.g. `fetch_add`, compare-exchange).
    ReadModifyWrite,
}
