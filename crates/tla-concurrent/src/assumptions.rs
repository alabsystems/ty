...
/// State space reduction technique applied during exploration.
///
/// Each reduction trades completeness for tractability and is recorded so the
/// result can honestly state what was (and was not) covered.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum Reduction {
    /// Process symmetry: K identical threads collapse a K!-factor of the state
    /// space into one representative interleaving.
    Symmetry {
        /// Number of interchangeable processes in the symmetry group.
        group_size: usize,
    },
    /// Partial order reduction: independent (commuting) transitions are not
    /// re-explored in every order.
    PartialOrderReduction,
    /// Preemption bounding: limit the number of preemptive context switches
    /// (CHESS observed 90%+ of bugs surface at K ≤ 2).
    PreemptionBounding {
        /// Maximum number of preemptions allowed along any explored path.
        bound: usize,
    },
    /// Context bounding: limit the total number of context switches explored.
    ContextBounding {
        /// Maximum number of context switches allowed along any explored path.
        bound: usize,
    },
}
