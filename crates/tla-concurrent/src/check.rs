...
pub enum ConcurrentError {
    /// The JSON `ConcurrentModel` payload could not be deserialized.
    ...
    /// The model is structurally invalid (e.g. no processes, a process with no
    /// transitions, or a reference to an unknown sync primitive). The string
    /// describes the specific failure.
    #[error("model validation error: {0}")]
    Validation(String),
    /// TLA+ module generation from the validated model failed.
    #[error("TLA+ generation error: {0}")]
    Generation(String),
    /// The underlying model checker reported an error during exploration.
...
///
/// Note that a property *violation* is not an error: it is reported as a
/// non-`AllPropertiesHold` status inside the returned
/// [`ConcurrentCheckResult`], with a source-mapped counterexample.
///
/// # Errors
///
/// Returns [`ConcurrentError::Validation`] if the model fails structural
/// validation — it has no processes, a process has no transitions, or a
/// transition references a sync primitive not declared in the model.
pub fn check_concurrent_model(
    ...
    Ok(ConcurrentCheckResult {
        ...
        generated_tla: if options.emit_tla {
            // Emit real TLA+ source via the workspace pretty-printer rather than a
            // Rust Debug dump of the AST.
            Some(tla_core::format_module(&module))
        } else {
            None
        },
    })
