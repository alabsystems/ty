...
pub struct CheckerStats {
    /// Number of distinct states found by the checker. (The underlying
    /// `CheckStats::states_found` is already a distinct count.)
    pub states_found: usize,
    /// Number of distinct states explored. Currently equal to `states_found`,
    /// kept as a separate field for forward compatibility and clarity.
    pub distinct_states: usize,
    /// Number of initial states the model admits.
    pub initial_states: usize,
    /// Wall-clock duration of the model-checking phase, in milliseconds.
    pub duration_ms: u64,
}
