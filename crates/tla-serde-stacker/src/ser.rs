    ...
    T: ?Sized + ser::Serialize,
{
    ...
    where
        S: ser::Serializer,
    {
        // TY FORK FIX: grow the stack here, exactly as the deserializer's `Visitor`
        // methods do. This wrapper is the ONLY recursion point for seq/tuple/map/struct
        // elements and fields, so without a `maybe_grow` an externally-tagged
        // tuple-variant tree (e.g. a deep `App(a, b)` spine) recursed with NO growth
        // checkpoint at all and overflowed the stack. Upstream grows only through
        // `SerializeSized` (the serialize_some / newtype paths).
