...
impl<I> FusedIterator for EncodeWide<I> where I: FusedIterator + Iterator<Item = u8> {}
                ...
                code_point
                    ...
                    .map(|offset| {
                        ...
                        self.surrogate = Some(unsafe {
                            NonZeroU16::new_unchecked((offset & 0x3FF) as u16 | MIN_LOW_SURROGATE)
                        });
...
pub(in super::super) fn encode_wide(string: &[u8]) -> EncodeWide<impl '_ + Iterator<Item = u8>> {
