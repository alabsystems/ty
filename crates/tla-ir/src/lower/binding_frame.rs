...
//! the trust-ir level, each iteration must
//!
...
//! the trust-ir optimizer and future widening passes can see the type
...
pub(super) struct BindingFrame {
    ...
    /// trust-ir block index for the loop header (bounds check + CondBr).
