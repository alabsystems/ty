...
pub fn value_to_itf(value: &Value) -> JsonValue {
    match value {
        ...
        // Bag: identical ITF shape to the equivalent Func (counts are Ints).
        Value::Bag(bag) => {
            let pairs: Vec<JsonValue> = bag
                .entries()
                .map(|(k, c)| json!([value_to_itf(k), value_to_itf(&Value::SmallInt(c))]))
                .collect();
            json!({ "#map": pairs })
        }
...
mod tests {
    ...
    fn test_itf_value_big_int() {
        let big = Value::Int(Rp::new(BigInt::from(i64::MAX) + BigInt::from(1)));
    ...
    fn test_itf_value_string_with_quotes() {
        let s = Value::String(Rp::from("say \"hi\""));
    ...
    fn test_itf_value_tuple() {
        let t = Value::Tuple(Rp::from(vec![int(1), Value::Bool(true), int(3)]));
        assert_eq!(
            value_to_itf(&t),
    ...
    fn test_itf_value_sequence() {
        let seq = Value::Seq(Rp::new(SeqValue::from_vec(vec![int(10), int(20)])));
        assert_eq!(
            value_to_itf(&seq),
    ...
    fn test_itf_value_model_value() {
        let mv = Value::ModelValue(Rp::from("p1"));
    ...
    fn test_itf_value_func_map() {
        ...
        let func = Value::Func(Rp::new(FuncValue::from_sorted_entries(vec![
            (Value::String(Rp::from("a")), int(1)),
            (Value::String(Rp::from("b")), int(2)),
        ])));
