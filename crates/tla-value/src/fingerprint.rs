...
pub mod value_tags {
    ...
    /// Type tag for booleans (TLC `BoolValue`).
    pub const BOOLVALUE: i64 = 0;
    /// Type tag for integers (TLC `IntValue`).
    pub const INTVALUE: i64 = 1;
    /// Type tag for strings (TLC `StringValue`).
    pub const STRINGVALUE: i64 = 3;
    /// Type tag for enumerated sets (TLC `SetEnumValue`).
    pub const SETENUMVALUE: i64 = 5;
    /// Type tag for predicate-defined sets `{x \in S : P(x)}` (TLC `SetPredValue`).
    pub const SETPREDVALUE: i64 = 6;
    /// Type tag for lambda functions `[x \in S |-> e]` (TLC `FcnLambdaValue`).
    pub const FCNLAMBDAVALUE: i64 = 8;
    /// Type tag for record-backed functions (TLC `FcnRcdValue`).
    pub const FCNRCDVALUE: i64 = 9;
    /// Type tag for operator lambdas (TLC `OpLambdaValue`).
    pub const OPLAMBDAVALUE: i64 = 10;
    /// Type tag for function sets `[S -> T]` (TLC `SetOfFcnsValue`).
    pub const SETOFFCNSVALUE: i64 = 13;
    /// Type tag for record sets `[f: S, ...]` (TLC `SetOfRcdsValue`).
    pub const SETOFRCDSVALUE: i64 = 14;
    /// Type tag for Cartesian-product sets `S \X T` (TLC `SetOfTuplesValue`).
    pub const SETOFTUPLESVALUE: i64 = 15;
    /// Type tag for powersets `SUBSET S` (TLC `SubsetValue`).
    pub const SUBSETVALUE: i64 = 16;
    /// Type tag for set difference `S \ T` (TLC `SetDiffValue`).
    pub const SETDIFFVALUE: i64 = 17;
    /// Type tag for set intersection `S \cap T` (TLC `SetCapValue`).
    pub const SETCAPVALUE: i64 = 18;
    /// Type tag for set union `S \cup T` (TLC `SetCupValue`).
    pub const SETCUPVALUE: i64 = 19;
    /// Type tag for the union-of-a-set-of-sets `UNION S` (TLC `UnionValue`).
    pub const UNIONVALUE: i64 = 20;
    /// Type tag for model values (TLC `ModelValue`).
    pub const MODELVALUE: i64 = 21;
    /// Type tag for user-defined module values (TLC `UserValue`).
...
pub fn fp64_smallint_lookup(n: i64) -> Option<u64> {
    if (SMALLINT_FP_TABLE_MIN..=SMALLINT_FP_TABLE_MAX).contains(&n) {
...
mod tests {
    ...
    fn fp64_seq_deterministic_without_cache() {
        let value = Value::Seq(Rp::new(SeqValue::from(vec![
            Value::Bool(true),
            Value::Bool(false),
        ])));
