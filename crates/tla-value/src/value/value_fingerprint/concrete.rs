...
pub(super) fn fingerprint_extend_concrete(value: &Value, fp: u64) -> FingerprintResult<u64> {
    match value {
        ...
        Value::Interval(intv) => {
            ...
            let mut fp = fp64_extend_i64(fp, SETENUMVALUE);
            // Use IntervalValue::len() (which guards low > high and returns 0)
            // rather than a raw high - low + 1, so an empty interval fingerprints
            // as SETENUMVALUE + i32(0) — byte-identical to the Eq-equal empty Set —
            // instead of a spurious negative length that breaks the fingerprint/Eq
            // contract for empty collections.
            let len = intv.len();
        ...
        Value::Bag(bag) => {
            // Bag is semantically a function — the TLC FP64 fingerprint MUST
            // be byte-identical to the equivalent Func's. Delegate to the
            // cached materialized Func (identical entries in identical order,
            // including the TLC-normalized permutation for non-safe domains).
            let func = bag.as_func_value();
            let mut fp = fp64_extend_i64(fp, FCNRCDVALUE);
            let order = Value::func_tlc_order(func)
                .map_err(|e| FingerprintError::TlcNormalization(e.to_string()))?;
            fp = fp64_extend_i32(
        ...
        Value::Record(rec) => {
            ...
            // SOUNDNESS: entries are stored in canonical field order (field-name
            // string, byte-lexicographic) so this iteration matches the order an
            // equal string-keyed FuncValue fingerprints in (func_tlc_order on a
            // string domain) — semantically equal Record/Func values MUST get
            // identical fingerprints for state dedup.
            let mut fp = fp64_extend_i64(fp, FCNRCDVALUE);
