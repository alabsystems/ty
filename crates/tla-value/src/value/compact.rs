...
//! Inline scalars (Bool, integers fitting i61, interned String, ModelValue)
//! require zero heap allocation. Compound types (Set, Func, Record, etc.) are
//! stored behind a heap pointer with the tag bits cleared.
//!
...
/// # Performance
///
/// - **Bool/i61 integer**: Zero heap allocation. Construction and comparison are
///   bitwise operations.
...
impl CompactValue {
    /// Raw bit-pattern equality: `true` guarantees value equality (identical
    /// inline scalars or the same heap pointer). `false` says nothing —
    /// callers needing semantic equality must fall back to `==`. Used by
    /// cache validators to detect when a validated-equal snapshot should be
    /// refreshed to share the live allocation.
    #[inline]
    #[must_use]
