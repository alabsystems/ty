...
impl EvalError {
    /// Construct a [`EvalError::TypeError`] from an offending value.
    ///
    /// Derives the `got` field from the value's [`Value::type_name`] so callers
    /// need only state the `expected` type description.
