...
impl Value {
    ...
    /// the equivalent optimization for ty's immutable FuncValue.
    ...
    ///
    /// Part of #1434: TLC's `FcnRcdValue.normalize()` equivalent for ty.
    ...
    pub(super) fn tlc_to_tuple(v: &Value) -> EvalResult<Option<Vec<Value>>> {
        match v {
            ...
            Value::Func(_) | Value::IntFunc(_) | Value::Bag(_) => {
    ...
    pub(super) fn tlc_to_record(
        v: &Value,
    ) -> EvalResult<Option<Vec<(std::sync::Arc<str>, Value)>>> {
        // Record keys unify on the interner's `std::sync::Arc<str>` (the type
        // `iter_str` resolves names to); a `Value::String` key (`Rp<str>`) is
        // converted at this boundary.
        match v {
            Value::Record(r) => Ok(Some(r.iter_str().map(|(k, vv)| (k, vv.clone())).collect())),
            Value::Func(f) => {
                let mut entries: Vec<(std::sync::Arc<str>, Value)> =
    ...
    pub(super) fn tlc_function_dom_vals(v: &Value) -> EvalResult<Option<(Vec<Value>, Vec<Value>)>> {
        match v {
            ...
            // Bag: identical to the Func arm on the cached materialized form
            // (same entries, same order, same TLC normalization caching).
            Value::Bag(b) => {
