...
/// Cached `Value::Set(Rp::new(SortedSet::new()))` — the empty set `{}`.
...
/// Cached `Value::Seq(Rp::new(SeqValue::new()))` — the empty sequence `<<>>`.
static EMPTY_SEQ_VALUE: OnceLock<Value> = OnceLock::new();

/// Cached `Value::Tuple(Rp::from([]))` — the empty tuple.
...
pub(crate) fn cached_empty_set() -> Value {
    EMPTY_SET_VALUE
        .get_or_init(|| Value::Set(Rp::new(SortedSet::new())))
        .clone()
}
...
pub(crate) fn cached_empty_seq() -> Value {
    EMPTY_SEQ_VALUE
        .get_or_init(|| Value::Seq(Rp::new(SeqValue::new())))
        .clone()
}
