...
fn cache_int_func_state_fp(func: &IntIntervalFunc) -> u64 {
    let fp = state_value_fingerprint_unwrap(&Value::IntFunc(Rp::new(func.clone())));
    func.cache_additive_fp(fp);
    fp
}
...
fn test_func_additive_fingerprint_propagates_nested_overflow_error() {
    ...
    assert_nested_overflow(Value::Func(Rp::new(FuncValue::from_sorted_entries(vec![
...
fn test_int_func_additive_fingerprint_propagates_nested_overflow_error() {
    ...
    // concurrently. All flag-flipping tests hold the same intern-state lock.
    let _intern_lock = crate::value::lock_intern_state();
    assert_nested_overflow(Value::IntFunc(Rp::new(IntIntervalFunc::new(
        1,
        1,
...
fn test_seq_additive_fingerprint_propagates_nested_overflow_error() {
    ...
    // concurrently. All flag-flipping tests hold the same intern-state lock.
    let _intern_lock = crate::value::lock_intern_state();
    assert_nested_overflow(Value::Seq(Rp::new(crate::SeqValue::from(vec![
...
fn test_func_except_updates_cached_additive_fingerprint() {
    ...
    let outer = FuncValue::from_sorted_entries(vec![
        (
            Value::SmallInt(1),
            Value::Func(Rp::new(sample_inner_bool_func(true))),
        ),
    ...
    let updated = outer.clone().except(
        Value::SmallInt(1),
        Value::Func(Rp::new(updated_inner.clone())),
    );

    let expected = state_value_fingerprint_unwrap(&Value::Func(Rp::new(
        FuncValue::from_sorted_entries(vec![
            (Value::SmallInt(1), Value::Func(Rp::new(updated_inner))),
            (Value::SmallInt(2), Value::Bool(true)),
        ]),
    )));
...
fn test_int_func_except_updates_cached_additive_fingerprint() {
    ...
    let expected = state_value_fingerprint_unwrap(&Value::IntFunc(Rp::new(IntIntervalFunc::new(
        1,
        2,
        vec![Value::Func(Rp::new(updated_inner)), Value::Bool(true)],
    ))));
...
fn test_func_overlay_take_restore_preserves_fingerprint() {
    ...
    let base = FuncValue::from_sorted_entries(vec![
        (Value::SmallInt(1), Value::Func(Rp::new(inner))),
        (Value::SmallInt(2), Value::Bool(true)),
    ]);
