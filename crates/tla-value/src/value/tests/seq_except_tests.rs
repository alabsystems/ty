...
fn test_seq_except_if_changed_short_circuits_when_same() {
    ...
    let s = SeqValue::from_vec((0..8).map(Value::SmallInt).collect());
...
fn test_seq_except_invalidates_fingerprint_cache() {
    ...
    let _ = crate::fingerprint::value_fingerprint(&Value::Seq(Rp::new(s.clone()))).unwrap();
    ...
    let fp_orig = crate::fingerprint::value_fingerprint(&Value::Seq(Rp::new(s))).unwrap();
    let fp_result = crate::fingerprint::value_fingerprint(&Value::Seq(Rp::new(result))).unwrap();
    assert_ne!(
        fp_orig, fp_result,
