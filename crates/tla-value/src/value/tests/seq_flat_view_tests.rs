...
fn test_seqvalue_tail_fingerprint_matches_manual_slice() {
    ...
    let tail_val = Value::Seq(Rp::new(tail_result));
    let manual_val = Value::Seq(Rp::new(manual_result));
...
fn test_seqvalue_append_fingerprint_matches_manual_push() {
    ...
    // Additive (dedup) fingerprint
    let appended_val = Value::Seq(Rp::new(appended));
    let manual_val = Value::Seq(Rp::new(manual));
...
fn test_seqvalue_chained_tail_append_fingerprint() {
    ...
    let chained_fp = state_value_fingerprint(&Value::Seq(Rp::new(chained))).unwrap();
    let manual_fp = state_value_fingerprint(&Value::Seq(Rp::new(manual))).unwrap();
    assert_eq!(
