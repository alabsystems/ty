...
fn test_func_tlc_normalized_model_value_domain_correctness() {
    // tlc_cmp on model values assigns registry indices during comparison; a
    // concurrent clear_model_value_registry() (model-value test families) can
    // reassign indices mid-sort and make distinct keys compare Equal
    // ("occurs multiple times in function domain" flake). Serialize on the
    // crate-wide intern-state lock like every other registry-touching test.
    ...
    let f1 = Value::Func(Rp::new(FuncValue::from_sorted_entries(vec![
        (mv_a.clone(), Value::int(1)),
        (mv_b.clone(), Value::int(2)),
        (mv_c.clone(), Value::int(3)),
    ])));
    let f2 = Value::Func(Rp::new(FuncValue::from_sorted_entries(vec![
