...
/// Serialize on the crate-wide intern-state lock: clearing the model-value
/// registry races with every other test family that compares model values
/// (tlc_cmp assigns registry indices during comparison; a mid-comparison
/// clear+reassign can make distinct model values compare Equal).
fn lock_model_value_test_state() -> crate::value::InternStateTestGuard {
    crate::value::lock_intern_state()
}
