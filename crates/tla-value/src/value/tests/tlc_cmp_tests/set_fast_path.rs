...
fn test_set_set_cmp_homogeneous_safe_fast_path() {
    ...
    // ordering: register the "x"/"y" strings before the "a"/"b" strings so sa
    // sorts first. Hold the intern-state test lock (serializes against other
    // tests that clear the global token registry) and use TEST-UNIQUE strings:
    // generic one-letter names like "a"/"x" get tokens eagerly assigned by any
    // concurrent test that interns them, which inverted the expected order
    // (the parallel-test flake: [{"a","b"},{"x","y"}] vs [{"x","y"},{"a","b"}]).
    {
        ...
        tlc_string_token(&Arc::from("__sfp_x"));
        tlc_string_token(&Arc::from("__sfp_y"));
        tlc_string_token(&Arc::from("__sfp_a"));
        tlc_string_token(&Arc::from("__sfp_b"));
        let sa = Value::set([Value::string("__sfp_x"), Value::string("__sfp_y")]);
        let sb = Value::set([Value::string("__sfp_a"), Value::string("__sfp_b")]);
