...
fn test_tlc_normalized_string_set_token_order() {
    ...
    // Serialize against every other test that clears/freezes the global token
    // registry. The strings below are test-unique so concurrent tests that
    // merely intern strings (eager token assignment) cannot pre-assign them.
