...
impl SetPredValue {
    ...
    /// Return the precomputed single bound-variable name and its `NameId`, or
    /// `None` if the binder is a tuple pattern rather than a simple variable.
    ///
    /// Caching avoids re-interning the name on each membership check.
    ...
    /// Return the precomputed tuple bound-variable names and their `NameId`s for
    /// a destructuring binder `{<<x, y>> \in S : P}`, or `None` for a simple
    /// (non-tuple) binder.
