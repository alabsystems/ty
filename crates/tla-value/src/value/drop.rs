...
impl Drop for Value {
    // Hot fast-path inlined into every drop site; the cold iterative machinery
    // lives in `drop_value_slow` so the common leaf/shared-Arc cases (the
    // overwhelming majority in BFS model checking) avoid an out-of-line call.
    // Part of value-churn-reduction: `Value::drop` + `drop_in_place<Value>` were
    // measured at ~6-7% self-time on Disruptor_MPMC/btree, dominated by the
    // per-Value call overhead on values that the fast paths discard immediately.
    #[inline]
    fn drop(&mut self) {
        ...
        // Cold path: sole-owner compound value requiring iterative descent.
        drop_value_slow(self);
    }
}

/// Cold, out-of-line slow path for dropping a sole-owner compound `Value`.
///
/// Separated from `Drop::drop` so the hot leaf/shared-Arc fast paths inline
/// cleanly at every drop site without dragging in the TLS access and iterative
/// loop. Behavior is identical to the previous inline body.
#[inline(never)]
#[cold]
fn drop_value_slow(this: &mut Value) {
    ...
    if in_iterative {
        if let Ok(()) = DROP_STACK.try_with(|cell| {
            ...
            let owned = std::mem::replace(this, Value::Bool(false));
            stack.push(owned);
        }) {
            return;
        }
...
fn is_shared_arc(val: &Value) -> bool {
    match val {
        ...
        Value::Bag(a) => Arc::strong_count(a) > 1,
        ...
        // Record is inline (not behind a single Arc), but its `entries` are an
        // `Arc<Vec<(NameId, Value)>>`. When that Arc is shared, dropping the
        // Record only decrements the refcount — no inner Value drops fire — so
        // the iterative machinery is pure overhead, just like the Arc variants
        // above. Records are heavily used as state variables in BFS, where the
        // queued-state copy keeps the Arc shared.
        Value::Record(r) => r.is_storage_shared(),
        _ => false,
    }
}
...
fn drop_iterative(owned: Value) {
    ...
    match &*md {
        ...
        Value::Bag(arc_ref) => {
            let _arc: Arc<super::BagValue> = unsafe { take(arc_ref) };
        }
...
mod tests {
    ...
    fn test_compound_type_drops() {
        ...
        let tup = Value::Tuple(Rp::from(vec![Value::int(1), Value::int(2)]));
        drop(tup);
