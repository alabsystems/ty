...
impl SortedSet {
    ...
    /// Build a `SortedSet` that shares an existing `Arc<[Value]>` without
    /// cloning or re-allocating its elements.
    ///
    /// CALLER CONTRACT: `elements` MUST already satisfy the `Normalized`
    /// storage invariant — sorted by `Value::cmp` and free of duplicates. This
    /// is exactly the invariant held by `FuncValue::domain_arc()`, enabling an
    /// O(1) `DOMAIN f` (Part of Q7). The Arc is not run through the set-intern
    /// table, so cross-set structural sharing is skipped; this only affects
    /// memory deduplication, never element values or set semantics.
    #[inline]
    pub fn from_normalized_arc_shared(elements: Arc<[Value]>) -> Self {
        Self::from_normalized_arc(elements)
    }
    ...
    /// Check membership of a tuple represented by its elements, without
    /// allocating a temporary `Value::Tuple`.
    ///
    /// The comparison covers every function-like representation that is
    /// equal to a TLA+ tuple. As with [`Self::contains`], raw unnormalized
    /// storage is scanned directly rather than forcing normalization.
    #[inline]
    pub fn contains_tuple_elements(&self, tuple: &[Value]) -> bool {
        let contains_normalized = |elements: &[Value]| {
            elements
                .binary_search_by(|candidate| {
                    cmp_tuple_elements_with_value(tuple, candidate).reverse()
                })
                .is_ok()
        };
        match &self.storage {
            ...
            SetStorage::Unnormalized {
                ...
                || {
                    elements
                        .iter()
                        .any(|candidate| eq_tuple_elements_with_value(tuple, candidate))
                },
