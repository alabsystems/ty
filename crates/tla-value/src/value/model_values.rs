...
pub struct MVPerm {
    ...
    /// Pointer-keyed snapshot of the FULL model value registry at construction
    /// time, sorted by the str data pointer of the interned name. Production
    /// `ModelValue`s are interned (`interned_model_value`), so every occurrence
    /// of the same name shares one allocation and `apply` resolves with a
    /// binary search over a handful of entries instead of a SipHash DashMap
    /// lookup of the string — the dominant cost of symmetry canonicalization.
    ///
    /// The key `Arc<str>` is held in the tuple to keep the pointed-to
    /// allocation alive for the lifetime of this perm. Lookup misses (a model
...
impl MVPerm {
    ...
    pub fn from_func_value(perm: &FuncValue) -> Result<Self, EvalError> {
        ...
        // Snapshot the registry into a pointer-keyed table for the hot path.
        let mut ptr_map: Vec<(usize, Arc<str>, Option<Arc<str>>)> = get_model_value_registry()
            .iter()
            .map(|entry| {
                let name = Arc::clone(entry.key());
                let idx = *entry.value() as usize;
                let mapped = elems.get(idx).and_then(Clone::clone);
                (name.as_ptr() as usize, name, mapped)
            })
            .collect();
    ...
    pub fn apply(&self, name: &Arc<str>) -> Option<&Arc<str>> {
        // Fast path: pointer-identity lookup against the construction-time
        // registry snapshot (interning makes equal names pointer-equal).
        let key = name.as_ptr() as usize;
        if let Ok(i) = self.ptr_map.binary_search_by(|probe| probe.0.cmp(&key)) {
            return self.ptr_map[i].2.as_ref();
        }
        // Slow path: registered after construction or a duplicate allocation.
