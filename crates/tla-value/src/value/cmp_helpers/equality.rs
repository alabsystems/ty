...
pub(in crate::value) fn eq_same_type(lhs: &Value, rhs: &Value) -> bool {
    match (lhs, rhs) {
        ...
        // Bag-Bag equality: fingerprint fast-reject + structural compare.
        // (Bag vs Func/IntFunc pairs have different discriminants and are
        // handled in eq_cross_type.)
        (Value::Bag(a), Value::Bag(b)) => super::cross_type::eq_bag_with_bag(a, b),
