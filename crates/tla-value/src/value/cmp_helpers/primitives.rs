...
#[inline]
pub(in crate::value) fn cmp_i64_with_value(n: i64, val: &Value) -> Ordering {
    match val {
...
pub(super) fn cmp_str_with_value(s: &Arc<str>, val: &Value) -> Ordering {
    match val {
        // `s` is an interner `Arc<str>` (name) and `other` a `Value::String`
        // payload (`Rp<str>`): distinct pointer types, so compare by content.
        Value::String(other) => (**s).cmp(&**other),
        // type_order(String) = 2
...
pub(in crate::value) fn type_order(v: &Value) -> u8 {
    match v {
        ...
        | Value::AnySet
        | Value::SeqSet(_) => 7,
        // Bag is an alternate function representation — it MUST share Func's
        // type_order group so Bag/Func pairs reach the function comparison
        // paths instead of being type-order rejected (soundness: a bag must
        // compare equal to its equivalent Func).
        Value::Func(_) | Value::IntFunc(_) | Value::Bag(_) => 8,
        Value::LazyFunc(_) => 9,
        Value::Closure(_) => 10,
    }
}
