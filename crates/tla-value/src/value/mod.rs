...
//! | Int       | `Value::Int(Rp::new(BigInt))` | Arbitrary-precision fallback |
//! | STRING    | `Value::String(Rp<str>)` | |
...
//! | Function  | `Value::Func(Rp::new(FuncValue))` | Split domain/value arrays with COW EXCEPT |
//! | Function  | `Value::IntFunc(Rp::new(IntIntervalFunc))` | Array backed (int domains) |
//! | Function  | `Value::LazyFunc(Rp<LazyFuncValue>)` | Non-enumerable domains, Arc-backed |
//! | Sequence  | `Value::Seq(Rp::new(SeqValue))` | Arc-backed, O(1) clone |
//! | Record    | `Value::Record(RecordValue)` | Array-backed fields |
//! | Tuple     | `Value::Tuple(Rp<[Value]>)` | Arc-backed, O(1) clone |
//! | Model     | `Value::ModelValue(Rp<str>)` | Symmetry sets |
//! | Closure   | `Value::Closure(Rp::new(ClosureValue))` | LAMBDA expressions |
//!
...
mod small_int_bitset;
#[cfg(test)]
mod test_support;
#[cfg(test)]
pub(crate) use test_support::{lock_intern_state, InternStateTestGuard};
...
pub use intern_tables::{
    ...
    set_skip_set_interning, InterningSkipGuard,
};
...
pub enum Value {
    ...
    /// Arc-wrapped to reduce Value enum size.
    Func(Rp<FuncValue>),
    /// Compact bag (multiset): a function from elements to positive Int
    /// counts, stored as sorted elements + parallel i64 counts with a
    /// precomputed state-dedup fingerprint. Semantically EQUAL to the
    /// corresponding `Func` — eq/cmp/hash/fingerprints must agree exactly
    /// (see `bag.rs`). Fail-closed ops materialize the general Func.
    Bag(Rp<BagValue>),
