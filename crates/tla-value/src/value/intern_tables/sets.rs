...
pub(in crate::value) const MAX_INTERN_SET_SIZE: usize = 8;
...
pub(crate) fn replace_skip_set_interning(skip: bool) -> bool {
    SKIP_SET_INTERNING.with(|cell| cell.replace(skip))
}
...
pub(crate) fn intern_set_array(elements: Vec<Value>) -> Arc<[Value]> {
    ...
    crate::churn_stats::churn_count(crate::churn_stats::ChurnSite::SetInternProbe);
    ...
    if let Some(arc) = table.get(&fp) {
        if arc.len() == elements.len() && arc.iter().zip(elements.iter()).all(|(a, b)| a == b) {
            ...
            crate::churn_stats::churn_count(crate::churn_stats::ChurnSite::SetInternHit);
