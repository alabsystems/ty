...
/// RAII guard that disables set and int-function interning on the current
/// thread for its lifetime, restoring the previous flags on drop (panic-safe).
///
/// Used by symmetry canonicalization (`fingerprint_with_symmetry*`): every
/// permuted value built there is a throwaway — it exists only to be compared
/// against the current orbit minimum and (at most) to have its fingerprint
/// taken — so the intern tables' hash+DashMap cost is pure overhead and the
/// table would be polluted with values no live state references.
...
impl Default for InterningSkipGuard {
    fn default() -> Self {
        Self::new()
    }
}

impl Drop for InterningSkipGuard {
    fn drop(&mut self) {
