...
impl FuncSetValue {
    ...
    pub(crate) fn contains(&self, v: &Value) -> Option<bool> {
        ...
        if let Value::Bag(b) = v {
            return self.contains(&Value::Func(Rp::clone(b.as_func_value())));
        }
        match v {
            Value::Func(f) => {
                // Check domain equality.
                // F2 (lever L2): materialized `Value::Set` domains compare by
                // reference via domain_eq_sorted_set — no to_sorted_set()
                // clone on the once-per-state TypeOK membership hot path.
                // Other domain shapes keep the existing materializing path.
                match self.domain.as_ref() {
                    Value::Set(domain_set) => {
                    ...
                    lazy_domain => {
                        let domain_set = match lazy_domain.to_sorted_set() {
                            Some(s) => s,
                            None => {
                                if debug {
                                    eprintln!(
