...
impl IntervalValue {
    ...
    /// Borrow the inclusive lower bound of the interval.
    pub fn low(&self) -> &BigInt {
        &self.low
    }

    /// Borrow the inclusive upper bound of the interval.
...
impl Iterator for IntervalValueIter {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            ...
            IntervalValueIter::BigInt(it) => it.next().map(|n| Value::Int(Rp::new(n))),
        }
    }
}
