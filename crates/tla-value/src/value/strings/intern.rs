...
/// Core interning + size-cap logic, parameterized over the table and counter.
///
/// Extracted from `intern_string` so the cap-reset behavior is unit-testable
/// against a LOCAL table instance: the global table is shared by every
/// concurrently-running test in the process, so exact-count assertions on it
/// are inherently racy (the long-standing `string_intern_table_respects_size_cap`
/// flake).
pub(super) fn intern_string_with_cap(
    table: &DashMap<String, Arc<str>>,
    entry_count: &AtomicUsize,
    cap: usize,
    s: &str,
    on_new_entry: impl Fn(&Arc<str>),
) -> Arc<str> {
