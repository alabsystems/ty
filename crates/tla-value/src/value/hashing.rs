...
impl Hash for Value {
    fn hash<H: Hasher>(&self, state: &mut H) {
        ...
        match self {
            ...
            // string), so iteration order IS alphabetical — no re-sort needed.
            Value::Record(r) => {
                let pairs: Vec<(std::sync::Arc<str>, &Value)> = r.iter_str().collect();
                debug_assert!(
                    pairs.windows(2).all(|w| w[0].0 <= w[1].0),
                    "record entries must be sorted by field-name string"
                );
            ...
            // Bag: MUST emit the exact hash stream of the equivalent Func
            // (elements are the domain, counts are SmallInt values). A count
            // hashes as Value::SmallInt does: integer discriminant (1u8) then
            // hash_i64_as_bigint — see the SmallInt arm above.
            Value::Bag(b) => {
                for e in b.elems() {
                    e.hash(state);
                }
                for (e, c) in b.entries() {
                    e.hash(state);
                    1u8.hash(state); // SmallInt discriminant
                    hash_i64_as_bigint(c, state);
                }
            }
            Value::IntFunc(f) => {
