...
impl CompactValue {
    ...
    /// intermediate `Value`. For inline types (Bool and integers fitting i61),
    /// this avoids the 24-byte Value construction entirely. For heap types,
    /// delegates to the inner Value's PartialEq.
    ///
    ...
    pub fn matches_value(&self, other: &Value) -> bool {
        match self.tag() {
            ...
            TAG_INT => match other {
                Value::SmallInt(n) => *n == self.as_int(),
                Value::Int(n) => n.to_i64().is_some_and(|n| n == self.as_int()),
                _ => false,
            },
    ...
    /// Compare against the exact `CompactValue` representation that
    /// `CompactValue::from(other)` would produce, without allocating an
    /// intermediate heap-backed compact value.
    ///
    /// This is stricter than [`Self::matches_value`]: semantic integer
    /// equality is accepted only when both values cross the compact-storage
    /// boundary with the same tag. It is used by collision witnesses that must
    /// use the same `CompactValue` equality relation as canonical
    /// materialization.
    #[inline]
    pub fn matches_compact_encoding(&self, other: &Value) -> bool {
        if let Some(inline) = Self::try_inline_from_value(other) {
            self == &inline
        } else {
            self.is_heap() && self.as_heap_value() == other
        }
    }
}
