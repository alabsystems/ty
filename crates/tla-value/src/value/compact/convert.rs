...
impl CompactValue {
    /// Return the canonical inline encoding used by both owned and borrowed
    /// `Value` conversions, or `None` when the value must remain heap-backed.
    /// Keeping this policy in one helper prevents collision-witness comparison
    /// from drifting away from state materialization.
    #[inline]
    pub(super) fn try_inline_from_value(value: &Value) -> Option<Self> {
        match value {
            Value::Bool(value) => Some(Self::from_bool(*value)),
            Value::SmallInt(value) => Self::try_from_int(*value),
            // Some evaluator/library boundaries construct `Value::Int`
            // directly even for tiny values (notably IOUtils!atoi). Normalize
            // those at the state-storage boundary so semantically equal small
            // integers cannot acquire different compact tags.
            Value::Int(value) => value.to_i64().and_then(Self::try_from_int),
            _ => None,
        }
    }

    /// Whether converting `value` to [`CompactValue`] uses the owned
    /// heap-backed representation instead of an inline tag.
    ///
    /// This exposes the authoritative encoding boundary without allocating.
    /// Exact collision witnesses use it when the canonical payload is stored
    /// as an inline [`Value`] rather than as a boxed [`CompactValue`].
    #[inline]
    #[must_use]
    pub fn value_uses_heap_encoding(value: &Value) -> bool {
        Self::try_inline_from_value(value).is_none()
    }
}
...
///
/// Inline scalars (Bool and integers that fit in i61) are stored directly in
/// the 8-byte representation. All other types are heap-allocated.
impl From<Value> for CompactValue {
    ...
    fn from(v: Value) -> Self {
        ...
        if let Some(compact) = Self::try_inline_from_value(&v) {
            compact
        } else {
            Self::from_heap(v)
        }
...
impl From<&Value> for CompactValue {
    fn from(v: &Value) -> Self {
        Self::try_inline_from_value(v).unwrap_or_else(|| Self::from_heap(v.clone()))
    }
}
