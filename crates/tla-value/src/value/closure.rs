...
pub trait TirBody: std::any::Any + Send + Sync {
    /// Clone the body behind a fresh `Box`, enabling `Clone for Box<dyn TirBody>`.
    fn clone_box(&self) -> Box<dyn TirBody>;
    /// Upcast to `&dyn Any` so `tla-eval` can downcast to the concrete TIR type.
...
impl ClosureValue {
    ...
    /// Create a new closure from an already-`Arc`-shared AST body.
    ///
    /// Behaviorally identical to [`ClosureValue::new`] — the closure `id` is
    /// still derived from `body.span`, so fingerprints/equality are unchanged —
    /// but the caller's existing `Arc<Spanned<Expr>>` is shared directly instead
    /// of deep-cloning the `Spanned<Expr>` and re-wrapping it in a fresh `Arc`.
    ///
    /// The AST body is immutable and evaluation over it is purely functional, so
    /// sharing the Arc is observationally indistinguishable from owning a private
    /// deep copy. This removes the per-construction AST deep-clone that dominated
    /// `LAMBDA`-heavy specs (e.g. SlidingPuzzles' `ChooseOne` lambda, constructed
    /// ~once per `move`).
    pub fn new_with_arc_body(
        params: Vec<String>,
