...
use super::state::{
    ...
    PARALLEL_INTERN_ACTIVE, PARALLEL_READONLY_VALUE_CACHES_ACTIVE, PARALLEL_VALUE_INTERN_RUN_LOCK,
    WORKER_INTERN, WORKER_TOKEN_COUNTER,
};
...
///
/// Set `TY_PARALLEL_NO_PRELOAD_FROZEN=1` to disable (for A/B debugging only).
///
...
fn preload_frozen_disabled() -> bool {
    ...
    tla_core::env_flag_is_set(&FLAG, "TY_PARALLEL_NO_PRELOAD_FROZEN")
}
...
pub fn install_worker_intern_scope() {
    ...
    let set_overlay_base = set_overlay.len();
    let int_func_overlay_base = int_func_overlay.len();
...
impl ParallelValueInternRunGuard {
    ...
    pub fn new(cache_mode: SharedValueCacheMode) -> Self {
        let run_lock = PARALLEL_VALUE_INTERN_RUN_LOCK
            .lock()
