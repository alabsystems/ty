...
#[test]
fn parallel_value_intern_run_guard_serializes_overlapping_runs() {
    let _lock = crate::value::lock_intern_state();
    ParallelValueInternRunGuard::reset_for_recovery();

    let first_guard = ParallelValueInternRunGuard::new(SharedValueCacheMode::Writable);
    let (started_tx, started_rx) = std::sync::mpsc::channel();
    let (acquired_tx, acquired_rx) = std::sync::mpsc::channel();

    let second_run = std::thread::spawn(move || {
        started_tx.send(()).expect("signal second run start");
        let _second_guard = ParallelValueInternRunGuard::new(SharedValueCacheMode::Writable);
        acquired_tx.send(()).expect("signal second run acquired");
    });

    started_rx.recv().expect("second run started");
    assert!(
        acquired_rx
            .recv_timeout(std::time::Duration::from_millis(500))
            .is_err(),
        "overlapping value-intern run must wait for the active run guard"
    );

    let worker_scope = WorkerInternGuard::new();
    drop(worker_scope);
    drop(first_guard);

    acquired_rx
        .recv_timeout(std::time::Duration::from_secs(5))
        .expect("second run acquired after first guard dropped");
    second_run.join().expect("second run thread joined");
    ParallelValueInternRunGuard::reset_for_recovery();
}
