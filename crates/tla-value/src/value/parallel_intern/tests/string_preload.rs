...
fn string_preload_preserves_frozen_set_and_intfunc_sharing() {
    ...
    // stayed string-only.
    //
    // The frozen snapshot is constructed SYNTHETICALLY instead of via
    // `freeze_value_interners()`: the global set/int-func intern tables are
    // capacity-capped and shared with every concurrently-running test, so an
    // ambient clear between this test's interns and its freeze evicted the
    // entries (the "set in frozen" flake). The subject under test — worker
    // lookups hitting the frozen path with string-only preload — only reads
    // the snapshot installed in WORKER_INTERN, so no global state is needed.
    ...
    let frozen_set: Arc<[Value]> = Arc::from(set_elems.clone());
    let frozen_func: Arc<Vec<Value>> = Arc::new(func_vals.clone());
    let hello: Arc<str> = Arc::from("preload_share_hello");

    let mut sets = FxHashMap::default();
    sets.insert(set_fp, Arc::clone(&frozen_set));
    let mut int_funcs = FxHashMap::default();
    int_funcs.insert(func_fp, Arc::clone(&frozen_func));
    let mut frozen_strings = FxHashMap::default();
    frozen_strings.insert("preload_share_hello".to_string(), Arc::clone(&hello));
    let mut string_tokens = FxHashMap::default();
    string_tokens.insert(Arc::clone(&hello), 1u32);

    let frozen = Arc::new(FrozenValueInterners {
        sets,
        int_funcs,
        strings: frozen_strings,
        string_tokens,
    });
