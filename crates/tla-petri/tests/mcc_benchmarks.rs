...
//!
//! # Out-of-scope here: see `scripts/ci-soundness-watchdog.sh`
//!
//! These in-process tests cover a handful of bundled models and exercise
//! `tla_petri` API surfaces directly. They intentionally do NOT cover:
//!
//!   * the `ty-mcc` binary's stdout-parsing surface (MCC manual §7),
//!   * the official MCC 2025 corpus and its property XML files,
//!   * multi-tool gold-truth wrong-row regressions (`wrong_units > 0`),
//!   * silent soundness regressions in CTL / LTL that score gains can mask.
//!
//! Those cases are guarded by `scripts/ci-soundness-watchdog.sh`, which
//! resolves the corpus via `ty-corpus ensure --version 2025`, runs a
//! curated ~10-model sweep across all 13 examinations through
//! `ty-mcc-csv-compare`, and exits non-zero on `wrong_units > 0`. Run it
//! before any score-affecting commit:
//!
//! ```text
//! cargo build --release -p tla-petri
//! scripts/ci-soundness-watchdog.sh
//! ```
//!
//! See `docs/mcc-2026/ci-soundness-watchdog.md` for the rationale, the
//! subset rotation policy, and the operational runbook.
...
fn check_non_property_verdict(
    ...
    assert_eq!(
        records[0].formula_id, exam_key,
        "{exam_key}: GlobalProperties formula ID must be the examination name"
    );
