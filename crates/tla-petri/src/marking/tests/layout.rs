...
#[test]
fn test_reconstruct_implied_places_panics_on_non_exact_division() {
    let implied = vec![ImpliedPlace {
        place: 0,
        reconstruction: ImpliedPlaceReconstruction {
            constant: 5,
    ...
    assert!(
        result.is_err(),
        "non-exact reconstruction must fail closed in release builds"
    );
}

#[test]
fn test_reconstruct_implied_places_panics_on_underflow() {
    let implied = vec![ImpliedPlace {
        place: 0,
        reconstruction: ImpliedPlaceReconstruction {
            constant: 0,
    ...
    assert!(
        result.is_err(),
        "underflowing reconstruction must fail closed in release builds"
    );
}

#[test]
fn test_reconstruct_implied_places_panics_on_overflow() {
    let implied = vec![ImpliedPlace {
        place: 0,
        reconstruction: ImpliedPlaceReconstruction {
            constant: u64::MAX,
    ...
    assert!(
        result.is_err(),
        "overflowing reconstruction must fail closed in release builds"
    );
}
...
fn test_prepared_marking_analyze_detects_implied_places() {
    ...
    assert_eq!(prepared.packed_capacity(), 1);
}
