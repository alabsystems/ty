...
fn parse_named_sort(node: &roxmltree::Node) -> Result<Option<ColorSort>, PnmlError> {
    ...
    for child in node.children() {
        ...
        match child.tag_name().name() {
            ...
            "finiteintrange" => {
                // PNML `<finiteintrange start="N" end="M"/>` — integer-range
                // sort. Inclusive on both ends. Mirrors the intconstant
                // precedent (9f50b8ce) by surfacing missing/unparseable
                // attributes as MissingElement rather than silent default.
                let start = child
                    .attribute("start")
                    .and_then(|value| value.parse::<i64>().ok())
                    .ok_or_else(|| {
                        PnmlError::MissingElement(String::from("finiteintrange start attribute"))
                    })?;
                let end = child
                    .attribute("end")
                    .and_then(|value| value.parse::<i64>().ok())
                    .ok_or_else(|| {
                        PnmlError::MissingElement(String::from("finiteintrange end attribute"))
                    })?;
                return Ok(Some(ColorSort::FiniteIntRange {
                    id,
                    name,
                    start,
                    end,
