...
pub(crate) enum ColorSort {
    ...
    /// Finite integer range `[start, end]` (HLPNML `<finiteintrange>`).
    FiniteIntRange {
        id: String,
        name: String,
        start: i64,
...
impl ColorSort {
    pub(crate) fn id(&self) -> &str {
        match self {
            ...
            | ColorSort::Product { id, .. }
            | ColorSort::FiniteIntRange { id, .. } => id,
        }
    }
    ...
    pub(crate) fn name(&self) -> &str {
        match self {
            ...
            | ColorSort::Product { name, .. }
            | ColorSort::FiniteIntRange { name, .. } => name,
        }
    }
}
...
pub(crate) enum ColorExpr {
    ...
    All { sort_id: String, count: u64 },
    /// `<numberof>` — multiplicity `count` of a color term.
    NumberOf { count: u64, color: Box<ColorTerm> },
    /// `<add>` — multiset union.
    Add(Vec<ColorExpr>),
    /// `<subtract>` — multiset difference `lhs - rhs`.
    ///
    /// Evaluated per color value with *truncated* (monus) semantics: the
    /// result multiplicity for each color is `max(0, lhs(color) - rhs(color))`.
    /// Arc weights are non-negative, so a color the right operand removes
    /// beyond what the left operand provides contributes zero rather than a
    /// negative weight. This matches the standard CPN/ISO-HLPN "broadcast to
    /// all-but-self" pattern (`1'all - 1'(x)`), where the subtraction is exact
    /// per binding.
    Subtract {
        lhs: Box<ColorExpr>,
        rhs: Box<ColorExpr>,
...
pub(crate) enum ColorTerm {
    ...
    UserConstant(String),
    /// `<intconstant/>` or `<finiteintrangeconstant/>` — integer literal.
    IntegerConstant(i64),
    /// `<all><usersort .../></all>` inside a product tuple.
    All(String),
...
pub(crate) enum GuardExpr {
    ...
    True,
    /// `<booleanconstant value="false"/>` or bare `<false/>`. Mirror of
    /// `True`; always evaluates to `false` regardless of binding, so a
    /// transition with this guard never fires.
    False,
}
