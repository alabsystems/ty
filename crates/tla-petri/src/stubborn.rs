...
impl DependencyGraph {
    ...
    pub(crate) fn build(net: &PetriNet) -> Self {
        ...
        // Step 3: build interferes_with[t] = transitions that can disable t
        // (i.e. transitions sharing any input place with t).
    ...
    /// Transitions that can disable `t` (share an input place with it). Exposed
    /// for the per-state D1+D2 soundness verifier in the differential gate.
    #[cfg(test)]
    pub(crate) fn interferers(&self, t: TransitionIdx) -> &[TransitionIdx] {
        &self.interferes_with[t.0 as usize]
    }

    /// Producers of place `p` (transitions with `p` as an output). Exposed for
    /// the per-state D1+D2 soundness verifier in the differential gate.
    #[cfg(test)]
    pub(crate) fn producers_of(&self, p: usize) -> &[TransitionIdx] {
        &self.producers[p]
    }
}
...
/// Compute the deadlock-preserving stubborn-set CLOSURE `T_s` (Schmidt 2000),
/// including disabled members, plus the enabled set `E(s)`.
///
/// Returns `None` when `|E(s)| ≤ 1` (deadlock state or single enabled transition
/// — no reduction is possible). Otherwise returns `(in_stubborn, enabled)` where
/// `in_stubborn[t]` is true iff transition `t` is in the closure `T_s`.
///
/// `T_s` is closed under the D2 conditions:
///   * enabled `t ∈ T_s` ⇒ every transition sharing an input place with `t`
///     (a potential disabler) is in `T_s`;
///   * disabled `t ∈ T_s` ⇒ all producers of one under-marked input place (the
///     key place) are in `T_s` (a necessary set for enabling `t`).
///
/// The fired set is `T_s ∩ E(s)`; the seed is the enabled transition with the
/// smallest interference set, so D1 (a key/enabled transition ∈ `T_s`) holds.
fn deadlock_closure(
    net: &PetriNet,
    marking: &[u64],
    dep: &DependencyGraph,
) -> Option<(Vec<bool>, Vec<TransitionIdx>)> {
    let num_transitions = net.num_transitions();
    ...
    Some((in_stubborn, enabled))
}

/// Test-only: expose the full closure `T_s` (as transition indices, including
/// disabled members) for the per-state D1+D2 soundness verifier. Returns `None`
/// when no reduction is possible (`|E(s)| ≤ 1`).
#[cfg(test)]
pub(crate) fn deadlock_stubborn_closure(
    net: &PetriNet,
    marking: &[u64],
    dep: &DependencyGraph,
) -> Option<Vec<TransitionIdx>> {
    let (in_stubborn, _enabled) = deadlock_closure(net, marking, dep)?;
    Some(
        in_stubborn
            .iter()
            .enumerate()
...
fn compute_deadlock_preserving(
    ...
    let (in_stubborn, enabled) = deadlock_closure(net, marking, dep)?;
...
/// Compute the safety-preserving stubborn-set CLOSURE `T_s` (including disabled
/// members) plus the enabled set `E(s)`. Same D2 closure as deadlock-preserving,
/// additionally enforcing the VISIBILITY proviso: if any visible transition
/// enters `T_s`, ALL visible transitions are pulled in.
///
/// Returns `None` when no reduction is possible — either all transitions are
/// visible (`visible.len() >= |T|`, POR gives no benefit) or `|E(s)| <= 1`.
/// Otherwise returns `(in_stubborn, enabled)` where `in_stubborn[t]` is true iff
/// `t` is in the closure `T_s`.
///
/// The visibility proviso is what upgrades D1+D2 (deadlock preservation) to
/// SAFETY preservation: because either zero or all visible transitions are
/// fired at a state, every interleaving of the visible (observable) transitions
/// is realized in the reduced graph, so any reachable violation expressed over
/// observable places is preserved.
fn safety_closure(
    net: &PetriNet,
    marking: &[u64],
    dep: &DependencyGraph,
...
/// Test-only: expose the full safety-preserving closure `T_s` (as transition
/// indices, including disabled members) for the per-state D1+D2+visibility
/// soundness verifier in the differential gate. Returns `None` when no reduction
/// is possible (all-visible, or `|E(s)| <= 1`).
#[cfg(test)]
pub(crate) fn safety_stubborn_closure(
    net: &PetriNet,
    marking: &[u64],
    dep: &DependencyGraph,
    visible: &[TransitionIdx],
) -> Option<Vec<TransitionIdx>> {
    let (in_stubborn, _enabled) = safety_closure(net, marking, dep, visible)?;
    Some(
        in_stubborn
            .iter()
            .enumerate()
...
fn compute_safety_preserving(
    ...
    let (in_stubborn, enabled) = safety_closure(net, marking, dep, visible)?;
...
#[cfg(test)]
#[path = "stubborn_differential_proptest.rs"]
mod stubborn_differential_proptest;

#[cfg(test)]
#[path = "stubborn_safety_differential_proptest.rs"]
mod stubborn_safety_differential_proptest;
