...
// Colored 1-safety shortcut. The file lives under
// `examination_non_property/colored_shortcuts.rs` (its conceptual home) but
// is mounted here as a model-private helper because the only caller is
// `execution::collect_examination_for_model_inner`. Mounting via `#[path]`
// avoids re-export contention with the `examination_non_property` module
// surface (that module is private to `examination`, not `pub(crate)`).
#[path = "../examination_non_property/colored_shortcuts.rs"]
mod colored_shortcuts;
...
pub fn run_examination_for_model(
    ...
    render::run_examination_for_model(model, examination, config, None);
}

pub(crate) fn run_examination_for_model_with_setup_evidence(
    model: &PreparedModel,
    examination: Examination,
    config: &ExplorationConfig,
    setup_evidence: crate::mcc_backend_evidence::MccSetupEvidence,
) {
    render::run_examination_for_model(model, examination, config, Some(setup_evidence));
}
