...
pub(crate) fn tarjan_scc_generic<E>(
    ...
    tarjan_scc_slices(adj.len(), |v| adj[v].as_slice(), successor)
}

/// Tarjan's SCC over any adjacency that can hand out per-node edge slices —
/// the single core shared by the nested (`&[Vec<E>]`) and CSR (audit S5)
/// layouts, so the trusted algorithm exists exactly once.
#[must_use]
pub(crate) fn tarjan_scc_slices<'a, E: 'a>(
    n: usize,
    neighbors: impl Fn(usize) -> &'a [E],
    successor: impl Fn(&E) -> u32,
) -> Vec<Vec<u32>> {
    ...
    for v in 0..n {
        if ctx.index[v] == u32::MAX {
            strongconnect(&mut ctx, &neighbors, &successor, v as u32);
        }
    }
...
fn strongconnect<'a, E: 'a>(
    ...
    while let Some(&mut (node, ref mut edge_idx)) = call_stack.last_mut() {
        ...
        let row = neighbors(ni);
        if *edge_idx < row.len() {
            let w = successor(&row[*edge_idx]);
            *edge_idx += 1;
            let wi = w as usize;
