...
/// The effect of formula simplification on a single property.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[non_exhaustive]
pub enum SimplificationOutcome {
    /// Simplification left the formula structurally identical.
    Unchanged,
    /// The formula changed but did not collapse to a constant.
    Simplified,
    /// The formula simplified to the constant `true`.
    ResolvedTrue,
    /// The formula simplified to the constant `false`.
    ResolvedFalse,
}

/// Per-property record of a formula simplification: its outcome and the
/// before/after formula-AST node counts.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
#[non_exhaustive]
pub struct PropertySimplificationReport {
    /// The property's identifier (from the property XML).
    pub property_id: String,
    /// How simplification affected this property.
    pub outcome: SimplificationOutcome,
    /// AST node count before simplification.
    pub original_nodes: usize,
    /// AST node count after simplification.
...
/// Aggregate counts across all properties' simplification outcomes.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
#[non_exhaustive]
pub struct SimplificationSummary {
    /// Total number of properties considered.
    pub total_properties: usize,
    /// Properties whose formula changed (anything but `Unchanged`).
    pub changed_properties: usize,
    /// Properties left structurally unchanged.
    pub unchanged_properties: usize,
    /// Properties that resolved to the constant `true`.
    pub resolved_true_properties: usize,
    /// Properties that resolved to the constant `false`.
...
/// The full formula-simplification report: a per-property breakdown plus the
/// aggregate [`SimplificationSummary`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
#[non_exhaustive]
pub struct SimplificationReport {
    /// One record per property, in input order.
    pub properties: Vec<PropertySimplificationReport>,
    /// Aggregate counts over [`properties`](Self::properties).
...
fn count_ctl_nodes(formula: &CtlFormula) -> usize {
    match formula {
        ...
        | CtlFormula::EG(inner)
        | CtlFormula::AG(inner)
        | CtlFormula::EGF(inner) => 1 + count_ctl_nodes(inner),
