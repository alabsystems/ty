...
fn test_airplane_ld_pt_0010_state_space_regression() {
    ...
    assert_eq!(
        stats.states,
        tla_bignum::BigUint::from(43463u32),
        "state count mismatch"
    );
    assert_eq!(
        stats.edges,
        tla_bignum::BigUint::from(183664u32),
        "edge count mismatch"
    );
...
fn test_bridge_and_vehicles_col_state_space_fails_closed() {
    ...
    assert_eq!(
        col_records[0].formula_id, "StateSpace",
        "COL StateSpace fail-closed record should use the MCC examination name"
    );
