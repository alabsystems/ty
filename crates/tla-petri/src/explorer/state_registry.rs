...
pub(crate) enum StateAdmission {
    ...
    LimitReached,
    /// A provider supplied a fingerprint that does not match the packed state.
    FingerprintMismatch { supplied: u128, canonical: u128 },
}
...
impl GraphStateRegistry {
    ...
    pub(crate) fn admit_packed(&mut self, packed: &[u8]) -> StateAdmission {
        ...
        self.admit_precomputed(fp, packed)
    }

    /// Attempt to admit a packed marking when the caller already computed the
    /// canonical fingerprint at the successor-provider boundary.
    pub(crate) fn admit_precomputed(&mut self, fingerprint: u128, packed: &[u8]) -> StateAdmission {
        let canonical = fingerprint_marking(packed);
        if fingerprint != canonical {
            return StateAdmission::FingerprintMismatch {
        ...
        self.state_ids.insert(fingerprint, next_id);
...
mod tests {
    ...
    #[test]
    fn test_graph_state_registry_rejects_mismatched_precomputed_fingerprint() {
        let initial = [1u8, 2, 3, 4];
        let mut reg = GraphStateRegistry::with_initial(&initial, 10);

        let new_marking = [5u8, 6, 7, 8];
        let result = reg.admit_precomputed(0, &new_marking);
        match result {
            StateAdmission::FingerprintMismatch {
                ...
                assert_eq!(canonical, fingerprint_marking(&new_marking));
            }
