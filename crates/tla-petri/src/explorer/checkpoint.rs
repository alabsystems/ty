    ...
    O: ParallelExplorationObserver + CheckpointableObserver,
{
    ...
    let packed_bytes = marking_config.packed_bytes;
    ...
    if !resumed {
        ...
        let initial_admission = visited.admit_fingerprint(fingerprint_marking(&initial_packed));
    ...
    // One adaptive probe (deadline + memory): this checkpointed BFS retains a
    // packed marking per queued state and `max_states` is `usize::MAX` on the
    // auto-sized MCC path, so item caps do not bound bytes. Ticked per pop AND
    // per transition (a wide expansion cannot wait for the next pop).
    ...
    while let Some(current_packed) = queue.pop_front() {
        ...
        for &trans in &enabled_transitions {
            // Per-transition tick bounds the byte overshoot WITHIN a single
            // wide expansion (the per-pop tick alone cannot). A hit marks the
            // run incomplete (CANNOT_COMPUTE) — never a verdict. (Inner break
            // only; the outer `while` then re-tests `!completed` and stops.)
            if probe.over_budget() {
                completed = false;
                break;
            }
            // Fail-closed (#22): a token-count overflow leaves `current_tokens`
            // partially mutated — discard it (do NOT undo) and mark the run
            // incomplete so the verdict is CANNOT_COMPUTE, never a wrong answer.
            if net.apply_delta(&mut current_tokens, trans).is_err() {
                completed = false;
                break;
            }
