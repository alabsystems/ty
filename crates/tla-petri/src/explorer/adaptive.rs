...
use super::seen::{FingerprintAdmission, LocalSeenSet};
...
pub(crate) fn analyze_observer_parallelism(
    ...
    while sampled_states < pilot_limit {
        ...
        for &trans in &enabled_transitions {
            // #22: this is a pilot branching-factor sample only (it picks a
            // worker count, never a verdict). On token-count overflow the
            // marking is partially mutated, so abandon this sample without
            // undoing; the estimate degrades gracefully, soundness is untouched.
            ...
            if visited.admit_fingerprint(fp).is_new() {
