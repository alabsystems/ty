...
use crate::marking::PreparedMarking;
use crate::memory::{
    compute_max_states_for_full_graph_packed_bytes, compute_max_states_for_packed_bytes,
};
...
pub enum StorageMode {
    ...
    Disk,
    /// Fingerprint-admission BFS using a lock-free `CasFingerprintSet`.
    ///
    /// Uses a 64-bit fingerprint set for admission and retains one resident
    /// packed marking per fingerprint so duplicate admissions can be confirmed
    /// exactly under the shared fail-closed collision policy.
    FingerprintOnly,
}
...
pub struct ExplorationConfig {
    ...
    /// Auto-sizing basis used only when a later checker needs an exact
    /// full-graph representation.
    ///
    /// File/fingerprint-backed MCC runs can leave ordinary exploration
    /// unbounded while still preserving enough memory information for CTL/LTL
    /// full-graph retries, which store unpacked markings and adjacency.
    full_graph_auto_sizing: Option<AutoSizingBasis>,
    ...
    pub(crate) storage_primary_capacity: Option<usize>,
    ...
    /// MCC examination this exploration serves, when known.
    ///
    /// Used by the symmetry-reduction soundness guard: place-swap
    /// canonicalization is only sound for σ-invariant examinations. If `None`,
    /// the explorer defaults to refusing canonicalization even when
    /// `TY_AUTO_SYMMETRY=1` is set, since it cannot verify soundness.
    examination: Option<Examination>,
}
...
impl ExplorationConfig {
    ...
    /// Set the examination this exploration serves.
    ///
    /// Required to enable place-swap canonicalization under
    /// `TY_AUTO_SYMMETRY=1`: the explorer refuses the optimization for
    /// examinations whose property predicate is not σ-invariant on the
    /// discovered place orbits. See
    /// `docs/theorems/2026-05-26-place-swap-symmetry-soundness.md` §2.5.
    #[must_use]
    pub fn with_examination(mut self, examination: Option<Examination>) -> Self {
        self.examination = examination;
        self
    }

    /// Return the examination this exploration is serving, if known.
    #[must_use]
    pub fn examination(&self) -> Option<Examination> {
        self.examination
    }
    ...
    pub fn with_fingerprint_dedup(mut self, enabled: bool) -> Self {
        ...
        if let Some(basis) = self.auto_sizing {
            self.max_states =
                compute_max_states_for_packed_bytes(basis.packed_bytes, basis.fraction, enabled);
        }
        self
    }
    ...
    /// Preserve memory-derived sizing information for future full-graph
    /// exploration without changing the current `max_states` budget.
    #[must_use]
    pub(crate) fn with_full_graph_auto_sizing(
        mut self,
        net: &PetriNet,
        memory_fraction: Option<f64>,
    ) -> Self {
        let prepared = PreparedMarking::analyze(net);
        self.full_graph_auto_sizing = Some(AutoSizingBasis {
            fraction: memory_fraction.unwrap_or(DEFAULT_MEMORY_FRACTION),
            packed_bytes: prepared.config.packed_bytes,
        });
        self
    }

    /// Read the configured primary-tier capacity for mmap/disk storage, if set.
    #[must_use]
    pub fn storage_primary_capacity(&self) -> Option<usize> {
        self.storage_primary_capacity
    }
    ...
    /// Remove checkpoint/resume settings from this exploration.
    #[must_use]
    pub(crate) fn without_checkpoint(mut self) -> Self {
    ...
    pub fn auto_sized(
        ...
        let packed_bytes = prepared.config.packed_bytes;
        let max_states = compute_max_states_for_packed_bytes(packed_bytes, fraction, true);
        Self {
            max_states,
            deadline,
            workers: 1,
            ...
            full_graph_auto_sizing: Some(AutoSizingBasis {
                fraction,
                packed_bytes,
            }),
            fingerprint_dedup: true,
    ...
    pub fn refitted_for_net(&self, net: &PetriNet) -> Self {
        ...
        let prepared = PreparedMarking::analyze(net);
        let packed_bytes = prepared.config.packed_bytes;
        let new_max = compute_max_states_for_packed_bytes(
            packed_bytes,
            basis.fraction,
            self.fingerprint_dedup,
        );
    ...
    pub(crate) fn refitted_for_full_graph(&self, net: &PetriNet) -> Self {
        let Some(basis) = self.auto_sizing.or(self.full_graph_auto_sizing) else {
            return self.clone();
        };
        let prepared = PreparedMarking::analyze(net);
        let packed_bytes = prepared.config.packed_bytes;
        let new_max = compute_max_states_for_full_graph_packed_bytes(
            net.num_places(),
            packed_bytes,
            basis.fraction,
        );
        Self {
            ...
            workers: self.workers,
            por_strategy: self.por_strategy.clone(),
            auto_sizing: self.auto_sizing.map(|basis| AutoSizingBasis {
                fraction: basis.fraction,
                packed_bytes,
            }),
            full_graph_auto_sizing: Some(AutoSizingBasis {
...
pub struct AutoSizeInfo {
    ...
    /// Exact number of bytes used by one packed marking.
...
pub(crate) trait ExplorationObserver {
    ...
    /// Called when a new unique state is discovered, passing the orbit size.
    fn on_new_state_with_orbit(&mut self, marking: &[u64], _orbit_size: u64) -> bool {
        self.on_new_state(marking)
    }
    ...
    /// Called when a transition fires successfully, with the source marking
    /// from which the transition fired.
    ///
    /// Required by orbit-quotient observers (e.g. StateSpace under
    /// place-swap canonicalization) that need to multiply each edge
    /// observation by the orbit size |G·m| of the source marking. The
    /// default implementation discards the source and delegates to
    /// [`Self::on_transition_fire`], so existing observers keep working
    /// without modification.
    fn on_transition_fire_with_source(&mut self, _source: &[u64], trans: TransitionIdx) -> bool {
        self.on_transition_fire(trans)
    }

    /// Called when a transition fires successfully, with the source marking and its orbit size.
    fn on_transition_fire_with_orbit(
        ...
        self.on_transition_fire_with_source(source, trans)
    }

    /// Called when a deadlock state is found (no enabled transitions).
    fn on_deadlock(&mut self, marking: &[u64]);

    /// Signal that an orbit-size computation overflowed and could not be
    /// represented exactly. Orbit-quotient observers (e.g. StateSpace under
    /// place-swap canonicalization) MUST fail closed here — mark themselves
    /// done so the exploration is reported incomplete (CANNOT_COMPUTE) instead
    /// of returning a truncated wrong count. Default is a no-op for observers
    /// that never multiply by an orbit size.
    fn on_orbit_overflow(&mut self) {}

    /// Check if exploration should stop early.
    fn is_done(&self) -> bool;

    /// Whether it is sound to apply place-swap canonicalization to *this*
    /// observer's exploration, given its runtime configuration.
    ///
    /// The examination-level guard
    /// [`symmetry_guard::canonicalization_is_sound`](super::symmetry_guard::canonicalization_is_sound)
    /// classifies whole examinations. This finer per-observer hook adds a second
    /// fail-closed gate for observers whose σ-invariance depends on runtime data
    /// the examination kind alone cannot express. The canonical example is
    /// `OneSafeObserver`: the plain PT predicate `∃p. m[p] > 1` is σ-invariant
    /// under any place permutation, but the colored-net path sums token counts
    /// over a *fixed* set of unfolded place indices per colored place, and a
    /// place-orbit permutation crossing colored-group boundaries corrupts those
    /// group sums. Returning `false` forces canonicalization to be skipped so
    /// exact reachability is used. The explorer consults this at every
    /// canonicalizer-construction site, so an unsafe observer can never be paired
    /// with a canonicalizing search.
    ///
    /// Default: `true` (canonicalization-neutral observer).
...
pub(crate) trait ParallelExplorationSummary: Send {
    ...
    /// Record a newly admitted unique state with the size of its place-symmetry
    /// orbit. Orbit-quotient summaries (e.g. StateSpace under place-swap
    /// canonicalization) override this to add `orbit_size` instead of `1`,
    /// recovering the true reachable-marking count from the quotient. The
    /// default defers to [`Self::on_new_state`] (i.e. `+1`), so non-symmetric
    /// summaries are unaffected.
    fn on_new_state_with_orbit(&mut self, marking: &[u64], _orbit_size: u64) {
        self.on_new_state(marking);
    }
    ...
    /// Record a transition firing from an explored state with the size of the
    /// *source* marking's place-symmetry orbit. Orbit-quotient summaries
    /// override this to add `orbit_size` instead of `1`, recovering the true
    /// edge count (`Σ_reps |orbit(rep)|·deg(rep)`). The default defers to
    /// [`Self::on_transition_fire`] (i.e. `+1`).
    fn on_transition_fire_with_orbit(&mut self, trans: TransitionIdx, _orbit_size: u64) {
        self.on_transition_fire(trans);
    }

    /// Signal that an orbit-size computation overflowed and the count cannot be
    /// represented exactly. Orbit-quotient summaries MUST fail closed (request
    /// stop / mark overflowed) so the merged result is CANNOT_COMPUTE rather
    /// than a truncated wrong number. Default no-op.
    fn on_orbit_overflow(&mut self) {}
