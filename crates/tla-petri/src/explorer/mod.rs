...
mod observer;
#[allow(dead_code)]
pub(crate) mod orbit_size;
mod seen;
mod setup;
mod state_registry;
mod successors;
pub(crate) mod symmetry;
pub(crate) mod symmetry_guard;
...
// `MarkingStore`'s named constructors (`from_unpacked`, `unpack_all`, …) are
// test-only; production code reaches the type through `FullReachabilityGraph`.
#[cfg(test)]
pub(crate) use graph::MarkingStore;
#[cfg(test)]
