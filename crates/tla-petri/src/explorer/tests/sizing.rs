...
fn test_refitted_for_net_noop_with_fingerprint_dedup() {
    ...
    // Refitting to the SAME net is a no-op.
    let refitted_same = config.refitted_for_net(&net);
    assert_eq!(
        refitted_same.max_states(),
        original_max,
        "refitting to the same net must not change max_states"
    );

    // Fingerprint sizing is byte-honest since the 2026-07-02 audit: the
    // estimate charges 32 B PLUS the packed marking each explorer actually
    // retains per admitted state (collision-guard arena / queue), so
    // refitting to a REDUCED (same-or-narrower) net can only allow the same
    // or more states — never fewer.
    let reduced = reduce_iterative(&net).expect("reduction should succeed");
    let refitted = config.refitted_for_net(&reduced.net);
    assert!(
        refitted.max_states() >= original_max,
        "a same-or-narrower reduced net must not shrink the state budget \
         (refitted {} < original {})",
        refitted.max_states(),
        original_max
    );
...
#[test]
fn test_refitted_for_full_graph_tightens_unbounded_fingerprint_auto_budget() {
    let net = PetriNet {
        name: Some("full-graph-fingerprint-refit".into()),
        places: (0..200)
            .map(|i| PlaceInfo {
                id: format!("P{i}"),
                name: None,
    ...
    let config = ExplorationConfig::new(usize::MAX)
        ...
        .with_storage_primary_capacity(1000)
        .with_full_graph_auto_sizing(&net, Some(0.01));

    assert_eq!(
        config.refitted_for_net(&net).max_states(),
        usize::MAX,
        "ordinary fingerprint/disk exploration should remain unbounded"
    );

    let refitted = config.refitted_for_full_graph(&net);
    assert!(
        refitted.max_states() < usize::MAX,
        "full-graph refit should recover a memory-derived cap from the stored sizing basis"
    );
}
