...
pub(crate) fn parse_pnml(content: &str) -> Result<PetriNet, PnmlError> {
    ...
    // Merge parallel arcs (duplicate input/output arcs between the same place and
    // a transition are summed) so the explicit `is_enabled` agrees with
    // `apply_delta` and the symbolic DD/MDD lowering, which already sum them —
    // closing a duplicate-arc wrong-verdict divergence. No-op on the standard
    // one-arc-per-pair MCC corpus (behavior-preserving there).
    net.canonicalize_parallel_arcs();
    // Fail-closed (#22): decline a malformed/oversized net whose token counts
    // could overflow u64 on a single firing, before any exploration allocates.
    net.validate_token_bounds()?;
    Ok(net)
}
