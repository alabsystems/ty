...
//! Thin shim around [`tla_petri::mccctl_cmd::sweep`].
//!
//! All CLI parsing, sweep orchestration, and unit tests live in the
//! library module so `ty-mccctl sweep` can invoke the same entry
//! point in-process without a subprocess hop.

use std::process::ExitCode;

fn main() -> ExitCode {
    tla_petri::mccctl_cmd::sweep::run()
}
