...
pub(crate) enum CtlFormula {
    ...
    /// EGF(φ) — E(GF φ): some path visits φ infinitely often.
    ///
    /// The fair-cycle carrier for LTL persistence: `A(FG p) ≡ ¬EGF(¬p)`. It is
    /// genuinely non-CTL and rides the GPU retained-graph CTL engine's
    /// `CtlOp::EGF` (and the CPU `CtlEngine::gfp_egf`), both with the
    /// deadlock-stutter successor.
    EGF(Box<CtlFormula>),
}
