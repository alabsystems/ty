...
#[test]
fn test_reject_duplicate_property_id_values() {
    let xml = r#"<?xml version="1.0"?>
<property-set xmlns="http://mcc.lip6.fr/">
  <property>
    <id>Model-UB-00</id>
  ...
  <property>
    ...
    let err = parse_properties_xml(xml).expect_err("duplicate property id values should fail");
    let msg = err.to_string();
    assert!(
        msg.contains("duplicate property id"),
        "error should mention duplicate id value: {msg}"
    );
}

#[test]
fn test_reject_property_id_with_internal_whitespace() {
    let xml = r#"<?xml version="1.0"?>
<property-set xmlns="http://mcc.lip6.fr/">
  <property>
    <id>Model UB 00</id>
    ...
    let err = parse_properties_xml(xml).expect_err("property id with whitespace should fail");
    let msg = err.to_string();
    assert!(
        msg.contains("whitespace or control character"),
        "error should mention invalid id characters: {msg}"
    );
}
