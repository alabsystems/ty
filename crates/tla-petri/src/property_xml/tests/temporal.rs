...
use crate::property_xml::{
    parse_properties, parse_property_ids, CtlFormula, Formula, IntExpr, LtlFormula, StatePredicate,
};
...
#[test]
fn test_parse_property_ids_ignores_formula_parse_errors() {
...
<property-set xmlns="http://mcc.lip6.fr/">
  ...
  <property>
    ...
    .expect("exam xml should write");

    let ids = parse_property_ids(tempdir.path(), "CTLFireability")
        .expect("ids should parse without formula support");
    assert_eq!(
        ids,
        vec!["Model-CTLFireability-00", "Model-CTLFireability-01"]
    );
}

#[test]
fn test_parse_property_ids_rejects_duplicate_and_whitespace_ids() {
    let tempdir = TempDir::new().expect("tempdir should be created");
    fs::write(
        tempdir.path().join("CTLFireability.xml"),
        r#"<?xml version="1.0"?>
<property-set xmlns="http://mcc.lip6.fr/">
  ...
  <property>
    ...
    let err = parse_property_ids(tempdir.path(), "CTLFireability")
        .expect_err("fallback id parser should reject whitespace in ids");
    assert!(
        err.to_string().contains("whitespace or control character"),
...
<property-set xmlns="http://mcc.lip6.fr/">
  ...
  <property>
    ...
    let err = parse_property_ids(tempdir.path(), "CTLFireability")
        .expect_err("fallback id parser should reject duplicate ids");
    assert!(
        err.to_string().contains("duplicate property id"),
        "unexpected error: {err}"
    );
}
