    ...
    F: FnMut(&roxmltree::Node<'a, 'input>) -> Result<Formula, PnmlError>,
{
    ...
    for prop_node in root.children().filter(|n| n.has_tag_name("property")) {
        ...
        let id = parse_property_id(&id_node, &mut seen_ids)?;
...
pub(super) fn parse_property_id(
    id_node: &roxmltree::Node<'_, '_>,
    ...
    let id = id_node
        ...
        .to_string();
    if id.is_empty() {
        return Err(PnmlError::MissingElement("property/id".into()));
    }
    if id
        .chars()
    ...
    {
        return Err(PnmlError::MissingElement(format!(
            "property/id contains whitespace or control character: {id:?}"
        )));
    }
    if !seen_ids.insert(id.clone()) {
        return Err(PnmlError::MissingElement(format!(
            "property-set: duplicate property id {id}"
        )));
    }
    Ok(id)
}
