...
fn mutually_exclusive_input_net() -> PetriNet {
    PetriNet {
        name: Some("mutually-exclusive-input".into()),
        places: vec![place("p0"), place("p1")],
        transitions: vec![
...
#[test]
fn test_formula_simplify_leaves_fireability_lp_false_to_reachability_phase() {
    // Each input place can individually hold one token, so structural
    // disabled-transition pruning cannot drop t_both. The LP conjunction
    // p0 >= 1 AND p1 >= 1 is still infeasible because p0 + p1 = 1.
    let net = mutually_exclusive_input_net();
    let aliases = PropertyAliases::identity(&net);
    let facts = compute_facts(&net);
    assert!(
        !facts.structurally_disabled[2],
        "test requires LP joint-input proof, not structural pruning"
    );

    let pred = StatePredicate::IsFireable(vec!["t_both".to_string()]);
    let result = simplify_predicate(&pred, &net, &facts, &aliases);
    assert!(matches!(result, StatePredicate::IsFireable(_)));
}

#[test]
fn test_formula_simplify_leaves_fireability_case_split_to_reachability_phase() {
    // In the mutex net, neither transition is always enabled, but the case
    // p_free <= 0 AND p_cs <= 0 is LP-infeasible, so one transition is always
    // fireable.
    let net = mutex_net();
    let aliases = PropertyAliases::identity(&net);
    let facts = compute_facts(&net);

    let pred = StatePredicate::IsFireable(vec!["t_enter".to_string(), "t_exit".to_string()]);
    let result = simplify_predicate(&pred, &net, &facts, &aliases);
    assert!(matches!(result, StatePredicate::IsFireable(_)));
}
...
#[test]
fn test_ctl_true_until_folds_to_temporal_shorthand() {
    let net = mutex_net();
    let aliases = PropertyAliases::identity(&net);
    let facts = compute_facts(&net);
    ...
    let eu = Formula::Ctl(CtlFormula::EU(
        Box::new(CtlFormula::Atom(StatePredicate::True)),
        Box::new(atom.clone()),
    ));
    let au = Formula::Ctl(CtlFormula::AU(
        Box::new(CtlFormula::Atom(StatePredicate::True)),
        Box::new(atom),
    ));

    match simplify_formula(&eu, &net, &facts, &aliases) {
        Formula::Ctl(CtlFormula::EF(_)) => {}
        other => panic!("expected EU(true, p) to fold to EF(p), got: {other:?}"),
    }
    match simplify_formula(&au, &net, &facts, &aliases) {
        Formula::Ctl(CtlFormula::AF(_)) => {}
        other => panic!("expected AU(true, p) to fold to AF(p), got: {other:?}"),
    }
}
...
#[test]
fn test_ltl_next_constant_folds() {
    let net = mutex_net();
    let aliases = PropertyAliases::identity(&net);
    let facts = compute_facts(&net);

    let formula = Formula::Ltl(LtlFormula::Next(Box::new(LtlFormula::Atom(
        StatePredicate::False,
    ))));
    ...
    match result {
        Formula::Ltl(LtlFormula::Atom(StatePredicate::False)) => {}
        other => panic!("expected X(false) to fold to false, got: {other:?}"),
    }
}

#[test]
fn test_ltl_idempotent_eventually_and_globally_fold() {
    let net = mutex_net();
    let aliases = PropertyAliases::identity(&net);
    let facts = compute_facts(&net);

    let atom = LtlFormula::Atom(StatePredicate::IsFireable(vec!["t_enter".to_string()]));
    let eventually = Formula::Ltl(LtlFormula::Finally(Box::new(LtlFormula::Finally(
        Box::new(atom.clone()),
    ))));
    let globally = Formula::Ltl(LtlFormula::Globally(Box::new(LtlFormula::Globally(
        Box::new(atom),
    ))));

    match simplify_formula(&eventually, &net, &facts, &aliases) {
        Formula::Ltl(LtlFormula::Finally(inner)) => {
            assert!(!matches!(*inner, LtlFormula::Finally(_)));
        }
        other => panic!("expected F(F(p)) to fold to F(p), got: {other:?}"),
    }
    match simplify_formula(&globally, &net, &facts, &aliases) {
        Formula::Ltl(LtlFormula::Globally(inner)) => {
            assert!(!matches!(*inner, LtlFormula::Globally(_)));
        }
        other => panic!("expected G(G(p)) to fold to G(p), got: {other:?}"),
    }
}

#[test]
fn test_ltl_boolean_absorption_folds_and_or_superset() {
    let net = mutex_net();
    let aliases = PropertyAliases::identity(&net);
    let facts = compute_facts(&net);

    let p = LtlFormula::Atom(StatePredicate::IsFireable(vec!["t_enter".to_string()]));
    let q = LtlFormula::Finally(Box::new(LtlFormula::Atom(StatePredicate::IsFireable(
        vec!["t_exit".to_string()],
    ))));

    let formula = Formula::Ltl(LtlFormula::And(vec![
        p.clone(),
        LtlFormula::Or(vec![p.clone(), q]),
    ]));

    match simplify_formula(&formula, &net, &facts, &aliases) {
        Formula::Ltl(result) => assert_eq!(result, p),
        other => panic!("expected p && (p || q) to fold to p, got: {other:?}"),
    }
}

#[test]
fn test_ltl_boolean_absorption_folds_or_and_superset() {
    let net = mutex_net();
    let aliases = PropertyAliases::identity(&net);
    let facts = compute_facts(&net);
    ...
    let formula = Formula::Ltl(LtlFormula::Or(vec![
        p.clone(),
        LtlFormula::And(vec![p.clone(), q]),
    ]));
...
#[test]
fn test_ltl_persistence_tail_normalizes_airplane_shape() {
    let net = mutex_net();
    let aliases = PropertyAliases::identity(&net);
    let facts = compute_facts(&net);

    let a = LtlFormula::Atom(StatePredicate::IsFireable(vec!["t_enter".to_string()]));
    let b = LtlFormula::Atom(StatePredicate::IsFireable(vec!["t_exit".to_string()]));
    let c = LtlFormula::Atom(StatePredicate::IsFireable(vec!["t_enter".to_string()]));
    let formula = Formula::Ltl(LtlFormula::Next(Box::new(LtlFormula::Finally(Box::new(
        LtlFormula::Globally(Box::new(LtlFormula::And(vec![
            LtlFormula::Finally(Box::new(a.clone())),
            LtlFormula::Globally(Box::new(LtlFormula::Until(
                Box::new(LtlFormula::Finally(Box::new(b))),
    ...
    let expected = LtlFormula::And(vec![
        LtlFormula::Globally(Box::new(LtlFormula::Finally(Box::new(a)))),
        LtlFormula::Finally(Box::new(LtlFormula::Globally(Box::new(c)))),
    ]);

    match simplify_formula(&formula, &net, &facts, &aliases) {
        Formula::Ltl(result) => assert_eq!(result, expected),
        other => panic!("expected Airplane persistence tail normalization, got: {other:?}"),
    }
}

#[test]
fn test_ltl_persistence_tail_does_not_rewrite_bare_globally_until() {
    let net = mutex_net();
    let aliases = PropertyAliases::identity(&net);
    let facts = compute_facts(&net);

    let p = LtlFormula::Atom(StatePredicate::IsFireable(vec!["t_enter".to_string()]));
    let q = LtlFormula::Atom(StatePredicate::IsFireable(vec!["t_exit".to_string()]));
    let formula = Formula::Ltl(LtlFormula::Globally(Box::new(LtlFormula::Until(
        Box::new(p),
        Box::new(LtlFormula::Globally(Box::new(q))),
    ))));

    match simplify_formula(&formula, &net, &facts, &aliases) {
        Formula::Ltl(LtlFormula::Globally(inner)) => {
            assert!(matches!(*inner, LtlFormula::Until(_, _)));
        }
        other => panic!("bare G(p U G q) must not rewrite as an FG tail, got: {other:?}"),
    }
}

#[test]
fn test_ltl_true_until_folds_to_eventually() {
    let net = mutex_net();
    let aliases = PropertyAliases::identity(&net);
    let facts = compute_facts(&net);

    let atom = LtlFormula::Atom(StatePredicate::IsFireable(vec!["t_enter".to_string()]));
    let formula = Formula::Ltl(LtlFormula::Until(
        Box::new(LtlFormula::Atom(StatePredicate::True)),
        Box::new(atom),
    ));

    match simplify_formula(&formula, &net, &facts, &aliases) {
        Formula::Ltl(LtlFormula::Finally(_)) => {}
        other => panic!("expected true U p to fold to F(p), got: {other:?}"),
    }
}
...
#[test]
fn test_ltl_airplane_ld_07_normalization() {
    let net = mutex_net();
    let aliases = PropertyAliases::identity(&net);
    let facts = compute_facts(&net);
    ...
    // Should simplify to G F a && F G c
    let expected = LtlFormula::And(vec![
        LtlFormula::Globally(Box::new(LtlFormula::Finally(Box::new(a)))),
        LtlFormula::Finally(Box::new(LtlFormula::Globally(Box::new(c)))),
    ]);
    ...
    match simplify_formula(&formula, &net, &facts, &aliases) {
        Formula::Ltl(result) => assert_eq!(result, expected),
        other => panic!("expected G F a && F G c, got: {other:?}"),
    }
}

#[test]
fn test_ltl_distribute_globally_over_and() {
    let net = mutex_net();
    let aliases = PropertyAliases::identity(&net);
    let facts = compute_facts(&net);
    ...
    // G(a && b) -> G a && G b
    let formula = Formula::Ltl(LtlFormula::Globally(Box::new(LtlFormula::And(vec![
        a.clone(),
        b.clone(),
    ]))));

    let expected = LtlFormula::And(vec![
        LtlFormula::Globally(Box::new(a)),
        LtlFormula::Globally(Box::new(b)),
    ]);

    match simplify_formula(&formula, &net, &facts, &aliases) {
        Formula::Ltl(result) => assert_eq!(result, expected),
        other => panic!("expected G a && G b, got: {other:?}"),
    }
}

#[test]
fn test_ltl_distribute_finally_over_and_of_globally() {
    let net = mutex_net();
    let aliases = PropertyAliases::identity(&net);
    let facts = compute_facts(&net);
    ...
    let formula = Formula::Ltl(LtlFormula::Finally(Box::new(LtlFormula::And(vec![
        LtlFormula::Globally(Box::new(a.clone())),
        LtlFormula::Globally(Box::new(b.clone())),
    ]))));

    let expected = LtlFormula::And(vec![
        LtlFormula::Finally(Box::new(LtlFormula::Globally(Box::new(a)))),
        LtlFormula::Finally(Box::new(LtlFormula::Globally(Box::new(b)))),
    ]);

    match simplify_formula(&formula, &net, &facts, &aliases) {
        Formula::Ltl(result) => assert_eq!(result, expected),
        other => panic!("expected F G a && F G b, got: {other:?}"),
    }
}
