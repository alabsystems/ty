        ...
        if let Some(derived) = derive_bound(inv, a) {
            if derived.place == b.place {
                match (derived.kind, b.kind) {
                    ...
                    (BoundKind::Upper, BoundKind::Upper) if derived.bound <= b.bound => {
                        return true;
                    }
                    // derived: q >= D, target: q >= B → implies if D >= B
                    (BoundKind::Lower, BoundKind::Lower) if derived.bound >= b.bound => {
                        return true;
                    }
                    _ => {}
                }
        ...
        if let Some(derived) = derive_bound(inv, a) {
            if derived.place == b.place {
                match (derived.kind, b.kind) {
                    ...
                    (BoundKind::Upper, BoundKind::Lower) if derived.bound < b.bound => {
                        return true;
                    }
                    // derived: q >= D, target: q <= B → contradicts if D > B
                    (BoundKind::Lower, BoundKind::Upper) if derived.bound > b.bound => {
                        return true;
                    }
                    _ => {}
                }
