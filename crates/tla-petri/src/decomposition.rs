...
fn find_interface_places(
    ...
    for (t_idx, transition) in net.transitions.iter().enumerate() {
        ...
        if !component_transitions.contains(&t_id) {
...
/// A human-readable structural decomposition report: how many loosely-coupled
/// components the net splits into (hypergraph min-cut), their sizes and interface
/// widths, and the technique each would route to.
///
/// DIAGNOSTIC ONLY — this never affects a verdict. Verifying components
/// independently and composing results (assume-guarantee) is future work that
/// needs a compose layer; until then this surfaces WHETHER a net is decomposable
/// (and how), which is exactly the signal that work needs. Exposed so the
/// examination entry can log it behind an env flag, giving the built-but-unwired
/// decomposition analysis a sound production caller.
pub(crate) fn decomposition_report(net: &PetriNet, min_component_size: usize) -> String {
    let components = decompose(net, min_component_size);
    if components.len() <= 1 {
        return format!(
            "decomposition: net is monolithic ({} places; not splittable at min_size={min_component_size})",
            net.num_places(),
        );
    }
    let routed = route_techniques(net, &components);
    let mut out = format!(
        "decomposition: {} loosely-coupled components (of {} places):",
        components.len(),
        net.num_places(),
    );
    for (i, (comp, tech)) in routed.iter().enumerate() {
        out.push_str(&format!(
            "\n  component {i}: {} places, {} transitions, {} interface → {tech:?}",
            comp.places.len(),
            comp.transitions.len(),
            comp.interface_places.len(),
        ));
    }
    out
}
