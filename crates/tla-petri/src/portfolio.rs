...
/// Frontend-neutral status for evidence that may be exact or still unknown.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum ExactOrUnknownStatus {
    Exact,
    Unknown,
}

impl ExactOrUnknownStatus {
    ...
    pub(crate) fn validation_status_code(self) -> &'static str {
        match self {
            Self::Exact => "validated",
            Self::Unknown => "validation_unknown",
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn exact_or_unknown_status_codes_are_shared_vocabulary() {
        assert_eq!(ExactOrUnknownStatus::Exact.code(), "exact");
        assert_eq!(
            ExactOrUnknownStatus::Exact.validation_status_code(),
            "validated"
        );
        assert_eq!(ExactOrUnknownStatus::Unknown.code(), "unknown");
        assert_eq!(
            ExactOrUnknownStatus::Unknown.validation_status_code(),
            "validation_unknown"
        );
    }
}
