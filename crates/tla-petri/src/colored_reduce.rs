...
pub(crate) fn collapse_constant_places(net: &mut ColoredNet, report: &mut ColorReductionReport) {
    ...
    let sort_cards: HashMap<String, usize> = net
        ...
        .filter_map(|s| match s {
            ...
            ColorSort::FiniteIntRange { id, start, end, .. } => {
                if end >= start {
                    let span = (*end as i128) - (*start as i128) + 1;
                    usize::try_from(span).ok().map(|n| (id.clone(), n))
                } else {
                    None
                }
            }
...
fn is_uniform_inscription(expr: &ColorExpr) -> bool {
    match expr {
        ...
        // A multiset difference cancels a specific color per binding, so it is
        // never uniform across all colors. Reporting `false` keeps any place
        // touched by a `Subtract` arc out of the Dot-collapse, preserving its
        // per-color structure (sound — at worst leaves a place un-collapsed).
        ColorExpr::Subtract { .. } => false,
    }
}
...
///
/// Only called on expressions already classified uniform, so a `Subtract`
/// (never uniform) is unreachable here; the conservative truncated count keeps
/// the function total without affecting any collapse decision.
fn per_color_count(expr: &ColorExpr) -> u64 {
    match expr {
        ColorExpr::All { count, .. } => *count,
        ColorExpr::NumberOf { count, .. } => *count,
        ColorExpr::Add(children) => children.iter().map(per_color_count).sum(),
        ColorExpr::Subtract { lhs, rhs } => {
            per_color_count(lhs).saturating_sub(per_color_count(rhs))
        }
    }
}
...
/// Rewrite `All { sort_id, count }` inscriptions to `NumberOf { count, DotConstant }`.
fn rewrite_all_to_dot(expr: &mut ColorExpr) {
    match expr {
        ...
        ColorExpr::Subtract { lhs, rhs } => {
            rewrite_all_to_dot(lhs);
            rewrite_all_to_dot(rhs);
        }
