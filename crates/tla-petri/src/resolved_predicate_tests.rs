...
use super::{
    ...
    remap_int_expr, remap_predicate, remap_predicate_scaled, resolve_int_expr,
    resolve_int_expr_with_aliases, resolve_place_bound, resolve_predicate,
    resolve_predicate_with_aliases, ResolvedIntExpr, ResolvedPredicate,
};
...
// --- Scale-aware slice remap (`remap_predicate_scaled`) ----------------------
//
// In a GCD-scaled slice a surviving place `p` holds `m_orig / g` where
// `g = place_scales[p]`. The slice path evaluates the remapped predicate
// directly on the scaled marking, so the remap must fold the scale into the
// comparison exactly. These tests pin that folding (FINDING #8 regression).

/// Identity place/trans maps for an `n`-place predicate on a same-size slice.
fn identity_place_map(n: usize) -> Vec<Option<PlaceIdx>> {
    (0..n).map(|i| Some(PlaceIdx(i as u32))).collect()
}

#[test]
fn scaled_remap_tokens_ge_const_is_exact_for_non_multiple_threshold() {
    // Original query: tokens(p0) >= 5, i.e. IntLe(Constant(5), tokens(p0)).
    // Place p0 is GCD-scaled by g=2: slice holds m_orig/2.
    // Exact rewrite: 5 <= 2*S  <=>  ceil(5/2)=3 <= S.
    let pred = ResolvedPredicate::IntLe(
        ResolvedIntExpr::Constant(5),
        ResolvedIntExpr::TokensCount(vec![PlaceIdx(0)]),
    );
    let remapped =
        remap_predicate_scaled(&pred, &identity_place_map(1), &[], &[2]).expect("remap must hold");

    // Cross-check against the original predicate on the unscaled marking for
    // every slice count S: slice S corresponds to original m_orig = 2*S.
    for s in 0u64..6 {
        let original_eval = eval_predicate(
            &pred,
            &[2 * s], // original-coordinate marking
            &simple_net(),
        );
        let slice_eval = eval_predicate(&remapped, &[s], &simple_net());
        assert_eq!(
            slice_eval, original_eval,
            "scaled tokens>=k must match original at S={s}"
        );
    }

    // Concretely: the threshold 5 (NOT a multiple of g=2) becomes ceil(5/2)=3.
    assert_eq!(
        remapped,
        ResolvedPredicate::IntLe(
            ResolvedIntExpr::Constant(3),
...
#[test]
fn scaled_remap_tokens_le_const_uses_floor() {
    // Original: tokens(p0) <= 5, scale g=2.  2*S <= 5  <=>  S <= floor(5/2)=2.
    let pred = ResolvedPredicate::IntLe(
        ResolvedIntExpr::TokensCount(vec![PlaceIdx(0)]),
        ResolvedIntExpr::Constant(5),
    );
    ...
    for s in 0u64..6 {
        assert_eq!(
            eval_predicate(&remapped, &[s], &simple_net()),
            eval_predicate(&pred, &[2 * s], &simple_net()),
        );
    }
}

#[test]
fn scaled_remap_is_identity_when_scale_is_one() {
    // No GCD scaling (g=1): behavior must match plain remap_predicate exactly.
    let pred = ResolvedPredicate::IntLe(
        ResolvedIntExpr::Constant(5),
        ResolvedIntExpr::TokensCount(vec![PlaceIdx(0)]),
    );
    let scaled = remap_predicate_scaled(&pred, &identity_place_map(1), &[], &[1]);
    let plain = remap_predicate(&pred, &identity_place_map(1), &[]);
    assert_eq!(scaled, plain);
    assert_eq!(scaled, Some(pred));
}

#[test]
fn scaled_remap_same_scale_sum_divides_out() {
    // tokens(p0)+tokens(p1) <= 7 with both places scaled by g=3:
    // 3*(S0+S1) <= 7  <=>  S0+S1 <= floor(7/3)=2.
    let pred = ResolvedPredicate::IntLe(
        ResolvedIntExpr::TokensCount(vec![PlaceIdx(0), PlaceIdx(1)]),
        ResolvedIntExpr::Constant(7),
    );
    let remapped = remap_predicate_scaled(&pred, &identity_place_map(2), &[], &[3, 3])
        .expect("equal-scale sum must remap");
    assert_eq!(
        remapped,
        ResolvedPredicate::IntLe(
            ResolvedIntExpr::TokensCount(vec![PlaceIdx(0), PlaceIdx(1)]),
...
#[test]
fn scaled_remap_declines_mixed_scales_in_one_sum() {
    // tokens(p0)+tokens(p1) <= 7 with DIFFERENT scales (2 and 3): the unweighted
    // token sum cannot represent 2*S0 + 3*S1 exactly. Must decline (None) so the
    // caller routes to the sound non-slice path.
    let pred = ResolvedPredicate::IntLe(
        ResolvedIntExpr::TokensCount(vec![PlaceIdx(0), PlaceIdx(1)]),
        ResolvedIntExpr::Constant(7),
    );
    assert!(remap_predicate_scaled(&pred, &identity_place_map(2), &[], &[2, 3]).is_none());
}

#[test]
fn scaled_remap_declines_scaled_tokens_vs_tokens() {
    // tokens(p0) <= tokens(p1) with different scales: not exactly expressible.
    let pred = ResolvedPredicate::IntLe(
        ResolvedIntExpr::TokensCount(vec![PlaceIdx(0)]),
        ResolvedIntExpr::TokensCount(vec![PlaceIdx(1)]),
    );
    ...
    // Equal scales DO divide out: g*S0 <= g*S1 <=> S0 <= S1.
    let same = remap_predicate_scaled(&pred, &identity_place_map(2), &[], &[2, 2])
        .expect("equal-scale tokens-vs-tokens must remap");
    assert_eq!(same, pred);
}

#[test]
fn scaled_remap_still_drops_missing_places_and_transitions() {
    // The decline-on-dropped-dependency contract from remap_predicate is kept.
    let pred = ResolvedPredicate::And(vec![
        ResolvedPredicate::IntLe(
            ResolvedIntExpr::TokensCount(vec![PlaceIdx(0)]),
            ResolvedIntExpr::Constant(1),
    ...
    assert!(remap_predicate_scaled(&pred, &[None], &[Some(TransitionIdx(2))], &[2]).is_none());
    assert!(remap_predicate_scaled(&pred, &[Some(PlaceIdx(3))], &[None], &[2]).is_none());
}
