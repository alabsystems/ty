...
/// #22 (fail-closed): a malformed/oversized PNML whose token counts could
/// overflow u64 on a single firing must be DECLINED at parse time — before any
/// exploration allocates — never silently accepted into a wrapping run. The net
/// here is tiny: P0 starts at u64::MAX and T0 produces 1 more token onto it.
#[test]
fn test_parse_declines_overflow_capable_marking() {
    let pnml = format!(
        r#"<?xml version="1.0"?>
<pnml xmlns="http://www.pnml.org/version-2009/grammar/pnml">
  <net id="overflow" type="http://www.pnml.org/version-2009/grammar/ptnet">
    <page id="p1">
    ...
    let err = parse_pnml(&pnml).expect_err("overflow-capable net must DECLINE at parse time");
    assert!(
        matches!(err, PnmlError::MarkingOverflow { .. }),
        "expected MarkingOverflow decline, got {err:?}"
    );
}
