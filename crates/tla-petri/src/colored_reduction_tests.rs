...
use crate::hlpnml::{
    self, ColorExpr, ColorTerm, ColoredArc, ColoredNet, ColoredPlace, ColoredTransition,
};
use crate::model::{
...
fn write_collapsible_all_cardinality_xml(dir: &TempDir) {
    let xml = r#"<?xml version="1.0"?>
<property-set xmlns="http://mcc.lip6.fr/">
  <property>
    <id>CollapseAll-ReachabilityCardinality-00</id>
    <description>P0 has at least two tokens across all colors initially</description>
    <formula>
      <exists-path>
        <finally>
          <integer-le>
    ...
    fs::write(dir.path().join("ReachabilityCardinality.xml"), xml).expect("fixture XML");
}
...
fn dot_token_expr(count: u64) -> ColorExpr {
    ColorExpr::NumberOf {
        count,
...
fn single_consumer_net(initial_marking: Option<ColorExpr>) -> ColoredNet {
    ColoredNet {
        name: Some("dead-pruning-fixture".to_string()),
        sorts: vec![],
        variables: vec![],
        places: vec![ColoredPlace {
            id: "p0".to_string(),
            name: Some("p0".to_string()),
            sort_id: "dot".to_string(),
            initial_marking,
        }],
        transitions: vec![ColoredTransition {
            id: "t0".to_string(),
        ...
        arcs: vec![ColoredArc {
            ...
            source: "p0".to_string(),
            target: "t0".to_string(),
            inscription: dot_token_expr(1),
        }],
    }
}

#[test]
fn test_dead_transition_removal_treats_zero_markings_as_empty() {
    let cases = [
        (
            "All(0)",
            ColorExpr::All {
        ...
        (
            ...
            ColorExpr::Add(vec![
                ...
                ColorExpr::Add(vec![dot_token_expr(0)]),
            ]),
        ),
    ];

    for (case, marking) in cases {
        let mut colored = single_consumer_net(Some(marking));
        let report = colored_dead_transitions::reduce(&mut colored);

        assert_eq!(
            report.transitions_removed, 1,
            "{case} should make the consumer transition dead"
        );
        assert!(
            colored.transitions.is_empty(),
            "{case} should remove the dead transition"
        );
        assert!(
            colored.arcs.is_empty(),
            "{case} should remove arcs attached to the dead transition"
        );
    }
}

#[test]
fn test_dead_transition_removal_keeps_add_with_nonzero_child() {
    let marking = ColorExpr::Add(vec![
        ColorExpr::All {
            sort_id: "dot".to_string(),
            count: 0,
        },
    ...
    assert_eq!(report.transitions_removed, 0);
    assert_eq!(colored.transitions.len(), 1);
    assert_eq!(colored.arcs.len(), 1);
}
...
#[test]
fn test_all_inscription_model_uses_uncollapsed_cardinality_semantics() {
    let dir = collapsible_all_fixture_dir();
    write_collapsible_all_cardinality_xml(&dir);
    ...
    let reduced_records =
        collect_examination_for_model(&reduced, Examination::ReachabilityCardinality, &config)
            .expect("collapsed cardinality should dispatch");
    let unreduced_records =
        collect_examination_for_model(&unreduced, Examination::ReachabilityCardinality, &config)
            .expect("unreduced cardinality should dispatch");

    assert_eq!(
        reduced_records, unreduced_records,
        "production cardinality must use the uncollapsed colored-place token total"
    );
    assert_eq!(reduced_records.len(), 1);
    assert_eq!(
        reduced_records[0].value,
        ExaminationValue::Verdict(Verdict::True),
        "P0 initially has one token in each of three colors"
    );
}

#[test]
fn test_all_inscription_model_uses_uncollapsed_one_safe_semantics() {
    let dir = collapsible_all_fixture_dir();
    let reduced = load_model_dir(dir.path()).expect("collapsed fixture should load");
    let config = ExplorationConfig::new(16).with_workers(1);

    let records = collect_examination_for_model(&reduced, Examination::OneSafe, &config)
        .expect("OneSafe should dispatch");

    assert_eq!(records.len(), 1);
    assert_eq!(
        records[0].value,
        ExaminationValue::Verdict(Verdict::False),
        "P0 has three total colored tokens initially and is not OneSafe"
    );
}
...
fn test_dead_transition_removal_on_neo_election() {
    ...
    assert!(
        !colored.places.is_empty(),
        "should retain at least one place"
    );
    assert!(
