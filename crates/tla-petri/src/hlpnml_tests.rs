...
#[test]
fn test_parse_finite_int_range_constant_color_term() {
    let xml = r#"<?xml version="1.0"?>
<pnml xmlns="http://www.pnml.org/version-2009/grammar/pnml">
  <net id="test" type="http://www.pnml.org/version-2009/grammar/symmetricnet">
    <page id="page0">
    ...
    let net = parse_hlpnml(xml).expect("finiteintrangeconstant should parse");
    let arc = net.arcs.iter().find(|arc| arc.id == "a1").expect("a1");
    match &arc.inscription {
        ColorExpr::NumberOf { color, .. } => {
            assert!(matches!(color.as_ref(), ColorTerm::IntegerConstant(42)));
        }
        other => panic!("expected numberof inscription, got {other:?}"),
    }
}
    ...
    <page id="page0">
      ...
      <arc id="a1" source="p1" target="t1">
        <hlinscription><structure>
          <numberof>
            <subterm><numberconstant value="1"/></subterm>
            <subterm><unknowncolorterm value="42"/></subterm>
          </numberof>
        </structure></hlinscription>
      </arc>
    </page>
