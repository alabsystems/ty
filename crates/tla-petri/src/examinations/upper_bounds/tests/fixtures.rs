...
pub(super) fn check_upper_bounds_properties_unsliced(
    ...
    if result.completed
        && trackers.iter().any(|tracker| {
            tracker
                .lp_bound
                .is_some_and(|lp| tracker.max_bound < lp && !tracker.is_structurally_resolved())
        })
    {
        let identity = ReducedNet::identity(net);
        let identity_config = config.refitted_for_net(net);
        let (identity_result, identity_trackers) =
            explore_upper_bounds_on_reduced_net(&identity, trackers, &identity_config)
                .expect("identity-net fallback should not overflow");
