...
fn apply_validated_witness(
    ...
    for &transition in &candidate.trace {
        ...
        if ctx.net.apply_delta(&mut marking, transition).is_err() {
            eprintln!(
                "Reachability witness rejected: {} trace overflows place token count for {} at {:?}",
                candidate.source.as_str(),
                tracker.id,
                transition,
            );
    ...
    let (source, depth) = match candidate.source {
        WitnessSeedSource::Bfs => (
            match tracker.quantifier {
                PathQuantifier::EF => ReachabilityResolutionSource::BfsWitness,
                PathQuantifier::AG => ReachabilityResolutionSource::BfsCounterexample,
            },
            Some(candidate.trace.len()),
        ),
        WitnessSeedSource::RandomWalk => (ReachabilityResolutionSource::RandomWalk, None),
        WitnessSeedSource::Heuristic => (ReachabilityResolutionSource::Heuristic, None),
        WitnessSeedSource::Aiger => (ReachabilityResolutionSource::Aiger, None),
    };
    let verdict = matches!(tracker.quantifier, PathQuantifier::EF);
    resolve_tracker(tracker, verdict, source, depth);
    true
}
...
mod tests {
    ...
    #[test]
    fn test_validation_accepts_valid_aiger_witness_with_aiger_source() {
        let net = linear_net();
        let mut trackers = vec![ef_tracker("ef", tokens_ge(1, 1))];
        let targets = validation_targets_from_trackers(&trackers);
        let ctx = WitnessValidationContext::new(&net, &targets);
    ...
    #[test]
    fn test_validation_accepts_valid_bfs_witness_with_depth() {
        let net = linear_net();
        let mut trackers = vec![ef_tracker("ef", tokens_ge(1, 1))];
        let targets = validation_targets_from_trackers(&trackers);
        let ctx = WitnessValidationContext::new(&net, &targets);
        ...
        assert_eq!(accepted, 1);
        assert_eq!(trackers[0].verdict, Some(true));
        let resolution = trackers[0].resolved_by.expect("bfs provenance");
        assert_eq!(resolution.source, ReachabilityResolutionSource::BfsWitness);
