...
impl ExplorationObserver for OneSafeObserver {
    ...
    fn canonicalization_safe(&self) -> bool {
        // PT path checks `∃p. m[p] > 1`, which is σ-invariant under any place
        // swap (the multiset of token counts — hence its max — is preserved).
        // The colored path instead sums token counts over a *fixed* set of
        // unfolded place indices per colored place; place-swap canonicalization
        // permutes indices within orbits, and an orbit crossing colored-group
        // boundaries corrupts those group sums, which can flip a 1-safe verdict.
        // Fail-closed: refuse canonicalization whenever colored groups are set.
        self.colored_groups.is_empty()
    }
}
