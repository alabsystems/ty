...
/// A structurally non-trivial safety predicate that is always true for
/// reachable markings (`tokens(p0) >= 0`) but whose negation does not fold
/// to a `Bool` literal. Used to exercise query-clause emission: ay-chc's
/// `add_clause` drops clauses whose body constraint simplifies to `false`,
/// so a `safety = True` property yields no query (`NOT True` => `false`).
fn nontrivial_true_safety_p0() -> ResolvedPredicate {
...
fn encode_simple_net_produces_valid_chc() {
    ...
    // Non-trivial always-true safety so the query clause is not dropped by
    // ay-chc's constant folding (`NOT true` simplifies to `false`, which
    // `add_clause` discards as a vacuous clause).
    let safety = nontrivial_true_safety_p0();
    let config = PdrConfig::default();

    let problem = encode_petri_net_pdr(&net, &safety, &config, None);
...
// These tests actually run ay-chc and require more time. They verify
...
fn solve_trivially_safe() {
    ...
    let result = solve_petri_net_pdr(&net, &safety, &config, None);
...
#[test]
fn bounded_exact_respects_expired_deadline() {
    let net = simple_two_place_net();
    let safety = ResolvedPredicate::False;
    let expired = Some(
    ...
    let result = solve_bounded_exact(&net, &safety, expired);
    assert_eq!(
        result,
...
#[test]
fn bounded_exact_reports_missing_structural_bounds() {
    let net = growing_net();
    let safety = ResolvedPredicate::True;

    let result = solve_bounded_exact(&net, &safety, None);
    assert_eq!(
        result,
        BoundedExactOutcome::Inconclusive(BoundedExactInconclusive::MissingStructuralBounds)
    );
}
