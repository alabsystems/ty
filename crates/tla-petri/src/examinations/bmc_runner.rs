...
use super::smt_encoding::{
    encode_transition_relation, encode_transition_relation_deadlock_stutter,
};
use super::smt_encoding::{
    encode_transition_relation_steps, run_ay_with_report_deadline, SolverOutcome, SolverRunReport,
};

/// Per-depth solver query constructed by a BMC depth-ladder caller.
pub(super) struct DepthQuery {
    /// SMT-LIB script to send to ay for this depth.
    pub script: String,
    /// Number of `(check-sat)` results expected back from ay.
...
pub(super) fn run_depth_ladder<T>(
    ...
    state: &mut T,
    build_query: impl FnMut(&mut T, usize) -> Option<DepthQuery>,
    ...
    run_depth_ladder_with_report(
        ...
        state,
        build_query,
        |state, depth, report| handle_result(state, depth, report.map(SolverRunReport::outcomes)),
    )
}

/// Run a depth ladder and preserve the solver run report for each depth.
pub(super) fn run_depth_ladder_with_report<T>(
    ay_path: &Path,
    depths: &[usize],
    deadline: Option<Instant>,
    per_depth_timeout: Duration,
    ...
    mut handle_result: impl FnMut(&mut T, usize, Option<&SolverRunReport>) -> DepthAction,
) -> Option<usize> {
    ...
    for &depth in depths {
        ...
        let report = run_ay_with_report_deadline(
            ...
            query.num_checks,
            timeout,
            timeout_is_deadline_limited(deadline, per_depth_timeout),
        );

        match handle_result(state, depth, report.as_ref()) {
...
/// Run a BMC depth ladder using a persistent incremental ay process.
///
/// The solver is spawned once and kept alive across all depths.
/// Transition-relation assertions accumulate (ay retains learned clauses).
...
pub(super) fn run_depth_ladder_incremental<T>(
    ...
    build_queries: impl FnMut(&mut T, usize) -> Option<IncrementalPropertyQuery>,
    mut handle_result: impl FnMut(&mut T, usize, Option<&[SolverOutcome]>) -> DepthAction,
    ...
    mut fallback_handle_result: impl FnMut(&mut T, usize, Option<&[SolverOutcome]>) -> DepthAction,
) -> Option<usize> {
    run_depth_ladder_incremental_with_report(
        ...
        |state, depth, report| {
            fallback_handle_result(state, depth, report.map(SolverRunReport::outcomes))
        },
    )
}

/// Run an incremental depth ladder and preserve per-depth solver reports.
pub(super) fn run_depth_ladder_incremental_with_report<T>(
    ay_path: &Path,
    depths: &[usize],
    deadline: Option<Instant>,
    per_depth_timeout: Duration,
    ...
    // Fallback closures — used only if incremental solver spawn fails.
    fallback_build_query: impl FnMut(&mut T, usize) -> Option<DepthQuery>,
    fallback_handle_result: impl FnMut(&mut T, usize, Option<&SolverRunReport>) -> DepthAction,
) -> Option<usize> {
    let mut solver = match IncrementalSolver::new(ay_path) {
        Some(s) => s,
        None => {
            eprintln!("Incremental solver spawn failed, falling back to batch mode");
            return run_depth_ladder_with_report(
                ay_path,
                depths,
                deadline,
                per_depth_timeout,
    ...
    for &depth in depths {
        ...
        let timeout_deadline_limited = timeout_is_deadline_limited(deadline, per_depth_timeout);

        // Check each property with push/pop.
        let mut outcomes = Vec::with_capacity(query.assertions.len());
        let mut last_solve_profile = None;
        ...
        for assertion in &query.assertions {
            ...
            let check_report = solver.check_sat_with_report(remaining, timeout_deadline_limited);
            if let Some(profile) = check_report.solve_profile() {
                last_solve_profile = Some(profile.clone());
            }
            outcomes.extend_from_slice(check_report.outcomes());
            if !solver.pop() {
                session_failed = true;
                break;
            }
        ...
        let mut report = SolverRunReport::from_outcomes(outcomes);
        if let Some(profile) = last_solve_profile {
            report = report.with_solve_profile(profile);
        }
        match handle_result(state, depth, Some(&report)) {
            DepthAction::Explored => max_explored_depth = Some(depth),
            DepthAction::StopDeepening => break,
        }
...
fn timeout_is_deadline_limited(deadline: Option<Instant>, per_depth_timeout: Duration) -> bool {
    deadline.is_some_and(|global_deadline| {
        global_deadline.saturating_duration_since(Instant::now()) <= per_depth_timeout
    })
}
...
/// Emit the BMC script preamble for the LTL lasso search, where the `stay`
/// (stutter) action is constrained to genuine deadlock markings.
///
/// Identical to [`emit_bmc_preamble`] except the transition relation forbids a
/// stutter at any marking that still has an enabled transition. This matches the
/// on-the-fly Büchi self-loop semantics (a system step only self-loops when the
/// enabled set is empty) and keeps the lasso encoding sound: the solver can no
/// longer manufacture a spurious self-loop at a live marking to close a fake
/// accepting lasso. ONLY the LTL lasso BMC must use this variant; reachability
/// BMC and k-induction rely on free stutter and must keep [`emit_bmc_preamble`].
pub(crate) fn emit_bmc_preamble_deadlock_stutter(s: &mut String, net: &PetriNet, depth: usize) {
    let np = net.num_places();
    let nt = net.num_transitions();
    s.push_str("(set-logic QF_LIA)\n");
    emit_marking_vars(s, np, depth);
    ...
    emit_initial_marking(s, net);
    emit_non_negativity(s, np, depth);
    encode_transition_relation_deadlock_stutter(s, net, np, nt, depth);
}
