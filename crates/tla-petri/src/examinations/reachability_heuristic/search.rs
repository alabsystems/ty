...
pub(crate) fn run_heuristic_seeding_params(
    ...
    // One adaptive probe for the deadline + the memory backstop.
    // `max_queue_size` is memory-sized so the open set is bounded in steady
    // state, but the probe polls actual footprint too, so any unforeseen
    // growth stops the search before an OOM. Seeding only ever produces TRUE
    // witnesses, so stopping early is verdict-preserving.
    ...
    while let Some(Reverse(entry)) = queue.pop() {
        ...
        // Success early-out (NOT a resource guard): all seed targets reached.
        // Kept on a coarse cadence to avoid scanning trackers every pop.
        if expansions & 0x3FF == 0 && all_walkable_resolved(trackers) {
            break;
        }
        ...
        for &tidx in &enabled {
            ...
            // #22: this heuristic seeding only ever produces concrete TRUE
            // witnesses (never a FALSE verdict). On a token-count overflow the
            // successor is non-representable, so skip it — the scratch buffer is
            // overwritten on the next iteration; soundness is untouched.
            if net.apply_delta(&mut successor, tidx).is_err() {
                continue;
            }
