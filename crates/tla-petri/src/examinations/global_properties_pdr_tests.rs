...
/// Delayed overflow: a single token walks along a chain and the last two
/// transitions both add to an accumulator place, making that place reach 2.
fn delayed_source_place_overflow_net() -> PetriNet {
    let mut places: Vec<PlaceInfo> = (0..=17).map(|idx| place(&format!("p{idx}"))).collect();
    let accumulator = PlaceIdx(18);
    places.push(place("p_acc"));

    let transitions = (0..=16)
        .map(|idx| {
            let mut outputs = vec![arc((idx + 1) as u32, 1)];
            if idx >= 15 {
                outputs.push(Arc {
                    place: accumulator,
                    weight: 1,
            ...
            trans(&format!("t{idx}"), vec![arc(idx as u32, 1)], outputs)
        })
        .collect();
    ...
    PetriNet {
        name: Some("delayed-source-place-overflow".to_string()),
        places,
        transitions,
        initial_marking,
    }
...
#[test]
fn test_pdr_one_safe_false_for_delayed_source_place_overflow() {
    let net = delayed_source_place_overflow_net();
    let result = run_one_safe_pdr(&net, None);
    assert_eq!(
        result,
        Some(false),
        "delayed accumulator overflow must not be proved 1-safe by PDR"
    );
}

#[test]
fn test_pdr_one_safe_expired_deadline_returns_none() {
    let net = one_safe_net();
    let result = run_one_safe_pdr(&net, Some(Instant::now()));
...
#[test]
fn test_pdr_deadlock_expired_deadline_returns_none() {
    let net = no_deadlock_net();
    let result = run_deadlock_pdr(&net, Some(Instant::now()));
    assert_eq!(result, None);
}
...
#[test]
fn test_pdr_stable_marking_expired_deadline_returns_none() {
    let net = stable_place_net();
    let result = run_stable_marking_pdr(&net, Some(Instant::now()));
