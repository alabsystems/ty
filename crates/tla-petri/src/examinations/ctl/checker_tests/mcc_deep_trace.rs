...
fn test_deep_trace_angiogenesis_property_14() {
    ...
    let k24_fireable: Vec<bool> = (0..n)
        .map(|s| net.is_enabled(&full.markings.unpack(s), k24_idx))
        .collect();
    ...
    let k10_fireable: Vec<bool> = (0..n)
        .map(|s| net.is_enabled(&full.markings.unpack(s), k10_idx))
        .collect();
    let k51_fireable: Vec<bool> = (0..n)
        .map(|s| net.is_enabled(&full.markings.unpack(s), k51_idx))
        .collect();
