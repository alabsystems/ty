...
fn test_diagnose_angiogenesis_atom_details() {
    ...
    for tid in &transitions_to_check {
        ...
        for s in 0..n {
            if net.is_enabled(&full.markings.unpack(s), trans) {
...
fn test_verify_atoms_by_raw_markings_angiogenesis() {
    ...
    for s in 0..n {
        ...
        let raw_enabled = k24_trans
            .inputs
            .iter()
            .all(|arc| full.markings.unpack(s)[arc.place.0 as usize] >= arc.weight);
        let api_enabled = net.is_enabled(
            &full.markings.unpack(s),
    ...
    for &s in &fireable_k24_raw {
        ...
        for arc in &k24_trans.inputs {
            let val = full.markings.unpack(s)[arc.place.0 as usize];
    ...
    for s in 0..n {
        if net.is_enabled(&full.markings.unpack(s), k10t)
            && net.is_enabled(&full.markings.unpack(s), k51t)
        {
