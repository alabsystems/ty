...
pub(crate) type ResolvedCtl = GenericCtlFormula<ResolvedPredicate>;

pub(crate) fn resolve_ctl_with_aliases(
    formula: &CtlFormula,
    ...
    match formula {
        ...
        CtlFormula::EGF(inner) => {
            ResolvedCtl::EGF(Box::new(resolve_ctl_with_aliases(inner, aliases)))
        }
    }
}
...
pub(super) fn resolve_ctl(
    ...
    let aliases = PropertyAliases {
        ...
        transition_aliases: trans_map
            ...
            .map(|(name, idx)| ((*name).to_string(), vec![*idx]))
            .collect(),
        colored_place_group_aliases: std::collections::HashSet::new(),
    };
