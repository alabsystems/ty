...
pub(super) fn build_ctl_slice_candidate(
    net: &PetriNet,
    properties: &[Property],
    ...
    let Some(support) = ctl_support(&reduced, properties, &place_map, &trans_map) else {
        return (reduced, None);
    };
...
pub(super) fn check_ctl_properties_unsliced(
    ...
    full.markings
        .expand_in_place(|marking| reduced.expand_marking(marking))
...
/// Evaluate CTL formulas on the FULL raw net with NO reduction and NO slicing.
///
/// This is the ground-truth oracle: it explores the complete reachability graph
/// of `net` and runs the standard fixpoint CTL checker, so it preserves every
/// disjoint livelock/self-loop and the exact deadlock structure. Used to pin the
/// sound answer that slicing/reduction must never contradict.
pub(super) fn check_ctl_properties_raw_oracle(
    net: &PetriNet,
    properties: &[Property],
    config: &ExplorationConfig,
) -> Vec<(String, Verdict)> {
    ...
    assert!(full.graph.completed, "raw oracle exploration must complete");
...
/// Two disjoint components designed to expose the suffix-slice soundness bug.
///
/// - Component A (the predicate support): `a0 --ta--> a1`. Acyclic, no
///   self-loop. The predicate `tokens(a0) >= 1` holds only at the initial
///   marking; firing `ta` falsifies it permanently.
/// - Component B (a disjoint LIVELOCK): `b0 --tb--> b0`, an always-enabled
///   self-loop that never touches a0/a1 and is therefore invisible to the
///   predicate.
///
/// `EG(tokens(a0) >= 1)` is TRUE on the FULL net: fire `tb` forever, keeping
/// `a0` frozen at 1 — an infinite path staying in the predicate region.
///
/// Query slicing for that predicate keeps only {a0, ta, a1} and DROPS the
/// livelock `tb`. On the A-only slice the only maximal path is `a0 --ta--> a1`,
/// which leaves the predicate region with no deadlock and no cycle inside it,
/// so the suffix analysis would wrongly conclude `EG(a0 >= 1) = FALSE`. This is
/// the catastrophic flip the fail-closed gate must prevent.
pub(super) fn livelock_disjoint_eg_net() -> PetriNet {
    PetriNet {
        name: Some(String::from("livelock-disjoint-eg")),
        places: vec![
            PlaceInfo {
                id: String::from("a0"),
                name: None,
...
/// `EG(tokens(a0) >= 1)` — exists a maximal path staying in the predicate.
pub(super) fn eg_a0_ge_1() -> CtlFormula {
    CtlFormula::EG(Box::new(CtlFormula::Atom(atom_pred("a0", 1))))
}
...
/// Finite variant of `ctl_local_fallback_net`.
///
/// A tight full-graph budget still truncates while exploring the noise branch,
/// but a larger budget can complete and provide an oracle for fallback parity.
pub(super) fn finite_ctl_local_fallback_net() -> PetriNet {
    PetriNet {
        name: Some(String::from("finite-ctl-local-fallback")),
        places: vec![
            PlaceInfo {
                id: String::from("start"),
                name: None,
            ...
            PlaceInfo {
                id: String::from("noise0"),
                name: None,
            },
            PlaceInfo {
                id: String::from("noise1"),
                name: None,
            },
            PlaceInfo {
                id: String::from("noise2"),
                name: None,
            },
        ],
        transitions: vec![
        ...
        initial_marking: vec![1, 0, 0, 0, 0, 0],
    }
}
