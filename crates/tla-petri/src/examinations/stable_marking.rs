...
impl ExplorationObserver for StableMarkingObserver {
    ...
    fn canonicalization_safe(&self) -> bool {
        // Stability is tracked per-place by index (`self.stable[i]` vs
        // `initial_tokens[i]`). Place-swap canonicalization permutes which
        // physical place occupies index `i` (it sorts token counts within each
        // orbit), so an actually-unstable place can masquerade as stable and
        // flip the verdict to a false TRUE. Stability is inherently per-place;
        // no orbit-quotient observer can recover it. This is redundant with the
        // examination-level guard (`StableMarking => false`), but kept as
        // defense-in-depth so a future change to that guard cannot silently
        // reintroduce the −16-point wrong-verdict bug.
        false
    }
}
