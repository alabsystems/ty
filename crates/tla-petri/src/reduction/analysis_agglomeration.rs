...
/// Resolve conflicts between Rule S candidates and caller-supplied claims.
///
/// Rule S runs BEFORE Rule R (Tapaal `Reducer.cpp:2893-2896`). In a single
/// planner pass, callers can pass any pre/post agglomerations that should
/// retain priority over Rule S. Reachability-mode planning passes empty slices
/// so strict Phase-1 Rule S candidates can preempt overlapping one-sided
/// agglomerations, then drops the conflicting pre/post entries before
/// materialization.
