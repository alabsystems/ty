...
///
/// `removed` marks places already removed by other rules OR already accepted
/// for removal earlier in the SAME planning pass. The LP certificate must not
/// lean on the enabling preconditions of such places: their input arcs will
/// not exist in the reduced net, so a certificate citing them would not be
/// closed under joint removal (two mutually-implicit places could otherwise
/// both certify against each other and both be removed — unsound).
pub(crate) fn is_lp_redundant(
    net: &PetriNet,
    place_idx: usize,
    dead_set: &[bool],
    removed: &[bool],
...
///
/// Preconditions on places in `removed` are NOT added as hypotheses: those
/// places will be gone from the reduced net, so their enabling constraints
/// no longer restrict the reduced transition. (Their state-equation rows are
/// kept — they remain true facts about the original net.) Dropping a
/// hypothesis can only lower the LP minimum, so this is strictly
/// conservative.
fn place_never_constrains_transition(
    ...
    // that still SURVIVE in the reduced net (see doc-comment on `removed`).
    let trans = &net.transitions[t];
    for arc in &trans.inputs {
        if arc.place == place || removed[arc.place.0 as usize] {
            continue;
        }
...
pub(crate) fn find_redundant_places(
    ...
    for &inv_idx in &sorted_indices {
        ...
        // LP-first selection: try each candidate, pick the first that passes
        // LP. `removed` is passed live: it contains other rules' removals AND
        // every candidate already accepted earlier in this pass, so each
        // certificate is proven without leaning on already-deleted places
        // (sequentially-exact greedy, Colom-Silva iteration order).
        for &candidate in &candidates {
            if !is_lp_redundant(net, candidate, &dead_set, &removed) {
                continue;
            }
