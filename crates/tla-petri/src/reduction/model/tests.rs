...
fn reduced_net_with_custom_recon(
    ...
    reduced_net_with_custom_recon_and_divisor(kept_marking, terms, constant, 1)
}

fn reduced_net_with_custom_recon_and_divisor(
    kept_marking: u64,
    terms: Vec<(PlaceIdx, u64)>,
    constant: u64,
    divisor: u64,
...
#[test]
fn test_expand_marking_into_returns_error_on_reconstruction_non_exact_division() {
    let reduced = reduced_net_with_custom_recon_and_divisor(0, Vec::new(), 5, 2);
    let mut full = Vec::new();

    let error = reduced
        .expand_marking_into(&[0], &mut full)
        .expect_err("non-exact reconstruction should fail closed");
    assert!(matches!(
        error,
        PnmlError::ReductionOverflow { ref context }
            if context.contains("non-exact division")
    ));
}

#[test]
fn test_expand_marking_into_returns_error_on_reconstruction_zero_divisor() {
    let reduced = reduced_net_with_custom_recon_and_divisor(0, Vec::new(), 5, 0);
    let mut full = Vec::new();

    let error = reduced
        .expand_marking_into(&[0], &mut full)
        .expect_err("zero-divisor reconstruction should fail closed");
    assert!(matches!(
        error,
        PnmlError::ReductionOverflow { ref context }
            if context.contains("zero divisor")
    ));
}
...
fn test_reduction_mode_reachability_allows_all_rules() {
    ...
    assert!(mode.allows_rule_r_agglomeration());
}
...
#[test]
fn test_reduction_mode_stutter_insensitive_ltl_forbids_rule_r() {
    let ltl = ReductionMode::StutterInsensitiveLTL;
    let ctl = ReductionMode::NextFreeCTL;
    // Stutter-insensitive LTL and next-free CTL allow the same non-agglomerating rules.
    assert_eq!(
    ...
    assert!(!ltl.allows_rule_r_agglomeration());
}
...
#[test]
fn test_reduction_mode_reachability_deadlock_preserving_subset() {
    let mode = ReductionMode::ReachabilityDeadlock;
    // Admissible (TRUE) — none change which transitions are enabled at any
    // reachable marking, so the reachable-deadlock-existence set is identical.
    assert!(mode.allows_dead_transition_removal());
    assert!(mode.allows_constant_place_removal());
    assert!(mode.allows_isolated_place_removal());
    assert!(mode.allows_redundant_place_removal());
    ...
    assert!(mode.allows_parallel_place_merge());
    assert!(mode.allows_never_disabling_arc_removal());
    // Broadened (proven deadlock-preserving by the random-net differential gate
    // `reduction_differential_proptest.rs`): a source place (Rule C) gates no
    // transition, and a non-decreasing place (Rule F) with m0 >= max_consume is
    // always-satisfied — neither changes the enabled set at any reachable
    // marking, so the deadlock-existence boolean is identical.
    assert!(mode.allows_source_place_removal());
    assert!(mode.allows_non_decreasing_place_removal());

    // Forbidden (FALSE) — each can create or hide a deadlock (the NO-rows of the
    // P1 gate table) or is conservatively off.
    assert!(!mode.allows_agglomeration());
    assert!(!mode.allows_rule_r_agglomeration());
    assert!(!mode.allows_rule_s_agglomeration());
    assert!(!mode.allows_token_cycle_merge());
    assert!(!mode.allows_self_loop_transition_removal());
    assert!(!mode.allows_self_loop_arc_removal());
    assert!(!mode.allows_sink_transition_removal());
    assert!(!mode.allows_token_eliminated_place_removal());
}
