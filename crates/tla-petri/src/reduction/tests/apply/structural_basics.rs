...
#[test]
fn test_reduce_iterative_structural_reports_fixpoint_lower_bound_rule_n() {
    let net = PetriNet {
        name: Some("rule-n-fixpoint-lower-bound".into()),
        places: vec![place("p_mono"), place("p_keep")],
        transitions: vec![trans("t_grow", vec![arc(0, 1)], vec![arc(0, 2), arc(1, 1)])],
    ...
    let rule_n = &reduced.report.never_disabling_arcs[0];
    assert_eq!(rule_n.transition, TransitionIdx(0));
    assert_eq!(rule_n.place, PlaceIdx(0));
    assert_eq!(
        rule_n.proof,
    ...
    assert_eq!(
        ...
        "Rule N proof is surfaced because Rule F removes the place"
    );
}
...
fn test_reduced_net_preserves_enabled_transitions() {
    ...
    let result = reduced
        .net
        .fire(&reduced.net.initial_marking, t)
        .expect("fire (test)");
...
#[test]
fn test_reduce_iterative_structural_uses_efmnop_workqueue_dead_seed() {
    let net = PetriNet {
        name: Some("efmnop-workqueue-dead-seed".into()),
        places: vec![place("p_token"), place("p_out")],
        transitions: vec![
    ...
    assert!(
        reduced.report.dead_transitions.contains(&TransitionIdx(1)),
        "EFMNOP workqueue should prove t_needs_two_tokens dead"
    );
    assert_eq!(reduced.transition_map[1], None);
}
