...
use crate::reduction::{
    apply_query_guarded_prefire, reduce_iterative_structural,
    reduce_iterative_structural_with_mode, reduce_query_guarded, ReductionMode,
};

use super::super::support::{arc, place, trans};

/// A net that the fixpoint would reduce over several rounds: a chain of
/// dead-on-first-fire pumps plus query-irrelevant cascade structure. We only
/// need it to be *reducible* (≥1 round of progress) so the deadline check is
/// meaningfully exercised.
fn reducible_chain_net() -> PetriNet {
    PetriNet {
        name: Some("reducible-chain".into()),
...
/// #4: a deadline already in the past must make the structural-reduction
/// fixpoint return PROMPTLY (before doing any reduction round) with a SOUND net.
/// With zero rounds elapsed, the accumulated result is the identity net — a
/// valid partially-reduced net (here, not reduced at all), never a crash, hang,
/// or a more-than-fully-reduced (and thus possibly unsound) net.
#[test]
fn test_with_mode_fixpoint_honors_past_deadline() {
    let net = reducible_chain_net();
    ...
    let reduced =
        reduce_iterative_structural_with_mode(&net, &[], ReductionMode::Reachability, Some(past))
            .expect("a past deadline must still return Ok (a sound partial net)");
    let elapsed = started.elapsed();

    // Promptness: returning before any round means well under a second.
    assert!(
        elapsed < Duration::from_millis(500),
        "past-deadline reduction must return promptly, took {elapsed:?}"
    );
    // Soundness of the partial net: identity (no rounds ran). It must preserve
    // the original structure exactly — never more reduced than the full fixpoint.
...
/// Control: the SAME net with NO deadline must still run to a real fixpoint and
/// actually reduce — proving the deadline (not a net defect) is what stops the
/// loop above.
#[test]
fn test_with_mode_fixpoint_no_deadline_still_reduces() {
    let net = reducible_chain_net();
    let reduced =
        reduce_iterative_structural_with_mode(&net, &[], ReductionMode::Reachability, None)
            .expect("no-deadline reduction should succeed");
    assert!(
        reduced.report.has_reductions(),
        "the reducible chain net must actually reduce when given no deadline"
    );
}

/// #4: a deadline already in the past is honored even when a base-protected mask
/// is supplied (the protected-mask setup runs after the deadline poll, so the
/// loop still returns the identity net promptly).
#[test]
fn test_with_mode_fixpoint_past_deadline_with_protected_mask() {
    let net = reducible_chain_net();
    let protected = vec![true, false, false, false];
    // `now - 1s` cannot underflow the monotonic clock in practice; this is a
    // test fixture constructing a deadline that is already in the past.
    #[allow(clippy::unchecked_time_subtraction)]
    let past = Instant::now() - Duration::from_secs(1);
    ...
    .expect("past deadline with protected mask must return a sound partial net");
    assert_eq!(reduced.net.num_places(), net.num_places());
    assert!(!reduced.report.has_reductions());
}
