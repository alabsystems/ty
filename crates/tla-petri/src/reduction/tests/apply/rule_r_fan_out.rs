...
fn test_rule_r_gated_off_for_ctl_with_next() {
    ...
    let reduced_ctl =
        reduce_iterative_structural_with_mode(&net, &[], ReductionMode::CTLWithNext, None)
            .expect("reduction must succeed");
    assert!(
    ...
    let reduced_stutter =
        reduce_iterative_structural_with_mode(&net, &[], ReductionMode::StutterSensitiveLTL, None)
            .expect("reduction must succeed");
    assert!(
...
/// stays off. Rule R fuses producer/consumer steps and removes intermediate
/// markings, so it belongs on the mode-gated Reachability path only.
///
...
/// before Rule R runs.
#[test]
fn test_rule_r_skips_under_exact_marking_semantics() {
    let net = PetriNet {
        name: None,
        places: vec![
            place("p_a"),      // 0
    ...
    assert!(
        ...
        "Rule R must not fire under ExactMarking"
    );
}
