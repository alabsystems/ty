...
/// Regression for #4303: a Rule H survivor can become isolated in the round
/// after its cycle transitions are dropped. The fixpoint loop must carry the
/// survivor forward as protected before planning that next round; otherwise the
/// absorbed place redirects to a reduced-net index that no longer exists.
#[test]
fn test_rule_h_cross_round_survivor_redirect_stays_live() {
    let net = PetriNet {
        name: Some("rule-h-cross-round-survivor".to_string()),
        places: vec![place("p_survivor"), place("p_absorbed")],
        transitions: vec![
            trans("t_to_absorbed", vec![arc(0, 1)], vec![arc(1, 1)]),
            trans("t_to_survivor", vec![arc(1, 1)], vec![arc(0, 1)]),
        ],
        initial_marking: vec![4, 6],
    };
    ...
    assert_eq!(
        reduced.report.token_cycle_merges.len(),
        1,
        "the first round should apply Rule H"
    );
    assert!(
        !reduced.report.isolated_places.contains(&PlaceIdx(0)),
        "the Rule H survivor must not be removed as isolated in a later round"
    );

    let survivor_idx = reduced.place_map[0].expect("survivor must stay mapped");
    assert_eq!(
        reduced.place_map[1],
        Some(survivor_idx),
        "absorbed place should redirect to the survivor"
    );
    assert!(
        (survivor_idx.0 as usize) < reduced.net.num_places(),
        "absorbed redirect must target a live reduced-net place"
    );
    assert_eq!(
        reduced.net.initial_marking[survivor_idx.0 as usize], 10,
        "survivor carries the aggregate cycle marking"
    );

    let expanded = reduced
        .expand_marking(&reduced.net.initial_marking)
        .expect("absorbed redirect should expand through a live survivor");
    assert_eq!(expanded, vec![10, 10]);
}
