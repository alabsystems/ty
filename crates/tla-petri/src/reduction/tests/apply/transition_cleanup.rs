...
// ---------------------------------------------------------------------------
// Rule K soundness gate: a self-loop arc pair is a TEST ARC
// (MCC 2026 UpperBounds wrong-value cluster regression)
// ---------------------------------------------------------------------------

/// REGRESSION: an equal-weight input+output arc pair gates enabling
/// (`m[p] >= w`) even though its marking effect is zero. Rule K used to strip
/// every such pair unconditionally, letting the transition fire from markings
/// where the original net disables it. The over-approximated reduced net then
/// reached inflated markings, and the UpperBounds pipeline published the
/// inflated maximum as exact (TY=3 vs true 2 on Szymanski-PT-a02, TY=5 vs
/// true 1 on CANInsertWithFailure-PT-005, TY=1 vs true 0 on
/// SimpleLoadBal-PT-05).
///
/// Here `p_gate` is dynamically marked and unmarked (no P-invariant lower
/// bound, not non-decreasing), so the pair on `t_gated` has no
/// never-disabling proof and must be KEPT.
#[test]
fn test_self_loop_arc_without_never_disabling_proof_kept() {
    let net = PetriNet {
        name: Some("rule-k-unproven-test-arc".into()),
        places: vec![
            place("p_idle"),
            place("p_gate"),
            place("p_src"),
        ...
        transitions: vec![
            // p_idle <-> p_gate: the gate is sometimes empty, sometimes marked.
            trans("t_open", vec![arc(0, 1)], vec![arc(1, 1)]),
            trans("t_close", vec![arc(1, 1)], vec![arc(0, 1)]),
            // t_gated tests the gate: self-loop pair on p_gate.
            trans(
                "t_gated",
                vec![arc(2, 1), arc(1, 1)],
                vec![arc(3, 1), arc(1, 1)],
    ...
    let arcs =
        crate::reduction::analysis_transitions::find_self_loop_arcs(&net, &[], &[], &[false; 4]);
    assert!(
        arcs.is_empty(),
        "a self-loop pair on an emptiable gate place is a test arc and must \
         not be stripped without a never-disabling proof: {arcs:?}"
    );
}

/// Positive control for the Rule K gate: when a P-invariant proves the gate
/// place can never drop below the pair weight, the pair never disables the
/// transition and stripping it is exact, so it must still be found.
///
/// Invariants: p_hold + p_slot = 1 (so ub(p_hold) = 1) and
/// p_gate + p_hold = 2, giving lb(p_gate) = 2 - 1 = 1 >= weight 1, even
/// though t_take can decrease p_gate.
#[test]
fn test_self_loop_arc_with_invariant_lower_bound_proof_stripped() {
    let net = PetriNet {
        name: Some("rule-k-invariant-proof".into()),
        places: vec![
            place("p_gate"),
            place("p_hold"),
            place("p_slot"),
        ...
        transitions: vec![
            trans("t_take", vec![arc(0, 1), arc(2, 1)], vec![arc(1, 1)]),
            trans("t_put", vec![arc(1, 1)], vec![arc(0, 1), arc(2, 1)]),
            // t_gated tests the gate: self-loop pair on p_gate.
            trans(
                "t_gated",
                vec![arc(3, 1), arc(0, 1)],
        ...
        initial_marking: vec![2, 0, 1, 1, 0],
    };

    let arcs =
        crate::reduction::analysis_transitions::find_self_loop_arcs(&net, &[], &[], &[false; 5]);
    assert_eq!(
        arcs.len(),
        1,
        "the proven never-disabling pair should be strippable: {arcs:?}"
    );
    assert_eq!(arcs[0].transition, TransitionIdx(2));
    assert_eq!(arcs[0].place, PlaceIdx(0));
    assert_eq!(arcs[0].weight, 1);
}

/// Exhaustive BFS max-token count for one place — tiny nets only.
fn reachable_max_tokens(net: &PetriNet, place: usize) -> u64 {
    ...
    let mut max = net.initial_marking[place];

    while let Some(marking) = queue.pop_front() {
        for t in &net.transitions {
            let mut required: std::collections::BTreeMap<u32, u64> =
                std::collections::BTreeMap::new();
            for arc in &t.inputs {
                *required.entry(arc.place.0).or_default() += arc.weight;
            }
            if required.iter().any(|(&p, &w)| marking[p as usize] < w) {
                continue;
            }
            ...
            max = max.max(next[place]);
            assert!(seen.len() < 100_000, "test net unexpectedly large");
...
/// REGRESSION (end-to-end defect shape): query-relevant structural reduction
/// must not inflate the reachable bound of the query place.
///
/// The only way to mark p_src is to burn the gate first, so t_gated is never
/// enabled and max m[p_dst] = 0 in the original net. Old Rule K stripped the
/// test-arc pair on p_gate, after which the reduced net fired
/// burn → late → gated and reached p_dst = 1 — the inflated value the
/// UpperBounds BFS then published as exact.
#[test]
fn test_query_reduction_preserves_reachable_bound_of_gated_place() {
    let net = PetriNet {
        name: Some("rule-k-bound-inflation".into()),
        places: vec![
            place("p_gate"),
            place("p_sink"),
            place("p_src"),
        ...
        transitions: vec![
            trans("t_burn", vec![arc(0, 1)], vec![arc(1, 1)]),
            trans("t_late", vec![arc(1, 1)], vec![arc(2, 1)]),
            trans(
                "t_gated",
                vec![arc(2, 1), arc(0, 1)],
                vec![arc(3, 1), arc(0, 1)],
    ...
    let original_max = reachable_max_tokens(&net, 3);
    assert_eq!(original_max, 0, "t_gated must never be enabled originally");

    // p_dst is the UpperBounds query place.
    let protected = vec![false, false, false, true];
    let reduced = reduce_iterative_structural_query_with_protected(&net, &protected)
        .expect("reduction should succeed");

    let dst = reduced.place_map[3].expect("protected query place must survive");
    let reduced_max = reachable_max_tokens(&reduced.net, dst.0 as usize);
    assert_eq!(
        reduced_max, original_max,
        "query-relevant reduction inflated the reachable bound of the query \
         place (Rule K stripped an unproven test arc)"
    );
}
...
fn test_reduce_removes_parallel_place() {
    ...
    // The duplicate's arcs must be DROPPED, not summed onto the canonical:
    // every arc on the merged place keeps its original weight 1. (The old
    // redirect-and-sum materialization produced weight 2 against an
    // un-doubled initial marking, distorting enabledness for m0 > 0.)
    for t in &reduced.net.transitions {
        for a in t.inputs.iter().chain(t.outputs.iter()) {
            if a.place == canonical {
                assert_eq!(
                    a.weight, 1,
                    "arc weight on merged canonical must stay 1 (not doubled) for {}",
                    t.id
                );
            }
        }
    }
    // The merged place carries the canonical's own initial marking.
    assert_eq!(
        reduced.net.initial_marking[canonical.0 as usize], 2,
        "canonical keeps its own initial marking"
    );

    // Behavior preservation: t0 is enabled initially in the original
    // (p0=2 >= 1, p1=2 >= 1, p2=3 >= 1) and must stay enabled in the
    // reduced net at the reduced initial marking.
    let reduced_t0 = reduced.transition_map[0].expect("t0 must survive");
    assert!(
        reduced
            .net
            .is_enabled(&reduced.net.initial_marking, reduced_t0),
        "t0 must remain enabled at the reduced initial marking \
         (the summed-weight materialization disabled it)"
    );
}
