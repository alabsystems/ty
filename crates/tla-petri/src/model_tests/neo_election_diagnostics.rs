...
fn require_neo_election_col2() -> Option<std::path::PathBuf> {
...
fn test_neo_election_rf10_predicate_initial_marking() {
    let Some(dir) = require_neo_election_col2() else {
        return;
    };
    if require_mcc_input_dir("NeoElection-PT-2").is_none() {
        return;
    }
        ...
        if !unreduced_set.contains(&expanded) {
            ...
            if not_in_unreduced <= 3 {
                let diff_places: Vec<(usize, u64)> = expanded
                    ...
                    .map(|(i, &v)| (i, v))
                    .collect();
                // English diagnostic, not an MCC output line.
                // mcc-keyword-guard: allow-spaced-mention
                eprintln!(
