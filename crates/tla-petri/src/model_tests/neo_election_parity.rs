...
use super::*;
use fixtures::{mcc_input_dir, require_mcc_input_dir};
use neo_election_ground_truth::NEO_ELECTION_COL2_RC_TRUTH;
use neo_election_ground_truth::NEO_ELECTION_COL2_RF_TRUTH;

fn require_col_pt_inputs() -> Option<()> {
    require_mcc_input_dir("NeoElection-COL-2")?;
    require_mcc_input_dir("NeoElection-PT-2")?;
    Some(())
}
...
fn test_neo_election_col_pt_place_id_parity() {
    if require_col_pt_inputs().is_none() {
        return;
    }
...
fn test_neo_election_rf_pt_ground_truth() {
    ...
    let Some(dir) = require_mcc_input_dir("NeoElection-PT-2") else {
        return;
    };
...
fn test_neo_election_execution_parity_reachability_cardinality() {
    ...
    for (col_rec, pt_rec) in col_records.iter().zip(pt_records.iter()) {
        ...
        if let (Some(col_v), Some(pt_v)) = (col_verdict, pt_verdict) {
            if col_v != pt_v {
                eprintln!("parity drift: RC-{idx:02}: COL={col_v:?} PT={pt_v:?} truth={truth}");
                pt_disagreements += 1;
            }
        }
    }
