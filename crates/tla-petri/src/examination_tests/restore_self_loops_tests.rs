...
fn test_deadlock_verdict_skips_blocked_constant_self_loop() {
    ...
    assert!(
        reduced.place_map[0].is_some() || reduced.constant_value(PlaceIdx(0)) == Some(1),
        "self-loop restoration needs p_const through place_map or constant_value"
    );
...
fn test_deadlock_verdict_restores_constant_self_loop() {
    ...
    assert!(
        reduced.place_map[0].is_some() || reduced.constant_value(PlaceIdx(0)) == Some(1),
        "self-loop restoration needs p0 through place_map or constant_value"
    );
...
fn test_quasi_liveness_restores_self_loop() {
    ...
    assert!(
        reduced
            ...
            .contains(&crate::petri_net::TransitionIdx(0)),
        "t_loop should be recognized as a restorable self-loop"
    );
    ...
    assert!(
        reduced
            ...
            || reduced
                ...
                .self_loop_transitions
                .contains(&crate::petri_net::TransitionIdx(0)),
        "blocked self-loop may be proven dead before self-loop restoration"
    );
...
fn test_liveness_restores_self_loop_in_live_net() {
    ...
    assert!(
        reduced
            .report
            .self_loop_transitions
            .contains(&crate::petri_net::TransitionIdx(2)),
        "t_loop should be recognized as a restorable self-loop"
    );
...
fn test_deadlock_self_loop_mixed_constant_and_active_places() {
    ...
    assert_eq!(reduced.report.self_loop_transitions.len(), 1);
    assert!(
        reduced.place_map[0].is_some() || reduced.constant_value(PlaceIdx(0)) == Some(3),
        "self-loop restoration needs p_const through place_map or constant_value"
    );
