...
fn require_mcc_input_dir(model: &str) -> Option<PathBuf> {
...
fn sorted_isolated_place_ids(model_name: &str) -> Option<Vec<String>> {
    let dir = require_mcc_input_dir(model_name)?;
    let colored = crate::hlpnml::parse_hlpnml_dir(&dir).expect("model should parse");
...
fn test_neo_election_rc03_full_pipeline_returns_false() {
    ...
    let model = load_model_dir(dir).expect("NeoElection-COL-2 should load");
...
fn simplified_rf_tracker(
    model_name: &str,
    property_suffix: &str,
) -> Option<(crate::model::PreparedModel, PropertyTracker)> {
    let dir = require_mcc_input_dir(model_name)?;
...
fn test_neo_election_rf09_collapsed_bfs_stays_false_with_and_without_por() {
    let Some((model, tracker)) = simplified_rf_tracker("NeoElection-COL-2", "09") else {
        return;
    };
...
fn test_other_neo_election_variants_skip_isolated_place_collapsing() {
    for model_name in OTHER_NEO_ELECTION_ISOLATED_PLACE_MODELS {
        let Some(isolated) = sorted_isolated_place_ids(model_name) else {
            continue;
        };
        assert_eq!(
            isolated,
        ...
        let dir = mcc_input_dir(model_name);
        let mut colored =
            crate::hlpnml::parse_hlpnml_dir(&dir).expect("NeoElection sibling HLPNML should parse");
        let report = crate::colored_reduce::reduce_colored(&mut colored);
        eprintln!(
...
fn test_production_vs_uncollapsed_firability_agreement() {
    ...
    let production_model = load_model_dir(&dir).expect("NeoElection-COL-2 should load");
