...
fn test_stubborn_preserves_deadlock_reachability() {
    ...
    let next = net.fire(&net.initial_marking, ts[0]).expect("fire (test)");
    ...
    let final_marking = net.fire(&next, remaining).expect("fire (test)");
...
fn test_d1_d2_hand_traced_four_place_pipeline() {
    ...
    // interferes_with: t0⊥t2 (no shared input), t1⊥t2 (no shared input), t0⊥t1 (no shared input)
    assert!(
        !dep.interferes_with[0].contains(&TransitionIdx(2)),
        "t0 does not interfere t2"
    );
    assert!(
    ...
    assert!(
        ...
        "t2 does not interfere t0"
    );
    assert!(
        !dep.interferes_with[2].contains(&TransitionIdx(1)),
        "t2 does not interfere t1"
    );
...
fn test_d1_d2_hand_traced_pipeline_all_successors_reach_deadlock() {
    ...
    while let Some(m) = queue.pop_front() {
        ...
        for t in to_fire {
            let next = net.fire(&m, t).expect("fire (test)");
...
fn bfs_por(net: &PetriNet, strategy: &PorStrategy) -> BfsResult {
    ...
    while let Some(marking) = queue.pop_front() {
        ...
        for trans in to_fire {
            let next = net.fire(&marking, trans).expect("fire (test)");
