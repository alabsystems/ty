...
#[test]
fn test_free_choice_incomplete_enumeration_not_falsely_deadlock_free() {
    // ORDINARY FREE-CHOICE net where single-place-seed siphon enumeration is
    // INCOMPLETE, so the old code emitted a WRONG `Some(true)`.
    //
    //   t0: {p1,p2} -> {p0,p3}
    //   t1: {p0,p3} -> {p1,p2}
    //   initial = [0,0,1,2]
    //
    // Free-choice (each place has a unique consumer), ordinary (weight 1).
    // At the initial marking p0=p1=0, so t0 (needs p1) and t1 (needs p0) are
    // BOTH disabled — the initial marking is a DEADLOCK. Ground truth: NOT
    // deadlock-free.
    //
    // The single-place-seed heuristic enumerates minimal siphons
    // {p0,p2},{p1,p3},{p2,p3} — all of which happen to contain a marked trap
    // (p2,p3 are marked) — but MISSES the minimal siphon {p0,p1}, which is
    // initially unmarked and has NO marked trap. The old code therefore
    // certified "all minimal siphons covered" => deadlock-free (WRONG).
    //
    // Because every transition here has TWO input places, the enumeration is
    // not provably complete, so the fixed code declines the positive shortcut
    // and returns None (→ exact path), never the wrong `Some(true)`.
    let net = PetriNet {
        name: None,
    ...
    // The net is free-choice, so the OLD guard (is_ordinary_free_choice) did
    // NOT reject it. The fix is the completeness gate: it must NOT report
    // deadlock-free.
    assert!(is_ordinary_free_choice(&net));
    assert_ne!(
        structural_deadlock_free(&net),
        Some(true),
        "incomplete free-choice siphon enumeration must not falsely certify \
         deadlock-freedom (the net deadlocks at its initial marking)"
    );
    // Specifically, it declines to the exact path.
    assert_eq!(structural_deadlock_free(&net), None);
}

#[test]
fn test_single_input_net_still_uses_sound_shortcut() {
    // Coverage-preservation guard: a net where every transition has exactly
    // one input place keeps the (now provably complete) shortcut. This mutex
    // cycle is deadlock-free and the fix must still return Some(true) via the
    // structural path, not regress to None.
    let net = PetriNet {
        name: None,
        places: vec![place("p0"), place("p1")],
        transitions: vec![
