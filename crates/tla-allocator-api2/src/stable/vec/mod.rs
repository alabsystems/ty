...
impl<T, A: Allocator> Vec<T, A> {
    ...
    unsafe fn append_elements(&mut self, other: *const [T]) {
        // Use the raw-pointer inherent `len()` (stable since 1.79) rather
        // than `(*other).len()` to avoid the `dangerous_implicit_autorefs`
        // lint (deny-by-default on current stable). Upstream allocator-api2
        // 0.2.21 predates the lint.
        let count = other.len();
        self.reserve(count);
        let len = self.len();
