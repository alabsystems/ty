...
/// THE single raw env-mutation site for this test binary — `EnvVarGuard` (the
/// only env mutator here) routes every set/remove through it. A raw
/// `set_var`/`remove_var` must exist somewhere to implement the primitive; this
/// one lock-scoped helper is that somewhere. `env_mutation` is the Trust
...
impl EnvVarGuard {
    fn set_many(vars: &[(&'static str, &'static str)]) -> Self {
        ...
        for (key, value) in vars {
            ...
            raw_env_write(key, Some(std::ffi::OsStr::new(value)));
        }
        Self {
            entries,
            lock: Some(lock),
...
====",
    ...
    let _guard = EnvVarGuard::set_many(&[("TY_BYTECODE_VM", "1"), ("TY_BYTECODE_VM_STATS", "1")]);
