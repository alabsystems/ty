...
///
/// Used by test code and callers without ctx access.
#[allow(dead_code)] // Used in tests (hoist_generation.rs) and by external crates
...
/// RAII scope installing the implied-action transition fingerprints
/// `(parent_fp, successor_fp)` on the evaluation runtime.
///
/// While installed (and outside ENABLED scope, kill switch
/// `TY_NO_IMPLIED_FP_CACHE` off), zero-arg state-level operator evaluations
/// participate in the fingerprint-keyed transition cache
/// (`cache::zero_arg_cache` transition partition): a derived state function
/// such as a refinement mapping (`token` / `pending` in EWD998PCal) is
/// computed once per state and revalidated by dep values on every reuse,
/// instead of being re-evaluated for every checked transition.
///
/// The caller MUST have the parent state bound as `state_env` (fingerprint
/// `parent_fp`) and the successor bound as `next_state_env` (fingerprint
/// `succ_fp`) for the lifetime of the scope. A fingerprint that does not
/// match its bound array cannot produce a wrong value (every hit revalidates
/// all recorded dep values against the bound arrays) — only wasted entries.
///
/// Restores the previous fingerprint pair on drop (supports nesting).
pub struct ImpliedFpScope {
    runtime_state: std::rc::Rc<crate::cache::EvalRuntimeState>,
    prev: Option<(u64, u64)>,
}

impl Drop for ImpliedFpScope {
    fn drop(&mut self) {
        self.runtime_state.implied_fp_pair.set(self.prev);
    }
}

/// Install the implied-action fingerprint pair for the duration of the
/// returned guard. See [`ImpliedFpScope`].
pub fn implied_fp_scope(ctx: &EvalCtx, parent_fp: u64, succ_fp: u64) -> ImpliedFpScope {
    let runtime_state = std::rc::Rc::clone(&ctx.runtime_state);
    let prev = runtime_state.implied_fp_pair.get();
    runtime_state
        .implied_fp_pair
        .set(Some((parent_fp, succ_fp)));
    ImpliedFpScope {
        runtime_state,
        prev,
    }
...
pub fn eval_entry_with<T>(ctx: &EvalCtx, eval_fn: impl FnOnce() -> EvalResult<T>) -> EvalResult<T> {
    ...
    // the guard clears stale substitution entries. On success we disarm before
    // updating the context-local trim counter below.
...
/// TY uses Value::Closure with empty params as the lazy thunk representation.
        ...
        if c.params().is_empty() {
            ...
            let force_ctx = {
                let mut fctx = ctx.clone();
                {
                    ...
                    // Compute the scope id (and recursive flag) eagerly (one HAMT walk
                    // per force) instead of INVALIDATED (a walk on every cache lookup
                    // inside the body). The captured local_ops Arc is immutable; the
                    // values are identical to recomputation.
                    let (forced_local_ops_id, forced_local_ops_recursive) = match &s.local_ops {
                        Some(ops) => crate::cache::openv_memo::scope_id_and_recursive_memoized(
                            ops,
                            crate::cache::openv_memo::ScopeIdSite::Lifecycle,
                        ),
                        None => (0, false),
                    };
                    s.scope_ids.local_ops = forced_local_ops_id;
                    s.scope_ids.local_ops_recursive = forced_local_ops_recursive;
                }
