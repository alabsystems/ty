...
fn test_state_var_apply_plain_uses_direct_borrowed_slot() {
    ...
    let func_val = Value::IntFunc(Rp::new(IntIntervalFunc::new(
        1,
        2,
...
fn test_state_var_apply_local_binding_shadows_state_slot() {
    ...
    ctx.register_var("f");
    // f in state: f[1] = 10
    let func_val = Value::IntFunc(Rp::new(IntIntervalFunc::new(1, 1, vec![Value::int(10)])));
    let state = vec![func_val];
    ctx.bind_state_array(&state);

    // Shadow f with a different function: f[1] = 99
    let shadow = Value::IntFunc(Rp::new(IntIntervalFunc::new(1, 1, vec![Value::int(99)])));
...
fn test_materialized_read_record_access_apply_borrows_function_result() {
    ...
    let mut ctx = EvalCtx::new();
    ctx.register_var("r");
    let func = Value::IntFunc(Rp::new(IntIntervalFunc::new(
        1,
        2,
        vec![Value::int(10), Value::int(20)],
    )));
...
fn test_materialized_read_apply_record_access_borrows_record_field() {
    ...
    let mut ctx = EvalCtx::new();
    ctx.register_var("f");
    let state = vec![Value::IntFunc(Rp::new(IntIntervalFunc::new(
        1,
        1,
        vec![Value::Record(
