...
fn test_tlc_eval_passthrough() {
    // TLCEval just evaluates its argument (TY is already eager)
...
fn test_atoi_numeric_string() {
    ...
    assert_eq!(v, Value::Int(Rp::new(42.into())));
}

#[test]
fn test_atoi_negative_string() {
    let v = eval_str("atoi(\"-7\")").unwrap();
    assert_eq!(v, Value::Int(Rp::new((-7).into())));
}
...
fn test_atoi_non_numeric_string_errors() {
    ...
    // TY previously returned 0 silently via .unwrap_or(0).
