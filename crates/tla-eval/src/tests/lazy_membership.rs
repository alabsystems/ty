...
"#;
    ...
    let gap_domain = Value::Func(Rp::new(FuncValue::from_sorted_entries(vec![
        (Value::int(1), Value::string("a")),
    ...
    let wrong_range = Value::Func(Rp::new(FuncValue::from_sorted_entries(vec![
        (Value::int(1), Value::string("a")),
...
fn test_set_contains_with_ctx_tuple_seq_and_intfunc_use_interval_domain_helper() {
    let ctx = EvalCtx::new();
    let func_set = Value::FuncSet(FuncSetValue::new(
        Value::Interval(Rp::new(IntervalValue::new(1.into(), 2.into()))),
        Value::set([Value::string("a"), Value::string("b")]),
    ));
    ...
    let intfunc = Value::IntFunc(Rp::new(IntIntervalFunc::new(
        1,
        2,
    ...
    let shifted_intfunc = Value::IntFunc(Rp::new(IntIntervalFunc::new(
        2,
        3,
        vec![Value::string("a"), Value::string("b")],
    )));
...
/// N6: `v \in Seq(S)` must dispatch on what `Seq` *resolves to*, not on the name
/// `Seq` syntactically. A module-defined `Seq(S) == {<<>>}` shadows the built-in,
/// so membership must agree with application and report no false violation.
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_user_defined_seq_shadows_builtin_membership_n6() {
    use tla_core::{lower, parse_to_syntax_tree, FileId};

    let src = r#"
---- MODULE Test ----
Seq(S) == {<<>>}
NotMember == <<1>> \in Seq({1})
InMember == <<>> \in Seq({1})
Inv == ~(<<1>> \in Seq({1}))
AppliedEq == Seq({1}) = {<<>>}
====
"#;
    ...
    // With the user-defined `Seq(S) == {<<>>}`, only <<>> is a member — the
    // built-in "all finite sequences" semantics must NOT apply.
    assert_eq!(
        ctx.eval_op("NotMember").unwrap(),
        Value::Bool(false),
        "N6: <<1>> is not in the user-defined Seq({{1}}) = {{<<>>}}"
    );
    assert_eq!(
        ctx.eval_op("InMember").unwrap(),
        Value::Bool(true),
        "N6: <<>> IS in the user-defined Seq({{1}}) = {{<<>>}}"
    );
    // The reported defect: this invariant must hold (no false counterexample).
    assert_eq!(
        ctx.eval_op("Inv").unwrap(),
        Value::Bool(true),
        "N6: user-defined Seq must not produce a false invariant violation"
    );
    // Membership MUST agree with plain application of the same operator.
    assert_eq!(
        ctx.eval_op("AppliedEq").unwrap(),
        Value::Bool(true),
        "N6: applying the user-defined Seq must yield {{<<>>}}"
    );
}

/// N6: a cfg operator replacement (`CONSTANT Seq <- MySeq`) must redirect
/// `Seq(S)` membership to the replacement operator, not the built-in.
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_cfg_seq_replacement_membership_n6() {
    use tla_core::{lower, parse_to_syntax_tree, FileId};
...
"#;
    ...
    let mut ctx = EvalCtx::new();
    ctx.load_module(&module);
    // Mirror `CONSTANT Seq <- MySeq` from a .cfg file.
    ctx.add_op_replacement("Seq".to_string(), "MySeq".to_string());

    assert_eq!(
        ctx.eval_op("NotMember").unwrap(),
        Value::Bool(false),
        "N6: `Seq <- MySeq` must redirect membership to MySeq = {{<<>>}}"
    );
    assert_eq!(ctx.eval_op("InMember").unwrap(), Value::Bool(true));
    assert_eq!(
        ctx.eval_op("Inv").unwrap(),
        Value::Bool(true),
        "N6: cfg operator replacement must not produce a false invariant violation"
    );
}
