...
#[test]
fn test_arith_exponentiation_huge_exponent_declines() {
    // Regression (audit bug #9): `2 ^ 4000000000` has an exponent that fits in
    // u32 but whose result would be ~4e9 bits (~500 MB). The evaluator must
    // DECLINE with an EvalError BEFORE calling pow(), never OOM/abort.
    // The cap check rejects on the estimated result size, so no large
    // allocation is attempted.
    let err = eval_str("2 ^ 4000000000").unwrap_err();
    match err {
        crate::error::EvalError::ArgumentError { op, position, .. } => {
            assert_eq!(op, "^");
            assert_eq!(position, "second");
        }
        other => panic!("expected ArgumentError decline, got: {other:?}"),
    }
}

#[test]
fn test_arith_exponentiation_one_base_huge_exponent_ok() {
    // 1 ^ huge stays size-1 (bits() <= 1), so the cap must NOT reject it: the
    // fail-closed bound only fires when the result actually grows.
    let v = eval_str("1 ^ 4000000000").unwrap();
    assert_eq!(v, Value::SmallInt(1));
}
