...
InTrue == 2 \\in {1, 2, 3}
InFalse == 5 \\in {1, 2, 3}
TupleSeqTrue == <<1, 2>> \\in {Append(<<1>>, 2)}
TupleSeqFalse == <<1, 3>> \\in {Append(<<1>>, 2)}
TupleRuntimeSetPred == LET S(dummy) == {x \\in {<<1, 2>>}: TRUE}
                       IN <<1, 2>> \\in S(0)
...
BigUnionSingletonLazy == UNION {[0..1 -> Nat]}
...
#[test]
fn test_tir_big_union_singleton_preserves_lazy_inner_set() {
    let module = parse_module(POWERSET_MODULE);
    let result = eval_tir_op(&module, "BigUnionSingletonLazy");
    assert!(
