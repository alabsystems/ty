...
#[test]
fn test_directed_subgraph_huge_universe_declines() {
    // Regression (audit bug #5): the subset enumerator computed `1u64 << n`,
    // which panics on shift overflow for n >= 64 and OOMs for moderately large
    // n. A 64-node universe must DECLINE with an EvalError (SetTooLarge),
    // never panic. The cap rejects before any 2^n allocation, so this test
    // does NOT enumerate 2^64 subsets.
    let result = eval_str("DirectedSubgraph([node |-> 1..64, edge |-> {}])");
    match result {
        Err(EvalError::SetTooLarge { .. }) => {}
        other => panic!("expected SetTooLarge decline, got: {other:?}"),
    }
}

#[test]
fn test_undirected_subgraph_huge_universe_declines() {
    // Same shift-overflow class via the undirected path.
    let result = eval_str("UndirectedSubgraph([node |-> 1..70, edge |-> {}])");
    match result {
        Err(EvalError::SetTooLarge { .. }) => {}
        other => panic!("expected SetTooLarge decline, got: {other:?}"),
    }
