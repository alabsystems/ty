...
#[test]
fn test_mkseq_huge_length_declines() {
    // Regression (audit bug #19): MkSeq does `Vec::with_capacity(n as usize)`
    // from an unbounded user `n`. A huge N (here ~9.2e18, i64::MAX) would abort
    // the process via the over-large with_capacity. It must DECLINE with an
    // EvalError BEFORE allocating. The cap rejects before any with_capacity, so
    // this test does NOT allocate gigabytes.
    let err = eval_with_ops("Id(i) == i", "MkSeq(9223372036854775807, Id)").unwrap_err();
    match err {
