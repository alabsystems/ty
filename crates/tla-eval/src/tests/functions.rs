...
fn test_eval_except_three_level_path_spill_preserves_nested_siblings() {
    ...
    let a = outer
        ...
        .expect("a should remain a record");
    let b = a
        .get(&Arc::from("b"))
        .and_then(Value::as_record)
        .expect("b should remain a record");

    assert_eq!(b.get(&Arc::from("c")), Some(&Value::int(7)));
    assert_eq!(b.get(&Arc::from("keep")), Some(&Value::int(9)));
}
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_eval_func_def_subset_domain_sorted() {
    // Regression: SUBSET domains iterate cardinality-first (e.g. {2} before
    // {1, 2}), which is NOT Value::cmp order. The constructed FuncValue must
    // still have a strictly cmp-sorted domain, or application reports a bogus
    // NotInDomain and equality reports false invariant violations.
    assert_eq!(
        eval_str(r"[s \in SUBSET {1, 2} |-> Cardinality(s)][{1, 2}]").unwrap(),
        Value::int(2)
    );
    assert_eq!(
        eval_str(r"[s \in SUBSET {1, 2} |-> Cardinality(s)][{}]").unwrap(),
        Value::int(0)
    );
    assert_eq!(
        eval_str(r"[s \in SUBSET {1, 2} |-> Cardinality(s)][{2}]").unwrap(),
        Value::int(1)
    );

    // Equality with the same function built over an explicitly enumerated
    // (already sorted) domain must hold — no false violation.
    assert_eq!(
        eval_str(
            r"[s \in SUBSET {1, 2} |-> Cardinality(s)] = [s \in {{}, {1}, {2}, {1, 2}} |-> Cardinality(s)]"
        )
        .unwrap(),
        Value::Bool(true)
    );

    // And with the same function built via TLC's :> / @@ constructors (the
    // original defect-report shape).
    assert_eq!(
        eval_str(
            r"[s \in SUBSET {1, 2} |-> Cardinality(s)] = (({} :> 0) @@ ({1} :> 1) @@ ({2} :> 1) @@ ({1, 2} :> 2))"
        )
        .unwrap(),
        Value::Bool(true)
    );

    // The internal domain must be strictly Value::cmp-sorted.
    let f = eval_str(r"[s \in SUBSET {1, 2} |-> Cardinality(s)]").unwrap();
    let func = f
        .as_func()
        .expect("SUBSET-domain function should be a Func");
    let keys: Vec<&Value> = func.domain_iter().collect();
    assert_eq!(keys.len(), 4);
    assert!(
        keys.windows(2).all(|w| w[0] < w[1]),
        "FuncValue domain not strictly sorted: {keys:?}"
    );
}

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_eval_except_tuple_out_of_domain_is_noop() {
    // TLA+ semantics: [f EXCEPT ![k] = v] with k \notin DOMAIN f is f (TLC
    // warns + no-ops). Tuples are functions with domain 1..n, so an
    // out-of-range index must be a no-op — matching the Seq/Func/IntFunc/
    // Record arms rather than erroring (representation-independent behavior).
    let expected = eval_str(r"<<1, 2>>").unwrap();
    assert_eq!(eval_str(r"[<<1, 2>> EXCEPT ![5] = 0]").unwrap(), expected);
    assert_eq!(eval_str(r"[<<1, 2>> EXCEPT ![0] = 0]").unwrap(), expected);
    assert_eq!(eval_str(r"[<<1, 2>> EXCEPT ![-1] = 0]").unwrap(), expected);

    // In-domain index still updates.
    assert_eq!(
        eval_str(r"[<<1, 2>> EXCEPT ![2] = 9]").unwrap(),
        eval_str(r"<<1, 9>>").unwrap()
    );
}
