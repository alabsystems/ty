...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_unused_lambda_param_does_not_force_infinite_argument_4502() {
    assert_eq!(
        eval_with_ops(
            r#"EXTENDS Naturals
Use(Filter(_)) == Filter(CHOOSE n \in Nat : FALSE)"#,
            "Use(LAMBDA x : TRUE)",
        )
        .unwrap(),
        Value::Bool(true)
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_forall_nat_with_unused_lambda_param_short_circuits_4502() {
    assert_eq!(
        eval_with_ops(
            r#"EXTENDS Naturals
Wrapper(Filter(_, _)) == \A l \in Nat : Filter("state", l) \in BOOLEAN"#,
            "Wrapper(LAMBDA s, l : TRUE)",
        )
        .unwrap(),
        Value::Bool(true)
    );
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_forall_nat_shortcut_rejects_direct_bound_dependency_4502() {
    let err = eval_with_ops("EXTENDS Naturals", r#"\A l \in Nat : l = FALSE"#).unwrap_err();
    assert!(matches!(err, EvalError::SetTooLarge { .. }), "{err:?}");
}

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_forall_nat_shortcut_rejects_used_closure_argument_4502() {
    let err = eval_with_ops(
        r#"EXTENDS Naturals
Wrapper(Filter(_)) == \A l \in Nat : Filter(l)"#,
        "Wrapper(LAMBDA x : x = FALSE)",
    )
    .unwrap_err();
    assert!(matches!(err, EvalError::SetTooLarge { .. }), "{err:?}");
}
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_forall_nat_shortcut_rejects_multi_bound_domains_4502() {
    let err = eval_with_ops("EXTENDS Naturals", r#"\A x \in Nat, y \in {x} : TRUE"#).unwrap_err();
    assert!(matches!(err, EvalError::SetTooLarge { .. }), "{err:?}");
}

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_lazy_closure_argument_still_forces_when_used_4502() {
    let err = eval_with_ops(
        r#"EXTENDS Naturals
Use(F(_)) == F(CHOOSE n \in Nat : FALSE)"#,
        "Use(LAMBDA x : x = x)",
    )
    .unwrap_err();
    assert!(matches!(err, EvalError::SetTooLarge { .. }), "{err:?}");
