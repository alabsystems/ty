...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_instance_substitution_preserves_outer_model_value_node_domain() {
    // EWD998ChanID shape: the outer module has model-value nodes and the instanced
    // module has its own integer Node == 0..4. Substitution RHS evaluation must keep
    // the outer operator scope, so Node2Nat(OuterInbox) applies OuterInbox to model
    // values, not to the inner module's integer Node domain.
    let mod_inner = r#"
---- MODULE IntNodeInner ----
EXTENDS Integers
Node == 0..4
...
OuterInbox == [n \in Node |-> node2nat[n]]
Node2Nat(fcn) == [i \in 0..4 |-> fcn[nat2node[i]]]

Inst == INSTANCE IntNodeInner WITH inbox <- Node2Nat(OuterInbox)
Op == Inst!Op
===="#;

    let module_inner = lower_module_for_instance_test(FileId(0), mod_inner);
    let module_main = lower_module_for_instance_test(FileId(0), mod_main);

    let mut ctx = EvalCtx::new();
    ctx.load_module(&module_main);
    ctx.load_instance_module("IntNodeInner".to_string(), &module_inner);

    let op_def = ctx.get_op("Op").expect("Op not found").clone();
    let value =
        eval(&ctx, &op_def.body).expect("model-value-domain substitution mapping should evaluate");
