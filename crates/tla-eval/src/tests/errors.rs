...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_error_division_inexact() {
    // `/` is TLA+ real division: on integers it is EXACT-OR-ERROR. An inexact
    // quotient must error (never truncate: `7 / 2 = 3` has no TLA+ meaning).
    let result = eval_str("7 / 2");
    let err = result.expect_err("inexact `/` must error");
    assert!(
        matches!(err, EvalError::ArgumentError { ref op, .. } if op == "/"),
        "Expected ArgumentError for `/`, got: {:?}",
        err
    );

    // Exact quotients (positive and negative) still evaluate.
    assert_eq!(eval_str("8 / 2").unwrap(), Value::int(4));
    assert_eq!(eval_str("(0 - 7) / 7").unwrap(), Value::int(-1));
}
