...
#[test]
fn test_membership_tuple_matches_sequence_representation() {
    let v = eval_str("<<1, 2>> \\in {Append(<<1>>, 2)}").unwrap();
    assert_eq!(v, Value::Bool(true));

    let v = eval_str("<<1, 3>> \\in {Append(<<1>>, 2)}").unwrap();
    assert_eq!(v, Value::Bool(false));
}

#[test]
fn test_membership_tuple_preserves_lazy_union_short_circuit() {
    // The invalid right operand must remain unevaluated after the left set hits.
    let v = eval_str("<<1, 2>> \\in ({<<1, 2>>} \\cup 7)").unwrap();
    assert_eq!(v, Value::Bool(true));
}

#[test]
fn test_membership_tuple_falls_back_for_runtime_set_pred() {
    // The operator application hides the lazy SetPred shape from static inspection.
    let v = eval_str("LET S(dummy) == {x \\in {<<1, 2>>} : TRUE} IN <<1, 2>> \\in S(0)").unwrap();
    assert_eq!(v, Value::Bool(true));
}
...
/// TLC calls Assert.fail on such errors; TY must not swallow them.
#[test]
