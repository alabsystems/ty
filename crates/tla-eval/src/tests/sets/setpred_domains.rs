...
#[test]
fn test_tuple_set_filter_materialization_boundary_preserves_semantics() {
    let defs = r#"
EXTENDS FiniteSets
Small == {t \in (1..256) \X {0} : t[1] <= 2}
Large == {t \in (1..257) \X {0} : t[1] <= 2}
"#;

    // Evaluate the filter expressions directly so the zero-argument operator cache
    // does not intentionally materialize a returned SetPred before caching it.
    let small = eval_str(r#"{t \in (1..256) \X {0} : t[1] <= 2}"#).unwrap();
    let large = eval_str(r#"{t \in (1..257) \X {0} : t[1] <= 2}"#).unwrap();
    assert!(
        matches!(small, Value::Set(_)),
        "a TupleSet filter at the 256-element cap should eagerly produce Value::Set, got {small:?}"
    );
    assert!(
        matches!(large, Value::SetPred(_)),
        "a TupleSet filter above the cap should remain Value::SetPred, got {large:?}"
    );

    assert_eq!(
        eval_with_ops(
            defs,
            r#"<<Cardinality(Small), Cardinality(Large),
                <<1, 0>> \in Small, <<1, 0>> \in Large,
                <<3, 0>> \in Small, <<3, 0>> \in Large>>"#,
        )
        .unwrap(),
        Value::tuple([
            Value::int(2),
        ...
        "eager and lazy threshold representations must have identical cardinality and membership semantics"
    );
}

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_tuple_set_filter_materialization_boundary_has_deterministic_error_timing() {
    let eager_error = eval_str(r#"{t \in (1..256) \X {0} : 1 \div 0 = t[1]}"#)
        .expect_err("the eager filter must evaluate its predicate during construction");
    assert!(
        matches!(eager_error, EvalError::DivisionByZero { .. }),
        "expected eager predicate evaluation to report division by zero, got {eager_error:?}"
    );

    let lazy = eval_str(r#"{t \in (1..257) \X {0} : 1 \div 0 = t[1]}"#)
        .expect("the lazy filter should defer predicate evaluation");
    assert!(
        matches!(lazy, Value::SetPred(_)),
        "the above-cap filter must retain the deferred SetPred representation, got {lazy:?}"
    );

    let forced_error = eval_str(r#"Cardinality({t \in (1..257) \X {0} : 1 \div 0 = t[1]})"#)
        .expect_err("forcing the lazy filter must evaluate its predicate");
    assert!(
        matches!(forced_error, EvalError::DivisionByZero { .. }),
        "expected forced lazy predicate evaluation to report division by zero, got {forced_error:?}"
    );
}
