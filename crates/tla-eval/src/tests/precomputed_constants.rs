...
/// INSTANCE-local operators must shadow outer precomputed constants.
///
/// EWD998ChanID has an outer model-value `CONSTANT Node` and the instanced
/// EWD998Chan module defines `Node == 0..N-1`. The fast-path ident hint for the
/// outer constant is `PrecomputedConstant`, but evaluating inside the instance
/// must use the local operator rather than the outer model-value set.
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_instance_local_op_shadows_precomputed_constant_hint() {
    let mut ctx = EvalCtx::new();
    let node_id = intern_name("Node");

    let shared = Arc::make_mut(ctx.shared_arc_mut());
    shared
        .precomputed_constants_mut()
        .insert(node_id, Value::int(999));
    shared.ident_hints = vec![crate::IdentHint::Unknown; node_id.0 as usize + 1];
    shared.ident_hints[node_id.0 as usize] = crate::IdentHint::PrecomputedConstant;

    let mut local_ops = tla_core::OpEnv::new();
    local_ops.insert(
        "Node".to_string(),
        Arc::new(tla_core::ast::OperatorDef {
            name: Spanned::dummy("Node".to_string()),
            params: vec![],
            body: Spanned::dummy(Expr::Int(3.into())),
            local: false,
            contains_prime: false,
            guards_depend_on_prime: false,
            has_primed_param: false,
    ...
    let scoped = ctx.with_instance_scope(local_ops, vec![]);
    let expr = Spanned::dummy(Expr::Ident("Node".to_string(), node_id));
    let result = eval(&scoped, &expr).expect("instance-local Node should evaluate");
    assert_eq!(
        result,
        Value::int(3),
        "instance-local operator must shadow the outer precomputed constant hint"
    );
}
