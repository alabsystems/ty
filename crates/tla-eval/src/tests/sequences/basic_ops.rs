...
#[test]
fn test_all_subseqs_huge_length_declines() {
    // Regression (audit bug #5, AllSubSeqs path): the enumerator computed
    // `1u64 << n` which panics on shift overflow for n >= 64 and OOMs earlier.
    // A 64-element sequence must DECLINE with SetTooLarge before allocating
    // 2^n subsequences — never panic. The cap rejects before any allocation.
    let result = eval_str("AllSubSeqs([i \\in 1..64 |-> i])");
    match result {
