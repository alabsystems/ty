...
fn test_streaming_setpred_over_tuple_set() {
    // (1..257) \X {10} has 257 tuples, one above the eager-reduction cap.
    // Evaluate the filter directly so zero-argument operator caching does not
    // intentionally materialize the SetPred before the streaming consumer sees it.
    let filtered = eval_str(r#"{t \in (1..257) \X {10} : t[1] > 1}"#).unwrap();
    assert!(
        matches!(filtered, Value::SetPred(_)),
        "a 257-element TupleSet source should stay lazy for streaming coverage, got {filtered:?}"
    );

    // 256 tuples pass: <<2,10>> through <<257,10>>.
    assert_eq!(
        eval_with_ops(
            "EXTENDS FiniteSets",
            r#"Cardinality(IF TRUE THEN {t \in (1..257) \X {10} : t[1] > 1} ELSE {})"#,
        )
        .unwrap(),
        Value::int(256),
        "Streaming Cardinality over TupleSet SetPred: 256 of 257 tuples"
    );

    // EXISTS: is the final tuple retained?
    assert_eq!(
        eval_str(
            r#"\E t \in (IF TRUE THEN {x \in (1..257) \X {10} : x[1] > 1} ELSE {}) :
                   t[1] = 257 /\ t[2] = 10"#,
        )
        .unwrap(),
        Value::Bool(true),
    ...
    assert_eq!(
        eval_str(
            r#"\A t \in (IF TRUE THEN {x \in (1..257) \X {10} : x[1] > 1} ELSE {}) :
                   t[1] > 1"#,
        )
        .unwrap(),
        Value::Bool(true),
