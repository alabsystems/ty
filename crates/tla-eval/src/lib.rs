...
// The full public API surface of this crate is documented; keep it that way.
#![deny(missing_docs)]
// Matrix-style operations (Tarjan SCC, dependency matrices, in-degree counters)
// index multiple parallel arrays by row/col. The `for i in 0..n` form mirrors
// the algorithm description.
#![allow(clippy::needless_range_loop)]
// Builders and arena guards leave Default implementation to explicit
// constructors; redundant else blocks document the alternative branch.
#![allow(clippy::redundant_else)]
// Style preferences not worth refactoring:
// - new_without_default: ArenaStateGuard::new() has side effects (begins an
//   arena state scope); Default would silently activate it.
// - explicit_counter_loop: sequence-index validation loops keep an explicit
//   counter parallel to the iterator so the failure message can name the
//   expected index without rebuilding it from enumerate.
// - while_let_loop: chained alias resolution loops use `let Expr::Ident else
//   break` to keep the early-exit and tail logic in the same block.
...
pub use eval_ident_zero_arg::{implied_parent_side_external_probe, state_lookup_mode_is_current};
mod eval_let;
mod eval_membership;
...
pub use self::cache::{
    ...
    clear_let_def_interning, clear_state_scoped_let_and_fold_caches, clear_subst_cache,
...
pub use self::eval_cache_lifecycle::{
    ...
    eval_entry_with, implied_fp_scope, invalidate_next_state_identity_tracking,
    invalidate_state_identity_tracking, invalidate_state_identity_tracking_with_ctx, stack_safe,
    ImpliedFpScope,
};

/// Number of entries in the fingerprint-keyed implied-action transition cache
/// on the current thread (observability for tests and diagnostics).
pub fn implied_transition_cache_len() -> usize {
    crate::cache::zero_arg_cache::zero_arg_transition_cache_entry_count()
}
...
pub use self::helpers::{
    ...
    prepare_registered_named_module_ref_call, push_bound_var_mut,
    registered_named_module_ref_dispatch_is_direct,
};
pub use self::helpers::{has_complete_builtin_override, should_prefer_builtin_override};
