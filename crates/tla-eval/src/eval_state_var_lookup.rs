...
use super::{
    ...
    EvalResult, StateLookupMode, SubstGuardEntry,
};
...
pub(super) fn force_lazy_binding(
    ...
    let mut eval_ctx = if is_instance {
        ...
        // Restore the lazy binding's definition-site local_ops (before inner
        // module merge). The forcing context may belong to a later nested
        // INSTANCE scope, so its pre_scope_local_ops is not a stable source of
        // truth for this lazy substitution.
        s.local_ops = lazy.def_site_local_ops.clone();
        ...
        // Clear the stale cached recursive flag so the INVALIDATED resolve path
        // recomputes it from the restored content via compute_local_ops_scope_id.
    ...
    let _subst_guard = if is_instance {
        ...
        && ctx.call_by_name_subs().is_none()
    {
        if let Some(state_env) = ctx.state_env {
            ...
            tla_value::churn_stats::churn_count(
                tla_value::churn_stats::ChurnSite::StateVarReadAstStateVar,
            );
...
/// Borrow a heap-backed `StateVar` value directly from the state array — the
/// truly borrowing variant of [`try_borrow_plain_state_var_slot`].
///
/// Churn elimination (top-ranked site on btree: 107M heap-backed state-var
/// read clones): read-only consumers (function application bases) can consume
/// the slot's boxed Value in place instead of cloning it out (Arc bump) and
/// dropping it moments later.
///
/// Guard conditions (each mirrors a resolution tier of `eval_state_var` that
/// could shadow the direct array read — any of them → `None`, caller falls
/// back to the unchanged owned flow):
/// - a binding-chain entry for this name (Local/LET/Instance/Liveness);
///   read-only probe, so falling back never duplicates side effects
/// - INSTANCE substitutions / eager substitution bindings / call-by-name
///   substitutions active (covers explicit AND implicit substitution tiers —
///   `eval_statevar_implicit_instance_substitution` requires
///   `instance_substitutions().is_some()`)
/// - inline scalar slot (owned materialization is free anyway)
///
/// In Next mode a bound sparse next-state slot wins (mirroring step 3.8 of
/// `eval_state_var`), with the same `record_next_read` bookkeeping. The
/// terminal array read performs the same `record_state_read` the owned path
/// performs, so dep tracking is identical.
pub(crate) fn try_borrow_plain_state_var_ref<'a>(
    ctx: &'a EvalCtx,
    name: &str,
    idx: u16,
    name_id: NameId,
    ...
    if !ctx.bindings.is_empty() {
        ...
        if ctx.bindings.lookup(lookup_name_id).is_some() {
            return None;
        }
    }
    ...
    // Sparse next-state overlay active (ENABLED WitnessState in Next mode):
    // fall back to the owned flow. The owned `try_borrow_materialized_state_var`
    // this fast path shadows does NOT consult the sparse overlay before its
    // terminal array read, so resolving the overlay here would CHANGE behavior.
    // Bail out instead — byte-identical semantics, borrowed or not.
    if current_state_lookup_mode(ctx) == crate::cache::StateLookupMode::Next
        && ctx.sparse_next_state_env.is_some()
    {
        return None;
    }
    ...
    debug_assert!(resolved_idx.as_usize() < state_env.env_len());
    // SAFETY: `resolved_idx` is validated against the current VarRegistry by
    // `resolve_state_var_slot`; the state array outlives this evaluation
    // (same contract as `get_value`).
    let v = unsafe { state_env.get_heap_value_ref(resolved_idx.as_usize())? };
    record_state_read(ctx, resolved_idx, v);
    Some(v)
}
...
pub(crate) fn try_borrow_materialized_state_var(
    ...
    let binding = if crate::cache::is_subst_guarded(lookup_name_id) {
        ctx.bindings.lookup_skip_instance(lookup_name_id)
    } else {
        ctx.bindings.lookup(lookup_name_id)
    };
    if let Some((bv, source)) = binding {
