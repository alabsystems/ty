...
// Performance profiling counters for eval() - enabled via TY_PROFILE_EVAL env var.
...
feature_flag!(pub(super) profile_eval, "TY_PROFILE_EVAL");
debug_flag!(pub(super) debug_prime, "TY_DEBUG_PRIME"); // #246
debug_flag!(pub(super) debug_unchanged, "TY_DEBUG_UNCHANGED"); // #295
debug_flag!(pub(super) debug_instance_subst, "TY_DEBUG_INSTANCE_SUBST"); // #295
debug_flag!(pub(super) debug_module_ref, "TY_DEBUG_MODULE_REF"); // #295
debug_flag!(pub(super) debug_or_eval, "TY_DEBUG_OR");
debug_flag!(pub(super) debug_zero_cache, "TY_DEBUG_ZERO_CACHE");
...
///
/// In TLC, LazyValue caching is scoped to a single evaluation. In TY, we use dependency-based
...
///
/// Set TY_NO_LOCAL_OPS_CACHE=1 to bypass caching entirely for debugging.
#[cfg(debug_assertions)]
pub(super) fn no_local_ops_cache() -> bool {
    use std::sync::OnceLock;
    static FLAG: OnceLock<bool> = OnceLock::new();
    *FLAG.get_or_init(|| {
        let enabled = std::env::var("TY_NO_LOCAL_OPS_CACHE").is_ok();
        if enabled {
            eprintln!("[INFO] TY_NO_LOCAL_OPS_CACHE enabled - bypassing zero-arg operator cache");
        }
        enabled
    })
}
...
pub fn print_eval_profile_stats() {
    // Part of #3805/#4391: Always print cache stats if enabled (separate flags).
