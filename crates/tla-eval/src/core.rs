...
pub struct SharedCtx {
    ...
    /// Per-state successor materialization cap (audit finding #12): the maximum
    /// number of successors a single state may materialize on the batch path
    /// before enumeration aborts fail-closed with `SetTooLarge`. `None` disables
    /// the cap. Defaults to the runaway-memory guard (~50M); tla-check overrides
    /// this from the model `Config` (which honors `TY_PER_STATE_SUCCESSOR_CAP`)
    /// when it builds the evaluation context, so the value is per-checker and
    /// never a process-global — keeping concurrent model-check tests isolated.
    pub per_state_successor_cap: Option<usize>,
...
/// Default per-state successor cap: 50 million. A `DiffSuccessor` is at least
/// ~40 bytes, so 50M is on the order of ~2 GB for one state's successor Vec —
/// far past any legitimate per-state fan-out and squarely in runaway-allocation
/// territory. This is the fallback baked into a fresh `SharedCtx`; tla-check
/// recomputes the effective cap from the model `Config` (honoring the
/// `TY_PER_STATE_SUCCESSOR_CAP` env var) when it builds the eval context.
pub const DEFAULT_PER_STATE_SUCCESSOR_CAP: usize = 50_000_000;
...
impl SharedCtx {
    ...
    pub fn new() -> Self {
        SharedCtx {
            ...
            per_state_successor_cap: Some(DEFAULT_PER_STATE_SUCCESSOR_CAP),
...
/// shares the entire stable context. This follows the EvalCtx hot/cold split
/// design (cold fields here; hot per-binding fields live on `EvalCtx`).
#[derive(Clone)]
...
/// Evaluation context: the environment against which every TLA+ expression is
/// evaluated.
///
/// An `EvalCtx` bundles everything [`eval`](crate::eval) needs to reduce a
/// lowered AST expression to a [`Value`]: the operator environment and instance
/// substitutions ([`SharedCtx`], shared by `Rc` across clones), the current and
/// next (primed) state-variable arrays, the local binding chain for quantifier
/// and `LET` variables, TLC configuration, and runtime statistics.
///
/// The struct is split into a cold `EvalCtxStable` half (shared by `Rc`, so a
/// clone is a single refcount bump) and a small set of hot fields kept inline to
/// avoid `Rc::make_mut` on the per-binding fast path. Read access to stable
/// fields auto-resolves through a [`Deref`](std::ops::Deref) impl; mutation goes
/// through `stable_mut()`, which performs copy-on-write.
///
/// Construct one with [`EvalCtx::new`] (empty) or [`EvalCtx::with_env`] (with an
/// initial variable environment), then load operators and bind state before
/// evaluating. In model checking, `tla-check` builds and reuses a context across
/// the BFS, calling the cache-lifecycle functions (e.g.
/// [`clear_for_state_boundary`](crate::clear_for_state_boundary)) at the
/// appropriate boundaries.
///
/// `EvalCtx` is `!Send`/`!Sync`: it holds borrowed raw-pointer state references
/// ([`StateEnvRef`]) and `Rc` internals, so evaluation is single-threaded within
/// a worker.
///
/// # Examples
///
/// ```
/// use tla_eval::{eval, EvalCtx};
/// use tla_core::ast::Expr;
/// use tla_core::Spanned;
/// use tla_value::Value;
///
/// let ctx = EvalCtx::new();
/// // `2 + 3` as a lowered expression.
/// let expr = Spanned::dummy(Expr::Add(
///     Box::new(Spanned::dummy(Expr::Int(2.into()))),
///     Box::new(Spanned::dummy(Expr::Int(3.into()))),
/// ));
/// let value = eval(&ctx, &expr).unwrap();
...
pub struct EvalCtx {
    ...
    pub(crate) active_lazy_func: Option<Rp<LazyFuncValue>>,
