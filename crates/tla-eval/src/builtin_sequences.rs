...
fn eval_select_seq_filter(
    ...
    for elem in seq {
        if first_iter {
            local_ctx.push_binding_preinterned(param_name.clone(), elem.clone(), param_id);
            first_iter = false;
        } else {
    ...
    match name {
        ...
        "Tail" => {
            ...
            if let Some(seq_value) = sv.as_seq_value() {
                ...
                return Ok(Some(Value::Seq(Rp::new(seq_value.tail()))));
            }
            ...
            Ok(Some(Value::Seq(Rp::new(seq[1..].to_vec().into()))))
        }
        ...
        "Append" => {
            ...
            if let Some(s) = sv.as_seq().or_else(|| sv.to_tuple_like_elements()) {
                let mut v = s.to_vec();
                v.push(elem);
                Ok(Some(Value::Seq(Rp::new(v.into()))))
            } else {
        ...
        "SubSeq" => {
            ...
            match &sv {
                ...
                Value::Seq(seq_value) => {
                    ...
                    let end = n as usize;
                    Ok(Some(Value::Seq(Rp::new(seq_value.subseq(start, end)))))
                }
                Value::Tuple(seq) => {
                    if m > n {
                        return Ok(Some(Value::Seq(Rp::new(Vec::new().into()))));
                    ...
                    let start = (m - 1) as usize;
                    let end = n as usize;
                    Ok(Some(Value::Seq(Rp::new(seq[start..end].to_vec().into()))))
                }
                ...
                Value::Func(f) => {
                    ...
                    Ok(Some(Value::Seq(Rp::new(out.into()))))
                }
