        ...
        if let Some(idx) = fast_var_idx_lookup(ctx, resolved_name_id) {
            ...
            if let Some(state_env) = ctx.state_env {
                ...
                tla_value::churn_stats::churn_count(
                    tla_value::churn_stats::ChurnSite::StateVarReadAstIdent,
                );
                ...
                IdentHint::PrecomputedConstant => {
                    ...
                    if let Some(value) = ctx.shared.precomputed_constants.get(&resolved_name_id) {
                        tla_value::churn_stats::churn_count_if_heap(
                            tla_value::churn_stats::ChurnSite::PrecomputedConstClone,
                            value,
                        );
                        return Ok(value.clone());
                    }
...
fn eval_ident_slow(
    ...
    if resolved_name != name {
        let binding = if crate::cache::is_subst_guarded(resolved_op_name_id) {
            ctx.bindings.lookup_skip_instance(resolved_op_name_id)
        } else {
            ctx.bindings.lookup(resolved_op_name_id)
        };
