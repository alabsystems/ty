...
pub(super) fn apply_builtin_binary_op(
    ...
    match op {
        ...
        // Division operations. `/` is TLA+ real division: EXACT-OR-ERROR on
        // integers (int_exact_div_op); `\div` is floored integer division.
        "/" => int_exact_div_op(left, right, span),
        "\\div" => int_div_op(
            left,
            right,
            |a, b| {
        ...
        "\\o" => {
            ...
            if let Some(elems) = left.as_seq_or_tuple_elements() {
                ...
                match &left {
                    ...
                    // Compact bag: seq-shaped bags concatenate like the
                    // equivalent Func.
                    Value::Bag(b) => {
                        ...
                        result.extend(f.mapping_values().cloned());
                    }
                    Value::Func(f) => {
