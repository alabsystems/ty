...
pub(super) fn eval_constructor_ops(
    ...
    match name {
        "Prefixes" => {
            ...
            for i in 0..=seq.len() {
                prefixes.push(Value::Seq(Rp::new(seq[..i].to_vec().into())));
            }
            Ok(Some(Value::Set(Rp::new(SortedSet::from_iter(prefixes)))))
        }
        ...
        "Suffixes" => {
            ...
            for i in 0..=seq.len() {
                suffixes.push(Value::Seq(Rp::new(seq[i..].to_vec().into())));
            }
            Ok(Some(Value::Set(Rp::new(SortedSet::from_iter(suffixes)))))
        }
        ...
        "BoundedSeq" | "SeqOf" => {
            ...
            fn generate_seqs(
                ...
                if current.len() <= max_len {
                    all.push(Value::Seq(Rp::new(current.clone().into())));
                    if current.len() < max_len {
                        for e in elements {
            ...
            Ok(Some(Value::Set(Rp::new(SortedSet::from_iter(all_seqs)))))
        }
        ...
        "TupleOf" => {
            ...
            if n == 0 {
                let result = vec![Value::Seq(Rp::new(Vec::new().into()))];
                return Ok(Some(Value::Set(Rp::new(SortedSet::from_iter(result)))));
            }
            ...
            fn generate_tuples(
                ...
                if current.len() == n {
                    all.push(Value::Seq(Rp::new(current.into())));
                } else {
                    for e in elements {
                        let mut next = current.clone();
                        next.push(e.clone());
            ...
            Ok(Some(Value::Set(Rp::new(SortedSet::from_iter(all_tuples)))))
        }
        ...
        "Indices" => {
            ...
            let seq = sv
                .as_seq()
                .ok_or_else(|| EvalError::type_error("Seq", &sv, Some(args[0].span)))?;
            Ok(Some(Value::Interval(Rp::new(IntervalValue::new(
                BigInt::one(),
