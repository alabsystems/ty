...
pub(super) fn eval_combinatorial_ops(
    ...
    match name {
        ...
        "SubSeqs" => {
            ...
            subseqs.push(Value::Seq(Rp::new(Vec::new().into())));

            let len = seq.len();
            for start in 0..len {
                for end in (start + 1)..=len {
                    let subseq: Vec<Value> = seq[start..end].to_vec();
                    subseqs.push(Value::Seq(Rp::new(subseq.into())));
                }
            }
            Ok(Some(Value::Set(Rp::new(SortedSet::from_iter(subseqs)))))
        }
        ...
        "SetToSeqs" => {
            ...
            fn generate_permutations(
                ...
                if current.len() == elements.len() {
                    result.push(Value::Seq(Rp::new(current.clone().into())));
                    return;
                }
        ...
        "AllSubSeqs" => {
            ...
            let n = seq.len();
            // Fail-closed: 2^n subsequences. Decline BEFORE allocating for
            // oversized input — `1u64 << n` overflows (panics) for n >= 64, and
            // even moderate n exhausts memory. 2^20 mirrors the powerset cap.
            if n > 20 {
                return Err(EvalError::SetTooLarge { span });
            }
            let mut result: Vec<Value> = Vec::with_capacity(1usize << n);
            ...
            for mask in 0..(1u64 << n) {
                ...
                result.push(Value::Seq(Rp::new(subseq.into())));
            }
            Ok(Some(Value::Set(Rp::new(SortedSet::from_iter(result)))))
        }
