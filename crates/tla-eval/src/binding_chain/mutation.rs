...
impl BindingChain {
    ...
    /// Clone this chain into a durable capture that may outlive the current
    /// evaluation-arena state.
    ///
    /// Ordinary [`Clone`] intentionally preserves the arena head for cheap
    /// within-state snapshots. Captured closures, lazy functions, and set
    /// predicates have a different lifetime: they can escape the current BFS
    /// state, so their arena segment must be promoted before it is stored
    /// behind the cross-crate trait object.
    pub(crate) fn durable_capture_box(&self) -> Box<dyn tla_value::CapturedChain> {
