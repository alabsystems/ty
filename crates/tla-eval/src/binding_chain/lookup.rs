...
impl BindingChain {
    ...
    /// Look up a name while ignoring INSTANCE substitution bindings for that name.
    ///
    /// Used while forcing the RHS of an INSTANCE substitution. The RHS is evaluated
    /// in the definition-site context, before the substitution being defined is in
    /// scope, so a same-name inherited INSTANCE binding must not shadow the outer
    /// state variable/operator/local value.
    #[inline]
    pub(crate) fn lookup_skip_instance(
        &self,
        target: NameId,
    ) -> Option<(BindingValueRef<'_>, BindingSourceRef<'_>)> {
        let mut arena_cur = self.arena_head;
        while !arena_cur.is_null() {
            // SAFETY: arena pointer chain is valid within current BFS state.
            let node = unsafe { &*arena_cur };
            if node.name == target {
                let source = arena_source_ref(node);
                if !matches!(source, BindingSourceRef::Instance(_)) {
                    return Some((arena_value_ref(node), source));
                }
            }
            arena_cur = node.next;
        }
        ...
        while let Some(node) = cur {
            if node.name == target {
                let source = BindingSourceRef::from(&node.source);
                if !matches!(source, BindingSourceRef::Instance(_)) {
                    return Some((BindingValueRef::from(&node.value), source));
                }
            }
            cur = &node.next;
        }
    ...
    /// Return ALL bindings in the chain as eager `(name, value)` pairs, or
    /// `None` if the chain contains any lazy binding.
    ///
    /// Part of #liveness-leaf-memo: callers that need a *complete* snapshot of
    /// the chain's bound values (e.g. to decide whether two liveness leaves'
    /// quantifier bindings can influence a shared subscript expression) must
    /// fail closed when a binding cannot be observed without forcing it.
    /// Unlike [`Self::iter_eager`], which silently skips lazy bindings, this
    /// returns `None` so the caller cannot mistake a partial view for the
    /// whole chain. Shadowed (duplicate-name) bindings are all included, in
    /// chain order.
    pub fn all_bindings_eager(&self) -> Option<Vec<(Arc<str>, Value)>> {
        let mut out = Vec::new();
        for (name_id, bv, _source) in self.iter() {
            match bv {
                BindingValueRef::Eager(v) => {
                    let name = tla_core::name_intern::resolve_name_id(name_id);
                    out.push((name, Value::from(v)));
                }
                BindingValueRef::Lazy(_) => return None,
            }
        }
        Some(out)
    }
