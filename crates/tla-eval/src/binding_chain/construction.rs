...
impl BindingChain {
    ...
    pub(crate) fn cons_local(
        ...
        if matches!(value, BindingValue::Eager(_)) && is_arena_active() {
            tla_value::churn_stats::churn_count(tla_value::churn_stats::ChurnSite::ChainConsArena);
            let ptr = arena_alloc(
                name,
                value,
        ...
        tla_value::churn_stats::churn_count(tla_value::churn_stats::ChurnSite::ChainConsHeap);
        let heap_head = promote_to_heap(self.arena_head, &self.heap_head);
        BindingChain {
            arena_head: std::ptr::null(),
            heap_head: Some(Arc::new(BindingNode {
