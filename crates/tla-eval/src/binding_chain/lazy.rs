...
/// returning `null` when the evaluation context has changed. TY's LazyBinding
...
/// cache until TY has a validity check strong enough to avoid reusing stale
...
pub(crate) struct LazyBinding {
    ...
    /// The local operator scope at the point of definition.
    pub def_site_local_ops: Option<Arc<OpEnv>>,
...
impl LazyBinding {
    ...
    pub(crate) fn new(
        ...
        Self::new_with_local_ops(expr_ptr, enclosing, None)
    }

    pub(crate) fn new_with_local_ops(
        expr_ptr: *const Spanned<tla_core::ast::Expr>,
        enclosing: &BindingChain,
