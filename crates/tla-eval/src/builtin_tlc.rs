...
use super::{
    apply_binary_op, apply_closure_with_values, check_arity, create_closure_from_arg, eval,
    eval_iter_set, eval_iter_set_tlc_normalized, generate_permutation_functions, EvalCtx,
...
pub(super) fn eval_builtin_tlc(
    ...
    match name {
        ...
        "SortSeq" => {
            ...
            // Comparator can be a user-defined operator name (Ident), a built-in
            // operator reference (OpRef) like <, <=, etc., or (N15) a LAMBDA /
            // any expression evaluating to a 2-ary closure — TLC accepts these.
            enum SortCmp {
                Op(OperatorRef),
                Closure(Rp<ClosureValue>),
            }
            let cmp = match &args[1].node {
                Expr::Ident(name, _) => SortCmp::Op(OperatorRef::UserDefined(name.clone())),
                Expr::OpRef(op) => SortCmp::Op(OperatorRef::BuiltIn(op.clone())),
                // A LAMBDA (or other closure-valued expression) comparator: build
                // a 2-ary closure via the shared higher-order-argument machinery.
                _ => match create_closure_from_arg(ctx, &args[1], "SortSeq", 2, span)? {
                    Value::Closure(ref closure) => SortCmp::Closure(closure.clone()),
                    other => {
                        return Err(EvalError::type_error(
                            "operator or LAMBDA comparator",
                            &other,
                            Some(args[1].span),
                        ));
                    }
            ...
            for i in 1..result.len() {
                ...
                while j > 0 {
                    ...
                    let cmp_result = match &cmp {
                        ...
                        SortCmp::Closure(closure) => apply_closure_with_values(
                            ctx,
                            closure,
                            &[result[j - 1].clone(), result[j].clone()],
                            span,
                        )?,
                    };
            ...
            Ok(Some(Value::Seq(Rp::new(result.into()))))
        }
        ...
        "Permutations" => {
            ...
            // generate_permutation_functions builds each permutation's entries
            // in domain order, and FuncValue requires Value::cmp-sorted keys.
            // eval_iter_set order is not guaranteed sorted for every set kind
            // (e.g. SUBSET domains iterate cardinality-first).
            ...
            Ok(Some(Value::Set(Rp::new(SortedSet::from_iter(perms)))))
        }
        ...
        // TLCEval(v) - force evaluation of lazy value (noop - TY is already eager)
        "TLCEval" => {
            check_arity(name, args, 1, span)?;
