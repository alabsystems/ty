...
impl EvalCtx {
    ...
    /// Expand an INSTANCE's explicit `WITH` substitutions with the implicit ones
    /// TLA+ supplies automatically.
    ///
    /// For `INSTANCE M WITH a <- x`, any variable or constant declared by `M`
    /// that is not explicitly substituted is implicitly mapped to the
    /// same-named symbol visible in the current scope (a state variable, config
    /// constant, active substitution, or zero-arg operator). Implicit targets
    /// with no such visible symbol are skipped. Returns `explicit_subs` followed
    /// by the synthesized implicit substitutions, in the module's declaration
    /// order.
    #[must_use]
    ...
    /// The substitution currently mapping `name`, if an INSTANCE substitution
    /// for that symbol is active in this scope.
    ...
    /// Whether an INSTANCE substitution for `name` is active in this scope.
    #[inline]
