...
pub(crate) fn eval_set_builder(
    ...
    Ok(Value::Set(Rp::new(SortedSet::from_iter(result))))
}
...
pub(crate) fn eval_set_filter(
    ...
    //
    // Extension: also reduce eagerly when the domain is a *small* finite enumerable
    // lazy set (e.g. a Cartesian product `{-1,0,1} \X {-1,0,1}`). The lazy path
    // deep-clones the bound-variable + predicate AST into a `SetPredValue` on every
    // evaluation AND on every const-set-cache hit (SetPredValue::clone is a deep AST
    // copy), which dominates allocation churn for specs that repeatedly filter a
    // constant product (e.g. GameOfLife's `nbrs`). Materializing yields an identical
    // concrete `Set` that is cheap to clone and cache. The cardinality cap keeps large
    // domains (powersets, function sets) lazy so short-circuiting `\E`/`\A` still avoids
    // forcing predicate evaluation over the whole domain.
    const SMALL_FILTER_MATERIALIZE_CAP: u64 = 256;
    // Keep the direct RecordSet/SetPred-chain fragment lazy even when small.
    // Init extraction supports exactly this shared structural fragment and
    // streams CoffeeCan-style filtered record domains. Set-algebra wrappers
    // deliberately retain the established eager small-set policy because the
    // bulk lane does not claim them (semantic results remain identical).
    let is_reducible = matches!(dv, Value::Set(_) | Value::Interval(_))
        || (!dv.is_streamable_record_filter_domain()
            && dv.is_small_finite_lazy_set(SMALL_FILTER_MATERIALIZE_CAP));
    if !is_reducible {
        if !dv.is_set() {
