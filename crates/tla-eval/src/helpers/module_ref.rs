...
debug_flag!(debug_param_instance, "TY_DEBUG_PARAM_INSTANCE");
...
pub(super) use super::module_ref_instance::get_instance_info;
pub use super::module_ref_instance::{
    apply_substitutions, compose_substitutions, expr_has_any_prime, expr_has_primed_param,
    prepare_registered_named_module_ref_call, registered_named_module_ref_dispatch_is_direct,
};
...
pub(crate) fn eval_module_ref_target(
    ...
    match target {
        ...
        ModuleTarget::Parameterized(name, param_exprs) => {
            ...
            //    arg expressions. Lazy substitution bindings then evaluate the RHS
            //    expressions against the definition-site BindingChain populated below.
            let param_ctx = if let Some(def) = ctx.get_op(name) {
                ...
                if !def.params.is_empty() {
                    ...
                    // lazy INSTANCE substitution evaluation.
                    ...
                    // operator-result caches can disambiguate entries by argument values.
                    ...
                    // OP_RESULT_CACHE) see the same Arc pointer across calls from the
                    // same callsite. The substitution content is
