...
use crate::cache::{
    enter_quantifier_hoist_scope, mark_hoistable, propagate_cached_deps, OpDepGuard,
...
fn is_known_infinite_domain(domain_value: &Value) -> bool {
    match domain_value {
        Value::ModelValue(name) => matches!(name.as_ref(), "Nat" | "Int" | "Real"),
        Value::StringSet | Value::AnySet | Value::SeqSet(_) => true,
        _ => false,
    }
}

fn try_eval_forall_unforced_true_path(
    ctx: &mut EvalCtx,
    first: &BoundVar,
    pre: &PreInternedBound,
    domain_value: &Value,
    ...
    if !is_known_infinite_domain(domain_value) {
        return Ok(None);
    }
    let Some(bound_name_id) = simple_bound_name_id(pre) else {
        return Ok(None);
    ...
    let mark = ctx.mark_stack();
    push_bound_var_owned_preinterned(ctx, first, Value::Bool(false), span, Some(pre))?;
    let guard = OpDepGuard::from_ctx(ctx, ctx.binding_depth);
    ...
    let Some(deps) = deps else {
        return Err(EvalError::Internal {
            message: "forall unforced true path dependency tracking stack unexpectedly empty"
                .into(),
            span: Some(body.span),
        });
    };
    ...
    if matches!(result, Ok(Value::Bool(true)))
        && !deps.inconsistent
        && !crate::cache::dep_tracking::dep_map_contains_key(&deps.local, &bound_name_id)
    {
...
fn eval_quantifier_single_streaming<B: BoundVarOps, E: FnMut(&mut EvalCtx) -> EvalResult<Value>>(
    ...
    if var.is_simple() {
        let mut first_iter = true;
        for elem_result in stream_iter.by_ref() {
...
pub(crate) fn eval_forall(
    ...
    if let Some(domain_len) = dv.set_len() {
        if domain_len.is_zero() {
            return Ok(Value::Bool(true));
        }
    } else if bounds.len() == 1 {
        if let Some(result) =
            try_eval_forall_unforced_true_path(ctx, first, prepared.pre(), &dv, body, span)?
        {
            return Ok(result);
        }
    }
