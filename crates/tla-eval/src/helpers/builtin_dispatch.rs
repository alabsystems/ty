...
fn has_module_scoped_complete_builtin_override(
    ...
    // Fast front gate: this predicate runs on EVERY operator application, and
    // the `extended_module_names.contains(...)` probes below each hash a
    // module-name string (1.5% of total cycles on op-application-heavy specs
    // like TLCSailfish1, where no override module is even loaded). Reject
    // names that cannot match ANY override arm with a cheap literal match
    // before touching the HashSet. Keep this list in sync with the arms
    // below — it is a pure superset filter, so a stale entry only costs one
    // redundant module probe, never a behavior change... unless an entry is
    // MISSING, so the exhaustive test `module_scoped_override_front_gate_
    // covers_all_arms` locks the two lists together.
    if !matches!(
        name,
        "Zero"
            | "One"
    ...
    // MapThenFoldSet, but TY has complete Rust builtin implementations.
...
/// Whether a built-in implementation should completely replace a user-defined
/// operator of the same name and arity.
///
/// Returns `true` when the operator is either (a) one of the standard community
/// operators a module has EXTENDS-imported, so its (often `O(2^n)` or `O(n^n)`)
/// reference definition can be swapped for the linear built-in, or (b) a
/// universally-standardized operator (`ReduceSet/3`, `SetToSeq/1`) overridden
/// unconditionally. When `true`, dispatch evaluates the built-in instead of the
/// spec body.
...
/// Return whether runtime dispatch will evaluate the Rust builtin instead of
/// the resolved TLA+ operator definition.
///
/// This includes both complete module-scoped overrides and placeholder-body
/// overrides. Static expansion and analysis must use this same predicate or
/// they can erase the call that runtime actually executes.
pub fn should_prefer_builtin_override(
    name: &str,
    def: &OperatorDef,
    arg_count: usize,
    ctx: &EvalCtx,
...
pub(crate) fn generate_permutation_functions(
    ...
    if range_prefix.len() == n {
        ...
        let entries: Vec<(Value, Value)> = range_prefix
            ...
            .collect();
        result.push(Value::Func(Rp::new(FuncValue::from_sorted_entries(
            entries,
        ))));
        return;
    }
...
mod tests {
    ...
    /// Lock the fast front gate of `has_module_scoped_complete_builtin_override`
    /// to the module-scoped arms: with ALL override modules loaded, every
    /// (name, arity) pair each arm accepts must still return true — i.e. the
    /// front-gate literal list is a superset of the arm names. A name missing
    /// from the gate would silently disable its override (behavior change);
    /// this test makes that impossible.
    #[cfg_attr(test, ntest::timeout(10000))]
    #[test]
    fn module_scoped_override_front_gate_covers_all_arms() {
        ...
        for (name, arity) in arms {
            assert!(
                has_module_scoped_complete_builtin_override(name, *arity, &ctx_all),
                "front gate must not reject module-scoped override arm ({name}, {arity})"
            );
        }
        // And a non-override name stays rejected on the fast path.
        assert!(!has_module_scoped_complete_builtin_override(
            "Cardinality",
            1,
            &ctx_all
        ));
