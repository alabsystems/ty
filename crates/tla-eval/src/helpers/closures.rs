...
pub(crate) fn create_closure_from_arg(
    ...
    match &arg.node {
        Expr::Lambda(lambda_params, body) => {
            ...
            Ok(Value::Closure(Rp::new(ctx.create_closure(
                params,
                (**body).clone(),
                ctx.local_ops.clone(),
            ))))
        ...
        Expr::Ident(name, orig_name_id) => {
            ...
            if let Some(Value::Closure(ref c)) = ctx.lookup(name) {
                ...
                return Ok(Value::Closure(Rp::clone(c)));
            }
...
pub(super) fn build_closure_ctx(ctx: &EvalCtx, closure: &ClosureValue) -> EvalResult<EvalCtx> {
    ...
    // Compute the scope id (and recursive flag) eagerly (one HAMT walk per closure
    // force) instead of leaving it INVALIDATED (which forces a walk on every cache
    // lookup inside the closure body). The closure's captured local_ops Arc is
    // immutable, so the id is value-identical to recomputation.
    // Pointer-memoized: the captured Arc is immutable and pinned by the memo,
    // so the (id, recursive) pair is derived once per allocation instead of
    // one full HAMT walk per closure force.
    let (closure_local_ops_id, closure_local_ops_recursive) = match &new_stable.local_ops {
        Some(ops) => crate::cache::openv_memo::scope_id_and_recursive_memoized(
            ops,
            crate::cache::openv_memo::ScopeIdSite::Closure,
        ),
        None => (0, false),
    };
    new_stable.scope_ids.local_ops = closure_local_ops_id;
    new_stable.scope_ids.local_ops_recursive = closure_local_ops_recursive;
...
pub(super) fn build_lazy_func_ctx(
    ...
    if f.name().is_some()
        && ctx
            .active_lazy_func
            .as_ref()
            .is_some_and(|active| Rp::ptr_eq(active, f))
    {
    ...
    new_stable.local_ops = f.local_ops().cloned(); // Use captured local_ops, not ctx's
                                                   // Compute the scope id eagerly (one HAMT walk per force) instead of
                                                   // leaving it INVALIDATED (a walk on every cache lookup inside the body).
                                                   // The captured local_ops Arc is immutable, so the id is value-identical
                                                   // to recomputation.
    let (closure_local_ops_id2, closure_local_ops_recursive2) = match &new_stable.local_ops {
        Some(ops) => crate::cache::openv_memo::scope_id_and_recursive_memoized(
            ops,
            crate::cache::openv_memo::ScopeIdSite::LazyFunc,
        ),
        None => (0, false),
    };
    new_stable.scope_ids.local_ops = closure_local_ops_id2;
    new_stable.scope_ids.local_ops_recursive = closure_local_ops_recursive2;
    ...
    if let Some(name) = f.name() {
        ...
        new_ctx.push_binding_preinterned(Arc::clone(name), Value::LazyFunc(Rp::clone(f)), name_id);
    }
    Ok(new_ctx)
}
...
pub(crate) fn build_lazy_func_from_ctx(
    ...
    // Part of #2955: Capture the BindingChain directly. Heap-only chains clone
    // in O(1); an active arena segment is promoted so the lazy function can
    // safely outlive the current BFS state. At application time,
    // build_lazy_func_ctx restores this chain instead of rebuilding from
    // captured_bindings (eliminates ~84M Rc allocs for GameOfLife).
    ...
    .with_captured_chain(ctx.bindings.durable_capture_box(), ctx.binding_depth);
    if bounds.len() == 1 {
