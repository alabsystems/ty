...
use super::module_ref::{
    apply_substitutions, compose_substitutions, get_instance_info, module_ref_compound_key,
};
...
use tla_core::{pretty_expr, Span, Spanned};

debug_flag!(debug_chain_subs, "TY_DEBUG_CHAIN_SUBS");
...
fn build_chained_ref_cache_entry(
    ...
    add_parent_operator_implicit_substitutions(
        ctx,
        base_target,
        &intermediate_modules,
        &instance_info.module_name,
        &mut instance_info.substitutions,
    );
    ...
    if debug_chain_subs() {
        eprintln!(
            "[CHAIN_SUBS] base={:?}!{} module={} intermediates={:?} subs=[{}]",
            base_target,
            intermediate_op,
            instance_info.module_name,
            intermediate_modules,
            instance_info
                .substitutions
                .iter()
                .map(|sub| format!("{} <- {}", sub.from.node, pretty_expr(&sub.to.node)))
                .collect::<Vec<_>>()
                .join("; ")
        );
    }
...
fn add_parent_operator_implicit_substitutions(
    ctx: &EvalCtx,
    parent_target: &ModuleTarget,
    intermediate_modules: &[String],
    final_module_name: &str,
    substitutions: &mut Vec<Substitution>,
) {
    let Some(parent_module_name) = intermediate_modules.last() else {
        return;
    };
    let Some(implicit_targets) = ctx.shared.instance_implicit_targets.get(final_module_name) else {
        return;
    };
    let Some(parent_ops) = ctx.shared.instance_ops.get(parent_module_name) else {
        return;
    };

    for target in implicit_targets {
        let Some(parent_def) = parent_ops.get(target.as_str()) else {
            continue;
        };
        if !parent_def.params.is_empty() {
            continue;
        }

        let parent_ref = Spanned::dummy(Expr::ModuleRef(
            parent_target.clone(),
            target.clone(),
            Vec::new(),
        ));

        if let Some(existing) = substitutions
            .iter_mut()
            .find(|sub| sub.from.node.as_str() == target.as_str())
        {
            if matches!(&existing.to.node, Expr::Ident(name, _) if name == target) {
                existing.to = parent_ref;
            }
        } else {
...
pub(crate) fn resolve_module_ref_chain(
    ...
    match target {
        ...
        ModuleTarget::Chained(base_expr) => {
            ...
            let def_site_target = ModuleTarget::Chained(base_expr.clone());
            let qualified_body = qualify_instance_expr_substitutions(
                ctx,
                &base_info.module_name,
                &def_site_target,
                &op_def.body,
            );
            let wrapped = Spanned::new(
                Expr::SubstIn(base_info.substitutions.clone(), Box::new(qualified_body)),
                op_def.body.span,
            );
...
pub(super) fn instance_info_from_substituted_expr(
    ...
    match &substituted.node {
        ...
        Expr::SubstIn(wrap_subs, body) => {
            let effective = inherit_substitutions(wrap_subs, outer_subs);
            match &body.node {
                ...
                Expr::InstanceExpr(module_name, inner_subs) => Ok(InstanceInfo {
                    module_name: module_name.clone(),
                    substitutions: inherit_substitutions(inner_subs, Some(&effective)),
                }),
                ...
                _ => {
                    let structural_subs = compose_substitutions(wrap_subs, outer_subs);
                    let substituted = apply_substitutions(body, &structural_subs);
                    instance_info_from_substituted_expr(
                        &substituted,
                        Some(&effective),
                        undefined_name,
