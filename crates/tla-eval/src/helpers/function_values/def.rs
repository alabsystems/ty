...
pub(crate) fn eval_func_def(
    ...
    tla_value::churn_stats::churn_count(tla_value::churn_stats::ChurnSite::FuncDefBuild);
    ...
    if bounds.len() == 1 {
        ...
        if let Value::ModelValue(name) = &dv {
            return match name.as_ref() {
                "Nat" => Ok(Value::LazyFunc(Rp::new(build_lazy_func_from_ctx(
                    ctx,
                    None,
                    LazyDomain::Nat,
                ...
                "Int" => Ok(Value::LazyFunc(Rp::new(build_lazy_func_from_ctx(
                    ctx,
                    None,
                    LazyDomain::Int,
                    std::slice::from_ref(bound),
                ...
                "Real" => Ok(Value::LazyFunc(Rp::new(build_lazy_func_from_ctx(
                    ctx,
                    None,
                    LazyDomain::Real,
                    std::slice::from_ref(bound),
        ...
        if let Value::TupleSet(ts) = &dv {
            ...
            if has_infinite {
                return Ok(Value::LazyFunc(Rp::new(build_lazy_func_from_ctx(
                    ctx,
                    None,
        ...
        if let Value::Interval(intv) = &dv {
            if let (Some(min), Some(max)) = (intv.low().to_i64(), intv.high().to_i64()) {
                ...
                if size <= 1_000_000 {
                    ...
                    local_ctx.pop_to_mark(&mark);
                    if min == 1 {
                        return Ok(Value::Seq(Rp::new(values.into())));
                    }
                    return Ok(Value::IntFunc(Rp::new(IntIntervalFunc::new(
                        min, max, values,
                    ))));
                }
            }
