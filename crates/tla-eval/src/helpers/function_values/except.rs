...
/// Caching the Arc avoids its per-operation allocation.  The NameId MUST NOT
/// be cached here: `tla_check::reset_global_state()` clears the process-global
/// name interner between runs, invalidating every previously issued NameId.
/// Re-interning the cached string is required for run isolation and normally
/// takes the interner's existing-name fast path.
static EXCEPT_AT_NAME: LazyLock<Arc<str>> = LazyLock::new(|| Arc::from("@"));
...
/// Resolved EXCEPT paths are overwhelmingly one or two elements deep. Keep
/// those paths inline while preserving the general case through SmallVec's
/// normal heap spill behavior.
pub(crate) type ResolvedExceptPath = SmallVec<[ResolvedExceptPathElement; 2]>;

// Temporary same-binary A/B switch for the inline EXCEPT-path buffer. The
...
pub(crate) fn collect_resolved_except_path(
    ...
    if no_except_path_smallvec() {
        iter.collect::<EvalResult<Vec<_>>>()
            .map(ResolvedExceptPath::from_vec)
    } else {
        iter.collect()
    }
}
...
impl LazyExceptHandler for AstLazyExceptHandler {
    fn handle_lazy_except(
        ...
        Ok(Value::LazyFunc(Rp::new(new_func)))
    }
}
...
fn resolve_ast_except_path_element(
    ...
    match elem {
        ExceptPathElement::Index(idx_expr) => {
            if matches!(&idx_expr.node, Expr::Tuple(elems) if !elems.is_empty()) {
                tla_value::churn_stats::churn_count(
                    tla_value::churn_stats::ChurnSite::TupleKeyExceptAstBuild,
                );
            }
...
#[cfg(test)]
mod tests {
    use super::{ResolvedExceptPath, ResolvedExceptPathElement};
    use crate::Value;

    #[test]
    fn resolved_except_path_keeps_two_elements_inline_and_spills_on_third() {
        let mut path = ResolvedExceptPath::new();
        path.push(ResolvedExceptPathElement::Index(Value::int(1)));
        path.push(ResolvedExceptPathElement::Index(Value::int(2)));
        assert!(
            !path.spilled(),
            "two-element EXCEPT path should stay inline"
        );

        path.push(ResolvedExceptPathElement::Index(Value::int(3)));
        assert!(path.spilled(), "three-element EXCEPT path should spill");
    }
}
...
pub(crate) fn apply_resolved_except_spec(
    ...
    if path.is_empty() {
        ...
        //
        // Part of #3895: retain the allocation-free Arc fast path.  Resolve the
        // NameId against the current interner generation, because a prior model
        // checking run may have reset the interner.
        let at_name = Arc::clone(&EXCEPT_AT_NAME);
        let at_name_id = intern_name(&at_name);
        let mut new_ctx = ctx.clone();
        new_ctx.push_binding_preinterned(at_name, value, at_name_id);
    ...
    match (&mut value, first) {
        ...
        (Value::Func(f), ResolvedExceptPathElement::Index(idx)) => {
            let fm = if let Some(inner) = Rp::get_mut(f) {
                inner
            } else {
                Rp::make_mut(f)
            };
        ...
        // Compact bag with index: fail-closed — materialize the general Func
        // (same entries/order) and re-dispatch. EXCEPT on a bag downgrades the
        // representation; correctness is unchanged (fingerprint/eq identical).
        (Value::Bag(b), ResolvedExceptPathElement::Index(_)) => {
            let func = Value::Func(Rp::new(b.to_func_value()));
            apply_resolved_except_spec(ctx, func, path, eval_new_value, lazy_handler, span)
        }
        // LazyFunc with index: delegate to handler
        (Value::LazyFunc(f), ResolvedExceptPathElement::Index(idx)) => {
            lazy_handler.handle_lazy_except(ctx, f.clone(), idx, rest, eval_new_value, span)
        }
        // Record with field
        ...
        (Value::Seq(s), ResolvedExceptPathElement::Index(idx)) => {
            ...
            let sv = if let Some(inner) = Rp::get_mut(s) {
                inner
            } else {
                Rp::make_mut(s)
            };
        ...
        (Value::Tuple(t), ResolvedExceptPathElement::Index(idx)) => {
            ...
            if i < 1 || i as usize > t.len() {
                ...
                // leaves f unchanged (TLC warns + no-ops). Mirrors the Seq,
                // Func, IntFunc, and Record arms above.
                return Ok(value);
            }
            let arr_idx = (i - 1) as usize;
            ...
            if let Some(slice) = Rp::get_mut(t) {
        ...
        | (Value::Bag(_), ResolvedExceptPathElement::Field { .. }) => Err(EvalError::Internal {
