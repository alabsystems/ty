...
/// Cached per-closure free-variable mask: one bool per parameter indicating
/// whether that parameter name occurs free in the closure body. Validated by
/// the parameter-name signature so deterministic id reuse cannot replay a stale
/// mask for a different closure.
#[derive(Clone)]
struct FreeMaskEntry {
    signature: u64,
    mask: Arc<[bool]>,
}

type ClosureFreeMaskCache = FxHashMap<u64, FreeMaskEntry>;
...
struct ParamCaches {
    ...
    closure_free_mask: ClosureFreeMaskCache,
}
...
thread_local! {
    static PARAM_CACHES: RefCell<ParamCaches> = RefCell::new(ParamCaches {
        ...
        closure_free_mask: FxHashMap::default(),
    });
}
...
pub(crate) fn clear_param_name_caches() {
    PARAM_CACHES.with(|caches| {
        ...
        c.operator.clear();
        c.closure.clear();
        c.closure_free_mask.clear();
    });
}

/// Get or compute the per-parameter "free in body" mask for a closure.
///
/// `apply_closure` needs to know, for each parameter, whether the parameter
/// name occurs free in the closure body — this decides eager-vs-lazy argument
/// binding. The closure body is immutable, so this mask is invariant across the
/// (potentially millions of) applications of the same closure. Previously
/// `apply_closure` recomputed `free_vars(body)` (a full-body AST walk plus a
/// `HashSet<String>` allocation) on EVERY application. SlidingPuzzles applies a
/// single `LAMBDA pc : s \in pc` ~20M times, so that recomputation was pure,
/// repeated overhead. Caching it (keyed by closure id, validated by the
/// parameter-name signature, mirroring `get_closure_param_cache`) reduces it to
/// one walk per distinct closure plus a cheap mask lookup per application.
///
/// Semantics-preserving: the returned mask is exactly `params[i] ∈
/// free_vars(body)` — identical to the prior per-call computation.
pub(super) fn get_closure_free_mask(closure: &ClosureValue) -> Arc<[bool]> {
    let key = closure.id();
    let signature = param_names_signature(closure.params().iter().map(|param| param.as_str()));
    PARAM_CACHES.with(|caches| {
        {
            let borrow = caches.borrow();
            if let Some(cached) = borrow.closure_free_mask.get(&key) {
                if cached.signature == signature {
                    return Arc::clone(&cached.mask);
                }
            }
        }

        let free = tla_core::free_vars(&closure.body().node);
        let mask: Arc<[bool]> = closure
            .params()
            .iter()
            .map(|param| free.contains(param.as_str()))
            .collect();
        caches.borrow_mut().closure_free_mask.insert(
            key,
            FreeMaskEntry {
                signature,
                mask: Arc::clone(&mask),
...
///
/// Caches the `TY_DISABLE_NARY_CACHE` env var check in a `OnceLock` so it's
...
pub(super) fn nary_cache_enabled() -> bool {
    ...
    *ENABLED.get_or_init(|| std::env::var("TY_DISABLE_NARY_CACHE").is_err())
}
