...
use tla_core::{ExprFold, Span, SpanPolicy, Spanned, SubstituteExpr};
...
pub(crate) fn get_instance_info(
    ...
    if let Some(def) = ctx.get_op(instance_name) {
        if let Expr::InstanceExpr(module_name, substitutions) = &def.body.node {
            ...
            let effective_instance_subs =
                inherit_substitutions(substitutions, ctx.instance_substitutions());
            ...
            let qualified_body = qualify_instance_expr_substitutions(
                ctx,
                module_name,
                &ModuleTarget::Named(instance_name.to_string()),
                &op_def.body,
            );
    ...
    if let Some(info) = ctx.get_instance(instance_name) {
        let effective_instance_subs =
            inherit_substitutions(&info.substitutions, ctx.instance_substitutions());
...
pub(crate) fn qualify_instance_expr_substitutions(
    ctx: &EvalCtx,
    def_site_module: &str,
    def_site_target: &ModuleTarget,
    body: &Spanned<Expr>,
) -> Spanned<Expr> {
    let Expr::InstanceExpr(module_name, substitutions) = &body.node else {
        return body.clone();
    };
    let Some(def_site_ops) = ctx.shared.instance_ops.get(def_site_module) else {
        return body.clone();
    };

    let replacements: Vec<(String, Spanned<Expr>)> = def_site_ops
        .iter()
        .filter(|(_, def)| def.params.is_empty())
        .map(|(name, _)| {
            (
                name.to_string(),
                Spanned::dummy(Expr::ModuleRef(
                    def_site_target.clone(),
                    name.to_string(),
                    Vec::new(),
                )),
            )
    ...
    let substitution_replacements: HashMap<&str, &Spanned<Expr>> = replacements
        .iter()
        .map(|(name, replacement)| (name.as_str(), replacement))
        .collect();

    let substitutions = substitutions
        .iter()
        .map(|substitution| {
            let mut qualifier = SubstituteExpr {
                subs: substitution_replacements.clone(),
                span_policy: SpanPolicy::Preserve,
            };
            Substitution {
                from: substitution.from.clone(),
                to: qualifier.fold_expr(substitution.to.clone()),
            }
        })
        .collect();

    Spanned::new(
        Expr::InstanceExpr(module_name.clone(), substitutions),
        body.span,
    )
}
...
/// Compose INSTANCE substitution scopes for lazy evaluation without rewriting RHS ASTs.
///
/// `compose_substitutions` is still the structural helper: it translates each
/// inner RHS through the outer substitutions. Lazy evaluation instead keeps the
/// RHS expression intact and inherits non-overridden outer bindings in the same
/// lookup chain. That preserves the definition-site operator scope captured by
/// the inherited lazy binding.
pub(crate) fn inherit_substitutions(
    inner_subs: &[Substitution],
    outer_subs: Option<&[Substitution]>,
) -> Vec<Substitution> {
    let Some(outer_subs) = outer_subs else {
