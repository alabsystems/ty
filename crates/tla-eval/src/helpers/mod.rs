...
pub use self::builtin_dispatch::{has_complete_builtin_override, should_prefer_builtin_override};
...
pub(super) use self::apply::{
    apply_closure_with_values, apply_user_op_with_values, apply_user_op_with_values_resolved,
    eval_apply, resolved_operator_name_id,
};
pub(crate) use self::bound_var_ops::{is_simple_tir_bound, push_tir_bound_var};
pub(super) use self::builtin_dispatch::{
    check_arity, eval_builtin, generate_permutation_functions,
};
...
use super::{
    eval, int_arith_op, int_div_op, int_exact_div_op, int_mod_op, int_pow_op, EvalCtx, EvalError,
    EvalResult,
};
...
/// Evaluate TLA+ real division `/`: EXACT-OR-ERROR on integers
/// (see `int_exact_div_op`; `\div` is the floored integer division).
pub(super) fn eval_real_div(
    ctx: &EvalCtx,
    a: &Spanned<Expr>,
    b: &Spanned<Expr>,
    span: Option<Span>,
