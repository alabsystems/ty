...
fn set_expr(values: &[i64]) -> Spanned<Expr> {
    spanned(Expr::SetEnum(
        values
            .iter()
            .map(|value| spanned(Expr::Int((*value).into())))
            .collect(),
    ))
}

fn lazy_offset_func(offset: i64) -> Value {
    let mut env = HashMap::default();
    env.insert(Arc::<str>::from("a"), Value::int(offset));
    let captures = LazyFuncCaptures::new(Arc::new(env), None, None, None);
    let bound = BoundVar {
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_recursive_fold_direct_apply_accepts_lazy_func() {
    crate::cache::lifecycle::clear_for_test_reset();

    let ctx = EvalCtx::new().bind_local("f", lazy_offset_func(1));
    let elem_expr = spanned(Expr::FuncApply(
        Box::new(spanned(ident("f"))),
        Box::new(spanned(ident("x"))),
    ));
    let func_val = try_resolve_func_apply_param(&ctx, &elem_expr, "x")
        .expect("lazy function parameter should use direct fold application");

    assert!(matches!(func_val, Value::LazyFunc(_)));
    let result = apply_resolved_func(&ctx, &func_val, &Value::int(4), None)
        .expect("lazy function application should succeed");
    assert_eq!(result, Value::int(5));
}

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_recursive_fold_cache_rejects_stateful_lazy_func_args() {
    crate::cache::lifecycle::clear_for_test_reset();

    let def = Arc::new(make_fold_op(
        "Sum",
        "S",
        "f",
        "x",
    ...
    let args = vec![spanned(ident("lf")), set_expr(&[1, 2, 3])];

    let ctx1 = EvalCtx::new().bind_local("lf", lazy_offset_func(1));
    let first = try_eval_recursive_fold(&ctx1, &def, &args, None)
        .expect("fold should evaluate")
        .expect("fold optimization should apply");
    assert_eq!(first, Value::int(9));

    let ctx2 = EvalCtx::new().bind_local("lf", lazy_offset_func(10));
    let second = try_eval_recursive_fold(&ctx2, &def, &args, None)
        .expect("fold should evaluate")
        .expect("fold optimization should apply");
    assert_eq!(second, Value::int(36));
}
