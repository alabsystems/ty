...
pub(crate) fn try_eval_recursive_fold(
    ...
    let cache_key = if all_arity_zero {
        ...
        if !fold_cache_args_safe(&evaluated_args) {
            None
        } else {
            // Check cache
        ...
        try_resolve_func_apply_param(&fold_ctx, pattern.elem_expr, pattern.elem_var_name)
    {
        for elem in elements.iter().rev() {
            let elem_val = apply_resolved_func(&fold_ctx, &func_val, elem, span)?;
        ...
        let elem_name_arc: Rp<str> = intern_string(pattern.elem_var_name);
        for elem in elements.iter().rev() {
            let elem_ctx = fold_ctx.bind(elem_name_arc.clone(), (*elem).clone());
...
/// Recursive-fold result caching is per-run, so cache keys must be fully
...
fn fold_cache_value_safe(value: &Value) -> bool {
    match value {
        Value::LazyFunc(_) | Value::Closure(_) => false,
        Value::Func(f) => {
            f.domain_iter().all(fold_cache_value_safe)
                && f.mapping_iter()
                    .all(|(key, val)| fold_cache_value_safe(key) && fold_cache_value_safe(val))
        }
        Value::IntFunc(f) => f.values().iter().all(fold_cache_value_safe),
        Value::Seq(seq) => seq.iter().all(fold_cache_value_safe),
        Value::Record(record) => record.values().all(fold_cache_value_safe),
        Value::Tuple(tuple) => tuple.iter().all(fold_cache_value_safe),
        Value::Set(set) => set.iter().all(fold_cache_value_safe),
        Value::Interval(_)
        | Value::Bool(_)
        | Value::SmallInt(_)
        | Value::Int(_)
        ...
        // Lazy compound/non-enumerable containers can hide identity-like values
        // behind their base domains or predicates. Prefer no memoization over a
        // per-run stale result.
        _ => false,
    }
}
...
/// to a function-like value. Returns None for any other pattern, falling through
...
fn try_resolve_func_apply_param(
    ...
    // Keep only values the shared function application path can apply directly.
    match &func_val {
        Value::Func(_)
        | Value::IntFunc(_)
        | Value::Seq(_)
        | Value::Tuple(_)
        | Value::Record(_)
        | Value::LazyFunc(_) => Some(func_val),
        _ => None,
    }
}
...
fn apply_resolved_func(
    ...
    match func_val {
        ...
        Value::Tuple(_) | Value::Record(_) | Value::LazyFunc(_) => {
            apply_resolved_func_value(ctx, func_val, arg.clone(), None, span, None)
        }
        _ => unreachable!("try_resolve_func_apply_param only returns function-like values"),
    }
}
