...
pub(super) fn merge_values(left: &Value, right: &Value) -> EvalResult<Value> {
    // Compact bags merge exactly like their equivalent Funcs.
    if let Value::Bag(b) = left {
        return merge_values(&Value::Func(Rp::clone(b.as_func_value())), right);
    }
    if let Value::Bag(b) = right {
        return merge_values(left, &Value::Func(Rp::clone(b.as_func_value())));
    }
    match (left, right) {
...
pub(super) fn value_to_json(val: &Value) -> String {
    match val {
        ...
        // Compact bag: serialize exactly like the equivalent Func.
        Value::Bag(b) => value_to_json(&Value::Func(Rp::clone(b.as_func_value()))),
...
pub(super) fn value_to_json_object(val: &Value) -> EvalResult<String> {
    // Compact bag: serialize exactly like the equivalent Func.
    if let Value::Bag(b) = val {
        return value_to_json_object(&Value::Func(Rp::clone(b.as_func_value())));
    }
    match val {
        Value::Record(fields) => {
            let items: Vec<String> = fields
