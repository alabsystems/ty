...
pub(super) fn eval_builtin_apalache(
    ...
    match name {
        ...
        "ApaFoldSeqLeft" => {
            ...
            let mut result = base;
            for elem in seq.iter() {
                result = apply_closure_with_values(ctx, op_closure, &[result, elem.clone()], span)?;
            }
            Ok(Some(result))
        }
        ...
        "MkSeq" => {
            ...
            // Fail-closed length bound. Both build paths below do
            // `Vec::with_capacity(n as usize)` from this unbounded user value;
            // a huge `n` (e.g. i64::MAX) makes with_capacity abort the process
            // and the `1..=n` loop run effectively forever. Reject oversized
            // requests BEFORE allocating. 100M elements is far beyond any
            // legitimate explicit-state sequence yet bounds the allocation.
            const MAX_MKSEQ_LEN: i64 = 100_000_000;
            if n > MAX_MKSEQ_LEN {
                return Err(EvalError::ArgumentError {
                    position: "first",
                    op: "MkSeq".to_string(),
                    expected_type: "non-negative integer within the supported length limit",
                    value_display: format!("{n}"),
                    span,
                });
            }
            ...
            if let Expr::Ident(op_name, _) = &args[1].node {
                if let Some(op_def) = ctx.get_op(op_name) {
                    ...
                    return Ok(Some(Value::Seq(Rp::new(result.into()))));
                }
            }
