...
use tla_core::ast::{
    BoundPattern, BoundVar, CaseArm, ExceptPathElement, ExceptSpec, Expr, ModuleTarget,
    OperatorDef, Substitution,
};
...
/// Resolve state variables in the right-hand side of an INSTANCE `WITH`
/// substitution, rewriting matching `Ident`s in `sub.to` to `StateVar`.
///
/// No names are treated as shadowed, since a substitution RHS is evaluated in the
/// enclosing module's scope where every state variable is in view.
...
/// Insert every name that a bound variable binds into `shadows`.
///
/// This is the binder name (`b.name`, which for a tuple binder is the synthetic
/// `<<a, b>>` placeholder string) PLUS, when the binder destructures a tuple
/// pattern `<<a, b, ...>> \in S`, each individual component name (`a`, `b`, ...).
///
/// Without inserting the tuple-component names, a body reference to a component
/// that happens to share a state-variable name would be wrongly rewritten to a
/// `StateVar` access — silently reading the wrong value. Because this resolution
/// pass also feeds the trust-cg / bytecode invariant compiler, that mis-resolution
/// would be a soundness hole: an invariant could be compiled against the state
/// array instead of the destructured tuple component. The fix is purely
/// structural — it is driven by the binder carrying a `BoundPattern::Tuple`,
/// never by any spec/variable name.
fn insert_bound_var_shadows<'a>(b: &'a BoundVar, shadows: &mut FxHashSet<&'a str>) {
    shadows.insert(b.name.node.as_str());
    if let Some(BoundPattern::Tuple(components)) = &b.pattern {
        for component in components {
            shadows.insert(component.node.as_str());
        }
    }
}
    ...
    match expr {
        ...
        Expr::Forall(bounds, body) | Expr::Exists(bounds, body) => {
            ...
            for b in bounds.iter() {
                insert_bound_var_shadows(b, &mut inner_shadows);
            }
            resolve_state_vars_in_spanned_with_shadows(body, registry, &inner_shadows);
        }
        Expr::Choose(bound, body) => {
            ...
            let mut inner_shadows: FxHashSet<&str> = shadows.clone();
            insert_bound_var_shadows(bound, &mut inner_shadows);
            resolve_state_vars_in_spanned_with_shadows(body, registry, &inner_shadows);
        }
