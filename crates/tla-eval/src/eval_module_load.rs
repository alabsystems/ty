...
impl EvalCtx {
    ...
    /// `__ty_subst_{name}` which resolve via the evaluator's normal local_stack
    ...
    fn let_wrap_substitutions(
        ...
        // 1. Build LET definitions: __ty_subst_{name} == <rhs_expr>
        let let_defs: Vec<OperatorDef> = substitutions
            .iter()
            .map(|sub| {
                let fresh_name = format!("__ty_subst_{}", sub.from.node);
                OperatorDef {
                    name: Spanned {
                        node: fresh_name,
                        span: sub.from.span,
        ...
        // 2. Build rename substitutions: original name -> Ident("__ty_subst_{name}")
