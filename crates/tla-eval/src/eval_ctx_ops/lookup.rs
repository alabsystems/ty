...
impl EvalCtx {
    ...
    fn lookup_inner(&self, name: &str, maybe_name_id: Option<NameId>) -> Option<Value> {
        ...
        if self.binding_depth > 0 {
            if let Some(lookup_id) = maybe_name_id {
                let binding = if crate::cache::is_subst_guarded(lookup_id) {
                    self.bindings.lookup_skip_instance(lookup_id)
                } else {
                    self.bindings.lookup(lookup_id)
                };
    ...
    /// Returns true if `name` resolves to anything in the *current local scope* —
    /// a quantifier/param binding, a LET-defined operator (overlay), or a local
    /// operator — as opposed to a module-level (shared) constant or operator.
    ///
    /// Used by the constant-domain cache to conservatively reject any domain that
    /// references a locally-scoped name, because such a name may capture an
    /// enclosing bound variable and therefore is not state-independent.
    #[inline]
    pub fn name_in_local_scope(&self, name: &str) -> bool {
