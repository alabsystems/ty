...
feature_flag!(no_implied_eq_fusion, "TY_NO_IMPLIED_EQ_FUSION");
...
/// Executor for compile-time constant folding (F1, lever L2): run a scratch
/// set-constructor function on the REAL VM with no state and an empty chunk.
///
/// Bit-identity with runtime by construction — the exact same opcode handlers
/// that would rebuild the value per state produce the folded constant once.
/// Any VM error refuses the fold (the compiler falls through to the normal
/// per-state path so the identical runtime error surfaces at the same point).
fn const_fold_vm_executor(
    func: &tla_tir::bytecode::BytecodeFunction,
    constants: &tla_tir::bytecode::ConstantPool,
) -> Result<Value, String> {
    let chunk = BytecodeChunk::new();
    let mut vm = super::BytecodeVm::new(&chunk, &[], None);
    vm.execute_detached_function(func, constants)
        .map_err(|e| e.to_string())
}

/// Install the const-fold executor into `tla-tir` (idempotent).
///
/// Called from every production compiler-construction site in this crate so
/// folding is available regardless of which pipeline compiles first. Until
/// installed, the compiler's fold hook silently never fires (fail-open).
pub(crate) fn ensure_const_fold_executor_installed() {
    static INSTALL: std::sync::Once = std::sync::Once::new();
    INSTALL.call_once(|| {
        tla_tir::bytecode::install_const_fold_executor(const_fold_vm_executor);
    });
}
...
pub fn compile_operators_to_bytecode_full(
    ...
    compile_operators_to_bytecode_full_with_state_vars(
        root,
        deps,
        op_names,
        resolved_constants,
...
/// Compile operators to bytecode with an explicit state-variable name→index map.
///
/// INSTANCE coverage fix: when an action/invariant references operators imported
/// through a named/unnamed `INSTANCE` layer (e.g. `Buffer!Write` where `Buffer ==
/// INSTANCE RingBuffer`), the instanced module's VARIABLE declarations (e.g.
/// `ringbuffer`) are mapped to the parent module's same-named state variables via
/// the instance's implicit same-name substitution. TIR seeding lowers those
/// references as bare `TirNameKind::Ident` (TIR lowering does not perform
/// state-var resolution), so the bytecode compiler cannot resolve them to a
/// `LoadVar`/`StoreVar` slot and rejects the body with `unresolved identifier`.
///
/// Passing the model checker's `VarRegistry` name→index map here lets the
/// compiler's `compile_name_expr` fallback resolve those substituted instance
/// state-variable Idents to the correct outer slot — exactly mirroring the
/// interpreter, which resolves the same names against the outer state. Bound
/// variables, constants, and operators are still resolved first, so this only
/// captures genuinely-unresolved Idents that name an outer state variable.
pub fn compile_operators_to_bytecode_full_with_state_vars(
    root: &Module,
    deps: &[&Module],
    op_names: &[String],
    resolved_constants: &crate::HashMap<NameId, Value>,
    ...
    compile_operators_to_bytecode_full_with_state_vars_impl(
        root,
        deps,
        op_names,
        resolved_constants,
...
/// Like [`compile_operators_to_bytecode_full_with_state_vars`] but with the
/// compiler's register recycling enabled. Used for implied-action term
/// compilation, where the fully inlined action disjunction would overflow the
/// 256-register file under the default (non-recycling) bump allocator.
/// Existing compilation paths keep the default allocator bit-for-bit.
#[allow(clippy::too_many_arguments)]
pub fn compile_operators_to_bytecode_with_reg_recycling(
    root: &Module,
    deps: &[&Module],
    op_names: &[String],
    resolved_constants: &crate::HashMap<NameId, Value>,
    ...
    tir_callee_bodies: Option<&std::collections::HashMap<String, tla_tir::bytecode::CalleeInfo>>,
    state_vars: Option<&std::collections::HashMap<String, u16>>,
    force_external: Option<&std::collections::HashSet<String>>,
) -> CompiledBytecode {
    compile_operators_to_bytecode_full_with_state_vars_impl(
        root,
        deps,
...
#[allow(clippy::too_many_arguments)]
/// Like [`compile_operators_to_bytecode_full_with_state_vars`] but with
/// single-accumulator And/Or chain FLATTENING enabled (no register recycling):
/// multi-arm action disjunctions compile to the canonical flat OR chain the
/// top-level-disjunction splitter detects, while registers stay
/// monotone-unique so the next-state transform's interference checks are
/// unaffected. Used for action compilation.
#[allow(clippy::too_many_arguments)]
pub fn compile_operators_to_bytecode_with_flat_bool_chains(
    root: &Module,
    deps: &[&Module],
    op_names: &[String],
    resolved_constants: &crate::HashMap<NameId, Value>,
    ...
    compile_operators_to_bytecode_full_with_state_vars_impl2(
        root,
        deps,
        op_names,
        resolved_constants,
        ...
        tir_callee_bodies,
        state_vars,
        CompileRegMode::FlatBoolChains,
        None,
    )
}

/// Register-allocation mode for the bytecode compiler entry points.
...
fn compile_operators_to_bytecode_full_with_state_vars_impl(
    root: &Module,
    deps: &[&Module],
    op_names: &[String],
    resolved_constants: &crate::HashMap<NameId, Value>,
    ...
    compile_operators_to_bytecode_full_with_state_vars_impl2(
        ...
        state_vars,
        if reg_recycle {
            CompileRegMode::RegRecycle
        } else {
            CompileRegMode::Default
        },
        force_external,
    )
}
...
#[allow(clippy::too_many_arguments)]
fn compile_operators_to_bytecode_full_with_state_vars_impl2(
    root: &Module,
    deps: &[&Module],
    op_names: &[String],
    resolved_constants: &crate::HashMap<NameId, Value>,
    ...
    ensure_const_fold_executor_installed();
    ...
    match reg_mode {
        CompileRegMode::RegRecycle => {
            ...
            // Eq-fusion superinstructions (EqFuncExcept / EqRecordNew):
            // enabled ONLY on the reg-recycled entry point, which is used
            // exclusively for implied-action term compilation (the eval-VM
            // fast path). Action/invariant/constraint bytecode consumed by
            // the trust-cg native lowering compiles through the other modes
            // and never contains fused opcodes. Kill switch:
            // TY_NO_IMPLIED_EQ_FUSION=1.
            if !no_implied_eq_fusion() {
                compiler.enable_eq_fusion();
            }
        }
        CompileRegMode::FlatBoolChains => compiler.enable_bool_chain_flattening(),
        CompileRegMode::Default => {}
    }
    if let Some(force) = force_external {
        if !force.is_empty() {
            compiler.set_force_external_ops(force.clone());
        }
    }
    if let Some(state_vars) = state_vars {
        if !state_vars.is_empty() {
            compiler.set_state_vars(state_vars.clone());
        }
    }
    ...
    let stats_phase1 = {
        ...
        *BVM_STATS
            .get_or_init(|| matches!(std::env::var("TY_BYTECODE_VM_STATS").as_deref(), Ok("1")))
    };
...
mod tests {
    ...
    /// Register recycling (implied-action compiles): a wide disjunction
    /// overflows the default 256-register bump allocator but compiles with
    /// `compile_operators_to_bytecode_with_reg_recycling`, and the recycled
    /// code computes the correct verdict on both sides of the boolean.
    #[test]
    fn test_wide_disjunction_needs_register_recycling() {
        let disjuncts: Vec<String> = (0..300).map(|i| format!("x = {i}")).collect();
        let src = format!(
            "---- MODULE BytecodeRegRecycle ----\nVARIABLE x\nBig == {}\n====",
            disjuncts.join(" \\/ ")
        );
        let module = parse_module(&src);
        let state_vars: std::collections::HashMap<String, u16> =
            [("x".to_string(), 0_u16)].into_iter().collect();
        let names = ["Big".to_string()];

        // Default allocator: must fail with register overflow.
        let plain = super::compile_operators_to_bytecode_full_with_state_vars(
            &module,
            &[],
            &names,
        ...
        assert!(
            plain.failed.iter().any(|(name, _)| name == "Big"),
            "300-way disjunction should overflow the default register allocator"
        );

        // Recycling allocator: compiles and evaluates correctly.
        let recycled = super::compile_operators_to_bytecode_with_reg_recycling(
            &module,
            &[],
            &names,
            &crate::HashMap::default(),
        ...
        assert!(
            recycled.failed.is_empty(),
            "register recycling should compile the wide disjunction: {:?}",
            recycled.failed
        );
        let func_idx = *recycled.op_indices.get("Big").expect("Big compiled");

        let in_range = [Value::int(150)];
        let mut vm = BytecodeVm::new(&recycled.chunk, &in_range, None);
        assert_eq!(
            vm.execute_function(func_idx).expect("recycled exec"),
            Value::Bool(true),
            "x=150 satisfies one disjunct"
        );

        let out_of_range = [Value::int(999)];
        let mut vm = BytecodeVm::new(&recycled.chunk, &out_of_range, None);
        assert_eq!(
            vm.execute_function(func_idx).expect("recycled exec"),
            Value::Bool(false),
            "x=999 satisfies no disjunct"
        );
    }

    /// Force-external pinning: a pinned operator compiles as `CallExternal`
    /// even though its body is compilable, and executing without an `EvalCtx`
    /// therefore fails (proof the callback is really external).
    #[test]
    fn test_force_external_pins_operator_to_call_external() {
        let module = parse_module(
            "\
---- MODULE BytecodeForceExternal ----
VARIABLE x
d == x
Use == d = x
====",
        ...
        let state_vars: std::collections::HashMap<String, u16> =
            [("x".to_string(), 0_u16)].into_iter().collect();
        let force: std::collections::HashSet<String> = ["d".to_string()].into_iter().collect();
        let compiled = super::compile_operators_to_bytecode_with_reg_recycling(
            &module,
            &[],
            &["Use".to_string()],
            &crate::HashMap::default(),
        ...
        let func_idx = *compiled.op_indices.get("Use").expect("Use compiled");
        let state = [Value::int(7)];
        let mut vm = BytecodeVm::new(&compiled.chunk, &state, None);
        let err = vm
            .execute_function(func_idx)
            .expect_err("pinned operator must require the interpreter callback");
        assert!(
            err.to_string().contains("EvalCtx"),
            "expected CallExternal-requires-EvalCtx error, got: {err}"
        );
    }
...
====",
    ...
    #[test]
    fn diag_tuple_pattern_forall_compiles_through_full_pipeline() {
        use tla_core::ast::Unit;
        use tla_core::VarRegistry;
        let mut module = parse_module(
            "\
---- MODULE BytecodeTuplePatternForall ----
EXTENDS Naturals
VARIABLES x
P == {<<1, 2>>, <<2, 3>>}
Init == x = 0
Next == x' = x
Inv == \\A <<a, b>> \\in P : a + b >= 0
====",
        );
        // Mirror the model-checker path: resolve state vars before lowering.
        let registry = VarRegistry::from_names(["x"]);
        for unit in &mut module.units {
            if let Unit::Operator(def) = &mut unit.node {
                crate::state_var::resolve_state_vars_in_op_def(def, &registry);
            }
        }
        let dep_refs: [&tla_core::ast::Module; 0] = [];
        let tir_callees = collect_bytecode_namespace_callees(&module, &dep_refs);
        ...
        assert!(
            compiled.failed.is_empty(),
            "tuple-pattern quantifier invariant should compile: {:?}",
            compiled.failed
        );
    }
...
====",
    ...
    #[test]
    fn test_named_instance_operator_compiles_as_direct_qualified_entry() {
        let inner = parse_module(
            "\
---- MODULE BytecodeNamedInstanceDirectEntry ----
CONSTANT K
Base == K
Derived == Base + 100
====",
        ...
        let wrapper = parse_module(
            "\
---- MODULE BytecodeNamedInstanceDirectWrapper ----
I == INSTANCE BytecodeNamedInstanceDirectEntry WITH K <- 7
====",
        );
        let dep_refs = [&inner];
        let compiled = compile_operators_to_bytecode_with_constants(
        ...
        assert!(
            compiled.failed.is_empty(),
            "direct named INSTANCE entry should compile: {:?}",
            compiled.failed
        );
        let func_idx = *compiled
            .op_indices
            .get("I!Derived")
            .expect("qualified instance entry should be compiled");
        let mut vm = BytecodeVm::new(&compiled.chunk, &[], None);
        let value = vm
            .execute_function(func_idx)
            .expect("qualified instance entry should execute");
        assert_eq!(value, Value::int(107));
    }
