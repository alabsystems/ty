...
impl<'a> BytecodeVm<'a> {
    ...
    // Intentional `&mut Vec<Value>` (not &mut [Value]): the dispatch loop may
    // resize regs via `push()` when a callee needs additional scratch slots.
                ...
                Opcode::SetPrimeMode { enable } => {
                    if self.is_action_execution() {
                        return Err(VmError::Unsupported(
                            "SetPrimeMode in action bytecode".to_string(),
                        ));
                    }
