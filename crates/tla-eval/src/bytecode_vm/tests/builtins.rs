...
#[test]
fn test_vm_builtin_remove_at() {
    // RemoveAt(<<10, 20, 30>>, 2) => <<10, 30>>
    let result = exec_simple(
        vec![
            Opcode::LoadImm { rd: 0, value: 10 },
            Opcode::LoadImm { rd: 1, value: 20 },
    ...
    assert_eq!(
        result,
        Value::seq(vec![Value::SmallInt(10), Value::SmallInt(30)])
    );
}

#[test]
fn test_vm_builtin_remove_at_first() {
    // RemoveAt(<<10, 20, 30>>, 1) => <<20, 30>>
    let result = exec_simple(
        vec![
            Opcode::LoadImm { rd: 0, value: 10 },
            Opcode::LoadImm { rd: 1, value: 20 },
...
#[test]
fn test_vm_builtin_remove_at_last() {
    // RemoveAt(<<10, 20, 30>>, 3) => <<10, 20>>
    let result = exec_simple(
        vec![
            Opcode::LoadImm { rd: 0, value: 10 },
            Opcode::LoadImm { rd: 1, value: 20 },
    ...
    assert_eq!(
        result,
        Value::seq(vec![Value::SmallInt(10), Value::SmallInt(20)])
    );
}
...
#[test]
fn test_vm_builtin_range_seq_dedups() {
    // Range(<<1, 0, 1>>) == {0, 1} — the set of values the sequence takes,
    // deduplicated like a normal TLA+ set. Mirrors the interpreter's "Range"
    // arm in builtin_stdlib_ext.rs (Value::set over the mapping values).
    let result = exec_simple(
        vec![
            Opcode::LoadImm { rd: 0, value: 1 },
    ...
    assert_eq!(result, Value::set([Value::SmallInt(0), Value::SmallInt(1)]));
}

#[test]
fn test_vm_builtin_range_membership() {
    // 0 \in Range(<<0, 2>>) should be TRUE — the load-bearing shape for the
    // RangeInvariantViolated soundness fixture.
    let result = exec_simple(
        vec![
