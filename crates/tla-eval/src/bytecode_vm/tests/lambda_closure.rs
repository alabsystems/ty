...
fn test_vm_capturing_lambda_via_make_closure() {
    ...
    let template_idx = constants.add_value(Value::Closure(Rp::new(template)));

    // Capture name "y" follows the template in the constant pool
    constants.add_value(Value::string("y"));
...
fn test_vm_capturing_lambda_multiple_captures() {
    ...
    constants.add_value(Value::string("a"));
    constants.add_value(Value::string("b"));
...
fn test_vm_make_closure_builds_correct_env() {
    ...
    .with_bytecode_func_idx(0);
    let template_idx = constants.add_value(Value::Closure(Rp::new(template)));
    constants.add_value(Value::string("captured_var"));
