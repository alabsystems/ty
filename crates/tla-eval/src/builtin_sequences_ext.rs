...
pub(super) fn eval_builtin_sequences_ext(
    ...
    match name {
        "Reverse" => {
            ...
            Ok(Some(Value::Seq(Rp::new(reversed.into()))))
        }
        ...
        "Front" => {
            ...
            if let Some(seq_value) = sv.as_seq_value() {
                ...
                return Ok(Some(Value::Seq(Rp::new(seq_value.front()))));
            }
            // Fallback for Tuple
            let seq = sv
                .as_seq()
        ...
        "SetToSeq" => {
            ...
            Ok(Some(Value::Seq(Rp::new(seq.into()))))
        }
        ...
        "SetToSortSeq" => {
            ...
            Ok(Some(Value::Seq(Rp::new(elements.into()))))
        }
        ...
        "ToSet" => {
            ...
            Ok(Some(Value::Set(Rp::new(SortedSet::from_iter(set)))))
        }
        ...
        "Cons" => {
            ...
            Ok(Some(Value::Seq(Rp::new(new_seq.into()))))
        }
        ...
        "Snoc" => {
            ...
            if let Some(seq_value) = sv.as_seq_value() {
                return Ok(Some(Value::Seq(Rp::new(seq_value.append(elem)))));
            }
            // Fallback for Tuple
            let seq = sv
                .as_seq()
        ...
        "LongestCommonPrefix" => {
            ...
            if seqs_set.is_empty() {
                ...
                return Ok(Some(Value::Seq(Rp::new(Vec::new().into()))));
            }
            ...
            Ok(Some(Value::Seq(Rp::new(lcp.into()))))
        }
        ...
        "CommonPrefixes" => {
            ...
            if seqs_set.is_empty() {
                ...
                return Ok(Some(Value::Set(Rp::new(SortedSet::from_iter(vec![
                    Value::Seq(Rp::new(Vec::new().into())),
                ])))));
            }
            ...
            result.push(Value::Seq(Rp::new(Vec::new().into()))); // Empty sequence is always a common prefix
            ...
            for len in 1..=min_len {
                ...
                {
                    result.push(Value::Seq(Rp::new(prefix.to_vec().into())));
                } else {
                    break; // Once we find a non-common prefix, no longer ones can be common
                }
            }

            Ok(Some(Value::Set(Rp::new(SortedSet::from_iter(result)))))
        }
