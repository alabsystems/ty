...
pub(super) fn eval_bag_union(
    ...
    // Promote eligible results to the compact bag rep (fail-closed to Func).
    Ok(Some(super::bag_from_sorted_entries(entries)))
}
    ...
    fn eval_unary_bag_map(
        ...
        match &op.node {
            ...
            Expr::Ident(_, _) => {
                ...
                let tmp = "__ty_bagofall_arg".to_string();
...
pub(super) fn eval_sub_bag(
    ...
    fn enumerate_subbags(
        ...
        if idx == elems.len() {
            out.push(Value::Func(Rp::new(FuncValue::from_sorted_entries(
                entries,
            ))));
            return;
        }
    ...
    Ok(Some(Value::Set(Rp::new(SortedSet::from_iter(out)))))
}
