...
/// kill switch off), else the general `Func` (fail-closed; identical value).
/// Value-level `SetToBag(S)` over already-iterated set elements — shared by
/// the interpreter arm and the bytecode VM `CallBuiltin` dispatch (results
/// value- and fingerprint-identical). Sorts, then promotes via
/// [`bag_from_sorted_entries`], exactly like the historical AST arm.
pub(crate) fn set_to_bag_from_elems(elems: impl IntoIterator<Item = Value>) -> Value {
    let mut entries: Vec<(Value, Value)> = elems
        .into_iter()
        .map(|elem| (elem, Value::SmallInt(1)))
        .collect();
    entries.sort_by(|a, b| a.0.cmp(&b.0));
    bag_from_sorted_entries(entries)
}

pub(crate) fn bag_from_sorted_entries(entries: Vec<(Value, Value)>) -> Value {
    match BagValue::try_from_entries(entries) {
        Ok(bag) => Value::Bag(Rp::new(bag)),
        Err(entries) => Value::Func(Rp::new(FuncValue::from_sorted_entries(entries))),
    }
}
...
pub(super) fn eval_builtin_bags(
    ...
    match name {
        ...
        "EmptyBag" => {
            ...
            if tla_value::compact_bags_enabled() {
                return Ok(Some(Value::Bag(BagValue::empty_arc())));
            }
            Ok(Some(Value::Func(Rp::new(FuncValue::from_sorted_entries(
                vec![],
            )))))
        }
        ...
        "IsABag" => {
            ...
            // Compact bag: counts are positive by construction invariant.
            if matches!(bv, Value::Bag(_)) {
        ...
        "SetToBag" => {
            ...
            // Shared value-level tail (also the bytecode VM's CallBuiltin
            // dispatch); #1828: eval_iter_set for SetPred-aware iteration.
            check_arity(name, args, 1, span)?;
            let sv = eval(ctx, &args[0])?;
            let set_iter = eval_iter_set(ctx, &sv, Some(args[0].span))?;
            Ok(Some(set_to_bag_from_elems(
                set_iter.into_iter().map(|e| e.clone()),
            )))
        }
        ...
        "BagToSet" => {
            ...
            check_arity(name, args, 1, span)?;
            let bv = eval(ctx, &args[0])?;
            // Compact bag: cached DOMAIN set (identical order to the general path).
            if let Value::Bag(b) = &bv {
                return Ok(Some(b.domain_set_value()));
            }
            let func = bv
                .to_func_coerced()
                .ok_or_else(|| EvalError::type_error("Bag/Function", &bv, Some(args[0].span)))?;
            let domain: Vec<Value> = func.domain_iter().cloned().collect();
            Ok(Some(Value::Set(Rp::new(SortedSet::from_sorted_vec(
                domain,
            )))))
        }
        ...
        "BagIn" => {
            ...
            // Compact bag: counts are positive by construction.
            if let Value::Bag(b) = &bv {
                return Ok(Some(Value::Bool(b.domain_contains(&ev))));
            }
            let func = bv
                .to_func_coerced()
        ...
        "CopiesIn" => {
            ...
            let bv = eval(ctx, &args[1])?;
            if let Value::Bag(b) = &bv {
                return Ok(Some(Value::SmallInt(b.count_of(&ev))));
            }
            let func = bv
                .to_func_coerced()
                .ok_or_else(|| EvalError::type_error("Bag/Function", &bv, Some(args[1].span)))?;
        ...
        "BagCardinality" => {
            ...
            check_arity(name, args, 1, span)?;
            let bv = eval(ctx, &args[0])?;
            // Compact bag: Σ counts (counts are positive by invariant).
            // Overflow (None) falls through to the general BigInt path.
            if let Value::Bag(b) = &bv {
                if let Some(total) = b.cardinality() {
                    return Ok(Some(Value::SmallInt(total)));
                }
            }
            let func = bv
                .to_func_coerced()
        ...
        "BagCup" | "\\oplus" => {
            ...
            // Compact fast path: sorted merge with added counts. Overflow
            // (None) falls through to the general BigInt path.
            if let (Value::Bag(b1), Value::Bag(b2)) = (&bv1, &bv2) {
                if let Some(merged) = b1.bag_cup(b2) {
                    return Ok(Some(Value::Bag(Rp::new(merged))));
                }
            }
            let func1 = bv1
                .to_func_coerced()
        ...
        "BagDiff" | "\\ominus" => {
            ...
            let bv1 = eval(ctx, &args[0])?;
            let bv2 = eval(ctx, &args[1])?;
            // Compact fast path: subtract counts, keep positives.
            if let (Value::Bag(b1), Value::Bag(b2)) = (&bv1, &bv2) {
                return Ok(Some(Value::Bag(Rp::new(b1.bag_diff(b2)))));
            }
            let func1 = bv1
                .to_func_coerced()
                .ok_or_else(|| EvalError::type_error("Bag/Function", &bv1, Some(args[0].span)))?;
