...
use super::{eval_iter_set, EvalCtx, EvalError, EvalResult, Expr, Span, Spanned, Value};
...
/// Maximum universe size for eager subset enumeration. 2^20 == ~1M subsets is
/// the same ceiling the core `powerset()` builder uses. Above this we DECLINE
/// (fail-closed) rather than (a) overflow `1u64 << n` for n >= 64, or
/// (b) attempt an enormous allocation / non-terminating enumeration.
const MAX_SUBSET_UNIVERSE: usize = 20;

/// Enumerate all subsets of a slice.
///
/// Returns `EvalError::SetTooLarge` when the universe exceeds
/// [`MAX_SUBSET_UNIVERSE`], BEFORE allocating, so a malformed/oversized input
/// declines instead of panicking on shift overflow or exhausting memory.
pub(super) fn subsets<T: Clone>(items: &[T]) -> EvalResult<Vec<Vec<T>>> {
    let n = items.len();
    if n > MAX_SUBSET_UNIVERSE {
        return Err(EvalError::SetTooLarge { span: None });
    }
    // n <= 20 here, so `1usize << n` cannot overflow on any supported target.
