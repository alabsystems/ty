...
pub(super) fn eval_expr(ctx: &EvalCtx, expr: &Expr, span: Option<Span>) -> EvalResult<Value> {
    // Constant set-producing subexpression cache. A cheap structural filter
    // rejects the vast majority of nodes (everything that is not a set
    // constructor); only set-producing kinds consult the per-run const cache.
    // The cache holds set values proven state- AND bound-var-independent, built
    // once per run and reused instead of rebuilt every state. It is consulted on
    // BOTH the hoist and non-hoist paths so constant set subexpressions inside
    // quantifier / set-builder loop bodies (which run under hoist) are also
    // served once-per-run instead of once-per-state-generation.
    if crate::cache::const_set_enabled() && crate::cache::is_const_cacheable_set(expr) {
        return eval_const_cacheable_set(ctx, expr, span);
    }
...
/// Dispatch a set node WITHOUT re-entering the const-set hook, respecting the
/// hoist path when a hoist scope is active. Used both as the normal fall-through
/// for set nodes and as the probe body for `const_set_probe_and_record`.
#[inline]
fn dispatch_set_node(ctx: &EvalCtx, expr: &Expr, span: Option<Span>) -> EvalResult<Value> {
    if ctx.runtime_state.hoist_active.get() {
        eval_expr_with_hoist(ctx, expr, span)
    } else {
        eval_expr_dispatch(ctx, expr, span)
    }
}

/// Const-set-cache path for set-producing nodes.
///
/// Probes the per-run constant-set cache; on a hit returns the cached interned
/// set without rebuilding the Vec. On a first-encounter miss, evaluates the node
/// once under dependency tracking to decide whether it is a true constant, then
/// either caches it (constant) or records it as non-constant (rebuild forever).
///
/// Part of the constant set-construction redundancy elimination: profiling
/// showed ~100% of `from_vec`/`intern_set_array` set builds rebuild content that
/// was already built, because constant set subexpressions are re-evaluated per
/// state. Caching the proven-constant ones removes the redundant rebuild.
#[inline]
fn eval_const_cacheable_set(ctx: &EvalCtx, expr: &Expr, span: Option<Span>) -> EvalResult<Value> {
    // Suppressed scope (ENABLED / call-by-name): never probe or cache — rebuild
    // (routed through the hoist-aware dispatch so loop bodies still hoist).
    if crate::cache::const_set_caching_suppressed(ctx) {
        return dispatch_set_node(ctx, expr, span);
    }

    let key = crate::cache::const_set_key_of(ctx, expr);

    // Fast path: proven-constant hit. A single map probe; no eval, no rebuild.
    // The span validates the entry against AST-pointer reuse (bug #1558).
    if let Some(v) = crate::cache::const_set_lookup(&key, span) {
        return Ok(v);
    }

    // Known non-constant: skip the probe; rebuild via the hoist-aware dispatch.
    if crate::cache::const_set_is_known_non_const(&key) {
        return dispatch_set_node(ctx, expr, span);
    }

    // First encounter: probe under dep tracking. The probe runs the dispatch for
    // THIS node (so it does not re-enter this hook); the node's children are
    // evaluated through the normal hook and may themselves be cached.
    crate::cache::const_set_probe_and_record(ctx, key, span, || dispatch_set_node(ctx, expr, span))
}
...
fn eval_expr_dispatch(ctx: &EvalCtx, expr: &Expr, span: Option<Span>) -> EvalResult<Value> {
    match expr {
        ...
        Expr::Div(a, b) => eval_real_div(ctx, a, b, span),
...
#[cold]
#[inline(never)]
#[allow(clippy::ptr_arg, clippy::borrowed_box)] // Intentional: need &Vec / &Box pointer identity for cache key
fn eval_subst_in(
    ctx: &EvalCtx,
