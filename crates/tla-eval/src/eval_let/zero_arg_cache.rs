...
/// Fix #4145: Check if a deferred value captures state and should not be treated as constant.
///
/// LazyFunc and SetPred values capture current-state arrays at definition time.
/// Their deferred bodies read state only when consumed, so dependency tracking
/// during wrapper construction sees no read. Without this structural guard the
/// LET caches can reuse a stale captured state in a later BFS state.
#[inline]
    ...
    for name_id in dep_names {
        ...
        match ctx.bindings.lookup(*name_id) {
            Some((bv, source)) => match bv.get_if_ready(StateLookupMode::Current, source) {
                Some(val) => {
                    // Part of #4462: use the additive-fingerprint cache-key
                    // hash instead of a deep structural walk. Compound dep
                    // values (e.g. a bound `deliveredVertices` set on
                    // dag-consensus specs) were deep-hashed on every probe —
                    // 10% of total cycles on TLCSailfish1. Sound because hits
                    // verify `stored_deps == dep_vals` before acceptance; the
                    // hash only routes to a bucket. Kill switch:
                    // TY_NO_FP_ARGS_HASH=1 restores the deep hash.
                    crate::cache::op_result_cache::hash_value_cache_key(&val, &mut hasher);
                    dep_values.push(val);
                }
