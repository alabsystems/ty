    ...
    for def in defs {
        if primed_refs.contains(def.name.node.as_str()) {
            ...
            bound_ctx.let_def_overlay = bound_ctx
                .let_def_overlay
                .cons(name, crate::cache::intern_let_def_arc(def));
            continue;
        }
        ...
        if let Expr::FuncDef(bounds, func_body) = &def.body.node {
            if expr_mentions_name_v(&func_body.node, def.name.node.as_str()) {
                ...
                bound_ctx.let_def_overlay = bound_ctx
                    .let_def_overlay
                    .cons(Arc::clone(&name), crate::cache::intern_let_def_arc(def));
                ...
                bound_ctx = bound_ctx.into_bind_local(name, Value::LazyFunc(Rp::new(lazy)));
                continue;
            }
        }
