...
use super::bytecode_integration::{
    bytecode_vm_enabled, record_bytecode_vm_fallback, BytecodeCache,
};
use super::eval_tir;
...
// remaining correctness regressions under TY_TIR_EVAL=all. The #3361 fix
...
impl<'a> TirProgram<'a> {
    ...
    ///
    /// When `TY_PARTIAL_EVAL=1` (or `--partial-eval`) is set AND `env` is
    ...
    pub fn eval_named_op(&self, ctx: &EvalCtx, name: &str) -> EvalResult<Value> {
        ...
        //
        // Run the VM attempt UNDER the same `eval_entry_with` cache-lifecycle
        // boundary the tree-walk path below uses. A VM execution that succeeds
        // still drives per-state cache growth (interned intermediates, subst
        // entries) exactly like a tree-walk of the same operator; without the
        // boundary those caches were only trimmed on the *fallback* path, so a
        // large invariant that flips from "VM error → tree-walk" to "VM success"
        // (e.g. `TypeOK` once `x \in (Nat \ {0})` stopped hard-erroring) would
        // halve the trim cadence and let working memory balloon (~4x observed on
        // TLCSailfish1). Wrapping the attempt keeps the boundary invariant.
        if bytecode_vm_enabled() {
            let vm_attempt =
                eval_entry_with(ctx, || match self.try_bytecode_eval(ctx, name, &tir_body) {
        ...
        if let Err(tir_error) = result {
            return match ctx.eval_op(name) {
                Ok(value) => Ok(value),
                Err(ast_error) => {
                    // Some focused tests build a TirProgram without loading the module
                    // into EvalCtx. In that case AST fallback cannot resolve the
                    // top-level op name, so preserve the real TIR tree-walk error.
                    if matches!(&ast_error, EvalError::UndefinedOp { name: op, .. } if op == name) {
                        return Err(tir_error);
                    }
                    Err(ast_error)
                }
            };
    ...
    pub fn try_eval_expr_detailed(
        ...
        // Instance-qualified config replacements such as `I!X <- XOverride`
        // are applied by the AST ModuleRef evaluator. TIR lowering resolves
        // ModuleRef nodes by inlining the target operator, which would bypass
        // that replacement and evaluate the original instanced body.
        if direct_module_ref_replacement(ctx, expr) {
        ...
        match tla_tir::lower_expr_with_env(&self.env, self.root, expr) {
            Ok(tir) => {
                ...
                // names. Only activates when `TY_PARTIAL_EVAL=1` is set and a
...
fn direct_module_ref_replacement(ctx: &EvalCtx, expr: &Spanned<Expr>) -> bool {
    let Expr::ModuleRef(target, op_name, _args) = &expr.node else {
        return false;
    };
    crate::helpers::module_ref_compound_key(target, op_name)
        .is_some_and(|key| ctx.op_replacements().contains_key(&key))
}
