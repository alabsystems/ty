...
#[test]
fn test_bytecode_vm_caches_next_state_requirement_with_compiled_result() {
    let module = parse_module(
        "\
---- MODULE BytecodeNextStateRequirementCache ----
Check == 1 + 2 = 3
====",
    );
    let _guard = enable_bytecode_vm_with_stats_for_test();
    clear_for_test_reset();
    crate::tir::reset_bytecode_next_state_scan_count();
    ...
    for _ in 0..2 {
        assert_eq!(
            program
                ...
                .expect("pure operator should execute via bytecode"),
            Value::Bool(true),
        );
    }

    assert_eq!(
        crate::tir::bytecode_next_state_scan_count(),
        1,
        "the immutable call graph should be analyzed only when the entry is compiled",
    );
    assert_eq!(bytecode_vm_stats(), (2, 0, 0));
}
...
fn test_bytecode_vm_enabled_defaults_on_and_disables_on_zero() {
    assert!(
        ...
        "bytecode VM should be enabled by default when TY_BYTECODE_VM is unset (#3670)"
    );
    assert!(
        bytecode_vm_enabled_from_value(Some("1")),
        "bytecode VM should be enabled when TY_BYTECODE_VM=1"
    );
    assert!(
        !bytecode_vm_enabled_from_value(Some("0")),
        "TY_BYTECODE_VM=0 should disable the bytecode VM"
    );
    assert!(
...
#[test]
fn test_tir_tree_walk_executes_three_level_except_path_spill() {
    let module = parse_module(
        "\
---- MODULE TirTreeWalkExceptPathSpill ----
Base == [a |-> [b |-> [c |-> 1, keep |-> 9]]]
Updated == [Base EXCEPT !.a.b.c = @ + 6]
Check == Updated.a.b.c = 7 /\\ Updated.a.b.keep = 9
====",
    );
    ...
    assert_eq!(
        program
            .eval_named_op(&ctx, "Check")
            .expect("three-level EXCEPT should execute through the TIR tree walker"),
        Value::Bool(true)
    );
    assert_eq!(
        bytecode_vm_stats(),
        (0, 0, 0),
        "bytecode must remain disabled so this exercises TIR path resolution",
    );
}
...
#[test]
fn test_bytecode_vm_subset_domain_function_sorted_parity() {
    // Regression: the VM's FuncDef loop collects entries in domain-iteration
    // order, and SUBSET domains iterate cardinality-first ({2} before {1, 2})
    // which is NOT Value::cmp order. The built FuncValue must still be sorted
    // so application and equality work — and the result must be identical to
    // the tree-walker's.
    let module = parse_module(
        "\
---- MODULE BytecodeSubsetDomainFunc ----
F == [s \\in SUBSET {1, 2} |-> s]
Check == /\\ [s \\in SUBSET {1, 2} |-> s][{1, 2}] = {1, 2}
         /\\ [s \\in SUBSET {1, 2} |-> s][{}] = {}
         /\\ [s \\in SUBSET {1, 2} |-> s] = [s \\in {{}, {1}, {2}, {1, 2}} |-> s]
====",
    );
    let _guard = enable_bytecode_vm_with_stats_for_test();
    clear_for_test_reset();
    ...
    let value = program
        .eval_named_op(&ctx, "Check")
        .expect("SUBSET-domain function checks should evaluate via bytecode");
    assert_eq!(
        value,
        Value::Bool(true),
        "SUBSET-domain function application/equality must hold in the VM"
    );

    // Cross-engine parity: the tree-walker and the bytecode VM must build the
    // exact same (sorted) function value.
    let vm_val = program
        .eval_named_op(&ctx, "F")
        .expect("SUBSET-domain function should evaluate via bytecode");
    let ast_val = ctx
        .eval_op("F")
        .expect("SUBSET-domain function should evaluate via the tree-walker");
    assert_eq!(
        ast_val, vm_val,
        "tree-walker and bytecode VM must agree on the SUBSET-domain function"
    );

    // Both entry points above must have executed via the bytecode VM.
    let (executed, fallbacks, errors) = bytecode_vm_stats();
    assert_eq!(
        (fallbacks, errors),
        (0, 0),
        "SUBSET-domain function should execute via bytecode without fallback",
    );
    assert!(
        executed >= 2,
        "expected bytecode VM executions, got {executed}"
    );
}
