...
//! guarded by `TY_TIR_RECURSIVE_RESOLVE=1`.
...
pub(crate) fn install_tir_program<'a>(program: &'a TirProgram<'a>) -> TirProgramGuard {
    let ptr = (program as *const TirProgram<'a>).cast::<()>();
...
///
/// DISABLED (#3392 measurement): recursive TIR resolution regresses trust-wp_cargo_lock
...
pub(crate) fn try_resolve_operator_tir(name: &str) -> Option<std::sync::Arc<Spanned<TirExpr>>> {
    ...
    let program: &TirProgram<'_> = unsafe { &*ptr.cast::<TirProgram<'_>>() };
...
pub(crate) use bytecode_integration::{
    bytecode_next_state_scan_count, bytecode_vm_enabled_from_env_value,
    reset_bytecode_next_state_scan_count, set_bytecode_vm_test_overrides,
    vm_round_step_eq_enabled_from_env_value, BytecodeVmTestOverridesGuard,
};
...
///
/// Enabled by default. Set `TY_NO_PREPROCESS=1` or pass `--no-preprocess` to
...
pub(crate) fn preprocess_enabled() -> bool {
    static ENABLED: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var("TY_NO_PREPROCESS").map_or(true, |v| v != "1"));
    *ENABLED
}

/// Whether TIR partial evaluation of CONSTANTS is enabled.
///
/// Disabled by default. Set `TY_PARTIAL_EVAL=1` or pass `--partial-eval` to
...
/// preprocessing pipeline runs. Downstream passes (const_prop, inlining, trust-ir
...
pub(crate) fn partial_eval_enabled() -> bool {
    static ENABLED: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var("TY_PARTIAL_EVAL").is_ok_and(|v| v == "1"));
    *ENABLED
}
