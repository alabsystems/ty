...
pub(super) fn eval_tir_in(
    ...
    if let TirExpr::Tuple(elements) = &elem.node {
        if let [first_expr, second_expr] = elements.as_slice() {
            // Preserve the ordinary tuple path's component-then-RHS order.
            let first = eval_tir(ctx, first_expr)?;
            let second = eval_tir(ctx, second_expr)?;
            let tuple = [first, second];
            let bv = eval_tir(ctx, set)?;

            if let Some(contains) = bv.try_set_contains_tuple_elements(&tuple) {
                tla_value::churn_stats::churn_count(
                    tla_value::churn_stats::ChurnSite::TupleKeyInFused,
                );
                return Ok(Value::Bool(contains));
            }

            tla_value::churn_stats::churn_count(tla_value::churn_stats::ChurnSite::TupleKeyInBuild);
            let av = Value::Tuple(tuple.into());
            return finish_eval_tir_in(ctx, &av, &bv, span);
        }
    }

    let av = eval_tir(ctx, elem)?;
    // Churn elimination: membership only READS the set, so borrow state-var
    // reads / constants / function-apply results (`key \in keysOf[focus]`)
    // instead of cloning them out. Same evaluation order (elem then set),
    // same semantics via the shared `finish_eval_tir_in` tail.
    let bv = super::dispatch_functions::eval_tir_operand(ctx, set)?;
    finish_eval_tir_in(ctx, &av, bv.as_value(), span)
}
...
pub(super) fn eval_tir_big_union(
    ...
    if let TirExpr::SetEnum(elems) = &inner.node {
        match elems.as_slice() {
            [] => return Ok(Value::empty_set()),
            [only] => {
                let value = eval_tir(ctx, only)?;
                if !value.is_set() {
                    return Err(EvalError::type_error("Set", &value, Some(only.span)));
                }
                return Ok(value);
            }
            _ => {}
...
pub(super) fn eval_tir_record_set(
    ...
    for (field, set_expr) in fields {
        ...
        let interned: Rp<str> = intern_string(&field.name);
...
pub(super) fn eval_tir_unchanged(
    ...
    // Virtual-tuple UNCHANGED (array-backed next state): `UNCHANGED <<x, y,
    // ...>>` inside operator-body actions reaches this generic path (the fast
    // path above requires `TirNameKind::StateVar` elements, but state-var
    // references inside operator bodies lower as `Ident`). The generic path
    // materialized BOTH sides — two K-element `Value::Tuple` allocations plus
    // an owned clone of every listed variable, per evaluation (second-ranked
    // churn site on btree: 5.64M tuple builds + ~40M value clones). Instead,
    // evaluate the components as borrowed operands and compare pairwise.
    //
    // EXACTNESS: the original path evaluates all current components (tuple
    // literal order), then all next components under the swapped context,
    // then compares via `values_equal`, which for two non-set `Value::Tuple`s
    // is exactly structural `==` — slice equality: element-wise `Value::eq`
    // in order with short-circuit. The loop below performs the identical
    // evaluation order, identical error propagation, and identical pairwise
    // comparison (both tuples always have the same arity K). eval_tir_operand
    // returns exactly what eval_tir would, borrowing instead of cloning where
    // provably safe (established Eq/∈ operand infrastructure).
    if let (TirExpr::Tuple(elems), Some(next_state_env)) = (&inner.node, ctx.next_state_env) {
        if !elems.is_empty() {
            use super::dispatch_functions::{eval_tir_operand, TirOperand};
            let mut cur: SmallVec<[TirOperand; 12]> = SmallVec::with_capacity(elems.len());
            for elem in elems {
                cur.push(eval_tir_operand(ctx, elem)?);
            }
            ...
            let mut next: SmallVec<[TirOperand; 12]> = SmallVec::with_capacity(elems.len());
            for elem in elems {
                next.push(eval_tir_operand(&next_ctx, elem)?);
            }
            let eq = cur
                .iter()
                .zip(next.iter())
                .all(|(c, n)| c.as_value() == n.as_value());
            return Ok(Value::Bool(eq));
        }
    }

    // Evaluate in current-state environment
    let is_tuple_inner = matches!(&inner.node, TirExpr::Tuple(elems) if !elems.is_empty());
    if is_tuple_inner {
        tla_value::churn_stats::churn_count(
            tla_value::churn_stats::ChurnSite::TupleKeyUnchangedTirBuild,
        );
    }
...
pub(super) fn eval_tir_let(
    ...
    for def in defs {
        if def.params.is_empty() {
            let interned: Rp<str> = intern_string(&def.name);
                ...
                (&mut val, lambda_body_recurses_on_name(&def.body, &def.name))
            {
                *closure = Rp::new(
                    closure
                        .as_ref()
                        .clone()
                        .with_name_if_missing((&interned).into()),
                );
            }
