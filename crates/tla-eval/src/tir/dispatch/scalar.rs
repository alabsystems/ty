...
use crate::eval_arith::{
    int_arith_op, int_cmp_op, int_div_op, int_exact_div_op, int_mod_op, int_pow_op,
};
...
pub(super) fn eval_tir_arith(
    ...
    match op {
        ...
        TirArithOp::Div => int_exact_div_op(lv, rv, span),
...
pub(super) fn eval_tir_cmp(
    ...
    // Churn elimination: Eq/Neq only READ their operands, so evaluate them via
    // eval_tir_operand which borrows state-var reads, constants, and
    // function-apply results instead of cloning them out (guard comparisons
    // like `state = READY` and `keysOf[node] = {}` are among the hottest
    // expressions in BFS). Same left-then-right evaluation order, same
    // values_equal semantics, same error behavior.
    ...
    match op {
        TirCmpOp::Eq => {
            let lv = super::super::dispatch_functions::eval_tir_operand(ctx, left)?;
            let rv = super::super::dispatch_functions::eval_tir_operand(ctx, right)?;
            let eq = values_equal(ctx, lv.as_value(), rv.as_value(), span)?;
            Ok(Value::Bool(eq))
        }
        TirCmpOp::Neq => {
            let lv = super::super::dispatch_functions::eval_tir_operand(ctx, left)?;
        ...
        TirCmpOp::Lt | TirCmpOp::Leq | TirCmpOp::Gt | TirCmpOp::Geq => {
            ...
            match op {
                ...
                TirCmpOp::Eq | TirCmpOp::Neq => unreachable!(),
            }
        }
    }
}
