...
impl EvalCtx {
    ...
    /// True when the binding chain holds bindings that the dep-tracking
    /// `base_stack_len` rule (which uses `binding_depth`) cannot observe.
    ///
    /// `record_local_read` records an outer-scope dependency only for bindings at
    /// chain index `< base_stack_len`, where `base_stack_len == binding_depth` at
    /// the start of the probed node. That rule is correct only while
    /// `binding_depth` tracks the chain depth (the normal quantifier/LET eval
    /// path, where `\E x` bumps `binding_depth`). Some init/constraint-solver
    /// paths install an `\E` variable on the chain WITHOUT a matching
    /// `binding_depth` increment (O(1) Init enumeration). In that desynced state,
    /// a chain binding at index `>= binding_depth` (e.g. the `\E node` variable at
    /// index 0 with `binding_depth == 0`) is silently treated as an internal
    /// iteration variable, so a value that depends on it would be misclassified
    /// as a constant. Callers (the const-set cache) use this to fail safe.
    ///
    /// SOUNDNESS (fix/pointer-keyed-cache-aliasing-mc): closes the
    /// `base_stack_len` undercount that the span-keyed const-set cache exposed.
    /// The old raw-pointer key happened to evaluate a freshly-cloned body per
    /// solver iteration, so the misclassified entry was never reused; the stable
    /// span key correctly unifies the node, so the undercount must be guarded.
    /// Wired into `const_set_cache::probe_and_record`, which makes NO cache
    /// decision when this returns `true`.
    pub(crate) fn has_unobservable_chain_bindings(&self) -> bool {
        // Cheap: only when the chain is non-empty can there be an undercount.
        // `binding_depth` is the index ceiling the dep rule can record; if the
        // chain has any binding it must have been consed at some depth, so when
        // `binding_depth == 0` the chain head sits at index >= 0 == base and is
        // invisible to `idx < base_stack_len`.
        self.binding_depth == 0 && !self.bindings.is_empty()
    }
    ...
    #[inline]
    pub fn push_binding_preinterned<S>(&mut self, _name: S, value: Value, name_id: NameId) {
        let depth = self.binding_depth;
        self.bindings = self.bindings.cons_local(
            name_id,
            crate::binding_chain::BindingValue::eager(value),
    ...
    pub fn create_closure(
        ...
        crate::value::ClosureValue::new(params, body, Arc::clone(&self.env), local_ops)
            .with_captured_chain(self.bindings.durable_capture_box(), self.binding_depth)
    }

    /// Like [`create_closure`], but shares an already-`Arc`-wrapped AST body
    /// instead of deep-cloning a `Spanned<Expr>`. Used by the TIR `LAMBDA`
    /// evaluator, where the AST body is preserved as an `Arc<Spanned<Expr>>`
    /// (`PreservedAstBody`) — deep-cloning it per closure construction was pure
    /// waste on `LAMBDA`-heavy specs.
    pub fn create_closure_arc(
        &self,
        params: Vec<String>,
        body: Arc<tla_core::Spanned<tla_core::ast::Expr>>,
        local_ops: Option<Arc<tla_core::OpEnv>>,
    ) -> crate::value::ClosureValue {
        crate::value::ClosureValue::new_with_arc_body(
            params,
            body,
            Arc::clone(&self.env),
            local_ops,
