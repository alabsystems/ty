...
use crate::cache::scope_ids::{compute_instance_subs_id, compute_local_ops_scope_id_and_recursive};
...
impl EvalCtx {
    ...
    /// Enter a `LET` scope in place: install the merged operator map and store a
    /// memoized scope id, instead of marking the scope id `INVALIDATED`.
    ///
    /// The successor enumerator rebuilds the merged `local_ops` (outer ops + LET
    /// defs) with identical content on every state. The previous pattern
    /// (`*ctx.local_ops_mut() = Some(arc)`) set the scope id to `INVALIDATED`,
    /// forcing every cache-key build inside the LET body to re-walk the merged
    /// HAMT — millions of redundant walks per run.
    ///
    /// This method instead resolves the *outer* scope id once, then computes the
    /// merged scope id via `let_scope_id_memoized` (keyed on the static LET-block
    /// identity + outer id), so the result is computed once per distinct LET
    /// block / outer scope rather than once per cache lookup. The stored id is
    /// value-identical to `compute_local_ops_scope_id(&merged)`, so cache keys
    /// are unchanged.
    ///
    /// Returns the scope id that was valid for the *enclosing* scope before this
    /// call, so the caller can restore it via `restore_local_ops_with_id` when
    /// leaving the LET block without recomputing it.
    pub fn enter_let_scope(&mut self, merged: Arc<OpEnv>, defs: &[OperatorDef]) -> u64 {
        // Resolve the outer scope id (cheap if already valid; one walk if the
        // enclosing scope was itself INVALIDATED, e.g. a nested LET). The
        // recursive check is pointer-memoized instead of re-walking the HAMT.
        let outer_requires_arc = crate::cache::openv_memo::recursive_flag_memoized(
            &self.stable.local_ops,
            crate::cache::openv_memo::ScopeIdSite::EnterLet,
        );
        let outer_id = crate::cache::scope_ids::resolve_local_ops_id_with_recursive(
            self.stable.scope_ids.local_ops,
            &self.stable.local_ops,
            outer_requires_arc,
        );
        let saved_outer_id = self.stable.scope_ids.local_ops;
        let first_def_span = defs
            .first()
            .map(|d| (d.name.span.file.0, d.name.span.start, d.name.span.end));
        let merged_id =
            crate::cache::scope_ids::let_scope_id_memoized(&merged, outer_id, defs, first_def_span);
        // Cache the recursive flag alongside the scope id so cache-key builds
        // inside the LET body resolve it O(1) instead of re-walking the HAMT.
        // Pointer-memoized: the flag is a pure function of the (pinned) Arc.
        let merged_recursive = crate::cache::openv_memo::scope_id_and_recursive_memoized(
            &merged,
            crate::cache::openv_memo::ScopeIdSite::EnterLet,
        )
        .1;
        let s = self.stable_mut();
        s.scope_ids.local_ops = merged_id;
        s.scope_ids.local_ops_recursive = merged_recursive;
        s.local_ops = Some(merged);
        saved_outer_id
    }

    /// Enter a `LET` scope whose merged env, scope id, and recursive flag were
    /// already produced by `merged_let_env_memoized` (cache/openv_memo.rs).
    ///
    /// This is the memo-hit counterpart of [`enter_let_scope`]: no HAMT clone,
    /// no id derivation, no recursive walk — three field writes. `scope_id`
    /// MUST be value-identical to `compute_local_ops_scope_id(&merged)` and
    /// `recursive` to `local_ops_requires_arc_identity(&merged)` (both are, by
    /// construction, for values returned from `merged_let_env_memoized`).
    ///
    /// Returns the raw stored scope id of the *enclosing* scope (possibly
    /// `INVALIDATED`), for symmetric restore via `restore_local_ops_with_id`.
    pub fn enter_let_scope_premerged(
        &mut self,
        merged: Arc<OpEnv>,
        scope_id: u64,
        recursive: bool,
    ...
    /// `evalImplSubstInKind` (Tool.java:1863-1872). TY saves this handle as
    ...
    pub fn with_outer_resolution_scope(&self) -> Self {
            ...
            .unwrap_or_else(crate::binding_chain::BindingChain::empty);
        {
            ...
            // The id is INVALIDATED (or 0); clear the stale cached recursive flag
            // so the INVALIDATED resolve path recomputes it from live content via
            // `compute_local_ops_scope_id` (which is recursive-aware).
            s.scope_ids.local_ops_recursive = false;
        }
        ctx
    }
    ...
    pub fn with_module_scope_shared(
        ...
        let (local_ops_id, local_ops_recursive) =
            compute_local_ops_scope_id_and_recursive(&local_ops);
        ...
        };
        {
            ...
            s.scope_ids.local_ops_recursive = local_ops_recursive;
        }
