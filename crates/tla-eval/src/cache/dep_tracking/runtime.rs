...
pub(crate) struct EvalRuntimeState {
    ...
    /// Implied-action fingerprint pair `(parent_fp, successor_fp)` for the
    /// transition currently being checked, or `None` outside implied-action
    /// checking.
    ///
    /// Installed via [`crate::implied_fp_scope`] by the checker's
    /// `check_eval_implied_actions_for_transition` while the parent state is
    /// bound as `state_env` and the successor as `next_state_env`. Enables the
    /// fingerprint-keyed zero-arg transition cache (see
    /// `cache::zero_arg_cache` transition partition): zero-arg state-level
    /// operator values (e.g. refinement-mapping operators like `token` /
    /// `pending`) are memoized per state fingerprint instead of being
    /// re-evaluated for every transition. Entries remain dep-validated by
    /// value on every hit, so a fingerprint collision degrades to a cache
    /// miss — it can never produce a wrong value.
    pub(crate) implied_fp_pair: std::cell::Cell<Option<(u64, u64)>>,
}
...
impl EvalRuntimeState {
    pub(crate) fn new() -> Self {
        Self {
            ...
            implied_fp_pair: std::cell::Cell::new(None),
        }
    }
}
...
pub(crate) fn record_local_read(
    ...
    // Tag the dep with its record-time binding depth so a later
    // `strip_internal_locals` (after this dep may have propagated up to ancestor
    // frames) can re-apply the same `index < base_stack_len` filter against any
    // frame's base without an ambiguous NameId re-lookup.
    if resolved_stack_index < top.base_stack_len {
        top.deps
            .record_local_at_depth(name_id, value, resolved_stack_index);
    }
