...
use super::subst_cache::{clear_subst_cache, evict_next_state_subst_entries, SUBST_STATE};
...
struct LifecycleState {
    ...
    /// (EvalControl.java:22-25, Tool.java:1949-1953). TY uses this flag to clear PerState
...
fn clear_run_reset_impl() {
    ...
    // Clear the LET-scope-id memo (pure static fn; memory reset only).
    super::scope_ids::clear_let_scope_id_memo();
    // Clear the OpEnv memos (merged LET envs + pointer-keyed scope ids).
    ...
    SMALL_CACHES.with(|sc| {
        ...
        // action_ctx_cache above).
        sc.lambda_tir_body_cache.clear();
    });
...
pub(crate) enum CacheEvent {
    ...
    /// Test hook for the legacy EvalExit trim path. Production `eval_entry_with`
    /// uses the context-local exit counter directly.
    #[cfg(test)]
    EvalExit,
...
pub(crate) fn on_cache_event(event: CacheEvent) {
    match event {
        ...
        CacheEvent::TestReset => {
            ...
            // Part of the P0 stale-replay family fix: drop run-pinned LET-def
            // Arcs only on the strongest (test/run-teardown) reset — never at
            // intra-run phase boundaries, where pointer-keyed caches that
            // outlive the phase could alias freed operator bodies.
            super::let_def_interning::clear_let_def_interning();
            LIFECYCLE_STATE.with(|ls| {
                ls.eval_exit_count.set(TRIM_INTERVAL - 1);
                ls.last_scope_boundary_gen.set(u64::MAX);
            });
