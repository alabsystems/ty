...
pub(crate) mod const_set_cache;
pub(crate) mod dep_tracking;
pub(crate) mod let_def_interning;
...
pub(crate) use op_result_cache::{
    hash_args, nary_insert, nary_lookup_value, NaryCacheHit, NaryOpCacheEntry, NaryOpCacheKey,
};
...
pub(crate) use small_caches::{
    ...
    hash_bindings_for_choose, hash_value_for_choose, lambda_tir_body_arc, propagate_thunk_deps,
    raw_subst_scope_key, store_thunk_deps, thunk_has_instance_lazy_taint, ChooseDeepKey,
    SMALL_CACHES,
};

// const_set_cache: per-run cache for proven-constant set-producing subexpressions.
pub(crate) use const_set_cache::{
    caching_suppressed as const_set_caching_suppressed, enabled as const_set_enabled,
    is_const_cacheable_set, is_known_non_const as const_set_is_known_non_const,
    key_of as const_set_key_of, lookup as const_set_lookup,
    probe_and_record as const_set_probe_and_record,
};
...
pub(crate) use quantifier_hoist::{
    advance_hoist_state_generation_ctx, bump_hoist_state_generation_ctx,
    enter_quantifier_hoist_scope, is_hoist_active, quantifier_hoist_lookup_ctx,
    quantifier_hoist_store_ctx, tir_hoist_lookup_ctx, tir_hoist_store_ctx, HoistGenerationGuard,
    QuantifierHoistScopeGuard,
};
#[cfg(test)]
pub(crate) use quantifier_hoist::{quantifier_hoist_lookup, quantifier_hoist_store};
