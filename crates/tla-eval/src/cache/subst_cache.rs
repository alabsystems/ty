...
/// Clear the substitution evaluation guard for test isolation.
/// Part of #3805: Uses consolidated SUBST_STATE.
#[cfg(test)]
...
///
/// Fix #2260: if `eval()` panics and the panic is caught by `catch_unwind` in a
/// parallel worker, the success-path trim step never runs, leaving stale INSTANCE
/// substitution results for the next top-level evaluation on that thread — a
/// soundness violation.
///
