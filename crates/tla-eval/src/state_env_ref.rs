...
pub enum StateEnvRef {
    ...
    Compact {
        /// Start of the borrowed `[CompactValue]` slice.
        ptr: *const CompactValue,
        /// Number of state-variable slots in the slice.
        len: usize,
    },
    /// Points to a `[Value]` array (closure captures, legacy paths).
    Legacy {
        /// Start of the borrowed `[Value]` slice.
        ptr: *const Value,
        /// Number of state-variable slots in the slice.
        len: usize,
    },
...
impl StateEnvRef {
    ...
    pub(crate) unsafe fn get_value(self, idx: usize) -> Value {
        use tla_value::churn_stats::{churn_count, churn_stats_enabled, ChurnSite};
        match self {
            StateEnvRef::Compact { ptr, len } => {
                debug_assert!(idx < len);
                let cv = &*ptr.add(idx);
                if churn_stats_enabled() {
                    churn_count(ChurnSite::StateVarRead);
                    if cv.is_heap() {
                        churn_count(ChurnSite::StateVarReadHeap);
                    }
                }
                Value::from(cv)
            }
            StateEnvRef::Legacy { ptr, len } => {
                debug_assert!(idx < len);
                let v = (*ptr.add(idx)).clone();
                if churn_stats_enabled() {
                    churn_count(ChurnSite::StateVarRead);
                    tla_value::churn_stats::churn_count_if_heap(ChurnSite::StateVarReadHeap, &v);
                }
                v
            }
        }
    ...
    /// Borrow the heap-backed `Value` at `idx` without cloning.
    ///
    /// Returns `None` for inline scalar slots (Bool/SmallInt) — those are free
    /// to materialize owned via [`Self::get_value`], so callers fall back.
    /// For `Compact` heap slots this borrows the boxed `Value` in place; for
    /// `Legacy` slots it borrows the slot directly.
    ///
    /// Part of churn elimination: `get_value` on a heap slot costs an Arc
    /// refcount bump plus a later drop (the top-ranked churn site on btree:
    /// 107M heap-backed state-var read clones). Read-only consumers (function
    /// application bases, comparisons) can borrow instead.
    ///
    /// # Safety
    ///
    /// Caller must ensure `idx < self.env_len()`. The returned reference
    /// borrows the underlying state array, which must remain live for `'a`
    /// (the same contract as `get_value` / `SparseStateEnvRef::get_unchecked`:
    /// the BFS state array outlives the evaluation that reads it).
    #[inline(always)]
    pub(crate) unsafe fn get_heap_value_ref<'a>(self, idx: usize) -> Option<&'a Value> {
        match self {
            StateEnvRef::Compact { ptr, len } => {
                debug_assert!(idx < len);
                let cv: &'a CompactValue = &*ptr.add(idx);
                if cv.is_heap() {
                    Some(cv.as_heap_value())
                } else {
                    None
                }
            }
    ...
    /// Like [`Self::slot_matches_compact`], but on a successful non-bitwise
    /// match REFRESHES `expected` to a clone of the bound slot's compact
    /// value. The replacement is validated-equal, so the entry's semantics
    /// are unchanged; subsequent probes against the same binding then hit
    /// the bitwise / pointer-equality fast paths instead of re-running deep
    /// structural comparison (the implied-action transition memo validates
    /// the same entry once per edge of a parent's successor batch).
    ///
    /// # Safety
    ///
    /// Caller must ensure `idx < self.env_len()`.
    #[inline]
    pub(crate) unsafe fn slot_matches_compact_refresh(
        self,
        idx: usize,
        expected: &mut CompactValue,
    ) -> bool {
        match self {
            StateEnvRef::Compact { ptr, len } => {
                debug_assert!(idx < len);
                let cv = &*ptr.add(idx);
                if cv.bits_eq(expected) {
                    return true;
                }
                if cv == expected {
                    *expected = cv.clone();
