...
pub(super) fn eval_builtin_misc(
    ...
    match name {
        ...
        "Add" => {
            ...
            // Scale both fractions to have the same denominator (CHECKED: a corpus
            // spec (Huang) legitimately drives these past u64/i64 range — that must be
            // an eval error, never a panic).
            let scale_err = || EvalError::Internal {
                message: "dyadic rational arithmetic overflow".to_string(),
                span,
            };
            let p_scaled = p_num.checked_mul(lcm / p_den).ok_or_else(scale_err)?;
            let q_scaled = q_num.checked_mul(lcm / q_den).ok_or_else(scale_err)?;

            // Add numerators (checked — same overflow class as the scaling above)
            let sum_num = p_scaled.checked_add(q_scaled).ok_or_else(scale_err)?;
        ...
        "Half" => {
            ...
            // Double the denominator and reduce (checked — the probe found a real
            // corpus spec overflowing here; an eval error is the honest outcome).
            let den2 = den.checked_mul(2).ok_or_else(|| EvalError::Internal {
                message: "dyadic rational arithmetic overflow".to_string(),
                span,
            })?;
            let (reduced_num, reduced_den) = reduce_fraction(num, den2);
