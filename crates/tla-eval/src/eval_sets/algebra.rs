...
pub(crate) fn set_minus_values(
    ...
    if let (Value::Set(sa), Value::Set(sb)) = (&av, &bv) {
        if let Some(elem) = sb.as_singleton() {
            // Singleton subtraction: binary search + copy two halves. O(log n + n).
            // Avoids n membership checks and avoids normalizing transient RHS
            // singleton sets produced outside the AST-level `S \ {x}` fast path.
            return Ok(Value::from_sorted_set(sa.without(elem)));
        }
...
mod tests {
    ...
    #[test]
    fn set_minus_values_handles_concrete_singleton_rhs() {
        let ctx = EvalCtx::new();
        let lhs = Value::set([Value::int(1), Value::int(2), Value::int(3)]);
        let rhs = Value::from_sorted_set(SortedSet::from_vec(vec![Value::int(2)]));

        let result = set_minus_values(&ctx, lhs, rhs, None, None, None)
            .expect("singleton concrete set-minus should evaluate");

        assert_eq!(result, Value::set([Value::int(1), Value::int(3)]));
    }
}
