...
/// Evaluate an exact two-element tuple membership expression without
/// allocating the temporary tuple when the RHS has direct value semantics.
///
/// Lazy-membership RHS expressions retain the existing AST-aware path below;
/// eagerly reducing those expressions can change short-circuit/error behavior
/// or force prohibitively large set materializations.
#[inline]
fn try_eval_tuple2_in(
    ctx: &EvalCtx,
    a: &Spanned<Expr>,
    b: &Spanned<Expr>,
    span: Option<Span>,
) -> Option<EvalResult<Value>> {
    let Expr::Tuple(elements) = &a.node else {
        return None;
    };
    let [first_expr, second_expr] = elements.as_slice() else {
        return None;
    };
    ...
    // Preserve tuple construction and membership's source evaluation order:
    // first component, second component, then RHS.
    let first = match eval(ctx, first_expr) {
        Ok(value) => value,
        Err(error) => return Some(Err(error)),
    };
    let second = match eval(ctx, second_expr) {
        Ok(value) => value,
        Err(error) => return Some(Err(error)),
    };
    ...
    if let Some(contains) = bv.try_set_contains_tuple_elements(&tuple) {
        tla_value::churn_stats::churn_count(tla_value::churn_stats::ChurnSite::TupleKeyInFused);
        return Some(Ok(Value::Bool(contains)));
    }

    // Lazy/runtime compound set values and errors need the ordinary candidate
    // representation and context-aware membership machinery.
    tla_value::churn_stats::churn_count(tla_value::churn_stats::ChurnSite::TupleKeyInBuild);
    let av = Value::Tuple(tuple.into());
    Some(finish_eval_in(ctx, &av, &bv, b.span, span))
}

/// Finish eager RHS membership once both values have been evaluated.
#[inline]
fn finish_eval_in(
    ctx: &EvalCtx,
    av: &Value,
    ...
    if let Value::ModelValue(name) = bv {
        return match name.as_ref() {
            ...
            _ => Err(EvalError::type_error("Set", bv, Some(rhs_span))),
        };
    }
...
pub(crate) fn eval_in(
    ...
    if let Some(result) = try_eval_tuple2_in(ctx, a, b, span) {
        return result;
    }
    ...
    let bv = eval(ctx, b)?;
    finish_eval_in(ctx, &av, &bv, b.span, span)
}
