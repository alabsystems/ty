...
pub(super) fn eval_let(
    ...
    // TY eagerly evaluates ALL LET defs before the body, wasting work
    ...
    // Merged env = ambient local_ops + the static subset of LET defs that must
    // remain operators. The subset predicate is a pure function of the static
    // AST (params / primed refs / InstanceExpr / self-recursive FuncDef), so
    // the merged env is memoized on (ambient Arc identity, inserted def
    ...
    let (ops, id, recursive) = crate::cache::merged_let_env_memoized(
        ...
        |def| {
            // Defs with parameters (arity > 0) always stay operators.
            if !def.params.is_empty() {
                return true;
            }
            // A zero-arg LET referenced under prime must remain an operator so
            // `a'` evaluates the primed body of `a`, not an eager value.
            if primed_refs.contains(def.name.node.as_str()) {
                return true;
            }
            // Zero-arg InstanceExpr must stay in local_ops (eval_module_ref).
            if matches!(&def.body.node, Expr::InstanceExpr(_, _)) {
                return true;
            }
            // Zero-arg recursive FuncDef stays in local_ops for recursive
            // lookup (a LazyFuncValue is also bound in local_stack below).
        ...
        if def.params.is_empty() {
            ...
            if let Expr::FuncDef(bounds, func_body) = &def.body.node {
                ...
                if expr_mentions_name_v(&func_body.node, def_name) {
                    ...
                    new_ctx = new_ctx.into_bind_local(name, Value::LazyFunc(Rp::new(lazy)));
                    continue;
                }
