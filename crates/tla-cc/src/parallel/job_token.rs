...
mod inprocess_jobserver {
    ...
    impl JobServer {
        ...
        pub(super) async fn acquire(&self) -> JobToken {
            loop {
                // `fetch_update` was renamed `try_update` in std (deprecated 1.99,
                // replacement stable since 1.95); this crate's MSRV is 1.63, so keep
                // the old name until the MSRV passes 1.95.
                #[allow(deprecated)]
                let res = self
                    .0
