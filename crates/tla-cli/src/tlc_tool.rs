...
pub(crate) fn emit_check_tool_cli_error(
    ...
    let _ = writeln!(&mut body, "ty check failed.");
...
pub(crate) fn format_tlc_version_message() -> String {
    ...
    //   "TLC2 %1%"
    format!("TLC2 ty {}", env!("CARGO_PKG_VERSION"))
}
...
pub(crate) fn format_tlc_mode_message(workers: usize) -> String {
    ...
    format!(
        "Running breadth-first search Model-Checking with fp 0 and seed 0 with {workers} worker on {cores} cores with 0MB heap and 0MB offheap memory [pid: {pid}] (ty)."
    )
}
