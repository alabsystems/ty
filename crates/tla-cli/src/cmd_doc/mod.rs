...
//! `ty doc` -- auto-generate documentation from TLA+ specifications.
//!
...
fn render_markdown(doc: &DocModel) -> String {
    ...
    if !doc.instances.is_empty() {
        ...
        for inst in &doc.instances {
            ...
            let _ = writeln!(out, "- `INSTANCE {}`{local_tag}", inst.module);
        }
        out.push('\n');
    }
    ...
    if !entry_ops.is_empty() {
        ...
        for op in &entry_ops {
            ...
            let _ = write!(out, "- **{}** `{}`", kind, format_signature(op));
            out.push('\n');
        }
        out.push('\n');
    }
...
fn render_html(doc: &DocModel) -> String {
    ...
    let _ = writeln!(out, "<h2>{}</h2>", html_escape(&doc.module_name));
    out.push_str("<ul>\n");
    if !doc.extends.is_empty() {
    ...
    if !doc.extends.is_empty() {
        ...
        for ext in &doc.extends {
            let _ = writeln!(out, "<li><code>{}</code></li>", html_escape(ext));
        }
        out.push_str("</ul>\n</section>\n");
    }
    ...
    if !doc.operators.is_empty() {
        ...
        for op in &doc.operators {
            ...
            let _ = write!(out, "<h3><code>{}</code>", html_escape(&sig));
            for tag in &tags {
                let _ = write!(out, " <span class=\"tag\">{tag}</span>");
            }
            out.push_str("</h3>\n");

            if !op.comment.is_empty() {
                let _ = writeln!(out, "<p>{}</p>", html_escape(&op.comment));
            }
    ...
    if !doc.theorems.is_empty() {
        ...
        for thm in &doc.theorems {
            ...
            if !thm.comment.is_empty() {
                let _ = writeln!(out, "<p>{}</p>", html_escape(&thm.comment));
            }
            out.push_str("</div>\n");
        }
        out.push_str("</section>\n");
