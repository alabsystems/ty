...
//! `ty summary` -- compact summary table of model checking results.
//!
//! Generates a formatted table summarizing model checking outcomes across
//! multiple TLA+ specifications. For each spec, prefers a `.ty-check.json`
//! sidecar file (produced by `ty check --output json`), and falls back to
//! running `ty check --output json` with the current executable when no
//! sidecar exists. The resulting rows include status, distinct/total states,
//! wall-clock time, invariants checked, and deadlock status.
//!
...
/// Subset of the `ty check --output json` format needed for summary.
#[derive(Deserialize)]
struct CheckSidecar {
...
///
/// Scans `paths` for `.tla` files, reads `.ty-check.json` sidecars when
/// present, runs `ty check --output json` for missing sidecars, and outputs
...
pub(crate) fn cmd_summary(
    ...
    if config.is_some() && tla_files.len() != 1 {
        bail!("--config can only be used with a single .tla file");
    }

    let exe = std::env::current_exe().context("resolve current executable")?;

    // Build summary rows from sidecar files, falling back to live checks.
    let mut rows: Vec<SummaryRow> = tla_files
        .iter()
        .map(|path| build_row_or_run_check(path, config, workers, &exe))
        .collect();

    // Apply status filter.
    let parsed_filter = match filter_status {
        Some("pass") => Some(SummaryStatus::Pass),
        Some("fail") => Some(SummaryStatus::Fail),
        Some("error") => Some(SummaryStatus::Error),
        Some(other) => bail!("invalid --status `{other}`; expected pass, fail, or error"),
        None => None,
    };
...
// Currently exercised only by the test suite; kept as a focused helper that
// mirrors the sidecar branch of `build_row_or_run_check`.
#[allow(dead_code)]
...
/// Build a summary row from a sidecar if present; otherwise run `ty check`.
fn build_row_or_run_check(
    tla_path: &Path,
    ...
    if let Some(sidecar) = read_sidecar(&sidecar_path) {
        return row_from_sidecar(&spec_name, tla_path, &sidecar);
    }

    match run_check_for_summary(exe, tla_path, config_override, workers) {
        Ok(sidecar) => row_from_sidecar(&spec_name, tla_path, &sidecar),
        Err(err) => SummaryRow {
            name: spec_name,
            status: SummaryStatus::Error,
            distinct_states: 0,
            ...
            error_message: Some(format!(
                "no sidecar file: {}; live check failed: {err}",
...
/// Spawn the current CLI as `ty check --output json` and parse stdout.
fn run_check_for_summary(
    exe: &Path,
    tla_path: &Path,
    config_override: Option<&Path>,
    workers: usize,
    ...
    let config_path = match config_override {
        Some(path) => Some(path),
        None => {
            default_cfg = default_config_path_for(tla_path);
            default_cfg.exists().then_some(default_cfg.as_path())
        }
    };
    ...
    let output = cmd
        .output()
        .with_context(|| format!("run {} check {}", exe.display(), tla_path.display()))?;
    let stdout = String::from_utf8_lossy(&output.stdout);
    serde_json::from_str::<CheckSidecar>(&stdout).with_context(|| {
        let stderr = String::from_utf8_lossy(&output.stderr);
        format!(
            "failed to parse check JSON (exit={}): stdout={} stderr={}",
            output.status, stdout, stderr
        )
    })
}

/// Default config path for a `.tla` file: same path with `.cfg` extension.
fn default_config_path_for(tla_path: &Path) -> PathBuf {
...
/// Convention: `<stem>.ty-check.json` in the same directory.
fn sidecar_path_for(tla_path: &Path) -> PathBuf {
    ...
    parent.join(format!("{stem}.ty-check.json"))
}
...
mod tests {
    ...
    fn write_sidecar(dir: &Path, stem: &str, json: &str) -> PathBuf {
        ...
        let sidecar = dir.join(format!("{stem}.ty-check.json"));
  ...
  "diagnostics": {{ "warnings": [], "infos": [] }}
    ...
    fn test_sidecar_path_for() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/specs/MCBakery.ty-check.json"),
        );
    }
    ...
    fn test_sidecar_path_for_relative() {
        ...
        assert_eq!(sidecar_path_for(&tla), PathBuf::from("Spec.ty-check.json"),);
    }
    ...
    fn test_read_sidecar_missing() {
        let path = PathBuf::from("/nonexistent/path.ty-check.json");
    ...
    fn test_read_sidecar_malformed() {
        let dir = tempfile::tempdir().expect("create tempdir");
        let bad_path = dir.path().join("Bad.ty-check.json");
    ...
    fn test_collect_tla_files_single_file() {
        ...
        let files = collect_tla_files(std::slice::from_ref(&tla)).expect("collect");
    ...
    #[test]
    fn test_cmd_summary_rejects_config_for_multiple_specs() {
        let dir = tempfile::tempdir().expect("create tempdir");
        write_sidecar(
            dir.path(),
            "Spec1",
            &minimal_sidecar_json("ok", None, 10, 20, 0.1),
        );
        write_sidecar(
            dir.path(),
            "Spec2",
        ...
        let cfg = dir.path().join("Spec.cfg");
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("--config can only be used with a single .tla file"));
    }

    #[test]
    fn test_cmd_summary_rejects_unknown_status_filter() {
        let dir = tempfile::tempdir().expect("create tempdir");
        write_sidecar(
            dir.path(),
            "MySpec",
            &minimal_sidecar_json("ok", None, 42, 84, 0.1),
        );
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("expected pass, fail, or error"));
    }
    ...
    #[cfg(unix)]
    #[test]
    fn test_build_row_or_run_check_falls_back_to_live_check() {
        use std::os::unix::fs::PermissionsExt;

        let dir = tempfile::tempdir().expect("create tempdir");
        let tla_path = dir.path().join("LiveSpec.tla");
        fs::write(&tla_path, "---- MODULE LiveSpec ----\n====\n").expect("write tla");

        let fake_ty = dir.path().join("fake-ty");
        let check_json = minimal_sidecar_json("ok", None, 77, 88, 0.25);
        fs::write(
            &fake_ty,
            format!("#!/bin/sh\ncat <<'JSON'\n{check_json}\nJSON\n"),
        )
        .expect("write fake executable");
        let mut permissions = fs::metadata(&fake_ty).expect("metadata").permissions();
        permissions.set_mode(0o755);
        fs::set_permissions(&fake_ty, permissions).expect("chmod fake executable");

        let row = build_row_or_run_check(&tla_path, None, 1, &fake_ty);
        assert_eq!(row.status, SummaryStatus::Pass);
        assert_eq!(row.distinct_states, 77);
        assert_eq!(row.total_states, 88);
        assert_eq!(row.invariants_checked, 2);
    }
