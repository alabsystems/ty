...
pub(super) fn update_check_cache(ctx: CacheUpdateCtx<'_>) {
    if let CheckResult::Success(stats) = ctx.result {
        // Vacuity gate (TRUST_VACUITY_GATE §1.A): do NOT cache a run that
        // recorded default-on V2/V3 WARNINGs. The cached-PASS fast path does not
        // replay the vacuity policy, so caching it would silently drop the
        // warning (and any `--strict-vacuity` promotion) on subsequent runs.
        if !stats.vacuity_warnings.is_empty() {
            return;
        }
