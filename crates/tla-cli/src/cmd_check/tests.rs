...
use super::{
    helpers::{apply_allow_incomplete_overflow, merge_strategy_info},
    normalize_portfolio_strategies_for_analytical,
};
use crate::cli_schema::{Cli, Command, OutputFormat, StrategyArg};
use clap::Parser;
use tla_check::InfraCheckError;
use tla_check::{CheckResult, CheckStats};

fn run_cli_parse_on_large_stack<F>(f: F)
where
    F: FnOnce() + Send + 'static,
{
    std::thread::Builder::new()
        ...
        .join()
        .expect("parser test thread panicked");
}

#[test]
fn flat_layout_unsupported_detail_detects_the_typed_error() {
    // The retry trigger: a CheckResult::Error carrying
    // RuntimeCheckError::FlatLayoutUnsupportedValue yields Some(detail).
    let result = CheckResult::from_error(
        tla_check::CheckError::flat_layout_unsupported_value(
            "scalar value 18446744073709551616 exceeds the i64 range of a single flat slot",
        ),
        CheckStats::default(),
    );
    let detail = super::flat_layout_unsupported_detail(&result)
        .expect("typed flat-layout error must be detected");
    assert!(detail.contains("18446744073709551616"), "detail: {detail}");

    // Other errors and non-error results must NOT trigger the retry.
    let other_error = CheckResult::from_error(
    ...
    assert!(super::flat_layout_unsupported_detail(&other_error).is_none());
    assert!(
        super::flat_layout_unsupported_detail(&CheckResult::Success(CheckStats::default()))
            .is_none()
    );
}
...
#[test]
fn analytical_portfolio_strategy_keeps_lane_and_adds_bfs_verifier() {
    let normalized = normalize_portfolio_strategies_for_analytical(
        vec!["analytical".to_string()],
        OutputFormat::Json,
    );

    assert_eq!(normalized, vec!["bfs", "analytical"]);
}

#[test]
fn analytical_portfolio_strategy_does_not_duplicate_bfs_verifier() {
    let normalized = normalize_portfolio_strategies_for_analytical(
        vec![
            "random".to_string(),
    ...
    assert_eq!(normalized, vec!["random", "analytical", "bfs"]);
    assert_eq!(normalized.iter().filter(|s| s.as_str() == "bfs").count(), 1);
}

#[test]
fn check_strategy_parser_accepts_public_value_enum_variants() {
    run_cli_parse_on_large_stack(|| {
        for strategy_name in ["quick", "full", "auto"] {
            let cli = Cli::try_parse_from(["ty", "check", "Spec.tla", "--strategy", strategy_name])
                .unwrap_or_else(|err| panic!("--strategy {strategy_name} should parse: {err}"));

            let strategy = match cli.command {
                Command::Check { strategy, .. } => strategy,
            ...
            assert!(
                matches!(
                    ...
                    ("quick", Some(StrategyArg::Quick))
                        | ("full", Some(StrategyArg::Full))
                        | ("auto", Some(StrategyArg::Auto))
                ),
                "unexpected parsed strategy for {strategy_name}: {strategy:?}"
            );
        }
    });
}
...
#[test]
fn check_strategy_parser_rejects_analytical_until_cli_exposure_is_safe() {
    run_cli_parse_on_large_stack(|| {
        let err = Cli::try_parse_from(["ty", "check", "Spec.tla", "--strategy", "analytical"])
            .expect_err(
                "--strategy analytical should stay hidden until parser tests prove it safe",
            );
...
fn cli_init_next_overrides_config_values() {
    let mut config = tla_check::Config {
        init: Some("OldInit".to_string()),
        next: Some("OldNext".to_string()),
        invariants: vec!["OldInv".to_string()],
        ..Default::default()
    };
...
fn cli_prop_overrides_config_values() {
    let mut config = tla_check::Config {
        properties: vec!["OldLiveness".to_string()],
        ..Default::default()
    };
