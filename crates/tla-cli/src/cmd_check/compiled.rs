...
//!
//! When `--compiled` is passed to `ty check`, this module:
...
// Kept as a backward-compatible thin wrapper over `run_compiled_check_with_config`;
...
pub(crate) fn run_compiled_check_with_config(cfg: &CompiledCheckConfig<'_>) -> Result<()> {
    ...
    if std::env::var("TY_COMPILED_DEBUG").is_ok() {
        ...
        if let Ok(entries) = std::fs::read_dir(&src_dir) {
            for entry in entries.flatten() {
                ...
                let is_rs = Path::new(&name).extension().is_some_and(|ext| ext == "rs");
                if name != "main.rs" && is_rs {
...
fn main() {{
    ...
    let max_states: usize = std::env::var("TY_MAX_STATES")
        .ok()
    ...
    println!("TY_COMPILED_CHECK_START");
...
fn run_binary(project_dir: &Path, max_states: usize, _workers: usize) -> Result<String> {
    ...
    let output = Command::new(&binary)
        .env("TY_MAX_STATES", effective_max.to_string())
        .output()
    ...
    if !output.status.success() && !stdout.contains("TY_COMPILED_CHECK_START") {
        bail!(
...
fn parse_output(output: &str) -> Result<CompiledCheckResult> {
    let start_marker = "TY_COMPILED_CHECK_START";
    let end_marker = "TY_COMPILED_CHECK_END";

    let start_idx = output
        .find(start_marker)
        .context("compiled binary output missing TY_COMPILED_CHECK_START marker")?;
    let end_idx = output
        .find(end_marker)
        .context("compiled binary output missing TY_COMPILED_CHECK_END marker")?;
...
mod tests {
    ...
    fn test_generate_compiled_main_contains_protocol() {
        ...
        assert!(
            main_rs.contains("TY_COMPILED_CHECK_START"),
            "generated main.rs should contain start marker"
        );
        assert!(
            main_rs.contains("TY_COMPILED_CHECK_END"),
    ...
    fn test_generate_tir_code_counter_spec() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                eprintln!("TIR codegen failed for Counter spec (may be expected): {e}");
            }
        }
    }
    ...
    fn test_max_states_zero_to_usize_max() {
        ...
        // by checking the code in generate_compiled_main: it reads TY_MAX_STATES
    ...
    fn test_generate_tir_code_multi_variable_spec() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                eprintln!("TIR codegen failed for DieHard spec (may be expected): {e}");
            }
        }
    }
    ...
    fn test_generate_tir_code_malformed_spec_returns_error() {
        ...
        std::fs::write(&spec_path, "this is not a valid TLA+ module at all\n").expect("write spec");
    ...
    fn test_parse_output_empty_block_defaults() {
        ...
        let output = "TY_COMPILED_CHECK_START\n\n\n  \nTY_COMPILED_CHECK_END\n";
        ...
        assert!(!result.deadlock, "empty block should not report deadlock");
    }
    ...
    fn test_parse_output_violation_without_invariant_name() {
        ...
        assert!(
            !result.deadlock,
            "should not report deadlock for violation status"
        );
    }
