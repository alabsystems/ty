...
pub(crate) struct CheckConfig {
    ...
    /// Force the CUDA GPU engine (testing lever; skips the auto probe).
    /// The GPU engine itself is part of the default AUTO engine family.
    pub(crate) gpu: bool,
    /// Disable the GPU engine entirely (testing lever).
    ...
    /// Vacuity classes to downgrade from VACUOUS verdict to WARNING
    /// (`--allow-vacuous=<class>[,...]`). Classes: empty-init, dead-action,
    /// vacuous-invariant. Vacuity gate: TRUST_VACUITY_GATE §1.A.
    pub(crate) allow_vacuous: Vec<String>,
    /// Promote default-on vacuity WARNINGs (V2/V3) to the hard VACUOUS verdict
    /// (exit 3). `--strict-vacuity`.
    pub(crate) strict_vacuity: bool,
    ...
    #[allow(dead_code)] // wired via env var; field stored for documentation
