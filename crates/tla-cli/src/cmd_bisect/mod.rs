...
//!
//! Usage:
//!   ty bisect Spec.tla --config Spec.cfg --constant N --low 1 --high 100
//!
//! At each step, creates a temporary .cfg with the constant set to the mid value,
//! runs `ty check --output json --continue-on-error --workers 1`, and narrows
...
// ── Subset of JSON output we parse from `ty check --output json` ───────────
...
fn run_probe(
    ...
    let tmp_dir = std::env::temp_dir().join(format!("ty-bisect-{}-{}", std::process::id(), value));
    ...
    let output = if timeout_secs > 0 {
        ...
        cmd.output().context("spawn ty check subprocess")?
    };
...
fn run_with_timeout(
    ...
    let mut child = cmd.spawn().context("spawn ty check subprocess")?;
        ...
        match child.try_wait() {
            ...
            Ok(None) => {
                ...
                if start.elapsed() >= deadline {
                    ...
                    drop(rx);
                    wait_thread.join().ok();
                    bail!("ty check timed out after {timeout_secs}s");
                }
                std::thread::sleep(poll_interval);
            }
            Err(e) => return Err(e).context("wait on ty check subprocess"),
        }
    }
}
