...
fn percentile(sorted: &[usize], pct: usize) -> usize {
    ...
    // Use floor (truncation) for nearest-rank percentile so that
    // p50 over 1..=100 returns 50 and p95 returns 95.
...
fn compute_violation_report(
    ...
    let mut per_invariant: Vec<InvariantViolationCount> = violations
        ...
        .collect();
    per_invariant.sort_by_key(|a| std::cmp::Reverse(a.count));
