...
//! `ty trace-gen` — targeted trace generation for TLA+ specifications.
//!
//! Unlike `ty check` (which finds violations) or `ty simulate` (which
...
use tla_check::{
    ...
    ModelChecker, RandomWalkConfig, RandomWalkResult, ResolvedSpec, Trace,
};
...
fn bfs_to_target(
    ...
    let _negated_invariant_name = format!("__TraceGenNeg_{}", target_name);
...
fn generate_coverage_traces(
    ...
    // Greedy set cover over labeled random walks: collect walks (every walk
    // yields a labeled Trace via random_walk_collect), keep each trace that
    // covers at least one not-yet-covered action, stop when every detected
    // action has been observed or attempts are exhausted.
    let max_attempts = total_actions * 200;
    let seed: u64 = 0xC0FE_0000_0000;
    let batch = 8;
    let mut attempt: usize = 0;

    while covered.len() < action_names.len() && attempt < max_attempts {
        let walk_config = RandomWalkConfig {
            num_walks: batch.min(max_attempts - attempt),
            max_depth,
        ...
        let collected = checker
            .random_walk_collect(&walk_config)
            .map_err(|e| anyhow::anyhow!("coverage walk failed: {}", e))?;
        attempt += walk_config.num_walks;

        for trace in collected {
            total_states += trace.len();
            // `covered.insert` both dedupes within the trace and updates the
            // global cover in one pass.
            let newly_covered: Vec<String> = extract_action_names_from_trace(&trace, &action_names)
                .into_iter()
                .filter(|a| covered.insert(a.clone()))
                .collect();
...
fn generate_random_traces(
    ...
    // random_walk_collect materializes every walk's path as a labeled Trace —
    // random_walk itself is a witness search and yields traces only for
    // violations/deadlocks, which a *correct* spec never produces.
    let mut checker = ModelChecker::new_with_extends(module, checker_modules, config);
    checker.set_deadlock_check(false);
    if let Some(resolved) = resolved_spec {
        checker.register_inline_next(resolved)?;
    ...
    let collected = checker
        .random_walk_collect(&walk_config)
        .map_err(|e| anyhow::anyhow!("random walk trace collection failed: {}", e))?;

    let mut total_states: usize = 0;
    let traces: Vec<GeneratedTrace> = collected
        .into_iter()
        .enumerate()
        .map(|(i, trace)| {
            total_states += trace.len();
...
fn print_header(
    ...
    println!();
    println!("ty trace-gen  {}", file.display());
    println!(
        "  config:    {}",
        config_path
            .file_name()
