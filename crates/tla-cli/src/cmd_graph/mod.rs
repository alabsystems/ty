...
//! `ty graph` -- state transition graph visualization from saved JSON output.
//!
//! Reads a JSON output file produced by `ty check --output json` and builds
...
fn render_dot(graph: &Graph, cluster_by_action: bool) -> String {
    ...
    for edge in &graph.edges {
        ...
        let _ = writeln!(out, "    {from_id} -> {to_id} [label=\"{label}\"];");
    }
...
fn render_mermaid(graph: &Graph, cluster_by_action: bool) -> String {
    ...
    for edge in &graph.edges {
        ...
        let _ = writeln!(out, "    {from_id} -->|{label}| {to_id}");
    }
