...
/// Part of #3283: verify that TY_* env vars change the cache signature.
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn signature_changes_with_ty_env_var() {
    let dir = TempDir::new();
    ...
    let tir_env = vec![("TY_TIR_EVAL".to_string(), "all".to_string())];
    ...
    assert_ne!(
        ...
        "cache signature must differ when TY_TIR_EVAL is set"
    );

    // Different env var values must also produce different signatures.
    let parity_env = vec![("TY_TIR_PARITY".to_string(), "Next".to_string())];
...
#[test]
fn collect_behavior_env_vars_includes_ty_prefix() {
    // We can't safely test env var collection in parallel tests (process-global state),
    // but we can verify the function filters on TY_ prefix and sorts.
    let vars = collect_behavior_env_vars();
    for (key, _) in &vars {
        assert!(
            key.starts_with("TY_"),
            "collect_behavior_env_vars should only return TY_* vars, got: {key}"
        );
    }
    // Verify sorted order
