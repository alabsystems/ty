...
const DIAGNOSE_DEFAULT_TIMEOUT_SECONDS: u64 = 1800;
const DIAGNOSE_DEFAULT_PARALLEL: usize = 1;
const SMALL_CATEGORY_DEFAULT_TIMEOUT_SECONDS: u64 = 120;
const SMALL_CATEGORY_DEFAULT_PARALLEL: usize = 4;
...
pub(crate) fn cmd_diagnose(mut args: DiagnoseArgs) -> Result<()> {
    ...
    apply_bounded_category_defaults(&mut args);
    ...
    if matches!(args.output, DiagnoseOutputFormat::Human) {
        ...
        eprintln!(
            "TY diagnose: {} specs to test (of {} in baseline)",
    ...
    // #4252 Stream 6: differential oracle harness (interpreter vs trust_cg).
    // When `--oracle-mode compare|fail-closed` (or `TY_ORACLE=` env) is set,
    ...
    if matches!(
        ...
        if matches!(args.output, DiagnoseOutputFormat::Human) {
            eprintln!(
                "Oracle mode: {mode_label} — running interpreter + trust-cg per spec (#4252 Stream 6)"
            );
        }
...
/// Keep full small-category diagnose runs finite when callers use the command
/// as a closure gate and leave the legacy global defaults in place.
fn apply_bounded_category_defaults(args: &mut DiagnoseArgs) {
    if args.category.as_deref() != Some("small") || !args.spec.is_empty() {
        return;
    }

    if args.timeout == DIAGNOSE_DEFAULT_TIMEOUT_SECONDS {
        args.timeout = SMALL_CATEGORY_DEFAULT_TIMEOUT_SECONDS;
    }
    if args.parallel == DIAGNOSE_DEFAULT_PARALLEL {
        args.parallel = SMALL_CATEGORY_DEFAULT_PARALLEL;
    }
}
...
fn run_all_specs(
    ...
    if args.parallel <= 1 {
        ...
        for name in spec_names {
            ...
            let (timeout, timeout_secs, timeout_src) = effective_timeout(cli_timeout_default, spec);
        ...
        std::thread::scope(|s| {
            for name in spec_names {
                ...
                let run_conditions = run_conditions.clone();
                s.spawn(move || {
                    ...
                    let mut locked = results.lock().expect("results lock poisoned");
                    locked.push(r);
                    write_progress_metrics(
                        &locked,
                        total_baseline,
...
fn write_progress_metrics(
    ...
    let Some(path) = output_metrics_path else {
        return;
    };

    let tally = Tally::from_results(results, total_baseline);
    if let Err(err) = output::write_metrics_file(
        results,
        &tally,
        total_baseline,
        results.len().min(total_to_run),
        exe,
        run_conditions.clone(),
        path,
    ) {
        eprintln!("warning: failed to write diagnose progress metrics: {err:#}");
    }
}
...
mod tests {
    ...
    fn make_diagnose_args(category: Option<&str>) -> DiagnoseArgs {
        DiagnoseArgs {
            baseline: PathBuf::from("tests/tlc_comparison/spec_baseline.json"),
            output: DiagnoseOutputFormat::Human,
            timeout: DIAGNOSE_DEFAULT_TIMEOUT_SECONDS,
            category: category.map(str::to_string),
            spec: Vec::new(),
            spec_list: None,
            retries: 0,
            parallel: DIAGNOSE_DEFAULT_PARALLEL,
    ...
    #[test]
    fn test_small_category_default_contract_is_bounded() {
        let mut args = make_diagnose_args(Some("small"));
        apply_bounded_category_defaults(&mut args);
        assert_eq!(args.timeout, SMALL_CATEGORY_DEFAULT_TIMEOUT_SECONDS);
        assert_eq!(args.parallel, SMALL_CATEGORY_DEFAULT_PARALLEL);
    }

    #[test]
    fn test_small_category_defaults_preserve_explicit_values() {
        let mut args = make_diagnose_args(Some("small"));
        args.timeout = 300;
        args.parallel = 2;
        apply_bounded_category_defaults(&mut args);
    ...
    #[test]
    fn test_small_category_defaults_do_not_apply_to_specific_spec_runs() {
        let mut args = make_diagnose_args(Some("small"));
        args.spec.push("CarTalkPuzzle_M1".to_string());
        apply_bounded_category_defaults(&mut args);
        assert_eq!(args.timeout, DIAGNOSE_DEFAULT_TIMEOUT_SECONDS);
        assert_eq!(args.parallel, DIAGNOSE_DEFAULT_PARALLEL);
    }
