...
pub(super) fn emit_human(
    ...
    if tally.false_pos > 0 {
        println!(
            "FALSE POSITIVE:          {:>4}  (TY error, TLC pass)",
            tally.false_pos
        );
    }
    if tally.false_neg > 0 {
        println!(
            "FALSE NEGATIVE:          {:>4}  (TLC error, TY pass)",
            tally.false_neg
        );
    }
    if tally.crash > 0 {
    ...
    print_detail_section(
        "FALSE POSITIVES (TY reports error, TLC passes)",
        SpecVerdict::FalsePositive,
        results,
    );
    print_detail_section(
        "FALSE NEGATIVES (TLC reports error, TY passes)",
...
/// Write metrics JSON without printing to stdout.
pub(super) fn write_metrics_file(
    results: &[SpecResult],
    tally: &Tally,
    total_baseline: usize,
    total_to_run: usize,
...
fn print_expected_mismatch_detail(result: &SpecResult) {
    println!(
        "  {}: TY={} TLC={}",
        result.name,
        result.ty_status.as_deref().unwrap_or("?"),
        result.tlc_status,
    );
