...
pub(super) fn parse_and_classify(
    ...
    let (ty_status, ty_states, ty_error) = match &parsed {
        Some(p) => {
            ...
            // Part of #3746: When TY reports runtime_error but the BFS state count
    ...
    let raw_verdict = classify_raw(ty_status.as_deref(), ty_states, &ty_error, spec);
...
/// Classify TY result against TLC baseline.
///
...
pub(super) fn classify_raw(
    ...
    if ty_ok && tlc_no_error {
        ...
        // When ty_expected_states is set, compare against that instead of TLC
        ...
        // TY running in simulate mode succeeds — this is correct behaviour.
        SpecVerdict::Pass
        ...
        let same_error = error_types_match(ty_error, &spec.tlc.error_type);
        if same_error {
...
/// Check if the TLC baseline has a real expected-violation error_type.
///
/// `tlc.status = "pass"` plus an expected violation type such as `invariant`
/// means TLC found that expected violation. Parser/semantic failures cannot also
/// be successful model-checking runs, so stale `parse` metadata on pass rows is
/// treated as non-violation metadata.
///
...
fn tlc_has_real_error_type(spec: &BaselineSpec) -> bool {
    match &spec.tlc.error_type {
        Some(et) => !matches!(et.as_str(), "" | "unknown" | "timeout" | "parse"),
        None => false,
    }
}
...
/// Check if TY's state count matches the expected count for this spec.
...
/// Compare TY and TLC error types with normalization.
/// TY uses "invariant_violation" while TLC baseline uses "invariant", etc.
pub(super) fn error_types_match(ty_error: &Option<String>, tlc_error: &Option<String>) -> bool {
    match (ty_error.as_deref(), tlc_error.as_deref()) {
...
pub(super) fn timeout_result(
    ...
    // Classify as BothFail (not Timeout) so it doesn't count as a TY-specific failure.
