...
/// Spawn `ty check` or `ty simulate` depending on the spec's mode.
/// Part of #3076: simulation specs require `ty simulate` to pass.
...
/// `trust_cg` (prototype/gated native path; may compile, fall back, or report
/// `backend_unavailable`).
/// Only applies to `check` mode; simulation mode always uses the interpreter.
pub(super) fn spawn_ty(
    exe: &Path,
    tla_path: &Path,
    cfg_path: &Path,
    mode: &str,
    ...
    match mode {
        "simulate" | "generate" => {
            ...
            // TY can run the simulation without crashing, not whether it finds the
        ...
        _ => {
            cmd.arg("check")
                ...
                // count_ty_instances(), which over-restricts each subprocess
                // when unrelated ty processes are running. 131072 MB = 128 GB
