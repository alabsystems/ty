...
use subprocess::{spawn_ty, wait_with_timeout, WaitResult};

/// Run `ty check` on a single spec and classify the result against TLC baseline.
///
...
/// Run `ty check` with an explicit backend selection — variant of
/// [`run_one_spec`] used by the differential oracle harness (#4252 Stream 6).
///
/// `backend` is forwarded to `ty check --backend {backend}`. Currently
/// supports `"interpreter"` and `"trust-cg"`. Other values are passed verbatim
...
fn run_once(
    ...
    match output {
        WaitResult::Completed(o) => {
            forward_child_stderr_for_nary_stats(&o);
            parse_and_classify(name, spec, &o, elapsed)
        }
...
fn forward_child_stderr_for_nary_stats(output: &std::process::Output) {
    if std::env::var_os("TY_NARY_CACHE_STATS").is_none() || output.stderr.is_empty() {
        return;
    }
