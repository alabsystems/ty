...
use crate::cmd_diagnose::types::{
    Baseline, BaselineSource, BaselineSpec, BaselineTlc, BaselineTy, SpecVerdict,
};
#[cfg(unix)]
...
fn test_classify_false_positive() {
    // TY error but TLC found no error
    let spec = make_spec("pass", Some(100), None);
...
#[test]
fn test_classify_tlc_pass_with_error_type_ty_ok() {
    // TLC status="pass" + error_type="invariant" → TLC expected an error.
    // TY says ok → FalseNegative.
    let spec = make_spec("pass", Some(50), Some("invariant"));
    let v = classify_raw(Some("ok"), Some(50), &None, &spec);
    assert_eq!(v, SpecVerdict::FalseNegative);
}
...
#[test]
fn test_classify_tlc_pass_with_parse_metadata_ty_ok() {
    // TLC status="pass" + error_type="parse" is stale warning metadata from
    // successful TLC runs with semantic warnings, not an expected violation.
    let spec = make_spec("pass", Some(45), Some("parse"));
    let v = classify_raw(Some("ok"), Some(45), &None, &spec);
    assert_eq!(v, SpecVerdict::Pass);
}

#[test]
fn test_classify_ty_status_none_tlc_no_error() {
    // ty_status=None (crash/unparseable) + TLC no error → FalsePositive
    let spec = make_spec("pass", Some(100), None);
...
#[test]
fn test_classify_ty_status_none_tlc_has_error() {
    // ty_status=None (crash) + TLC has error → BothFail (error types can't match)
    let spec = make_spec("error", Some(50), Some("invariant"));
    let v = classify_raw(None, None, &None, &spec);
    assert_eq!(v, SpecVerdict::BothFail);
}
...
#[test]
fn test_classify_ty_ok_tlc_timeout() {
    // TY succeeds but TLC timed out → we can't compare, treat as Pass
...
#[test]
fn test_classify_ty_ok_tlc_timeout_with_error_type() {
...
#[test]
fn test_classify_ty_error_tlc_timeout() {
    // TY errors but TLC timed out → BothFail (can't confirm either way)
    let spec = make_spec("timeout", None, None);
    let err = Some("invariant_violation".to_string());
    let v = classify_raw(Some("error"), Some(50), &err, &spec);
    assert_eq!(v, SpecVerdict::BothFail);
...
fn test_classify_fallback_crash() {
    // Unrecognized TLC status with ty_ok → falls through to Crash
...
fn test_smokeewd998_sc_baseline_entry_classifies_as_pass() {
    ...
    assert_eq!(
        spec.ty.status, "pass",
        "SmokeEWD998_SC TY baseline should record the verified pass result"
    );
    ...
    assert_eq!(
        raw,
        SpecVerdict::Pass,
        "raw classification should reflect the exact TLC/TY parity"
    );
    assert_eq!(apply_expected_mismatch(raw, spec), SpecVerdict::Pass);
    assert!(
...
fn test_invariant_specs_get_effective_first_error_mode() {
    ...
    // Verify explicit overrides still work (SlidingPuzzles, trust-wp).
...
#[test]
fn test_classify_ty_expected_states_overrides_tlc_states() {
    // SpanTreeRandom: TLC has 87898 states, TY has 27242 (PRNG difference).
    // With ty_expected_states = 27242, classify should compare against that.
    let mut spec = make_spec("pass", Some(87898), None);
    spec.ty_expected_states = Some(27242);
    let v = classify_raw(Some("ok"), Some(27242), &None, &spec);
    assert_eq!(v, SpecVerdict::Pass);
}

#[test]
fn test_classify_ty_expected_states_mismatch() {
    // If TY produces wrong count even against ty_expected_states, still mismatch.
    let mut spec = make_spec("pass", Some(87898), None);
    spec.ty_expected_states = Some(27242);
...
fn test_classify_simulation_ok_tlc_error_is_pass() {
    ...
    // TY succeeds in simulate mode → should be Pass, not FalseNegative.
...
fn test_classify_non_simulation_ok_tlc_error_is_false_negative() {
    // Non-simulation spec: TY ok + TLC error → FalseNegative (unchanged).
    let spec = make_spec("error", Some(50), Some("invariant"));
    let v = classify_raw(Some("ok"), Some(50), &None, &spec);
    assert_eq!(v, SpecVerdict::FalseNegative);
}
...
fn test_parse_and_classify_downgrades_liveness_checker_setup_failure() {
    ...
    assert_eq!(result.ty_status.as_deref(), Some("ok"));
    assert_eq!(result.ty_states, Some(42));
...
fn test_parse_and_classify_downgrades_fingerprint_only_replay_failure() {
    ...
    assert_eq!(result.verdict, SpecVerdict::Pass);
    assert_eq!(result.ty_status.as_deref(), Some("ok"));
    assert_eq!(result.ty_states, Some(9));
    assert_eq!(result.error_details, None);
}
...
fn test_parse_and_classify_keeps_non_liveness_runtime_error() {
    ...
    // Use different state counts (TLC=100, TY=9) so the state-count-aware
    ...
    assert_eq!(result.ty_status.as_deref(), Some("runtime_error"));
    assert_eq!(result.ty_states, Some(9));
...
fn test_parse_and_classify_downgrades_runtime_error_when_state_counts_match() {
    ...
    assert_eq!(result.verdict, SpecVerdict::Pass);
    assert_eq!(result.ty_status.as_deref(), Some("ok"));
    assert_eq!(result.ty_states, Some(214));
    assert_eq!(result.error_details, None);
}
...
fn test_classify_simulation_ok_tlc_pass_is_pass() {
    ...
    // error), TY runs with --no-invariants so it also succeeds. Simulation
...
fn test_apply_expected_mismatch_maps_oom_killed() {
    ...
    spec.expected_mismatch = true;
    spec.expected_mismatch_reason =
        Some("Intractable spec; both TLC and TY timeout/OOM".to_string());
    assert_eq!(
...
fn test_timeout_result_maps_to_expected_mismatch() {
    ...
    assert_eq!(result.verdict, SpecVerdict::ExpectedMismatch);
    assert_eq!(result.ty_status.as_deref(), Some("timeout"));
    assert_eq!(
        result.expected_mismatch_reason.as_deref(),
        Some("Exponential state space; TLC could not complete")
    );
