...
pub(super) fn refresh_baseline_value(
    ...
    let git_commit = option_env!("TY_GIT_COMMIT")
        .unwrap_or("unknown")
        .to_string();
    ...
    let (stats, specs_digest, specs_updated) = {
        ...
        for result in results {
            ...
            specs_updated += 1;
            if let Some(ty_obj) = entry.get_mut("ty").and_then(|value| value.as_object_mut()) {
                ty_obj.insert(
                    "status".into(),
                ...
                ty_obj.insert("last_run".into(), serde_json::Value::String(now.clone()));
                ty_obj.insert(
                    "git_commit".into(),
                    serde_json::Value::String(git_commit.clone()),
                );
                match runtime_seconds_value(result) {
                    Some(runtime_seconds) => {
                        ty_obj.insert("runtime_seconds".into(), runtime_seconds);
                    }
                    None => {
                        ty_obj.remove("runtime_seconds");
                    }
                }
            }
    ...
    root_obj.insert(
        "ty_refresh".into(),
        serde_json::Value::Object(build_ty_refresh(
            &git_commit,
            &now,
            results.len(),
            specs_updated,
...
fn runtime_seconds_value(result: &SpecResult) -> Option<serde_json::Value> {
    if !matches!(
        result.verdict,
        SpecVerdict::Pass | SpecVerdict::FlakyTimeout
    ) {
    ...
    if !result.time_seconds.is_finite() || result.time_seconds <= 0.0 {
        return None;
    }
    Some(serde_json::json!(result.time_seconds))
}
...
fn compute_baseline_stats(
    ...
    for spec in specs_obj.values() {
        ...
        let Some(ty) = spec.get("ty").and_then(serde_json::Value::as_object) else {
            ty_untested += 1;
            continue;
        };
    ...
    stats.insert("ty_fail".into(), serde_json::json!(ty_fail));
    stats.insert("ty_match".into(), serde_json::json!(ty_match));
    stats.insert("ty_mismatch".into(), serde_json::json!(ty_mismatch));
    stats.insert("ty_untested".into(), serde_json::json!(ty_untested));
