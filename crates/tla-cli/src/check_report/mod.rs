...
/// Emit a counterexample trace to stderr in the requested format — the same
/// renderer the explicit-state path uses, so ALIAS-rendered fields and the TLA+
/// `/\ var = value` form appear identically. Exposed for fused mode, whose
/// violation/deadlock output previously omitted the counterexample trace entirely
/// (default `ty check` reported "Invariant violated" with no trace).
pub(crate) fn emit_counterexample_trace(
    trace: &tla_check::Trace,
    format: crate::cli_schema::TraceFormat,
    difftrace: bool,
    label: &str,
) {
    trace_format::emit_trace(trace, format, difftrace, label);
}
...
fn bytecode_vm_stats_line() -> Option<String> {
    match std::env::var("TY_BYTECODE_VM_STATS").as_deref() {
...
pub(crate) fn print_storage_stats(stats: &StorageStats, to_stderr: bool) {
