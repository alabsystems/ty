...
pub(crate) fn report_check_itf(result: &CheckResult) -> Result<()> {
    match result {
        ...
        // Vacuity gate (TRUST_VACUITY_GATE §1.A): VACUOUS verdict → exit 3.
        // No counterexample trace exists; emit a minimal ITF document and report
        // the reason on stderr.
        CheckResult::Vacuous { reason, .. } => {
            eprintln!("Error: VACUOUS — {}", reason.message());
            emit_itf_success();
            std::process::exit(3);
        }
        _ => bail!(
...
mod tests {
    ...
    fn test_itf_liveness_trace_phases() {
        let s0 = State::from_pairs(vec![("pc", Value::String(Rp::from("init")))]);
        let s1 = State::from_pairs(vec![("pc", Value::String(Rp::from("loop")))]);
