...
pub(crate) fn report_check_json(ctx: &JsonReportCtx<'_>) -> Result<()> {
    ...
    match ctx.result {
        ...
        // Vacuity gate (TRUST_VACUITY_GATE §1.A): VACUOUS is a distinct verdict
        // (exit 3), not a property failure (exit 1). JSON already carries the
        // full vacuity diagnostic; exit directly so main.rs does not re-emit.
        CheckResult::Vacuous { .. } => std::process::exit(3),
