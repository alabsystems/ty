...
fn emit_property_check_stats(stats: &tla_check::CheckStats, to_stderr: bool) {
    let property_stats = &stats.property_check;
    if !property_stats.has_data() {
        return;
    }
    macro_rules! out {
        ($($arg:tt)*) => {
    ...
    out!(
        "  PROPERTY implied-action transitions: {}",
        property_stats.implied_action_transition_checks
    );
    out!(
        "  PROPERTY implied-action term evals: {}",
        property_stats.implied_action_term_evals
    );
    out!(
        "  PROPERTY implied-action time: {:.3}s",
        property_stats.implied_action_time_seconds()
    );
}
...
pub(crate) fn report_check_human(
    ...
    match result {
        CheckResult::Success(stats) => {
            ...
            emit_property_check_stats(&stats, false);
        ...
        CheckResult::Error { error, stats, .. } => {
            ...
            emit_property_check_stats(&stats, true);
        ...
        CheckResult::LimitReached { limit_type, stats } => {
            let (limit_name, hint, known_limit) = match limit_type {
                ...
                _ => (
                    "unknown",
                    "Exploration limit reached; upgrade ty for limit details",
                    false,
                ),
            };
        ...
        CheckResult::Vacuous { reason, stats } => {
            // Vacuity gate (TRUST_VACUITY_GATE §1.A): a distinct VACUOUS verdict
            // (exit code 3), NOT a property failure (exit 1). Exit directly so
            // the verdict maps to 3 rather than anyhow's bail (exit 1).
        ...
        _ => bail!(
            "Model checking produced a result variant unsupported by this ty version; upgrade ty"
        ),
    }
}
