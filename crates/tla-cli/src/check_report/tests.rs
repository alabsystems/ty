...
fn run_report_check_human_case(case: &str) -> std::process::Output {
    ...
    Command::new(exe)
        ...
        .env("TY_CHECK_REPORT_CAPTURE_CASE", case)
        .env("TY_BYTECODE_VM_STATS", "1")
        .output()
...
fn test_bytecode_vm_stats_line_respects_env_gate() {
    ...
    {
        let _guard = EnvVarGuard::remove("TY_BYTECODE_VM_STATS");
        assert_eq!(super::bytecode_vm_stats_line(), None);
    }

    {
        let _guard = EnvVarGuard::set("TY_BYTECODE_VM_STATS", "1");
        assert_eq!(
...
fn test_report_check_human_capture_helper() {
    let Ok(case) = std::env::var("TY_CHECK_REPORT_CAPTURE_CASE") else {
        return;
    };
