...
//! `ty inline` -- inline INSTANCE modules into a self-contained TLA+ spec.
//!
...
mod tests {
    ...
    fn test_apply_substitutions_binary() {
        ...
        let result = apply_substitutions(&expr, &subs);
        // pretty_expr renders Range without surrounding spaces (matches Pow `^`);
        // see tla_core::pretty::expr / fmt_expr.
