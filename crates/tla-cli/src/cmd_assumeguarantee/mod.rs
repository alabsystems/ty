...
//! `ty assume-guarantee` -- compositional verification for TLA+ specifications.
//!
...
/// Entry point for `ty assume-guarantee`.
...
enum Verdict {
    ...
    // Part of the public verdict vocabulary (rendered by `as_str`, exercised by
    // tests); not yet produced by the current decomposition path.
    #[allow(dead_code)]
    Skipped,
}
...
// `op_table` is threaded through the recursion for symmetry with the caller and
// to support resolving operator references in future revisions; currently only
// forwarded, not queried.
...
fn run_single_group_check(
    ...
    let extended_refs: Vec<&tla_core::ast::Module> = extended_modules.to_vec();
...
fn emit_human(report: &AgReport) {
    ...
    if report.overall_verdict == Verdict::Fail {
        ...
        println!("Run `ty check` on the full spec to confirm.");
    }
}
