...
//! `ty profile` -- performance profiling for model checking runs.
//!
//! Runs `ty check` as a subprocess with profiling flags enabled, parses the
...
// ── Subset of JSON output from `ty check --output json` ───────────────────
...
fn run_profiling(
    ...
    let mut child = cmd.spawn().context("failed to spawn ty check subprocess")?;
    ...
    let stderr_thread = std::thread::spawn(move || {
        ...
        // Preserve existing behavior: skip individual read errors and continue
        // draining the child's stderr rather than stopping at the first Err
        // (which `map_while(Result::ok)` would do).
    ...
    let output = child.wait_with_output().context("wait for ty check")?;
    ...
    let parsed: CheckJsonOutput = serde_json::from_str(&stdout).with_context(|| {
        let stderr_text = stderr_lines
            ...
            .join("\n");
        format!(
            "failed to parse JSON from ty check (exit={}):\nstdout (first 500): {}\nstderr (last 500): {}",
            output.status,
...
fn parse_eval_hotspots(lines: &[String], top: usize) -> Vec<EvalHotspot> {
    ...
    hotspots.sort_by_key(|a| Reverse(a.count));
