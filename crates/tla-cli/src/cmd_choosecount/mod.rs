...
//! `ty choose-count` — count CHOOSE expressions.
...
pub(crate) fn cmd_choosecount(file: &Path, format: ChoosecountOutputFormat) -> Result<()> {
    ...
    for unit in &module.units {
        if let Unit::Operator(def) = &unit.node {
            let count = count_choose(&def.body.node);
            total += count;
            if count > 0 {
