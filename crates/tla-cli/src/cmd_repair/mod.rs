...
//! `ty repair` -- counterexample trace analysis with fix suggestions.
//!
//! Reads a JSON output file produced by `ty check --output json`, parses the
...
pub(crate) fn cmd_repair(config: RepairConfig) -> Result<()> {
    ...
    let counterexample = match output.counterexample.as_ref() {
        ...
        _ => {
            ...
            println!("Run: ty check --output json <spec.tla> to generate one.");
            return Ok(());
        }
    };
