...
//! `ty deadlock` — deadlock analysis for TLA+ specifications.
//!
...
#[derive(Default)]
struct ConfigInfo {
    /// INIT operator name. Kept alongside `next` for a symmetric config record;
    /// only `next` is currently consulted by the static analyzer.
    #[allow(dead_code)]
    init: Option<String>,
    next: Option<String>,
}
...
fn run_full_analysis(
    ...
    // In full mode, advise the user to use `ty check` with deadlock detection
