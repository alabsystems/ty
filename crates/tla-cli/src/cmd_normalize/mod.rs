...
//! `ty normalize` — normalize a specification to canonical form.
//!
...
pub(crate) fn cmd_normalize(file: &Path, format: NormalizeOutputFormat) -> Result<()> {
    ...
    let _ = writeln!(normalized, "---- MODULE {} ----", module.name.node);

    if !constants.is_empty() {
        let _ = writeln!(normalized, "CONSTANTS {}", constants.join(", "));
    }
    if !variables.is_empty() {
        let _ = writeln!(normalized, "VARIABLES {}", variables.join(", "));
    }
    normalized.push('\n');

    for (name, params, body) in &operators {
        if params.is_empty() {
            let _ = writeln!(normalized, "{name} == {body}\n");
        } else {
            let _ = writeln!(normalized, "{}({}) == {}\n", name, params.join(", "), body);
        }
    }
