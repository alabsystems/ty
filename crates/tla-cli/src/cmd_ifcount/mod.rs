...
//! `ty if-count` — count IF-THEN-ELSE expressions.
//!
//! Counts conditional expressions per operator, useful for
//! identifying branching complexity.
...
pub(crate) fn cmd_ifcount(file: &Path, format: IfcountOutputFormat) -> Result<()> {
    ...
    for unit in &module.units {
        if let Unit::Operator(def) = &unit.node {
            let count = count_ifs(&def.body.node);
            total += count;
            if count > 0 {
    ...
    ops.sort_by_key(|a| Reverse(a.1));

    let elapsed = start.elapsed().as_secs_f64();
    match format {
        IfcountOutputFormat::Human => {
            println!("if-count: {}", file.display());
            println!("  total IF-THEN-ELSE: {total}");
            println!("  operators with IFs: {}", ops.len());
            println!();
