...
//! `ty dep-graph` — operator dependency graph in DOT format.
//!
...
pub(crate) fn cmd_depgraph(file: &Path, format: DepgraphOutputFormat) -> Result<()> {
    ...
    let op_set: std::collections::BTreeSet<&str> = op_names.iter().map(|s| s.as_str()).collect();
