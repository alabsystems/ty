...
//! `ty thread-check` -- verify a ConcurrentModel JSON from tRust.
//!
...
pub(crate) fn cmd_threadcheck(
    ...
    if emit_tla {
        if let Some(tla) = &result.generated_tla {
            // Decorative delimiters on stderr; the TLA+ source itself on stdout so
            // `ty threadcheck --emit-tla > out.tla` yields a valid module.
