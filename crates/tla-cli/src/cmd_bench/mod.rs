...
pub(crate) fn cmd_bench(config: BenchConfig) -> Result<()> {
    ...
    if let Some(ref cfg_override) = config.config {
        if spec_files.len() == 1 {
            ...
            bail!(
                "--config can only be used with a single spec file, not with multiple files or directories"
            );
        }
    }
...
/// Spawn `ty check --output json` and parse the JSON output.
fn run_check_subprocess(
    ...
    let output = cmd
        ...
        .output()
        .context("failed to spawn ty check subprocess")?;
    ...
    serde_json::from_str::<CheckJsonOutput>(&stdout).with_context(|| {
        ...
        format!(
            "failed to parse JSON output from ty check (exit={}):\nstdout: {}\nstderr: {}",
...
fn compute_baseline_comparison(
    ...
    // First try bench baseline (previously saved TY runs).
...
fn print_human(results: &[BenchResult]) {
    ...
    if !with_mem.is_empty() {
        let max_mem = with_mem.iter().copied().fold(0.0f64, f64::max);
