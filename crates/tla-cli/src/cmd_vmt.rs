...
//! `ty vmt` subcommand: export a TLA+ spec as VMT format for external model checkers.
//!
...
pub(crate) fn cmd_vmt(file: &Path, config_path: Option<&Path>) -> Result<()> {
    ...
    #[cfg(feature = "ay")]
    {
        ...
        print!("{}", vmt.content);
        eprint!("{}", render_vmt_stderr_report(&vmt));
        Ok(())
    }

    #[cfg(not(feature = "ay"))]
    {
        bail!("VMT export requires the ay feature. Rebuild with --features ay");
    }
}

#[cfg(feature = "ay")]
fn render_vmt_stderr_report(vmt: &tla_check::VmtOutput) -> String {
    let mut report = String::new();
    for row in &vmt.evidence_rows {
        writeln!(report, "[vmt-export-evidence] {row}").expect("write to String");
    }
    writeln!(
        report,
...
mod tests {
    ...
    #[test]
    fn vmt_stderr_report_keeps_evidence_out_of_artifact_stdout() {
        let vmt = tla_check::VmtOutput {
            content: "(set-logic QF_LIA)\n(define-fun .prop () Bool true)\n".to_string(),
            num_vars: 1,
            num_invariants: 1,
            evidence_rows: vec![
                "TY shared_engine_adoption origin_frontend=vmt_interchange".to_string(),
                "TY vmt_export_identity frontend_kind=vmt_interchange output_format=vmt"
                    .to_string(),
            ],
        };

        let stderr = render_vmt_stderr_report(&vmt);

        assert!(vmt.content.starts_with("(set-logic QF_LIA)"));
        assert!(
            !vmt.content.contains("[vmt-export-evidence]"),
            "VMT artifact stdout must not include report evidence: {}",
            vmt.content
        );
        assert!(stderr.contains(
            "[vmt-export-evidence] TY shared_engine_adoption origin_frontend=vmt_interchange"
        ));
        assert!(stderr.contains(
            "[vmt-export-evidence] TY vmt_export_identity frontend_kind=vmt_interchange output_format=vmt"
        ));
        assert!(stderr.ends_with("VMT export complete: 1 variable(s), 1 invariant(s)\n"));
    }
}
