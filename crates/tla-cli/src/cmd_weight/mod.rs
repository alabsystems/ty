...
//! `ty weight` — action weight computation for guided search.
//!
...
pub(crate) fn cmd_weight(
    ...
    if let Some(next_op) = operators.get(next_name) {
        collect_action_weights(&next_op.body.node, &operators, &mut action_weights, 0);
    }
...
fn collect_action_weights(
    ...
    match expr {
        Expr::Or(a, b) => {
            collect_action_weights(&a.node, operators, weights, depth);
            collect_action_weights(&b.node, operators, weights, depth);
        }
        Expr::Ident(name, _) => {
            if let Some(op) = operators.get(name.as_str()) {
                if matches!(&op.body.node, Expr::Or(_, _)) {
                    collect_action_weights(&op.body.node, operators, weights, depth + 1);
                } else {
