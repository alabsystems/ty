...
//!
//! - **Record mode** (`--update`): Run `ty check --output json` on each spec,
...
/// Spawn `ty check --output json --continue-on-error` and parse the JSON output.
...
fn compare_snapshot(
    ...
    if current.status == "error"
        && saved.status == "error"
        && current.error_type != saved.error_type
    {
        regressions.push(format!(
