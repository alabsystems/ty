...
//! `ty refine` -- refinement checking for TLA+ specifications.
//!
...
/// KERNEL-CERTIFIED refinement certificates (`ty refine-certify` / `ty refine-check`) —
/// the re-checkable, Clean-CIC-gated upgrade of this module's Rust simulation checking.
...
use tla_core::{lower_main_module, FileId, SyntaxNode};
...
use self::types::{RefineResult, RefineStatistics, RefinementViolation, SpecInfo, ViolationKind};
...
fn run_refinement_check(
    ...
    let states_explored = explored;
    let transitions_explored = trans_count;
