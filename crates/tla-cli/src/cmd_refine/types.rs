...
pub(super) struct SpecInfo {
    ...
    // Populated for diagnostic context; not yet read by the checker.
    #[allow(dead_code)]
...
pub(super) enum ViolationKind {
    ...
    // Not yet constructed by the checker, but part of the complete
    // `ViolationKind` taxonomy and handled in `report.rs` / serialized output.
    #[allow(dead_code)]
    InvariantViolation,
