...
//! `ty symmetry` — symmetry set analysis for TLA+ specifications.
//!
...
fn analyze_set(
    ...
    let element_names: HashSet<String> = candidate.elements.iter().cloned().collect();
    let mut violations = Vec::new();
    let reachable_ops = compute_reachable_ops(cfg, all_ops);

    let empty_bound: HashSet<String> = HashSet::new();
...
///
/// `element_names` is the literal set of model values that the candidate set
/// names (`p1`, `p2`, `p3` for `Procs = {p1, p2, p3}`). `bound_elem_vars`
/// accumulates in-scope bound variables that range over the candidate set
/// (e.g. `p` in `\E p \in Procs : ...`). Operations that may compare/operate
/// on individual elements (ordering, arithmetic, sequence indexing) treat
/// both as references; the DistinguishedElement check still uses only the
/// literal element_names because the distinguished-element pattern requires
/// a *literal* element on one side of the equality.
fn analyze_expr(
    expr: &Expr,
    set_name: &str,
    element_names: &HashSet<String>,
    bound_elem_vars: &HashSet<String>,
    current_op: &str,
    all_ops: &HashMap<String, OperatorDef>,
    ...
    match expr {
        ...
        Expr::Lt(l, r) | Expr::Leq(l, r) | Expr::Gt(l, r) | Expr::Geq(l, r) => {
            if refs_set(&l.node, set_name, element_names, bound_elem_vars)
                || refs_set(&r.node, set_name, element_names, bound_elem_vars)
            {
        ...
        // Uses only the literal element_names (not bound_elem_vars) so that
        // `\E p \in Procs : p = q` (where both sides are bound variables)
        // does not trigger a false-positive DistinguishedElement violation.
        ...
        Expr::Choose(bv, body) => {
            ...
            let extended = extend_bound_with(bound_elem_vars, bv, set_name, element_names);
            analyze_expr(
                &body.node,
                set_name,
                element_names,
                extended.as_ref().unwrap_or(bound_elem_vars),
                current_op,
                all_ops,
                violations,
                visited,
        ...
        // Quantifiers: extend bound_elem_vars with any bound variable whose
        // domain is the candidate set, then recurse into the body.
        Expr::Forall(bounds, body) | Expr::Exists(bounds, body) => {
            let extended = extend_bound_with_many(bound_elem_vars, bounds, set_name, element_names);
            let bound = extended.as_ref().unwrap_or(bound_elem_vars);
            for bv in bounds {
                if let Some(d) = &bv.domain {
                    analyze_expr(
                        &d.node,
        ...
        // Other binders (set builder, function def, set filter).
        Expr::SetBuilder(body, bvs) | Expr::FuncDef(bvs, body) => {
            let extended = extend_bound_with_many(bound_elem_vars, bvs, set_name, element_names);
            let bound = extended.as_ref().unwrap_or(bound_elem_vars);
            for bv in bvs {
                if let Some(d) = &bv.domain {
                    analyze_expr(
        ...
        Expr::FuncApply(func, arg) => {
            if refs_set(&arg.node, set_name, element_names, bound_elem_vars)
                && is_likely_sequence(&func.node)
            {
                violations.push(SymmetryViolation {
        ...
        // Follow operator references transitively. Operator bodies are
        // analyzed at their own scope, so we drop in-scope bound variables.
...
/// Helper to recurse into all children of an expression. The caller is
/// responsible for handling binder cases (Choose/Forall/Exists/etc.)
/// directly so that scope extension is correct; this helper is only used
/// for expressions whose children share the same bound-variable scope.
fn recurse_children(
    expr: &Expr,
    set_name: &str,
    element_names: &HashSet<String>,
...
fn refs_set(
    ...
    match expr {
        Expr::Ident(n, _) | Expr::StateVar(n, _, _) => {
            element_names.contains(n.as_str())
                || bound_elem_vars.contains(n.as_str())
                || n == set_name
        }
        _ => {
            let mut found = false;
            visit_expr_children(expr, |child| {
                if !found {
                    found = refs_set(child, set_name, element_names, bound_elem_vars);
                }
            });
            found
        }
...
fn specific_element<'a>(expr: &'a Expr, element_names: &HashSet<String>) -> Option<&'a str> {
    match expr {
...
fn bv_ranges_over_set(bv: &BoundVar, set_name: &str, element_names: &HashSet<String>) -> bool {
...
/// If `bv` ranges over the candidate set, return an extended copy of
/// `bound_elem_vars` that includes `bv.name`. Otherwise return `None`.
fn extend_bound_with(
    bound_elem_vars: &HashSet<String>,
    bv: &BoundVar,
    set_name: &str,
    element_names: &HashSet<String>,
    ...
    if bv_ranges_over_set(bv, set_name, element_names) {
        let mut extended = bound_elem_vars.clone();
        extended.insert(bv.name.node.clone());
        Some(extended)
    } else {
        None
    }
...
/// Same as `extend_bound_with` but for a slice of bound variables (used for
/// `Forall`/`Exists` quantifier prefixes and other multi-binder forms).
fn extend_bound_with_many(
    bound_elem_vars: &HashSet<String>,
    bounds: &[BoundVar],
    set_name: &str,
    element_names: &HashSet<String>,
) -> Option<HashSet<String>> {
    let mut extended: Option<HashSet<String>> = None;
    for bv in bounds {
        if bv_ranges_over_set(bv, set_name, element_names) {
            let next = extended.get_or_insert_with(|| bound_elem_vars.clone());
            next.insert(bv.name.node.clone());
        }
    }
    extended
}
