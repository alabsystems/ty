...
//! `ty codegen` command: generate Rust code from TLA+ specifications.
...
// CLI entry point whose bool flags mirror distinct command-line switches
// (--checker/--kani/--proptest/--emit-source-map); bundling them into a struct
// would not improve the call site.
...
pub(crate) fn cmd_codegen(
    ...
    if let (true, Some(output_path)) = (emit_source_map, output) {
...
pub(crate) fn cmd_codegen_tir(
    ...
    if emit_source_map && output.is_none() {
        bail!("--source-map requires --output when used with --tir");
    }
    ...
    match output {
        Some(path) => {
            ...
            if emit_source_map {
                let source_map = tla_codegen::source_map_from_generated_rust(
                    &rust_code,
                    &path.display().to_string(),
                    &file.display().to_string(),
                );
                let sm_path = PathBuf::from(format!("{}.source_map.json", path.display()));
                let sm_json =
    ...
    if let Ok(entries) = std::fs::read_dir(spec_dir) {
        for entry in entries.flatten() {
            ...
            if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                // TLA+ specs use a lowercase `.tla` extension by convention; the
                // exact, case-sensitive match here is intentional (we do not want
                // to pick up `.TLA`/`.Tla` files).
                #[allow(clippy::case_sensitive_file_extension_comparisons)]
                let is_mc_spec = name.starts_with("MC") && name.ends_with(".tla");
                if is_mc_spec && !candidates.contains(&path) {
...
/// - Optionally `src/kani_harness.rs` with trust_mc/Kani verification harnesses
///
/// Part of #3929.
pub(crate) fn cmd_codegen_scaffold(
    ...
    let main_rs = format!(
        "//! BFS model checker for {machine_type} (generated by ty codegen --scaffold).\n\
         \n\
         mod {mod_name};\n\
         \n\
    ...
    // Optionally write Kani/trust_mc harness
    if kani {
        ...
        eprintln!(
            "Kani/trust_mc harness written to {}/src/kani_harness.rs",
            output_dir.display()
        );
...
/// Generate a Kani/trust_mc verification harness for the scaffold project.
fn generate_scaffold_kani_harness(mod_name: &str, machine_type: &str, state_type: &str) -> String {
    format!(
        "//! Kani/trust_mc verification harness for {machine_type}.\n\
         //!\n\
         //! Generated by `ty codegen --scaffold --kani`.\n\
         //!\n\
         //! Run with:\n\
