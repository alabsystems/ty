...
//! `ty translate` — translate spec to pseudocode.
//!
...
pub(crate) fn cmd_translate(file: &Path, format: TranslateOutputFormat) -> Result<()> {
    ...
    let _ = writeln!(pseudocode, "// Module: {}", module.name.node);
    pseudocode.push('\n');

    if !const_names.is_empty() {
        let _ = writeln!(pseudocode, "constants: {}", const_names.join(", "));
    }
    if !var_names.is_empty() {
        let _ = writeln!(pseudocode, "variables: {}", var_names.join(", "));
    }
    pseudocode.push('\n');

    for (name, params, body) in &operators {
        if params.is_empty() {
            let _ = writeln!(pseudocode, "procedure {name}():");
        } else {
            let _ = writeln!(pseudocode, "procedure {}({}):", name, params.join(", "));
        }
        for line in body.lines() {
            let _ = writeln!(pseudocode, "    {line}");
        }
        pseudocode.push('\n');
    }
