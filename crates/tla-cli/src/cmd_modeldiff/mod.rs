...
//! `ty model-diff` -- structural comparison of two TLA+ specifications.
//!
...
// Variants share the `Changed` postfix on purpose: with `rename_all =
// "snake_case"` these names are serialized verbatim into the JSON output
// (e.g. `expr_type_changed`), so renaming them would change the public output.
