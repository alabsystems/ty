...
//! `ty merge` — merge two TLA+ specifications at the AST level.
//!
...
fn emit_merged_source_with_conflicts(
    ...
    if !merge_result.conflicts.is_empty() {
        ...
        if let Some(pos) = output.rfind(separator) {
            ...
            for conflict in &merge_result.conflicts {
                insert.push_str("\n\\* <<<<<<< base\n");
                let _ = writeln!(insert, "\\* {} == {}", conflict.name, conflict.base_body);
                insert.push_str("\\* =======\n");
                let _ = writeln!(insert, "\\* {} == {}", conflict.name, conflict.patch_body);
                insert.push_str("\\* >>>>>>>\n");
            }
