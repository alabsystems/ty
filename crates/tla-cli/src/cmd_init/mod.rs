...
//! `ty init` subcommand: scaffold a new TLA+ specification project.
//!
...
mod tests {
    ...
    fn test_cmd_init_creates_files() {
        let dir = std::env::temp_dir().join("ty_test_init_basic");
    ...
    fn test_cmd_init_rejects_existing_without_force() {
        let dir = std::env::temp_dir().join("ty_test_init_no_force");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_cmd_init_force_overwrites() {
        let dir = std::env::temp_dir().join("ty_test_init_force");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
