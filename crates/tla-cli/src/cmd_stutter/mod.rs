...
//! `ty stutter` — stuttering and UNCHANGED analysis.
//!
...
pub(crate) fn cmd_stutter(
    ...
    if let Some(next_op) = operators.get(next_name) {
        extract_action_changes(&next_op.body.node, &operators, &mut actions, 0);
    }
...
fn extract_action_changes(
    ...
    match expr {
        Expr::Or(a, b) => {
            extract_action_changes(&a.node, operators, actions, depth);
            extract_action_changes(&b.node, operators, actions, depth);
        }
        Expr::Ident(name, _) => {
            if let Some(op) = operators.get(name.as_str()) {
                if matches!(&op.body.node, Expr::Or(_, _)) {
                    extract_action_changes(&op.body.node, operators, actions, depth + 1);
                } else {
