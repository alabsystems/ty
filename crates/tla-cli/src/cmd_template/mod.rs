...
//! `ty template` subcommand: generate complete protocol template TLA+ specs.
//!
//! Unlike `ty init` (which scaffolds project structure), `ty template`
...
pub(crate) fn cmd_template(
    ...
    println!("  2. Run: ty check {name}.tla");
...
mod tests {
    ...
    fn test_cmd_template_creates_files() {
        let dir = std::env::temp_dir().join("ty_test_template_mutex");
    ...
    fn test_cmd_template_rejects_empty_name() {
        let dir = std::env::temp_dir().join("ty_test_template_empty");
    ...
    fn test_cmd_template_rejects_zero_processes() {
        let dir = std::env::temp_dir().join("ty_test_template_zero");
