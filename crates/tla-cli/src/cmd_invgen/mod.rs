...
//! `ty inv-gen` — automatic invariant generation for TLA+ specifications.
//!
...
fn verify_candidates(
    ...
    let ty_bin = std::env::current_exe().context("resolve ty binary path")?;

    for candidate in candidates.iter_mut() {
        let result = verify_single_candidate(file, config, &ty_bin, candidate);
        match result {
            Ok(holds) => {
...
/// Verify a single candidate invariant by running `ty check` with a
...
fn verify_single_candidate(
    ...
    let tmp_dir = std::env::temp_dir().join("ty_invgen");
    ...
    let output = std::process::Command::new(ty_bin)
        ...
        .output()
        .context("run ty check for invariant verification")?;
...
fn print_tla(module_name: &str, candidates: &[CandidateInvariant]) {
    ...
    println!("\\* Generated by: ty inv-gen");
    println!("\\* =========================================================");
    println!();
