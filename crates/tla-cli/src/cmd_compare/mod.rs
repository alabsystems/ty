...
//! `ty compare` — structural comparison of two TLA+ specifications.
//!
...
pub(crate) fn cmd_compare(left: &Path, right: &Path, format: CompareOutputFormat) -> Result<()> {
    ...
    let report = CompareReport {
        extends: compare_extends(&left_module, &right_module),
        variables: compare_variables(&left_module, &right_module),
        constants: compare_constants(&left_module, &right_module),
        operators: compare_operators(&left_module, &right_module, &left_source, &right_source),
        instances: compare_instances(&left_module, &right_module),
    };
