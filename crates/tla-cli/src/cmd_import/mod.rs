...
//! `ty import` -- import specifications from other formats into TLA+.
//!
...
fn import_json_sm(content: &str, file: &Path) -> Result<(String, Option<String>)> {
    ...
    let _ = writeln!(o, "---- MODULE {} ----", sm.name);
    o.push_str("EXTENDS Integers, Sequences, FiniteSets, TLC\n\n");
    let _ = writeln!(o, "VARIABLE {}", vars.join(", "));
    let _ = writeln!(o, "\nvars == <<{}>>", vars.join(", "));
    // Init
    o.push_str("\nInit ==\n");
    ...
    for tr in &sm.transitions {
        ...
        for (k, v) in &tr.effect {
            let _ = writeln!(o, "    /\\ {k}' = {}", effect_val(v));
        }
...
struct PChan {
    name: String,
    /// Channel buffer capacity, parsed from the Promela source; retained for
    /// fidelity (asserted in tests) but not yet emitted into the TLA+ model.
    #[allow(dead_code)]
    capacity: usize,
    /// Parsed message type; retained for fidelity but not yet emitted.
    #[allow(dead_code)]
    msg_type: String,
}
struct PVar {
    name: String,
    /// Source type keyword (byte/int/...), parsed for fidelity but not yet
    /// emitted into the TLA+ model.
    #[allow(dead_code)]
    type_name: String,
    init: Option<String>,
}
...
fn import_promela(content: &str, file: &Path) -> Result<(String, Option<String>)> {
    ...
    let mut o = String::new();
    let _ = writeln!(o, "---- MODULE {} ----", spec.name);
    o.push_str("EXTENDS Integers, Sequences, FiniteSets, TLC\n\n");
    let _ = writeln!(
        o,
        "VARIABLE {}\n\nvars == <<{}>>",
    ...
    if spec.procs.is_empty() {
        ...
        for (i, p) in spec.procs.iter().enumerate() {
            let pfx = if i == 0 { "    " } else { "    \\/ " };
            let _ = writeln!(o, "{pfx}{}_step", p.name);
        }
    }
    o.push_str("\nSpec == Init /\\ [][Next]_vars\n\n====\n");
    Ok((o, Some("INIT Init\nNEXT Next\n".into())))
...
struct ASig {
    name: String,
    /// Whether the Alloy signature was declared `abstract`; parsed for fidelity
    /// but not yet reflected in the emitted TLA+ model.
    #[allow(dead_code)]
    is_abstract: bool,
...
struct APred {
    name: String,
    params: Vec<String>,
    /// Raw Alloy predicate body lines, captured during parsing; the body is not
    /// yet translated into TLA+ (emitted as a TODO placeholder instead).
    #[allow(dead_code)]
    body: Vec<String>,
}
struct AFact {
...
fn import_alloy(content: &str, file: &Path) -> Result<(String, Option<String>)> {
    ...
    if !sig_names.is_empty() {
        let _ = writeln!(o, "CONSTANT {}", sig_names.join(", "));
    }
