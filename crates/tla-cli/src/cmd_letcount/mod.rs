...
//! `ty let-count` — count LET-IN definitions.
//!
...
pub(crate) fn cmd_letcount(file: &Path, format: LetcountOutputFormat) -> Result<()> {
...
fn count_lets(expr: &Expr) -> usize {
    match expr {
        ...
        | Expr::NotIn(a, b)
        | Expr::Implies(a, b)
        | Expr::Subseteq(a, b) => count_lets(&a.node) + count_lets(&b.node),
        ...
        Expr::SetEnum(elems) | Expr::Times(elems) => {
            elems.iter().map(|e| count_lets(&e.node)).sum()
        }
        Expr::Apply(f, args) => {
            count_lets(&f.node) + args.iter().map(|a| count_lets(&a.node)).sum::<usize>()
        }
