...
//! `ty witness` -- structural feasibility analysis for a target predicate.
//!
//! Analyzes whether and how a state satisfying a target predicate could be
//! reached: it parses the spec, identifies the target operator, reports which
//! actions/operators reference it, and estimates how deep a BFS search would
//! need to go. Unlike `ty check` (which searches for violations), `witness`
//! reasons about *positive examples*. Concrete trace generation (bounded BFS
//! from an initial state to a target-satisfying state) is future work — see the
//! BFS search mode below.
//!
//! Modes:
//!
    ...
    match status {
        "target_not_found" => format!(
            "Target operator `{}` not found in module `{}`. \
             Available operators: check with `ty search operator {}`.",
    ...
    if analysis.target_found && analysis.has_init && analysis.has_next {
        ...
        println!(
            "\x1b[2mTip: Full witness trace generation via BFS will be available in a\n\
             future release. For now, use `ty check {} --config <cfg>` and inspect\n\
