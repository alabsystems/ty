...
//!
//! Usage:
//!   ty constrain Spec.tla --config Spec.cfg --strategy minimize
//!   ty constrain Spec.tla --config Spec.cfg --strategy incremental --output dir/
//!   ty constrain Spec.tla --config Spec.cfg --strategy symmetric
...
// `text`/`lineno` are retained for the complete parse model and Debug output;
// the current callers read only `directive`.
#[allow(dead_code)]
#[derive(Debug, Clone)]
struct CfgLine {
    /// Original line text.
...
// Several variant payloads are retained for the complete parse model and Debug
// output even though only `Constant`/`Symmetry` are inspected by callers today.
#[allow(dead_code)]
#[derive(Debug, Clone)]
enum CfgDirective {
    Init(String),
    ...
    for line in source.lines() {
        ...
        if let Some(rest) = strip_kw(trimmed, "CONSTANT").or_else(|| strip_kw(trimmed, "CONSTANTS"))
        {
            for name in rest.split(',') {
                let name = name.trim().trim_end_matches(['(', '_']);
                let name = name.trim();
...
fn generate_minimized_cfg(
    ...
    if overrides.is_empty() {
        ...
        return format!(
            "\\* Generated by: ty constrain --strategy minimize\n{}\n{}",
            comments.join("\n"),
            cfg_text,
        );
    }
    ...
    let _ = writeln!(result, "\\* Generated by: ty constrain --strategy minimize");
    for c in &comments {
...
fn generate_incremental_cfgs(
    ...
    for step in 1..=step_count {
        ...
        let _ = writeln!(
            result,
            "\\* Generated by: ty constrain --strategy incremental (step {}/{})",
            step, step_count,
        );
        result.push_str(&apply_constant_overrides(cfg_text, &overrides));
        configs.push(result);
...
fn generate_symmetric_cfg(
    ...
    if symmetric_sets.is_empty() {
        ...
        let _ = writeln!(
            result,
            "\\* Generated by: ty constrain --strategy symmetric"
        );
        let _ = writeln!(
            result,
...
mod tests {
    ...
    fn test_symmetric_preserves_existing_symmetry() {
        ...
        let _symmetry_count = result.matches("SYMMETRY").count();
