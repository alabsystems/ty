...
//! `ty convert` — format conversion for TLA+ and related formats.
//!
...
fn tla_to_markdown(config: &ConvertConfig) -> Result<()> {
    ...
    if !header_comment.is_empty() {
        let _ = write!(out, "{header_comment}\n\n");
    }
    ...
    if !constants.is_empty() {
        ...
        for c in &constants {
            ...
            let _ = writeln!(out, "| `{}` | {} |", c.name.node, arity_str);
        }
        out.push('\n');
    }
    ...
    if !operators.is_empty() {
        ...
        for op in &operators {
            ...
            let _ = write!(out, "### {sig}{visibility}\n\n");
        }
    }
    ...
    if !instances.is_empty() {
        ...
        for inst in &instances {
            ...
            let _ = writeln!(out, "- `INSTANCE {}`{local}", inst.module.node);
        }
        out.push('\n');
    }
...
fn json_module_to_tla(module: &JsonModule) -> String {
    ...
    let _ = writeln!(out, "{sep} MODULE {} {sep}", module.name);

    if !module.extends.is_empty() {
        let _ = writeln!(out, "EXTENDS {}", module.extends.join(", "));
    }
    ...
    for inst in &module.instances {
        ...
        if !inst.substitutions.is_empty() {
            let subs: Vec<String> = inst
                ...
                .collect();
            let _ = write!(out, " WITH {}", subs.join(", "));
        }
        out.push('\n');
    }
...
fn json_expr_to_tla(expr: &JsonExpr) -> String {
    match expr {
        ...
        JsonExpr::LetIn { defs, body } => {
            ...
            let _ = write!(s, "IN {}", json_expr_to_tla(body));
            s
        }
        ...
        JsonExpr::Case { arms, other } => {
            ...
            if let Some(o) = other {
                let _ = write!(s, " [] OTHER -> {}", json_expr_to_tla(o));
            }
            s
        }
        JsonExpr::ModuleRef {
...
fn format_trace_table(states: &[StateInfo], var_names: &[String]) -> String {
    ...
    // Header row.
    let _ = write!(out, "| {step_header:<step_width$} ");
    let _ = write!(out, "| {action_header:<action_width$} ");
    for (i, name) in var_names.iter().enumerate() {
        let _ = write!(out, "| {:<width$} ", name, width = col_widths[i]);
    }
    out.push_str("|\n");

    // Separator row.
    let _ = write!(out, "|-{}-", "-".repeat(step_width));
    let _ = write!(out, "|-{}-", "-".repeat(action_width));
    for w in &col_widths {
    ...
    for (state_idx, state) in states.iter().enumerate() {
        let _ = write!(out, "| {:<step_width$} ", state.index);
        let _ = write!(out, "| {:<action_width$} ", state.action.name);
        for (i, val) in cell_values[state_idx].iter().enumerate() {
            let _ = write!(out, "| {:<width$} ", val, width = col_widths[i]);
        }
        out.push_str("|\n");
    }
