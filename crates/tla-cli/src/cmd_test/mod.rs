...
/// Configuration for the `ty test` subcommand, built from CLI args.
...
fn run_parallel(
    ...
    std::thread::scope(|s| {
        ...
        for worker_id in 0..num_workers {
            let worker_runs = runs_per_worker + usize::from(worker_id < remainder);
            if worker_runs == 0 {
                continue;
            }
            ...
            let walk_offset: usize = (0..worker_id)
                .map(|w| runs_per_worker + usize::from(w < remainder))
                .sum();
