...
//! `ty ast-depth` — compute maximum AST depth.
//!
...
pub(crate) fn cmd_astdepth(file: &Path, format: AstdepthOutputFormat) -> Result<()> {
    ...
    ops.sort_by_key(|op| std::cmp::Reverse(op.1));
...
fn ast_depth(expr: &Expr, current: usize) -> usize {
    ...
    match expr {
        ...
        Expr::Not(inner) | Expr::Neg(inner) | Expr::Prime(inner) => ast_depth(&inner.node, next),
        Expr::If(c, t, e) => {
        ...
        Expr::SetEnum(elems) | Expr::Times(elems) => elems
            .iter()
            .map(|e| ast_depth(&e.node, next))
            .max()
            .unwrap_or(next),
        Expr::Apply(f, args) => {
            ...
            let da = args
                .iter()
                .map(|a| ast_depth(&a.node, next))
                .max()
                .unwrap_or(next);
            df.max(da)
        }
        Expr::Forall(_, body) | Expr::Exists(_, body) => ast_depth(&body.node, next),
        _ => next,
    }
}
