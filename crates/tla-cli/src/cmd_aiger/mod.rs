...
//! `ty aiger` subcommand: check AIGER bit-level hardware model checking benchmarks.
//!
...
//! + IC3 variants) or the CHC-based ay-chc portfolio solver.
//!
...
const DISABLE_IC3_UNKNOWN_FALLBACK_ENV: &str = "TY_AIGER_DISABLE_IC3_UNKNOWN_FALLBACK";
...
pub(crate) fn cmd_aiger(
    ...
    if verbose {
        ...
        let engine_desc = match engine {
            ...
            AigerEngine::Chc => "CHC (ay-chc adaptive portfolio)",
        ...
        if ic3_unknown_fallback_disabled_by_env() {
            eprintln!(
                "IC3 consecutive-Unknown fallback disabled by {DISABLE_IC3_UNKNOWN_FALLBACK_ENV}"
            );
        }
    }
    ...
    let results = if let Some(deadline) = timeout {
        ...
        match rx.recv_timeout(hard_deadline) {
            ...
            Err(_) => {
                ...
                // way to interrupt them from userspace since ay-sat's
...
fn check_aiger_sat_with_portfolio(
    ...
    let portfolio_result = if matches!(portfolio, AigerPortfolio::Adaptive) {
        ...
        config = apply_ic3_unknown_fallback_env(config);
    ...
    check_result_to_aiger(circuit, portfolio_result.result, n)
}
...
fn check_aiger_single_engine(
    ...
    let bmc_result = match engine {
        ...
        AigerEngine::KindStrengthened => {
            ...
            return check_result_to_aiger(circuit, result, properties_count);
        }
        AigerEngine::Ic3 => {
            // Run IC3 via single-engine portfolio config.
            let config = apply_ic3_unknown_fallback_env(tla_aiger::PortfolioConfig {
                timeout: Duration::from_secs(3600),
    ...
    match bmc_result.verdict {
        ...
        Some(false) => {
            if properties_count > 1 {
                // The engines decide the OR of all bad literals; attribute the
                // violation to the specific violated properties. The literal
                // witness is keyed to the engine's (COI-reduced / compacted)
                // variable numbering; project it onto the original frame
                // best-effort and let the attribution helper verify it (it
                // re-derives an original-frame counterexample when the frames
                // do not line up, and reports Unknown when even that fails).
                let map_trace =
                    lit_witness_to_named_trace(&ts, bmc_result.witness.as_deref().unwrap_or(&[]));
                return tla_aiger::attributed_unsafe_results(
                    circuit,
                    &map_trace,
                    bmc_result.depth,
                    properties_count,
...
/// Project a literal-level witness onto the `l{idx}`/`i{idx}` naming of `ts`
/// (one map per time step). Literals whose variable is neither a latch nor an
/// input of `ts` are dropped; consumers verify the projected trace by
/// simulation before trusting it.
fn lit_witness_to_named_trace(
    ts: &tla_aiger::transys::Transys,
    witness: &[Vec<tla_aiger::sat_types::Lit>],
) -> Vec<rustc_hash::FxHashMap<String, bool>> {
    witness
        .iter()
        .map(|step| {
            ...
            for lit in step {
                let var = lit.var();
                if let Some(pos) = ts.latch_vars.iter().position(|&v| v == var) {
                    named.insert(format!("l{pos}"), lit.is_positive());
                } else if let Some(pos) = ts.input_vars.iter().position(|&v| v == var) {
                    named.insert(format!("i{pos}"), lit.is_positive());
                }
            }
            named
        })
...
fn apply_ic3_unknown_fallback_env(
    config: tla_aiger::PortfolioConfig,
) -> tla_aiger::PortfolioConfig {
    if ic3_unknown_fallback_disabled_by_env() {
        config.with_ic3_unknown_fallback_disabled()
    } else {
        config
    }
}

fn ic3_unknown_fallback_disabled_by_env() -> bool {
    std::env::var(DISABLE_IC3_UNKNOWN_FALLBACK_ENV)
        .ok()
        .as_deref()
        .is_some_and(env_value_disables_ic3_unknown_fallback)
}

fn env_value_disables_ic3_unknown_fallback(value: &str) -> bool {
    matches!(
...
mod tests {
    ...
    #[test]
    fn ic3_unknown_fallback_env_parser_is_explicitly_truthy() {
        for value in ["1", "true", "TRUE", "yes", "on", "On"] {
            assert!(env_value_disables_ic3_unknown_fallback(value));
        }
    ...
    /// Finding-10 trigger: 2 properties, bad0 = constant FALSE (holds),
    /// bad1 = constant TRUE (violated at depth 0).
    const TWO_PROP_TRIGGER: &str = "aag 0 0 0 0 0 2\n0\n1\n";

    fn assert_attributed_to_property_1(results: &[tla_aiger::AigerCheckResult]) {
        assert_eq!(results.len(), 2);
        assert!(
            !matches!(results[0], tla_aiger::AigerCheckResult::Sat { .. }),
            "property 0 (bad = FALSE) must not be reported violated: {:?}",
            results[0]
        );
        assert!(
            matches!(results[1], tla_aiger::AigerCheckResult::Sat { .. }),
            "property 1 (bad = TRUE) must be reported violated: {:?}",
            results[1]
        );
    }

    /// Regression: the SAT portfolio path used to attribute ANY Unsafe to
    /// property 0 unconditionally (and the witness header named b0).
    #[test]
    fn aiger_multi_property_portfolio_attributes_violated_index() {
        let circuit = tla_aiger::parse_aag(TWO_PROP_TRIGGER).unwrap();
        let results = check_aiger_sat_with_portfolio(
            &circuit,
            Some(Duration::from_secs(120)),
            AigerPortfolio::Default,
            false,
        );
        assert_attributed_to_property_1(&results);
    }

    /// Regression: the single-engine (BMC) path had the same unconditional
    /// property-0 attribution.
    #[test]
    fn aiger_multi_property_single_engine_bmc_attributes_violated_index() {
        let circuit = tla_aiger::parse_aag(TWO_PROP_TRIGGER).unwrap();
        let results = check_aiger_single_engine(&circuit, AigerEngine::Bmc, 100, 2);
        assert_attributed_to_property_1(&results);
    }

    /// Same trigger through the k-induction single-engine path.
    #[test]
    fn aiger_multi_property_single_engine_kind_attributes_violated_index() {
        let circuit = tla_aiger::parse_aag(TWO_PROP_TRIGGER).unwrap();
        let results = check_aiger_single_engine(&circuit, AigerEngine::Kind, 100, 2);
        assert_attributed_to_property_1(&results);
    }
}
...
///
/// The engines decide the OR of all bad literals, so a multi-property
/// `Unsafe` verdict is attributed to the specific violated properties via
/// [`tla_aiger::attributed_unsafe_results`] (simulation of the verified
/// witness on the original circuit), never blindly to property 0.
fn check_result_to_aiger(
    ...
    match result {
        ...
        tla_aiger::CheckResult::Unsafe { trace, depth } => {
            if n == 1 {
                ...
                tla_aiger::attributed_unsafe_results(circuit, &trace, depth, n)
            }
        }
