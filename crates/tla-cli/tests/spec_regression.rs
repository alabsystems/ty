...
//! in the requested category, then runs `ty check` (or `ty simulate`) for each
...
fn error_types_match(ty_error: Option<&str>, tlc_error: Option<&str>) -> bool {
    match (ty_error, tlc_error) {
...
fn is_state_count_match(ty_states: Option<u64>, spec: &BaselineSpec) -> bool {
    let expected = spec.ty_expected_states.or(spec.tlc.states);
    matches!((ty_states, expected), (Some(t2), Some(exp)) if t2 == exp)
}
...
fn classify(
    ty_status: Option<&str>,
    ty_states: Option<u64>,
    ty_error: Option<&str>,
    spec: &BaselineSpec,
) -> Verdict {
    let ty_ok = ty_status == Some("ok");
    ...
    if ty_ok && tlc_no_error {
        if is_simulation_mode(spec) {
            return Verdict::Pass;
        }
        let expected_states = spec.ty_expected_states.or(spec.tlc.states);
        match (ty_states, expected_states) {
            (Some(t2), Some(exp)) if t2 == exp => Verdict::Pass,
            _ => Verdict::StateMismatch,
        }
    } else if ty_ok && tlc_has_error && is_simulation_mode(spec) {
        Verdict::Pass
    } else if ty_ok && tlc_has_error {
        Verdict::FalseNegative
    } else if !ty_ok && tlc_no_error {
        let expected_states = spec.ty_expected_states.or(spec.tlc.states);
        let state_count_matches = matches!(
            (ty_states, expected_states),
        ...
        if error_types_match(ty_error, spec.tlc.error_type.as_deref()) {
            Verdict::Pass
        } else {
            Verdict::BothFail
        }
...
fn run_spec(
    ...
    let tla_path = resolve_source_path(examples_dir, &source.tla_path);
    let cfg_path = resolve_source_path(examples_dir, &source.cfg_path);
    ...
    // The baseline records full (non-symmetry-reduced) distinct-state counts, and
    // the ty-vs-TLC comparison is apples-to-apples only without symmetry reduction.
    // ty auto-detects symmetry by default now (sound — orbit-reduces counts,
    // verdicts unchanged), which would otherwise read as a state-count "regression"
    // against the full-count baseline. Disable it for the coverage comparison.
    //
    // NOTE: this is now the `--no-auto-symmetry` CLI flag, NOT the old
    // `TY_AUTO_SYMMETRY=0` env var — env vars no longer control search semantics
    // (the binary warns and ignores the env), so the flag is passed to `check`
    // below. `simulate`/`generate` mode does not do exhaustive distinct-state
    // counting, so symmetry reduction does not apply there.
    match mode {
        "simulate" | "generate" => {
            cmd.arg("simulate")
                .arg(&tla_path)
        ...
        _ => {
            cmd.arg("check")
                ...
                .arg("--memory-limit")
                .arg("131072");
            // Large explicit-state PASSING specs (e.g. MCBakery, 655200 states)
            // explore the full space exhaustively, so their distinct-state count is
            // identical under any search order. They parallelize well but are slow
            // single-threaded in the default fused / auto-sequential mode (one BFS
            // lane), which can exceed the per-spec timeout in the unoptimized test
            // (debug) build. Compare these in parallel pure-BFS mode — the same mode
            // the BFS-based baseline effectively used. Everything else keeps
            // --workers 0 (auto), which handles inline-NEXT SPECIFICATIONs (the
            // EWD998 family) via the adaptive path and runs small specs fine. The
            // threshold (200k) cleanly isolates the one outlier large spec.
            if spec.tlc.status == "pass" && spec.tlc.states.unwrap_or(0) > 200_000 {
                cmd.arg("--bfs-only").arg("--workers").arg("4");
            } else {
                cmd.arg("--workers").arg("0");
            }
    ...
    match wait_with_timeout(child, timeout) {
        ...
        WaitOutcome::Timeout => {
            ...
            SpecRunResult {
                ...
                ty_states: None,
                expected_states: spec.ty_expected_states.or(spec.tlc.states),
...
fn parse_and_classify_output(
    ...
    let (ty_status, ty_states, ty_error) = match &parsed {
        Some(p) => {
    ...
    let raw = classify(ty_status.as_deref(), ty_states, ty_error.as_deref(), spec);
    let verdict = apply_expected_mismatch(raw, spec);
    ...
    SpecRunResult {
        ...
        ty_states,
        expected_states: spec.ty_expected_states.or(spec.tlc.states),
        detail: ty_error.unwrap_or_default(),
        elapsed_secs: elapsed,
    }
}
...
fn examples_dir(baseline: &Baseline) -> PathBuf {
    // 1. $TLAPLUS_EXAMPLES (may point AT `specifications` or its parent).
    ...
    // 2. The examples_dir recorded in the baseline (if it resolves here).
    let path = PathBuf::from(&baseline.inputs.examples_dir);
    if path.is_dir() {
        return path;
    }
    // 3. Fallback: try $HOME/tlaplus-examples/specifications
...
/// Resolve a baseline source path. Relative paths resolve under the examples
/// dir first, then under the workspace root (some baseline entries point at
/// in-repo specs, e.g. `specs/ay/...` or `tests/apalache_parity/...`).
fn resolve_source_path(examples_dir: &Path, rel: &str) -> PathBuf {
    let p = Path::new(rel);
    if p.is_absolute() {
        return p.to_path_buf();
    }
    let under_examples = examples_dir.join(p);
    if under_examples.is_file() {
        return under_examples;
    }
    let under_repo = workspace_root().join(p);
...
fn spec_coverage_regression() {
    ...
    if !regressions.is_empty() {
        ...
        for r in &regressions {
            ...
            eprintln!("  TY states: {:?}", r.ty_states);
