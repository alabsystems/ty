...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn output_json_no_cfg_infers_init_next_and_emits_valid_json() {
    // With no .cfg present, ty infers the conventional Init/Next operators and runs
    // (a deliberate convenience: "No config file found. Using convention names: Init,
    // Next"), rather than treating a missing cfg as a hard setup error. The shared
    // harness bounds the run to --max-states 1, so the inferred run cleanly stops at
    // the state limit (status "limit_reached", exit 0) and emits well-formed JSON —
    // the point is that the missing cfg was INFERRED, not rejected as a setup error.
    ...
    assert_eq!(
        v.get("result")
            ...
            .and_then(|s| s.as_str()),
        Some("limit_reached"),
        "no-cfg run should be inferred and bounded (not a setup error): {v:?}"
    );
}

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn output_jsonl_no_cfg_infers_init_next_and_emits_valid_jsonl() {
    // As above, in JSONL: a missing cfg is inferred (not a setup error) and the run,
    // bounded to --max-states 1 by the harness, cleanly hits the state limit and
    // emits a well-formed JSONL summary line.
    ...
    assert_eq!(
        v.get("result")
            ...
            .and_then(|s| s.as_str()),
        Some("limit_reached"),
        "no-cfg inference JSONL run should be inferred and bounded (not a setup error): {v:?}"
    );
}
