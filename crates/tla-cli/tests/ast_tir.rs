...
fn ast_without_tir_keeps_lowered_ast_output() {
    let dir = TempDir::new("ty-ast-output");
...
fn ast_tir_dumps_bootstrap_tir_for_single_module() {
    let dir = TempDir::new("ty-ast-tir");
    let main = dir.path.join("Main.tla");
    common::write_file(
        &main,
        br#"
...
fn ast_tir_loads_instance_modules_before_lowering() {
    let dir = TempDir::new("ty-ast-tir-instance");
    let main = dir.path.join("Main.tla");
...
"#,
    ...
    let out = Command::new(bin)
        .args(["ast", "--tir", main.to_str().expect("utf-8 main path")])
        .output()
        .expect("run ty ast --tir with instance module");
...
fn ast_tir_loads_imported_parameterized_instance_modules_before_lowering() {
    let dir = TempDir::new("ty-ast-tir-imported-instance");
    let main = dir.path.join("Main.tla");
...
"#,
    ...
    let out = Command::new(bin)
        .args(["ast", "--tir", main.to_str().expect("utf-8 main path")])
        .output()
        .expect("run ty ast --tir with imported parameterized instance");
...
fn ast_tir_resolves_let_bound_instance_aliases() {
    let dir = TempDir::new("ty-ast-tir-let-instance");
    let main = dir.path.join("Main.tla");
    let inner = dir.path.join("Inner.tla");
...
"#,
    ...
    let out = Command::new(bin)
        .args(["ast", "--tir", main.to_str().expect("utf-8 main path")])
        .output()
        .expect("run ty ast --tir with let-bound instance");
