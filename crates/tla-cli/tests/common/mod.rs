...
/// Run the ty binary with the given arguments and return raw output.
#[allow(dead_code)]
pub fn run_tla(args: &[&str]) -> Output {
    run_tla_with_env(args, &[])
}

/// Run the ty binary with custom environment variables.
#[allow(dead_code)]
pub fn run_tla_with_env(args: &[&str], env: &[(&str, &str)]) -> Output {
    ...
    cmd.output().expect("failed to execute ty")
}

/// Run the ty binary with a specific timeout and return raw output.
#[allow(dead_code)]
pub fn run_tla_with_timeout(args: &[&str], timeout: Duration) -> Output {
    run_tla_with_env_timeout(args, &[], timeout)
}

/// Run the ty binary with custom environment variables and a specific timeout.
#[allow(dead_code)]
...
/// Run the ty binary and return (exit_code, stdout, stderr).
#[allow(dead_code)]
pub fn run_tla_parsed(args: &[&str]) -> (i32, String, String) {
    run_tla_parsed_with_env(args, &[], &[])
}

/// Run the ty binary with a specific timeout and return (exit_code, stdout, stderr).
#[allow(dead_code)]
...
/// Run the ty binary with env vars and env removals, return (exit_code, stdout, stderr).
...
pub fn run_tla_parsed_with_env(
    ...
    let output = cmd.output().expect("failed to execute ty");
    (
...
/// Run the ty binary with env vars, env removals, and a specific timeout.
#[allow(dead_code)]
...
fn run_command_with_timeout(mut cmd: Command, args: &[&str], timeout: Duration) -> Output {
    let mut child = cmd.spawn().expect("failed to execute ty");
    ...
    loop {
        if start.elapsed() > timeout {
            ...
            let stderr = stderr_reader
                ...
                .unwrap_or_default();
            panic!(
                "ty subprocess timed out after {timeout:?}: {args:?}\nstdout:\n{}\nstderr:\n{}",
                String::from_utf8_lossy(&stdout),
                String::from_utf8_lossy(&stderr)
            );
        }

        if let Some(status) = child.try_wait().expect("try_wait on ty") {
            let stdout = stdout_reader
                .join()
                .expect("join stdout reader")
                .expect("read stdout");
