...
//!
//! Exercises Apalache-specific features supported by TY:
//! - BMC mode (bounded model checking via ay)
...
// ===========================================================================
// BMC mode tests (require ay feature)
// ===========================================================================
...
===="#,
    ...
    // `Safety` genuinely holds (flag is set TRUE only when x' >= 3), so the
    // soundness contract is: BMC must NEVER report a counterexample on this spec,
    // and it must TERMINATE. `TY_BMC_TIMEOUT_SECS=2` bounds the standalone solve
    // (the ay LIA `solve_lia_eager_split_loop` does not converge at depth 8 — a
    // known solver limitation; the BMC watchdog converts the would-be hang into a
    // bounded `Unknown`). The result is therefore either a sound "NO BUG FOUND"
    // (if/when the ay LIA solve lands) or a bounded inconclusive verdict — but
    // never a false bug. See the BMC-watchdog fix and the ay LIA root-cause in
    ...
    let (code, stdout, stderr) = common::run_tla_parsed_with_env_timeout(
        ...
        &[("TY_BMC_TIMEOUT_SECS", "2")],
        &[],
        Duration::from_secs(60),
    );
    // Must terminate (a non-124 exit) — the watchdog guarantees no hang.
    assert_ne!(
        code, 124,
        "standalone --bmc must not hang.\nstdout:\n{stdout}\nstderr:\n{stderr}"
    );
    // Soundness: never a false counterexample on a safe spec.
    let combined = format!("{stdout}\n{stderr}");
    assert!(
        !combined.contains("VIOLATION") && !combined.contains("Counterexample"),
        "BMC must not report a false bug on a safe spec.\nstdout:\n{stdout}\nstderr:\n{stderr}"
    );
    // Terminates with a sound BMC verdict (proved safe, or a bounded inconclusive).
    assert!(
        combined.contains("NO BUG FOUND")
            || combined.contains("UNKNOWN")
            || combined.contains("solve budget"),
        "BMC should terminate with a sound verdict.\nstdout:\n{stdout}\nstderr:\n{stderr}"
    );
}

// ===========================================================================
// k-Induction tests (require ay feature)
// ===========================================================================
...
// ===========================================================================
// Inductive invariant convenience mode (require ay feature)
// ===========================================================================
...
/// Explicit `--workers N>1` with `--trace-inv` must never silently skip the
/// trace invariant and report a false PASS on a violating spec.
///
/// With the "ay" feature (default build), the fused/CDEMC default engine
/// handles the request: symbolic lanes cannot claim a covering proof when
/// trace invariants are present (`symbolic_safety_proof_covers_all_obligations`
/// requires none), so the sequential BFS lane evaluates the trace invariant
/// and reports the violation. Without "ay" (lean build), there is no fused
/// engine, the request routes to ParallelChecker (which has no trace-invariant
/// implementation), and it must be rejected up front.
///
/// The explicit `--bfs-only` path always routes to ParallelChecker for
/// `--workers N>1`, so the rejection gate is asserted there in both builds.
#[cfg_attr(test, ntest::timeout(30000))]
#[test]
fn test_trace_invariant_parallel_workers_checked_not_skipped() {
    let (_dir, spec, cfg) = setup_spec(
        "TraceInvParallel",
        r#"---- MODULE TraceInvParallel ----
EXTENDS Naturals, Sequences
VARIABLE x
Init == x = 0
Next == x < 3 /\ x' = x + 1
...
===="#,
    ...
    // The spec violates the trace invariant, so exit 0 here would mean the
    // parallel path silently skipped it (the former false-PASS defect).
    assert_ne!(
        code, 0,
        "--workers 4 with --trace-inv must not silently pass.\nstdout:\n{stdout}\nstderr:\n{stderr}"
    );
    if cfg!(feature = "ay") {
        // Fused default engine: the sequential BFS lane evaluates the trace
        // invariant and reports the actual violation (2-state trace x=0 -> x=1).
        assert!(
            stderr.contains("Invariant DecreasingTrace is violated"),
            "expected the trace-invariant violation to be reported (fused engine).\nstderr:\n{stderr}"
        );
        assert!(
            stderr.contains("State 2"),
            "expected the 2-state violating trace.\nstderr:\n{stderr}"
        );
    } else {
        // Lean build: no fused engine; the ParallelChecker route must be
        // rejected up front (it would silently skip trace invariants).
        assert!(
            stderr.contains("--trace-inv is only supported with --workers 0 or --workers 1"),
            "expected the explicit --trace-inv/--workers rejection message.\nstderr:\n{stderr}"
        );
    }

    // The explicit full-BFS path routes --workers N>1 to ParallelChecker,
    // which has no trace-invariant implementation; the up-front rejection
    // gate must remain so it can never silently skip them.
    let (bfs_code, bfs_stdout, bfs_stderr) = run(&[
        "check",
        &spec,
        "--config",
        &cfg,
    ...
    assert_ne!(
        bfs_code, 0,
        "--bfs-only --workers 4 with --trace-inv must be rejected.\nstdout:\n{bfs_stdout}\nstderr:\n{bfs_stderr}"
    );
    assert!(
        bfs_stderr.contains("--trace-inv is only supported with --workers 0 or --workers 1"),
        "expected the explicit --trace-inv/--workers rejection message on --bfs-only.\nstderr:\n{bfs_stderr}"
    );
}
...
===="#,
    ...
    // ITF is a counterexample-trace format (`--trace-format itf`), surfaced in the
    // structured JSON output. A SAFE spec has no counterexample, so no ITF trace is
    // emitted — the run simply succeeds with a valid JSON `ok` verdict. (The old
    // `--output itf` form was refactored away; `--output` is human/json/jsonl/tlc-tool.)
    let (code, stdout, stderr) = run(&[
        "check",
        &spec,
        "--config",
    ...
    assert_eq!(
        code, 0,
        "ITF-mode check of a safe spec should succeed.\nstdout:\n{stdout}\nstderr:\n{stderr}"
    );
    let parsed: Result<serde_json::Value, _> = serde_json::from_str(&stdout);
    assert!(
        parsed.is_ok(),
        "ITF-mode output should be valid JSON.\nstdout:\n{stdout}\nparse error: {:?}",
        parsed.err()
    );
    assert!(
        stdout.contains("\"status\": \"ok\""),
        "safe spec should report an ok verdict.\nstdout:\n{stdout}"
    );
}
...
===="#,
    ...
    // The ITF counterexample trace is surfaced in the structured JSON output, so
    // request both `--output json` and `--trace-format itf`.
    let (code, stdout, stderr) = run(&[
        "check",
        &spec,
        "--config",
    ...
    // The counterexample trace must be rendered in ITF format.
    if !stdout.is_empty() {
...
fn test_apalache_parity_batch() {
    ...
    if !specs_dir.exists() {
        // External fixtures not present in this checkout — skip rather than fail.
        eprintln!(
            "SKIP apalache_parity batch: specs directory not found: {}",
            specs_dir.display()
        );
        return;
    }
...
/// Resolve the (spec, cfg) paths for a parity test, or `None` when the out-of-repo
/// `tests/apalache_parity/` fixtures are absent.
///
/// These are external Apalache-parity fixtures NOT checked into the repo. A
/// missing fixture is an environment gap, not a TY regression, so the test must
/// SKIP (return early), not FAIL with a spurious "No such file". Callers use
/// `let Some((spec, cfg)) = apalache_spec_path("..") else { return; };`.
fn apalache_spec_path(name: &str) -> Option<(String, String)> {
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let workspace_root = std::path::Path::new(manifest_dir)
        .parent()
        .unwrap()
        .parent()
        .unwrap();
    let spec_path = workspace_root.join(format!("tests/apalache_parity/specs/{name}.tla"));
    if !spec_path.exists() {
        eprintln!(
            "SKIP {name}: apalache_parity fixtures absent ({}). These are external \
             fixtures not checked into the repo.",
            spec_path.display()
        );
        return None;
    }
...
fn test_case_expressions() {
    let Some((spec, cfg)) = apalache_spec_path("CaseExpressions") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg]);
    assert_eq!(
...
fn test_constants_config_model_values() {
    let Some((spec, cfg)) = apalache_spec_path("ConstantsConfig") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg]);
    assert_eq!(
...
fn test_disjunctive_actions() {
    let Some((spec, cfg)) = apalache_spec_path("DisjunctiveActions") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg]);
    assert_eq!(
...
fn test_function_composition() {
    let Some((spec, cfg)) = apalache_spec_path("FunctionComposition") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg]);
    assert_eq!(
...
fn test_let_in_parametric() {
    let Some((spec, cfg)) = apalache_spec_path("LetInParametric") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg]);
    assert_eq!(
...
fn test_let_without_in() {
    let Some((spec, cfg)) = apalache_spec_path("LetWithoutIn") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg]);
    assert_eq!(
...
fn test_nested_except() {
    let Some((spec, cfg)) = apalache_spec_path("NestedExcept") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg]);
    assert_eq!(
...
#[test]
fn test_option_ops() {
    let Some((spec, cfg)) = apalache_spec_path("OptionOps") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg]);
    assert_eq!(
...
fn test_recursive_factorial() {
    let Some((spec, cfg)) = apalache_spec_path("RecursiveFactorial") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg]);
    assert_eq!(
...
fn test_recursive_fibonacci() {
    let Some((spec, cfg)) = apalache_spec_path("RecursiveFibonacci") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg]);
    assert_eq!(
...
fn test_string_operations() {
    let Some((spec, cfg)) = apalache_spec_path("StringOperations") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg]);
    assert_eq!(
...
fn test_subset_powerset() {
    let Some((spec, cfg)) = apalache_spec_path("SubsetPowerset") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg]);
    assert_eq!(
...
fn test_tuple_quantifiers() {
    let Some((spec, cfg)) = apalache_spec_path("TupleQuantifiers") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg]);
    assert_eq!(
...
fn test_assign_op_basic() {
    let Some((spec, cfg)) = apalache_spec_path("AssignOp") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg]);
    assert_eq!(
...
fn test_itf_trace_spec() {
    let Some((spec, cfg)) = apalache_spec_path("ItfTrace") else {
        return;
    };
