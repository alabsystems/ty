...
fn check_skip_liveness_disables_store_states_auto_enable() {
    let dir = TempDir::new("ty-check-skip-liveness-test");
...
"#,
    ...
    // Baseline run without TY_SKIP_LIVENESS — must succeed.
    ...
    // With TY_SKIP_LIVENESS=1, PROPERTY/liveness checks are skipped and
    // store-states auto-enable does NOT fire (this is the contract this
    // test pins).
    ...
    assert!(stdout.contains("TY_SKIP_LIVENESS=1"), "stdout:\n{stdout}");
    assert!(
...
fn check_skip_liveness_skips_tautology_rejection() {
    let dir = TempDir::new("ty-check-skip-liveness-tautology-test");
...
"#,
    ...
    // With TY_SKIP_LIVENESS=1, PROPERTY/liveness checks are skipped.
    let (code, stdout, stderr) = run_tla(
        &["check", spec_str, "--config", cfg_str],
        &[("TY_SKIP_LIVENESS", "1")],
    );
