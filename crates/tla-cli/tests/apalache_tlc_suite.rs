...
/// intractable state spaces that TY cannot handle in test time.
...
fn test_apalache_tlc_suite_batch() {
    ...
    if !suite_dir.exists() {
        ...
        // (these are TLC-compatibility specs not checked into the repo).
        eprintln!(
            "SKIP apalache_tlc_suite batch: directory not found: {}",
            suite_dir.display()
        );
        return;
    }
...
/// Resolve the (spec, cfg) paths for a suite test, or `None` when the out-of-repo
/// `tests/apalache_tlc_suite/` fixtures are absent.
///
/// These are external TLC-compatibility fixtures that are NOT checked into the
/// repo (like the `~/tlaplus-examples` corpus). When they are missing the test
/// must SKIP (return early), not FAIL with a spurious "No such file" — a missing
/// fixture is an environment gap, not a TY regression. Callers use
/// `let Some((spec, cfg)) = tlc_suite_spec_path("..") else { return; };`.
fn tlc_suite_spec_path(name: &str) -> Option<(String, String)> {
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let workspace_root = std::path::Path::new(manifest_dir)
        .parent()
        .unwrap()
        .parent()
        .unwrap();
    let spec_path = workspace_root.join(format!("tests/apalache_tlc_suite/{name}.tla"));
    if !spec_path.exists() {
        eprintln!(
            "SKIP {name}: apalache_tlc_suite fixtures absent ({}). These are external \
             TLC-compatibility fixtures not checked into the repo.",
            spec_path.display()
        );
        return None;
    }
...
fn test_tlc_suite_set_equality() {
    let Some((spec, cfg)) = tlc_suite_spec_path("test1") else {
        return;
    };
...
fn test_tlc_suite_function_equality() {
    let Some((spec, cfg)) = tlc_suite_spec_path("test2") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg, "--no-deadlock"]);
    assert_eq!(
...
fn test_tlc_suite_tuple_ops() {
    let Some((spec, cfg)) = tlc_suite_spec_path("test3") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg, "--no-deadlock"]);
    assert_eq!(
...
fn test_tlc_suite_strings_records() {
    let Some((spec, cfg)) = tlc_suite_spec_path("test4") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg, "--no-deadlock"]);
    assert_eq!(
...
fn test_tlc_suite_cartesian_product() {
    let Some((spec, cfg)) = tlc_suite_spec_path("test5") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg, "--no-deadlock"]);
...
fn test_tlc_suite_arithmetic() {
    let Some((spec, cfg)) = tlc_suite_spec_path("test10") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg, "--no-deadlock"]);
    assert_eq!(
...
fn test_tlc_suite_sequences() {
    let Some((spec, cfg)) = tlc_suite_spec_path("test14") else {
        return;
    };
    let (code, stdout, stderr) = run(&["check", &spec, "--config", &cfg, "--no-deadlock"]);
    assert_eq!(
...
fn test_tlc_suite_error_too_many_args() {
    let Some((spec, cfg)) = tlc_suite_spec_path("etest1") else {
        return;
    };
...
fn test_tlc_suite_error_too_few_args() {
    let Some((spec, cfg)) = tlc_suite_spec_path("etest2") else {
        return;
    };
    let (code, _stdout, _stderr) = run(&["check", &spec, "--config", &cfg, "--no-deadlock"]);
    assert_ne!(
