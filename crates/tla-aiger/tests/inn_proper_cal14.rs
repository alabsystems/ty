...
//!
//! Exercises internal-signal latch promotion (FMCAD'21).
//! cal14 (23 latches, wide-unconstrained inputs) is the canonical target: it
//! is trivially solvable, yet tla-aiger's default IC3 configs time out. The
...
fn cal14_path() -> PathBuf {
    let root = match std::env::var_os("HWMCC_BENCHMARKS") {
        Some(env) if !env.is_empty() => PathBuf::from(env),
        _ => std::env::var_os("HOME")
            .map(PathBuf::from)
            .unwrap_or_else(|| PathBuf::from("."))
            .join("hwmcc/benchmarks"),
    };
    root.join("bitlevel/safety/2019/goel/industry/cal14/cal14.aig")
}
...
fn inn_proper_transforms_cal14() {
    let cal14 = cal14_path();
    if !cal14.exists() {
        eprintln!("skip: cal14 benchmark not available");
        return;
    }

    let circuit = tla_aiger::parse_file(&cal14).expect("parse cal14.aig");
...
fn inn_proper_ic3_cal14_runs_without_crash() {
    ...
    let config = PortfolioConfig {
        ...
        engines: vec![EngineConfig::Ic3Configured {
            ...
            name: "ic3-isig-proper-cal14-test".into(),
        }],
        max_depth: 50000,
    ...
    match result {
        CheckResult::Safe => {
            eprintln!("cal14 inn-proper SOLVED UNSAT (bonus — matches the known HWMCC verdict)");
        }
        CheckResult::Unsafe { .. } => {
