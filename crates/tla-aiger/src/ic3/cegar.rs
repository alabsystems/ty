...
/// Abstraction mode for CEGAR-IC3.
///
/// These modes control how aggressively the abstraction removes information
/// from the concrete model. Both are instances of the classic CEGAR recipe
/// (Clarke et al., JACM 2003 — see the module doc): drop information, model
/// check the smaller system, refine on spurious counterexamples. They
/// differ only in WHICH information the abstraction drops:
///
...
pub(crate) enum AbstractionMode {
    /// Abstract only constraint clauses ("cegar-const" in portfolio names).
    /// Keeps all transition relations but relaxes constraint enforcement.
    AbstractConstraints,
    /// Abstract both constraints and transition relation ("cegar-full" in
    /// portfolio names). Non-abstract latches become free variables.
    #[default]
    AbstractAll,
}
...
impl CegarIc3 {
    ...
    ///
    /// - `AbstractConstraints` (cegar-const): keeps all transition clauses,
    ///   only relaxes constraint enforcement on non-abstract latches. Fewer
    ///   refinement iterations but less abstraction benefit.
    /// - `AbstractAll` (cegar-full): removes both constraints and transition
    ...
    pub(crate) fn with_mode(
        ...
        let mode_name = match mode {
            AbstractionMode::AbstractConstraints => "cegar-const",
            AbstractionMode::AbstractAll => "cegar-full",
        };
        eprintln!(
    ...
    ///
    /// Refinement starts with only the `bad` variables' constraints;
    /// constraint variables are gradually refined in.
