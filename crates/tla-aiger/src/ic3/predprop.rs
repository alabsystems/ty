...
//!
//! Predecessor propagation provides an alternative bad-state
...
/// lemmas from IC3's infinity frame.
...
impl PredPropSolver {
    ...
    /// Update the solver backend (e.g., after fallback from ay-sat to SimpleSolver).
...
mod tests {
    ...
    fn test_predprop_backend_switch_preserves_lemmas() {
        ...
        // when ay-sat produces persistent FINALIZE_SAT_FAIL errors.
        //
        ...
        // Start with default backend (AYSat in production, Simple in tests).
        let mut pp = PredPropSolver::new(
            &ts,
            &next_vars,
            max_var,
