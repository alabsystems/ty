...
//! Unit tests for the IC3/PDR engine (extracted from ic3/mod.rs).

use std::fmt::Write as _;

use super::*;
use crate::parser::parse_aag;
use crate::sat_types::{AYSatCdclSolver, SatSolver};
...
mod env_guard {
    ...
    /// THE single raw env-mutation site — every other path routes here. A raw
    /// `set_var`/`remove_var` must exist somewhere to implement the primitive;
    /// this one lock-guarded helper is that somewhere. `env_mutation` is the
    /// Trust toolchain's deny-by-default env wall; `unknown_lints` keeps the
    /// stock-rustc build green (the lint is Trust-only).
    ...
    /// Acquire the process-wide environment lock explicitly. Poison is recovered.
...
fn test_toggle_simple_vs_ay() {
    ...
    // Debug: test ay-sat solver directly for the toggle
    let mut vs = AYSatCdclSolver::new(3);
    ...
    eprintln!("ay-sat direct: bad reachable? {r:?}");
    vs.add_clause(&[crate::sat_types::Lit::neg(crate::sat_types::Var(1))]);
    let r2 = vs.solve(&[crate::sat_types::Lit::pos(crate::sat_types::Var(2))]);
    eprintln!("ay-sat direct: predecessor? {r2:?}");

    // Test with AYSolver via IC3
    let mut engine_ay = Ic3Engine::new(ts);
    let result_ay = engine_ay.check();
    eprintln!("AYSolver IC3 result: {result_ay:?}");
    ...
    assert!(
        matches!(result_ay, Ic3Result::Unsafe { .. }),
        "AYSolver should find Unsafe, got {result_ay:?}"
    );
}
...
";
    ...
    // Test with AYSat (production solver). SimpleSolver is too slow for
...
fn test_is_poisoned_trait_method() {
    ...
    // AYSatCdclSolver should start unpoisoned.
    let ay = AYSatCdclSolver::new(5);
    assert!(
        !ay.is_poisoned(),
        "fresh AYSatCdclSolver should not be poisoned"
    );
}
...
fn test_circuit_adapt_ctg_params_adjusted() {
    ...
    //   - num_latches <  100 => boost: ctg_max = max(5), ctg_limit = max(12)
    ...
    // values must be capped to ty's conservative baseline.
    let circuit = parse_aag("aag 1 0 1 0 0 1\n2 0\n2\n").unwrap();
    let ts = crate::transys::Transys::from_aiger(&circuit);
    assert!(
        ts.num_latches < 30,
    ...
    // Case 3: small-medium circuits (30..100 latches) should exercise the
    // boost branch that this regression test originally intended to pin.
    let mut small_medium_aag = String::from("aag 40 0 40 0 0 1\n");
    for latch in 1..=40 {
        let _ = writeln!(small_medium_aag, "{} 0", latch * 2);
    }
    small_medium_aag.push_str("2\n");

    let circuit3 = parse_aag(&small_medium_aag).unwrap();
    let ts3 = crate::transys::Transys::from_aiger(&circuit3);
    assert!(
        (30..100).contains(&ts3.num_latches),
        "test circuit must be small-medium, got {} latches",
        ts3.num_latches
    );

    let boosted_config = Ic3Config {
        circuit_adapt: true,
        ctg_max: 1,
        ctg_limit: 1,
        ..Ic3Config::default()
    };
    let engine3 = Ic3Engine::with_config(ts3, boosted_config);
    assert_eq!(
        engine3.config.ctg_max, 5,
        "ctg_max should be boosted to 5 for small-medium circuit"
    );
    assert_eq!(
        engine3.config.ctg_limit, 12,
        "ctg_limit should be boosted to ty's raised budget (12) for small-medium circuit"
    );
}
...
fn test_ic3_extend_with_solver_cloning_toggle() {
    ...
    // Test with AYSat backend (uses clause-log replay cloning).
    let config_ay = Ic3Config {
        solver_backend: crate::sat_types::SolverBackend::AYSat,
        ..Ic3Config::default()
    };
    let result_ay = check_ic3_with_config(&circuit, config_ay);
    assert!(
        matches!(result_ay, Ic3Result::Unsafe { .. }),
        "AYSat with cloning: expected Unsafe, got {result_ay:?}"
    );
...
fn test_ic3_extend_with_solver_cloning_safe() {
    ...
    let result = check_ic3_with_config(&circuit, config_ay);
    assert!(
        matches!(result, Ic3Result::Safe { .. }),
        "AYSat with cloning: expected Safe, got {result:?}"
    );
}
...
fn test_no_preprocess_config_safe() {
    // No-preprocess variant (CTG=0, no parent lemma) should still work.
    let circuit = parse_aag("aag 1 0 1 0 0 1\n2 0\n2\n").unwrap();
    let config = Ic3Config {
        ctg_max: 0,
        ctg_limit: 0,
...
fn test_no_parent_config_safe() {
    // No-parent variant (parent_lemma=false) should work.
    let circuit = parse_aag("aag 1 0 1 0 0 1\n2 0\n2\n").unwrap();
    let config = Ic3Config {
        parent_lemma: false,
        ..Ic3Config::default()
...
fn test_ic3_non_unit_init_sat_check_both_paths() {
    ...
    assert!(!engine
        .cube_sat_consistent_with_init(&[crate::sat_types::Lit::pos(crate::sat_types::Var(3))]));

    // Cube [A=false]: partial cube in Init, SAT check should return true.
    assert!(engine
        .cube_sat_consistent_with_init(&[crate::sat_types::Lit::neg(crate::sat_types::Var(2))]));
...
fn test_consecution_verify_interval_full_small_circuit_skip() {
    ...
    // rejecting ay-sat's correct UNSAT lemmas indefinitely. Post-
...
/// ay-sat says UNSAT but SimpleSolver disagrees (false SAT on constraint-
...
//
// Internal-signals mode (FMCAD'21) extends IC3 cubes to include
...
/// Uses SimpleSolver to avoid ay-sat crash bugs on small circuits.
...
/// IC3 isig + CTP configuration correctness (internal-signal predicates + CTP).
#[test]
fn test_ic3_isig_ctp_safe() {
    let circuit = parse_aag("aag 1 0 1 0 0 1\n2 0\n2\n").unwrap();
    let config = Ic3Config {
        internal_signals: true,
        ctp: true,
    ...
    assert!(
        matches!(result, Ic3Result::Safe { .. }),
        "ic3_isig_ctp should prove safe, got {result:?}"
    );
}

/// IC3 isig + CTP on unsafe circuit.
#[test]
fn test_ic3_isig_ctp_unsafe() {
    let circuit = parse_aag("aag 1 0 1 0 0 1\n2 3\n2\n").unwrap();
    let config = Ic3Config {
        internal_signals: true,
        ctp: true,
    ...
    assert!(
        matches!(result, Ic3Result::Unsafe { .. }),
        "ic3_isig_ctp should find bugs, got {result:?}"
    );
}

/// IC3 isig + CTG-down (internal-signal predicates + model-based shrinking).
#[test]
fn test_ic3_isig_ctg_down_safe() {
    let circuit = parse_aag("aag 1 0 1 0 0 1\n2 0\n2\n").unwrap();
    let config = Ic3Config {
        internal_signals: true,
        ctg_down: true,
    ...
    assert!(
        matches!(result, Ic3Result::Safe { .. }),
        "ic3_isig_ctg_down should prove safe, got {result:?}"
    );
}

/// IC3 isig + CTG-down on unsafe circuit.
#[test]
fn test_ic3_isig_ctg_down_unsafe() {
    let circuit = parse_aag("aag 1 0 1 0 0 1\n2 3\n2\n").unwrap();
    let config = Ic3Config {
        internal_signals: true,
        ctg_down: true,
    ...
    assert!(
        matches!(result, Ic3Result::Unsafe { .. }),
        "ic3_isig_ctg_down should find bugs, got {result:?}"
    );
}

/// IC3 isig + dynamic generalization (internal-signal predicates + dynamic).
#[test]
fn test_ic3_isig_dynamic_safe() {
    let circuit = parse_aag("aag 1 0 1 0 0 1\n2 0\n2\n").unwrap();
    let config = Ic3Config {
        internal_signals: true,
        dynamic: true,
    ...
    assert!(
        matches!(result, Ic3Result::Safe { .. }),
        "ic3_isig_dynamic should prove safe, got {result:?}"
    );
}

/// IC3 isig + dynamic on unsafe circuit.
#[test]
fn test_ic3_isig_dynamic_unsafe() {
    let circuit = parse_aag("aag 1 0 1 0 0 1\n2 3\n2\n").unwrap();
    let config = Ic3Config {
        internal_signals: true,
        dynamic: true,
    ...
    assert!(
        matches!(result, Ic3Result::Unsafe { .. }),
        "ic3_isig_dynamic should find bugs, got {result:?}"
    );
}
...
/// Uses SimpleSolver to avoid ay-sat crash bugs.
...
";
    ...
    let _cube = [Lit::pos(Var(2))]; // v2 = true
...
fn test_shift_register_4deep_simple_solver() {
    // First-draft encoding (kept for reference) was incorrect:
    //   aag 9 1 4 0 4 1 / 2 / 4 2 / 6 4 / 8 6 / 10 8 / 18 /
    //   12 4 6 / 14 12 8 / 16 14 10 / 18 16 10
    // v1=input, v2..v5=latches, AND: v6=v2&v3, v7=v6&v4, v8=v7&v5, bad=v9=v8&v5
    // Re-done below with correct AIGER encoding.
...
// -----------------------------------------------------------------------
// Constrained shift-register regression (originally added with #4151)
// -----------------------------------------------------------------------

/// 3-deep constrained shift register must be proven Safe.
///
/// Circuit: input -> v2 -> v3 -> v4. Bad = v2 AND v3 AND v4.
/// Init: all latches 0, and the constraint pins the input to 0, so the
/// initial 0 propagates through the pipeline forever and the 3-way AND is
/// never satisfied. Exercises constraint handling through the full
/// block/generalize pipeline on a multi-frame proof.
#[test]
fn test_constrained_shift_register_3deep_safe() {
...
";
    let circuit = parse_aag(aag).unwrap();
    let result = check_ic3_with_config(&circuit, Ic3Config::default());
    assert!(
        matches!(result, Ic3Result::Safe { .. }),
        "3-deep constrained shift register should be Safe, got {result:?}"
    );
}
...
/// Verify ay-sat bucket-queue VSIDS is activated on an IC3 frame-solver
/// domain query. TLA observes this at the adapter boundary: the frame solver
/// is in IC3 mode, `set_domain()` is issued for the consecution query, and
/// the easy query does not cross ay-sat's heap-fallback restart threshold.
#[test]
fn test_ic3_frame_solver_records_bucket_queue_vsids_domain_query() {
    let mut aag = String::from("aag 20 0 20 0 0 1\n");
    for latch in 1..=20 {
    ...
    let cube = vec![crate::sat_types::Lit::pos(crate::sat_types::Var(1))];
    let _ = engine.is_inductive(1, &cube);

    let stats = engine.solvers[0].ic3_bucket_queue_vsids_stats();
    assert_eq!(
        stats.domain_sets, 1,
        "IC3 frame solver should call set_domain for the consecution query"
    );
    assert_eq!(
        stats.domain_queries, 1,
        "IC3 frame solver should solve while the ay-sat domain is active"
    );
    assert_eq!(
        stats.heap_fallback_queries, 0,
        "trivial frame query should not fall back from bucket queue to heap"
    );
    assert_eq!(
        stats.max_query_restarts, 0,
        "trivial frame query should complete before any restart"
    );
}

/// Benchmark-backed variant of the frame-solver observation for #4343.
///
/// This intentionally performs one cal14 frame query, not a broad HWMCC
/// sweep. The test is fixture-guarded so CI without local HWMCC data skips.
#[test]
fn test_cal14_ic3_frame_solver_records_bucket_queue_vsids_domain_query() {
    // cal14.aig under the HWMCC benchmark root: `$HWMCC_BENCHMARKS` if set,
    // else `$HOME/hwmcc/benchmarks`.
    let root = match std::env::var_os("HWMCC_BENCHMARKS") {
        Some(env) if !env.is_empty() => std::path::PathBuf::from(env),
        _ => std::env::var_os("HOME")
            .map(std::path::PathBuf::from)
            .unwrap_or_else(|| std::path::PathBuf::from("."))
            .join("hwmcc/benchmarks"),
    };
    let path = root.join("bitlevel/safety/2019/goel/industry/cal14/cal14.aig");
    ...
    let circuit = crate::parse_file(path).expect("parse cal14.aig");
    let ts = crate::transys::Transys::from_aiger(&circuit);
    assert!(
        ts.latch_vars.len() >= 20,
        "cal14 should exercise IC3 domain restriction"
    );

    let latch_set: rustc_hash::FxHashSet<crate::sat_types::Var> =
        ts.latch_vars.iter().copied().collect();
    let Some(init_lit) = ts
        .init_clauses
        .iter()
        .filter_map(|clause| {
            clause
                ...
                .first()
                .copied()
                .filter(|_| clause.lits.len() == 1)
        })
        .find(|lit| latch_set.contains(&lit.var()))
    else {
        eprintln!("skip: cal14 has no unit latch init clause for frame-query probe");
        return;
    };
    ...
    let cube = vec![!init_lit];
    let _ = engine.is_inductive(1, &cube);

    let stats = engine.solvers[0].ic3_bucket_queue_vsids_stats();
    assert_eq!(
        stats.domain_sets, 1,
        "cal14 IC3 frame solver should call set_domain for the consecution query"
    );
    assert_eq!(
        stats.domain_queries, 1,
        "cal14 IC3 frame solver should solve while the ay-sat domain is active"
    );
    assert_eq!(
        stats.heap_fallback_queries, 0,
        "cal14 one-query probe should not fall back from bucket queue to heap"
    );
    assert_eq!(
        stats.max_query_restarts, 0,
        "cal14 one-query probe should complete before any restart"
    );
}

/// Test verify_lemma_consecution refutes a genuinely non-inductive cube and
/// hands back a usable predecessor witness.
///
/// Consecution at frame k is RELATIVE to F_{k-1}, and F_0 = Init (#4560):
/// on the 3-deep shift register (latches v2 <- input, v3 <- v2, v4 <- v3,
/// all initialized to 0):
///
/// - `{v2}` at frame 1 is refutable: from the (init) state v2=v3=v4=0, input=1
///   drives v2'=1, so F_0 ∧ T ∧ ¬v2 ∧ v2' is SAT.
/// - `{v4}` at frame 1 is VALID (v4'=v3=0 in every init state — v4 needs
///   3 steps to rise), but at frame 2 with no frame lemmas F_1 is
///   unconstrained, so a state with v3=1 refutes it.
#[test]
...
";
    ...
    engine.extend(); // frame 2

    let v2 = crate::sat_types::Lit::pos(crate::sat_types::Var(2));
    let cube_a = vec![v2];
    match engine.verify_lemma_consecution(1, &cube_a) {
        validate::LemmaVerdict::Refuted { predecessor } => {
            // The witness is a state satisfying ¬cube: v2 must be false in it.
            assert!(
                predecessor.contains(&!v2),
                "refutation witness must satisfy ¬{{v2}}, got {predecessor:?}"
            );
        }
        validate::LemmaVerdict::Verified => {
            panic!("cube {{v2}} should NOT pass consecution at frame 1 (input drives v2')")
        }
    }
    ...
    assert!(
        matches!(
            engine.verify_lemma_consecution(1, &cube_c),
            validate::LemmaVerdict::Verified
        ),
        "cube {{v4}} IS relatively inductive at frame 1: F_0 = Init forces v4'=v3=0"
    );
    assert!(
        matches!(
            engine.verify_lemma_consecution(2, &cube_c),
            validate::LemmaVerdict::Refuted { .. }
        ),
        "cube {{v4}} should NOT pass consecution at frame 2: F_1 has no lemmas, \
         so a v3=1 state reaches v4'=1"
    );
}
...
fn test_verify_lemma_consecution_accepts_inductive() {
    ...
    assert!(
        matches!(
            engine.verify_lemma_consecution(1, &cube_latch_true),
            validate::LemmaVerdict::Verified
        ),
...
";
    ...
    let ts2 = crate::transys::Transys::from_aiger(&circuit2);
    let mut engine_ay = Ic3Engine::new(ts2);
    let result_ay = engine_ay.check();
    assert!(
        matches!(result_ay, Ic3Result::Unsafe { .. }),
        "3-deep shift register should be Unsafe with AYSat, got {result_ay:?}"
    );

    let stats = &engine_ay.consecution_stats;
    assert!(
...
";
    ...
    // Use SimpleSolver to avoid ay-sat bugs.
    let mut engine = Ic3Engine::new(ts).with_simple_solver();
    let result = engine.check();
    assert!(
        matches!(result, Ic3Result::Safe { .. }),
...
fn test_push_generalize_preserves_safe_on_stuck_at_zero() {
    ...
    let push_gen_cfg = Ic3Config {
        push_generalize: true,
    ...
    let zero_budget_cfg = Ic3Config {
        push_generalize: true,
        push_generalize_budget: 0,
        ..Ic3Config::default()
    };
...
///      break path: blocking budget, repeated cube #4139, cancellation).
///      The shortcut MUST NOT fire and `propagate()` MUST
...
fn test_propagate_convergence_proof_gates_shortcut_4317() {
    ...
    // blocking loop (budget, #4139 repeat, cancellation). The
