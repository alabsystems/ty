...
/// (budget, repeated-cube #4139, cancellation) MUST pass
...
impl Ic3Engine {
    ...
    pub(super) fn push_lemma(&mut self, frame: usize, mut cube: Vec<Lit>) -> (usize, Vec<Lit>) {
        ...
        for f in (frame + 1)..=depth {
            ...
            // additional propagation. Cf. Bradley's `pushForward` in IC3ref.
            //
    ...
    pub(super) fn propagation_blocked(&mut self, frame: usize, cube: &[Lit]) -> Option<Vec<Lit>> {
        ...
        // Set domain on the full frame solver for ay-sat native domain BCP.
        ...
        let domain_vars: Vec<Var> = (0..=self.max_var)
            ...
            .map(Var)
            .collect();
        // small_circuit_mode (#4259, ay#8802): skip set_domain so ay-sat uses
    ...
    /// repeated-cube limit #4139, cooperative cancellation) — in those
    /// cases the top frame may still admit reachable
    ...
    pub(super) fn propagate(
        ...
        let depth = self.frames.depth();
        let _blocking_completed_naturally = blocking_witness.is_some();
        if depth < 2 {
        ...
        for k in 1..depth - 1 {
            ...
            for (idx, lemma) in lemmas.iter().enumerate() {
                ...
                for _ctp_attempt in 0..ctp_max {
                    ...
                    if !self.block_ctg_chain(k, pred, &mut tb_limit) {
                        break;
                    }
                }
                if !succeeded {
            ...
            if self.frames.frames[k].lemmas.is_empty() && orig_count > 0 {
                ...
                // path (budget, #4139 repeated cube), F_top may still
        ...
        // each trivially solvable, but TY timed out at 100s+.
        //
        ...
        // cancellation), some reachable bad cube at F_top
    ...
    pub(super) fn propagate_to_inf(&mut self) {
        ...
        // small_circuit_mode (#4259, ay#8802): skip set_domain on inf_solver so
        // ay-sat uses search_propagate_standard (plain BCP).
        ...
        for (idx, lemma) in lemmas.iter().enumerate() {
            ...
            // Activate ay-sat native domain restriction for the inf solver.
            if use_domain {
