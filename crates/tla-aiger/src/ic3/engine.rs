...
use super::config::{
    Ic3Config, RestartStrategy, MAX_SOLVER_REBUILDS_PER_FRAME, PANIC_FALLBACK_THRESHOLD,
    SOLVER_REBUILD_BASE,
};
...
use super::predprop;
use super::vsids;
use crate::sat_types::{Lit, SatSolver, SimpleSolver, SolverBackend, Var};
...
pub struct Ic3Engine {
    ...
    /// Reference: the GipSAT bucket-VSIDS concept (arXiv:2502.13605).
    ...
    /// tla-aiger's realization of COI-restricted SAT (published for GipSAT
    /// in arXiv:2502.13605 §3):
    /// since ay-sat doesn't support internal domain restriction, we achieve
    ...
    ///
    /// When ay-sat produces FINALIZE_SAT_FAIL (InvalidSatModel), the solver
    ...
    /// automatically switches to `AYNoPreprocess` and rebuilds all solvers.
    ///
    ...
    /// Whether the engine has already fallen back to AYNoPreprocess due to
    ...
    ///
    /// This shortcut avoids redundant
    ...
    /// `verify_consecution_independent` has disagreed with ay-sat at each frame.
    /// When a frame exceeds `MAX_CROSSCHECK_FAILURES`, the cross-check is
    /// disabled for that frame to break the infinite rebuild loop where ay-sat
    ...
    /// patterns where ay-sat produces false UNSAT across many different frames,
    ...
    /// Count of ay-sat panics caught during this IC3 run (#4092).
    ///
    /// ay-sat has known bugs in conflict analysis / backtrack that cause panics
    ...
    /// constraint-dense circuits like microban_1 where ay-sat repeatedly
    ...
    /// visibility into ay-sat's consecution accuracy without requiring
    /// IC3_DEBUG mode.
    pub(super) consecution_stats: ConsecutionStats,
    /// Current nesting depth of CTG generalization (#4288 TL1f).
    ///
    /// `block_ctg_chain` generalizes every lemma it learns via `mic()`, and
    /// that MIC may itself want to discharge CTGs — which would learn more
    /// lemmas, generalize those, and so on. This counter tracks how deeply
    /// that has nested; the MIC variants consult it and stop opening new CTG
    /// work once it reaches `MAX_CTG_RECURSION`, degrading to the plain
    /// drop-literal MIC instead. Incremented by `block_ctg_chain` around its
    /// `mic()` call, decremented on return.
    pub(super) ctg_recursion_depth: usize,
}

/// Nesting cap for CTG generalization inside `block_ctg_chain`.
///
/// Depth 0 is the outermost MIC, with CTG fully enabled. A cap of 1 permits
/// exactly one nested level: lemmas learned while discharging a CTG are
/// themselves CTG-generalized — the primary win on clause-heavy industrial
/// UNSAT circuits such as the cal family (#4288) — but generalization below
/// that level runs without CTG. Each additional level would multiply SAT
/// work roughly geometrically for diminishing extra strengthening.
...
/// rates indicate ay-sat is producing false UNSAT on the circuit.
...
impl Ic3Engine {
    ...
    pub fn with_config(ts: Transys, config: Ic3Config) -> Self {
        ...
        if config.circuit_adapt {
            ...
            if num_latches < 30 {
                ...
                // recursion (5 * 12 = 60 attempts per literal drop) dominates
                // runtime without improving generalization quality, so cap
                // both knobs at ty's conservative baseline (ctg_max=3,
                // ctg_limit=1).
                config.ctg_max = config.ctg_max.min(3);
                config.ctg_limit = config.ctg_limit.min(1);
            } else if num_latches < 100 {
                // Small-medium circuits: aggressive CTG (deep recursion is
                // cheap). 12 is ty's raised CTG budget — see the
                // `Ic3Config::ctg_limit` doc for the derivation.
                config.ctg_max = config.ctg_max.max(5);
                config.ctg_limit = config.ctg_limit.max(12);
        ...
        // Auto-enable small_circuit_mode on small circuits (#4259, ay#8802).
        //
        // When `circuit_adapt` is on and `num_latches < 50`, enable
        // `small_circuit_mode` unless the caller has already set it. This
        // routes ay-sat through `search_propagate_standard` (plain BCP) on
        ...
        // (COI-restricted SAT, arXiv:2502.13605 §3).
        let domain_computer = DomainComputer::new(&ts, &next_vars, max_var);

        // Initialize bucket-queue VSIDS with seed-based perturbation — ty's
        // portfolio-diversity axis: different seeds produce different MIC
        // literal orderings, leading to complementary generalization paths.
        // Reference: the GipSAT bucket-VSIDS concept (arXiv:2502.13605).
    ...
    /// lift solver) so ay-sat's CDCL loop can exit promptly when cancelled (#4057).
    ...
    /// and BVE/BCE can corrupt incremental state. An IC3-oriented SAT core
    /// wants a minimal CDCL loop with no inprocessing — we get that shape
    /// by calling `set_ic3_mode()` on the underlying ay-sat solver, which
    ...
    ///
    /// This is the single hottest per-query overhead lever in ay-sat for
    ...
    pub(super) fn make_solver(&self) -> Box<dyn SatSolver> {
        ...
        Self::apply_restart_strategy(solver.as_mut(), self.config.restart_strategy);
        // Wire cancellation flag into ay-sat's interrupt mechanism (#4057).
        // When the portfolio sets cancelled=true, ay-sat's internal CDCL loop
        // detects it via is_interrupted() and returns Unknown promptly.
        solver.set_cancelled(self.cancelled.clone());
        solver
    }

    pub(super) fn apply_restart_strategy(solver: &mut dyn SatSolver, strategy: RestartStrategy) {
        match strategy {
            RestartStrategy::Default => {}
            RestartStrategy::Luby { unit } => {
                let unit = u64::try_from(unit.max(1)).unwrap_or(u64::MAX);
                solver.set_luby_restarts(unit);
            }
            RestartStrategy::Geometric { base, factor } => {
                let initial = base.max(1) as f64;
                let factor = if factor.is_finite() && factor > 1.0 {
                    factor
                } else {
                    1.5
                };
                solver.set_geometric_restarts(initial, factor);
            }
        }
    }
    ...
    /// against ay-sat. The original false UNSAT issue was caused by MIC
    /// over-generalization producing init-inconsistent cubes at the IC3
    /// algorithm level, not by ay-sat's SAT solving. SimpleSolver serves
    ...
    /// use AYNoPreprocess (disables BVE/subsumption) which is significantly faster
    ...
    pub(super) fn make_fast_validation_solver(&self) -> Box<dyn SatSolver> {
        if self.ts.latch_vars.len() <= 60 {
            ...
            SolverBackend::AYNoPreprocess.make_solver_no_inprocessing(self.max_var + 1)
        }
    }
    ...
    ///
    /// This is the core of #4062: clone the infinity solver when
    /// extending to a new frame, carrying forward the transition
    ...
    ///
    /// For AYSatCdclSolver, cloning replays the clause_log into a fresh solver
    /// (since ay-sat doesn't derive Clone). For SimpleSolver, it does a direct
    ...
    ///
    /// Even with push/pop (#4016), ay-sat's internal data structures may
    ...
    ///
    /// When native incremental cloning is available (ay-sat `clone_for_incremental`),
    ...
    ///
    /// A full rebuild is more aggressive than periodic learned-clause
    /// pruning; ty rebuilds because
    /// ay-sat doesn't expose a learned-clause pruning API.
    ...
    ///
    /// Used to recover from solver poisoning: when a ay-sat panic is caught
    ...
    ///
    /// This is the key recovery mechanism for #4040: ay-sat panics during
    ...
    pub(super) fn rebuild_solver_at(&mut self, idx: usize) {
        ...
        // Track whether this rebuild is due to a ay-sat panic (#4092).
    ...
    /// Handle a ay-sat panic by tracking it and triggering fallback (#4092).
    ///
    /// Called after `rebuild_solver_at` when the rebuild was due to a poisoned
    /// solver (ay-sat panic). Increments the panic counter and, when the
    ...
    pub(super) fn handle_solver_panic(&mut self) -> bool {
            ...
            && self.solver_backend != SolverBackend::Simple
        {
            ...
            eprintln!(
                "IC3: ay-sat panic #{} — falling back to SimpleSolver and purging \
            ...
            // SimpleSolver. This is aggressive but necessary: the ay-sat
    ...
    /// Fall back to a simpler solver backend when ay-sat produces persistent
    /// FINALIZE_SAT_FAIL (InvalidSatModel) errors (#4074).
    ///
    /// ay-sat's CDCL solver has a known model reconstruction bug that causes
    ...
    ///
    /// Two-stage fallback:
    /// 1. First attempt: AYNoPreprocess (disables BVE/subsumption). This
    ///    avoids BVE-specific model reconstruction bugs.
    /// 2. Second attempt: SimpleSolver (basic DPLL). This avoids ALL ay-sat
    ...
    pub(super) fn fallback_solver_backend(&mut self) {
        if !self.config.unknown_fallback_enabled {
            if std::env::var("IC3_DEBUG").is_ok() {
                eprintln!(
                    "IC3: Unknown fallback disabled by config; keeping {:?} after {} \
                     consecutive Unknown results (#4493)",
                    self.solver_backend, self.unknown_count,
                );
            }
            self.unknown_count = 0;
            return;
        ...
        if !self.fell_back_to_no_preprocess {
            ...
            eprintln!(
                "IC3: falling back to AYNoPreprocess due to {} consecutive Unknown results \
                 (FINALIZE_SAT_FAIL from ay-sat)",
                self.unknown_count,
            );
            self.solver_backend = SolverBackend::AYNoPreprocess;
            self.fell_back_to_no_preprocess = true;
        } else {
            // Stage 2: AYNoPreprocess also failed. Fall back to SimpleSolver.
...
#[cfg(test)]
mod tests {
    use super::super::config::UNKNOWN_FALLBACK_THRESHOLD;
    use super::*;
    use crate::parser::parse_aag;
    use crate::sat_types::SatResult;

    fn small_safe_engine(config: Ic3Config) -> Ic3Engine {
    ...
    struct RecordingSolver {
        luby_unit: Option<u64>,
        geometric: Option<(f64, f64)>,
    }
    ...
    impl SatSolver for RecordingSolver {
        ...
        fn set_luby_restarts(&mut self, unit: u64) {
            self.luby_unit = Some(unit);
        }

        fn set_geometric_restarts(&mut self, initial: f64, factor: f64) {
            self.geometric = Some((initial, factor));
        }
    }

    #[test]
    fn test_apply_restart_strategy_luby_forwards_unit() {
        let mut solver = RecordingSolver::default();

        Ic3Engine::apply_restart_strategy(&mut solver, RestartStrategy::Luby { unit: 512 });

        assert_eq!(solver.luby_unit, Some(512));
        assert_eq!(solver.geometric, None);
    }

    #[test]
    fn test_apply_restart_strategy_geometric_forwards_parameters() {
        let mut solver = RecordingSolver::default();

        Ic3Engine::apply_restart_strategy(
            &mut solver,
        ...
        assert_eq!(solver.luby_unit, None);
        assert_eq!(solver.geometric, Some((100.0, 1.5)));
    }

    #[test]
    fn test_apply_restart_strategy_clamps_invalid_values() {
        let mut solver = RecordingSolver::default();
        ...
        assert_eq!(solver.geometric, Some((1.0, 1.5)));

        Ic3Engine::apply_restart_strategy(&mut solver, RestartStrategy::Luby { unit: 0 });

        assert_eq!(solver.luby_unit, Some(1));
    }

    #[test]
    fn test_unknown_fallback_disabled_keeps_configured_backend() {
        let config = Ic3Config {
            solver_backend: SolverBackend::AYSat,
            unknown_fallback_enabled: false,
            ..Ic3Config::default()
        };
        let mut engine = small_safe_engine(config);
        engine.unknown_count = UNKNOWN_FALLBACK_THRESHOLD;

        engine.fallback_solver_backend();

        assert_eq!(engine.solver_backend, SolverBackend::AYSat);
        assert_eq!(engine.unknown_count, 0);
        assert!(!engine.fell_back_to_no_preprocess);
    }

    #[test]
    fn test_unknown_fallback_default_escalates_two_stage_backend() {
        let config = Ic3Config {
            solver_backend: SolverBackend::AYSat,
            ..Ic3Config::default()
        };
        ...
        engine.unknown_count = UNKNOWN_FALLBACK_THRESHOLD;
        engine.fallback_solver_backend();
        assert_eq!(engine.solver_backend, SolverBackend::AYNoPreprocess);
        assert_eq!(engine.unknown_count, 0);
        assert!(engine.fell_back_to_no_preprocess);

        engine.unknown_count = UNKNOWN_FALLBACK_THRESHOLD;
        engine.fallback_solver_backend();
        assert_eq!(engine.solver_backend, SolverBackend::Simple);
        assert_eq!(engine.unknown_count, 0);
        assert!(engine.fell_back_to_no_preprocess);
    }
}
