...
//!
//! Domain-restricted SAT: each IC3 query restricts the SAT solver to
//! cone-of-influence (COI) variables only, so each SAT call only processes
//! relevant variables (the COI-restricted BCP technique published for the
//! GipSAT solver in arXiv:2502.13605 §3).
//!
...
//! 2. **ay-sat native domain BCP** (`set_domain()`): within the mini-solver,
//!    ay-sat's `propagate_domain_bcp()` further restricts watcher propagation
//!    and VSIDS branching to domain variables only.
//!
//! The combination: domain-restricted mini-solver
//! (clause subset) + ay-sat domain BCP (watcher filtering). Used in MIC
...
//!    domain restriction is always enabled. The old
...
//!    clause-filtered mini-solver and ay-sat's domain BCP both help.
//!
//! Reference: GipSAT's domain-restricted solving (arXiv:2502.13605); the
//! domain is traced through the AND-gate dependency graph.
...
impl DomainComputer {
    ...
    /// - Pre-computed constraint variable COIs
    ///
    /// Reference: GipSAT domain restriction (arXiv:2502.13605) — the bad
    /// query runs on a domain seeded from the bad literal.
    ...
    pub(crate) fn verify_domain_completeness(
        ...
        for &lit in cube {
            ...
            let enqueue = |v: Var, visited: &mut Vec<bool>, worklist: &mut Vec<Var>| {
                let idx = v.0 as usize;
                if idx < visited.len() && !visited[idx] && v != Var(0) {
                    visited[idx] = true;
                    worklist.push(v);
...
/// activation. ay-sat's native domain BCP (set_domain) has near-zero
...
// latches whose transitive COI covers >95% of variables, so a threshold
// mostly disabled the optimization. Always-on is strictly better here:
// the clause-filtered
...
pub(crate) fn build_domain_restricted_solver(
    ...
    // Coverage check removed (#4091 validation): domain restriction is
    // always enabled regardless of coverage ratio. Even at 100%
    // coverage, the clause-filtered mini-solver still helps by providing
    // a clean solver without accumulated learned clauses, and ay-sat's
    ...
    // IC3 domain-restricted solvers use full IC3/PDR mode so ay-sat's
    // domain-restricted dispatch reaches propagate_bcp_ic3.
    let mut solver = solver_backend.make_solver_ic3_mode(max_var + 1);
    ...
    // Wire ay-sat's native domain restriction (#4091).
    //
    // In addition to clause filtering (only adding domain-relevant clauses to
    // the solver), activate ay-sat's internal domain-restricted BCP. This is
    // a two-layer optimization:
    // 1. Clause filtering: the solver has fewer clauses to process.
    // 2. Domain BCP: within those clauses, ay-sat's `propagate_domain_bcp()`
    //    skips non-domain watchers, further reducing per-call overhead.
    //
    // The combination: domain-restricted mini-solver
    // (clause subset) + ay-sat domain BCP (watcher filtering).
    //
...
mod tests {
    ...
    #[test]
    fn test_domain_restricted_ay_solver_uses_ic3_mode() {
        let circuit = parse_aag("aag 3 2 0 0 1 1\n2\n4\n6\n6 2 2\n").expect("parse failed");
        let ts = Transys::from_aiger(&circuit);
        ...
        .expect("domain-restricted solver should be built for non-trivial circuits");

        assert!(
            solver.is_ic3_mode(),
            "domain-restricted ay solver must enable IC3 mode"
        );
    }
    ...
    fn test_build_domain_restricted_solver_with_frames() {
        ...
        // Coverage check removed: domain restriction is always enabled.
        let solver = build_domain_restricted_solver(
            &domain,
            &ts,
            &[],
    ...
    fn test_domain_restriction_activates_at_full_coverage() {
        ...
        // This is the key fix (#4091 validation): the previous 95% threshold
        // prevented activation on most benchmarks.
        let circuit = parse_aag("aag 3 1 1 0 1 1\n2\n4 6\n4\n6 2 4\n").expect("parse failed");
        let ts = Transys::from_aiger(&circuit);
        ...
        // accumulated learned clauses, and ay-sat's domain BCP still helps.
        assert!(
            solver.is_some(),
            "domain restriction must activate at 100% coverage"
        );
    }
    ...
    //
    // Per-query COI limiting (cf. arXiv:2502.13605 §3):
    //   reset -> seed with input cube vars -> worklist-expand through the
    //   AND-gate dependency graph.
    //
    ...
    fn test_audit_criterion_c_empty_cube_produces_minimal_domain() {
        ...
        //
        // Per-query domain behavior on an empty seed set: after a
        // reset, only the fixed vars remain (CONST); the worklist
