...
impl Ic3Engine {
    ...
    pub fn check(&mut self) -> Ic3Result {
        ...
        // infinite loops where an early-out path (obligation-depth cap,
        // Unknown-requeue cap, spurious-init-pred loop breaker #4105) leaves a
        // bad state unblocked, causing get_bad() to rediscover the same cube
        ...
        loop {
            ...
            // bad cube is returned by get_bad(). When an early-out path leaves
            // a proof obligation unresolved without blocking the bad state,
            // get_bad() rediscovers the same cube, creating an infinite loop.
            // If any cube is seen more than MAX_BAD_CUBE_REPEATS times, advance
            // to the next depth instead of looping forever.
            ...
            // Early-out paths (blocking_budget, #4139 repeated cube,
            ...
            if matches!(
                ...
                //
                // An IC3 implementation may soundly trust the algorithm's
                // incremental per-lemma consecution checks. Our
                ...
                //   SimpleSolver is fast enough here and catches ay-sat false UNSAT.
                // - Medium circuits (80-200 latches): SOUND budgeted validation
                //   (validate_invariant_budgeted uses AYNoPreprocess in this tier and
                //   correctly primes internal signals via next_link_clauses + next_vars,
                //   engine.rs:350-399 / validate.rs:211-242). Previously this tier
                //   returned Some(true) WITHOUT any inductiveness check, pushing the
                //   entire soundness burden onto the portfolio-level validate_safe —
                //   which mis-primed internal-signal lemmas in this exact latch band
                //   and could ACCEPT a non-inductive invariant => false SAFE. We no
                //   longer skip: on budget exhaustion validate_invariant_budgeted
                //   returns None (handled below as "continue IC3 / unproven"), so a
                //   Safe verdict in this band is NEVER returned unvalidated.
                // - Large circuits (>200 latches): skip validation entirely. These
                //   circuits are far beyond SimpleSolver's capability and even
                //   AYNoPreprocess validation can timeout, wasting portfolio time;
                //   the (now internal-signal-aware) portfolio validate_safe is the
                //   gate for this band.
                //
                ...
                let validation_result = if num_latches > 200 {
                    ...
                    // Small/medium circuit (<=200 latches): sound budgeted validation.
                    // validate_invariant_budgeted scales its budget and solver tier with
                    // latch count and primes internal signals soundly; on budget-out it
                    // returns None (continue IC3), never a false Some(true).
                ...
                match validation_result {
                    ...
                    Some(false) => {
                        // Invariant validation failed: ay-sat produced false UNSAT
                        ...
                        // avoid infinite loops where ay-sat is systematically unsound
                        // on this circuit.
    ...
    ///
    /// On frame extension we clone a solver holding only Trans +
    ...
    pub(super) fn extend(&mut self) {
        ...
        // Rebuild predprop solver on frame extension (#4101), with all
        // infinity lemmas included.
    ...
    pub(super) fn get_bad(&mut self) -> Option<GetBadResult> {
        ...
        //
        // TL168 wired ay-sat native domain BCP on 3 IC3 hot paths
        ...
        // that COI gives the same speedup as the other paths (COI-restricted
        // BCP per arXiv:2502.13605 §3).
        //
...
pub fn check_ic3_with_config(circuit: &crate::types::AigerCircuit, config: Ic3Config) -> Ic3Result {
    ...
    // practice in competitive model checkers (e.g. ABC does "BMC then IC3",
    // and portfolio checkers race BMC alongside IC3).
    //
