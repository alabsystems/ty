...
/// Default restart count after which the bucket queue permanently yields to
/// the binary heap.
///
/// ty engineering choice. The bucket queue's edge is constant factors on the
/// short queries that dominate early IC3 runs; restarts are the engine's
/// signal that queries have grown long enough for restart policies to
/// trigger, at which point exact heap ordering is worth O(log n). The switch
/// is one-way and both modes are correct, so only the order of magnitude
/// matters — ty picks 8 (2^3, "first handful of restarts") as an untuned
/// round default; `bucket_queue_restarts` overrides it per config.
const DEFAULT_SWITCH_TO_HEAP_AFTER_RESTARTS: usize = 8;
...
pub(crate) struct VsidsConfig {
    ...
    /// restarts. A value of `0` starts directly in heap mode. See
    /// [`DEFAULT_SWITCH_TO_HEAP_AFTER_RESTARTS`] for the default's rationale.
...
impl Default for VsidsConfig {
    fn default() -> Self {
        Self {
            decay: DEFAULT_DECAY,
            switch_to_heap_after_restarts: DEFAULT_SWITCH_TO_HEAP_AFTER_RESTARTS,
            random_seed: 0,
...
impl BucketQueueVsids {
    ...
    ///
    /// This is the ascending activity sort used by forward-iteration
    /// MIC paths (activity-guided drop order). With
