...
pub struct ProofObligation {
    ...
    /// Activity score: incremented by one each time blocking this
    /// obligation fails (a predecessor is found and the obligation goes
    /// back on the queue). High activity means the cube keeps resisting
    /// blocking. Its sole consumer is dynamic generalization (GAP-5): when
    /// a predecessor spawned from this obligation is finally blocked, the
    /// predecessor's generalization effort is selected from this
    /// obligation's activity (activity-threshold scheme published in
    /// arXiv:2501.02480 Alg. 5). Obligations are always reprocessed —
    /// activity never causes one to be discarded.
    pub act: f64,
...
impl ProofObligation {
    /// Create a proof obligation: block `cube` at `frame`, reached at trace
    /// `depth`, optionally chained to `next` (its successor toward the bad state).
    ...
    /// Record a failed blocking attempt: a predecessor of this obligation
    /// was found, so the cube survives another round. The +1-per-failure
    /// count is what the dynamic-strategy activity thresholds of
    /// arXiv:2501.02480 (Alg. 5; values from its Sec. V-A) are stated over.
    pub fn bump_act(&mut self) {
        self.act += 1.0;
    }
}

/// Two obligations are the same queue entry iff frame, depth, and cube all
/// coincide. Must agree with `Ord` below (`cmp == Equal` exactly here).
...
/// Priority order for the obligation queue.
///
/// The queue is a `BTreeSet` consumed with `pop_last()`, so "greater" means
/// "dispatched sooner". The first two keys are the ones the IC3/PDR
/// algorithm itself dictates:
///
/// - **Frame, ascending.** An obligation only ever spawns further work at
///   lower frames (its predecessors), so the lowest-frame obligation is the
///   tip of the predecessor chain currently under construction. It must be
///   resolved first — either it touches the initial states (a real
///   counterexample) or blocking it is what lets the obligations above it
///   proceed.
/// - **Depth, descending.** Within a frame, keep extending the longest
///   predecessor chain before starting siblings: that completes a genuine
///   counterexample (or refutes a spurious one) soonest.
///
/// The algorithm forces nothing further. The last key — lexicographic
/// comparison of the literal codes — carries no heuristic; it only makes
/// the order total and consistent with `Eq`, so the `BTreeSet` can
/// deduplicate re-discovered obligations.
...
impl ObligationQueue {
    /// Create an empty obligation queue.
    pub fn new() -> Self {
        Self::default()
    }
