...
fn count_cone_gates(
    ...
    count_cone_gates_inner(root, root, &leaf_vars, and_defs, fanout, &mut visited)
}
...
fn count_cone_gates_inner(
    ...
    // The root is always replaced by this rewrite attempt, so it is counted
    // even when it is directly referenced by a bad/constraint/next literal.
    let fo = fanout.get(&var).copied().unwrap_or(0);
    let self_count = usize::from(var == root || fo <= 1);
    self_count
        + count_cone_gates_inner(root, rhs0.var(), leaf_vars, and_defs, fanout, visited)
        + count_cone_gates_inner(root, rhs1.var(), leaf_vars, and_defs, fanout, visited)
}
...
mod tests {
    ...
    #[test]
    fn test_dag_rewrite_does_not_credit_shared_chain_gates() {
        // Each redundant chain node is externally referenced, so replacing one
        // node cannot remove its shared children. The cone saving estimate must
        // not count those shared children as eliminated rewrite credit.
        for base in (0..64).map(|idx| idx * 10) {
            let a = Var(base + 1);
            let b = Var(base + 2);
            let g1 = Var(base + 3);
            let g2 = Var(base + 4);
            ...
            let fanout = compute_fanout(&ts);
            assert_eq!(fanout.get(&g1), Some(&2));
            assert_eq!(fanout.get(&g2), Some(&2));
            assert_eq!(fanout.get(&g3), Some(&2));
            assert_eq!(
                count_cone_gates(g4, &[Lit::pos(a), Lit::pos(b)], &ts.and_defs, &fanout),
                1,
                "shared child gates must not be counted for base {base}",
            );

            let (result, elim) = dag_rewrite(&ts);
            assert_eq!(
                elim, 0,
                "shared chain must not report false savings for base {base}"
            );
            assert_eq!(
                result.and_defs.len(),
                ts.and_defs.len(),
                "shared chain rewrite must not change gate count for base {base}",
            );
            assert_valid_ts(&result);
        }
    }
