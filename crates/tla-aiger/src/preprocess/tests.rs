...
#[test]
fn test_preprocess_full_starts_with_coi_reduction() {
    // Only `live` and `live_in` are in the bad-state cone. `dead`, `dead_in`,
    // and `dead_gate` are reachable from each other but irrelevant to
    // bad/constraints, so COI should remove them before later passes run.
    let live = Var(1);
    let dead = Var(2);
    let dead_in = Var(3);
    let dead_gate = Var(4);
    ...
    let mut next_state = FxHashMap::default();
    next_state.insert(live, Lit::pos(live_in));
    next_state.insert(dead, Lit::pos(dead_gate));

    let mut and_defs = FxHashMap::default();
    and_defs.insert(dead_gate, (Lit::pos(dead), Lit::pos(dead_in)));
    ...
    let ts = build_ts(
        ...
        vec![dead_in, live_in],
        next_state,
        vec![Clause::unit(Lit::neg(live)), Clause::unit(Lit::neg(dead))],
        vec![Lit::pos(live)],
        Vec::new(),
        and_defs,
    );
    ...
    assert_eq!(stats.orig_latches, 2);
    assert_eq!(
        stats.after_trivial_latches, 1,
        "COI should remove the dead latch before trivial simplification"
    );
    assert_eq!(stats.after_trivial_inputs, 1);
    assert_eq!(stats.after_trivial_gates, 0);
    assert_eq!(result.latch_vars, vec![Var(1)]);
    assert_eq!(result.input_vars, vec![Var(2)]);
    assert!(result.and_defs.is_empty());
    assert_eq!(result.bad_lits, vec![Lit::pos(Var(1))]);
}
