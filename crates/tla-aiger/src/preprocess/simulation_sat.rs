...
//!    initial states by solving init constraints with ay-sat, then forward-
...
//!
//! The combination — SAT-enumerated valid initial states, forward simulation
//! from those states, and budgeted SAT-based init equivalence checking —
//! keeps candidate quality high near the init states SCORR must verify.
...
/// ty choice: 4 — richer coverage of the init state space than a single
/// enumerated state, without excessive SAT overhead. (A deliberate
/// divergence: the reference implementation enumerates fewer.)
const SAT_INIT_STATES: usize = 4;

/// Number of forward simulation steps from each SAT-enumerated init state.
/// ty choice: 10 — enough transitions to separate latches that only diverge
/// a few cycles after init, while keeping simulation cost negligible.
const FORWARD_SIM_STEPS: usize = 10;

/// Enumerate valid initial states by solving the init constraints with ay-sat.
///
...
fn enumerate_init_states(ts: &Transys, max_states: usize) -> Vec<FxHashMap<Var, bool>> {
    ...
    let mut solver = AYSatCdclSolver::new(ts.max_var + 1);
...
///
/// Enumerates up to `SAT_INIT_STATES` valid initial states via ay-sat, then
