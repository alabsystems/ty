...
//! AIG synthesis preprocessing: iterative balance + rewrite + refactor.
//!
...
//!
//! We implement a simplified version: `balance; rewrite; refactor; strash`
//! iterated to fixpoint (or a maximum number of rounds). This captures the most
...
//! - **Refactor**: distributive factoring of small cones
...
pub(crate) struct SynthesisStats {
    ...
    pub refactor_eliminations: usize,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            "synthesis: {} rounds, gates {}->{}({:+}), depth {}->{}, \
             balance={} rewrite={} dag_rewrite={} refactor={} strash={}",
            self.rounds,
            self.orig_gates,
            self.final_gates,
...
pub(crate) fn aig_synthesis(ts: &Transys) -> (Transys, SynthesisStats) {
    aig_synthesis_configurable(ts, MAX_SYNTHESIS_ROUNDS, true)
}
...
pub(crate) fn aig_synthesis_configurable(
    ...
    for round in 0..max_rounds {
        ...
        // Phase 4: Refactor (distributive factoring).
        let (after_refactor, rf_elim) = if enable_refactor && after_dag_rw.and_defs.len() >= 3 {
            refactor(&after_dag_rw)
        } else {
            (after_dag_rw, 0)
        };
        stats.refactor_eliminations += rf_elim;

        // Phase 5: Structural hashing (merge duplicates).
        let after_strash = if bal_red > 0 || rw_elim > 0 || dag_rw_elim > 0 || rf_elim > 0 {
            let before_strash = after_refactor.and_defs.len();
            let result = structural_hash(&after_refactor);
