...
///
/// Increased from 1 to 100 (ty tuning): a generous ceiling lets cascading
/// latch equivalences resolve to fixpoint. Each round re-runs simulation +
/// SAT-based equivalence checking after the previous round's substitutions,
/// finding cascading latch equivalences. Early termination (0 new
/// equivalences found) makes higher round counts effectively free when
...
pub struct PreprocessConfig {
    ...
    /// exposed by prior eliminations. The default 100 is a generous
    /// ceiling for fixpoint convergence; the competition preset uses
    /// 1000. Early termination (0 new equivalences in a round) makes
    /// high round counts free when equivalences converge early.
    ...
    /// Enable bounded AIG refactoring (distributive factoring).
    ...
    /// 0 means use the default size-scaled budget (2-8s based on circuit
    /// size), which deliberately keeps FRTS a small fraction of a check run.
...
pub struct PreprocessStats {
    /// Latch count before preprocessing.
    pub orig_latches: usize,
    /// Input count before preprocessing.
    pub orig_inputs: usize,
    /// AND-gate count before preprocessing.
    pub orig_gates: usize,
    /// Transition-clause count before preprocessing.
    pub orig_trans_clauses: usize,
    /// Latch count after trivial simplification (constant folding).
    pub after_trivial_latches: usize,
    /// Input count after trivial simplification.
    pub after_trivial_inputs: usize,
    /// AND-gate count after trivial simplification.
    pub after_trivial_gates: usize,
    /// Latches removed by constant-latch elimination.
    pub const_eliminated_latches: usize,
    /// Latches merged away by sequential equivalence (SCORR).
    ...
    /// Signals removed by forward reduction (FRTS).
    pub frts_eliminated: usize,
    /// Variables removed by bounded variable elimination.
    pub bve_eliminated_vars: usize,
    /// Number of interleaved FRTS+BVE iterations run.
    pub frts_bve_iterations: usize,
    /// Gates removed by local AIG rewriting.
    pub rewrite_eliminated: usize,
    /// Gates removed by DAG-aware rewriting.
    pub dag_rewrite_eliminated: usize,
    /// Gates removed by refactoring (multi-input cut rewriting).
    pub refactor_eliminated: usize,
    /// Number of synthesis rounds run.
    pub synthesis_rounds: usize,
    /// Net AND-gate reduction attributable to synthesis.
    pub synthesis_gate_reduction: usize,
    /// Logic-depth reduction attributable to synthesis.
    pub synthesis_depth_reduction: usize,
    /// Constant signals discovered by ternary propagation.
    pub ternary_constants: usize,
    /// Number of latches eliminated by sequential ternary simulation.
    pub ternary_sim_eliminated: usize,
    /// Number of variables collapsed by the final compaction/renumber pass.
    pub renumber_compacted: usize,
    /// Latch count after preprocessing.
    pub final_latches: usize,
    /// Input count after preprocessing.
    pub final_inputs: usize,
    /// AND-gate count after preprocessing.
    pub final_gates: usize,
    /// Transition-clause count after preprocessing.
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            "preprocess: latches {}->{}->{} inputs {}->{}->{} gates {}->{}->{} \
             ...
             frts_bve_iters={} rewrite_elim={} dag_rewrite_elim={} refactor_elim={} \
