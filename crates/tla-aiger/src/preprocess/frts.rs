...
//! - Proven equivalences fed back to the solver as clauses
//! - Skips primary-input/primary-input pairs, which have no structural path
//! - Time-limited to avoid spending too long on large circuits
//! - Intermediate COI cleanup when many signals eliminated
...
/// (COI + BVE). Periodic cleanup keeps the working circuit small mid-pass;
/// the threshold is tuned low since our circuits tend to be smaller.
...
/// Representative map: for each variable, the simulation-derived
...
fn build_init_solver(ts: &Transys) -> Box<dyn SatSolver> {
    let mut solver = SolverBackend::AYSat.make_solver(ts.max_var + 1);
...
/// requiring the solver to rediscover the relationship.
#[inline]
...
///
/// Triggered at `INTERMEDIATE_CLEANUP_THRESHOLD` (2000) proven equivalences
/// so the working circuit shrinks mid-pass instead of only at iteration ends.
...
/// Run one FRTS iteration using map-based linear traversal.
///
...
/// - Feeds proven equivalences back to the solver
...
fn frts_one_iteration(ts: &Transys, deadline: Instant) -> (Transys, usize) {
    ...
    // Linear iteration: iterate through all variables in increasing
    ...
    for var_idx in 1..=max_var_idx {
        ...
        // Budget-limited SAT check: is var equivalent to rep_var (with
        // polarity)? We use two assumption-based calls.
        ...
        // Periodic intermediate cleanup once enough merges accumulate.
