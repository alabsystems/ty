...
//! - cone-of-influence reduction from bad/constraint literals
...
//! - bounded AIG refactoring (distributive factoring)
//! - AIG synthesis (iterative balance + rewrite + refactor + strash, ABC-style)
...
///
/// Thin wrapper over the authoritative `portfolio::factory::is_sat_likely` so
/// preprocessing can make pass-level decisions with the exact same predicate
/// (the structural cutoffs and prior-art citations #4247/#4259/#4149 live
/// there). Kept as a local alias to read naturally at the preprocess call site.
fn is_sat_likely_pattern(ts: &Transys) -> bool {
    crate::portfolio::factory::is_sat_likely(ts)
}

/// Run the full IC3 preprocessing pipeline with a given configuration.
///
/// Pipeline (ty's pass ordering; cheap structural passes first, SAT-backed
/// passes later, renumbering last):
/// 0. COI reduction (remove logic outside bad/constraint fanin)
/// 1. Ternary constant propagation (fast, no SAT -- circuit structure only)
/// 2. Trivial constant propagation (AND gate folding)
/// 3. Iterative constant latch elimination
/// 4. Structural hashing (merge duplicate AND gates)
/// 5. SCORR (sequential latch equivalence via init-seeded + random simulation + SAT)
...
/// 7. FRTS + BVE fixpoint loop (iterate FRTS -> strash -> BVE until no change)
/// 8. Local AIG rewriting (idempotent merge, associativity absorption)
/// 9. AIG synthesis (iterative balance + rewrite + refactor + strash -- ABC-style)
/// 10. Final structural hashing (clean up after synthesis)
/// 11. Variable renumbering (compact variable IDs to dense range)
pub fn preprocess_with_config(
    ...
    // Phase 0: COI reduction. HWMCC safety instances often carry monitor
    // logic outside the bad/constraint cone; removing it early shrinks every
    // later SAT-backed pass. If a synthetic caller has no property/constraint,
    // preserve the input to avoid reducing non-safety test circuits to empty.
    let after_coi = if ts.bad_lits.is_empty() && ts.constraint_lits.is_empty() {
        ts.clone()
    } else {
        ts.coi_reduce()
    };
    if config.verbose {
        eprintln!(
            "  preprocess phase 0 (coi): latches {}->{} inputs {}->{} gates {}->{}",
            ts.num_latches,
            after_coi.num_latches,
            ts.num_inputs,
            after_coi.num_inputs,
            ts.and_defs.len(),
            after_coi.and_defs.len(),
        );
    }

    // Phase 1: Ternary constant propagation (fast, SAT-free).
    ...
    let after_ternary = ternary_constant_propagation(&after_coi);
    let ternary_constants = after_coi
        .and_defs
        .len()
    ...
    // Phase 7c: AIG refactoring (bounded distributive factoring).
    let (after_refactor, refactor_eliminated) =
        if config.enable_refactor && after_dag_strash.and_defs.len() >= 3 {
            refactor(&after_dag_strash)
        } else {
            (after_dag_strash, 0)
        };
    if config.verbose {
        eprintln!(
            "  preprocess phase 7c (refactor): {} gates eliminated",
            refactor_eliminated
        );
    }

    // Phase 8: AIG synthesis (iterative balance + rewrite + strash).
    let pre_synth_gates = after_refactor.and_defs.len();
    let (after_synthesis, synth_stats) = if config.enable_synthesis {
        aig_synthesis_with_rounds(
            &after_refactor,
            config.synthesis_rounds.min(16),
            config.enable_refactor,
        )
    } else {
        (after_refactor, synthesis::SynthesisStats::default())
    };
    ...
    let after_strash2 = if rewrite_eliminated > 0
        || dag_rewrite_eliminated > 0
        || refactor_eliminated > 0
        || synthesis_gate_reduction > 0
    {
    ...
    let stats = PreprocessStats {
        ...
        rewrite_eliminated,
        dag_rewrite_eliminated,
        refactor_eliminated: refactor_eliminated + synth_stats.refactor_eliminations,
...
fn aig_synthesis_with_rounds(
    ...
    synthesis::aig_synthesis_configurable(ts, max_rounds, enable_refactor)
}
