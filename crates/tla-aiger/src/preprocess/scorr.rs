...
//! 3. **SAT-seeded simulation** — ay-sat enumerates valid init states,
...
//!
//! The pass combines init-state simulation seeding, forward simulation,
//! and budget-limited SAT verification.

use rustc_hash::FxHashMap;

use crate::sat_types::{AYSatCdclSolver, Lit, SatResult, SatSolver, SolverBackend, Var};
...
fn make_solver(ts: &Transys, backend: SolverBackend) -> Box<dyn SatSolver> {
    match backend {
        SolverBackend::AYSat => Box::new(AYSatCdclSolver::new(ts.max_var + 1)),
        _ => backend.make_solver(ts.max_var + 1),
    }
}

fn build_init_solver(ts: &Transys) -> Box<dyn SatSolver> {
    let mut solver = make_solver(ts, SolverBackend::AYSat);
...
/// most true equivalences are trivially provable. This keeps SCORR
...
/// 3. SAT-seeded simulation — ay-sat enumerates full init states and
...
/// check uses a conflict budget to keep
