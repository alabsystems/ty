...
pub(crate) fn promote_internal_signals_to_latches(ts: &Transys) -> Transys {
    ...
    //
    //    For simplicity, we allocate fresh vars for
    ...
    //
    // No explicit init clauses are added for promoted signals: the init
    // query conjoins Trans, which already binds each promoted gate to its
    // AND definition.
...
/// the AND-gate dependency graph (the input-fanout set):
