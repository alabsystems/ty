...
#[test]
fn test_aysat_cdcl_basic() {
    let mut s = AYSatCdclSolver::new(3);
...
#[test]
fn test_aysat_cdcl_incremental_fixed() {
    // ay#7987 fixed in ay ec8d049a5. Incremental add_clause between
    ...
    assert_eq!(
        r2,
        SatResult::Sat,
        "v2=true after adding !v1 should be SAT (ay#7987 fixed)"
    );
}

#[test]
fn test_aysat_cdcl_model() {
    let mut s = AYSatCdclSolver::new(3);
...
#[test]
fn test_solver_backend_aysat_incremental() {
    let mut s = SolverBackend::AYSat.make_solver(3);
    s.add_clause(&[Lit::TRUE]);
    s.add_clause(&[Lit::neg(Var(2)), Lit::neg(Var(1))]);
    s.add_clause(&[Lit::pos(Var(2)), Lit::pos(Var(1))]);
    let r1 = s.solve(&[Lit::pos(Var(1))]);
    ...
    assert_eq!(
        r2,
        SatResult::Sat,
        "AYSat should handle incremental correctly"
    );
}
...
#[test]
fn test_solver_backend_default_is_aysat() {
    assert_eq!(SolverBackend::default(), SolverBackend::AYSat);
}
...
#[test]
fn test_aysat_solve_with_temporary_clause_isolation() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
    let b = Var(2);
...
#[test]
fn test_aysat_solve_with_temporary_clause_sat() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
    let b = Var(2);
...
#[test]
fn test_aysat_solve_with_temporary_clause_unsat_core() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
    ...
    // UNSAT core should exist (ay-sat supports it).
    let core = s.unsat_core();
    assert!(
        core.is_some(),
        "ay-sat should provide UNSAT core through push/pop"
    );
}

/// Verify that UNSAT cores from solve_with_temporary_clause only contain
/// user-facing literals — no ay-sat internal scope-selector variables (#4024).
///
...
/// from the cube. If the core contains internal ay-sat variables (scope
/// selectors created by push()), MIC would try to look up those variables
/// in the cube and produce incorrect generalizations.
#[test]
fn test_aysat_solve_with_temporary_clause_unsat_core_no_scope_lits() {
    let mut s = AYSatCdclSolver::new(4);
    let a = Var(1);
    let b = Var(2);
    let c = Var(3);
    ...
    for lit in &core {
        assert!(
            ...
            "UNSAT core contains non-user variable {:?} (index {} >= user_num_vars {}); \
             likely a ay-sat scope selector leaked through (#4024)",
            lit,
            lit.var().0,
            user_num_vars,
        );
    ...
    // with the temp clause (!a), so ay-sat can prove UNSAT without any
    // assumptions. An empty core is a valid (and minimal) UNSAT core.
    // With JIT disabled (#4040), ay-sat's interpreter conflict analysis
...
#[test]
fn test_aysat_repeated_push_pop_core_stays_clean() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
    let b = Var(2);
...
#[test]
fn test_aysat_solve_with_temporary_clause_empty() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
    let b = Var(2);
...
#[test]
fn test_aysat_permanent_clauses_survive_temporary_solve() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
    let b = Var(2);
...
/// the solver.
#[test]
fn test_aysat_repeated_temporary_clauses_no_accumulation() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
    let b = Var(2);
    ...
    // With push/pop, the AYSatCdclSolver's num_vars should NOT have grown
    // by 200 (which is what the activation-literal default would do).
    // ay-sat's push/pop allocates internal scope selector variables, but
    // those are managed inside ay-sat and not exposed through our num_vars.
    assert_eq!(
...
#[test]
fn test_aysat_interleaved_temp_and_permanent_solves() {
    let mut s = AYSatCdclSolver::new(4);
    let a = Var(1);
    let b = Var(2);
    let c = Var(3);
...
/// Verify that a poisoned AYSatCdclSolver returns Unknown for all
/// subsequent calls and does not panic or corrupt state.
#[test]
fn test_aysat_poisoned_solver_returns_unknown() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
    let b = Var(2);
    ...
    // Manually poison the solver (simulating a caught ay-sat panic).
    s.poisoned = true;
...
#[test]
fn test_aysat_catch_unwind_in_solve() {
    ...
    // We cannot easily trigger a ay-sat panic in a unit test without
...
/// Verify that set_cancelled wires the AtomicBool into ay-sat's interrupt
/// mechanism. When the flag is pre-set to true, solve() should return
/// Unknown immediately instead of running to completion.
#[test]
fn test_aysat_set_cancelled_interrupts_solve() {
...
#[test]
fn test_aysat_set_cancelled_interrupts_temp_clause_solve() {
    use std::sync::atomic::AtomicBool;
    use std::sync::Arc;
...
#[test]
fn test_aysat_set_cancelled_cooperative_lifecycle() {
...
mod ay_variant_tests {
    use super::*;

    /// Each ay-sat variant backend should solve a basic SAT instance.
    #[test]
    fn test_ay_variants_basic_sat() {
        let backends = [
            SolverBackend::AYSat,
    ...
    /// Each ay-sat variant should detect UNSAT.
    #[test]
    fn test_ay_variants_basic_unsat() {
        let backends = [
            SolverBackend::AYSat,
            SolverBackend::AYLuby,
            SolverBackend::AYStable,
    ...
    /// Each ay-sat variant should handle assumptions correctly.
    #[test]
    fn test_ay_variants_assumptions() {
        let backends = [
            SolverBackend::AYLuby,
            SolverBackend::AYStable,
            SolverBackend::AYVmtf,
    ...
    /// Incremental stress test with ay-sat Luby backend.
    #[test]
    fn test_ay_luby_incremental_stress() {
        let mut s = SolverBackend::AYLuby.make_solver(10);
    ...
    /// ay-sat VMTF backend factory works through SolverBackend.
    #[test]
    fn test_ay_vmtf_backend_factory() {
        let mut s = SolverBackend::AYVmtf.make_solver(3);
        s.add_clause(&[Lit::TRUE]);
        s.add_clause(&[Lit::pos(Var(1)), Lit::pos(Var(2))]);
...
/// Verify that AYSatCdclSolver::clone_solver produces a solver with the
/// same clause database via clause log replay (#4062).
#[test]
fn test_aysat_clone_identical_results() {
    let mut orig = AYSatCdclSolver::new(4);
    let a = Var(1);
    let b = Var(2);
    let c = Var(3);
    ...
    let mut cloned = orig
        .clone_solver()
        .expect("AYSatCdclSolver clone should succeed");
...
#[test]
fn test_aysat_clone_isolation() {
    let mut orig = AYSatCdclSolver::new(3);
    let a = Var(1);
    let b = Var(2);
...
/// Verify that cloning a poisoned AYSatCdclSolver returns None (#4062).
#[test]
fn test_aysat_clone_poisoned_returns_none() {
    let mut s = AYSatCdclSolver::new(3);
    s.add_clause(&[Lit::pos(Var(1))]);
    s.poisoned = true;
    assert!(
...
fn test_default_clone_solver_returns_none() {
    ...
    let z = SolverBackend::AYSat.make_solver(3);
    assert!(
        z.clone_solver().is_some(),
        "AYSatCdclSolver should support clone"
    );
}

#[test]
fn test_make_solver_ic3_mode_sets_ay_ic3_mode() {
    let default_solver = SolverBackend::AYSat.make_solver(3);
    assert!(
        !default_solver.is_ic3_mode(),
        "plain ay solver construction should preserve non-IC3 defaults"
    );

    let ic3_solver = SolverBackend::AYSat.make_solver_ic3_mode(3);
    assert!(
        ic3_solver.is_ic3_mode(),
        "IC3 solver construction must call ay-sat set_ic3_mode"
    );
}

#[test]
fn test_make_solver_ic3_mode_simple_solver_is_noop() {
    let simple_solver = SolverBackend::Simple.make_solver_ic3_mode(3);
    assert!(
        !simple_solver.is_ic3_mode(),
        "non-ay backends should keep the default non-IC3 observable state"
    );
}

#[test]
fn test_aysat_ic3_domain_query_records_bucket_queue_vsids_observation() {
    let mut solver = SolverBackend::AYSat.make_solver_ic3_mode(6);
    let a = Var(1);
    let b = Var(2);

    solver.add_clause(&[Lit::pos(a), Lit::pos(b)]);
    solver.set_domain(&[a, b]);
    assert_eq!(solver.solve(&[Lit::neg(a)]), SatResult::Sat);
    solver.clear_domain();

    let stats = solver.ic3_bucket_queue_vsids_stats();
    assert_eq!(
        stats.domain_sets, 1,
        "set_domain under IC3 mode should activate ay-sat bucket-queue VSIDS"
    );
    assert_eq!(
        stats.domain_queries, 1,
        "active IC3 domain solve should be recorded"
    );
    assert_eq!(
        stats.heap_fallback_queries, 0,
        "easy IC3 domain query should not cross ay-sat's heap-fallback threshold"
    );
    assert_eq!(
        stats.max_query_restarts, 0,
        "easy IC3 domain query should complete without restarts"
    );
}
...
#[test]
fn test_aysat_clone_many_clauses() {
    let num_vars = 20;
    let mut orig = AYSatCdclSolver::new(num_vars + 1);
...
#[test]
fn test_aysat_clone_temp_clause_works() {
    let mut orig = AYSatCdclSolver::new(3);
    let a = Var(1);
    let b = Var(2);
...
#[test]
fn test_aysat_budget_zero_returns_unknown() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
...
#[test]
fn test_aysat_budget_large_easy_problem_sat() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
    let b = Var(2);
    s.add_clause(&[Lit::pos(a), Lit::pos(b)]);
...
#[test]
fn test_aysat_budget_large_easy_problem_unsat() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
    // a AND !a
    s.add_clause(&[Lit::pos(a)]);
...
#[test]
fn test_aysat_budget_small_hard_problem_returns_unknown() {
    ...
    let mut s = AYSatCdclSolver::new(total_vars);
...
#[test]
fn test_aysat_budget_solver_usable_after_unknown() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
    let b = Var(2);
...
#[test]
fn test_aysat_budget_poisoned_returns_unknown() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
    s.add_clause(&[Lit::pos(a)]);
    s.poisoned = true;
...
fn test_budget_via_trait_object() {
    let mut s: Box<dyn SatSolver> = SolverBackend::AYSat.make_solver(3);
    s.add_clause(&[Lit::pos(Var(1))]);
...
#[test]
fn test_aysat_budget_trivial_unsat() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
...
#[test]
fn test_aysat_budget_with_assumptions() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
    let b = Var(2);
    // (a OR b)
...
/// Verify that AYSatCdclSolver::clone_for_incremental produces a solver
/// that preserves learned clauses and gives correct results.
#[test]
fn test_aysat_clone_for_incremental_basic() {
    let mut orig = AYSatCdclSolver::new(4);
    let a = Var(1);
    let b = Var(2);
    let c = Var(3);
    ...
    let mut cloned = orig
        ...
        .expect("AYSatCdclSolver incremental clone should succeed");
...
/// affect clone.
#[test]
fn test_aysat_clone_for_incremental_isolation() {
    let mut orig = AYSatCdclSolver::new(3);
    let a = Var(1);
    let b = Var(2);
...
#[test]
fn test_aysat_clone_for_incremental_poisoned() {
    let mut s = AYSatCdclSolver::new(3);
    s.add_clause(&[Lit::pos(Var(1))]);
    s.poisoned = true;
    assert!(
...
#[test]
fn test_aysat_set_domain_basic_sat() {
    let mut s = AYSatCdclSolver::new(4);
    let a = Var(1);
    let b = Var(2);
    let c = Var(3);
...
#[test]
fn test_aysat_set_domain_unsat() {
    let mut s = AYSatCdclSolver::new(3);
    let a = Var(1);
...
#[test]
fn test_aysat_set_domain_poisoned_noop() {
    let mut s = AYSatCdclSolver::new(3);
    s.poisoned = true;
    // Should not panic.
    s.set_domain(&[Var(1)]);
...
// --- unassign_model_value / minimize_model tests (#4091) ---

/// Verify that unassign_model_value on AYSatCdclSolver works after a SAT result.
#[test]
fn test_aysat_unassign_model_value_basic() {
    let mut s = AYSatCdclSolver::new(4);
    let a = Var(1);
    let b = Var(2);
    let c = Var(3);
    ...
    // a is forced by a unit clause — the retraction should fail.
    let unassigned_a = s.unassign_model_value(a);
    assert!(
        !unassigned_a,
        "a is forced by unit clause, should stay assigned"
    );

    // b or c may be retractable depending on the model.
    // We just verify no panic occurs.
    let _ = s.unassign_model_value(b);
    let _ = s.unassign_model_value(c);
}

/// Verify that minimize_model returns a subset of the original model.
#[test]
fn test_aysat_minimize_model_basic() {
    let mut s = AYSatCdclSolver::new(5);
    let a = Var(1);
    let b = Var(2);
    let c = Var(3);
    let d = Var(4);
...
#[test]
fn test_aysat_minimize_model_poisoned() {
    let mut s = AYSatCdclSolver::new(3);
    s.poisoned = true;
...
/// Default trait implementation: unassign_model_value returns false for SimpleSolver.
#[test]
fn test_simple_solver_unassign_model_value_default() {
    let mut s = SimpleSolver::new();
    let a = s.new_var();
    s.add_clause(&[Lit::pos(a)]);
    let r = s.solve(&[]);
    ...
    assert!(
        !s.unassign_model_value(a),
        "SimpleSolver default unassign_model_value returns false"
    );
}
