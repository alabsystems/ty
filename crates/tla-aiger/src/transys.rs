...
impl Transys {
    ...
    /// Build a `Var -> bool` assignment from one named trace step
    /// (`l{idx}` / `i{idx}` keys indexing `latch_vars` / `input_vars`).
    fn build_step_assignment(&self, step: &FxHashMap<String, bool>) -> FxHashMap<Var, bool> {
    ...
    /// Evaluate a literal under a partial assignment (`None` = undetermined).
    fn eval_trace_lit(lit: Lit, assign: &FxHashMap<Var, bool>) -> Option<bool> {
        if lit == Lit::FALSE {
            return Some(false);
        }
        if lit == Lit::TRUE {
    ...
    /// Evaluate all AND gates given current assignments, adding results to
    /// `assign` in place. Uses topological evaluation: iterate until fixpoint.
    fn eval_gates_to_fixpoint(&self, assign: &mut FxHashMap<Var, bool>) {
        ...
        while changed {
            ...
            for (&out_var, &(rhs0, rhs1)) in &self.and_defs {
                ...
                if let (Some(v0), Some(v1)) = (
                    Self::eval_trace_lit(rhs0, assign),
                    Self::eval_trace_lit(rhs1, assign),
                ) {
    ...
    pub fn verify_witness(&self, trace: &[FxHashMap<String, bool>]) -> Result<(), String> {
        ...
        let eval_lit = Self::eval_trace_lit;

        // Step 0: verify init constraints
        let mut assign = self.build_step_assignment(&trace[0]);
        self.eval_gates_to_fixpoint(&mut assign);
        for clause in &self.init_clauses {
        ...
        for k in 0..trace.len() {
            let mut assign = self.build_step_assignment(&trace[k]);
            self.eval_gates_to_fixpoint(&mut assign);
            ...
            if k + 1 < trace.len() {
                let next_assign = self.build_step_assignment(&trace[k + 1]);
                for (idx, &latch_var) in self.latch_vars.iter().enumerate() {
...
/// Attribute a counterexample trace to the bad-state properties it violates.
///
/// Simulates the circuit on the FINAL step of `trace` (a witness in this
/// system's `l{idx}`/`i{idx}` frame, as produced by the engines and checked
/// by [`Transys::verify_witness`]) and returns one flag per entry of
/// `ts.bad_lits`: `true` iff that property's bad literal evaluates to TRUE at
/// the final step.
///
/// The SAT engines decide the Tseitin OR of all bad literals, so a bare
/// `Unsafe` verdict does not say WHICH property failed; this computes the
/// per-property attribution needed for multi-property AIGER results.
/// Undetermined literals (values missing from the trace) count as NOT
/// violated, so a frame-mismatched trace can only under-attribute — it can
/// never fabricate a violation. An empty trace attributes nothing.
pub fn attribute_bad_indices(ts: &Transys, trace: &[FxHashMap<String, bool>]) -> Vec<bool> {
    let Some(last) = trace.last() else {
        return vec![false; ts.bad_lits.len()];
    };
    let mut assign = ts.build_step_assignment(last);
    ts.eval_gates_to_fixpoint(&mut assign);
    ts.bad_lits
        .iter()
        .map(|&bad| Transys::eval_trace_lit(bad, &assign).unwrap_or(false))
        .collect()
}
...
impl VarRenamer {
    /// Create a renamer for `ts`. Step 0 reuses the base variables; fresh
    /// variables for later steps are allocated starting at `ts.max_var + 1`.
...
mod tests {
    ...
    #[test]
    fn test_attribute_bad_indices_flags_only_violated_properties() {
        // Two latches, two bad properties: bad0 = l0 (toggles), bad1 = l1
        // (stuck at 0). The verified witness (0,0) -> (1,0) violates ONLY
        // property 0 at its final step.
        let circuit = parse_aag("aag 2 0 2 0 0 2\n2 3\n4 0\n2\n4\n").unwrap();
        let ts = Transys::from_aiger(&circuit);

        let mut step0 = FxHashMap::default();
        step0.insert("l0".to_string(), false);
        step0.insert("l1".to_string(), false);
        let mut step1 = FxHashMap::default();
        step1.insert("l0".to_string(), true);
        step1.insert("l1".to_string(), false);
        let trace = vec![step0, step1];

        assert!(
            ts.verify_witness(&trace).is_ok(),
            "trace must be a valid witness"
        );
        assert_eq!(attribute_bad_indices(&ts, &trace), vec![true, false]);
    }

    #[test]
    fn test_attribute_bad_indices_constant_bads_and_empty_trace() {
        // bad0 = constant FALSE (holds), bad1 = constant TRUE (violated).
        let circuit = parse_aag("aag 0 0 0 0 0 2\n0\n1\n").unwrap();
        let ts = Transys::from_aiger(&circuit);
        let step = FxHashMap::default();
        assert_eq!(attribute_bad_indices(&ts, &[step]), vec![false, true]);
        // Empty trace attributes nothing (fail-closed).
        assert_eq!(attribute_bad_indices(&ts, &[]), vec![false, false]);
    }
