...
//! Adaptive preset-rotation scheduler.
//!
...
//! microban_1 per #4247), no recovery mechanism exists. The adaptive
//! scheduler replaces each
...
use super::runner::{
    engine_verified_label, ic3_to_check_result, join_with_grace_period, portfolio_check_detailed,
    wrap_engine_verified, wrap_kind_verified, PORTFOLIO_GRACE_PERIOD,
};
...
impl AdaptiveScheduler {
    ...
    /// Construct from the default preset pool.
    #[must_use]
    pub fn with_default_presets(base_rseed: u64) -> Self {
        Self::new(default_preset_pool(), base_rseed)
    }

    /// Number of presets in the rotation pool.
    #[must_use]
    pub fn preset_pool_size(&self) -> usize {
        self.presets.len()
    }

    /// Number of presets dispatched so far (the current rotation index).
    #[must_use]
    pub fn iteration(&self) -> usize {
        self.iter
    }

    /// Iteration cap after which [`next`](Self::next) returns `None`.
    #[must_use]
    ...
    // Intentionally named `next` to mirror iterator semantics without
    // committing to the `Iterator` trait (the scheduler maintains additional
    // state — `iter`, `base_rseed` — that doesn't fit `Iterator`).
...
/// Default preset pool: 6 curated configs
...
pub fn default_preset_pool() -> Vec<Ic3Config> {
    vec![
        ...
        // 1: internal signals only
        Ic3Config {
            internal_signals: true,
            random_seed: 2,
            ..Ic3Config::default()
        },
        // 2: CTP + infinity frame
        Ic3Config {
            ctp: true,
            inf_frame: true,
            random_seed: 3,
        ...
        // 3: dynamic generalization (arXiv:2501.02480 §IV)
        Ic3Config {
            dynamic: true,
            random_seed: 4,
            ..Ic3Config::default()
        },
        // 4: internal signals + a distinct high seed (ty-assigned)
        Ic3Config {
            internal_signals: true,
            random_seed: 1001,
            ..Ic3Config::default()
        },
        // 5: CTP + a distinct high seed (ty-assigned)
        Ic3Config {
            ctp: true,
            random_seed: 1002,
            ..Ic3Config::default()
        },
        // 6: tla-aiger extension — deep CTG (ty raised budget, see
        // `Ic3Config::ctg_limit`) + random-shuffle MIC ordering
        Ic3Config {
            ctg_max: 5,
            ctg_limit: 12,
                    ...
                    if matches!(done.result.result, CheckResult::Safe) {
                        ...
                        match validate_safe(witness_ref, &ts) {
                            ...
                            SafeValidation::Indeterminate { reason } => {
                                ...
                                // (`safe_witness.rs` `Unknown => Accepted`). A
                                // budget-out / solver-`Unknown` on the
                                // independent inductive-invariant re-check is
                                // "could not confirm", NOT "disproved".
                                // Respawning instead of accepting would risk
                                // losing a correct SAFE verdict; accept the
                                // engine's own internally-verified Safe exactly
                                // as before. Only a genuine counterexample
                                // (`Rejected`) blocks acceptance.
                                eprintln!(
                                    "AdaptivePortfolio: validate_safe \
                                     indeterminate for {} (#4315): {}. \
                                     Accepting the engine's internally-verified \
                                     Safe (fail-open — a correct SAFE is never \
                                     downgraded).",
                                    done.result.solver_name, reason,
                                );
                                // Fall through to accept (same as Accepted).
                            }
...
fn run_single_engine(
    ...
    let safe_engine = engine_verified_label(&engine);
    match engine {
        EngineConfig::Bmc { step } => {
            let mut e = crate::bmc::BmcEngine::new(ts, step);
            e.set_cancelled(cancelled);
            wrap_engine_verified(e.check(max_depth), safe_engine)
        }
        ...
        EngineConfig::Kind => {
            ...
            e.set_cancelled(cancelled);
            wrap_kind_verified(e.check(max_depth), safe_engine, false, false, max_depth)
        }
        EngineConfig::KindSkipBmc => {
