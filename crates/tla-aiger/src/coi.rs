...
//! all competitive HWMCC solvers perform (ABC, super-prove, etc.).
//!
//! The algorithm:
...
#[derive(Debug, Clone)]
pub struct CoiStats {
    /// Number of latches before reduction.
    pub orig_latches: usize,
    /// Number of inputs before reduction.
    pub orig_inputs: usize,
    /// Number of AND gates before reduction.
    pub orig_and_gates: usize,
    /// Number of transition clauses before reduction.
    pub orig_trans_clauses: usize,
    /// Number of latches retained in the cone of influence.
    pub reduced_latches: usize,
    /// Number of inputs retained in the cone of influence.
    pub reduced_inputs: usize,
    /// Number of AND gates retained in the cone of influence.
    pub reduced_and_gates: usize,
    /// Number of transition clauses retained in the cone of influence.
...
impl Transys {
    ...
    ///
    /// Applied after COI refinement, prioritizing latches, inputs, and
    /// bad/constraint variables in the renumbering to improve SAT solver
    /// variable ordering.
    pub fn compact_vars(&self) -> Transys {
        ...
        // Priority order: latches, inputs, bad/constraint vars first,
    ...
    ///
    /// Like [`Self::preprocess`] but allows portfolio configs to customize
