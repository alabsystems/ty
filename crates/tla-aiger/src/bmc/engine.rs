...
/// Run BMC with a size-adaptive step (ty ladder policy — see `new_dynamic`).
///
/// Automatically adjusts step size to the circuit: small circuits get large
/// steps (fast deep exploration), large circuits get step=1 (thorough
/// per-depth checking).
///
...
pub enum BmcStepStrategy {
    ...
    Fixed(usize),
    /// Size-adaptive step (ty ladder policy): the largest rung of
    /// `DYNAMIC_STEP_LADDER` whose unroll window fits under
    /// `DYNAMIC_STEP_CLAUSE_BUDGET` — see `dynamic_step_for`.
    ///
...
pub struct BmcEngine {
    ...
    renamer: VarRenamer,
    /// How many depths to unroll between consecutive SAT checks.
    ...
    ///
    /// Tracks how many times ay-sat returned SAT but the witness failed
...
/// Step-size ladder for the size-adaptive BMC engine (ty policy).
///
/// `new_dynamic` picks the LARGEST rung whose unroll window — the clauses
/// loaded between two consecutive SAT checks — fits under
/// [`DYNAMIC_STEP_CLAUSE_BUDGET`]. Order-of-magnitude rungs suffice: the
/// portfolio's fixed-step slots already cover fine gradations, so the
/// adaptive engine only needs to land in the right regime, and a small
/// published ladder keeps the chosen step reproducible and easy to read
/// off a log line.
const DYNAMIC_STEP_LADDER: [usize; 4] = [1, 10, 100, 500];

/// Clause budget for one dynamic-BMC unroll window (ty constant).
///
/// With the any-bad accumulator, a window's cost is the incremental load of
/// `per_step_clauses * step` clauses followed by one SAT call. ty budgets
/// that load at one million clauses: a window of ~10^6 short clauses is
/// tens of MB of solver state, which one incremental add+solve absorbs
/// quickly, while windows an order of magnitude larger make each check
/// visibly laggy and slow the engine's response to portfolio cancellation.
/// The constant is a round engineering budget, not a tuned optimum.
const DYNAMIC_STEP_CLAUSE_BUDGET: usize = 1_000_000;

/// Pick the dynamic BMC step for a circuit: the largest ladder rung whose
/// unroll window stays within the clause budget.
///
/// The per-step clause estimate counts what `unroll_one_step` actually
/// appends per depth: the shifted transition relation, the 2-clause latch
/// wiring equivalences, and the constraint units.
fn dynamic_step_for(ts: &Transys) -> usize {
    let per_step =
        (ts.trans_clauses.len() + 2 * ts.latch_vars.len() + ts.constraint_lits.len()).max(1);
    DYNAMIC_STEP_LADDER
        .iter()
        .rev()
        .copied()
        .find(|&step| per_step.saturating_mul(step) <= DYNAMIC_STEP_CLAUSE_BUDGET)
        .unwrap_or(1)
}

impl BmcEngine {
    /// Create a new BMC engine with a size-adaptive step (ty ladder policy).
    ///
    /// Picks the largest step from [`DYNAMIC_STEP_LADDER`] whose unroll
    /// window (estimated clauses loaded between SAT checks) stays under
    /// [`DYNAMIC_STEP_CLAUSE_BUDGET`]. Small circuits land on the 500 rung
    /// (deep reach, few SAT calls); large circuits fall back to step=1
    /// (every check window is already expensive).
    pub fn new_dynamic(ts: Transys) -> Self {
        let step = dynamic_step_for(&ts);
        Self::new(ts, step)
    }
    ...
    /// Create a new BMC engine with a size-adaptive step and a specific solver backend.
    ///
    /// Same as `new_dynamic` (ty ladder policy) but allows selecting the
    /// solver backend (e.g., ay-sat Luby, Stable, VMTF variants for
    /// portfolio diversity).
    ...
    /// ## Solver backend (#4073)
    ///
    /// Uses `AYNoPreprocess` by default. For incremental BMC, ay-sat's
    ...
    pub fn new(ts: Transys, step: usize) -> Self {
        Self::new_with_backend(ts, step, SolverBackend::AYNoPreprocess)
    }

    /// Create a new BMC engine with a specific solver backend.
    ///
    /// This allows the portfolio to spawn BMC engines backed by different
    /// ay-sat configurations (Luby restarts, stable mode, VMTF branching,
    /// etc.) for diversity — solver-config diversity entirely within ty's
    /// own SAT stack, with no external solver dependencies.
    ...
    ///
    /// Propagates the flag to the underlying SAT solver so ay-sat's CDCL loop
    ...
    ///
    /// Returns the count of ay-sat SAT results where the witness trace
    /// failed simulation against the original circuit. Each spurious SAT
    /// represents a solver bug (ay-sat returned an incorrect satisfying
    ...
    /// Unknown instead of Unsafe. This guards against ay-sat bugs that
    ...
    ///
    /// Includes witness verification (#4103) to guard against ay-sat bugs.
