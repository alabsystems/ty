...
//!
//! Standard incremental BMC over ty's SAT types.

pub mod engine;
pub(crate) mod gpu_exhaustive;
pub(crate) mod gpu_random_sim;
pub(crate) mod random_sim;
pub mod witness;

pub use engine::{BmcEngine, BmcStepStrategy};
pub(crate) use gpu_exhaustive::{try_gpu_exhaustive_bmc, GpuExhaustBmc};
pub(crate) use gpu_random_sim::try_gpu_random_sim;
...
mod tests {
    ...
    fn test_bmc_dynamic_step_small_circuit() {
        // Small circuit: the ladder policy should land on the top rung (500).
        let circuit = parse_aag("aag 1 0 1 0 0 1\n2 3\n2\n").unwrap();
        let ts = Transys::from_aiger(&circuit);
        // per_step ≈ 2 clauses (one latch's wiring) => 500-step window ≈ 1000
        // clauses, far under the clause budget => top ladder rung 500.
        let engine = BmcEngine::new_dynamic(ts);
        assert_eq!(
            engine.step, 500,
            "ladder policy should pick the top rung (500) for a tiny circuit"
        );
    }
    ...
    #[test]
    fn test_check_kind_nonunit_init_agrees_with_bmc() {
        // Regression (HIGH soundness, non-unit init clauses): l1's reset is
        // l0's VALUE (reset lit 2), producing binary init clauses l1 <=> l0.
        // These used to be added permanently to k-induction's single solver,
        // constraining the induction-step start state to init and yielding a
        // false Safe at k=1. BMC (ground truth here) finds the bad state at
        // depth 2; check_kind must agree.
        let circuit = parse_aag("aag 2 0 2 0 0 1\n2 1 0\n4 2 2\n4\n").unwrap();
        let ts = Transys::from_aiger(&circuit);

        let bmc = check_bmc(&ts, 10);
        assert_eq!(bmc.verdict, Some(false), "BMC ground truth: UNSAFE");
        assert_eq!(bmc.depth, 2, "shortest counterexample at depth 2");

        let kind = check_kind(&ts, 10);
        assert_ne!(
            kind.verdict,
            Some(true),
            "false Safe: k-induction step must not be constrained by init"
        );
        assert_eq!(kind.verdict, Some(false), "k-induction must agree with BMC");
        assert_eq!(kind.depth, 2);
        assert!(kind.witness.is_some());
    }
    ...
    // ----- ay-sat variant backend BMC tests -----

    mod ay_variant_bmc_tests {
        use super::*;
        use crate::sat_types::SolverBackend;

        #[test]
        fn test_bmc_ay_luby_trivially_unsafe() {
            // output=1 (constant TRUE) => bad at step 0
            let circuit = parse_aag("aag 0 0 0 1 0\n1\n").unwrap();
            let ts = Transys::from_aiger(&circuit);
            let mut bmc = BmcEngine::new_with_backend(ts, 1, SolverBackend::AYLuby);
            let result = bmc.check(10);
            assert!(matches!(result, CheckResult::Unsafe { depth: 0, .. }));
        }

        #[test]
        fn test_bmc_ay_stable_trivially_safe_bounded() {
            // output=0 (constant FALSE) => never bad
            let circuit = parse_aag("aag 0 0 0 1 0\n0\n").unwrap();
            let ts = Transys::from_aiger(&circuit);
            let mut bmc = BmcEngine::new_with_backend(ts, 1, SolverBackend::AYStable);
            let result = bmc.check(10);
            assert!(matches!(result, CheckResult::Unknown { .. }));
        }

        #[test]
        fn test_bmc_ay_vmtf_toggle_reachable() {
            // Toggle flip-flop: latch starts at 0, toggles each step.
            // Bad = latch. Reachable at step 1.
            let circuit = parse_aag("aag 1 0 1 0 0 1\n2 3\n2\n").unwrap();
            let ts = Transys::from_aiger(&circuit);
            let mut bmc = BmcEngine::new_with_backend(ts, 1, SolverBackend::AYVmtf);
            let result = bmc.check(10);
            match &result {
                CheckResult::Unsafe { depth, trace } => {
                    assert_eq!(*depth, 1);
        ...
        #[test]
        fn test_bmc_ay_geometric_dynamic_step() {
            // Trivially unsafe: dynamic step BMC should find it at depth 0.
            let circuit = parse_aag("aag 0 0 0 1 0\n1\n").unwrap();
            let ts = Transys::from_aiger(&circuit);
            let mut bmc = BmcEngine::new_dynamic_with_backend(ts, SolverBackend::AYGeometric);
            let result = bmc.check(1000);
            assert!(
                matches!(result, CheckResult::Unsafe { depth: 0, .. }),
                "ay-sat Geometric dynamic BMC should find Unsafe at depth 0, got {result:?}"
            );
        }

        #[test]
        fn test_bmc_ay_chb_two_step_counter() {
            // 2-step counter: l0 toggles, l1 = delayed l0, bad = !l0 AND l1.
            // Bad reachable at step 2 (00 -> 10 -> 01, and !0 AND 1 = 1).
            let aag = "aag 3 0 2 0 1 1\n2 3\n4 2\n6\n6 3 4\n";
            let circuit = parse_aag(aag).unwrap();
            let ts = Transys::from_aiger(&circuit);
            let mut bmc = BmcEngine::new_with_backend(ts, 1, SolverBackend::AYChb);
            let result = bmc.check(10);
            assert!(
                matches!(result, CheckResult::Unsafe { depth: 2, .. }),
                "ay-sat CHB BMC should find 2-step counter bug at depth 2, got {result:?}"
            );
        }

        #[test]
        fn test_bmc_ay_nopreprocess_cancellation() {
            let circuit = parse_aag("aag 1 0 1 0 0 1\n2 0\n2\n").unwrap();
            let ts = Transys::from_aiger(&circuit);
            let mut bmc = BmcEngine::new_with_backend(ts, 1, SolverBackend::AYNoPreprocess);
        ...
        #[test]
        fn test_bmc_ay_luby_step_5_with_accumulator() {
            // Toggle flip-flop with step=5 and ay-sat Luby backend.
            // The any-depth accumulator should find the bug at depth 1.
            let circuit = parse_aag("aag 1 0 1 0 0 1\n2 3\n2\n").unwrap();
            let ts = Transys::from_aiger(&circuit);
            let mut bmc = BmcEngine::new_with_backend(ts, 5, SolverBackend::AYLuby);
            let result = bmc.check(20);
            assert!(
                matches!(result, CheckResult::Unsafe { depth: 1, .. }),
                "ay-sat Luby BMC with step=5 should find bug at depth 1, got {result:?}"
            );
        }

        /// ay-sat variant and default BMC should produce identical results on the same circuit.
        #[test]
        fn test_bmc_ay_variant_default_agreement() {
            let aag = "aag 3 0 2 0 1 1\n2 3\n4 2\n6\n6 3 4\n";
            let circuit = parse_aag(aag).unwrap();
            let ts = Transys::from_aiger(&circuit);
            ...
            let mut luby_bmc = BmcEngine::new_with_backend(ts, 1, SolverBackend::AYLuby);
            ...
            match (&default_result, &luby_result) {
                (
                    ...
                    assert_eq!(
                        default_d, luby_d,
                        "ay-sat default and Luby should find bug at same depth"
                    );
                }
                _ => panic!(
        ...
        /// ay-sat Luby BMC with dynamic step finds intermediate bugs.
        #[test]
        fn test_bmc_ay_luby_dynamic_two_step_counter() {
            let aag = "aag 3 0 2 0 1 1\n2 3\n4 2\n6\n6 3 4\n";
            let circuit = parse_aag(aag).unwrap();
            let ts = Transys::from_aiger(&circuit);
            let mut bmc = BmcEngine::new_dynamic_with_backend(ts, SolverBackend::AYLuby);
            let result = bmc.check(1000);
            assert!(
                matches!(result, CheckResult::Unsafe { depth: 2, .. }),
                "ay-sat Luby dynamic BMC should find 2-step counter bug at depth 2, got {result:?}"
            );
        }
    }
    ...
    fn test_bmc_deep_shift_register_chain_depth_128() {
        ...
        // Latch 0: toggle
        lines.push("2 3".to_string()); // lit 2, next = NOT 2 = 3
    ...
    mod geometric_backoff_tests {
        ...
        #[test]
        fn test_geometric_backoff_ay_variant() {
            // Geometric backoff with ay-sat Luby backend.
            let aag = "aag 3 0 2 0 1 1\n2 3\n4 2\n6\n6 3 4\n";
            let circuit = parse_aag(aag).unwrap();
            let ts = Transys::from_aiger(&circuit);
            let mut bmc = BmcEngine::new_geometric_backoff_with_backend(ts, SolverBackend::AYLuby);
            let result = bmc.check_geometric_backoff(1000, 50, 20, 64);
            assert!(
                matches!(result, CheckResult::Unsafe { depth: 2, .. }),
                "geometric backoff ay-Luby BMC should find bug at depth 2, got {result:?}"
            );
        }
    }
    ...
    mod witness_verification_tests {
        ...
        fn test_bmc_witness_toggle_valid() {
            ...
            match engine.check(10) {
                CheckResult::Unsafe { depth: _, trace } => {
        ...
        fn test_cal76_no_spurious_sat() {
            ...
            // Test all ay-sat variant backends used by the portfolio.
            ...
            let variants: &[(&str, usize, SolverBackend)] = &[
                ("AYLuby step=10", 10, SolverBackend::AYLuby),
                ("AYStable step=100", 100, SolverBackend::AYStable),
                ("AYVmtf step=500", 500, SolverBackend::AYVmtf),
                ("AYGeometric step=1", 1, SolverBackend::AYGeometric),
                ("AYChb step=1", 1, SolverBackend::AYChb),
                ("AYNoPreprocess step=1", 1, SolverBackend::AYNoPreprocess),
            ];
