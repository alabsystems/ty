...
impl RandomSimEngine {
    ...
    fn run_walk(&self, seed: u64, total_steps: &mut u64) -> Option<CheckResult> {
        ...
        // Keep a complete prefix. If a walk runs past the bounded trace budget,
        // later bugs are ignored rather than returning an incomplete witness.
        let max_trace_len = self.steps_per_walk.min(10000) + 1;
        trace_history.push(self.build_trace_step(&values));

        let mut logical_step = 1;
        let mut attempts = 0usize;
        while logical_step <= self.steps_per_walk {
            attempts = attempts.wrapping_add(1);
            if attempts % 4096 == 0 && self.cancelled.load(Ordering::Relaxed) {
                return None;
            }
            ...
            // Check constraints. If violated, discard these random inputs and
            // retry from the same latch state; illegal inputs are not a
            // transition and must not be counted as simulation steps.
            ...
            if self.is_bad(&values) {
                if trace_history.len() >= max_trace_len {
                    eprintln!(
                        "RandomSim: bug reached after trace budget ({} steps, walk seed={}); ignoring incomplete witness",
                        max_trace_len - 1,
                        seed,
                    );
                    return None;
...
#[inline(always)]
pub(crate) fn eval_lit_fast(lit: Lit, values: &[bool]) -> bool {
    if lit == Lit::FALSE {
        return false;
    }
    if lit == Lit::TRUE {
...
pub(crate) fn compute_topo_order(ts: &Transys) -> Vec<Var> {
...
pub(crate) fn extract_init_values(ts: &Transys) -> Vec<bool> {
...
mod tests {
    ...
    fn test_random_sim_two_step_counter() {
        ...
        let result = engine.check();
        match result {
            CheckResult::Unsafe { depth, trace } if (2..=3).contains(&depth) => {
                assert_eq!(trace.len(), depth + 1, "trace should keep full prefix");
            }
            other => {
                panic!("random sim should find 2-step counter bug at depth 2-3, got {other:?}")
            }
        }
    }
    ...
    #[test]
    fn test_random_sim_constraint_violation_does_not_advance_latch() {
        // One input, one latch, latch next = input, bad = latch, constraint = !input.
        // Any input=true transition is illegal. Advancing through it would make the
        // bad latch reachable, but under the constraint the latch must stay false.
        let aag = "aag 2 1 1 0 0 1 1\n2\n4 2\n4\n3\n";
        let circuit = parse_aag(aag).unwrap();
        let ts = Transys::from_aiger(&circuit);
        let engine = RandomSimEngine::new(ts, 2, 1, 3);
        let mut total_steps = 0;

        let result = engine.run_walk(3, &mut total_steps);

        assert!(
            result.is_none(),
            "illegal input must be discarded instead of creating a false bug: {result:?}"
        );
        assert_eq!(total_steps, 2, "only legal input steps should be counted");
    }

    #[test]
    fn test_random_sim_constraint_violation_retries_same_logical_step() {
        // First random input from seed 3 is illegal (input=true), second is legal
        // (input=false), and the third is legal (input=false). The completed
        // walk should therefore count two legal transitions, not stop after the
        // illegal attempt consumes one loop iteration.
        let aag = "aag 2 1 1 0 0 1 1\n2\n4 2\n0\n3\n";
        let circuit = parse_aag(aag).unwrap();
        let ts = Transys::from_aiger(&circuit);
        let engine = RandomSimEngine::new(ts, 2, 1, 3);
        ...
        assert!(
            result.is_none(),
            "safe walk should not find a bug: {result:?}"
        );
        assert_eq!(
            total_steps, 2,
            "illegal attempts must retry the same logical simulation step"
        );
    }

    #[test]
    fn test_random_sim_constraint_violation_does_not_inflate_depth() {
        // First random input from seed 3 is illegal and must not count toward
        // depth. The next legal input sets the latch, and the following legal
        // step observes bad at logical depth 2 with a matching full trace.
        let aag = "aag 2 1 1 0 0 1 1\n2\n4 2\n4\n2\n";
        let circuit = parse_aag(aag).unwrap();
        let ts = Transys::from_aiger(&circuit);
        let engine = RandomSimEngine::new(ts, 2, 1, 3);
        ...
        match result {
            Some(CheckResult::Unsafe { depth, trace }) => {
                assert_eq!(depth, 2, "illegal attempt must not inflate depth");
                assert_eq!(
                    trace.len(),
                    depth + 1,
                    "trace length must match logical unsafe depth"
                );
                assert_eq!(
                    total_steps, 1,
                    "the unsafe transition is reported before latch advance is counted"
                );
            }
            other => panic!("expected logical-depth unsafe result, got {other:?}"),
        }
    }

    #[test]
    fn test_random_sim_completed_walk_counts_steps_once() {
        let circuit = parse_aag("aag 0 0 0 1 0\n0\n").unwrap();
        let ts = Transys::from_aiger(&circuit);
        let engine = RandomSimEngine::new(ts, 7, 1, 42);
        let mut total_steps = 0;

        let result = engine.run_walk(42, &mut total_steps);
