...
pub struct KindStrengthenedEngine {
    ...
    /// Solver backend, retained to build the consecution solver on demand.
    backend: SolverBackend,
    /// Lazily-built two-step consecution solver for the 1-INDUCTIVENESS gate
    /// that every BMC-discovered candidate must pass before it is admitted as
    /// a global invariant (soundness — see [`Self::candidate_is_inductive`]).
    /// `consec_renamer` owns its (independent) variable namespace.
    consec_solver: Option<Box<dyn SatSolver>>,
    consec_renamer: VarRenamer,
    /// How many admitted single-/pair-invariants are already asserted at step 0
    /// of the consecution solver (the admitted sets only grow).
    consec_synced_lits: usize,
    consec_synced_pairs: usize,
}

impl KindStrengthenedEngine {
    /// Create a strengthened k-induction engine for `ts` using the default
    /// (`AYSat`) solver backend.
    pub fn new(ts: Transys) -> Self {
        Self::with_backend(ts, SolverBackend::AYSat)
    }

    /// Create a strengthened k-induction engine for `ts` using a specific SAT
    /// solver backend (for portfolio diversity).
    pub fn with_backend(ts: Transys, backend: SolverBackend) -> Self {
        ...
        let consec_renamer = VarRenamer::new(&ts);
        ...
        KindStrengthenedEngine {
            ...
            consec_solver: None,
            consec_renamer,
            consec_synced_lits: 0,
            consec_synced_pairs: 0,
        }
    }

    /// Install a shared cancellation flag; when set, the engine and both of its
    /// SAT solvers abort at the next check point.
    pub fn set_cancelled(&mut self, cancelled: Arc<AtomicBool>) {
        ...
        if let Some(consec) = self.consec_solver.as_mut() {
            consec.set_cancelled(cancelled.clone());
        }
    ...
    /// Build the two-step consecution solver on first use.
    ///
    /// Loads the transition relation at steps 0 and 1 (with the latch
    /// next-state equalities wiring step 0 to step 1) into an independent
    /// solver/renamer namespace. Admitted invariants are asserted at step 0
    /// incrementally by [`Self::candidate_is_inductive`]; the transition
    /// relation and wiring are one-time and never change.
    fn ensure_consec(&mut self) {
        if self.consec_solver.is_some() {
            return;
        }
        let mut solver: Box<dyn SatSolver> = self.backend.make_solver(self.ts.max_var + 1);
        solver.set_cancelled(self.cancelled.clone());
        // AIGER convention: Var(0) = constant FALSE.
        solver.add_clause(&[Lit::TRUE]);
        self.consec_renamer.ensure_step(0);
        self.consec_renamer.ensure_step(1);
        solver.ensure_vars(self.consec_renamer.max_allocated());

        // Transition relation at steps 0 and 1.
        {
            let renamer = &self.consec_renamer;
            self.ts
                .load_trans_renamed(solver.as_mut(), &|lit| renamer.rename_lit(lit, 0));
            self.ts
                .load_trans_renamed(solver.as_mut(), &|lit| renamer.rename_lit(lit, 1));
        }
        // Latch next-state wiring from step 0 to step 1: curr@1 <=> next(latch)@0.
        for &latch_var in &self.ts.latch_vars {
            let next_lit = self.ts.next_state[&latch_var];
            let next_at_0 = self.consec_renamer.rename_lit(next_lit, 0);
            let curr_at_1 = Lit::pos(self.consec_renamer.rename_var(latch_var, 1));
            solver.add_clause(&[!curr_at_1, next_at_0]);
            solver.add_clause(&[curr_at_1, !next_at_0]);
        }
        self.consec_solver = Some(solver);
    }

    /// Return `true` iff the candidate FACT is 1-INDUCTIVE relative to the
    /// already-admitted invariants — the soundness gate for admitting any
    /// BMC-discovered candidate as a global invariant.
    ///
    /// `fact_at0` is the candidate fact as a clause (the disjunction of its
    /// literals) to assert at step 0; `neg_at1` are the assumption literals
    /// that force the fact FALSE at step 1 (the negation of that clause).
    /// With the admitted invariants asserted at step 0, the transition
    /// relation between steps 0 and 1, and the candidate fact held at step 0,
    /// the fact is preserved iff `!fact@1` is UNSAT. Admits ONLY on UNSAT;
    /// Sat OR Unknown (budget/cancellation) both REJECT (fail-closed).
    ///
    /// Bounded-BMC facts (a latch that cannot flip within a few steps, a cube
    /// unreachable within `check_depth`) are NOT globally inductive in general,
    /// so admitting them unchecked lets the strengthened k-induction assert
    /// non-invariants at every depth and return a false `Safe`. This gate
    /// rejects any candidate that is not genuinely inductive. Every candidate
    /// also provably holds in the initial state (each is derived from
    /// "unreachable from init within N steps", and init is depth 0), so an
    /// inductive candidate is a true invariant.
    fn candidate_is_inductive(&mut self, fact_at0: &[Lit], neg_at1: &[Lit]) -> bool {
        self.ensure_consec();

        // Sync newly-admitted single-literal invariants at step 0.
        while self.consec_synced_lits < self.invariant_lits.len() {
            let inv = self.invariant_lits[self.consec_synced_lits];
            let renamed = self.consec_renamer.rename_lit(inv, 0);
            self.consec_solver
                .as_mut()
                .expect("consec solver built")
                .add_clause(&[renamed]);
            self.consec_synced_lits += 1;
        }
        // Sync newly-admitted pair invariants ([!l_i, !l_j]) at step 0.
        while self.consec_synced_pairs < self.pair_invariants.len() {
            let (l_i, l_j) = self.pair_invariants[self.consec_synced_pairs];
            let a = self.consec_renamer.rename_lit(l_i, 0);
            let b = self.consec_renamer.rename_lit(l_j, 0);
            self.consec_solver
                .as_mut()
                .expect("consec solver built")
                .add_clause(&[!a, !b]);
            self.consec_synced_pairs += 1;
        }

        // Candidate fact at step 0 (temporary — do not pollute the solver if it
        // is rejected) and its negation assumed at step 1.
        let temp_clause: Vec<Lit> = fact_at0
            .iter()
            .map(|&l| self.consec_renamer.rename_lit(l, 0))
            .collect();
        let assumptions: Vec<Lit> = neg_at1
            .iter()
            .map(|&l| self.consec_renamer.rename_lit(l, 1))
            .collect();
        ...
        matches!(
            self.consec_solver
                .as_mut()
                .expect("consec solver built")
                .solve_with_temporary_clause(&assumptions, &temp_clause),
            SatResult::Unsat
        )
    }
    ...
    pub(crate) fn discover_init_invariants(&mut self) {
        ...
        // Collect "latch stays at its init value" candidates first, then gate
        // each by 1-inductiveness below. Collecting up front keeps the
        // `&self.ts.latch_vars` borrow disjoint from the `&mut self`
        // consecution check.
        let mut init_candidates: Vec<Lit> = Vec::new();
        for &latch_var in &self.ts.latch_vars {
            if self.cancelled.load(Ordering::Relaxed) {
                break;
            }
            ...
            if let Some(&init_val) = init_vals.get(&latch_var) {
                ...
                if !can_flip {
                    ...
                    init_candidates.push(inv);
                }
            }
        }

        // Admit a "stays at init value" candidate ONLY if it is 1-inductive.
        // "Cannot flip within `discovery_depth` steps" is a bounded-BMC fact,
        // not a global invariant; without this gate the strengthened
        // k-induction asserts it at every depth and can return a false Safe.
        for inv in init_candidates {
            if self.candidate_is_inductive(&[inv], &[!inv]) {
    ...
    fn ceg_strengthen(&mut self, k: usize) -> usize {
        ...
        for &cex_lit in &cex_s0_lits {
            ...
            if !reachable {
                let inv = !cex_lit;
                // Gate: "cex_lit unreachable within check_depth from init" is a
                // bounded-BMC fact; admit `!cex_lit` as a global invariant ONLY
                // if it is 1-inductive relative to the admitted invariants.
                if !self.invariant_lits.contains(&inv)
                    && self.candidate_is_inductive(&[inv], &[!inv])
                {
                    self.invariant_lits.push(inv);
                    new_count += 1;
                }
    ...
    fn discover_pair_invariants(&mut self, cex_lits: &[Lit], check_depth: usize) -> usize {
        ...
        for i in 0..limit {
            ...
            for j in (i + 1)..limit {
                ...
                // Gate: the pair FACT (!l_i OR !l_j) — "l_i AND l_j unreachable
                // within check_depth from init" — is a bounded-BMC fact; admit
                // it as a global invariant ONLY if it is 1-inductive. The fact
                // at step 0 is the clause [!l_i, !l_j]; its negation at step 1
                // is (l_i AND l_j), assumed as [l_i, l_j].
                if !reachable && self.candidate_is_inductive(&[!l_i, !l_j], &[l_i, l_j]) {
