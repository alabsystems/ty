...
// Public accessors for use by the lift module's ternary_x_drop pass.
impl TernarySim {
...
impl TernarySim {
    /// Build a ternary simulator from a transition system, snapshotting its
    /// AND-gate definitions (sorted by output var), bad literals, and next-state map.
