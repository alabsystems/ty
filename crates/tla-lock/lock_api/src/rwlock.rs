...
pub unsafe trait RawRwLockFair: RawRwLock {
    ...
    unsafe fn bump_shared(&self) {
        // SAFETY: The caller must ensure a shared lock is held in the current context.
    ...
    unsafe fn bump_exclusive(&self) {
        // SAFETY: The caller must ensure an exclusive lock is held in the current context.
...
pub unsafe trait RawRwLockUpgradeFair: RawRwLockUpgrade + RawRwLockFair {
    ...
    unsafe fn bump_upgradable(&self) {
        // SAFETY: The caller must ensure an upgradable lock is held in the current context.
...
impl<R: RawRwLock, T: ?Sized> RwLock<R, T> {
    ...
    pub unsafe fn force_unlock_read(&self) {
        // SAFETY: The caller must logically own a forgotten read guard, so a shared lock is held.
    ...
    pub unsafe fn force_unlock_write(&self) {
        // SAFETY: The caller must logically own a forgotten write guard, so an exclusive lock is held.
