...
impl<R: RawMutex, G: GetThreadId> RawReentrantMutex<R, G> {
    ...
    pub unsafe fn unlock(&self) {
        ...
        if lock_count == 0 {
            ...
            // SAFETY: The caller holds this reentrant mutex, and this is the outermost unlock.
...
impl<R: RawMutexFair, G: GetThreadId> RawReentrantMutex<R, G> {
    ...
    pub unsafe fn bump(&self) {
        if self.lock_count.get() == 1 {
            ...
            // SAFETY: With a single reentrant hold, the underlying mutex is held by this context.
...
impl<R: RawMutex, G: GetThreadId, T: ?Sized> ReentrantMutex<R, G, T> {
    ...
    pub unsafe fn force_unlock(&self) {
        // SAFETY: The caller must logically own a forgotten guard, so the mutex is locked.
