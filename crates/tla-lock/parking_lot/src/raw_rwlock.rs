...
unsafe impl lock_api::RawRwLockFair for RawRwLock {
    ...
    unsafe fn unlock_shared_fair(&self) {
        ...
        // SAFETY: The caller holds a shared lock, which is exactly the
        // precondition for the normal shared unlock path.
    ...
    unsafe fn bump_shared(&self) {
        if self.state.load(Ordering::Relaxed) & WRITER_BIT != 0 {
            // SAFETY: The caller holds a shared lock; the slow bump path
            // temporarily releases and reacquires that same shared lock.
...
impl RawRwLock {
    ...
    unsafe fn bump_shared_slow(&self) {
        // SAFETY: `bump_shared_slow` is called only while the caller holds a
        // shared lock, satisfying `unlock_shared`'s precondition.
        unsafe { self.unlock_shared() };
        self.lock_shared();
    }
