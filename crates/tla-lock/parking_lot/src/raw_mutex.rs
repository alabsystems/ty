...
unsafe impl lock_api::RawMutex for RawMutex {
    ...
    unsafe fn unlock(&self) {
        // SAFETY: The caller of `unlock` must hold this mutex, so this
        // releases the matching deadlock-tracking resource acquired on lock.
...
unsafe impl lock_api::RawMutexFair for RawMutex {
    ...
    unsafe fn unlock_fair(&self) {
        // SAFETY: The caller of `unlock_fair` must hold this mutex, so this
        // releases the matching deadlock-tracking resource acquired on lock.
        unsafe { deadlock::release_resource(self as *const _ as usize) };
        if self
            .state
