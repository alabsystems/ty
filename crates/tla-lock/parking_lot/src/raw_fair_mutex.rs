...
unsafe impl lock_api::RawMutex for RawFairMutex {
    ...
    unsafe fn unlock(&self) {
        // SAFETY: `RawMutex::unlock` requires the caller to hold the lock.
        // A fair unlock has the same precondition and only changes handoff policy.
...
unsafe impl lock_api::RawMutexFair for RawFairMutex {
    ...
    unsafe fn unlock_fair(&self) {
        // SAFETY: `RawFairMutex` delegates locking to the inner `RawMutex`, so
        // the caller's ownership guarantee applies to the inner mutex as well.
        unsafe { self.0.unlock_fair() }
    }

    #[inline]
    unsafe fn bump(&self) {
        // SAFETY: The caller holds this fair mutex; the inner `RawMutex`
        // represents the same lock state.
        unsafe { self.0.bump() }
    }
}
