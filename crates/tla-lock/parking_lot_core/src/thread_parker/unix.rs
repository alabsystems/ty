...
impl super::ThreadParkerT for ThreadParker {
    ...
    unsafe fn prepare_park(&self) {
        ...
        if !self.initialized.get() {
            // SAFETY: The caller guarantees this parker has not moved since any
            // previous pthread use.
            unsafe { self.init() };
    ...
    unsafe fn timed_out(&self) -> bool {
        ...
        // SAFETY: The `ThreadParkerT` contract guarantees this pthread mutex
        // remains at its initialized address while these methods are used.
    ...
    unsafe fn park(&self) {
        // SAFETY: The `ThreadParkerT` contract guarantees this pthread mutex
        // and condvar remain at their initialized addresses while parked.
        let r = unsafe { libc::pthread_mutex_lock(self.mutex.get()) };
        debug_assert_eq!(r, 0);
...
impl ThreadParker {
    ...
    unsafe fn init(&self) {
        ...
        // SAFETY: `attr` is uninitialized storage for pthread to initialize, and
        // `self.condvar` remains at its initialized address by the parker contract.
...
impl super::UnparkHandleT for UnparkHandle {
    ...
    unsafe fn unpark(self) {
        // SAFETY: `unpark_lock` created this handle while holding the parker
        // mutex, preventing the target `ThreadData` from exiting before unpark.
