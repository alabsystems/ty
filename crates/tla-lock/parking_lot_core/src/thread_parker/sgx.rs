...
use std::{
    ...
    os::fortanix_sgx::{
        ...
        usercalls::{
            self,
            raw::{Tcs, EV_UNPARK, WTY_INDEFINITE},
        },
    },
    thread,
};
...
impl super::ThreadParkerT for ThreadParker {
    ...
    unsafe fn park(&self) {
        while self.parked.load(Ordering::Acquire) {
            let result = usercalls::wait(EV_UNPARK, WTY_INDEFINITE);
    ...
    unsafe fn park_until(&self, timeout: Instant) -> bool {
        while self.parked.load(Ordering::Acquire) {
            ...
            let remaining_nanos =
                u128::min(remaining.as_nanos(), WTY_INDEFINITE as u128 - 1) as u64;
...
pub(crate) struct UnparkHandle(Tcs);
