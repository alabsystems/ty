...
impl KeyedEvent {
    ...
    #[allow(non_snake_case)]
    pub(super) fn create() -> Option<KeyedEvent> {
    ...
    #[inline]
    pub(super) fn prepare_park(&'static self, key: &AtomicUsize) {
        key.store(STATE_PARKED, Ordering::Relaxed);
    }

    #[inline]
    pub(super) fn timed_out(&'static self, key: &AtomicUsize) -> bool {
        key.load(Ordering::Relaxed) == STATE_TIMED_OUT
    }

    #[inline]
    pub(super) unsafe fn park(&'static self, key: &AtomicUsize) {
        let status = unsafe { self.wait_for(key as *const _ as *mut ffi::c_void, ptr::null_mut()) };
        debug_assert_eq!(status, STATUS_SUCCESS);
    }

    #[inline]
    pub(super) unsafe fn park_until(&'static self, key: &AtomicUsize, timeout: Instant) -> bool {
        let now = Instant::now();
        if timeout <= now {
    ...
    #[inline]
    pub(super) unsafe fn unpark_lock(&'static self, key: &AtomicUsize) -> UnparkHandle {
...
impl UnparkHandle {
    ...
    #[inline]
    pub(super) unsafe fn unpark(self) {
        if !self.key.is_null() {
