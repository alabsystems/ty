...
impl WaitAddress {
    #[allow(non_snake_case)]
    pub(super) fn create() -> Option<WaitAddress> {
    ...
    #[inline]
    pub(super) fn park(&'static self, key: &AtomicUsize) {
    ...
    #[inline]
    pub(super) fn park_until(&'static self, key: &AtomicUsize, timeout: Instant) -> bool {
        while key.load(Ordering::Acquire) != 0 {
            let now = Instant::now();
            if timeout <= now {
                return false;
    ...
    #[inline]
    pub(super) fn unpark_lock(&'static self, key: &AtomicUsize) -> UnparkHandle {
