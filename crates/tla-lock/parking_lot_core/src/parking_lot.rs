...
impl Bucket {
    #[inline]
    fn new(timeout: TimeoutInstant, seed: u32) -> Self {
        Self {
            mutex: WordLock::new(),
...
unsafe fn rehash_bucket_into(bucket: &'static Bucket, table: &mut HashTable) {
    ...
    while !current.is_null() {
        // SAFETY: The caller guarantees that `bucket` contains a valid queue of
        // live `ThreadData` nodes while it is being rehashed.
        ...
        if table.entries[hash].queue_tail.get().is_null() {
            ...
            // SAFETY: The destination bucket has a non-null tail which is part
            // of a valid queue built from live `ThreadData` nodes.
            unsafe {
        ...
        current_ref.next_in_queue.set(ptr::null());
        current = next;
    }
}
...
unsafe fn unlock_bucket_pair(bucket1: &Bucket, bucket2: &Bucket) {
    // SAFETY: The caller guarantees that `bucket1` is locked.
    unsafe { bucket1.mutex.unlock() };
    if !ptr::eq(bucket1, bucket2) {
        // SAFETY: Distinct bucket pairs are locked individually by the caller.
...
pub unsafe fn park(
    ...
    with_thread_data(|thread_data| {
        ...
        // SAFETY: `thread_data` is not moved while it is queued and parked.
        unsafe { thread_data.parker.prepare_park() };
        if !bucket.queue_head.get().is_null() {
            // SAFETY: A non-empty bucket queue has a non-null tail pointing to
            // a live `ThreadData` while the bucket lock is held.
        ...
        let unparked = match timeout {
            // SAFETY: The thread has been queued and the queue lock has been released.
            ...
            None => {
                ...
                // SAFETY: `thread_data` is still valid after returning from park.
        ...
        // SAFETY: The bucket lock is held while checking the parker timeout state.
        ...
        while !current.is_null() {
            // SAFETY: The bucket lock is held and the queue contains live
            // `ThreadData` nodes until they are unlinked.
            let current_ref = unsafe { &*current };
            if current == thread_data {
                ...
                if bucket.queue_tail.get() == current {
                    ...
                    let mut scan = next;
                    while !scan.is_null() {
                        // SAFETY: `scan` walks the same locked queue of live
                        // `ThreadData` nodes.
...
pub unsafe fn unpark_one(
    ...
    while !current.is_null() {
        // SAFETY: The bucket lock is held and `current` points into the bucket
        // queue, whose nodes remain live until removed and parker-locked.
        let current_ref = unsafe { &*current };
        if current_ref.key.load(Ordering::Relaxed) == key {
            ...
            // SAFETY: The bucket lock gives exclusive access to its fair-timeout state.
            result.be_fair = unsafe { (&mut *bucket.fair_timeout.get()).should_timeout() };
            ...
            // SAFETY: The target thread is still linked or just unlinked while
            // the bucket lock is held, so its `ThreadData` cannot be freed yet.
            let handle = unsafe { current_ref.parker.unpark_lock() };
            // SAFETY: We hold the lock here, as required
            unsafe { bucket.mutex.unlock() };
            // SAFETY: The queue lock has been released after obtaining the handle.
...
pub unsafe fn unpark_all(key: usize, unpark_token: UnparkToken) -> usize {
    ...
    while !current.is_null() {
        ...
        if current_ref.key.load(Ordering::Relaxed) == key {
            ...
            // up.
            // SAFETY: The target thread was just unlinked while the bucket lock
            // is held, so its `ThreadData` remains valid until the handle is used.
    ...
    for handle in threads.into_iter() {
        // SAFETY: Handles are used after the queue lock has been released.
        unsafe { handle.unpark() };
    }
...
pub unsafe fn unpark_requeue(
    ...
    while !current.is_null() {
        // SAFETY: The source bucket lock is held and `current` points into its
        // queue, whose nodes remain live while queued or parker-locked.
        let current_ref = unsafe { &*current };
        if current_ref.key.load(Ordering::Relaxed) == key_from {
                ...
                && wakeup_thread.is_none()
            {
                ...
                if !requeue_threads.is_null() {
                    // SAFETY: `requeue_threads_tail` is the tail of the
                    // temporary requeue list built from live `ThreadData` nodes.
            ...
            if op == RequeueOp::UnparkOne || op == RequeueOp::RequeueOne {
                ...
                let mut scan = next;
                while !scan.is_null() {
                    // SAFETY: `scan` walks the same locked source queue of live
                    // `ThreadData` nodes.
                    let scan_ref = unsafe { &*scan };
    ...
    if !requeue_threads.is_null() {
        // SAFETY: `requeue_threads_tail` is the non-null tail of the temporary
        // requeue list when `requeue_threads` is non-null.
        unsafe { (*requeue_threads_tail).next_in_queue.set(ptr::null()) };
        if !bucket_to.queue_head.get().is_null() {
            // SAFETY: A non-empty destination bucket has a valid non-null tail,
            // and both source and destination bucket locks are held.
            unsafe {
    ...
    if result.unparked_threads != 0 {
        // SAFETY: The source bucket lock gives exclusive access to its fair-timeout state.
        result.be_fair = unsafe { (&mut *bucket_from.fair_timeout.get()).should_timeout() };
    }
    ...
    if let Some(wakeup_thread) = wakeup_thread {
        // SAFETY: `wakeup_thread` was removed from the locked source bucket and
        // remains valid until its parker is locked for unparking.
        let wakeup_thread_ref = unsafe { &*wakeup_thread };
        wakeup_thread_ref.unpark_token.set(token);
        // SAFETY: Both bucket locks are still held, preventing the target
        // `ThreadData` from being freed before the handle is acquired.
        let handle = unsafe { wakeup_thread_ref.parker.unpark_lock() };
        // SAFETY: Both buckets are locked, as required.
        unsafe { unlock_bucket_pair(bucket_from, bucket_to) };
        // SAFETY: The bucket locks have been released after obtaining the handle.
        unsafe { handle.unpark() };
    } else {
        // SAFETY: Both buckets are locked, as required.
        unsafe { unlock_bucket_pair(bucket_from, bucket_to) };
...
pub unsafe fn unpark_filter(
    ...
    for t in threads.iter_mut() {
        // SAFETY: Each pointer was removed from the locked queue above and
        // remains valid until its parker is locked for unparking.
        let thread_ref = unsafe { &*t.0 };
        thread_ref.unpark_token.set(token);
        // SAFETY: The bucket lock is still held, preventing the target
        // `ThreadData` from being freed before the handle is acquired.
        t.1 = Some(unsafe { thread_ref.parker.unpark_lock() });
    }
    ...
    for (_, handle) in threads.into_iter() {
        // SAFETY: A handle was stored for every thread in `threads` above.
        let handle = unsafe { handle.unchecked_unwrap() };
        // SAFETY: Handles are used after the queue lock has been released.
        unsafe { handle.unpark() };
    }
...
pub mod deadlock {
    ...
    pub unsafe fn acquire_resource(_key: usize) {
        #[cfg(feature = "deadlock_detection")]
        {
            // SAFETY: This forwards the caller's contract that the resource has
            // just been acquired by the current thread.
            unsafe { deadlock_impl::acquire_resource(_key) };
        }
    }
    ...
    pub unsafe fn release_resource(_key: usize) {
        #[cfg(feature = "deadlock_detection")]
        {
            // SAFETY: This forwards the caller's contract that the resource is
            // still owned by the current thread and is about to be released.
            unsafe { deadlock_impl::release_resource(_key) };
        }
    }
    ...
    pub(super) unsafe fn on_unpark(_td: &super::ThreadData) {
        #[cfg(feature = "deadlock_detection")]
        {
            // SAFETY: The caller guarantees `_td` is the current parked thread's
            // live `ThreadData`.
            unsafe { deadlock_impl::on_unpark(_td) };
        }
    }
}
...
mod deadlock_impl {
    ...
    pub(super) unsafe fn on_unpark(td: &ThreadData) {
        if td.deadlock_data.deadlocked.get() {
            // SAFETY: The deadlock detector sets this sender while holding the
            // thread's bucket lock before unparking this exact `ThreadData`.
            ...
            // SAFETY: `td` is the current thread's `ThreadData` and has not moved
            // between parker calls.
            unsafe {
    ...
    pub(super) unsafe fn acquire_resource(key: usize) {
        with_thread_data(|thread_data| {
            // SAFETY: Only the current thread mutates its own resource list.
            unsafe { (*thread_data.deadlock_data.resources.get()).push(key) };
        });
    }

    pub(super) unsafe fn release_resource(key: usize) {
        with_thread_data(|thread_data| {
            // SAFETY: Only the current thread mutates its own resource list.
    ...
    pub(super) fn check_deadlock() -> Vec<Vec<DeadlockedThread>> {
        // SAFETY: The checking functions take the bucket locks needed to inspect
        // parked-thread queues and deadlock metadata.
        unsafe {
            // fast pass
    ...
    unsafe fn check_wait_graph_fast() -> bool {
        ...
        for b in &(*table).entries[..] {
            ...
            while !current.is_null() {
                // SAFETY: The bucket lock is held while traversing its queue, so
                // `current` points to a live parked `ThreadData`.
                let current_ref = unsafe { &*current };
                if !current_ref.parked_with_timeout.get()
                    && !current_ref.deadlock_data.deadlocked.get()
                {
                    // .resources are waiting for their owner
                    // SAFETY: The deadlock detector owns this read while the
                    // parked thread is stopped under the bucket lock.
                    let resources = unsafe { &*current_ref.deadlock_data.resources.get() };
                    ...
                    graph.add_edge(
                        current as usize,
                        current_ref.key.load(Ordering::Relaxed),
                        (),
                    );
                }
    ...
    unsafe fn check_wait_graph_slow() -> Vec<Vec<DeadlockedThread>> {
        ...
        for b in &table.entries[..] {
            let mut current = b.queue_head.get();
            while !current.is_null() {
                // SAFETY: All buckets in the current table are locked while
                // traversing their queues, so `current` points to live data.
                let current_ref = unsafe { &*current };
                if !current_ref.parked_with_timeout.get()
                    && !current_ref.deadlock_data.deadlocked.get()
                {
                    // .resources are waiting for their owner
                    // SAFETY: The parked thread cannot mutate its resource list
                    // while all bucket locks are held for the slow pass.
                    let resources = unsafe { &*current_ref.deadlock_data.resources.get() };
                    for &resource in resources {
                    ...
                    graph.add_edge(
                        Thread(current),
                        Resource(current_ref.key.load(Ordering::Relaxed)),
                        (),
                    );
                }
                current = current_ref.next_in_queue.get();
        ...
        for cycle in cycles {
            ...
            for td in cycle {
                // SAFETY: Cycle nodes are collected from locked wait-graph
                // traversal and refer to live parked `ThreadData` values.
                let td_ref = unsafe { &*td };
                let bucket = lock_bucket(td_ref.key.load(Ordering::Relaxed));
                td_ref.deadlock_data.deadlocked.set(true);
                // SAFETY: We hold the thread's bucket lock, so no other
                // detector path can mutate this sender slot concurrently.
                unsafe { *td_ref.deadlock_data.backtrace_sender.get() = Some(sender.clone()) };
                // SAFETY: The bucket lock is held, preventing `td_ref` from
                // being freed before the unpark handle is acquired.
                let handle = unsafe { td_ref.parker.unpark_lock() };
                // SAFETY: We hold the lock here, as required
                unsafe { bucket.mutex.unlock() };
                // unpark the deadlocked thread!
                // on unpark it'll notice the deadlocked flag and report back
                // SAFETY: The bucket lock has been released after obtaining the handle.
                unsafe { handle.unpark() };
            }
        ...
        // SAFETY: This function acquired the detector lock before the scan.
