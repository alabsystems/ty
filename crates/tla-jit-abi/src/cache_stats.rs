...
//! Pure-data compilation statistics for the native ABI.
//!
//! These structs capture compile-time and outcome counts for each per-action
//! native compilation pass. They are pure data and live here so `tla-check`
//! can hold `Option<CacheBuildStats>` fields without depending on compiler
//! implementation crates. Originally moved in Wave 16 Gate 1 Batches A/B and
//! landed via TL-W14 cherry-pick from TL-W13's preserved checkpoint
//! (Part of #4267 / #4291).

use std::fmt;
use std::time::Duration;

/// Per-action native compilation statistics.
///
...
/// trust-codegen implementation crate.
///
...
pub struct CompileStats {
    ...
    /// Wall-clock time for the native compilation pass.
...
/// without depending on compiler implementation crates.
///
...
pub struct CacheBuildStats {
    ...
    /// eligibility checks, not just native compilation).
