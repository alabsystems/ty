...
//! consumed by trust-codegen without creating a cargo cycle. They are pure data
//! structures — no compiler dependencies — which is why they belong in the
//! `tla-jit-abi` leaf crate.
//!
//! Part of #4267: the trust-codegen migration consolidated these descriptors in the
//! stable ABI leaf crate.
//!
//! # Overview
//!
...
///
/// Not marked `#[non_exhaustive]` because trust-codegen lowering and interpreter
/// fallback exhaustively pattern-match on this enum. Adding a new variant is
/// intentionally source-breaking for all consumers.
#[derive(Debug, Clone)]
