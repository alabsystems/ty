...
impl<T: Ord + Clone> TlaSet<T> {
    ...
    /// whose concrete element type was erased during code generation.
...
impl Value {
    ...
    ///
    /// Returns `None` if `self` is not a [`Value::Func`] or `key` is outside
    /// its domain.
...
/// Allow `for x in value` over a [`Value`] that is a set, sequence, or tuple.
///
/// Sets are materialized into a `Vec` (in sorted order) before iterating so
/// that the iterator type is uniform across variants.
///
/// # Panics
///
/// Panics if the `Value` is not a [`Value::Set`], [`Value::Seq`], or
/// [`Value::Tuple`]. Generated code only constructs this iterator in contexts
/// where the value is statically known to be iterable, so the panic signals a
/// codegen/type-inference bug rather than ordinary spec data.
...
/// Compute the powerset (`SUBSET S`): the set of all subsets of `set`.
///
/// The result has `2^|set|` elements, so the cardinality is bounded to keep
/// explicit-state model checking tractable.
///
/// # Panics
///
/// Panics if `set` has more than 20 elements, since the powerset of a larger
/// set (over a million subsets) is assumed to be a modelling error rather than
/// an intended computation.
...
/// Pick an element from a set (`CHOOSE` / `RandomElement`).
///
/// Despite the name, the choice is deterministic: it returns the smallest
/// element under the set's sort order. Determinism is required so that
/// model-checking runs are reproducible.
///
/// # Panics
///
/// Panics if `set` is empty; `RandomElement` (and `CHOOSE`) are undefined on
/// the empty set in TLA+.
