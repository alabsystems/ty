...
//! TLA+ Runtime Library
//!
//! Runtime support for Rust code generated from TLA+ specifications by the
//! `ty` toolchain. When `ty` compiles a spec to native Rust, the emitted code
//! does not stand alone: it links against this crate for the value types,
//! state-machine interface, and checking infrastructure that mirror TLA+
//! semantics. This crate is therefore the runtime half of `ty`'s
//! ahead-of-time code generation path, complementing the interpreted
//! evaluator and explicit-state checker in the other crates.
//!
//! # What it provides
//!
//! - [`StateMachine`] — the trait every generated spec implements, exposing
//!   `Init`/`Next` as `init`/`next` (plus allocation-free streaming variants)
//!   and optional invariant predicates.
//! - Runtime value types that reproduce TLA+ value semantics deterministically:
//!   [`TlaSet`], [`TlaFunc`], [`TlaRecord`], and the dynamically-typed
//!   [`Value`] used when codegen cannot infer a concrete Rust type.
//!   Helper constructors such as [`powerset`], [`range_set`],
//!   [`cartesian_product`], and [`func_merge`] implement the corresponding
//!   TLA+ operators.
//! - A breadth-first [`model_check`]er for verifying generated code, finding
//!   invariant violations and deadlocks in the reachable state space.
//! - A [`MonitoredStateMachine`] for checking invariants on every transition
//!   in production ("Layer 4" runtime monitoring).
//! - The [`temporal`] module, which checks trace-level temporal properties
//!   (`[]`, `<>`, `~>`, `WF`, `SF`) that cannot be evaluated on a single state.
//!
//! The [`prelude`] re-exports everything generated code commonly needs, along
//! with the [`tla_set!`] and [`tla_record!`] convenience macros.
//!
//! # Example
//!
//! ```rust
