...
use crate::{
    storage::{
        FingerprintAdmission, FingerprintSet, InMemoryFingerprintSet, ShardedFingerprintSet,
        StorageFault,
    },
...
///
/// Scoped worker threads otherwise inherit the platform default (~2 MiB on
/// macOS/Linux), far below the main thread's ~8 MiB. The per-state successor /
/// guard / partial-order machinery `run_parallel_worker` drives recurses with
/// depth and frame size that scale with the net (wide nets — hundreds of
/// places, thousands of transitions, e.g. NoC3x3-PT-8A: 317 places, 4293
/// transitions — overflow a 2 MiB worker stack). A stack overflow aborts the
/// WHOLE process (SIGABRT) instead of declining fail-closed, so we give each
/// worker a generous stack that comfortably exceeds the main thread's. The
/// reservation is virtual (lazily backed) — it costs no real memory until
/// touched. Fail-closed invariant: a worker must never abort the process.
const PARALLEL_BFS_WORKER_STACK_BYTES: usize = 64 * 1024 * 1024;
        ...
        for (worker_id, local_queue) in local_queues.into_iter().enumerate() {
            ...
            handles.push(
                std::thread::Builder::new()
                    .name(format!("mc-bfs-worker-{worker_id}"))
                    .stack_size(PARALLEL_BFS_WORKER_STACK_BYTES)
                    ...
                    .expect("spawn parallel BFS worker thread"),
            );
        }
    ...
    FS: FingerprintSet<TS::Fingerprint>,
{
    ...
    match storage.admit_fingerprint(fingerprint)? {
        FingerprintAdmission::Duplicate => return Ok(true),
    ...
    S: ParallelObserverSummary<TS>,
{
    let fingerprint = system.fingerprint(&state);
    match storage.admit_fingerprint(fingerprint)? {
        FingerprintAdmission::Duplicate => return Ok(()),
        FingerprintAdmission::New => {}
    }
...
mod tests {
    use super::*;
    use crate::{
        CompositeObserver, ExplorationObserver, FingerprintSet, InsertOutcome, NoopObserver,
        ParallelObserver, ParallelObserverSummary, StorageFault,
    };
