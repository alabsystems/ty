...
impl CasFingerprintSet {
    ...
    pub fn new(capacity: usize) -> Self {
        Self::try_new(capacity)
            .expect("CasFingerprintSet allocation failed; prefer try_new for recovery")
    }

    /// Fallible counterpart of [`Self::new`].
    ///
    /// Returns `Err(TryReserveError)` when the underlying CAS table cannot be
    /// reserved from the global allocator.
    ///
    /// # Panics
    ///
    /// Panics if capacity is 0.
    pub fn try_new(capacity: usize) -> Result<Self, TryReserveError> {
        ...
        table.extend((0..capacity).map(|_| AtomicU64::new(EMPTY)));
        Ok(Self {
...
impl PartitionedCasFingerprintSet {
    ...
    pub fn new(partition_bits: u32, total_capacity: usize) -> Self {
        Self::try_new(partition_bits, total_capacity)
            .expect("PartitionedCasFingerprintSet allocation failed; prefer try_new for recovery")
    }

    /// Fallible counterpart of [`Self::new`].
    ///
    /// Returns `Err(TryReserveError)` when any partition's underlying CAS
    /// table cannot be reserved from the global allocator. Sized correctly
    /// for the contest memory budget, this lets the caller surface
    /// `CANNOT_COMPUTE` instead of aborting with SIGABRT.
    ///
    /// # Panics
    ///
    /// Panics if `partition_bits` is 0 or > 16 — that is an API misuse, not
    /// an allocator failure.
    pub fn try_new(partition_bits: u32, total_capacity: usize) -> Result<Self, TryReserveError> {
        assert!(
        ...
        let partitions: Vec<CasFingerprintSet> = (0..num_partitions)
            .map(|_| CasFingerprintSet::try_new(per_partition))
            .collect::<Result<_, _>>()?;
...
impl FingerprintSet<u64> for PartitionedCasFingerprintSet {
    ...
    fn len(&self) -> usize {
        self.partitions.iter().map(FingerprintSet::len).sum()
    }
