...
macro_rules! t {
    ($name: ident, $strategy: ty) => {
        mod $name {
            ...
            // NOTE: Upstream arc-swap 1.9.1 shipped `storm_link_list_large`,
            // `storm_unroll_large`, and `load_parallel_large` as
            // high-iteration manual-run variants gated out of default test
            // runs. The ty org rule forbids gated test attributes
            // ( tests must
            // PASS, FAIL, or be DELETED. These are deleted; the `_small`
            // variants above exercise the same code paths in normal CI.
