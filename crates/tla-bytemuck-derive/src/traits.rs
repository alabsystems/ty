    ...
    fn asserts(input: &DeriveInput, crate_name: &TokenStream) -> Result<TokenStream> {
        ...
        match &input.data {
            Data::Struct(DataStruct { .. }) => {
                let assert_no_padding = generate_assert_no_padding(&input, None, "NoUninit")?;
        ...
        attr.parse_nested_meta(|meta| {
            if meta.path.is_ident(ATTR_NAME) {
                ...
                }) = value
                {
                    ...
                    bail!("Expected bytemuck `crate` attribute to be a string: `crate = \"...\"`",)
                }
            }
            Ok(())
        })
...
const GENERATED_TYPE_DOCUMENTATION: &str = " `bytemuck`-generated type for internal purposes only.";
