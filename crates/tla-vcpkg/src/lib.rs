...
mod tests {
    ...
    /// The sole raw process-environment mutation boundary for this test module.
    /// Every test holds `LOCK` for its full body before calling these helpers.
    #[allow(unknown_lints, env_mutation)]
    fn write_env(key: &std::ffi::OsStr, value: Option<&std::ffi::OsStr>) {
        match value {
    ...
    fn set_env(key: impl AsRef<std::ffi::OsStr>, value: impl AsRef<std::ffi::OsStr>) {
        write_env(key.as_ref(), Some(value.as_ref()));
    }

    fn remove_env(key: impl AsRef<std::ffi::OsStr>) {
        write_env(key.as_ref(), None);
    }
    ...
    fn do_nothing_for_unsupported_target() {
        let _g = LOCK.lock();
        set_env("VCPKG_ROOT", "/");
        set_env("TARGET", "x86_64-pc-windows-gnu");
    ...
    fn do_nothing_for_bailout_variables_set() {
        let _g = LOCK.lock();
        set_env("VCPKG_ROOT", "/");
        set_env("TARGET", "x86_64-pc-windows-msvc");
    ...
    fn dynamic_build_finds_lib() {
        let _g = LOCK.lock();
        clean_env();
        set_env("VCPKG_ROOT", vcpkg_test_tree_loc("no-status"));
        set_env("TARGET", "x86_64-pc-windows-msvc");
        set_env("VCPKGRS_DYNAMIC", "1");
        let tmp_dir = tempdir::TempDir::new("vcpkg_tests").unwrap();
        set_env("OUT_DIR", tmp_dir.path());
    ...
    fn handle_multiline_description() {
        let _g = LOCK.lock();
        clean_env();
        set_env("VCPKG_ROOT", vcpkg_test_tree_loc("multiline-description"));
        set_env("TARGET", "i686-pc-windows-msvc");
        set_env("VCPKGRS_DYNAMIC", "1");
        let tmp_dir = tempdir::TempDir::new("vcpkg_tests").unwrap();
        set_env("OUT_DIR", tmp_dir.path());
    ...
    fn custom_target_triplet_in_config() {
        ...
        clean_env();
        set_env("VCPKG_ROOT", vcpkg_test_tree_loc("normalized"));
        set_env("TARGET", "aarch64-apple-ios");
        set_env("VCPKGRS_DYNAMIC", "1");
        let tmp_dir = tempdir::TempDir::new("vcpkg_tests").unwrap();
        set_env("OUT_DIR", tmp_dir.path());
    ...
    fn custom_target_triplet_by_env_no_default() {
        ...
        clean_env();
        set_env("VCPKG_ROOT", vcpkg_test_tree_loc("normalized"));
        set_env("TARGET", "aarch64-apple-doesnotexist");
        set_env("VCPKGRS_DYNAMIC", "1");
        let tmp_dir = tempdir::TempDir::new("vcpkg_tests").unwrap();
        set_env("OUT_DIR", tmp_dir.path());
        ...
        set_env("VCPKGRS_TRIPLET", "x64-osx");
    ...
    fn pc_files_reordering() {
        ...
        clean_env();
        set_env("VCPKG_ROOT", vcpkg_test_tree_loc("normalized"));
        set_env("TARGET", "x86_64-unknown-linux-gnu");
        // env::set_var("VCPKGRS_DYNAMIC", "1");
        let tmp_dir = tempdir::TempDir::new("vcpkg_tests").unwrap();
        set_env("OUT_DIR", tmp_dir.path());
    ...
    fn clean_env() {
        remove_env("TARGET");
        remove_env("VCPKG_ROOT");
        remove_env("VCPKGRS_DYNAMIC");
        remove_env("RUSTFLAGS");
        remove_env("CARGO_CFG_TARGET_FEATURE");
        remove_env("VCPKGRS_DISABLE");
        remove_env("VCPKGRS_NO_LIBMYSQL");
        remove_env("VCPKGRS_TRIPLET");
    }
