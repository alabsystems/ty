...
use crate::nodes::hamt_chunk::{HamtChunk, Iter as ChunkIter, IterMut as ChunkIterMut};
...
// The trailing_zeros-based `HamtChunk` (TY divergence from upstream
// `SparseChunk`) is fixed at 32 slots; the HAMT level width must match.
const _: () = assert!(HASH_WIDTH == crate::nodes::hamt_chunk::CAPACITY);

pub(crate) fn hash_key<K: Hash + ?Sized, S: BuildHasher>(bh: &S, key: &K) -> HashBits {
    let mut hasher = bh.build_hasher();
    key.hash(&mut hasher);
    let hash = hasher.finish();
    // TY divergence from upstream (`hasher.finish() as HashBits`, a plain
    // low-half truncation): fold the high half into the low half. The HAMT
    // consumes the hash five bits at a time starting from the LSB, and
    // multiply-based hashers (the fork's default FxHash) concentrate their
    // avalanche in the high bits, so a plain truncation would keep the
    // weakest 32 bits. Purely an internal tree-shape concern — key collisions
    // at the full HashBits width are handled by `CollisionNode` regardless.
    ((hash >> 32) ^ hash) as HashBits
}
...
pub(crate) struct Node<A> {
    data: HamtChunk<Entry<A>>,
}

// The `pool` feature is unsupported in this fork (see the threadsafe
// compile_error! in lib.rs), so `PoolDefault`/`PoolClone` are marker-only:
// the pool-specific uninit constructors upstream gates behind
// `#[cfg(feature = "pool")]` are permanently compiled out.
...
impl<A> Node<A> {
    ...
    pub(crate) fn unit(index: usize, value: Entry<A>) -> Self {
        Node {
            data: HamtChunk::unit(index, value),
        }
    }
    ...
    pub(crate) fn pair(index1: usize, value1: Entry<A>, index2: usize, value2: Entry<A>) -> Self {
        Node {
            data: HamtChunk::pair(index1, value1, index2, value2),
        }
    }
    ...
    pub(crate) fn single_child(pool: &Pool<Node<A>>, index: usize, node: Self) -> Self {
        Node {
            data: HamtChunk::unit(index, Entry::from_node(pool, node)),
        }
    }
