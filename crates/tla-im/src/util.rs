...
// `Ref` == `Rp` when threadsafe (TY divergence from upstream `Arc`).
//
// `tla_rp::Rp<A>` is an `Arc<A>` drop-in (`Send + Sync`, same API surface used
// here: `new`/`from`/`make_mut`/`ptr_eq`/`try_unwrap`/`Deref`) whose refcount
// ops are atomic by default — byte-identical behaviour to `Arc` — and become
// non-atomic only while the model checker's process-global single-threaded
// mode flag is set (see tla-rp's safety model). This removes the atomic
// `ldadd` on every HAMT/B-tree/RRB node clone/drop in single-threaded runs.
#[cfg(threadsafe)]
pub(crate) type Ref<A> = tla_rp::Rp<A>;
