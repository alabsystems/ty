...
// TY divergence from upstream `std::sync::Arc`: the threadsafe pool ref is
// tla-rp's `Rp` — an `Arc` drop-in whose refcount ops are atomic by default
// (byte-identical to `Arc`) and non-atomic only inside the model checker's
// provably single-threaded phase (same process-global flag as `util::Ref`).
// `PoolRef` fronts every Vector/OrdMap chunk allocation, so this is the
// hottest refcount in the crate.
use tla_rp::Rp as RArc;
