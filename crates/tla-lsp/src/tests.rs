...
// ───────────────────────────────────────────────────────────
// Position conversion: multi-byte (UTF-8) and boundary behaviour
// ───────────────────────────────────────────────────────────

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_position_roundtrip_with_multibyte_chars() {
    // `α` is 2 bytes in UTF-8 and `€` is 3 bytes. The conversions must operate
    // on byte offsets (Span uses byte offsets) while LSP columns count UTF-8
    // scalar values per char, so the byte arithmetic must use char widths.
    let source = "αβ\n€x\ny";

    // Line 1 (`€x`) starts immediately after `αβ\n` = 2+2+1 = 5 bytes.
    let line1_start = position::offset_to_position(source, 5);
    assert_eq!(line1_start.line, 1, "byte 5 is the start of line 1");
    assert_eq!(line1_start.character, 0);

    // The `x` after `€` (3 bytes) is at byte 5+3 = 8, column 1 on line 1.
    let x_pos = position::offset_to_position(source, 8);
    assert_eq!(x_pos.line, 1);
    assert_eq!(
        x_pos.character, 1,
        "column counts chars, not bytes: `€` is one column"
    );

    // position_to_offset is the inverse and must land back on the same byte.
    assert_eq!(
        position::position_to_offset(source, Position::new(1, 0)),
        5,
        "line 1 col 0 must map back to byte 5"
    );
    assert_eq!(
        position::position_to_offset(source, Position::new(1, 1)),
        8,
        "line 1 col 1 (the `x`) must map back to byte 8, not byte 6"
    );

    // Line 2 (`y`) starts after `αβ\n€x\n` = 5 + 3 + 1 + 1 = 10 bytes.
    assert_eq!(
        position::position_to_offset(source, Position::new(2, 0)),
        10
    );
}
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_offset_to_position_at_and_past_end() {
    let source = "ab\ncd";
    // Offset exactly at end-of-buffer (len) → last line, one past last char.
    let end = position::offset_to_position(source, source.len() as u32);
    assert_eq!(end.line, 1);
    assert_eq!(
        end.character, 2,
        "offset == len lands one past the last char"
    );

    // An offset well past the end must clamp to end without panicking.
    let past = position::offset_to_position(source, 1000);
    assert_eq!(past.line, 1);
    assert_eq!(past.character, 2);
}
...
fn test_position_to_offset_clamps_out_of_range() {
    let source = "ab\ncd";
    // An over-large column does not run off the end: the column scan iterates
    // `source[line_start..]` and breaks when exhausted, clamping to buffer end
    // (5 bytes) rather than panicking. (Note: the scan does not stop at the
    // newline — it trusts the client to send in-range columns.)
    let off = position::position_to_offset(source, Position::new(0, 99));
    assert_eq!(
        off,
        source.len() as u32,
        "over-large column clamps to buffer end"
    );

    // A line past the last line: the loop never matches `current_line`, so the
    // offset walks the whole buffer and the trailing column scan adds nothing.
    let off = position::position_to_offset(source, Position::new(99, 0));
    assert_eq!(off, source.len() as u32);
}
...
fn test_span_to_range_multiline() {
    use tla_core::FileId;
    let source = "abc\ndef\nghi";
    // Span covering "c\nd" — crosses a line boundary.
    let span = tla_core::Span::new(FileId(0), 2, 5);
    let range = position::span_to_range(source, span);
...
// ───────────────────────────────────────────────────────────
// Completions: transitive EXTENDS, stdlib constants, detail formatting
// ───────────────────────────────────────────────────────────

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_stdlib_operator_completions_transitive_extends() {
    // EXTENDS Reals must pull in the full chain Reals → Integers → Naturals and
    // expose each module's synthetic constants exactly once.
    let mut doc = DocumentState::new("\n---- MODULE R ----\nEXTENDS Reals\n====\n".to_string());
    doc.analyze();
    let module = doc.module.as_ref().expect("Reals module should resolve");
    let completions = completions::get_stdlib_operator_completions(module);
    let labels: Vec<_> = completions.iter().map(|c| c.label.as_str()).collect();

    for constant in &["Nat", "Int", "Real", "Infinity"] {
        assert!(
            labels.contains(constant),
            "transitive EXTENDS Reals should expose `{}`, got: {:?}",
            constant,
            labels
        );
    }

    // Each synthetic constant must appear exactly once (the `seen` set prevents
    // re-processing Naturals via both Integers and Reals).
    let nat_count = labels.iter().filter(|l| **l == "Nat").count();
    assert_eq!(
        nat_count, 1,
        "`Nat` must not be duplicated by transitive extends"
    );

    // The synthetic constants carry CONSTANT kind, not FUNCTION.
    let real = completions
        .iter()
        .find(|c| c.label == "Real")
        .expect("Real present");
    assert_eq!(real.kind, Some(CompletionItemKind::CONSTANT));
}

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_stdlib_operator_completions_arity_detail() {
    // Sequences has positive-arity operators; the detail must read "<mod> (arity N)".
    let mut doc = DocumentState::new("\n---- MODULE S ----\nEXTENDS Sequences\n====\n".to_string());
    doc.analyze();
    let module = doc
        .module
        .as_ref()
        .expect("Sequences module should resolve");
    let completions = completions::get_stdlib_operator_completions(module);

    let append = completions
        .iter()
        .find(|c| c.label == "Append")
        .expect("Append should be an exposed Sequences operator");
    assert_eq!(append.kind, Some(CompletionItemKind::FUNCTION));
    assert_eq!(
        append.detail.as_deref(),
        Some("Sequences (arity 2)"),
        "positive-arity operator detail should report its arity"
    );
}

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_stdlib_operator_completions_constant_arity_detail() {
    // Bags::EmptyBag has arity 0, exercising the `(constant)` detail branch.
    let mut doc = DocumentState::new("\n---- MODULE B ----\nEXTENDS Bags\n====\n".to_string());
    doc.analyze();
    let module = doc.module.as_ref().expect("Bags module should resolve");
    let completions = completions::get_stdlib_operator_completions(module);

    let empty_bag = completions
        .iter()
        .find(|c| c.label == "EmptyBag")
        .expect("EmptyBag should be an exposed Bags operator");
    assert_eq!(
        empty_bag.detail.as_deref(),
        Some("Bags (constant)"),
        "arity-0 operator detail should read `(constant)`"
    );
}

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_stdlib_operator_completions_empty_without_extends() {
    // A module that EXTENDS nothing yields no stdlib operator completions.
    let mut doc = DocumentState::new(
        "\n---- MODULE NoExt ----\nVARIABLE x\nInit == x = 0\n====\n".to_string(),
    );
    doc.analyze();
    let module = doc.module.as_ref().expect("module should resolve");
    let completions = completions::get_stdlib_operator_completions(module);
    assert!(
        completions.is_empty(),
        "no EXTENDS means no stdlib operator completions, got: {:?}",
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_local_symbol_completions_kind_and_detail() {
    // TEST_MODULE has Inc(n) (arity-1 operator → FUNCTION + "arity 1"),
    // Init/Next/Spec (arity-0 operators → CONSTANT + no detail),
    // x/y (VARIABLE + no detail) and N (CONSTANT + no detail).
    let mut doc = DocumentState::new(TEST_MODULE.to_string());
    doc.analyze();
    let resolve = doc.resolve.as_ref().expect("TEST_MODULE resolves");
    let completions = completions::get_local_symbol_completions(resolve);
    ...
    let inc = completions
        ...
        .find(|c| c.label == "Inc")
        .expect("Inc present");
    assert_eq!(inc.kind, Some(CompletionItemKind::FUNCTION));
    assert_eq!(inc.detail.as_deref(), Some("arity 1"));

    let init = completions
        .iter()
        .find(|c| c.label == "Init")
        .expect("Init present");
    assert_eq!(
        ...
        "a nullary operator should complete as CONSTANT"
    );
    assert_eq!(init.detail, None, "nullary operators carry no arity detail");
    ...
    let n = completions
        ...
        .find(|c| c.label == "N")
        .expect("N present");
    assert_eq!(n.kind, Some(CompletionItemKind::CONSTANT));
}

// ───────────────────────────────────────────────────────────
// Symbol lookups: misses must be None (no panics, no false hits)
// ───────────────────────────────────────────────────────────

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_symbol_lookups_return_none_off_symbol() {
    let mut doc = DocumentState::new(TEST_MODULE.to_string());
    doc.analyze();
    let resolve = doc.resolve.as_ref().expect("TEST_MODULE resolves");

    // The `====` terminator is inside the module text but on no symbol or
    // reference (and outside every dummy stdlib span).
    let term = doc.source.rfind("====").expect("terminator present") as u32;
    assert_eq!(symbols::find_symbol_at_position(resolve, term), None);
    assert_eq!(
        symbols::find_definition_span_at_position(resolve, term),
        None
    );
    assert_eq!(symbols::get_hover_info(resolve, term), None);
}

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_symbol_lookups_out_of_bounds_offset() {
    let mut doc = DocumentState::new(TEST_MODULE.to_string());
    doc.analyze();
    let resolve = doc.resolve.as_ref().expect("TEST_MODULE resolves");

    // An offset far past the source must not panic and must find nothing.
    let huge = 1_000_000u32;
    assert_eq!(symbols::find_symbol_at_position(resolve, huge), None);
    assert_eq!(
        symbols::find_definition_span_at_position(resolve, huge),
        None
    );
    assert_eq!(symbols::get_hover_info(resolve, huge), None);
    assert!(symbols::find_references_to_def(
        resolve,
        tla_core::Span::new(tla_core::FileId(0), huge, huge + 1)
    )
    .is_empty());
}
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_find_definition_span_on_definition_itself() {
    // Operators are not in `references` keyed by their own def span, so
    // find_definition_span_at_position must fall through to the symbol table.
    let mut doc = DocumentState::new(TEST_MODULE.to_string());
    doc.analyze();
    let resolve = doc.resolve.as_ref().expect("TEST_MODULE resolves");
    ...
    let inc = resolve
        ...
        .find(|s| s.name == "Inc")
        .expect("Inc defined");
    // Clicking on the Inc definition name returns the definition span itself.
    let found = symbols::find_definition_span_at_position(resolve, inc.def_span.start + 1);
    assert_eq!(found, Some(inc.def_span));
}

// ───────────────────────────────────────────────────────────
// Hover formatting: parameterised operators and constants
// ───────────────────────────────────────────────────────────

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_hover_info_includes_arity_for_parameterised_operator() {
    let mut doc = DocumentState::new(TEST_MODULE.to_string());
    doc.analyze();
    let resolve = doc.resolve.as_ref().expect("TEST_MODULE resolves");
    ...
    let inc = resolve
        ...
        .expect("Inc defined");
    let info =
        symbols::get_hover_info(resolve, inc.def_span.start + 1).expect("Inc has hover info");
    assert!(
        info.contains("OPERATOR") && info.contains("Inc") && info.contains("arity 1"),
        "hover for Inc(n) should report OPERATOR Inc (arity 1), got: {}",
        info
    );
}
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_hover_info_constant_has_no_arity_suffix() {
    let mut doc = DocumentState::new(TEST_MODULE.to_string());
    doc.analyze();
    let resolve = doc.resolve.as_ref().expect("TEST_MODULE resolves");
    ...
    let n = resolve
        ...
        .find(|s| s.name == "N")
        .expect("N defined");
    let info = symbols::get_hover_info(resolve, n.def_span.start + 1).expect("N has hover info");
    assert_eq!(
        info, "CONSTANT N",
        "a nullary CONSTANT hover should be exactly `CONSTANT N` with no arity"
    );
}

// ───────────────────────────────────────────────────────────
// Semantic analysis: duplicate definitions and INSTANCE-driven leniency
// ───────────────────────────────────────────────────────────

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_semantic_diagnostics_duplicate_definition() {
    let source = "\n---- MODULE Dup ----\nVARIABLE x\nVARIABLE x\n====\n";
    let mut doc = DocumentState::new(source.to_string());
    doc.analyze();
    ...
    let dups: Vec<_> = doc
        .semantic_diagnostics
        .iter()
        .filter(|d| d.kind == SemanticDiagnosticKind::DuplicateDefinition)
        .collect();
    assert_eq!(
        dups.len(),
        1,
        "redeclaring VARIABLE x should yield exactly 1 duplicate diagnostic, got: {:?}",
        doc.semantic_diagnostics
            .iter()
    ...
    assert!(
        ...
        "duplicate diagnostic should mention `x`, got: {}",
        dups[0].message
    );
}
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_semantic_diagnostics_instance_makes_undefined_possibly() {
    // A bare INSTANCE unit (no EXTENDS) sets `has_instance`, so unresolved names
    // are downgraded to PossiblyUndefined rather than a hard UndefinedName.
    let source =
        "\n---- MODULE Inst ----\nVARIABLE x\nINSTANCE Naturals\nInit == x = Bogus\n====\n";
    let mut doc = DocumentState::new(source.to_string());
    doc.analyze();
    ...
    assert!(
        doc.module.is_some(),
        "bare INSTANCE module should lower/resolve"
    );
    ...
    let possibly: Vec<_> = doc
        ...
        .collect();
    assert!(
        possibly.iter().any(|d| d.message.contains("Bogus")),
        "with a bare INSTANCE, `Bogus` should be PossiblyUndefined, got: {:?}",
        doc.semantic_diagnostics
            .iter()
            .map(|d| (d.kind, &d.message))
            .collect::<Vec<_>>()
    ...
    assert!(
        hard.is_empty(),
        "INSTANCE present should suppress hard UndefinedName, got: {:?}",
        hard.iter().map(|d| &d.message).collect::<Vec<_>>()
    );
}
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_semantic_diagnostics_stdlib_symbols_not_flagged_unused() {
    // EXTENDS Naturals injects synthetic symbols (e.g. Nat) with a dummy span.
    // These must never be reported as unused declarations even when unreferenced.
    let source =
        "\n---- MODULE StdlibUnused ----\nEXTENDS Naturals\nVARIABLE x\nInit == x = 0\n====\n";
    let mut doc = DocumentState::new(source.to_string());
    doc.analyze();
    ...
    let unused: Vec<_> = doc
        ...
        .collect();
    assert!(
        unused.iter().all(|d| !d.message.contains("Nat")),
        "injected stdlib symbols (dummy span) must not be flagged unused, got: {:?}",
        unused.iter().map(|d| &d.message).collect::<Vec<_>>()
    );
    // x is referenced; nothing user-declared is unused here.
    assert!(
        unused.is_empty(),
        "no user declaration is unused, got: {:?}",
        unused.iter().map(|d| &d.message).collect::<Vec<_>>()
    );
}

// ───────────────────────────────────────────────────────────
// SymbolInformation construction (workspace-symbol building block)
// ───────────────────────────────────────────────────────────

#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_mk_symbol_information_field_mapping() {
    let uri = Url::parse("file:///spec.tla").expect("valid uri");
    let range = Range::new(Position::new(2, 4), Position::new(2, 7));
    let loc = Location::new(uri.clone(), range);
    let info = symbols::mk_symbol_information(
        "Foo".to_string(),
        SymbolKind::FUNCTION,
        loc.clone(),
    ...
    assert_eq!(info.container_name.as_deref(), Some("MyModule"));
