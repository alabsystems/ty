...
fn test_wrappers() {
    fn set(name: &str, value: Option<bool>) {
        match value {
            Some(true) => support::env_guard::env_set(name, "/usr/bin/env"),
            Some(false) => support::env_guard::env_set(name, "/bin/false"),
            None => support::env_guard::env_remove(name),
        }
    }
    ...
    support::env_guard::env_set("RUSTC_WRAPPER", "./tests/wrap_ignored");
    support::env_guard::env_set("RUSTC_WORKSPACE_WRAPPER", "/bin/false");
    let ac = autocfg::AutoCfg::with_dir(out.as_ref()).unwrap();
