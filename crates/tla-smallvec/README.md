...
TY fork of the [`smallvec`](https://github.com/servo/rust-smallvec) crate.
...
TY is moving toward a zero-external-dependency posture (Tier 3 of the
supremacy plan). `smallvec` is small, stable, and touches many hot paths
(`tla-value`, `tla-eval`, `tla-check`, and trust-cg-facing data paths).
Forking it lets us own the full stack without giving up the SSO
...
`LICENSE-APACHE`). TY modifications (if any) are licensed under the same
...
Features dropped vs upstream (not used in TY):
