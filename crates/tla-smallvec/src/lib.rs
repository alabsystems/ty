...
///
/// ```
/// # use tla_smallvec::{smallvec, SmallVec};
/// # fn main() {
...
///
/// ```
/// # use tla_smallvec::{smallvec_inline, SmallVec};
/// # fn main() {
...
macro_rules! debug_unreachable {
    ...
    ($e:expr) => {
        if cfg!(debug_assertions) {
            ...
            // SAFETY: This private macro is used only for SmallVecData variant states
            // that are ruled out by the caller's unsafe accessor contract.
            unsafe {
...
///
/// ```rust
/// use tla_smallvec::{ExtendFromSlice, SmallVec};
///
...
unsafe fn deallocate<T>(ptr: NonNull<T>, capacity: usize) {
    ...
    // SAFETY: Callers pass a non-null pointer allocated by the global allocator
    // with the same element type and capacity, so the reconstructed layout
...
impl<A: Array> SmallVecData<A> {
    ...
    unsafe fn inline(&self) -> ConstNonNull<A::Item> {
        // SAFETY: The caller guarantees the inline union field is active.
        let ptr = unsafe { self.inline.as_ptr() };
        ConstNonNull::new(ptr as *const A::Item).unwrap()
    }
    #[inline]
    unsafe fn inline_mut(&mut self) -> NonNull<A::Item> {
        // SAFETY: The caller guarantees the inline union field is active.
        let ptr = unsafe { self.inline.as_mut_ptr() };
        NonNull::new(ptr as *mut A::Item).unwrap()
    }
    #[inline]
    ...
    unsafe fn into_inline(self) -> MaybeUninit<A> {
        // SAFETY: The caller guarantees the inline union field is active.
        unsafe { core::mem::ManuallyDrop::into_inner(self.inline) }
    }
    #[inline]
    unsafe fn heap(&self) -> (ConstNonNull<A::Item>, usize) {
        // SAFETY: The caller guarantees the heap union field is active.
        let (ptr, len) = unsafe { self.heap };
        (ConstNonNull(ptr), len)
    }
    #[inline]
