...
//! # tla-dialect — verification-observability dialect for TY
//!
//! This crate provides the `verif::` verification-primitive dialect and its
//! terminal [`LlvmLeaf`] boundary. Its live role is **observability**: the
//! `tla-check` BFS worker loop constructs, [`DialectOp::verify`]s, and lowers
//! `verif::` ops to structured [`LlvmLeaf`] records under the
//! `TY_DIALECT_TRACE=1` env hook, giving a dialect-shaped trace of the model
//! checker's inner loop (seed/expand steps, fingerprint batches, frontier
//! drains, invariant/enabled checks).
//!
//! The crate defines the small op **framework** the `verif::` dialect is built
//! on — the [`DialectOp`] trait, the [`Dialect`] trait, the [`Lowered`]
//! lowering result, the [`OpKind`] classifier, and the [`DialectError`] error
//! type — plus the capability-interface registry in [`interface`] (`Pure`,
//! `HasParallelism`, `DivergenceClass`) used for GPU-eligibility queries.
//!
//! # Ops and boundary
//!
//! ```text
//!     verif:: (verification primitives: VerifBfsStep, VerifStateFingerprint,
//!              VerifFingerprintBatch, VerifFrontierDrain, ...)
//!       |  DialectOp::lower
//!       v
//!     LlvmLeaf (terminal, structured record — consumed by trust-cg / TY_DIALECT_TRACE)
//! ```
//!
//! # Scope
//!
//! - The `verif::` ops and their structured [`LlvmLeaf`] lowerings.
//! - The op framework traits + the capability-interface registry.
//! - No trust-ir or LLVM IR emission; the terminal boundary is [`LlvmLeaf`].
//!
//! The former `tla::` surface dialect and its `tla → verif → llvm` pass tower
//! were a skeleton with no production callers and have been removed; the
//! production lowering path lives in `trust-ir` / `trust-cg`.
...
pub enum DialectError {
    ...
    VerifyFailed {
        /// Dialect name of the op that failed verification (e.g. `"tla"`).
        dialect: &'static str,
        /// Mnemonic of the failing op (e.g. `"tla.set_union"`).
        op: &'static str,
        /// Human-readable description of which structural invariant was
        /// violated.
        message: String,
    },
    ...
    LoweringFailed {
        /// Dialect name of the op that could not be lowered.
        dialect: &'static str,
        /// Mnemonic of the op that could not be lowered.
        op: &'static str,
        /// Human-readable description of why lowering failed (e.g. an operand
        /// produced the wrong `Lowered` shape, or the op is a stub).
        message: String,
    },
    /// A pass was asked to consume ops from the wrong dialect.
    #[error("pass expected dialect `{expected}` but received op from `{actual}`")]
    DialectMismatch {
        /// Dialect the pass was configured to consume (its `from_dialect()`).
        expected: &'static str,
        /// Dialect of the op actually handed to the pass.
        actual: &'static str,
    },
    ...
    NotImplemented {
        /// Dialect name of the unimplemented op.
        dialect: &'static str,
        /// Mnemonic of the op whose lowering is still a skeleton stub.
        op: &'static str,
    },
}
...
/// concrete backend (initially `tla-trust_cg`, future: GPU shader emitter, WASM,
/// etc.).
///
...
///    `TY_DIALECT_TRACE=1`) constructs and [`DialectOp::verify`]s it on the
///    live code path.
///
...
mod tests {
    ...
    #[test]
    fn dialect_error_display_covers_all_variants() {
        // Only `VerifyFailed`'s Display was previously asserted. Each variant
        // has a distinct `#[error(...)]` format string that is public-facing
        // (surfaced to users / logs); pin the salient tokens of each so a
        // future reword of a format string is caught.
        let mismatch = DialectError::DialectMismatch {
            expected: "verif",
            actual: "tla",
        };
        let m = format!("{mismatch}");
        assert!(m.contains("expected dialect `verif`"), "got: {m}");
        assert!(m.contains("received op from `tla`"), "got: {m}");

        let not_impl = DialectError::NotImplemented {
            dialect: "tla",
            op: "tla.state",
        };
        let n = format!("{not_impl}");
        assert!(n.contains("tla"), "got: {n}");
        assert!(n.contains("tla.state"), "got: {n}");
        assert!(n.contains("not yet implemented"), "got: {n}");

        let lowering = DialectError::LoweringFailed {
            dialect: "verif",
            op: "verif.bfs_step",
            message: "no concrete backend".into(),
        };
        let l = format!("{lowering}");
        assert!(l.contains("verif"), "got: {l}");
        assert!(l.contains("verif.bfs_step"), "got: {l}");
        assert!(l.contains("lowering failed"), "got: {l}");
        assert!(l.contains("no concrete backend"), "got: {l}");
    }

    /// Minimal dialect using the *default* `fold` hook (no override).
    struct DefaultFoldDialect;
    impl Dialect for DefaultFoldDialect {
        fn name(&self) -> &'static str {
            "df"
        }
        fn op_names(&self) -> &'static [&'static str] {
    ...
    /// Dialect that overrides `fold` to rewrite any `verif.scalar_int` op into
    /// a fixed `verif.scalar_bool`.
    struct RewritingDialect;
    impl Dialect for RewritingDialect {
        fn name(&self) -> &'static str {
            "rw"
        }
        fn op_names(&self) -> &'static [&'static str] {
        ...
        fn fold(&self, op: &dyn DialectOp) -> Option<Box<dyn DialectOp>> {
            // Rewrite only `verif.scalar_int` ops; leave everything else alone.
            if op.op_name() == "verif.scalar_int" {
                Some(Box::new(crate::verif::VerifScalarBool::new(true)))
            } else {
                None
            }
        }
    ...
    #[test]
    fn dialect_fold_default_hook_returns_none() {
        // The trait's documented default: `fold` is a no-op returning None for
        // every op. Confirm a dialect that doesn't override it never rewrites.
        let d = DefaultFoldDialect;
        let op = crate::verif::VerifScalarInt::new(0);
        assert!(d.fold(&op).is_none());
        let op2 = crate::verif::VerifScalarBool::new(false);
        assert!(d.fold(&op2).is_none());
    }

    #[test]
    fn dialect_fold_override_rewrites_matching_op_only() {
        // A dialect that overrides `fold` rewrites the ops it matches and
        // leaves the rest unchanged (returns None). This is the contract that
        // per-dialect rewriting relies on.
        let d = RewritingDialect;
        // Matching op -> Some(replacement).
        let target = crate::verif::VerifScalarInt::new(7);
        let folded = d.fold(&target).expect("scalar_int should be folded");
        assert_eq!(folded.op_name(), "verif.scalar_bool");
        assert_eq!(folded.dialect(), "verif");
        // Non-matching op -> None (left unchanged).
        let other = crate::verif::VerifScalarBool::new(true);
        assert!(d.fold(&other).is_none());
    }
