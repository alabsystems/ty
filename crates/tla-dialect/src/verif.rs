...
//!     for in mail trust_cg#4268. Graduated Wave 14 TL3 to
//!     [`LlvmLeaf::FrontierDrain`].
//!   - [`VerifFingerprintBatch`] — batched fingerprint computation over a
//!     slab of states. Pins the namespace asked for in mail trust_cg#4268.
...
//!
//! Migration of the real `tla-trust_cg` emit paths into these ops is follow-up
...
///   BFS worker loop via the [`crate`] `TY_DIALECT_TRACE=1` env hook.
...
/// variable-reference lowering. The surface name of the variable is
...
pub struct VerifScalarInt {
    /// The integer literal value. TLA+ integers are unbounded; `i64` is the
    /// representable subset at the `verif::` boundary.
    pub value: i64,
}
...
pub struct VerifScalarBool {
    /// The boolean literal value (`true` = TLA+ `TRUE`, `false` = `FALSE`).
    pub value: bool,
}
...
///   both fields; wired into the BFS worker loop `TY_DIALECT_TRACE=1` path
...
///   every field; wired into the BFS worker loop `TY_DIALECT_TRACE=1`
...
///   `TY_DIALECT_TRACE=1` path via `DialectTracer::emit_frontier_drain`.
#[derive(Debug, Clone, Copy)]
...
/// asked for in mail trust_cg#4268.
///
/// # Fields
///
...
///   `TY_DIALECT_TRACE=1` path via `DialectTracer::emit_fingerprint_batch`.
#[derive(Debug, Clone, Copy)]
