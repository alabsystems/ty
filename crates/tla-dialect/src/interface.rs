...
//!
//! Ported from MLIR's `OpInterface` concept ("Idea 4: Op Interfaces").
//! Instead of passes enumerating op kinds
...
/// not implement `HasCapabilities`. Pass-level queries go through generic
/// free functions that take a concrete op by type parameter — e.g.
/// [`op_is_gpu_eligible`], which calls `op.capabilities()` and applies the
/// [`is_gpu_eligible`] predicate.
