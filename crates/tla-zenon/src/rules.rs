...
pub enum RuleResult {
    ...
    Alpha {
        /// The proof rule that justifies this decomposition (records the
        /// principal formula so the step can be reconstructed in a proof tree).
        rule: ProofRule,
        /// Formulas to append to the current branch. All are added together;
        /// the branch is not split.
        conclusions: Vec<Formula>,
    },
    /// Beta rule: branch splits into two.
    Beta {
        /// The proof rule that justifies this branching decomposition.
        rule: ProofRule,
        /// Formulas added to the left successor branch.
        left: Vec<Formula>,
        /// Formulas added to the right successor branch.
        right: Vec<Formula>,
    },
    /// Gamma rule: universal instantiation (can be applied multiple times).
    Gamma {
        /// The proof rule that justifies this instantiation.
        rule: ProofRule,
        /// The instantiated formula(s) to add to the branch (the body with
        /// the bound variable `var` replaced by `term`).
        conclusions: Vec<Formula>,
    ...
    Delta {
        /// The proof rule that justifies introducing the witness.
        rule: ProofRule,
        /// The instantiated formula(s) to add to the branch (the body with
        /// the bound variable replaced by the fresh `witness` constant).
        conclusions: Vec<Formula>,
    ...
    match formula {
        ...
        Formula::Forall(x, body) => {
            ...
            Some(RuleResult::Gamma {
                rule: ProofRule::ForallElim(formula.clone(), term.clone()),
