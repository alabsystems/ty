...
use tla_cert::{
    Axiom, Backend, CaseBranch, Certificate, Formula as CertFormula, Justification, StepId,
};
...
fn process_node(
    ...
    // Branching (β) rules must NOT be linearized into single-premise
    // decompositions — that is unsound (deriving one disjunct of a disjunction).
    // Emit an explicit, per-branch-scoped case split instead.
    if beta_principal(&node.rule).is_some() {
        handle_case_split(node, builder, parent_premise);
        return;
    }
...
/// If `rule` is a branching (β) tableau rule, return the principal formula it
/// decomposes (the disjunction being split on); otherwise `None`.
fn beta_principal(rule: &ProofRule) -> Option<&ZenonFormula> {
    match rule {
        ProofRule::OrElim(f)
        | ProofRule::NotAnd(f)
        | ProofRule::ImpliesElim(f)
        | ProofRule::NotEquiv(f) => Some(f),
        _ => None,
    }
}

/// Emit a sound [`Justification::CaseSplit`] for a β-rule node.
///
/// The node's principal formula β-decomposes into two branches (e.g. `A → B`
/// splits into assuming `¬A` and assuming `B`). We generate each branch's
/// sub-derivation in a *scoped* child builder that assumes only that branch's
/// disjunct(s), close each branch with `FALSE` (every tableau branch closes by
/// contradiction), and conclude `FALSE` at the split. The surrounding proof then
/// derives the goal from that `FALSE` exactly as it would from a direct
/// contradiction.
fn handle_case_split(
    node: &ProofNode,
    builder: &mut CertificateBuilder,
    parent_premise: Option<StepId>,
) {
    let Some(principal) = beta_principal(&node.rule) else {
        return;
    };
    let principal_cert = convert_formula(principal);
    let Some(premise_step) = ensure_tableau_formula(builder, principal_cert, parent_premise) else {
        // Without a premise establishing the disjunction we cannot soundly
        // split; leave it to the fail-closed checker.
        return;
    };

    // The exact per-branch assumptions come from the β-rule, so they line up
    // with the checker's `beta_decompose` (same left/right convention).
    let Some(RuleResult::Beta { left, right, .. }) = rules::try_beta(principal) else {
        return;
    };
    let branch_assumptions = [left, right];

    // A well-formed β node has exactly two successor branches.
    if node.children.len() != branch_assumptions.len() {
        return;
    }

    let mut branches = Vec::with_capacity(branch_assumptions.len());
    for (assumptions_zenon, child) in branch_assumptions.iter().zip(&node.children) {
        let mut branch_builder = builder.child_scope();

        // Seed this branch's assumption(s) as scoped facts.
        let mut assumption_pairs = Vec::with_capacity(assumptions_zenon.len());
        for assumption in assumptions_zenon {
            let assumption_cert = convert_formula(assumption);
            let id = branch_builder.add_assumption(assumption_cert.clone());
            assumption_pairs.push((id, assumption_cert));
        }

        // Derive the branch's sub-proof, then close it with FALSE.
        process_node(child, &mut branch_builder, Some(premise_step));
        close_branch(&mut branch_builder, premise_step);

        builder.absorb_counter(&branch_builder);
        branches.push(CaseBranch {
            assumptions: assumption_pairs,
            steps: branch_builder.steps,
        });
    }

    // The case split establishes FALSE (each branch reached a contradiction).
    let false_formula = CertFormula::Bool(false);
    if builder.lookup(&false_formula).is_none() {
        builder.add_step(
            false_formula,
            Justification::CaseSplit {
                premise: premise_step,
                branches,
            },
        );
    }
...
/// Close a case-split branch by establishing `FALSE` in its scope. Every tableau
/// branch closes by contradiction, so the checker accepts this via its scoped
/// contradiction check. If a nested case split already established `FALSE`, reuse
/// it.
fn close_branch(builder: &mut CertificateBuilder, premise_step: StepId) {
    let false_formula = CertFormula::Bool(false);
    if builder.lookup(&false_formula).is_some() {
        return;
    }
    builder.add_step(
        false_formula,
