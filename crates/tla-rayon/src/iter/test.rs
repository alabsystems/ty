...
// Upstream had a `check_repeat_unbounded` test here which was skipped on most
// platforms because the overflow takes too long on non-32-bit targets. The
// ty test-skip policy (#341) requires tests to PASS, FAIL, or be DELETED,
// so the case was removed. Overflow behavior is still exercised by the
// saturating arithmetic tests in `src/math.rs`.
