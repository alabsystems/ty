...
/// THE single raw env-mutation site in this crate. `assert_panic` toggles
/// `RUST_BACKTRACE` around a panic-catching call and restores it; every raw
/// mutation routes through here. Sound because these tests run single-threaded
/// (`RUST_TEST_THREADS=1`). `env_mutation` is the Trust toolchain's
/// deny-by-default env wall; `unknown_lints` keeps the stock-rustc build green.
#[allow(unknown_lints, env_mutation)]
#[allow(unused_unsafe)] // `set_var`/`remove_var` are safe on old rustc/editions
fn raw_env_write(key: &str, value: Option<&std::ffi::OsStr>) {
    // SAFETY: single-threaded (RUST_TEST_THREADS=1) during these tests, so no
...
pub(crate) fn assert_panic<T: std::fmt::Debug>(f: impl FnOnce() -> T) -> std::string::String {
    ...
    raw_env_write("RUST_BACKTRACE", Some(std::ffi::OsStr::new("0"))); // Suppress backtrace
    std::panic::set_hook(std::boxed::Box::new(|_| {})); // Suppress panic msg
    let res = std::panic::catch_unwind(std::panic::AssertUnwindSafe(f));
    std::panic::set_hook(hook);
    raw_env_write("RUST_BACKTRACE", backtrace.as_deref()); // Restore prior value
