...
/// THE single raw env-mutation site in this crate. `ToolConfig::from_workspace`
/// promotes the deprecated `INSTA_FORCE_UPDATE*` flags to `INSTA_UPDATE=force` —
/// a deliberate, permanent process-global promotion (not a scoped test
/// override), serialized behind a process-wide lock so parallel callers don't
/// race the write.
///
/// A raw `set_var` must exist somewhere to implement the primitive; this one
...
impl ToolConfig {
    ...
    pub fn from_workspace(workspace_dir: &Path) -> Result<ToolConfig, Error> {
        ...
        if force_update_old_env_vars {
            set_process_env_var("INSTA_UPDATE", "force");
        }
...
/// `/home/janedoe/projects/insta` when passed
/// `/home/janedoe/projects/insta/insta/Cargo.toml`.
///
