...
//!   to [`IndexMap`] and [`IndexSet`].
//! * `borsh` (**deprecated upstream, removed in the TY fork**): Upstream
//!   used this feature for [`BorshSerialize`] and [`BorshDeserialize`] impls.
//!   Due to cyclic dependencies between [`borsh`] and `indexmap`, `borsh`
//!   v1.5.6 added an `indexmap` feature that should be used instead. TY
//!   does not expose `indexmap/borsh`.
//!
