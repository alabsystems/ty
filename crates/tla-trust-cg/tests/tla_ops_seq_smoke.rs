...
#[test]
fn smoke_tla_seq_remove_at_drops_1_indexed() {
    clear_tla_arena();
    type FnRemoveAt = unsafe extern "C" fn(TlaHandle, i64) -> TlaHandle;
    let f: FnRemoveAt = unsafe { lookup("tla_seq_remove_at") };
    let s = int_seq(&[10, 20, 30, 40]);
    // RemoveAt(s, 2) = <<10, 30, 40>>
    assert_eq!(decode_seq_ints(unsafe { f(s, 2) }), vec![10, 30, 40]);
    // Remove first element.
    let s = int_seq(&[10, 20, 30, 40]);
    assert_eq!(decode_seq_ints(unsafe { f(s, 1) }), vec![20, 30, 40]);
    // Remove last element.
    let s = int_seq(&[10, 20, 30, 40]);
    assert_eq!(decode_seq_ints(unsafe { f(s, 4) }), vec![10, 20, 30]);
    // Out-of-range index (< 1 or > len) → NIL so tir_lower falls back.
    assert_eq!(unsafe { f(int_seq(&[1, 2, 3]), 0) }, NIL_HANDLE);
    assert_eq!(unsafe { f(int_seq(&[1, 2, 3]), 4) }, NIL_HANDLE);
    // Non-seq operand → NIL.
    let not_a_seq = handle_from_value(&Value::Bool(true));
    assert_eq!(unsafe { f(not_a_seq, 1) }, NIL_HANDLE);
}
