...
//! Native trust-codegen regression for executing the EWD998 `Inv` shape on compact state.
...
fn compile_ewd998_inv(layout: &StateLayout) -> (tla_trust_cg::NativeLibrary, JitInvariantFn) {
    let (chunk, entry_idx) = ewd998_inv_chunk();
    let entry = &chunk.functions[entry_idx];
    let lib = tla_trust_cg::compile_entry_invariant_native_with_chunk_and_layout(
        entry,
        &chunk,
        SYMBOL,
        layout,
