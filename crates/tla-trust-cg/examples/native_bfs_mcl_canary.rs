...
mod native {
    ...
    use tla_trust_cg::{
        compile_bfs_level_native_actions_only, compile_module_native, ActionDescriptor, OptLevel,
        TrustCgBfsLevelNativeAction, TrustCgSuccessorArena,
    };
    ...
    use trust_ir::{Block, FieldDef, Function, InstrNode, Module, StructDef, StructRepr};

    const STATE_LEN: usize = 89;
    const ACTION_COUNT: usize = 27;
    const PARENT_COUNT: usize = 4;
    const ACTION_VALUE_BASE: i64 = 10_000;
    const LOCAL_DEDUP_DISABLE_ENV: &str = "TY_TRUST_CG_NATIVE_FUSED_DISABLE_LOCAL_DEDUP";
    ...
    impl EnvGuard {
        fn set(key: &'static str, value: &str) -> Self {
            ...
            tla_trust_cg::env_guard::set_var(key, value);
            Self { key, old }
        }
    }
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            match &self.old {
                Some(value) => tla_trust_cg::env_guard::set_var(self.key, value),
                None => tla_trust_cg::env_guard::remove_var(self.key),
            }
        }
    }
    ...
    fn jit_callout_struct(id: StructId) -> StructDef {
        StructDef {
            ...
            size: None,
            align: None,
            // trust_ir StructDef gained `repr`; layout-default Rust matches the
            // pre-field behavior (this literal predates the field).
