...
//! Runtime helper function signatures for trust_cg-compiled code.
//!
...
//! functions implemented in `tla-trust_cg-runtime`. This module catalogs the
//! helper function signatures so the trust-codegen backend can declare and
//! resolve them at link time.
//!
//! # Architecture
//!
//! ```text
//! trust-ir (Call @helper) --[trust_cg]--> native call instruction
//!                                        |
//!                                        v
//!                             runtime helper (Rust extern "C")
//! ```
//!
//! The helpers are `#[no_mangle] pub extern "C"` functions. trust_cg-compiled
...
/// All runtime helper functions that trust_cg-compiled TLA+ code may call.
///
/// These are implemented as `#[no_mangle] pub extern "C"` helpers in the
/// trust-codegen runtime library, mirroring the historical `jit_*` helper ABI.
pub static RUNTIME_HELPERS: &[RuntimeHelper] = &[
    ...
    RuntimeHelper {
        symbol: "tla_handle_box_int",
        params: &[Ty::I64],
        ret: Ty::I64,
    },
    // Native-on-general-Value state ABI bridges (compound-state native path).
    // tla_handle_from_state_slot(state_ptr_as_i64, slot) — compound LoadVar:
    // deserialize a (scalar or compound) state var from the flat buffer into a
    // handle. tla_handle_from_scratch(tagged_offset) — compound LoadPrime:
    // read back a value a prior StoreVar committed to the shared scratch this
    // action. tla_handle_store_to_scratch(handle) — compound StoreVar: commit a
    // handle to the shared tla_jit_abi scratch, returning a
    // COMPOUND_SCRATCH_BASE-tagged offset the unflatten side already decodes.
    // All fail closed to NIL_HANDLE on any serialization edge.
    RuntimeHelper {
        symbol: "tla_handle_from_state_slot",
        params: &[Ty::I64, Ty::I64],
        ret: Ty::I64,
    },
    RuntimeHelper {
        symbol: "tla_handle_from_scratch",
        params: &[Ty::I64],
        ret: Ty::I64,
    },
    RuntimeHelper {
        symbol: "tla_handle_store_to_scratch",
        params: &[Ty::I64],
        ret: Ty::I64,
    },
    ...
    // ========================================================================
    // tla_hybrid_compound_* — allocation-lean compound-READ callout (item 4 M1)
    //
    // Deliberately NOT part of the boxed `tla_ops` handle family: these read a
    // scalar leaf out of the parent `ArrayState`'s BORROWED `Value` and return
    // it as a raw i64. No deserialization, no arena box, no `Value` clone of
    // the container — see `runtime_abi::compound_read`. The extern audit keeps
    // them in their own sanctioned set so they can never be confused with the
    // handle-mode kernels.
    //
    // Each returns a STATUS (`CR_OK` / `CR_ERR_*`) and writes the scalar
    // through the trailing out-pointer only on success, so there is no in-band
    // error sentinel a valid value could collide with.
    // ========================================================================
    RuntimeHelper {
        symbol: "tla_hybrid_compound_read_i64",
        // (var_idx, expect_kind, out) -> status
        params: &[Ty::I64, Ty::I64, Ty::Ptr],
        ret: Ty::I64,
    },
    RuntimeHelper {
        symbol: "tla_hybrid_compound_apply1_i64",
        // (var_idx, key0, key0_kind, expect_kind, out) -> status
        params: &[Ty::I64, Ty::I64, Ty::I64, Ty::I64, Ty::Ptr],
        ret: Ty::I64,
    },
    RuntimeHelper {
    ...
    // Choose over a set domain (see `trust_ir_lower.rs:1813-2145`):
    ...
    // tla_func_except — `[f EXCEPT ![key] = val]` single-key function/record/
    // sequence/tuple update across every eager function-like Value variant
    // (Func / IntFunc / Seq / Tuple / Record). Takes three handles (f, key,
    // val) and returns a handle to the updated value, or `NIL_HANDLE` when the
    // update is not well-defined in the eager path (non-function operand,
    // LazyFunc, out-of-bounds index, non-string record key) so tir_lower can
    // fall back to the interpreter. Multi-key `![a][b]` paths are lowered by
    // the TIR compiler into nested single-key FuncExcept opcodes.
    RuntimeHelper {
        symbol: "tla_func_except",
        params: &[Ty::I64, Ty::I64, Ty::I64],
        ret: Ty::I64,
    },
    // ========================================================================
    ...
    // tla_seq_remove_at — `RemoveAt(s, i)`. Drop the 1-indexed element `i`.
    // `i` is a raw i64 scalar. NIL on out-of-range/non-seq.
    RuntimeHelper {
        symbol: "tla_seq_remove_at",
        params: &[Ty::I64, Ty::I64],
        ret: Ty::I64,
    },
...
mod tests {
    ...
    fn test_tla_record_func_helpers_registered() {
        ...
        // tla_func_except — `[f EXCEPT ![key] = val]` single-key update.
        let except = find_helper("tla_func_except")
            .expect("missing RuntimeHelper entry for tla_func_except");
        assert_eq!(
            except.params.len(),
            3,
            "tla_func_except takes (f, key, val)"
        );
        assert_eq!(except.params[0], Ty::I64);
        assert_eq!(except.params[1], Ty::I64);
        assert_eq!(except.params[2], Ty::I64);
        assert_eq!(except.ret, Ty::I64);
    }
