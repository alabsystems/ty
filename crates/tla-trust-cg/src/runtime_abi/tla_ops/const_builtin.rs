...
//! by the BFS driver before JIT dispatch. trust-ir already resolves
//! compile-time-scalar constants (`SmallInt`/`Bool`/`String`) directly via
//! [`tla_ir::lower::constants`]; the remaining `LoadConst` calls that
//! survive to trust-codegen are compound constants (pre-materialised sets,
...
///
/// trust-ir already resolves compile-time-scalar constants (`SmallInt`,
/// `Bool`, `String`, `ModelValue`, small `Interval`) directly in
/// `tla_ir::lower::constants`. This runtime helper services the
/// remaining `LoadConst` calls — compound constants (general `Set`,
/// `Seq`, `Record`, `Tuple`, `FuncVal`, etc.) that survive to `trust_cg`.
///
...
mod tests {
    ...
    fn tostring_preserves_set_content() {
        ...
        // canonicalization).
        clear_tla_arena();
        let a = Value::Set(Rp::new(SortedSet::from_vec(vec![
            Value::SmallInt(3),
            Value::SmallInt(1),
            Value::SmallInt(2),
        ])));
