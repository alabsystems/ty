...
pub extern "C" fn tla_seq_tail(s: TlaHandle) -> TlaHandle {
    ...
    match &v {
        Value::Seq(seq) => {
            ...
            handle_from_value(&Value::Seq(Rp::new(seq.tail())))
        }
        Value::Tuple(elems) => {
            if elems.is_empty() {
                return NIL_HANDLE;
...
pub extern "C" fn tla_seq_append(s: TlaHandle, x: TlaHandle) -> TlaHandle {
    ...
    match &v {
        Value::Seq(seq) => handle_from_value(&Value::Seq(Rp::new(seq.append(elem)))),
        Value::Tuple(elems) => {
...
// ============================================================================
// tla_seq_remove_at — `RemoveAt(s, i)`
// ============================================================================

/// `RemoveAt(s, i)` — remove the element at 1-indexed position `i`.
///
/// Mirrors `crate::builtin_sequences_ext_ops::mutation` "`RemoveAt`" (from the
/// `SequencesExt` community module): the result has length `Len(s) - 1`.
///
/// Returns [`NIL_HANDLE`] on a non-sequence operand or an out-of-range index
/// (`i < 1` or `i > Len(s)`), so `tir_lower` can surface the interpreter's
/// precise `IndexOutOfBounds` error in the fallback path.
#[no_mangle]
pub extern "C" fn tla_seq_remove_at(s: TlaHandle, i: i64) -> TlaHandle {
    let v = handle_to_value(s);
    let Some(elems) = v.as_seq_or_tuple_elements() else {
        return NIL_HANDLE;
    };
...
mod tests {
    ...
    #[test]
    fn seq_remove_at_drops_one_element_1_indexed() {
        clear_tla_arena();
        // RemoveAt(<<10,20,30,40>>, 2) = <<10, 30, 40>>
        let s = handle_from_value(&int_seq(&[10, 20, 30, 40]));
        assert_eq!(decode_seq_ints(tla_seq_remove_at(s, 2)), vec![10, 30, 40]);
    }

    #[test]
    fn seq_remove_at_first_and_last() {
        clear_tla_arena();
        let s = handle_from_value(&int_seq(&[10, 20, 30]));
        assert_eq!(decode_seq_ints(tla_seq_remove_at(s, 1)), vec![20, 30]);
        let s = handle_from_value(&int_seq(&[10, 20, 30]));
        assert_eq!(decode_seq_ints(tla_seq_remove_at(s, 3)), vec![10, 20]);
    }

    #[test]
    fn seq_remove_at_out_of_range_returns_nil() {
        clear_tla_arena();
        // index < 1 or index > len must fall back via NIL.
        let s = handle_from_value(&int_seq(&[1, 2, 3]));
        assert_eq!(tla_seq_remove_at(s, 0), NIL_HANDLE);
        let s = handle_from_value(&int_seq(&[1, 2, 3]));
        assert_eq!(tla_seq_remove_at(s, 4), NIL_HANDLE);
        // Empty sequence: any index is out of range.
        let empty = handle_from_value(&int_seq(&[]));
        assert_eq!(tla_seq_remove_at(empty, 1), NIL_HANDLE);
    }
