...
//! Stable ABI types + runtime helpers for trust_cg-compiled code.
//!
...
//! is the stable native call-boundary crate shared by `tla-ir`,
//! `tla-check`, and `tla-trust_cg`.
//!
...
//! `tla-trust_cg::runtime_abi` where `compound_layout` is owned.
//!
//! # Overflow Strategy
//!
//! trust_cg-compiled code uses **i64-only with overflow rejection** (Part of #746):
//!
...
//! | tla-trust_cg      | i64 only     | `ArithmeticOverflow` runtime error |

// JIT runtime callbacks invoked from compiled native code: pub extern "C"
// signatures dereference raw pointers but cannot be marked `unsafe fn`
// because trust_cg-emitted call sites do not generate `unsafe` blocks. Each
// such function documents its safety contract in its body comment.
...
// trust_cg's ImportLinkage. They bridge the gap between the flat i64 state buffer
...
///
/// trust-codegen takes the function pointer directly when building its extern symbol
