...
impl FlatState {
    ...
    /// Compute the admissible seeded fingerprint of this flat state.
    ///
    /// Hashes the entire buffer (index + compound region) through the
    /// `ty_compiled_fp_u64` identity used by generated native code.
    #[must_use]
    pub fn fingerprint(&self) -> u64 {
        super::fingerprint::admissible_flat_state_fingerprint_64(
            self.buf.as_ptr(),
            self.buf.len() as u32,
        )
    }
...
mod tests {
    ...
    fn test_schema_with_string() {
        let values = vec![Value::String(Rp::from("hello")), Value::SmallInt(5)];
        let schema = FlatStateSchema::from_initial_state(&values);
    ...
    fn test_roundtrip_string_values() {
        let values = vec![Value::String(Rp::from("active")), Value::SmallInt(3)];
        let schema = FlatStateSchema::from_initial_state(&values);
        assert!(schema.is_all_scalar());
    ...
    fn test_roundtrip_with_set() {
        let set = Value::Set(Rp::new(SortedSet::from_sorted_vec(vec![
            Value::SmallInt(1),
            Value::SmallInt(2),
            Value::SmallInt(3),
        ])));
    ...
    fn test_roundtrip_multiple_compound() {
        ...
        let seq = Value::Seq(Rp::new(SeqValue::from_vec(vec![Value::SmallInt(7)])));
        let rec = Value::Record(RecordValue::from_sorted_str_entries(vec![(
            Arc::from("x"),
            Value::SmallInt(99),
        )]));
    ...
    fn test_state_to_flat_reuse_compound() {
        let seq = Value::Seq(Rp::new(SeqValue::from_vec(vec![Value::SmallInt(5)])));
    ...
    fn test_state_to_flat_reuse_wrong_count() {
        ...
        let schema = FlatStateSchema::from_initial_state(&[Value::SmallInt(1), Value::SmallInt(2)]);
        let mut buf = Vec::new();
        assert!(
    ...
    #[cfg_attr(test, ntest::timeout(10000))]
    #[test]
    fn test_flat_state_fingerprint_matches_seeded_compiled_identity() {
        let values = vec![Value::SmallInt(7), Value::SmallInt(11), Value::SmallInt(42)];
        let schema = FlatStateSchema::from_initial_state(&values);
        let flat = state_to_flat(&values, &schema).expect("state_to_flat");

        let fp_via_flat_state = flat.fingerprint();
        let byte_len = flat.total_slots() * std::mem::size_of::<i64>();
        // SAFETY: `flat.as_slice()` is valid for `byte_len` bytes.
        let fp_via_identity = unsafe {
            crate::runtime_abi::ty_compiled_fp_u64(flat.as_slice().as_ptr().cast::<u8>(), byte_len)
        };
        assert_eq!(
            fp_via_flat_state, fp_via_identity,
            "FlatState fingerprint must match the seeded ty_compiled_fp_u64 identity"
        );

        let fp_via_legacy = crate::runtime_abi::fingerprint::jit_xxh3_fingerprint_64(
            flat.as_slice().as_ptr(),
            flat.total_slots() as u32,
        );
        assert_eq!(
            fp_via_flat_state, fp_via_legacy,
            "legacy ABI symbol should route to the seeded identity"
        );
    }
