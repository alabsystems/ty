...
//! trust_cg-compiled liveness predicate evaluation.
//!
//! This module takes [`LivenessPredInfo`] descriptors (defined in
//! `tla-jit-abi`) and compiles them through the trust-ir/trust_cg pipeline. The
...
//!
//! ```text
//! LivenessPredInfo[] ──► build trust-ir Module ──► compile_module_native()
//!                                                       │
//!     CompiledStatePredBatchFn ◄────────────────────────┘
//!     CompiledActionPredBatchFn ◄───────────────────────┘
//! ```
//!
//! The generated trust-ir module contains one or two functions:
...
//!
//! Part of #4198: Compiled liveness predicates via trust-ir/trust_cg.
...
/// Result of trust_cg-compiled liveness predicate batch.
///
/// Wraps trust_cg-compiled predicate batch function pointers with the same
/// bitmask ABI as the interpreter fallback path. Direct callers of cached
/// function pointers in this struct must keep the owning compiled artifact alive
/// and call [`crate::ensure_jit_execute_mode`] on the invoking thread immediately
/// before invoking them.
#[derive(Debug)]
pub struct TrustCgCompiledLivenessBatch {
...
impl TrustCgCompiledLivenessBatch {
    ...
    pub unsafe fn eval_state_preds(&self, state_buf: &[i64]) -> u64 {
        match self.state_pred_fn {
            Some(f) => {
                self.ensure_liveness_symbol_published(
                    "liveness_state_pred_batch",
                    f as *const () as *mut std::ffi::c_void,
                );
    ...
    fn ensure_liveness_symbol_published(&self, symbol: &str, ptr: *mut std::ffi::c_void) {
        #[cfg(feature = "native")]
        {
            if let Some(native_lib) = &self._native_lib {
                native_lib
                    .ensure_published_symbol_ptr(symbol, ptr)
                    .unwrap_or_else(|err| {
                        panic!(
                            "[trust-cg] failed to re-publish liveness JIT symbol '{symbol}' executable before native call: {err}"
                        );
                    });
                #[cfg(feature = "native")]
        ...
        // Ownerless liveness batches only occur in non-native builds or
        // hand-assembled tests. Real trust-codegen liveness compilation retains the
        // NativeLibrary in `_native_lib`, so normal calls avoid the broad
...
/// Compile liveness predicates via the trust-codegen pipeline.
///
/// Generates LLVM IR for the predicate batch functions and compiles them
/// to native code through the trust-codegen backend.
///
/// # Errors
///
/// Returns [`TrustCgError`] if IR generation or native compilation fails.
/// Predicates that cannot be compiled are tracked in fallback lists (not errors).
pub fn compile_liveness_predicates_trust_cg(
    predicates: &[LivenessPredInfo],
    layout: &StateLayout,
) -> Result<TrustCgCompiledLivenessBatch, TrustCgError> {
    ...
    Ok(TrustCgCompiledLivenessBatch {
        state_pred_fn,
        compiled_state_tags,
        fallback_state_tags,
        action_pred_fn,
...
fn generate_liveness_ir(
    ...
    .map_err(|e| TrustCgError::Emission(e.to_string()))?;
    writeln!(ir).map_err(|e| TrustCgError::Emission(e.to_string()))?;
...
fn emit_state_pred_batch_fn(
    ...
    .map_err(|e| TrustCgError::Emission(e.to_string()))?;
    writeln!(ir, "entry:").map_err(|e| TrustCgError::Emission(e.to_string()))?;
    ...
    writeln!(ir, "  ret i64 {bitmask_reg}").map_err(|e| TrustCgError::Emission(e.to_string()))?;
    writeln!(ir, "}}").map_err(|e| TrustCgError::Emission(e.to_string()))?;
    Ok(())
}
...
/// Attempt native compilation of the liveness IR through trust_cg.
///
...
> {
    ...
    // Compile through trust-codegen native pipeline.
    // The legacy raw-IR-text API (`compile_and_link`) was removed, so we build
    // a trust-ir module and use the direct pipeline: `compile_module_native`
    // expects a trust-ir module. For liveness, we generate the trust-ir module
    // programmatically instead.
    let module = build_liveness_trust_ir_module(state_preds, action_preds)?;
...
/// Build a trust-ir module representing the liveness predicate batch functions.
///
/// This generates trust-ir instructions programmatically to implement the same
/// logic as the LLVM IR text generation above, but going through the proper
/// trust-ir -> trust-codegen pipeline for native compilation.
#[cfg(feature = "native")]
fn build_liveness_trust_ir_module(
    state_preds: &[&LivenessPredInfo],
    action_preds: &[&LivenessPredInfo],
) -> Result<trust_ir::Module, TrustCgError> {
    ...
    if !state_preds.is_empty() {
        ...
        let state_ptr_val = ValueId::new(0);
        let state_len_val = ValueId::new(1);
        let mut block = Block::new(entry_block_id)
            .with_param(state_ptr_val, Ty::Ptr)
            .with_param(state_len_val, Ty::I32);
        let mut val_counter = 2u32;
        ...
        for pred in state_preds {
            let (pred_result, new_val_counter) =
                emit_trust_ir_state_pred_check(&mut block, val_counter, pred, state_ptr_val);
            val_counter = new_val_counter;

            // Convert bool predicate to i64.
            let (ext_val, new_val_counter) =
                emit_trust_ir_bool_as_i64(&mut block, val_counter, pred_result);
            val_counter = new_val_counter;
    ...
    if !action_preds.is_empty() {
        ...
        let current_ptr_val = ValueId::new(0);
        let next_ptr_val = ValueId::new(1);
        let state_len_val = ValueId::new(2);
        let mut block = Block::new(entry_block_id)
            .with_param(current_ptr_val, Ty::Ptr)
            .with_param(next_ptr_val, Ty::Ptr)
            .with_param(state_len_val, Ty::I32);
        let mut val_counter = 3u32;
        ...
        for pred in action_preds {
            let (pred_result, new_val_counter) = emit_trust_ir_action_pred_check(
                &mut block,
                val_counter,
                pred,
                current_ptr_val,
...
#[cfg(feature = "native")]
fn emit_trust_ir_bool_as_i64(
    block: &mut trust_ir::Block,
    mut val_counter: u32,
    pred_result: trust_ir::value::ValueId,
) -> (trust_ir::value::ValueId, u32) {
    use trust_ir::constant::Constant;
    ...
    let one = ValueId::new(val_counter);
    val_counter += 1;
    block.body.push(
        InstrNode::new(Inst::Const {
            ty: Ty::I64,
            value: Constant::Int(1i128),
        })
        .with_result(one),
    );
    ...
    let as_i64 = ValueId::new(val_counter);
    val_counter += 1;
    block.body.push(
...
/// Emit trust-ir instructions for a single state predicate check.
///
/// Returns (result_value_id, updated_val_counter).
#[cfg(feature = "native")]
fn emit_trust_ir_state_pred_check(
    block: &mut trust_ir::Block,
    mut val_counter: u32,
    pred: &LivenessPredInfo,
    state_ptr: trust_ir::value::ValueId,
) -> (trust_ir::value::ValueId, u32) {
    use trust_ir::constant::Constant;
    use trust_ir::inst::Inst;
    use trust_ir::ty::Ty;
...
/// Emit trust-ir instructions for a single action predicate check.
#[cfg(feature = "native")]
fn emit_trust_ir_action_pred_check(
    block: &mut trust_ir::Block,
    mut val_counter: u32,
    pred: &LivenessPredInfo,
    current_ptr: trust_ir::value::ValueId,
    next_ptr: trust_ir::value::ValueId,
) -> (trust_ir::value::ValueId, u32) {
    use trust_ir::constant::Constant;
...
/// Map `ScalarCompOp` to trust-ir `ICmpOp`.
#[cfg(feature = "native")]
fn scalar_comp_to_trust_ir(op: ScalarCompOp) -> trust_ir::inst::ICmpOp {
...
pub fn generate_liveness_ir_text(predicates: &[LivenessPredInfo]) -> Result<String, TrustCgError> {
    let mut state_preds: Vec<&LivenessPredInfo> = Vec::new();
    let mut action_preds: Vec<&LivenessPredInfo> = Vec::new();
...
mod tests {
    ...
    fn test_compile_batch_empty() {
        ...
        let batch =
            compile_liveness_predicates_trust_cg(&[], &layout).expect("empty should succeed");
    ...
    fn test_compile_batch_classifies_ineligible() {
        ...
        let batch = compile_liveness_predicates_trust_cg(&preds, &layout).expect("should classify");
    ...
    #[cfg(feature = "native")]
    #[test]
    fn test_liveness_trust_ir_entry_params_validate() {
        use trust_ir::inst::Inst;
        use trust_ir::ty::Ty;
        use trust_ir::value::ValueId;

        let state_pred = LivenessPredInfo {
            tag: 0,
            is_state_pred: true,
            var_indices: vec![0],
            kind: LivenessPredKind::ScalarComparison {
        ...
        let action_pred = LivenessPredInfo {
            tag: 1,
            is_state_pred: false,
            var_indices: vec![0],
            kind: LivenessPredKind::StateChangeCheck {
        ...
        let module = build_liveness_trust_ir_module(&[&state_pred], &[&action_pred])
            .expect("liveness trust-ir module should build");
        let errors = trust_ir_build::validate_module(&module);
        assert!(
            errors.is_empty(),
            "liveness trust-ir module should validate: {errors:?}"
        );

        let state_func = module
            .function_by_name("liveness_state_pred_batch")
            .expect("state batch function");
        let state_entry = state_func.entry_block().expect("state entry block");
        assert_eq!(
            state_entry.params,
            vec![(ValueId::new(0), Ty::Ptr), (ValueId::new(1), Ty::I32)]
        );
        assert_eq!(
            state_entry
                .body
                .first()
                .and_then(|node| node.results.first()),
            Some(&ValueId::new(2)),
            "state SSA allocation should start after formal params"
        );

        let action_func = module
            .function_by_name("liveness_action_pred_batch")
            .expect("action batch function");
        let action_entry = action_func.entry_block().expect("action entry block");
        assert_eq!(
            action_entry.params,
            vec![
        ...
        assert_eq!(
            action_entry
                ...
                .and_then(|node| node.results.first()),
            Some(&ValueId::new(3)),
            "action SSA allocation should start after formal params"
        );
        assert!(matches!(
            state_entry.body.last().map(|node| &node.inst),
            Some(Inst::Return { .. })
        ));
        assert!(matches!(
            action_entry.body.last().map(|node| &node.inst),
            Some(Inst::Return { .. })
        ));
    }
