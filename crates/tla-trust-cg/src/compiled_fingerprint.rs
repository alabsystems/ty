...
//! Compiled fingerprint functions for the trust-codegen backend.
//!
...
//! `ty_compiled_fp_u64` extern. The byte length of the state buffer is
...
//!   +-- call ty_compiled_fp_u64(state_ptr, state_len_bytes) -> i64
//! ```
//!
//! # Canonical extern (#4319 Phase 2)
//!
//! The emitted IR calls `@ty_compiled_fp_u64(ptr, i64)` — the canonical
...
//! # Future work
//!
//! A hot-path optimisation would inline xxh3 as trust-ir instructions to avoid
//! the extern call overhead entirely. That is a separate performance issue
//! (#4198 Phase 3), not a soundness concern.
//!
//! Part of #4319 Phase 2 (canonical extern retarget) and #4198 (trust-ir
...
///
/// It calls the canonical `ty_compiled_fp_u64(state_ptr, state_len_bytes)`
...
/// rationale for routing through `ty_compiled_fp_u64` (#4319 Phase 2).
///
/// # Arguments
///
...
// Currently exercised only by tests; retained as the canonical text-IR emitter
// for the compiled fingerprint extern.
#[allow(dead_code)]
pub(crate) fn emit_fingerprint_64_ir(state_len: usize, name: &str) -> String {
    ...
    // `ty_compiled_fp_u64(buf: *const u8, len: usize) -> u64` takes the
    ...
    // `@ty_compiled_fp_u64(ptr, i64)` so the emitted IR and the Rust
    // driver's `fingerprint_flat_compiled` share one symbol and one seed.
    writeln!(ir, "declare i64 @ty_compiled_fp_u64(ptr, i64)").unwrap();
    writeln!(ir).unwrap();
    ...
    writeln!(
        ir,
        "  %1 = call i64 @ty_compiled_fp_u64(ptr %0, i64 {state_len_bytes})"
    )
    .unwrap();
...
mod tests {
    ...
    fn test_emit_fingerprint_64_ir_structure() {
        ...
        assert!(
            ir.contains("declare i64 @ty_compiled_fp_u64(ptr, i64)"),
        ...
        assert!(
            ir.contains("call i64 @ty_compiled_fp_u64(ptr %0, i64 40)"),
    ...
    fn test_emit_fingerprint_64_empty_state() {
        ...
        assert!(
            ir.contains("call i64 @ty_compiled_fp_u64(ptr %0, i64 0)"),
    ...
    fn test_emit_fingerprint_64_large_state() {
        ...
        assert!(
            ir.contains("call i64 @ty_compiled_fp_u64(ptr %0, i64 8000)"),
    ...
    mod native_tests {
        use super::*;
        use crate::runtime_abi::fingerprint::jit_xxh3_fingerprint_64;

        /// Helper: compile IR text through the trust-codegen native pipeline.
        ///
        /// The legacy `compile_and_link` raw-IR-text path was removed; instead we test that the
        /// IR text is well-formed and structurally correct. End-to-end native
        /// execution would require piping the IR text into the trust-codegen JIT, which
        /// currently only accepts trust-ir modules. Phase 2 will build proper trust-ir
        ...
        fn test_fingerprint_64_ir_is_well_formed() {
            ...
            // Canonical extern (#4319 Phase 2).
            assert!(ir.contains("declare i64 @ty_compiled_fp_u64(ptr, i64)"));
        }
    ...
    // =========================================================================
    //
    // These tests verify that `ty_compiled_fp_u64` — the symbol the emitted
    ...
    #[test]
    fn test_ty_compiled_fp_u64_matches_seeded_xxh3_empty() {
        use crate::runtime_abi::ty_compiled_fp_u64;
        ...
        assert_eq!(
            actual, expected,
            "ty_compiled_fp_u64 must match seeded xxh3 on empty buffer \
             (soundness gate for #4319 mixed BFS fingerprints)"
        );
    }

    #[test]
    fn test_ty_compiled_fp_u64_matches_seeded_xxh3_various_sizes() {
        use crate::runtime_abi::ty_compiled_fp_u64;
        ...
        for state_len in [0, 1, 2, 5, 17, 32, 64, 128, 256, 1000] {
            ...
            assert_eq!(
                actual, expected,
                "ty_compiled_fp_u64 diverged from seeded xxh3 at state_len={state_len} \
                 (soundness gate for #4319 mixed BFS fingerprints)"
            );
        }
    }
    ...
    #[test]
    fn test_ty_compiled_fp_u64_does_not_collide_with_bare_xxh3() {
        use crate::runtime_abi::ty_compiled_fp_u64;
        ...
        let seeded = unsafe { ty_compiled_fp_u64(bytes.as_ptr(), bytes.len()) };
        let bare = xxhash_rust::xxh3::xxh3_64(bytes);
        assert_ne!(
            seeded, bare,
            "ty_compiled_fp_u64 MUST be domain-separated from bare xxh3_64 \
