...
//! Direct TIR/trust-ir bytecode -> LLVM IR text lowering.
//!
//! # WARNING: LEGACY, TEST-ONLY — DO NOT WIRE INTO PRODUCTION
//!
//! This module is a **legacy direct TIR -> LLVM-IR-text emitter with NO
//! production callers**. It is reachable only through the `pub use` re-export
//! in `lib.rs` and this crate's tests. Production native codegen does NOT go
//! through this module: it goes through `tla_ir::lower`
//! (`tla-ir/src/lower/arithmetic.rs` and friends) into trust_cg's pure-Rust
//! pipeline (`compile::compile_module_native`).
//!
//! **CRITICAL safety gap versus the production path:** the guard-failure
//! helpers here — `emit_divzero_checked`, `emit_div_min_overflow_checked`,
//! `emit_mod_nonpositive_checked` (and the overflow-checked binop path) — emit
//! `call void @llvm.trap()` + `unreachable`, i.e. a **whole-process abort
//! (SIGILL)** when a guard fires on otherwise-valid input. The production
//! lowering (`tla-ir/src/lower/arithmetic.rs`) instead reports typed,
//! per-state `JitRuntimeErrorKind` statuses through the runtime-error ABI
//! (`emit_runtime_error_and_return`), letting the checker attribute the
//! failure to a state and continue.
//!
//! Therefore this module **MUST NOT be wired into production** until those
//! trap-emitting helpers are rewritten to mirror the typed
//! `emit_runtime_error_and_return` error ABI. Its restored IntDiv/ModInt
//! *value* semantics (floored division / Euclidean modulo) are correct and
//! kept under test; only the guard-failure *delivery mechanism* is unsafe for
//! production use.
//!
//! Module renamed from `tir_lower` to `trust_ir_lower` (design doc §11 Stream 3)
//! to reflect the trust-ir pipeline naming. External callers using
//! `tla_trust_cg::tir_lower::*` continue to work via the `pub use trust_ir_lower as
//! tir_lower` alias in `lib.rs`.
//!
//! This module provides a fast path from TIR bytecode opcodes directly to LLVM
//! IR text, bypassing the intermediate trust-ir representation. For scalar
//! operations (arithmetic, comparison, boolean logic), the mapping is
//! straightforward and the extra trust-ir layer adds overhead without benefit.
//!
...
//! - **Arithmetic**: `AddInt`, `SubInt`, `MulInt`, `IntDiv`, `ModInt`, `NegInt`, `PowInt` (with overflow checks)
...
//! ```
//!
//! This is a parallel lowering path to the existing trust-ir-based pipeline:
//!
//! ```text
//! TIR -> trust-ir -> LLVM IR   (existing: lower.rs + emit.rs)
...
//! | `AddInt(rd,r1,r2)` | widened add + overflow br | Overflow checked |
//! | `SubInt(rd,r1,r2)` | widened sub + overflow br | Overflow checked |
//! | `MulInt(rd,r1,r2)` | widened mul + overflow br | Overflow checked |
//! | `IntDiv(rd,r1,r2)` | `icmp eq` + br + `sdiv` + floor select | Zero + MIN/-1 checks; TLA+ floored `\div` |
//! | `ModInt(rd,r1,r2)` | `icmp sle` + br + `srem` + select | Positive-divisor check; TLA+ Euclidean `%` |
//! | `NegInt(rd,rs)` | widened sub | 0 - rs with overflow check |
//! | `PowInt(rd,r1,r2)` | square-and-multiply loop | i64 `checked_pow`; traps on neg exp / overflow |
...
/// # Errors
///
/// Returns [`TrustCgError::UnsupportedInst`] if the function contains opcodes
/// that cannot be lowered (compound values, quantifiers, etc.).
pub fn lower_tir_to_llvm_ir(
    func: &BytecodeFunction,
    name: &str,
) -> Result<TirLoweredModule, TrustCgError> {
...
pub fn lower_tir_to_llvm_ir_with_state(
    ...
    // rationale. Covers the state-access path that tla-check's trust-codegen
...
/// that can be lowered without the trust-ir intermediate representation.
pub fn is_tir_eligible(func: &BytecodeFunction) -> bool {
    func.instructions.iter().all(is_scalar_opcode)
}
...
pub fn is_tir_eligible_with_state(func: &BytecodeFunction) -> bool {
    func.instructions.iter().all(is_state_eligible_opcode)
}
...
pub fn is_tir_eligible_with_quantifiers(func: &BytecodeFunction) -> bool {
    func.instructions.iter().all(is_quantifier_eligible_opcode)
}
...
impl TirLowerCtx {
    ...
    fn analyze_blocks(&mut self, func: &BytecodeFunction) {
        ...
        for (pc, op) in func.instructions.iter().enumerate() {
            match *op {
                Opcode::Jump { offset } => {
                    let target = (pc as i64 + i64::from(offset)) as usize;
                    if target < n {
                ...
                Opcode::FuncExcept { .. } => {
                    self.runtime_decls
                        .insert("declare i64 @tla_func_except(i64, i64, i64)".to_string());
                }
                Opcode::Domain { .. } => {
                    self.runtime_decls
                ...
                Opcode::CallBuiltin { builtin, .. } => {
                    let decl = match builtin {
                        ...
                        BuiltinOp::RemoveAt => "declare i64 @tla_seq_remove_at(i64, i64)",
                        ...
                        // Range has no direct-LLVM runtime symbol; it is routed to the
                        // bytecode-VM path (see `is_tir_lowering_eligible`), so emit no
                        // runtime declaration here.
                        BuiltinOp::Range
                        | BuiltinOp::FoldFunctionOnSetSum
                        | BuiltinOp::RoundApply => continue,
                        // Bag ops execute on the bytecode-VM path only (shared
                        // value-level helpers); no native runtime symbol —
                        // native lowering fails closed at the emission match.
                        BuiltinOp::BagAdd | BuiltinOp::BagRemove | BuiltinOp::SetToBag => continue,
                    };
                ...
                | Opcode::ChooseBegin { loop_end, .. } => {
                    ...
                    let end_pc = (pc as i64 + i64::from(loop_end)) as usize;
                    if end_pc < n {
                ...
                | Opcode::ChooseNext { loop_begin, .. } => {
                    ...
                    let body_pc = (pc as i64 + i64::from(loop_begin)) as usize;
                    if body_pc < n {
    ...
    fn lower_function(&mut self, func: &BytecodeFunction) -> Result<(), TrustCgError> {
        ...
        .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output).map_err(|e| TrustCgError::Emission(e.to_string()))?;
        ...
        for decl in &runtime_decls {
            writeln!(self.output, "{decl}").map_err(|e| TrustCgError::Emission(e.to_string()))?;
        }
        writeln!(self.output).map_err(|e| TrustCgError::Emission(e.to_string()))?;
        ...
        writeln!(self.output, ") {{").map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "entry:").map_err(|e| TrustCgError::Emission(e.to_string()))?;
    ...
    fn emit_load_reg(&mut self, reg: u8) -> Result<String, TrustCgError> {
    ...
    fn emit_store_reg(&mut self, reg: u8, val: &str) -> Result<(), TrustCgError> {
        writeln!(self.output, "  store i64 {val}, ptr %r{reg}_ptr")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        Ok(())
    }

    /// Emit an overflow-checked signed arithmetic operation.
    ///
    /// Pattern:
    /// ```llvm
    /// %lhs_w = sext i64 %a to i128
    /// %rhs_w = sext i64 %b to i128
    /// %wide = <op> i128 %lhs_w, %rhs_w
    /// %res = trunc i128 %wide to i64
    /// %roundtrip = sext i64 %res to i128
    /// %ovf = icmp ne i128 %wide, %roundtrip
    ...
    fn emit_overflow_checked_binop(
        ...
        let lhs_wide = self.fresh_val();
        let rhs_wide = self.fresh_val();
        let wide_result = self.fresh_val();
        let res = self.fresh_val();
        let roundtrip = self.fresh_val();
        let ovf = self.fresh_val();
        let overflow_block = self.fresh_block("overflow");
        let continue_block = self.fresh_block("continue");

        writeln!(self.output, "  {lhs_wide} = sext i64 {lhs} to i128")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  {rhs_wide} = sext i64 {rhs} to i128")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        let op_name = match op {
            SignedOverflowOp::Add => "add",
            SignedOverflowOp::Sub => "sub",
            SignedOverflowOp::Mul => "mul",
        };
        writeln!(
            self.output,
            "  {wide_result} = {op_name} i128 {lhs_wide}, {rhs_wide}"
        )
        .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  {res} = trunc i128 {wide_result} to i64")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  {roundtrip} = sext i64 {res} to i128")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(
            self.output,
            "  {ovf} = icmp ne i128 {wide_result}, {roundtrip}"
        )
        .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(
            self.output,
    ...
    /// Emit an overflow-checked signed `i64` multiply, returning the product
    /// SSA value. Traps (and is `unreachable`) on signed overflow, mirroring the
    /// `llvm.trap()` policy of [`emit_overflow_checked_binop`] but yielding a
    /// value instead of storing into a register slot. Used by [`emit_pow_checked`].
    fn emit_checked_mul_i64(&mut self, lhs: &str, rhs: &str) -> Result<String, TrustCgError> {
        let lhs_wide = self.fresh_val();
        let rhs_wide = self.fresh_val();
        let wide_result = self.fresh_val();
        let res = self.fresh_val();
        let roundtrip = self.fresh_val();
        let ovf = self.fresh_val();
        let overflow_block = self.fresh_block("pow_overflow");
        let continue_block = self.fresh_block("pow_mul_ok");
        ...
        writeln!(
            self.output,
            "  {wide_result} = mul i128 {lhs_wide}, {rhs_wide}"
        )
        .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  {res} = trunc i128 {wide_result} to i64")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
    ...
    /// Emit integer exponentiation `rd = base ^ exp` (TLA+ `^`) with the exact
    /// semantics of the bytecode-VM / tree-walking interpreter's i64 fast path
    /// (`i64::checked_pow`, see `tla-tir` const-prop and `tla-eval` `int_pow`).
    ///
    /// Soundness contract: the trust-cg direct-LLVM value model is i64-only,
    /// whereas the interpreter computes `^` in arbitrary precision (`BigInt`). This
    /// lowering therefore mirrors the interpreter ONLY where the result fits in
    /// i64; on any case the interpreter would either reject or where the i64
    /// result would diverge, it traps (`llvm.trap()` + `unreachable`) rather than
    /// produce a different number — identical to how [`emit_overflow_checked_binop`]
    /// handles `Mul`/`Add`/`Sub` overflow. Concretely it traps on:
    ///   * a **negative exponent** (the interpreter errors: `to_u32()` fails), and
    ///   * any **intermediate or final overflow** of the squaring algorithm
    ///     (the interpreter would promote to `BigInt`, which i64 cannot represent).
    ///
    /// The squaring schedule is a bit-for-bit transcription of Rust's
    /// `i64::checked_pow`: it squares `base` inside the `exp > 1` loop and folds
    /// the residual factor in once after the loop, so its overflow behavior is
    /// exactly the interpreter's. `0^0 == 1` falls out naturally (the loop and
    /// tail are skipped, leaving `acc == 1`). Loop state lives in `alloca` slots
    /// (allocated once, before the loop) to avoid phi nodes, consistent with the
    /// register-file design (`mem2reg` promotes them during `opt`).
    fn emit_pow_checked(&mut self, base: &str, exp: &str, rd: u8) -> Result<(), TrustCgError> {
        // Mutable loop state: acc (accumulator), b (running base), e (running exp).
        let acc_ptr = self.fresh_val();
        let b_ptr = self.fresh_val();
        let e_ptr = self.fresh_val();
        writeln!(self.output, "  {acc_ptr} = alloca i64")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  {b_ptr} = alloca i64")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  {e_ptr} = alloca i64")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;

        // Negative-exponent guard: the interpreter rejects (errors) on e < 0, and
        // i64 cannot represent a fractional/inverse result, so trap.
        let e_neg = self.fresh_val();
        let neg_block = self.fresh_block("pow_neg_exp");
        let init_block = self.fresh_block("pow_init");
        writeln!(self.output, "  {e_neg} = icmp slt i64 {exp}, 0")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(
            self.output,
            "  br i1 {e_neg}, label %{neg_block}, label %{init_block}"
        )
        .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "{neg_block}:").map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  call void @llvm.trap()")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  unreachable")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;

        // init: acc = 1; b = base; e = exp.
        writeln!(self.output, "{init_block}:")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  store i64 1, ptr {acc_ptr}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  store i64 {base}, ptr {b_ptr}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  store i64 {exp}, ptr {e_ptr}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;

        let head_block = self.fresh_block("pow_loop_head");
        let body_block = self.fresh_block("pow_loop_body");
        let tail_block = self.fresh_block("pow_tail");
        let done_block = self.fresh_block("pow_done");

        // loop head: continue while e > 1.
        writeln!(self.output, "  br label %{head_block}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "{head_block}:")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        let e_cur = self.fresh_val();
        let more = self.fresh_val();
        writeln!(self.output, "  {e_cur} = load i64, ptr {e_ptr}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  {more} = icmp sgt i64 {e_cur}, 1")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(
            self.output,
            "  br i1 {more}, label %{body_block}, label %{tail_block}"
        )
        .map_err(|e| TrustCgError::Emission(e.to_string()))?;

        // loop body:
        //   if (e & 1) == 1 { acc = checked_mul(acc, b) }
        //   e = e >> 1   (logical; e > 1 here so non-negative)
        //   b = checked_mul(b, b)
        writeln!(self.output, "{body_block}:")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        let b_cur = self.fresh_val();
        let acc_cur = self.fresh_val();
        let lowbit = self.fresh_val();
        let odd = self.fresh_val();
        writeln!(self.output, "  {b_cur} = load i64, ptr {b_ptr}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  {acc_cur} = load i64, ptr {acc_ptr}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  {lowbit} = and i64 {e_cur}, 1")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  {odd} = icmp eq i64 {lowbit}, 1")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        let odd_block = self.fresh_block("pow_odd");
        let even_block = self.fresh_block("pow_after_odd");
        writeln!(
            self.output,
            "  br i1 {odd}, label %{odd_block}, label %{even_block}"
        )
        .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "{odd_block}:").map_err(|e| TrustCgError::Emission(e.to_string()))?;
        let acc_mul = self.emit_checked_mul_i64(&acc_cur, &b_cur)?;
        writeln!(self.output, "  store i64 {acc_mul}, ptr {acc_ptr}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  br label %{even_block}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "{even_block}:")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        // e = e >> 1 (e > 1 here, so logical/arithmetic shift coincide and stay >= 0).
        let e_shifted = self.fresh_val();
        writeln!(self.output, "  {e_shifted} = lshr i64 {e_cur}, 1")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  store i64 {e_shifted}, ptr {e_ptr}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        // b = checked_mul(b, b).
        let b_sq = self.emit_checked_mul_i64(&b_cur, &b_cur)?;
        writeln!(self.output, "  store i64 {b_sq}, ptr {b_ptr}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  br label %{head_block}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;

        // tail: if e == 1 { acc = checked_mul(acc, b) }
        writeln!(self.output, "{tail_block}:")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        let e_tail = self.fresh_val();
        let is_one = self.fresh_val();
        writeln!(self.output, "  {e_tail} = load i64, ptr {e_ptr}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  {is_one} = icmp eq i64 {e_tail}, 1")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        let fold_block = self.fresh_block("pow_fold");
        writeln!(
            self.output,
            "  br i1 {is_one}, label %{fold_block}, label %{done_block}"
        )
        .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "{fold_block}:")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        let acc_t = self.fresh_val();
        let b_t = self.fresh_val();
        writeln!(self.output, "  {acc_t} = load i64, ptr {acc_ptr}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  {b_t} = load i64, ptr {b_ptr}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        let acc_final = self.emit_checked_mul_i64(&acc_t, &b_t)?;
        writeln!(self.output, "  store i64 {acc_final}, ptr {acc_ptr}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  br label %{done_block}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;

        // done: result = acc.
        writeln!(self.output, "{done_block}:")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        let result = self.fresh_val();
        writeln!(self.output, "  {result} = load i64, ptr {acc_ptr}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
    ...
    fn emit_divzero_checked(&mut self, divisor: &str) -> Result<String, TrustCgError> {
    ...
    /// Emit the `i64::MIN / -1` overflow guard for `sdiv`/`srem`.
    ///
    /// That operand pair is UB in LLVM (the true quotient `2^63` does not fit
    /// in `i64`), so trap before the division — the same fail-closed policy as
    /// [`Self::emit_overflow_checked_binop`].
    fn emit_div_min_overflow_checked(
        &mut self,
        dividend: &str,
        divisor: &str,
    ) -> Result<String, TrustCgError> {
        let is_min = self.fresh_val();
        let is_neg1 = self.fresh_val();
        let overflows = self.fresh_val();
        let trap_block = self.fresh_block("divminovf");
        let safe_block = self.fresh_block("divminok");
        ...
        .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  {is_neg1} = icmp eq i64 {divisor}, -1")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "  {overflows} = and i1 {is_min}, {is_neg1}")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(
            self.output,
            "  br i1 {overflows}, label %{trap_block}, label %{safe_block}"
        )
        .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "{trap_block}:")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
    ...
    /// Emit the strictly-positive-divisor guard for TLA+ `%`
    /// (ModulusNotPositive for divisor <= 0), trapping like
    /// [`Self::emit_divzero_checked`] does for division by zero.
    fn emit_mod_nonpositive_checked(&mut self, divisor: &str) -> Result<String, TrustCgError> {
        let is_nonpositive = self.fresh_val();
        let trap_block = self.fresh_block("modnonpos");
        let safe_block = self.fresh_block("modsafe");

        writeln!(
            self.output,
            "  {is_nonpositive} = icmp sle i64 {divisor}, 0"
        )
        .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(
            self.output,
            "  br i1 {is_nonpositive}, label %{trap_block}, label %{safe_block}"
        )
        .map_err(|e| TrustCgError::Emission(e.to_string()))?;
        writeln!(self.output, "{trap_block}:")
            .map_err(|e| TrustCgError::Emission(e.to_string()))?;
    ...
    fn lower_opcode_with_pc(
        ...
        match *op {
            ...
            Opcode::AddInt { rd, r1, r2 } => {
                ...
                self.emit_overflow_checked_binop(SignedOverflowOp::Add, &a, &b, rd)?;
                self.stats.opcodes_lowered += 1;
            }
            Opcode::SubInt { rd, r1, r2 } => {
                let a = self.emit_load_reg(r1)?;
                let b = self.emit_load_reg(r2)?;
                self.emit_overflow_checked_binop(SignedOverflowOp::Sub, &a, &b, rd)?;
                self.stats.opcodes_lowered += 1;
            }
            Opcode::MulInt { rd, r1, r2 } => {
                let a = self.emit_load_reg(r1)?;
                let b = self.emit_load_reg(r2)?;
                self.emit_overflow_checked_binop(SignedOverflowOp::Mul, &a, &b, rd)?;
                self.stats.opcodes_lowered += 1;
            }
            Opcode::NegInt { rd, rs } => {
                ...
                writeln!(self.output, "  {zero} = add i64 0, 0")
                    .map_err(|e| TrustCgError::Emission(e.to_string()))?;
                self.emit_overflow_checked_binop(SignedOverflowOp::Sub, &zero, &v, rd)?;
                self.stats.opcodes_lowered += 1;
            }
            Opcode::PowInt { rd, r1, r2 } => {
                let base = self.emit_load_reg(r1)?;
                let exp = self.emit_load_reg(r2)?;
                self.emit_pow_checked(&base, &exp, rd)?;
                self.stats.opcodes_lowered += 1;
            }

            // =================================================================
            // Integer division (TLA+/TLC semantics: floored `\div`,
            // Euclidean `%` with a strictly-positive divisor)
            // =================================================================
            Opcode::IntDiv { rd, r1, r2 } => {
                ...
                // i64::MIN / -1 guard: `sdiv`/`srem` of i64::MIN by -1 is UB
                // (the quotient 2^63 does not fit in i64); trap like the
                // overflow-checked binops do.
                self.emit_div_min_overflow_checked(&a, &b)?;
                ...
                // Floor adjustment: if signs differ and remainder != 0, q -= 1.
                let zero = self.fresh_val();
            ...
            Opcode::ModInt { rd, r1, r2 } => {
                ...
                // TLA+/TLC `%` requires a STRICTLY POSITIVE divisor
                // (ModulusNotPositive for b <= 0). The guard subsumes the
                // division-by-zero check and makes i64::MIN % -1 unreachable.
                let _safe = self.emit_mod_nonpositive_checked(&b)?;

                // r = srem (safe: b > 0 here).
                let r = self.fresh_val();
                writeln!(self.output, "  {r} = srem i64 {a}, {b}")
                    .map_err(|e| TrustCgError::Emission(e.to_string()))?;

                // Euclidean adjustment: if r < 0, add b (b > 0, so |b| = b;
                // -b < r < 0 on that lane, so the add cannot overflow).
                let zero = self.fresh_val();
                ...
                writeln!(self.output, "  {r_neg} = icmp slt i64 {r}, {zero}")
                    .map_err(|e| TrustCgError::Emission(e.to_string()))?;
                writeln!(self.output, "  {r_adjusted} = add i64 {r}, {b}")
                    .map_err(|e| TrustCgError::Emission(e.to_string()))?;
                writeln!(
                    self.output,
            ...
            // =================================================================
            Opcode::Jump { offset } => {
                let target_pc = (pc as i64 + i64::from(offset)) as usize;
            ...
            Opcode::FuncExcept {
                ...
                // `[f EXCEPT ![path] = val]` — single-key update. The TIR
                // compiler expands multi-key `![a][b]` paths into nested
                // single-key FuncExcept opcodes, so this one runtime call
                // covers every shape. `tla_func_except` returns NIL_HANDLE
                // (which propagates as the next-state value) on operand shapes
                // it cannot update eagerly, matching the interpreter's
                // `value_except` error arms.
                let f = self.emit_load_reg(func)?;
                let p = self.emit_load_reg(path)?;
                let v = self.emit_load_reg(val)?;
                let result = self.fresh_val();
                writeln!(
                    self.output,
                    "  {result} = call i64 @tla_func_except(i64 {f}, i64 {p}, i64 {v})"
                )
                .map_err(|e| TrustCgError::Emission(e.to_string()))?;
                self.emit_store_reg(rd, &result)?;
                self.stats.opcodes_lowered += 1;
            ...
            Opcode::CallBuiltin {
                ...
                let func_name = match builtin {
                    ...
                    BuiltinOp::RemoveAt => "tla_seq_remove_at",
                    ...
                    BuiltinOp::FoldFunctionOnSetSum => {
                        return Err(TrustCgError::UnsupportedInst(
                            "CallBuiltin(FoldFunctionOnSetSum) requires trust-ir lowering"
                                .to_string(),
                        ));
                    }
                    BuiltinOp::Range => {
                        return Err(TrustCgError::UnsupportedInst(
                            "CallBuiltin(Range) requires bytecode-VM lowering".to_string(),
                        ));
                    }
                    BuiltinOp::RoundApply => {
                        return Err(TrustCgError::UnsupportedInst(
                            "CallBuiltin(RoundApply) requires bytecode-VM lowering".to_string(),
                        ));
                    }
                    // Fail closed: bag ops run on the bytecode-VM path via the
                    // shared value-level helpers; there is no native runtime
                    // implementation, so native lowering declines the action.
                    BuiltinOp::BagAdd | BuiltinOp::BagRemove | BuiltinOp::SetToBag => {
                        return Err(TrustCgError::UnsupportedInst(
                            "CallBuiltin(bag op) requires bytecode-VM lowering".to_string(),
                        ));
                    }
                };
                let args_str = arg_vals
            ...
            // reports/2026-04-20-r26-trust_cg-phase2-readiness.md):
            // Do NOT add a compiled CHOOSE result cache here without
            // state-identity keying. The interpreter's CHOOSE cache was
            // twice made unsound by: (1) missing cache-clear on state
            // transition, (2) pointer-identity keys ABA-unsound. Any
            // compiled CHOOSE cache MUST include state_env.identity() in
            // the key AND be cleared via invalidate_state_identity_tracking().
            // EWD998PCal regresses if this discipline is broken.
            // =================================================================
            Opcode::ForallBegin {
                rd,
                r_binding,
            ...
            Opcode::RoundStepEq { .. } => {
                self.stats.opcodes_skipped += 1;
                return Err(TrustCgError::UnsupportedInst(
                    "RoundStepEq requires bytecode-VM lowering".to_string(),
                ));
            }
            ...
            _ => {
                self.stats.opcodes_skipped += 1;
                return Err(TrustCgError::UnsupportedInst(format!(
                    "TIR opcode {op:?} not supported in direct LLVM IR lowering",
                )));
            }
        }
        Ok(())
    ...
    fn lower_quantifier_begin(
        ...
        let exit_pc = (pc as i64 + i64::from(loop_end)) as usize;
...
fn is_tir_lowering_eligible(op: &Opcode) -> bool {
    if let Opcode::CallBuiltin { builtin, .. } = op {
        // These builtins have no direct-LLVM runtime symbol. RoundApply is also
        // deliberately VM-only because its argument/result use full Value semantics.
        if matches!(
            builtin,
            BuiltinOp::Range | BuiltinOp::FoldFunctionOnSetSum | BuiltinOp::RoundApply
        ) {
            return false;
        }
    }
    ...
    matches!(
        ...
        Opcode::LoadImm { .. }
            ...
            | Opcode::SubInt { .. }
            | Opcode::MulInt { .. }
            // `IntDiv` (`\div`) and `ModInt` (`%`): the emitter implements the
            // interpreter's guarded semantics — FLOORED `\div` (sdiv + floor
            // adjust, i64::MIN/-1 guard) and Euclidean `%` with the
            // strictly-positive-divisor guard — so they are native-eligible.
            // `DivInt` (`/`, exact real division) has never been lowered by
            // this text emitter and stays ineligible here (the trust-ir path
            // in tla-ir handles it).
            | Opcode::IntDiv { .. }
            | Opcode::ModInt { .. }
            | Opcode::NegInt { .. }
            | Opcode::PowInt { .. }
...
mod tests {
    ...
    fn assert_signed_overflow_check(ir: &str, op: &str) {
        assert!(ir.contains("sext i64"), "Should widen operands. IR:\n{ir}");
        assert!(
            ir.contains(&format!("{op} i128")),
            "Should use widened {op}. IR:\n{ir}"
        );
        assert!(
            ir.contains("trunc i128"),
            "Should truncate arithmetic result. IR:\n{ir}"
        );
        assert!(
            ir.contains("icmp ne i128"),
            "Should compare widened roundtrip for overflow. IR:\n{ir}"
        );
        assert!(
            !ir.contains("with.overflow"),
            "Direct lowering should not emit LLVM overflow intrinsics. IR:\n{ir}"
        );
    }

    fn assert_trust_ir_overflow_op(func: &BytecodeFunction, expected: trust_ir::OverflowOp) {
        let module = tla_ir::lower::lower_invariant(
            func,
            "trust_ir_overflow_test",
            tla_ir::lower::LoweringOptions::new(),
        )
        .expect("trust-ir lowering should succeed");
        let found = module
            .functions
            .iter()
    ...
    fn make_powint() -> BytecodeFunction {
        let mut func = BytecodeFunction::new("powint".to_string(), 0);
    ...
    fn test_lower_add_has_overflow_check() {
        ...
        assert_signed_overflow_check(ir, "add");
        assert_trust_ir_overflow_op(&func, trust_ir::OverflowOp::AddOverflow);
        assert!(
            !ir.contains("extractvalue"),
            "Direct lowering should not extract an intrinsic result. IR:\n{ir}"
        );
        assert!(
    ...
    fn test_lower_sub_has_overflow_check() {
        ...
        assert_signed_overflow_check(ir, "sub");
        assert_trust_ir_overflow_op(&func, trust_ir::OverflowOp::SubOverflow);
        assert_eq!(result.stats.overflow_checks, 1);
    }
    ...
    fn test_lower_mul_has_overflow_check() {
        ...
        assert_signed_overflow_check(ir, "mul");
        assert_trust_ir_overflow_op(&func, trust_ir::OverflowOp::MulOverflow);
        assert_eq!(result.stats.overflow_checks, 1);
    }
    ...
    fn test_lower_neg_has_overflow_check() {
        ...
        // Negation is 0 - x, so it uses widened subtraction.
        assert_signed_overflow_check(ir, "sub");
        assert_trust_ir_overflow_op(&func, trust_ir::OverflowOp::SubOverflow);
        assert_eq!(result.stats.overflow_checks, 1);
    }
    ...
    #[test]
    fn test_lower_intdiv_floored_with_guards() {
        ...
        // Floor adjustment for opposite-sign operands: xor + icmp slt + select.
        assert!(
            ir.contains("xor i64") && ir.contains("select"),
            "IntDiv should floor-adjust via xor sign test + select. IR:\n{ir}"
        );
        // i64::MIN / -1 guard (UB in LLVM; quotient 2^63 does not fit i64).
        assert!(
            ir.contains("-9223372036854775808"),
            "IntDiv should guard the i64::MIN / -1 overflow. IR:\n{ir}"
        );
    ...
    fn test_lower_modint_euclidean_with_positive_divisor_guard() {
        ...
        // TLA+ `%` requires a STRICTLY POSITIVE divisor (ModulusNotPositive
        // for b <= 0): the guard is an `icmp sle ..., 0` branch to a trap.
        assert!(
            ir.contains("icmp sle i64"),
            "ModInt should guard the non-positive divisor (b <= 0). IR:\n{ir}"
        );
        assert!(
            ir.contains("select"),
            "ModInt should have the Euclidean correction via select. IR:\n{ir}"
        );
        assert_eq!(result.stats.divzero_checks, 1);
    }

    #[test]
    fn test_powint_is_eligible() {
        // Regression: PowInt was previously excluded from direct-LLVM lowering
        // (functions using `^` fell back to the bytecode VM). It must now be
        // eligible so `^`-bearing invariants/next-state can be natively compiled.
        let func = make_powint();
        assert!(
            is_tir_eligible(&func),
            "a function whose only arithmetic is PowInt must be TIR-lowering eligible"
        );
    }

    #[test]
    fn test_intdiv_modint_eligible_for_native_with_correct_semantics() {
        // `\div` (IntDiv) and `%` (ModInt) are native-eligible again: the
        // emitter now implements the interpreter's guarded semantics exactly
        // (floored `\div` with the i64::MIN/-1 guard, Euclidean `%` with the
        // strictly-positive-divisor guard) — see
        // `test_lower_intdiv_floored_with_guards` and
        // `test_lower_modint_euclidean_with_positive_divisor_guard`.
        for op in [
            Opcode::IntDiv {
                rd: 2,
                r1: 0,
            ...
            assert!(
                is_tir_lowering_eligible(&op),
                "{op:?} must be eligible for native lowering (guarded floored/Euclidean semantics)"
            );
        }

        // `/` (DivInt, exact real division) has never been lowered by this
        // text emitter; it stays ineligible for this fast path (the trust-ir
        // lowering path in tla-ir handles it).
        assert!(
            !is_tir_lowering_eligible(&Opcode::DivInt {
                rd: 2,
                r1: 0,
                r2: 1,
            }),
            "DivInt has no text-emitter lowering and must stay ineligible here"
        );

        // Sanity: a benign arithmetic opcode (AddInt) remains eligible.
        assert!(
            is_tir_lowering_eligible(&Opcode::AddInt {
                rd: 2,
                r1: 0,
                r2: 1,
            }),
            "AddInt must stay eligible for native lowering"
        );

        // A whole function whose only arithmetic is `\div` / `%` is accepted
        // at every native-eligibility level.
        let intdiv = make_intdiv();
        assert!(is_tir_eligible(&intdiv));
        assert!(is_tir_eligible_with_state(&intdiv));
        assert!(is_tir_eligible_with_quantifiers(&intdiv));
        let modint = make_modint();
        assert!(is_tir_eligible(&modint));
        assert!(is_tir_eligible_with_state(&modint));
        assert!(is_tir_eligible_with_quantifiers(&modint));
    }

    #[test]
    fn test_lower_powint_square_and_multiply_with_traps() {
        let func = make_powint();
        let result = lower_tir_to_llvm_ir(&func, "powint_test").expect("should lower");
        let ir = &result.llvm_ir;

        // Square-and-multiply loop structure.
        assert!(
            ir.contains("pow_loop_head"),
            "PowInt should emit a square-and-multiply loop head. IR:\n{ir}"
        );
        assert!(
            ir.contains("lshr i64"),
            "PowInt should halve the exponent via lshr. IR:\n{ir}"
        );
        assert!(
            ir.contains("and i64") && ir.contains(", 1"),
            "PowInt should test the low exponent bit. IR:\n{ir}"
        );
        // Soundness: negative exponent traps (interpreter errors on neg exp).
        assert!(
            ir.contains("pow_neg_exp") && ir.contains("icmp slt i64"),
            "PowInt should guard against negative exponents and trap. IR:\n{ir}"
        );
        // Soundness: overflow of the i64 result traps (interpreter would use BigInt).
        assert!(
            ir.contains("call void @llvm.trap()"),
            "PowInt should trap on overflow / negative exponent. IR:\n{ir}"
        );
        assert!(
            ir.contains("mul i128"),
            "PowInt should use i128-widened overflow-checked multiplies. IR:\n{ir}"
        );
        // Each checked multiply (acc*=base in loop, base*=base in loop, acc*=base
        // in the tail fold) registers an overflow check; there must be at least one.
        assert!(
            result.stats.overflow_checks >= 1,
            "PowInt should register overflow checks, got {}",
            result.stats.overflow_checks
        );
        // 2x LoadImm + PowInt + Ret.
    ...
    #[test]
    fn test_overflow_intrinsics_not_declared() {
        let func = make_add();
        ...
        assert!(
            !ir.contains("with.overflow"),
            "Overflow intrinsics should not be declared or called. IR:\n{ir}"
        );
    }
    ...
    fn test_unsupported_opcode_error() {
        // FuncSet (`[S -> T]`) is NOT supported by direct lowering — it has no
        // runtime helper and falls through to the catch-all UnsupportedInst
        // bail. (FuncExcept used to live here but is now lowered as a
        // @tla_func_except runtime call.)
    ...
    #[test]
    fn round_step_eq_is_rejected_by_native_lowering() {
        let mut func = BytecodeFunction::new("round_step_vm_only".to_string(), 2);
        ...
        let err = lower_tir_to_llvm_ir(&func, "round_step_vm_only").unwrap_err();
        assert!(
            err.to_string()
                .contains("RoundStepEq requires bytecode-VM lowering"),
            "VM-only opcode must fail closed: {err}"
        );
    }

    #[test]
    fn test_lower_func_except_emits_runtime_call() {
        // `[f EXCEPT ![k] = v]` lowers to a @tla_func_except runtime call over
        // the three operand handles (func, path, val).
        let mut func = BytecodeFunction::new("func_except".to_string(), 0);
        func.emit(Opcode::LoadImm { rd: 0, value: 1 }); // f handle
        func.emit(Opcode::LoadImm { rd: 1, value: 2 }); // path handle
        func.emit(Opcode::LoadImm { rd: 2, value: 3 }); // val handle
        ...
        let result = lower_tir_to_llvm_ir(&func, "func_except").expect("should lower");
        let ir = &result.llvm_ir;
        assert!(
            ir.contains("@tla_func_except"),
            "FuncExcept should call @tla_func_except. IR:\n{ir}"
        );
        assert!(
            ir.contains("declare i64 @tla_func_except(i64, i64, i64)"),
            "FuncExcept should declare the runtime helper. IR:\n{ir}"
        );
    }
    ...
    // =========================================================================
    // Comparison: trust-ir-based pipeline vs direct TIR lowering
    // =========================================================================

    #[test]
    fn test_tir_direct_vs_trust_ir_pipeline_both_produce_ir() {
        ...
        // trust-ir pipeline path.
        let trust_ir_result = crate::compile::compile_invariant(&func, "add_trust_ir")
            .expect("trust-ir should compile");
        ...
        assert!(
            trust_ir_result.llvm_ir.contains("define"),
            "trust-ir IR should have function definition"
        );

        // Both paths should contain overflow-checked addition.
        assert!(
            tir_result.llvm_ir.contains("add i128"),
            "Direct TIR IR should use widened add. IR:\n{}",
            tir_result.llvm_ir
        );
        assert_trust_ir_overflow_op(&func, trust_ir::OverflowOp::AddOverflow);
    }
    ...
    #[test]
    fn test_tir_direct_vs_trust_ir_pipeline_comparisons() {
        let func = make_comparison();

        let tir_result = lower_tir_to_llvm_ir(&func, "cmp_tir").expect("TIR should lower");
        let trust_ir_result = crate::compile::compile_invariant(&func, "cmp_trust_ir")
            .expect("trust-ir should compile");
        ...
        assert!(
            trust_ir_result.llvm_ir.contains("icmp sgt"),
            "trust-ir IR should have icmp sgt"
        );
    }

    #[test]
    fn test_tir_direct_vs_trust_ir_pipeline_boolean_ops() {
        let func = make_boolean_and();

        let tir_result = lower_tir_to_llvm_ir(&func, "and_tir").expect("TIR should lower");
        let trust_ir_result = crate::compile::compile_invariant(&func, "and_trust_ir")
            .expect("trust-ir should compile");
        ...
        assert!(
            trust_ir_result.llvm_ir.contains("and i1"),
            "trust-ir IR should have `and i1` (boolean lowering). IR:\n{}",
            trust_ir_result.llvm_ir
        );
    }
    ...
    fn test_lower_if_then_else_with_arithmetic() {
        ...
        assert_signed_overflow_check(ir, "add");
        assert!(ir.contains("sub i128"), "IR:\n{ir}");
    ...
    #[test]
    fn test_fold_function_on_set_sum_requires_trust_ir_pipeline() {
        ...
        assert!(
            err.to_string().contains("requires trust-ir lowering"),
            "{err}"
        );
    }

    #[test]
    fn test_round_apply_requires_bytecode_vm() {
        let mut func = BytecodeFunction::new("round_apply_direct_eligibility".to_string(), 0);
        func.emit(Opcode::LoadImm { rd: 0, value: 0 });
        func.emit(Opcode::CallBuiltin {
            rd: 1,
        ...
        let err = lower_tir_to_llvm_ir(&func, "round_apply_direct_eligibility").unwrap_err();
        assert!(
            err.to_string().contains("requires bytecode-VM lowering"),
            "{err}"
        );
    }
    ...
    fn test_is_tir_eligible_with_quantifiers_rejects_closures() {
        ...
        // Inject an unsupported opcode (FuncSet `[S -> T]` has no runtime
        // helper and is not in quantifier scope). FuncExcept used to be the
        // rejected example here but is now an eligible @tla_func_except call.
        func.emit(Opcode::FuncSet {
            rd: 3,
            domain: 0,
            range: 0,
        ...
        assert!(
            !is_tir_eligible_with_quantifiers(&func),
            "is_tir_eligible_with_quantifiers should reject FuncSet"
        );
    }
