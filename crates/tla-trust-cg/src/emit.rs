...
//! LLVM IR text emission from trust-ir modules.
//!
//! Produces LLVM-compatible `.ll` text IR from a [`trust_ir::Module`]. This is
//! the bridge layer: trust-ir is already close to LLVM IR (SSA, typed, explicit
//! memory ops), so the translation is mostly syntactic.
//!
//! When the trust-codegen crate becomes available as a workspace dependency, this
//! textual emitter can be replaced by direct trust-codegen API calls. For now, the
...
use trust_ir::inst::{
    AtomicRMWOp, BinOp, CastOp, FCmpOp, ICmpOp, Inst, Ordering, OverflowOp, UnOp,
};
use trust_ir::ty::Ty;
use trust_ir::value::BlockId;
use trust_ir::{Block, Function, Module};

/// Helper macro to reduce `.map_err(|e| TrustCgError::Emission(e.to_string()))`
/// boilerplate on every `write!`/`writeln!` call.
macro_rules! emit {
    ($dst:expr, $($arg:tt)*) => {
        write!($dst, $($arg)*).map_err(|e| TrustCgError::Emission(e.to_string()))
    };
}

macro_rules! emitln {
    ($dst:expr) => {
        writeln!($dst).map_err(|e| TrustCgError::Emission(e.to_string()))
    };
    ($dst:expr, $($arg:tt)*) => {
        writeln!($dst, $($arg)*).map_err(|e| TrustCgError::Emission(e.to_string()))
    };
}

/// Emit LLVM IR text for an entire trust-ir module.
///
/// Returns the complete `.ll` file content as a string.
///
/// # Errors
///
/// Returns [`TrustCgError::Emission`] if any instruction cannot be translated.
pub(crate) fn emit_module(module: &Module) -> Result<String, TrustCgError> {
...
struct EmitCtx<'m> {
    ...
    assert_counter: u32,
    /// Counter for generating unique temporary register names (%_`tmp_N`).
...
impl<'m> EmitCtx<'m> {
    ...
    /// Resolve a trust-ir type to its LLVM IR text representation.
    ///
    ...
    fn emit_module(&mut self) -> Result<(), TrustCgError> {
        ...
        emitln!(self.output, "declare i8 @llvm.ctpop.i8(i8)")?;
        emitln!(self.output, "declare i16 @llvm.ctpop.i16(i16)")?;
        emitln!(self.output, "declare i32 @llvm.ctpop.i32(i32)")?;
        emitln!(self.output, "declare i64 @llvm.ctpop.i64(i64)")?;
        emitln!(self.output, "declare i128 @llvm.ctpop.i128(i128)")?;
    ...
    fn emit_function(&mut self, func: &Function) -> Result<(), TrustCgError> {
        ...
        let entry_block = func.blocks.first().ok_or_else(|| {
            TrustCgError::Emission(format!("function '{}' has no blocks", func.name))
        })?;
    ...
    fn emit_block(&mut self, block: &Block, is_entry: bool) -> Result<(), TrustCgError> {
    ...
    fn emit_instruction(&mut self, node: &trust_ir::InstrNode) -> Result<(), TrustCgError> {
        emit!(self.output, "  ")?;
        ...
        match &node.inst {
            // Rust-backend constructs (heap alloc / module-global address);
            // TLA+ model-checking lowering never emits them (rejected earlier
            // by `check_inst_supported`), so reaching here is an upstream bug.
            Inst::HeapAlloc { .. } | Inst::GlobalAddr { .. } => {
                return Err(TrustCgError::Emission(
                    "HeapAlloc/GlobalAddr are not supported by LLVM text emission".to_string(),
                ));
            }
            // Coroutine yield terminator. TLA+ model-checking lowering never
            // emits coroutines (rejected earlier by `check_inst_supported`), and
            // this text emitter has no lowering for it, so reaching here is an
            // upstream bug.
            Inst::CoroSuspend { .. } => {
                return Err(TrustCgError::Emission(
                    "CoroSuspend (coroutine yield) is not supported by LLVM text emission"
                        .to_string(),
                ));
            }
            ...
            Inst::UnOp { op, ty, operand } => {
                ...
                match op {
                    ...
                    UnOp::FAbs | UnOp::FSqrt | UnOp::FFloor | UnOp::FCeil | UnOp::FTrunc => {
                        return Err(TrustCgError::Emission(format!(
                            "floating-point UnOp {op:?} is not supported by the TLA+ trust-cg backend"
                        )));
                    }
                }
            }
            ...
            Inst::Cast {
                ...
                if matches!(op, CastOp::PtrToPtr) {
                ...
                } else if let Some(cast_op) = llvm_cast(op) {
                    emitln!(
                        self.output,
                        "%{} = {} {} %{} to {}",
                        r.index(),
                    ...
                    return Err(TrustCgError::Emission(format!(
                        "cast op {op:?} is not supported by LLVM text emission"
                    )));
                }
            }
            Inst::Load { ty, ptr, .. } => {
            ...
            Inst::Const { ty, value } => {
                let r = result
                    .ok_or_else(|| TrustCgError::Emission("Const has no result".to_string()))?;
            ...
            Inst::NullPtr => {
                let r = result
                    .ok_or_else(|| TrustCgError::Emission("NullPtr has no result".to_string()))?;
            ...
            Inst::Copy { ty, operand } => {
                ...
                match ty {
                    ...
                    // Aggregate and wide-pointer values cannot use `add`;
                    // lower to alloca+store+load for a correct copy.
                    Ty::Struct(_)
                    | Ty::Array(_, _)
                    | Ty::Vector(_, _)
                    | Ty::Tuple(_)
            ...
            // they are higher-level trust-ir constructs for ownership tracking.
            Inst::Borrow { .. }
            | Inst::BorrowMut { .. }
            | Inst::EndBorrow { .. }
            | Inst::Retain { .. }
            ...
            // Binding-frame instructions (trust_ir 67f1fdc+) track SSA binding
            ...
            | Inst::CloseFrame { .. } => {
                return Err(TrustCgError::UnsupportedInst(
                    "binding-frame instruction not yet supported in trust-codegen emission"
                        .to_string(),
                ));
            }
            // Pointer metadata operations are explicit trust-ir operations for
            // wide pointer decomposition/reconstruction. They must be lowered
            // through a layout-aware trust-codegen path before text emission.
            Inst::PtrData { .. } | Inst::PtrMetadata { .. } | Inst::PtrFromParts { .. } => {
                return Err(TrustCgError::UnsupportedInst(
                    "pointer metadata instruction not yet supported in trust-codegen emission"
                        .to_string(),
                ));
            }
            // Sequence give-back ops (structural element-wise sequence maps,
            // trust_ir SeqMapAddK/SeqMapNot). The LLVM-text emitter has no
            // lowering for sequence maps and TLA+ model-checking lowering never
            // emits them, so fail closed rather than emit incorrect IR.
            Inst::SeqMapAddK { .. } | Inst::SeqMapNot { .. } | Inst::SeqMap { .. } => {
                return Err(TrustCgError::UnsupportedInst(
                    "structural sequence map (SeqMapAddK/SeqMapNot/SeqMap) not supported in \
                     trust-codegen emission"
                        .to_string(),
                ));
            }
            // Dialect-specific instructions carry opaque payloads and must be
            // lowered to core trust-ir before reaching LLVM emission.
            Inst::DialectOp(_) => {
                return Err(TrustCgError::UnsupportedInst(
                    "dialect op not yet supported in trust-codegen emission".to_string(),
                ));
            }
            // Exception-handling opcodes (trust_ir exception scaffold). The
            // LLVM-text emitter has no lowering for the unwind path — Invoke's
            // landing-pad edge, LandingPad's EH-register reads, Resume's
            // _Unwind_Resume tail — and TLA+ model-checking lowering never emits
            // them, so fail closed rather than emit incorrect IR.
    ...
    fn format_constant(&self, c: &Constant, ty: &Ty) -> Result<String, TrustCgError> {
        ...
        match c {
            // Rust-backend relocatable symbol address; TLA+ lowering never
            // produces it (addresses are only assigned at link time).
            Constant::SymbolAddr { .. } => Err(TrustCgError::Emission(
                "SymbolAddr constants are not supported by LLVM text emission".to_string(),
            )),
            Constant::Int(v) => Ok(format!("{v}")),
            // trust-ir v24 U128: TLA+ lowering never produces it (naturals
            // lower through I64/I128 as Int) — fail closed like SymbolAddr.
            Constant::U128(_) => Err(TrustCgError::Emission(
                "U128 constants are not produced by TLA+ lowering (v24 carrier)".to_string(),
            )),
            // v25 Bytes: likewise never produced by TLA+ lowering.
            Constant::Bytes { .. } => Err(TrustCgError::Emission(
                "Bytes constants are not produced by TLA+ lowering (v25 carrier)".to_string(),
            )),
            Constant::Float(v) => Ok(format!("{v:e}")),
            Constant::Bool(b) => {
                if *b {
                    Ok("1".to_string())
                } else {
            ...
            Constant::Aggregate(elems) | Constant::Array(elems) | Constant::Vector(elems) => {
                let parts: Vec<String> = match ty {
                    ...
                    _ => {
                        return Err(TrustCgError::Emission(format!(
                            "aggregate constants for type {ty:?} are not supported by LLVM text emission"
                        )));
                    }
                };
            ...
            | Constant::Set(_)
            | Constant::Record(_)
            | Constant::Closure { .. } => Ok("null".to_string()),
            Constant::FnDef(func) => {
                let name = self.func_names.get(&func.index()).ok_or_else(|| {
                    TrustCgError::Emission(format!(
                        "FnDef constant references unknown function {}",
                        func.index()
                    ))
                })?;
                Ok(format!("@{name}"))
            }
            Constant::PhantomData => Err(TrustCgError::Emission(
                "PhantomData constants have no LLVM IR value representation".to_string(),
            )),
        }
    }
}
...
fn llvm_cast(op: &CastOp) -> Option<&'static str> {
    match op {
        CastOp::Trunc => Some("trunc"),
        CastOp::ZExt => Some("zext"),
        CastOp::SExt => Some("sext"),
        CastOp::FPTrunc => Some("fptrunc"),
        CastOp::FPExt => Some("fpext"),
        CastOp::FPToUI => Some("fptoui"),
        CastOp::FPToSI => Some("fptosi"),
        CastOp::UIToFP => Some("uitofp"),
        CastOp::SIToFP => Some("sitofp"),
        CastOp::PtrToInt => Some("ptrtoint"),
        CastOp::IntToPtr => Some("inttoptr"),
        CastOp::Bitcast => Some("bitcast"),
        // Saturating float->int casts have no single LLVM-text cast keyword
        // (they are the `llvm.fptosi.sat`/`llvm.fptoui.sat` intrinsics): None
        // = fail-closed, like Transmute — never the non-saturating fpto*i,
        // whose semantics differ exactly on the NaN/out-of-range edge cases.
        CastOp::PtrToPtr
        | CastOp::Transmute
        | CastOp::ReifyFnPointer
        | CastOp::FPToSISat
        | CastOp::FPToUISat => None,
    }
}
...
fn llvm_ctpop_intrinsic(ty: &Ty) -> Option<&'static str> {
    match ty {
        ...
        Ty::I16 | Ty::U16 => Some("llvm.ctpop.i16"),
        Ty::I32 | Ty::U32 => Some("llvm.ctpop.i32"),
        Ty::I64 | Ty::U64 => Some("llvm.ctpop.i64"),
        Ty::I128 | Ty::U128 => Some("llvm.ctpop.i128"),
        _ => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn test_emit_unop_ctpop() {
        let mut module = Module::new("ctpop");
        let ft = module.add_func_type(FuncTy {
            params: vec![Ty::I64],
            returns: vec![Ty::I64],
            is_vararg: false,
        ...
        let entry = BlockId::new(0);
        let mut func = Function::new(FuncId::new(0), "count_bits", ft, entry);
        let mut block = Block::new(entry).with_param(ValueId::new(10), Ty::I64);
        block.body.push(
        ...
        let ir = emit_module(&module).expect("should emit ctpop");
        assert!(ir.contains("declare i64 @llvm.ctpop.i64(i64)"));
        assert!(ir.contains("%0 = call i64 @llvm.ctpop.i64(i64 %10)"));
    }

    #[test]
    fn test_emit_lowered_compact_set_bitmask_cardinality_uses_ctpop_not_runtime() {
        ...
        .expect("compact SetBitmask Cardinality should lower to trust-ir ctpop");
        let ir = emit_module(&module).expect("lowered compact cardinality should emit LLVM IR");

        assert_eq!(
            ir.matches("call i64 @llvm.ctpop.i64").count(),
            1,
            "compact SetBitmask Cardinality should emit one ctpop call. IR:\n{ir}"
        );
        assert!(
            !ir.contains("call i64 @tla_cardinality"),
            "compact SetBitmask Cardinality must not fall back to runtime cardinality. IR:\n{ir}"
        );
        assert!(
            !ir.contains("inttoptr"),
            "compact SetBitmask Cardinality must not materialize the mask as a handle pointer. IR:\n{ir}"
        );
    }

    #[test]
    fn test_ctpop_intrinsic_names_integer_widths() {
        assert_eq!(llvm_ctpop_intrinsic(&Ty::I8).unwrap(), "llvm.ctpop.i8");
        assert_eq!(llvm_ctpop_intrinsic(&Ty::U32).unwrap(), "llvm.ctpop.i32");
        assert_eq!(llvm_ctpop_intrinsic(&Ty::I64).unwrap(), "llvm.ctpop.i64");
        assert!(llvm_ctpop_intrinsic(&Ty::Ptr).is_none());
    }
