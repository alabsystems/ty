...
use ay_chc::{engines, AdaptiveConfig, AdaptivePortfolio, PdrConfig, VerifiedChcResult};
...
pub(crate) fn bmc_preprocess(
    ...
    match result {
        VerifiedChcResult::Unsafe(cex) => {
            ...
            // inputs) from the derivation witness, so a bit-blast-ineligible net
            // whose counterexample the BMC lane finds still yields a standard,
            // replay-verified witness. `program` here is what was translated, so
            // its state node ids align with the reconstruction.
        ...
        VerifiedChcResult::Safe(inv) => {
            ...
            // with proof, we should trust it. Independently re-verify the
            // invariant on a fresh solver before surfacing SAFE. Fail-CLOSED:
            // only a confirmed Ok(true) (the invariant provably excludes every
            // bad state) surfaces SAFE/Unsat. An Err means the re-verification
            // could not be completed, so the proof is unverifiable -> Unknown,
            // never SAFE. Ok(false) means the invariant permits a bad state.
            let results: Vec<Btor2CheckResult> =
