...
//!
//! Parses the line-oriented BTOR2 format (Niemetz, Preiner, Wolf, Biere; CAV
//! 2018). Each non-comment line defines one node:
//! `<id> <opcode> [<sort>] [<args>...] [<symbol>]`. Text from `;` to end of line
//! is a comment; blank and comment-only lines are skipped.
//!
//! The entry points are [`parse`] (from a string) and [`parse_file`] (from a
//! path); [`parse_btor2`] is a backward-compatibility alias for [`parse`]. After
//! tokenizing, the parser fully validates the netlist: node ids must be positive
//! and unique, operator operand counts must match, every referenced node id must
//! be defined, and every sort reference must point to a `sort` node. A returned
//! [`Btor2Program`] is therefore well-formed, with all cross-references resolved.
...
/// # Errors
///
/// Returns [`Btor2Error`] on malformed input:
/// - [`Btor2Error::ParseError`] — a line is not `<id> <opcode> ...`, an id is
///   non-positive or non-numeric, an opcode is unknown, or a bitvector sort
///   exceeds [`MAX_BV_WIDTH`].
/// - [`Btor2Error::DuplicateId`] — the same node id is defined twice.
/// - [`Btor2Error::InvalidArgCount`] — an operator has the wrong operand count.
/// - [`Btor2Error::InvalidSort`] / [`Btor2Error::NotASort`] — a sort reference
///   is undefined or points to a non-`sort` node.
/// - [`Btor2Error::UndefinedNode`] — an operand references an undefined node id.
...
/// # Errors
///
/// Returns [`Btor2Error::Io`] if the file cannot be read, or any error from
/// [`parse`] if its contents are not valid BTOR2.
...
fn parse_line(
    ...
    match opcode {
        ...
        // -- Init / Next -----------------------------------------------------
        "init" => parse_ternary_sorted(id, tokens, line_num, Btor2Node::Init),
        "next" => parse_ternary_sorted(id, tokens, line_num, Btor2Node::Next),
    ...
    match tokens[2] {
        "bitvec" => {
            ...
            if width > MAX_BV_WIDTH {
                // Fail-closed: widths above MAX_BV_WIDTH cannot be represented
                // faithfully (the u128 value/arith model would truncate/mask
                // mod 2^128, yielding a verdict over the wrong model).
                return Err(Btor2Error::ParseError {
                    line: line_num,
                    message: format!(
                        "bitvec width {width} exceeds maximum supported width {MAX_BV_WIDTH}"
                    ),
                });
            }
            Ok(Btor2Line {
...
";
    ...
    // Regression: bug #17. parse_sort previously only rejected width == 0, so a
    // width above MAX_BV_WIDTH (128) was accepted and later truncated/masked.
    // The parser must now DECLINE oversized widths up front. This only builds a
    // 2-line string and asserts the ParseError — no wide allocation occurs.
    #[test]
    fn regression_parse_sort_rejects_oversized_width() {
        let input = "1 sort bitvec 200\n";
        let err = parse(input).unwrap_err();
        assert!(
            matches!(err, Btor2Error::ParseError { .. }),
            "expected ParseError for 200-bit sort, got: {err}"
        );

        // Boundary: exactly MAX_BV_WIDTH is accepted; one above is not.
        assert!(parse(&format!("1 sort bitvec {MAX_BV_WIDTH}\n")).is_ok());
        assert!(parse(&format!("1 sort bitvec {}\n", MAX_BV_WIDTH + 1)).is_err());
    }
}
