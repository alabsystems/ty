...
//! Direct BTOR2-to-CHC translation and safety checking via the `ay-chc`
//! portfolio solver.
//!
//! Each BTOR2 `bad` property is encoded as a CHC safety query over a fresh
...
//! ```
//!
//! The portfolio solver (PDR, BMC, k-induction, ...) then decides reachability
//! of `bad`: UNSAT means the property holds, SAT yields a counterexample trace.
//! [`check_btor2`] is the entry point; it builds one query per `bad` property
//! and returns one [`Btor2CheckResult`] for each.
//!
//! This is the straightforward node-expanding translator. For HWMCC-scale
//! benchmarks prefer [`crate::to_chc::check_btor2_adaptive`] (memoized,
//! DAG-aware) or the full [`crate::portfolio::check_btor2_portfolio`].
...
pub enum Btor2CheckResult {
    ...
    Sat {
        ...
        /// Concrete per-frame word-level model (array cells + inputs) recovered
        /// from `ay-chc`'s derivation witness, re-keyed to BTOR2 node ids. Set by
        /// the word-level CHC lanes ([`crate::to_chc::check_btor2_adaptive`],
        /// [`crate::check_btor2_portfolio`]); `None` on the bit-level/GPU/BMC
        /// lanes whose traces are already bit-level replayable. Enables a
        /// standard, replay-verified witness for bit-blast-ineligible nets (e.g.
        /// wide-index arrays) via [`crate::build_word_level_witness`].
        model: Option<crate::word_replay::WordLevelModel>,
    },
    /// Solver could not determine the result.
    Unknown {
        /// Why the result is indeterminate (timeout, solver `unknown`, etc.).
        reason: String,
    },
}

/// Translate a [`Btor2Program`] into a CHC problem and solve each `bad`
/// property.
///
/// Returns one [`Btor2CheckResult`] per `bad` property, in property order. A
/// program with no `bad` properties returns an empty vector.
///
/// # Errors
///
/// Returns [`Btor2Error`] if translation fails — for example a bitvector sort
/// wider than [`MAX_BV_WIDTH`], an unparseable constant, or an operand
/// referencing an undefined node id.
///
/// # Panics
///
/// Translation indexes operator operands positionally (`args[0]`, `args[1]`,
/// ...). A [`Btor2Program`] produced by [`crate::parse`] is operand-count
/// validated and is safe; a hand-constructed program whose operator nodes carry
/// too few operands can panic with an out-of-bounds index.
...
fn check_single_bad(
    ...
    let portfolio_config = ay_chc::PortfolioConfig::default();
    let solver = ay_chc::testing::new_portfolio_solver(problem, portfolio_config);
    let result = solver.solve();

    match result {
        ay_chc::ChcEngineResult::Safe(_) => Ok(Btor2CheckResult::Unsat),
        ay_chc::ChcEngineResult::Unsafe(cex) => {
            let trace = cex
                .steps
                .iter()
                .map(|step| {
                ...
                .collect();
            Ok(Btor2CheckResult::Sat { trace, model: None })
        }
    ...
    fn translate_line(&self, line: &crate::types::Btor2Line) -> Result<ChcExpr, Btor2Error> {
        match &line.node {
            ...
            Btor2Node::Const(bits) => {
                ...
                // Fail-closed: an unparseable/overflowing constant must DECLINE,
                // never silently become 0 (a wrong-model verdict).
                let val = u128::from_str_radix(bits, 2).map_err(|_| Btor2Error::ParseError {
                    line: 0,
            ...
            Btor2Node::ConstD(dec) => {
                ...
                let val: u128 = if let Some(stripped) = dec.strip_prefix('-') {
                    let abs: u128 = stripped.parse().map_err(|_| Btor2Error::ParseError {
                        line: 0,
                    ...
                    dec.parse().map_err(|_| Btor2Error::ParseError {
                        line: 0,
                        message: format!("decimal const out of range for width {width}: {dec}"),
                    })?
                };
            ...
            Btor2Node::ConstH(hex) => {
                let width = self.resolve_bv_width(line.sort_id);
                let val = u128::from_str_radix(hex, 16).map_err(|_| Btor2Error::ParseError {
                    line: 0,
...
fn btor2_sort_to_chc(sorts: &HashMap<i64, Btor2Sort>, sort_id: i64) -> Result<ChcSort, Btor2Error> {
    match sorts.get(&sort_id) {
        Some(sort) => btor2_sort_to_chc_direct(sort),
        None => Err(Btor2Error::InvalidSort { line: 0, sort_id }),
    }
}

fn btor2_sort_to_chc_direct(sort: &Btor2Sort) -> Result<ChcSort, Btor2Error> {
    match sort {
        Btor2Sort::BitVec(w) => {
            // Fail-closed: a width above MAX_BV_WIDTH cannot be modeled with the
            // u128-backed constant/arith encoding (truncation/masking mod 2^128
            // would yield a verdict over the wrong model). Decline instead.
            if *w > MAX_BV_WIDTH {
