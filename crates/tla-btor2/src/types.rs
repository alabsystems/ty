...
//! Core intermediate representation for the BTOR2 format.
//!
//! A parsed program is a [`Btor2Program`]: a flat vector of [`Btor2Line`]s in
//! source order, each carrying a node id, a result [`Btor2Sort`], an opcode
//! ([`Btor2Node`]), and its operand ids. Operand ids may be *negative* to denote
//! the bitwise negation of the referenced node — a BTOR2 shorthand the
//! translators honor everywhere. The program additionally pre-indexes the
//! resolved sort table and the line ids of properties (`bad`, `constraint`,
//! `fair`, `justice`) so consumers need not rescan the netlist.
//!
//! The opcode list mirrors `enum Btor2Tag` in `btor2parser.h` (btor2tools,
//! Biere et al.).
...
pub enum Btor2Sort {
    ...
    Array {
        /// Sort of array indices (the address space).
        index: Box<Btor2Sort>,
        /// Sort of array elements (the stored values).
...
pub enum Btor2Node {
    ...
    // -- Indexed ops ---------------------------------------------------------
    /// `slice <upper> <lower>` — extract the inclusive bit range
    /// `upper` down to `lower`.
    Slice(u32, u32),
