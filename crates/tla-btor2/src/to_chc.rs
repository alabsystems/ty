...
//! Memoized BTOR2-to-CHC translation for the `ay-chc` PDR/BMC/k-induction
//! portfolio.
//!
//! Like [`crate::translate`] this lowers a BTOR2 program to a CHC safety query,
//! but it shares sub-expressions through a memoized translator so a BTOR2 DAG
//! does not blow up into an exponential tree — the form required for HWMCC-scale
//! benchmarks. The translation scheme is:
//!
//! ```text
//! state      -> ChcVar argument of the invariant predicate Inv
//! input      -> variable free in the transition clause (existential)
//! init       -> init clause:        init_constraint        => Inv(state)
//! next       -> transition clause:  Inv(curr) /\ trans      => Inv(next)
//! bad        -> query clause:       Inv(curr) /\ bad        => false
//! constraint -> conjoined into the transition relation
//! ```
//!
//! [`translate_to_chc`] returns the [`TranslationResult`] (problem, state-var
//! map, and per-`bad` query-clause indices); [`check_btor2_adaptive`] is the
//! one-shot translate-and-solve entry point, with proof-backed SAFE verdicts.

use std::sync::Arc;
use std::time::Duration;

use ay_chc::{
    engines, AdaptiveConfig, AdaptivePortfolio, ChcExpr, ChcOp, ChcProblem, ChcSort, ChcVar,
    ClauseBody, ClauseHead, HornClause, PdrConfig, VerifiedChcResult,
};
...
impl<'a> Translator<'a> {
    ...
    fn resolve_sorts(&mut self) -> Result<(), Btor2Error> {
        for (&sort_id, sort) in &self.program.sorts {
            // Fail-closed: declines width > MAX_BV_WIDTH before any wide
            // constant is materialized, so check_btor2_adaptive is covered too.
    ...
    fn translate_node_inner(&mut self, node_id: i64) -> Result<ChcExpr, Btor2Error> {
        ...
        match &line.node {
            ...
            Btor2Node::Const(bits) => {
                let sort = self.get_sort(line.sort_id)?;
                parse_binary_const(bits, &sort)
            }
            Btor2Node::ConstD(dec) => {
                let sort = self.get_sort(line.sort_id)?;
                parse_decimal_const(dec, &sort)
            }
            Btor2Node::ConstH(hex) => {
                let sort = self.get_sort(line.sort_id)?;
                parse_hex_const(hex, &sort)
            }
    ...
    /// Body of the init clause (`init_constraint`), exactly as the fact
    /// `init_constraint => Inv(curr)` uses it. `Bool(true)` when there is no
    /// `init` line.
    fn init_constraint_expr(&self) -> ChcExpr {
        if self.init_conjuncts.is_empty() {
            ChcExpr::Bool(true)
        } else {
            ChcExpr::and_all(self.init_conjuncts.clone())
        }
    }

    /// Body constraint of the transition clause (`trans`), exactly as
    /// `Inv(curr) /\ trans => Inv(next)` uses it: the conjunction of every
    /// `next_var = next_expr` equality and every `constraint`. `None` when the
    /// transition places no constraint.
    fn trans_constraint_expr(&self) -> Option<ChcExpr> {
    ...
    /// Per-`bad` query bodies (`bv1_to_bool(bad)`), exactly as
    /// `Inv(curr) /\ bad => false` uses them (one per property, in order).
    fn bad_body_exprs(&self) -> Vec<ChcExpr> {
        self.bad_exprs
            .iter()
            .map(|bad| bv1_to_bool(bad.clone()))
            .collect()
    }
    ...
    fn assemble(self) -> Result<TranslationResult, Btor2Error> {
        ...
        // Bodies of the three clause families, shared verbatim with the
        // independent certifier (array_cert) so its VCs are the exact same
        // obligations this problem encodes.
        let init_constraint = self.init_constraint_expr();
        let trans_constraint = self.trans_constraint_expr();
        let bad_bodies = self.bad_body_exprs();
        ...
        let mut property_indices = Vec::with_capacity(bad_bodies.len());
        for bad_body in bad_bodies {
            let clause_idx = problem.clauses().len();
            property_indices.push(clause_idx);
            problem.add_clause(HornClause::new(
                ClauseBody::new(vec![(inv_pred, curr_args.clone())], Some(bad_body)),
                ClauseHead::False,
            ));
        }
...
/// The exact bodies of the three CHC clause families, extracted so the
/// independent SAFE certifier ([`crate::array_cert`]) can rebuild the verification
/// conditions from the *same* expressions the PDR portfolio solved — never a
/// re-derivation that could drift.
#[derive(Debug, Clone)]
pub(crate) struct VcComponents {
    /// State variables (current + next), in predicate-argument order.
    pub state_entries: Vec<StateVarEntry>,
    /// `init_constraint` — body of `init_constraint => Inv(curr)`.
    pub init_constraint: ChcExpr,
    /// `trans` — body constraint of `Inv(curr) /\ trans => Inv(next)`.
    pub trans_constraint: Option<ChcExpr>,
    /// `bv1_to_bool(bad)` per property — body of `Inv(curr) /\ bad => false`.
    pub bad_bodies: Vec<ChcExpr>,
}

/// Translate to CHC and also return the raw [`VcComponents`] (init/trans/bad
/// clause bodies). The returned [`TranslationResult`] is identical to
/// [`translate_to_chc`]'s; the components are the same expressions its clauses
/// are assembled from.
pub(crate) fn translate_to_chc_with_vc(
    program: &Btor2Program,
) -> Result<(TranslationResult, VcComponents), Btor2Error> {
    ...
    let components = VcComponents {
        state_entries: translator.state_entries.clone(),
        init_constraint: translator.init_constraint_expr(),
        trans_constraint: translator.trans_constraint_expr(),
        bad_bodies: translator.bad_body_exprs(),
    };
    let result = translator.assemble()?;
...
/// Translate a parsed BTOR2 program into a ay-chc `ChcProblem`.
///
...
/// # Errors
///
/// Returns [`Btor2Error`] if the program cannot be faithfully lowered — for
/// example a bitvector sort wider than [`MAX_BV_WIDTH`], an unparseable
/// constant, or a reference to an undefined node id.
///
/// # Panics
///
/// Operator operands are read positionally. A program from [`crate::parse`] is
/// operand-count validated and safe; a hand-built program whose operator nodes
/// carry too few operands can panic on an out-of-bounds index.
...
///
/// Returns one [`Btor2CheckResult`](crate::translate::Btor2CheckResult) per
/// `bad` property (empty if there are none). `time_budget` bounds the solver;
/// `None` uses the portfolio default. SAFE verdicts are proof-backed: the
/// discovered invariant is independently re-verified before being reported as
/// `Unsat`, and downgraded to `Unknown` (never reported SAFE) if that
/// re-verification fails.
///
/// # Errors
///
/// Returns [`Btor2Error`] if [`translate_to_chc`] fails on the program
/// (over-wide sort, unparseable constant, undefined node reference).
///
/// # Panics
///
/// Shares the positional-operand panic contract of [`translate_to_chc`] for
/// hand-constructed programs with too few operator operands.
pub fn check_btor2_adaptive(
    ...
    // Capture the problem before it is moved into the portfolio, so a Safe
    // verdict's invariant can be independently re-verified (proof-backed SAFE).
    ...
    match result {
        VerifiedChcResult::Safe(inv) => {
            // Proof-back SAFE: independently re-verify the invariant excludes the
            // bad states on a fresh solver. Fail-CLOSED: only a confirmed
            // Ok(true) (the invariant provably excludes all bad states) surfaces
            // SAFE/Unsat. An Err means the re-verification itself could not be
            // completed, so the safety proof is unverifiable -> Unknown, never
            // SAFE. Ok(false) means the invariant permits a bad state -> Unknown.
            match engines::external_invariant_model_excludes_error(
                &chc_problem,
                inv.model(),
                &PdrConfig::default(),
            ) {
                Ok(true) => Ok((0..n).map(|_| Btor2CheckResult::Unsat).collect()),
                Ok(false) => Ok((0..n)
                    .map(|_| Btor2CheckResult::Unknown {
                        reason: "btor2 SAFE invariant permits a bad state on independent re-verify"
                            .into(),
                    })
                    .collect()),
                Err(e) => Ok((0..n)
                    .map(|_| Btor2CheckResult::Unknown {
                        reason: format!(
                            "btor2 SAFE invariant could not be independently re-verified: {e}"
                        ),
                    })
                    .collect()),
            }
        ...
        VerifiedChcResult::Unsafe(cex) => {
            ...
            // Recover the concrete per-frame word-level model (array cells +
            // inputs) from the derivation witness — the `check_btor2_adaptive`
            // path translates the ORIGINAL program, so its state node ids align
            // directly with the caller's program for a replay-verified witness.
...
fn btor2_sort_to_chc(sort: &Btor2Sort) -> Result<ChcSort, Btor2Error> {
    match sort {
        Btor2Sort::BitVec(w) => {
            // Fail-closed: a width above MAX_BV_WIDTH cannot be modeled with the
            // u128-backed constant/arith encoding (a >128-bit constant would
            // truncate / arithmetic would mask mod 2^128), so decline up front
            // before btor2_sort_to_chc materializes anything.
            if *w > MAX_BV_WIDTH {
                return Err(Btor2Error::ParseError {
                    line: 0,
                    message: format!(
                        "bitvec width {w} exceeds maximum supported width {MAX_BV_WIDTH}"
                    ),
                });
            }
            Ok(ChcSort::BitVec(*w))
        }
        Btor2Sort::Array { index, element } => Ok(ChcSort::Array(
...
fn parse_binary_const(bits: &str, sort: &ChcSort) -> Result<ChcExpr, Btor2Error> {
    let width = match sort {
        ChcSort::BitVec(w) => *w,
        _ => bits.len() as u32,
    };
    // Fail-closed: an unparseable/overflowing constant (e.g. a >128-bit value)
    // must DECLINE, never silently become 0 — a 0 here is a wrong-model verdict.
    let value = u128::from_str_radix(bits, 2).map_err(|_| Btor2Error::ParseError {
        line: 0,
        message: format!("binary const out of range for width {width}: {bits}"),
    })?;
    Ok(ChcExpr::BitVec(value & bv_mask(width), width))
}

fn parse_decimal_const(dec: &str, sort: &ChcSort) -> Result<ChcExpr, Btor2Error> {
    let width = match sort {
        ChcSort::BitVec(w) => *w,
        _ => 32,
    };
    let value = if let Some(stripped) = dec.strip_prefix('-') {
        let abs_val = stripped
            .parse::<u128>()
            .map_err(|_| Btor2Error::ParseError {
                line: 0,
                message: format!("decimal const out of range for width {width}: {dec}"),
            })?;
        if width < 128 {
        ...
        dec.parse::<u128>().map_err(|_| Btor2Error::ParseError {
            line: 0,
            message: format!("decimal const out of range for width {width}: {dec}"),
        })? & bv_mask(width)
    };
...
fn parse_hex_const(hex: &str, sort: &ChcSort) -> Result<ChcExpr, Btor2Error> {
    let width = match sort {
        ChcSort::BitVec(w) => *w,
        _ => (hex.len() * 4) as u32,
    };
    let value = u128::from_str_radix(hex, 16).map_err(|_| Btor2Error::ParseError {
        line: 0,
        message: format!("hex const out of range for width {width}: {hex}"),
    })?;
    Ok(ChcExpr::BitVec(value & bv_mask(width), width))
}
...
mod tests {
    ...
    fn test_decimal_const_negative() {
        ...
        assert_eq!(
            parse_decimal_const("-1", &sort).unwrap(),
            ChcExpr::BitVec(255, 8)
        );
        assert_eq!(
            parse_decimal_const("42", &sort).unwrap(),
            ChcExpr::BitVec(42, 8)
        );
    }
...
";
    ...
    // -- Regression: bug #1 (CRITICAL WRONG VERDICT) ------------------------
    //
    // A sort wider than MAX_BV_WIDTH (128) plus a >128-bit constant must
    // DECLINE before any value is materialized. Previously the u128-backed
    // const path silently truncated the constant (or made it 0) and masked
    // arithmetic mod 2^128, computing SAFE/UNSAFE over the WRONG model.
    //
    // These tests assert the validation rejects the program WITHOUT allocating
    // anything wide — they only build a tiny IR and check for an Err return.

    /// 200-bit state with a 200-bit binary const (>128 bits) as init/next, and
    /// a `bad` property. No actual wide allocation occurs: validation must
    /// reject during sort resolution / const translation.
    fn make_wide_const_program() -> Btor2Program {
        let mut sorts = HashMap::new();
        sorts.insert(1, Btor2Sort::BitVec(1)); // for the bad bit
        sorts.insert(2, Btor2Sort::BitVec(200)); // oversized

        // A 200-bit binary constant that does NOT fit in u128.
        let wide_bits: String = "1".repeat(200);
        ...
        let lines = vec![
            ...
            Btor2Line {
                id: 3,
                sort_id: 2,
                node: Btor2Node::Const(wide_bits),
                args: vec![],
            },
            Btor2Line {
                id: 4,
                sort_id: 2,
                node: Btor2Node::State(2, Some("w".to_string())),
                args: vec![],
            },
            Btor2Line {
                id: 5,
                sort_id: 2,
                node: Btor2Node::Init(2, 4, 3),
                args: vec![],
            },
            Btor2Line {
                id: 6,
                sort_id: 2,
                node: Btor2Node::Next(2, 4, 3),
                args: vec![],
            },
            // bad: a 1-bit constant (use One) so the property references a 1-bit node.
            Btor2Line {
                id: 7,
                sort_id: 1,
                node: Btor2Node::One,
    ...
    #[test]
    fn regression_wide_sort_translate_declines() {
        // translate_to_chc must DECLINE on a >128-bit sort (covers resolve_sorts
        // gate), never silently produce a problem over a truncated model.
        let program = make_wide_const_program();
        let result = translate_to_chc(&program);
        assert!(
            result.is_err(),
            "expected translate_to_chc to decline a 200-bit sort, got Ok"
        );
    }

    #[test]
    fn regression_wide_const_adaptive_declines_never_sat_unsat() {
        // check_btor2_adaptive must DECLINE (Err) on the oversized model and
        // MUST NEVER return Sat or Unsat for any property.
        let program = make_wide_const_program();
        match check_btor2_adaptive(&program, None) {
            Err(_) => {} // correct: fail-closed decline
            Ok(results) => {
                for r in &results {
                    match r {
                        crate::translate::Btor2CheckResult::Unknown { .. } => {}
                        other => {
                            panic!("wide-const model must never yield a verdict; got {other:?}")
                        }
                    }
                }
            }
    ...
    #[test]
    fn regression_btor2_sort_to_chc_gates_width() {
        // Direct unit check: the sort converter declines >128 and accepts <=128.
        assert!(btor2_sort_to_chc(&Btor2Sort::BitVec(129)).is_err());
        assert!(btor2_sort_to_chc(&Btor2Sort::BitVec(200)).is_err());
        assert!(btor2_sort_to_chc(&Btor2Sort::BitVec(128)).is_ok());
        assert!(btor2_sort_to_chc(&Btor2Sort::BitVec(1)).is_ok());
    }

    #[test]
    fn regression_oversized_const_str_declines() {
        // A >128-bit binary const must DECLINE rather than become 0.
        let sort = ChcSort::BitVec(128);
        let wide: String = "1".repeat(200);
        assert!(parse_binary_const(&wide, &sort).is_err());
        let wide_hex: String = "f".repeat(40); // 160 bits
        assert!(parse_hex_const(&wide_hex, &sort).is_err());
        let wide_dec = "340282366920938463463374607431768211456"; // 2^128
        assert!(parse_decimal_const(wide_dec, &sort).is_err());
    }
}
