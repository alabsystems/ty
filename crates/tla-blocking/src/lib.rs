...
mod tests {
    ...
    /// the one `env_mutation` allow lives on `raw_env_write`. Shape copied from
    /// the ny `ny-test-utils` env module (no `unsafe` — the crate forbids it).
    mod env_guard {
        #![allow(dead_code)]
