...
// `future_incompatible` is a lint GROUP that grows with every rustc release, so
// `forbid` here turns each newly-added member into a hard error on newer
// compilers (this vendored crate built on old nightly but hard-errors on stable
// 1.96 and the Trust rustc 1.99-dev). Keep the group at `deny`, but allow the
// one member that fires on proptest's own numeric extension-trait constants:
// `unstable_name_collisions` warns that std may later add associated constants
// with these names, which for a vendored crate is churn we don't own rather than
// a real defect. Every other member of the group stays a hard error.
