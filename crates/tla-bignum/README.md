...
Verbatim fork of the `rust-num` stack for TY's zero-external-dep posture
...
"rust-num project contributors" alongside the TY maintainer.
...
TY is migrating to zero-external-dep so the verification toolchain can be
...
Downstream `tla-value` / `tla-eval` / `tla-check` / `tla-ay` / `tla-trust-cg`
depend on tla-bignum via the workspace dep table and keep their existing
