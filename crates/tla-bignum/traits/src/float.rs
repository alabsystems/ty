...
// The float-constant macros below intentionally spell out high-precision
// literals (truncated per type); silence clippy's approx-constant lint,
// which fires inside the macro expansions when this fork is compiled as a
// path dependency (registry builds are never linted).
