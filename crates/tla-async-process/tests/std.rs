...
/// the one `env_mutation` allow lives on `raw_env_write`. Shape copied from the
/// ny `ny-test-utils` env module.
mod env_guard {
    #![allow(dead_code)]
...
fn test_capture_env_at_spawn() {
    future::block_on(async {
        ...
        // been captured above. The RAII guard removes it again on scope exit
        // (its prior state was unset), preserving the original semantics.
        let _new_env2 = env_guard::ScopedEnvVar::set("RUN_TEST_NEW_ENV2", "456");
        let result = cmd.output().await.unwrap();
