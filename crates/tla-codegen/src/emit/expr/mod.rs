...
fn emit_floor_int_mod(lhs: String, rhs: String) -> String {
    // TLA+ % requires a strictly-positive divisor (b > 0) per TLC semantics, and
    // the interpreter/VM raise ModulusNotPositive for b <= 0. Emit the same guard
    // in the compiled path (fail-STOP on a non-positive divisor) so `ty check
    // --compiled` matches the reference engines instead of computing a wrong
    // Euclidean value for a negative divisor. The b > 0 guard also excludes the
    // i64::MIN % -1 overflow. (audit-2026-07 #8)
    // Euclidean mod (for b > 0): let r = a % b; if r < 0 { r + b } else { r }
    format!(
