...
///
/// `state_vars`, `constants`, and `invariant_names` come from the spec's `.cfg`
/// and TIR lowering; `options` may override the module / `Init` / `Next` names.
///
/// # Errors
///
/// Returns [`CodegenError`] for the symmetry of the emit-path API. The TIR
/// emitter currently emits a stub for any unsupported construct rather than
/// failing, so this function does not produce an error in practice; the
/// `Result` return type reserves room for future fallible lowering.
...
/// # Errors
///
/// Forwards any [`CodegenError`] from [`generate_rust_from_tir`] after the
/// INSTANCE operators have been imported into a cloned module.
...
fn scan_tir_expr_for_records_inner(
    ...
    match expr {
        TirExpr::Record(fields) => {
            ...
            let typed_fields: Vec<(FieldName, TlaType)> = fields
                .iter()
                .map(|(f, v)| {
                    (
                        FieldName::new(f.name.clone()),
                        infer_type_from_tir_expr(&v.node),
                    )
                })
                .collect();
        ...
        TirExpr::RecordSet(fields) => {
            ...
            let typed_fields: Vec<(FieldName, TlaType)> = fields
                ...
                .map(|(f, v)| {
                    (
                        FieldName::new(f.name.clone()),
                        infer_element_type_from_tir_set(&v.node),
                    )
                })
                .collect();
            registry.try_register_record(&typed_fields);
...
fn tir_type_to_tla_type(ty: &TirType) -> TlaType {
    match ty {
        ...
        TirType::Record(fields) => {
            ...
            let resolved: Vec<(FieldName, TlaType)> = fields
                .iter()
                .map(|(name_id, ty)| {
                    let name = tla_core::resolve_name_id(*name_id).to_string();
                    (FieldName::new(name), tir_type_to_tla_type(ty))
                })
                .collect();
