        ...
        TirExpr::ArithBinOp { left, op, right } => {
            ...
            match op {
                ...
                TirArithOp::Mod => format!(
                    "({{ let __a = {l}; let __b = {r}; assert!(__b > 0, \"TLA+ % requires a positive divisor (got {{}})\", __b); let __r = __a % __b; if __r < 0 {{ __r + __b }} else {{ __r }} }})"
                ),
    ...
    __result
        ...
        TirExpr::Prime(inner) => {
            if let TirExpr::Name(name_ref) = &inner.node {
                ...
                "/* unsupported prime expression */".to_string()
            }
        }
