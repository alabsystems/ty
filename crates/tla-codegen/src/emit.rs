...
///
/// Convenience wrapper around [`generate_rust_with_context`] with an empty
/// [`CodeGenContext`]; use it when the spec is self-contained (no `EXTENDS` /
/// `INSTANCE` of user modules beyond the standard library).
///
/// # Errors
///
/// Returns the same [`CodegenError`] variants as [`generate_rust_with_context`]:
/// [`CodegenError::TypeInference`] when a construct cannot be typed or
/// translated, and (only when `options.checker_map` is set) the
/// `CheckerMap*` / [`CodegenError::InvalidRustFragment`] validation variants.
///
/// # Examples
///
/// ```no_run
...
///
/// # fn parsed_module() -> Module { unimplemented!() }
/// let module: Module = parsed_module();
/// let rust = generate_rust(&module, &CodeGenOptions::default())?;
/// assert!(rust.contains("StateMachine"));
/// # Ok::<(), tla_codegen::CodegenError>(())
/// ```
...
///
/// The `context` supplies the non-main modules reachable via `EXTENDS` /
/// `INSTANCE` so their operators can be inlined into the generated code.
///
/// # Errors
///
/// - [`CodegenError::TypeInference`] — a TLA+ construct could not be typed or
///   lowered to Rust (collects every such error encountered in one pass).
/// - [`CodegenError::CheckerMapRequiresChecker`] — `options.checker_map` is set
///   but `options.generate_checker` is `false`.
///
/// When a checker map is supplied, validating it can additionally return
/// [`CodegenError::CheckerMapModuleMismatch`], [`CodegenError::CheckerMapNoImpls`],
/// [`CodegenError::CheckerMapDuplicateField`], [`CodegenError::CheckerMapUnknownFields`],
/// [`CodegenError::CheckerMapMissingField`], or
/// [`CodegenError::InvalidRustFragment`] (a user-supplied field expression that
/// is not a safe single-line Rust expression).
