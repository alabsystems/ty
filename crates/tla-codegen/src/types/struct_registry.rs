...
pub struct GeneratedStruct {
    ...
    pub fields: Vec<(FieldName, TlaType)>,
}
...
impl StructRegistry {
    /// Create an empty registry with no recorded struct definitions.
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub fn try_register_record(&mut self, fields: &[(FieldName, TlaType)]) -> Option<String> {
        if fields.is_empty() {
            return None;
        }
    ...
    pub fn lookup_record(&self, fields: &[(FieldName, TlaType)]) -> Option<&GeneratedStruct> {
        if fields.is_empty() {
            return None;
        }
        let key = canonical_key(fields);
...
fn canonical_key(fields: &[(FieldName, TlaType)]) -> String {
...
fn generate_struct_name(fields: &[(FieldName, TlaType)]) -> String {
...
mod tests {
    ...
    fn test_register_simple_record() {
        ...
        let fields = vec![
            (FieldName::new("a"), TlaType::Int),
            (FieldName::new("b"), TlaType::Bool),
        ];
    ...
    fn test_register_rejects_unresolved() {
        let mut reg = StructRegistry::new();
        let fields = vec![
            (FieldName::new("x"), TlaType::Int),
            (FieldName::new("y"), TlaType::Unknown),
        ];
        assert_eq!(reg.try_register_record(&fields), None);

        let fields_var = vec![
            (FieldName::new("x"), TlaType::Int),
            (FieldName::new("y"), TlaType::Var(0)),
        ];
    ...
    fn test_emit_definition() {
        let mut reg = StructRegistry::new();
        let fields = vec![
            (FieldName::new("count"), TlaType::Int),
            (FieldName::new("active"), TlaType::Bool),
        ];
    ...
    fn test_different_types_different_structs() {
        ...
        let fields1 = vec![
            (FieldName::new("x"), TlaType::Int),
            (FieldName::new("y"), TlaType::Int),
        ];
        let fields2 = vec![
            (FieldName::new("x"), TlaType::Int),
            (FieldName::new("y"), TlaType::Bool),
        ];
