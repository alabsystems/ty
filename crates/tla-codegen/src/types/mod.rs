...
/// A validated record field name.
///
/// This is a thin newtype over `String` that makes record-field handling
/// type-safe (so a field name can't be confused with an arbitrary string) while
/// preserving the exact string value. It `Deref`s to `str` and implements
/// `Display`, so it behaves like the underlying string for comparisons,
/// formatting, and lookups — the migration from `Vec<(String, TlaType)>` to
/// `Vec<(FieldName, TlaType)>` is byte-for-byte behaviour-neutral.
...
impl TlaType {
    ...
    /// [`StructRegistry`](struct_registry::StructRegistry) for records with
    /// known fields.
    ///
    ...
    pub fn as_record_fields(&self) -> Option<&[(FieldName, TlaType)]> {
        match self {
...
pub enum TypeInferError {
    ...
    TypeMismatch {
        ...
        expected: TlaType,
        /// The type actually inferred for the expression.
        actual: TlaType,
