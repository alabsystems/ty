...
//! real TLA+ specs via `ty codegen --tir --scaffold`, then compares
...
/// `ty codegen --tir --scaffold`, (2) compile successfully, (3) run BFS
...
/// Root directory of the ty repo (derived from CARGO_MANIFEST_DIR).
...
///
/// Honors `TLAPLUS_EXAMPLES` (pointing at the corpus checkout or its
/// `specifications/` dir); falls back to `$HOME/tlaplus-examples/specifications`.
fn specs_root() -> PathBuf {
    if let Ok(env) = std::env::var("TLAPLUS_EXAMPLES") {
        if !env.is_empty() {
            let p = PathBuf::from(&env);
            return if p.file_name().and_then(|s| s.to_str()) == Some("specifications") {
                p
            } else {
    ...
    PathBuf::from(home).join("tlaplus-examples/specifications")
}

/// Find the ty binary, checking common build output locations.
///
...
fn find_ty_binary() -> Option<PathBuf> {
    ...
    let mut candidates: Vec<PathBuf> = vec![
        root.join("target/release/ty"),
        root.join("target/user/release/ty"),
        root.join("target/debug/ty"),
    ];
    // Also check the main repo root (worktree parent)
    if let Some(main_root) = root.parent().and_then(|p| p.parent()) {
        candidates.push(main_root.join("target/release/ty"));
        candidates.push(main_root.join("target/user/release/ty"));
    }
    // Common temp build dirs used by agents
    candidates.push(PathBuf::from("/tmp/tl4k-build/release/ty"));
    candidates.push(PathBuf::from("/tmp/ty-hwmcc/release/ty"));

    candidates.into_iter().find(|p| p.exists())
}

/// Run `ty codegen --tir --scaffold` to generate a complete Cargo project.
fn run_codegen_scaffold(
    ...
    let output = Command::new(ty_bin)
        ...
        .output()
        .map_err(|e| format!("failed to run ty codegen: {e}"))?;
...
fn test_codegen_state_counts_match_tlc() {
    ...
    let ty_bin = match find_ty_binary() {
        Some(bin) => bin,
        None => {
            eprintln!("SKIP: ty binary not found. Build with: cargo build --release --bin ty");
            return;
        }
    };
    ...
    for spec in TEST_SPECS {
        ...
        if let Err(e) = run_codegen_scaffold(&ty_bin, &tla_file, &cfg_file, &output_dir) {
