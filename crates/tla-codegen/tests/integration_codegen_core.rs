...
VARIABLE __ty_yield_state_fn

Init == __ty_yield_state_fn = 0

Next == __ty_yield_state_fn' = __ty_yield_state_fn
====
"#;
    ...
    assert!(
        code.contains("fn for_each_init<F>(&self, mut __ty_yield_state_fn_0: F)"),
        "Expected for_each_init closure parameter to avoid capturing state variable name"
    );
    assert!(
        code.contains(
            "fn for_each_next<F>(&self, state: &Self::State, mut __ty_yield_state_fn_0: F)"
        ),
