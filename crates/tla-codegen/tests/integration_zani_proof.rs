...
//! First trust_mc/Kani proof on generated TLA+ code.
//!
//! This test demonstrates the complete pipeline:
//!   TLA+ spec -> Rust codegen -> trust_mc/Kani verification proof
//!
...
//!
//! Without Kani/trust_mc installed, these run as standard tests with exhaustive
...
#[test]
fn test_trust_mc_proof_bounded_counter() {
    use std::fs;
    ...
    // Create temp project
    let temp_dir = std::env::temp_dir().join(format!("ty_trust_mc_proof_{}", std::process::id()));
    ...
    // Write the Kani/trust_mc harness with test fallbacks
    //
    ...
    let harness_code = r#"//! Kani/trust_mc verification harness for BoundedCounter.
//!
...
// ---------------------------------------------------------------------------
// Kani/trust_mc proofs (active only under `cargo kani`)
// ---------------------------------------------------------------------------
...
mod trust_mc_harness;
"#;
    fs::write(src_dir.join("lib.rs"), lib_rs).expect("write lib.rs");
    fs::write(src_dir.join("trust_mc_harness.rs"), harness_code)
        .expect("write trust_mc_harness.rs");
    ...
    if !test_output.status.success() {
        panic!("trust_mc harness tests failed!\nstdout:\n{stdout}\nstderr:\n{stderr}");
    }
    ...
    println!("trust_mc proof pipeline validated successfully!");
