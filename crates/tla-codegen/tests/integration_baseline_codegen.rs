...
/// (Ported from tla-cli's corpus_sweep_regression.rs.)
...
/// Resolve a spec's TLA+ source file to an absolute path under the corpus dir.
///
/// Returns `None` if the file cannot be found.
fn resolve_tla_path(corpus_specs: &Path, tla_path: &str) -> Option<PathBuf> {
    let p = corpus_specs.join(tla_path);
...
fn test_baseline_small_spec_codegen() {
    // The TLA+ corpus is NOT in this repo (see CLAUDE.md); skip cleanly when
    // it is absent instead of failing with zero successes.
    let corpus_specs = match resolve_corpus_specs() {
        Some(dir) => dir,
        None => {
            eprintln!(
                "skipping test_baseline_small_spec_codegen: corpus not installed \
    ...
    for (name, entry) in &small_specs {
        ...
        let tla_abs = match resolve_tla_path(&corpus_specs, tla_rel) {
            Some(p) => p,
            None => {
