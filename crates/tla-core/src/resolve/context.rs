    ...
    pub(crate) fn define(
        ...
        if let Some(scope) = self.scopes.last_mut() {
            ...
            if let Some(existing) = scope.symbols.get(name) {
                ...
                // Allow a module's own (local) operator definition to override an
                // EXTENDS/INSTANCE-imported operator of the same name. TLC permits this:
                // a local definition shadows the imported one (e.g. Huang defines
                // Sum(_,_) while EXTENDS'ing SequencesExt -> FiniteSetsExt, which also
                // exports Sum). Imported symbols are injected with a dummy span, so a
                // dummy-spanned existing Operator is import-provenance, never a real
                // duplicate of two source-level definitions.
                let can_override_imported_operator = existing.kind == SymbolKind::Operator
                    && kind == SymbolKind::Operator
                    && existing.def_span == Span::dummy()
                    && span != Span::dummy();

                if can_replace_forward_decl || can_override_imported_operator {
