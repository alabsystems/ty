...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_sequencesext_reexports_finitesets_cardinality() {
    // SequencesExt.tla: EXTENDS Sequences, Naturals, FiniteSets, FiniteSetsExt, Folds, Functions,
    // Bags. EXTENDS publicly re-exports, so a module that EXTENDS SequencesExt must see
    // `Cardinality` (from FiniteSets). Regression for the SlidingPuzzles_anim resolution gap.
    let result = resolve_source(
        r"
        ---- MODULE Test ----
        EXTENDS SequencesExt
        VARIABLE board
        Sz == Cardinality(board)
        ====
        ",
    );
    assert!(
        result.is_ok(),
        "Cardinality should resolve transitively through SequencesExt; errors: {:?}",
        result.errors
    );
}
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_local_definition_overrides_extends_imported_operator() {
    // FiniteSetsExt exports `Sum` (arity 1). A module that EXTENDS SequencesExt
    // (which transitively EXTENDS FiniteSetsExt) but defines its own `Sum(_,_)`
    // must not get a duplicate-definition error: the local definition overrides the
    // imported one. TLC accepts this (e.g. the Huang spec). Regression for the
    // transitive-EXTENDS injection interacting with local redefinition.
    let result = resolve_source(
        r"
        ---- MODULE Test ----
        EXTENDS SequencesExt
    ...
    assert!(
        result.is_ok(),
        "local Sum(_,_) should override imported Sum; errors: {:?}",
        result.errors
    );
}
...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_finitesetsext_reexports_finitesets_and_functions() {
    // FiniteSetsExt.tla: EXTENDS Integers, FiniteSets, Folds, Functions.
    // A module EXTENDS'ing FiniteSetsExt must see both `Cardinality` (FiniteSets) and
    // `Range` (Functions).
    let result = resolve_source(
        r"
        ---- MODULE Test ----
    ...
    assert!(
        result.is_ok(),
        "Cardinality/Range should resolve transitively through FiniteSetsExt; errors: {:?}",
        result.errors
    );
}
