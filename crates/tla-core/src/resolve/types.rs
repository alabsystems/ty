...
/// Options controlling name resolution behaviour.
...
impl ResolveOptions {
    /// Options tuned for model checking: proof bodies are not resolved.
    ///
    /// TLC does not semantically validate TLAPS proofs during checking, and many
    /// corpus specs use proof-language constructs ty does not yet fully lower, so
    /// skipping proof resolution avoids spurious errors.
