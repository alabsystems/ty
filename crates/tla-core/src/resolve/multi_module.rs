...
/// Resolve a module with extended and instanced modules pre-loaded, under the
/// given [`ResolveOptions`].
///
/// This is the most general entry point; [`resolve_with_extends`] and
/// [`resolve_with_extends_and_instances`] delegate here with default options.
/// Stdlib symbols (including transitive ones via the extends chain) are injected
/// first, taking care not to shadow same-named modules actually loaded from
/// source, then `extended_modules` and `instanced_modules` are made visible
/// before the main module's bodies are resolved.
