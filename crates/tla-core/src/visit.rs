...
pub trait VisitorOutput: Default + Clone {
    /// Fold this result together with a child's result into one.
    ///
    /// For `bool` this is logical OR; for `HashSet` it is set union; for `()`
    /// it is a no-op. `Default::default()` is the identity element.
    fn combine(self, other: Self) -> Self;

    /// Whether this value is "final" and traversal can stop early.
    ///
    /// Defaults to `false` (never short-circuit). The `bool` impl returns the
    /// value itself, so a `true` result halts further traversal.
...
pub trait ExprVisitor {
    /// The result type accumulated across the traversal; its [`VisitorOutput`]
    /// impl defines how per-node results are combined and when to stop early.
