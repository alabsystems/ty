...
pub struct ParseError {
    ...
    pub message: String,
    /// Start byte offset of the offending range (inclusive).
    pub start: u32,
    /// End byte offset of the offending range (exclusive).
    pub end: u32,
}

/// Result of parsing
pub struct ParseResult {
    /// The rowan green (untyped, lossless) syntax tree. Always produced, even
    /// when `errors` is non-empty (the parser recovers and continues).
    pub green_node: GreenNode,
    /// Any syntax errors encountered, in source order.
