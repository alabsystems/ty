...
/// True when `module_name` is a stdlib/community module natively implemented by
/// the toolchain AND `op_name` is one of its declared operators.
///
/// Used by INSTANCE resolution (`G == INSTANCE Graphs` ... `G!Transpose(...)`):
/// stdlib modules are never loaded from `.tla` sources, so their operators are
/// absent from `instance_ops` and must be recognized here instead. Mirrors
/// TLC's Java module overrides, which resolve regardless of module loading.
pub fn stdlib_module_has_op(module_name: &str, op_name: &str) -> bool {
    is_stdlib_module(module_name)
        && get_module_operators(module_name)
            .iter()
            .any(|(n, _)| *n == op_name)
}
...
fn inject_module_symbols(
    ...
    match module_name {
        ...
        // Community modules.
        //
        // These EXTENDS chains are transcribed from the CommunityModules sources
        // (CommunityModules.jar). EXTENDS publicly re-exports the extended module's
        // operators, so a module that EXTENDS (directly or transitively) one of these
        // must see the full closure. LOCAL INSTANCE does NOT re-export, so modules that
        // use LOCAL INSTANCE (e.g. SVG) intentionally have no case here.
        "Functions" => {
            // Functions.tla: EXTENDS Integers, Folds
            inject_module_symbols(ctx, "Integers", processed, span);
        ...
        "FiniteSetsExt" => {
            ...
            inject_module_symbols(ctx, "Integers", processed, span);
            inject_module_symbols(ctx, "FiniteSets", processed, span);
            inject_module_symbols(ctx, "Folds", processed, span);
        ...
        "SequencesExt" => {
            ...
            inject_module_symbols(ctx, "Naturals", processed, span);
            inject_module_symbols(ctx, "FiniteSets", processed, span);
            inject_module_symbols(ctx, "FiniteSetsExt", processed, span);
            inject_module_symbols(ctx, "Folds", processed, span);
            inject_module_symbols(ctx, "Functions", processed, span);
            inject_module_symbols(ctx, "Bags", processed, span);
        }
        "VectorClocks" => {
