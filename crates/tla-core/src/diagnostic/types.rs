...
pub enum Severity {
    /// A hard failure: the input is rejected.
    Error,
    /// A potential problem that does not stop processing.
    Warning,
...
pub enum LabelColor {
    /// The main span the diagnostic is about (rendered most prominently).
    Primary,
    /// A related, supporting span (rendered less prominently).
    Secondary,
    /// An informational span.
    Info,
}
