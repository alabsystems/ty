...
//! This module brings the same approach to TY:
//!
...
/// # Lifetime invariant
///
/// A `NameId` is valid only until [`clear_global_name_interner`] is called.
/// It must therefore never be retained in a process-wide `static`, `OnceLock`,
/// or `LazyLock`; such caches outlive the interner generation and can silently
/// alias an unrelated name after a reset. Cache the source string and call
/// [`intern_name`] again at the point of use instead.
...
impl NameInterner {
    ...
    pub(crate) fn intern(&mut self, name: &str) -> NameId {
        ...
        // Diagnostic (intern-order debugging): TY_TRACE_INTERN="*" prints every
        // new interning; TY_TRACE_INTERN="a,b,c" prints id + backtrace for the
        // listed names. Slow path only (first interning of a name), so the env
        // read costs nothing on the hot path. Used to root-cause the BagsTest
        // record-field-order nondeterminism (im::HashMap-ordered precompute).
        if let Ok(trace) = std::env::var("TY_TRACE_INTERN") {
            if trace == "*" {
                let tid = std::thread::current().id();
                eprintln!(
                    "[INTERN-TRACE] '{}' -> NameId({}) thread={:?}",
                    name,
                    self.names.len(),
                    tid
                );
            } else if trace.split(',').any(|t| t == name) {
                eprintln!(
                    "[INTERN-TRACE] '{}' -> NameId({}) thread={:?}\n{}",
                    name,
                    self.names.len(),
...
/// Epoch-managed pointer to the frozen interner snapshot. Null when not frozen.
///
/// Reads pin the epoch (`epoch::pin()`) and dereference the loaded `Shared`; the
/// pin is a thread-local operation that is cheap when nested inside a coarser
/// held guard (the BFS pins once per state — see `epoch_pin_guard()`), replacing
/// the former per-read `AcqRel` reader-count RMW pair.
///
/// `freeze_interner()`/`unfreeze_interner()` swap in the new snapshot and
/// `defer_destroy` the old one: reclamation is DEFERRED until every thread that
/// could still hold the old `Shared` has passed the current epoch, so it is both
/// use-after-free-safe under CONCURRENT freezes (parallel tests / multi-check
/// processes share this global interner) AND non-blocking (freeze returns
/// immediately instead of spin-waiting for readers to drain).
static FROZEN: Atomic<FrozenInterner> = Atomic::null();

/// Pin the current thread's epoch for the duration of the returned guard.
///
/// Held once per BFS state so the many `epoch::pin()` calls in `intern_name`/
/// `resolve_name_id` during that state's evaluation nest cheaply (a thread-local
/// counter bump) instead of each doing a full epoch registration + fence.
#[inline]
#[must_use]
pub fn epoch_pin_guard() -> epoch::Guard {
    epoch::pin()
}
...
pub fn freeze_interner() {
    let lock = global_interner().read();
    let frozen = Owned::new(FrozenInterner {
        lookup: lock.lookup.clone(),
        names: lock.names.clone(),
        string_fp64: lock.string_fp64.clone(),
    });
    drop(lock);
    let epoch_guard = &epoch::pin();
    let old = FROZEN.swap(frozen, Ordering::AcqRel, epoch_guard);
    if !old.is_null() {
        // SAFETY: `old` was published by a prior freeze and the swap above has
        // just unpublished it. `defer_destroy` frees it only once every thread
        // that could still hold this `Shared` has passed the current epoch, so
        // no lock-free reader can observe a freed snapshot — even under
        // concurrent freezes from other model-checking runs.
        unsafe {
            epoch_guard.defer_destroy(old);
        }
    }
}
...
fn unfreeze_interner() {
    let epoch_guard = &epoch::pin();
    let old = FROZEN.swap(Shared::null(), Ordering::AcqRel, epoch_guard);
    if !old.is_null() {
        // SAFETY: see `freeze_interner`.
        unsafe {
            epoch_guard.defer_destroy(old);
        }
    }
...
pub fn intern_name(name: &str) -> NameId {
    // Fast path: frozen snapshot (no lock, just a pinned epoch read).
    {
        let guard = &epoch::pin();
        // SAFETY: the pin keeps any snapshot loaded from FROZEN alive for the
        // guard's lifetime — a concurrent `defer_destroy` cannot reclaim it until
        // this thread passes the current epoch (drops the guard).
        if let Some(frozen) = unsafe { FROZEN.load(Ordering::Acquire, guard).as_ref() } {
            if let Some(&id) = frozen.lookup.get(name) {
                return id;
            }
        }
    }
...
pub fn lookup_name_id(name: &str) -> Option<NameId> {
    ...
    {
        let guard = &epoch::pin();
        // SAFETY: pin keeps the loaded snapshot alive (see intern_name).
        if let Some(frozen) = unsafe { FROZEN.load(Ordering::Acquire, guard).as_ref() } {
            if let Some(&id) = frozen.lookup.get(name) {
                return Some(id);
            }
...
/// Compare two NameIds by their resolved name strings (byte-lexicographic).
///
/// This is the CANONICAL record-field order: `RecordValue` keeps its entries
/// sorted by this order so that record iteration, fingerprinting, and
/// cross-type comparison against string-keyed `FuncValue`s (which sort their
/// domains by string content) all agree.
///
/// NameId NUMERIC order is interning order — it is run-dependent (allocator/
/// startup dependent) and has no semantic meaning. It MUST NOT be used as an
/// ordering proxy for field names (BagsTest `h (+) h` nondeterministic-verdict
/// bug: records sorted by NameId zipped pairwise against string-sorted funcs).
///
/// Lock-free when the interner is frozen; no Arc clones on the frozen path.
///
/// # Panics
///
/// Panics if either NameId is out of range.
#[inline]
pub fn name_id_str_cmp(a: NameId, b: NameId) -> std::cmp::Ordering {
    if a == b {
        return std::cmp::Ordering::Equal;
    }
    // Fast path: frozen snapshot (borrowed &str compare, no Arc clones)
    {
        let guard = &epoch::pin();
        // SAFETY: pin keeps the loaded snapshot alive (see intern_name).
        if let Some(frozen) = unsafe { FROZEN.load(Ordering::Acquire, guard).as_ref() } {
            if (a.0 as usize) < frozen.names.len() && (b.0 as usize) < frozen.names.len() {
                return frozen.names[a.0 as usize]
                    .as_ref()
                    .cmp(frozen.names[b.0 as usize].as_ref());
            }
        }
    }
    let guard = global_interner().read();
    guard.names[a.0 as usize]
        .as_ref()
        .cmp(guard.names[b.0 as usize].as_ref())
}
