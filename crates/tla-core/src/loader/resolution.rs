    ...
    pub fn modules_for_semantic_resolution<'a>(
        ...
        while let Some(name) = queue.pop_front() {
            ...
            if let Some(loaded) = self.get(&name) {
                ...
                // A standalone `INSTANCE M` imports *all* of M's public definitions into the
                // unqualified scope, which includes everything M re-exports via its own EXTENDS.
                // For example: TLCSailfish2 INSTANCEs Sailfish, Sailfish INSTANCEs BlockDag, and
                // BlockDag EXTENDS Utils (which defines Max). `Max` must therefore be visible
                // unqualified in TLCSailfish2. We follow EXTENDS of instanced modules here so their
                // re-exported operators land in the instance scope. (Only operators are imported via
                // INSTANCE, so EXTENDS'd constants/variables are intentionally not promoted.)
                for ext in &loaded.module.extends {
                    queue.push_back(ext.node.clone());
                }
            }
        }
