...
#[cfg_attr(test, ntest::timeout(10000))]
#[test]
fn test_standalone_instance_promotes_extended_module_operators() {
    // Regression for TLCSailfish2: `Max` (defined in Util) must be visible unqualified.
    //
    // Main INSTANCE Mid, Mid INSTANCE Base, Base EXTENDS Util.
    // A standalone `INSTANCE M` imports all of M's *public* definitions, which includes
    // everything M re-exports via its own EXTENDS. So Util (reached via Base's EXTENDS, which is
    // reached via Mid's INSTANCE) must land in the instance scope of Main.
    let temp = TempDir::new().unwrap();
    create_test_module(
        temp.path(),
        "Util",
    ...
    let (_extends, instances) = loader.modules_for_semantic_resolution(&main);
    let inst_names: Vec<_> = instances.iter().map(|m| m.name.node.as_str()).collect();
    assert!(
        inst_names.contains(&"Util"),
        "Util (Base's EXTENDS) must be in the instance scope; got {inst_names:?}"
    );
}
