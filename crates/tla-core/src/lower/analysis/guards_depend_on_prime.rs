...
/// Compute and record, for every operator in `module`, whether any of its
/// guards transitively depends on a primed variable.
///
/// A "guard" is a condition that selects a branch or disjunct (e.g. an `IF`
/// condition or the antecedent of `cond => action`). Dependence is propagated
/// to a fixed point across operator calls, so an operator whose guard merely
/// calls another guard-on-prime operator is also marked. The result is written
/// back into each [`OperatorDef::guards_depend_on_prime`] field.
///
/// [`OperatorDef::guards_depend_on_prime`]: crate::ast::OperatorDef::guards_depend_on_prime
