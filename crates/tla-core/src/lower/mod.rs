...
pub struct LowerResult {
    /// The lowered module, or `None` if lowering could not produce one (e.g.
    /// no module node was found). May be `Some` even with errors present.
    pub module: Option<Module>,
    /// Errors encountered during lowering, in source order.
