...
// ===== Span-keyed memo for get_primed_var_refs (Target 2) =====
//
// The set of primed-variable references inside an expression is a pure function
// of the (immutable) AST subtree. The successor enumerator computes it for each
// LET body on every state (millions of times), but the body is static. Keying a
// memo on the body's source `Span` (file + byte range, which uniquely identifies
// one AST subtree) turns the per-state O(subtree) walk into one walk per
// distinct LET body.
//
// Soundness: `Span` uniquely identifies a source AST node; two distinct bodies
// cannot share a span, so a hit returns the value that recomputation would.
std::thread_local! {
    static PRIMED_REFS_MEMO: std::cell::RefCell<
        rustc_hash::FxHashMap<crate::span::Span, Arc<HashSet<Arc<str>>>>,
    > = std::cell::RefCell::new(rustc_hash::FxHashMap::default());
}

/// Cap on memo entries to bound memory; LET-body spans are O(spec size).
const PRIMED_REFS_MEMO_CAP: usize = 65_536;

/// Span-keyed, memoized variant of [`get_primed_var_refs_v`].
///
/// Returns the set of primed variable names referenced in `body`, computed once
/// per distinct source span and cached for subsequent calls. Use this on hot
/// per-state paths where the same static subtree is re-analyzed repeatedly.
pub fn get_primed_var_refs_spanned_v(body: &Spanned<Expr>) -> Arc<HashSet<Arc<str>>> {
    let span = body.span;
    PRIMED_REFS_MEMO.with(|memo| {
        if let Some(cached) = memo.borrow().get(&span) {
            return Arc::clone(cached);
        }
        let mut visitor = CollectPrimedVarRefs {
            refs: HashSet::new(),
        };
        let _ = walk_expr(&mut visitor, &body.node);
        let result = Arc::new(visitor.refs);
        let mut m = memo.borrow_mut();
        if m.len() >= PRIMED_REFS_MEMO_CAP {
            m.clear();
        }
        m.insert(span, Arc::clone(&result));
        result
    })
}

/// Clear the primed-var-refs memo (run/test reset). Pure-function memo; clearing
/// is a memory reset only.
pub fn clear_primed_var_refs_memo() {
    PRIMED_REFS_MEMO.with(|memo| memo.borrow_mut().clear());
}
