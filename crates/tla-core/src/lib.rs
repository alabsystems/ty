...
//! ty (TLA+ / Trust-TY) — Core TLA+ parser, AST, and semantic analysis
//!
//! ty is a Rust implementation of a TLA+ verification toolchain. The name
//! **ty** refers to the TLA+ project and its role in the **Trust** ecosystem.
//!
//! This crate provides the foundation for all ty tools:
...
//! ```
//!
//! The pipeline is two-staged. [`parse`] turns source text into a lossless
//! `rowan` concrete syntax tree (CST) that retains every token and is resilient
//! to errors; [`lower`] then folds that CST into the typed [`ast`] used by the
//! rest of the toolchain. [`resolve`](resolve()) checks names, arities, and
//! scopes against the resulting [`ast::Module`].
//!
//! # Example
//!
//! Parse a module to an AST and resolve its names:
//!
//! ```
//! use tla_core::{parse_to_syntax_tree, lower, resolve, FileId};
//!
//! let src = "---- MODULE M ----\nVARIABLE x\nInit == x = 0\n====";
...
//! let module = lowered.module.expect("module lowered");
//!
...
pub use name_intern::{
    ...
    name_id_str_cmp, resolve_name_id, resolve_name_id_string_fp64, NameId,
};
...
// Stdlib re-exports
pub use stdlib::{get_module_operators, is_stdlib_module, stdlib_module_has_op, STDLIB_MODULES};

// Expression visitor re-exports
pub use expr_visitors::{
    apply_substitutions_v, arg_has_name_in_set_v, clear_primed_var_refs_memo,
    collect_bound_names_v, collect_conjuncts_v, expr_contains_any_prime_v, expr_contains_at_v,
    expr_contains_enabled_v, expr_contains_exists_v, expr_contains_ident_v,
    expr_contains_operator_application_v, expr_contains_prime_not_assignment_v,
    expr_contains_prime_v, expr_contains_primed_param_v, expr_contains_temporal_v,
    expr_has_any_prime_legacy_v, expr_has_free_var_in_set_v, expr_mentions_name_spanned_v,
    expr_mentions_name_v, expr_references_any_free_name_v, expr_references_state_vars_v,
    get_primed_var_refs_spanned_v, get_primed_var_refs_v, is_guard_expression_v,
    is_simple_prime_var, rewrite_static_prime_in_set_in_op_def,
};
