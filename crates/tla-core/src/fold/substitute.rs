...
pub struct SubstituteExpr<'a> {
    /// Map from identifier name to the expression replacing it. Names bound by
    /// inner binders (quantifiers, LET, lambdas, etc.) are filtered out before
    /// recursing into those bodies to avoid variable capture.
    pub subs: HashMap<&'a str, &'a Spanned<Expr>>,
    /// How spans are assigned to the rewritten expression.
                ...
                let new_defs = defs
                    ...
                    .map(|def| {
                        let param_names: HashSet<_> = def
                            ...
                            .collect();
                        // Derive from `filtered_subs` (already filtered by the LET's
                        // def_names), not raw `self.subs`: LET def names are in scope
                        // inside every def body, so substitution into a def body must
                        // be capture-avoiding w.r.t. def_names too — otherwise an
                        // INSTANCE WITH replacement whose free var equals a def name is
                        // captured, or a target equal to a def name wrongly applies
                        // (a silently wrong model). Mirrors the LET-body / binder arms.
                        // (audit-2026-07 #7)
                        let body_subs = if param_names.is_empty() {
                            filtered_subs.clone()
                        } else {
                            filter_substitutions_capture_avoiding(&filtered_subs, &param_names)
                        };
...
pub struct SubstituteAt<'a> {
    /// The expression substituted for each in-scope `@`.
