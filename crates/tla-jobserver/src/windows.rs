...
const WTY_ABANDONED: DWORD = 128u32;
const WTY_FAILED: DWORD = 4294967295u32;
const WTY_OBJECT_0: DWORD = 0u32;
const WTY_TIMEOUT: DWORD = 258u32;
...
impl Client {
    ...
    pub fn try_acquire(&self) -> io::Result<Option<Acquired>> {
        match unsafe { WaitForSingleObject(self.sem.0, 0) } {
            WTY_OBJECT_0 => Ok(Some(Acquired)),
            WTY_TIMEOUT => Ok(None),
            WTY_FAILED => Err(io::Error::last_os_error()),
            // We believe this should be impossible for a semaphore, but still
            // check the error code just in case it happens.
            WTY_ABANDONED => Err(io::Error::new(
                io::ErrorKind::Other,
                "Wait on jobserver semaphore returned WTY_ABANDONED",
            )),
...
pub(crate) fn spawn_helper(
    ...
    let thread = Builder::new().spawn(move || {
        ...
        state.for_each_request(|_| {
            const WTY_OBJECT_1: u32 = WTY_OBJECT_0 + 1;
            match unsafe { WaitForMultipleObjects(2, objects.as_ptr(), FALSE, INFINITE) } {
                WTY_OBJECT_0 => {}
                WTY_OBJECT_1 => f(Ok(crate::Acquired {
