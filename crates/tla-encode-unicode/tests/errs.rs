...
fn utf8_overlong() {
    ...
    for o in overlongs.iter() {
        for &last in &[0x80, 0xbf] {
            ...
            assert_eq!(
                char::from_utf8_slice_start(&arr),
                Err(InvalidUtf8Slice::Utf8(OverLong))
            );
            assert_eq!(
                char::from_utf8_array(arr),
                Err(InvalidUtf8Array::Utf8(OverLong))
            );
            assert_eq!(
                Utf8Char::from_slice_start(&arr),
                Err(InvalidUtf8Slice::Utf8(OverLong))
            );
            assert_eq!(
                Utf8Char::from_array(arr),
                Err(InvalidUtf8Array::Utf8(OverLong))
            );
        }
    }
...
fn utf8_codepoint_is_too_high() {
    assert_eq!(
        Utf8Char::from_array([0xf4, 0x90, 0x80, 0x80]),
        Err(a::Codepoint(TooHigh))
    );
    assert_eq!(
        char::from_utf8_array([0xf4, 0x90, 0x80, 0x80]),
        Err(a::Codepoint(TooHigh))
    );
    assert_eq!(
        Utf8Char::from_slice_start(&[0xf4, 0x90, 0x80, 0x80]),
        Err(s::Codepoint(TooHigh))
    );
    assert_eq!(
        char::from_utf8_slice_start(&[0xf4, 0x90, 0x80, 0x80]),
        Err(s::Codepoint(TooHigh))
    );

    assert_eq!(
        Utf8Char::from_array([0xf5, 0x88, 0x99, 0xaa]),
        Err(a::Codepoint(TooHigh))
    );
    assert_eq!(
        char::from_utf8_array([0xf5, 0xaa, 0xbb, 0x88]),
        Err(a::Codepoint(TooHigh))
    );
    assert_eq!(
        Utf8Char::from_slice_start(&[0xf5, 0x99, 0xaa, 0xbb]),
        Err(s::Codepoint(TooHigh))
    );
    assert_eq!(
        char::from_utf8_slice_start(&[0xf5, 0xbb, 0x88, 0x99]),
        Err(s::Codepoint(TooHigh))
    );
}
...
fn utf8_codepoint_is_utf16_reserved() {
    assert_eq!(
        Utf8Char::from_array([0xed, 0xa0, 0x80, 0xff]),
        Err(a::Codepoint(Utf16Reserved))
    );
    assert_eq!(
        char::from_utf8_array([0xed, 0xa0, 0x8f, 0x00]),
        Err(a::Codepoint(Utf16Reserved))
    );
    assert_eq!(
        Utf8Char::from_slice_start(&[0xed, 0xa0, 0xbe, 0xa5]),
        Err(s::Codepoint(Utf16Reserved))
    );
    assert_eq!(
        char::from_utf8_slice_start(&[0xed, 0xa0, 0xbf]),
        Err(s::Codepoint(Utf16Reserved))
    );
    assert_eq!(
        Utf8Char::from_array([0xed, 0xbf, 0x80, 0xff]),
        Err(a::Codepoint(Utf16Reserved))
    );
    assert_eq!(
        char::from_utf8_array([0xed, 0xbf, 0x8f, 0x00]),
        Err(a::Codepoint(Utf16Reserved))
    );
    assert_eq!(
        Utf8Char::from_slice_start(&[0xed, 0xbf, 0xbe, 0xa5]),
        Err(s::Codepoint(Utf16Reserved))
    );
    assert_eq!(
        char::from_utf8_slice_start(&[0xed, 0xbf, 0xbf]),
        Err(s::Codepoint(Utf16Reserved))
    );
}
...
fn utf8_first_is_continuation_byte() {
    for first in 0x80..0xc0 {
        ...
        assert_eq!(
            Utf8Char::from_array(arr),
            Err(a::Utf8(FirstByte(ContinuationByte)))
        );
        assert_eq!(
            char::from_utf8_array(arr),
            Err(a::Utf8(FirstByte(ContinuationByte)))
        ...
        assert_eq!(
            Utf8Char::from_slice_start(&arr[..len]),
            Err(s::Utf8(FirstByte(ContinuationByte)))
        );
        assert_eq!(
            char::from_utf8_slice_start(&arr[..len]),
            Err(s::Utf8(FirstByte(ContinuationByte)))
        );
    }
}
...
fn utf8_too_long() {
    for first in 0xf8..0x100 {
        ...
        assert_eq!(
            Utf8Char::from_array(arr),
            Err(a::Utf8(FirstByte(TooLongSeqence)))
        );
        assert_eq!(
            char::from_utf8_array(arr),
            Err(a::Utf8(FirstByte(TooLongSeqence)))
        ...
        assert_eq!(
            Utf8Char::from_slice_start(slice),
            Err(s::Utf8(FirstByte(TooLongSeqence)))
        );
        assert_eq!(
            char::from_utf8_slice_start(slice),
            Err(s::Utf8(FirstByte(TooLongSeqence)))
        );
    }
}
