...
/// THE single raw env-mutation site in this crate. Completion dispatch clears
/// the activation variable so child completer processes don't re-enter
/// completion — a deliberate, permanent removal performed only during
/// application initialization (documented single-threaded), serialized behind a
/// process-wide lock so any concurrent dispatch can't race the write.
///
/// A raw `remove_var` must exist somewhere to implement the primitive; this one
/// lock-scoped helper is that somewhere. `env_mutation` is the Trust
/// toolchain's deny-by-default env wall; `unknown_lints` keeps the stock-rustc
/// build green (the lint is Trust-only).
#[allow(unknown_lints, env_mutation)]
fn remove_process_env_var(key: &str) {
    ...
    // SAFETY: edition-2024 `remove_var` is unsafe. This runs only during
    // application initialization (see the sole caller), and the lock above
    // serializes any concurrent completion dispatch, so no other thread can be
...
impl<'s, F: Fn() -> clap::Command> CompleteEnv<'s, F> {
    ...
    fn try_complete_(
        ...
        // completion logic. Routed through the crate's single lock-scoped env
        // choke point (called only during application initialization).
        remove_process_env_var(self.var);
