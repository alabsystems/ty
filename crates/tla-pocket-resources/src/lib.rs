...
where
    I: IntoIterator<Item = &'a (P1, P2)>,
    P1: AsRef<Path> + 'a,
    P2: AsRef<Path> + 'a,
{
    let entries = files
        .into_iter()
        .map(|&(ref base_dir, ref file)| {
    ...
    try!(writeln!(
        file,
        r#"#[derive(Debug, Clone, Hash, PartialEq, Eq)]"#
    ));
    try!(writeln!(file, r#"pub enum ResourceId {{"#));
    for entry in &entries {
        try!(writeln!(file, r"{},", entry.enum_name));
    }
    try!(writeln!(file, r#"}}"#));
    ...
    try!(writeln!(
        file,
        r#"    pub fn from_name(name: &str) -> Option<ResourceId> {{"#
    ));
    for entry in &entries {
        try!(writeln!(
            file,
        ...
        if entry.resource_str_name != entry.resource_str_name_no_ext {
            if entries
                .iter()
                .filter(|e| {
                    e.resource_str_name_no_ext == entry.resource_str_name_no_ext
                        || e.resource_str_name == entry.resource_str_name_no_ext
                })
                .count()
                == 1
            {
    ...
    W: Write,
{
    ...
    for entry in entries {
        ...
        try!(write!(
            output,
            "#[allow(missing_docs)] pub const {}: ",
            entry.struct_name
        ));
        for _ in 0..base.len() {
            try!(write!(output, r"super::"));
        }
        ...
        if entry.struct_name != entry.struct_name_no_ext {
            if entries
                .iter()
                .filter(|e| {
                    e.struct_name_no_ext == entry.struct_name_no_ext
                        || e.struct_name == entry.struct_name_no_ext
                })
                .count()
                == 1
            {
...
fn path_to_enum_variant<P>(path: P) -> String
where
    P: AsRef<Path>,
{
    ...
    let components = path
        ...
        .map(|val| {
            ...
            format!(
                "{}{}",
                val[..1].to_ascii_uppercase(),
                val[1..].to_ascii_lowercase()
            )
        })
        .collect::<Vec<_>>();
