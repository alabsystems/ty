---------------------------- MODULE BagsTest ----------------------------
(*
   TLC-parity test for Bags module operators.
   Adapted from ~/tlaplus/tlatools/org.lamport.tlatools/test-model/BagsTest.tla

   All tests are in ASSUME statements, evaluated at constant initialization.
   The spec generates 0 states since Next is never enabled (FALSE).
*)
...
\* A simple spec to make TY run model checking
\* The ASSUME statements above are checked during constant evaluation
VARIABLE dummy
Init == dummy = 0
Next == FALSE /\ dummy' = dummy
