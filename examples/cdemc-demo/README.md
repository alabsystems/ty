...
Build TY with the ay SMT solver enabled:
...
```bash
ty check TokenRing.tla --config TokenRing.cfg
```
...
```bash
ty check TokenRingBuggy.tla --config TokenRingBuggy.cfg
```
...
```bash
ty check TokenRing.tla --config TokenRing.cfg --bfs-only
```
...
2. **BMC lane** — ay bounded model checking, seeded by BFS wavefronts
3. **PDR lane** — ay IC3 property-directed reachability
4. **k-Induction lane** — ay bounded inductive safety proofs
