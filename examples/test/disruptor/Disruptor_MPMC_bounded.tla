...
TypeOk ==
  ...
  \* WP-33 capability demonstration: the checked capacity bound that lets the
  \* sequence-capacity proof reach `consumed`'s Sequence nested in a Function
  \* range. Absent from the shipped Disruptor_MPMC, which is why its `consumed`
  \* stays `bound=Observed` and never becomes flat-admissible.
