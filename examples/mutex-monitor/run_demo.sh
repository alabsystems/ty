...
#
# Usage: ./run_demo.sh
#        ./run_demo.sh --skip-ty   # Skip TY steps (just build + run generated code)
...
echo "========================================"
echo "  TY Codegen Demo: Peterson's Mutex"
echo "========================================"
echo

# -----------------------------------------------------------------------
# Step 1: Model check the TLA+ spec with ty
# -----------------------------------------------------------------------
if [[ "$SKIP_TY" == false ]]; then
    echo "--- Step 1: Model check the TLA+ spec ---"
    echo "\$ ty check PetersonMutex.tla --config PetersonMutex.cfg"
    echo
    if command -v ty &>/dev/null; then
        ty check "$SCRIPT_DIR/PetersonMutex.tla" --config "$SCRIPT_DIR/PetersonMutex.cfg"
    else
        (cd "$REPO_ROOT" && cargo run --release --bin ty -- check \
    ...
    echo "\$ ty codegen --tir --config PetersonMutex.cfg PetersonMutex.tla"
    echo
    if command -v ty &>/dev/null; then
        ty codegen --tir \
            --config "$SCRIPT_DIR/PetersonMutex.cfg" \
        ...
        (cd "$REPO_ROOT" && cargo run --release --bin ty -- codegen --tir \
            --config "$SCRIPT_DIR/PetersonMutex.cfg" \
            "$SCRIPT_DIR/PetersonMutex.tla")
    fi
    echo
...
echo "  4. BFS model check in compiled Rust matches TY (32 states)"
