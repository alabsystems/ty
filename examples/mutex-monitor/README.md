# Peterson's Mutex: TY Codegen Demo

**TLA+ specification -> verified Rust code -> runtime monitoring**

This example demonstrates TY's unique code generation pipeline: take a
...
```bash
# From the ty repo root:
./examples/mutex-monitor/run_demo.sh

# Or skip TY steps and just run the generated code:
./examples/mutex-monitor/run_demo.sh --skip-ty
```
...
```bash
cargo run --release --bin ty -- check \
...
```bash
cargo run --release --bin ty -- codegen --tir \
    --config examples/mutex-monitor/PetersonMutex.cfg \
    examples/mutex-monitor/PetersonMutex.tla
```
...
Same result as the TY model checker, but running as compiled Rust.
...
generated/
  ...
  src/
    ...
    peterson_mutex.rs -- Generated state machine (from ty codegen)
...
```
  PetersonMutex.tla          ty codegen         peterson_mutex.rs
...
```
TY Runtime Monitor Demo: Peterson's Mutex
============================================
