...
\* as implemented in ay-cp (~/ay/crates/ay-cp/src/propagators/alldifferent.rs):
...
\*
\* Reference: ay-cp alldifferent.rs filter_lower/filter_upper (bounds version)
\*            ay-cp alldifferent_ac/mod.rs filter_unsupported (AC version)
HallSetPropagate ==
    /\ status = "propagating"
    /\ \E S \in SUBSET Vars :
        /\ IsHallSet(S)
...
\*
\* This models the filter_lower/filter_upper operations in ay-cp: when the
\* minimum value in a domain is claimed by a Hall interval below, raise
\* the lower bound; symmetrically for upper bound.
\*
\* Reference: ay-cp alldifferent.rs filter_lower(), filter_upper()
BoundsPropagate ==
    /\ status = "propagating"
    /\ \E v \in Vars :
...
\*
\* Reference: ay-cp alldifferent_ac/mod.rs filter_unsupported()
ValueRemove ==
    /\ status = "propagating"
    /\ \E v \in Vars :
