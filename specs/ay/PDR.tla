...
\*
\* Models the PDR algorithm used in ay-chc for Constrained Horn Clause
...
\* This matches ay-chc where frame[0] represents the initial constraint
...
\*
\* Reference: ay-chc/src/pdr/ (PdrSolver, strengthen, check_fixed_point)
...
\* blocking constraints). Corresponds to ay-chc's push_frame().
Unfold ==
    /\ status = "running"
    /\ obligations = {}
    /\ Len(frames) < MaxDepth
...
\* and create an obligation to block it. Models ay-chc's strengthen()
...
\*
\* Corresponds to ay-chc's strengthen() obligation loop with
...
\* Models ay-chc's push_lemmas(): pushing blocking clauses forward when
...
\* that proves safety. Corresponds to ay-chc's check_fixed_point().
DetectFixpoint ==
    /\ status = "running"
    /\ \E i \in 1..(Len(frames) - 1) :
        /\ frames[i] = frames[i + 1]
